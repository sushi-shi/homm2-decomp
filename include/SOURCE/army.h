...
public:
    ...
    void PowEffect(H2_ENUM_PARAM(CombatEffectType, i32) effect, i32 resetLimits, i32 effectX, i32 effectY);
    u32l Strength(void);
    i32 LeaveNoBody(void);
    void ProcessDeath(i32 immediate);
    void SpellEffect(H2_ENUM_PARAM(CombatEffectType, i32) effect, i32 effectFrameDelay, i32 animateCreature);
    ...
    i32 RightX(void);
    i32 LeftX(void);
    i32 OtherArmyAdjacent(H2_ENUM_PARAM(CombatSide, i32) side, i32 index);
...
#define ARMY_HAS_BERSERK_OR_HYPNOTIZE(a)                                                           \
    ((a).m_spellInfluence[IDX(ARMY_SPELL_INFLUENCE_BERSERK)]                                       \
     || (a).m_spellInfluence[IDX(ARMY_SPELL_INFLUENCE_HYPNOTIZE)])
#define ARMY_HAS_INCAPACITATING_SPELL(a)                                                           \
    ((a).m_spellInfluence[IDX(ARMY_SPELL_INFLUENCE_BLIND)]                                         \
     || (a).m_spellInfluence[IDX(ARMY_SPELL_INFLUENCE_PARALYZE)]                                   \
     || (a).m_spellInfluence[IDX(ARMY_SPELL_INFLUENCE_PETRIFIED)])
