...
constexpr RecruitSiteType DecodeRecruitSiteType(u32 packedValue) {
    return RecruitSiteTypeFromCode(packedValue & EVENT_RECRUIT_TYPE_MASK);
}
...
} MapObjectEncodingConstant;

typedef enum MapEventRewardConstant {
...
} MapEventRewardConstant;

typedef enum MapEventStorageConstant {
    EVENT_TEXT_BUFFER_SIZE    = 500,
    SPHINX_ANSWER_BUFFER_SIZE = 52,
    SPHINX_INPUT_LENGTH       = 12,
} MapEventStorageConstant;

typedef enum MapEventTextId {
...
typedef enum MapEventSpatialConstant {
...
typedef enum ArtifactEventMode {
...
constexpr ArtifactEventMode ArtifactEventModeFromCode(i32 value) {
    return static_cast<ArtifactEventMode>(value); // H2_ENUM_CODE_BOUNDARY
}
