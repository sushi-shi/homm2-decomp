#ifndef HOMM2_X_GLOBAL_H
#define HOMM2_X_GLOBAL_H

#include <SOURCE/KB_TYPES.h>
#include <SOURCE/CONFIG_TYPES.h>
#include <SOURCE/REMOTE_TYPES.h>
#include <Ints.h>
#include <SOURCE/CURSOR.h>
#include <SOURCE/combatTypes.h>
#include <SOURCE/dimPalette.h>
#include <SOURCE/hero.h>
#include <SOURCE/Overview.h>
#include <EDITOR/mapcell.h>
#include <SOURCE/GAME.h>
#include <BASE/display.h>

class ExpCampaign;
typedef i32 ExpansionCampaignId;
typedef i32 CampaignSide;

typedef enum SecondarySkillValueTableConstant {
    SECONDARY_SKILL_VALUE_LEVEL_COUNT = (HERO_SKILL_LEVEL_COUNT) - 1
} SecondarySkillValueTableConstant;

typedef enum ElevationOverlayConstant {
    COMBAT_OBSTACLE_CELL_OFFSET_COUNT = 8
} ElevationOverlayConstant;

typedef enum TownEligibleBuildMaskConstant {
    TOWN_ELIGIBLE_BUILD_KNIGHT_MASK      = 0x3ff8bf9f,
    TOWN_ELIGIBLE_BUILD_BARBARIAN_MASK   = 0x1bf8bf9f,
    TOWN_ELIGIBLE_BUILD_SORCERESS_MASK   = 0x0ff8bf9f,
    TOWN_ELIGIBLE_BUILD_WARLOCK_MASK     = 0x69f8bf9f,
    TOWN_ELIGIBLE_BUILD_WIZARD_MASK      = 0x35f8bf9f,
    TOWN_ELIGIBLE_BUILD_NECROMANCER_MASK = 0x1ff8bf9b
} TownEligibleBuildMaskConstant;

enum {
    ADV_DISPOSE_NONE    = 0,
    ADV_DISPOSE_PARTIAL = 1,
    ADV_DISPOSE_FULL    = 2
};
typedef i32 AdventureDisposeLevel;
enum {
    CALENDAR_PERIOD_NONE     = -1,
    CALENDAR_PERIOD_NORMAL   = 0,
    CALENDAR_PERIOD_CREATURE = 1,
    CALENDAR_PERIOD_PLAGUE   = 2
};
typedef i32 CalendarPeriodType;
typedef enum DiffSendRecipient {
    DIFF_SEND_FORCE_WHOLE = -2,
    DIFF_SEND_NONE        = -1
} DiffSendRecipient;

#pragma pack(push, 1)
struct SCmbtObstacle {
    u32 terrainMask;
    u8 minimumColumn;
    u8 cellCount;
    i8 cellOffsets[COMBAT_OBSTACLE_CELL_OFFSET_COUNT];
};

struct SElevationOverlay {
    u16 terrainMask;
    i8 cellOffsets[COMBAT_ELEVATION_OVERLAY_CELL_COUNT];
};
#pragma pack(pop)

typedef enum GlobalConstant {
    EXPANSION_HIGH_SCORE_CAMPAIGN_COUNT     = 4,
    X_GLOBAL_RECRUIT_EMPTY_COUNT            = 5,
    X_GLOBAL_RECRUIT_BUY_COUNT              = 5,
    X_GLOBAL_PASSWORD_STRING_COUNT          = 211,
    X_GLOBAL_STABLE_TEXT_COUNT              = 4,
    X_GLOBAL_SETUP_HELP_COUNT               = 3,
    X_GLOBAL_EXPANSION_CAMPAIGN_COUNT       = 4,
    X_GLOBAL_EXPANSION_CAMPAIGN_MAP_COUNT   = 8,
    X_GLOBAL_EXPANSION_CAMPAIGN_AWARD_COUNT = 11,
    X_GLOBAL_EXPANSION_HERO_NAME_COUNT      = 6,
    X_GLOBAL_SHORT_SKILL_LEVEL_COUNT        = 3,
    X_GLOBAL_NEW_HERO_ALIGNMENT_COUNT       = 12,
    X_GLOBAL_PASSWORD_STRING_INDEX_COUNT    = 8,
    GLOBAL_MAP_NAME_SIZE                    = 0x14,
    GLOBAL_TCP_TEXT_SIZE                    = 0x18,
    GLOBAL_AGGREGATE_PATH_SIZE              = 0x160,
    GLOBAL_LAST_FILENAME_SIZE               = 0x15f,
    GLOBAL_COMMAND_LINE_SIZE                = 0x40,
    GLOBAL_GAME_PATH_SIZE                   = 0x14,
    GLOBAL_MAP_PATH_SIZE                    = 0x14,
    GLOBAL_DRIVER_NAME_SIZE                 = 0x10,
    GLOBAL_BOTTOM_VIEW_TEXT_SIZE            = 0x98,
    GLOBAL_BOTTOM_VIEW_MESSAGE_SIZE         = 92,
    GLOBAL_SHORT_MAP_NAME_SIZE              = 16,
    GLOBAL_PLAYER_NAME_SIZE                 = 21,
    GLOBAL_WINDOW_TEXT_SIZE                 = 0x12c,
    GLOBAL_TEXT_BUFFER_SIZE                 = 768
} GlobalConstant;

typedef enum GameBalanceConstant {
    HERO_RECRUITMENT_GOLD_COST      = 2500,
    ULTIMATE_ARTIFACT_AVERAGE_VALUE = 16500,
    TOWN_VISIBILITY_RADIUS          = 5
} GameBalanceConstant;

enum {
    SPELL_EFFECT_DISPLAY_HIGHLIGHT     = 0,
    SPELL_EFFECT_DISPLAY_SPELL_ICON    = 1,
    SPELL_EFFECT_DISPLAY_EFFECT_STATUS = 2,
    SPELL_EFFECT_DISPLAY_COUNT         = 3
};
typedef i32 SpellEffectDisplayType;

inline SpellEffectDisplayType NextSpellEffectDisplayType(SpellEffectDisplayType type) {
    return (type + 1) % (SPELL_EFFECT_DISPLAY_COUNT);
}

extern b8 xIsExpansionMap;
extern ExpansionCampaignId xLastChoice;
extern u8 xIsPlayingExpansionCampaign;
extern class ExpCampaign xCampaign;
extern const char* xSetupCampaignGameHelp[X_GLOBAL_SETUP_HELP_COUNT];
extern const char* xSetupStandardGameHelp[X_GLOBAL_SETUP_HELP_COUNT];
extern const char* xCampaignAwards[X_GLOBAL_EXPANSION_CAMPAIGN_AWARD_COUNT];
extern const char* xScenarioName[X_GLOBAL_EXPANSION_CAMPAIGN_COUNT]
                          [X_GLOBAL_EXPANSION_CAMPAIGN_MAP_COUNT];
extern const char* xScenarioDescription[X_GLOBAL_EXPANSION_CAMPAIGN_COUNT]
                                 [X_GLOBAL_EXPANSION_CAMPAIGN_MAP_COUNT];
extern const char* xShortCampaignNames[X_GLOBAL_EXPANSION_CAMPAIGN_COUNT];
extern const char* xHSCampaignNames[EXPANSION_HIGH_SCORE_CAMPAIGN_COUNT];
extern i32 xTheSpell;
extern const char* xNecromancerShrine;
extern const char* xNecromancerShrineDesc;
extern const char* xStableText[X_GLOBAL_STABLE_TEXT_COUNT];
extern const char* xJosephName[X_GLOBAL_EXPANSION_HERO_NAME_COUNT];
extern const char* xUncleIvanName[X_GLOBAL_EXPANSION_HERO_NAME_COUNT];
extern const char* xShortSSLevelNames[X_GLOBAL_SHORT_SKILL_LEVEL_COUNT];
extern const char* xPasswordStrings[X_GLOBAL_PASSWORD_STRING_COUNT];
extern u8 xNewHeroAlignment[X_GLOBAL_NEW_HERO_ALIGNMENT_COUNT];
extern i32 xPasswordStringsIndex[X_GLOBAL_PASSWORD_STRING_INDEX_COUNT];
extern i32 xShrineBuildingCost[(RES_COUNT)];

typedef enum RadarColorTableCount {
    RADAR_MAP_COLOR_COUNT    = 12,
    RADAR_OBJECT_COLOR_COUNT = 16,
    RADAR_OWNER_COLOR_COUNT  = 8
} RadarColorTableCount;

typedef enum GroundTableConstant {
    GROUND_TILE_IMAGE_COUNT = 432,
    GROUND_SHAPE_FLIPPED    = 0x80
} GroundTableConstant;

typedef enum KbControlTableConstant {
    MOVEMENT_FRAME_FLIP_COUNT          = 16,
    CASTLE_RESOURCE_SLOT_COUNT         = 4,
    CASTLE_AMOUNT_COUNT                = 4,
    VESA_MODE_VALUE_COUNT              = 6,
    VESA_SET_MODE_FUNCTION             = 0x4f02,
    VESA_MODE_640_480_256              = 0x0101
} KbControlTableConstant;

typedef enum KbPaletteTableConstant {
    PUZZLE_DRAW_TABLE_COUNT   = 64,
    PUZZLE_PIECE_COUNT        = 48,
    PUZZLE_PIECE_STORAGE_SIZE = (PUZZLE_PIECE_COUNT + 7) / 8
} KbPaletteTableConstant;

typedef enum KbGameTableConstant {
    KB_MAP_SIZE_COUNT                   = 4,
    KB_INTERFACE_TYPE_COUNT             = 37,
    KB_INTERFACE_VARIANT_COUNT          = 2,
    KB_COMBAT_SPEED_COUNT               = 3,
    KB_TOWN_COMMAND_COUNT               = 28,
    KB_ARMY_EFFECT_COUNT                = 20,
    KB_MUSIC_TRACK_COUNT                = 0x3c,
    KB_ARTIFACT_LEVEL_COUNT             = (ARTIFACT_COUNT) + 1,
    KB_STAT_POWER_COUNT                 = 41,
    BATTLE_STAT_TABLE_MAX_INDEX         = KB_STAT_POWER_COUNT - 1,
    KB_SPELL_LIMIT_COUNT                = 5,
    KB_SPELL_MOD_COUNT                  = 11,
    KB_QUICK_COMBAT_SPELL_TYPE_COUNT    = 7,
    KB_CLOUD_MASK_COUNT                 = 256,
    KB_HERO_LEVEL_BAND_COUNT            = 2,
    KB_SPELL_INFLUENCE_MAP_COUNT        = 16,
    KB_COMBAT_BACKGROUND_COUNT          = 19,
    KB_COMBAT_OBSTACLE_COUNT            = 32,
    KB_TERRAIN_TYPE_COUNT               = (TERRAIN_COUNT) + 1,
    KB_TERRAIN_STEP_TYPE_COUNT          = 2,
    KB_TRIGGER_TYPE_COUNT               = 124,
    KB_CASTLE_WALL_SEGMENT_COUNT        = 4,
    KB_CASTLE_TOWER_COUNT               = 4,
    KB_CASTLE_DOOR_POSITION_COUNT       = 2,
    KB_TRADING_POST_EFFICIENCY_COUNT    = 11,
    KB_MOAT_CELL_COUNT                  = 9,
    KB_ALIGNMENT_NAME_COUNT             = (FACTION_COUNT) + 2,
    KB_QUICK_VIEW_TEXT_COUNT            = 124,
    KB_EVENT_TEXT_TABLE_COUNT           = 95,
    KB_CONTROL_PANEL_HELP_COUNT         = 5,
    KB_COMBAT_SPELL_PANEL_HELP_COUNT    = 7,
    KB_ADVENTURE_PANEL_HELP_COUNT       = 5,
    KB_INIT_MENU_HELP_COUNT             = 5,
    KB_ADVENTURE_MENU_HELP_COUNT        = 8,
    KB_LUCK_TEXT_COUNT                  = 7,
    KB_MORALE_TEXT_COUNT                = 7,
    KB_ON_OFF_TEXT_COUNT                = 11,
    KB_WALK_SPEED_TEXT_COUNT            = 5,
    KB_MONTH_NAME_COUNT                 = 10,
    KB_WEEK_NAME_COUNT                  = 15,
    KB_HERO_SCREEN_TEXT_COUNT           = 25,
    KB_CASTLE_INFO_TEXT_COUNT           = 16,
    KB_LUCK_INFO_TEXT_COUNT             = 21,
    KB_IQ_NAME_COUNT                    = 5,
    KB_SPELL_HELP_TEXT_COUNT            = 9,
    KB_SPEED_TEXT_COUNT                 = 10,
    KB_ARMY_DETAIL_TEXT_COUNT           = 9,
    KB_WELL_DETAIL_TEXT_COUNT           = 9,
    KB_KINGDOM_OVERVIEW_TEXT_COUNT      = 3,
    KB_NEW_TURN_TEXT_COUNT              = 7,
    KB_VIEW_GENERAL_LABEL_COUNT         = 7,
    KB_VIEW_GENERAL_HELP_COUNT          = 7,
    KB_VIEW_GENERAL_LONG_HELP_COUNT     = 4,
    KB_COMBAT_MESSAGE_COUNT             = 12,
    KB_HERO_LEVEL_TEXT_COUNT            = 3,
    KB_COMBAT_HELP_COUNT                = 5,
    KB_LONG_COMBAT_HELP_COUNT           = 5,
    KB_NEW_GAME_HELP_COUNT              = 8,
    KB_SETUP_BAUD_HELP_COUNT            = 5,
    KB_SETUP_COM_PORT_HELP_COUNT        = 5,
    KB_SETUP_DC_BAUD_HELP_COUNT         = 5,
    KB_SETUP_DC_COM_PORT_HELP_COUNT     = 5,
    KB_SETUP_HOT_SEAT_HELP_COUNT        = 6,
    KB_SETUP_MODEM_HELP_COUNT           = 4,
    KB_SETUP_DIRECT_CONNECT_HELP_COUNT  = 4,
    KB_SETUP_MULTIPLAYER_HELP_COUNT     = 5,
    KB_SETUP_NETWORK_HELP_COUNT         = 3,
    KB_SETUP_NETWORK_SECOND_HELP_COUNT  = 4,
    KB_SETUP_GAME_HELP_COUNT            = 4,
    KB_BATTLE_RESULT_TEXT_COUNT         = 11,
    KB_MORALE_INFO_TEXT_COUNT           = 32,
    KB_MAP_SIZE_TEXT_COUNT              = 4,
    KB_START_DIFFICULTY_TEXT_COUNT      = 4,
    KB_CAMPAIGN_LEADER_TEXT_COUNT       = 4,
    KB_WIN_TEXT_COUNT                   = 5,
    KB_MUSIC_QUALITY_TEXT_COUNT         = 3,
    KB_SECONDARY_SKILL_LEVEL_TEXT_COUNT = (HERO_SKILL_LEVEL_COUNT) - 1,
    KB_NEUTRAL_BUILDING_TEXT_COUNT      = 19,
    KB_WELL_EXTRA_NAME_COUNT            = 7,
    KB_SPECIAL_BUILDING_NAME_COUNT      = 7,
    KB_NEUTRAL_BUILDING_INFO_COUNT      = 19,
    KB_SPECIAL_BUILDING_INFO_COUNT      = (FACTION_COUNT),
    KB_DIRECTION_TEXT_COUNT             = 9,
    KB_RUMOUR_TERRAIN_DESCRIPTION_COUNT = (TERRAIN_COUNT),
    KB_INTERFACE_TYPE_TEXT_COUNT        = 3,
    KB_BW_MOUSE_TEXT_COUNT              = 2,
    KB_COMBAT_MINI_INFO_TEXT_COUNT      = 3,
    KB_COMMAND_LINE_HELP_COUNT          = 14,
    KB_OVERVIEW_TEXT_COUNT              = 6,
    KB_WIN_COM_ERROR_TEXT_COUNT         = 6,
    KB_MINI_VIEW_TEXT_COUNT             = 9,
    KB_FILE_REQUEST_HELP_COUNT          = 15,
    KB_PERSONALITY_TEXT_COUNT           = 4,
    KB_ARMY_SIZE_NAME_COUNT             = 9,
    KB_ARMY_SIZE_NAME_VARIANT_COUNT     = 3,
    KB_RANDOM_TAVERN_TEXT_COUNT         = 8,
    KB_RANDOM_SIGN_TEXT_COUNT           = 4,
    KB_CAMPAIGN_AWARD_TEXT_COUNT        = 12,
    KB_SLOW_VIDEO_LEVEL_TEXT_COUNT      = 2,
    KB_SETTINGS_PANEL_HELP_COUNT        = 10,
    KB_BARRIER_COLOR_NAME_COUNT         = 8,
    KB_GENERIC_SITE_NAME_COUNT          = 7,
    KB_RECRUITMENT_SITE_NAME_COUNT      = 5,
    KB_WIN_SETUP_COUNT                  = 73,
    KB_INIT_MENU_HOTSPOT_COUNT          = 5
} KbGameTableConstant;

enum {
    INIT_MENU_HOTSPOT_X           = 0,
    INIT_MENU_HOTSPOT_Y           = 1,
    INIT_MENU_HOTSPOT_WIDTH       = 2,
    INIT_MENU_HOTSPOT_HEIGHT      = 3,
    INIT_MENU_HOTSPOT_FIELD_COUNT = 4
};
typedef i32 InitMenuHotSpotField;
extern u8 giGroundShape[GROUND_TILE_IMAGE_COUNT];
extern u8 gColorTableTan[PALETTE_COLOR_COUNT];
extern u8 gColorTableGray[PALETTE_COLOR_COUNT];
extern u8 gColorTableRed[PALETTE_COLOR_COUNT];
extern u8 gColorTableDarkBrown[PALETTE_COLOR_COUNT];
extern i32 giMainVideoModeWidth;
extern i32 giMainVideoModeHeight;
extern u8 gMapColors[RADAR_MAP_COLOR_COUNT];
extern u8 gObjectColors[RADAR_OBJECT_COLOR_COUNT];
extern u8 gOwnerColors[RADAR_OWNER_COLOR_COUNT];
extern const char* gTilesetFiles[(TILESET_COUNT)];
extern u8 bPuzzleDraw[PUZZLE_DRAW_TABLE_COUNT];
extern u8 gColorTableLighten[PALETTE_COLOR_COUNT];
extern u8 gColorTableNoCycle[PALETTE_COLOR_COUNT];
extern b32 gbReturnAfterComputeExtent;
extern b32 gbAllowTextEntryEscape;
extern b32 giScreenScroll;
extern i32 giMenuCommand;
extern b32 gbSendMouseMoveMessages;
extern b32 gbColorMice;
extern u32l gTownEligibleBuildMask[(FACTION_COUNT)];
extern u8 giMapSizes[KB_MAP_SIZE_COUNT];
extern b32 gbUseEvilInterface;
extern const char* cEvilTranslate[KB_INTERFACE_TYPE_COUNT][KB_INTERFACE_VARIANT_COUNT];
extern char gcAnimPath[GLOBAL_AGGREGATE_PATH_SIZE];
extern char gcGamePath[GLOBAL_GAME_PATH_SIZE];
extern char gcMapPath[GLOBAL_MAP_PATH_SIZE];
extern char gcMusicPath[GLOBAL_AGGREGATE_PATH_SIZE];
extern float gfCombatSpeedMod[KB_COMBAT_SPEED_COUNT];
extern i32 giDialogTimeout;
extern i32 giNewMonsterCycleFrame;
extern b32 gbLeaveNetBoxAlone;
extern b32 gbUseWaveout;
extern b32 gbShowAllMaps;
extern const char* gCombatFxNames[(COMBAT_EFFECT_COUNT)];
extern i16 horseFrameFlip[MOVEMENT_FRAME_FLIP_COUNT];
extern i16 boatFrameFlip[MOVEMENT_FRAME_FLIP_COUNT];
extern i8 gCastleResources[CASTLE_RESOURCE_SLOT_COUNT];
extern i16 gCastleAmounts[CASTLE_AMOUNT_COUNT];
extern i16 gVesaMode[VESA_MODE_VALUE_COUNT];
extern i32 gInitResourcesHuman[(DIFFICULTY_COUNT)][(RES_COUNT)];
extern i32 gInitResourcesComputer[(DIFFICULTY_COUNT)][(RES_COUNT)];
extern i32 gMineCharacteristics[(RES_COUNT)];
extern i32 gSSValues[(HERO_SKILL_COUNT)][SECONDARY_SKILL_VALUE_LEVEL_COUNT];
extern u8 gArtifactLevel[KB_ARTIFACT_LEVEL_COUNT];
extern i32 gUltArtifactAvgValue;
extern i8 giVisRangeTown;
extern float gfStatPower[KB_STAT_POWER_COUNT];
extern float gfBattleStat[KB_STAT_POWER_COUNT];
extern i8 gSpellLimits[KB_SPELL_LIMIT_COUNT];
extern float gfSpellCastableCombatMod[KB_SPELL_MOD_COUNT];
extern float gfSpellCastNumMod[KB_SPELL_MOD_COUNT];
extern float gfPhilAISpellPowerMod[KB_SPELL_MOD_COUNT];
extern float gfPhilAIDurationMod[KB_SPELL_MOD_COUNT];
extern float gfSpellTypeNumMod[KB_QUICK_COMBAT_SPELL_TYPE_COUNT];
extern i8 gbArrow[(MAP_DIRECTION_COUNT)][(MAP_DIRECTION_COUNT)];
extern u8 giCloudType[KB_CLOUD_MASK_COUNT];
enum {
    TOWN_MUSIC_NONE        = 0,
    TOWN_MUSIC_WARLOCK     = 5,
    TOWN_MUSIC_WIZARD      = 6,
    TOWN_MUSIC_NECROMANCER = 7,
    TOWN_MUSIC_KNIGHT      = 8,
    TOWN_MUSIC_BARBARIAN   = 9,
    TOWN_MUSIC_SORCERESS   = 10,
    TOWN_MUSIC_TABLE_SIZE  = 8
};
typedef i32 TownMusicTrack;
extern i8 townTheme[(TOWN_MUSIC_TABLE_SIZE)];
extern i8 gHeroSkillBonus[(FACTION_COUNT)][KB_HERO_LEVEL_BAND_COUNT][HERO_PRIMARY_STAT_COUNT];
extern i32 giMonoIconSkip;
extern i32 giScrollX;
extern i32 giScrollY;
extern b32 gbNoBorder;
extern b32 gbEnlargeScreenBlit;
extern ConfigExecutable giCurExe;
extern b32 gbInDialog;
extern b32 gbInSetupDialog;
extern b32 gbMinimized;
extern b32 gbInSmackMgr;
extern i32 glBottomRefresh;
extern b32 gbBothMachinesWin95;
extern b32 gbGotFirstHeartbeat;
extern b32 gbProcessingCombatAction;
extern RemoteNetworkProtocol iMPNetProtocol;
extern i32 iLastDiffSendTo;
extern u8 gcSpellInfluenceIcons[KB_SPELL_INFLUENCE_MAP_COUNT];
extern u8 giSpellInfluenceToSpell[KB_SPELL_INFLUENCE_MAP_COUNT];
extern u8 giNumPowFrames[(COMBAT_EFFECT_COUNT)];
extern SpellEffectDisplayType giSpellEffectShowType;
extern i8 gcColorToPlayerPos[RADAR_OWNER_COLOR_COUNT];
extern const char* cCombatBkgNames[KB_COMBAT_BACKGROUND_COUNT];
extern struct SCmbtObstacle sCmbtObstacles[KB_COMBAT_OBSTACLE_COUNT];
extern i32 gEstatesGoldLevel[(HERO_SKILL_LEVEL_COUNT)];
extern float gfSSLogisticsMod[(HERO_SKILL_LEVEL_COUNT)];
extern float gfSSNavigationMod[(HERO_SKILL_LEVEL_COUNT)];
extern float gfSSArcheryMod[(HERO_SKILL_LEVEL_COUNT)];
extern float gfSSAIArcheryMod[(HERO_SKILL_LEVEL_COUNT)];
extern i8 giVisRange[(HERO_SKILL_LEVEL_COUNT)];
extern u8 gStartingHeroStats[(FACTION_COUNT)][HERO_STARTING_STAT_COUNT];
extern i32 giTerrainCost[KB_TERRAIN_TYPE_COUNT][(HERO_SKILL_LEVEL_COUNT)]
                        [KB_TERRAIN_STEP_TYPE_COUNT];
extern const char* gTownPrefixNames[(FACTION_COUNT)];
extern const char* gTownObjNames[(BUILDING_SLOT_COUNT)];
extern i32 giDebugBuildingToBuild;
extern u8 giTerrainToMusicTrack[(TERRAIN_COUNT)];
extern const char* cHeroTypeShortName[(FACTION_COUNT)];
extern char cHeroTypeInitial[(FACTION_COUNT)];
extern i32 giDeferObjDrawX;
extern i32 giDeferObjDrawY;
extern class heroWindow* gpInitWin;
extern u8 iWallToHexCell[KB_CASTLE_WALL_SEGMENT_COUNT];
extern u8 iTowerToHexCell[KB_CASTLE_TOWER_COUNT];
extern u16 wallPos[KB_CASTLE_WALL_SEGMENT_COUNT][(COORDINATE_AXIS_COUNT)];
extern u16 towerPos[KB_CASTLE_TOWER_COUNT][(COORDINATE_AXIS_COUNT)];
extern u16 doorPos[KB_CASTLE_DOOR_POSITION_COUNT][(COORDINATE_AXIS_COUNT)];
extern float fTradingPostEfficency[KB_TRADING_POST_EFFICIENCY_COUNT];
extern struct SElevationOverlay sElevationOverlay[COMBAT_ELEVATION_OVERLAY_COUNT];
extern b32 gbDrawingPuzzle;
extern i32 giWalkingFrom;
extern i32 giWalkingFrom2;
extern i32 giWalkingTo;
extern i32 giWalkingTo2;
extern i32 giWalkingYMod;
extern u8 moatCell[KB_MOAT_CELL_COUNT];
extern char* congratsText;
extern const char* gArtifactNames[(ARTIFACT_COUNT)];
extern const char* gArtifactDesc[(ARTIFACT_COUNT)];
extern const char* gArtifactEvent[(ARTIFACT_COUNT)];
extern const char* gStatNames[HERO_PRIMARY_STAT_COUNT];
extern const char* gStatDesc[HERO_PRIMARY_STAT_COUNT];
extern const char* gAlignmentNames[KB_ALIGNMENT_NAME_COUNT];
extern const char* gTerrainNames[(TERRAIN_COUNT)];
extern const char* gResourceNames[(RES_COUNT)];
extern const char* gMineNames[(RES_COUNT)];
extern const char* gQuickViewText[KB_QUICK_VIEW_TEXT_COUNT];
extern const char* gEventText[KB_EVENT_TEXT_TABLE_COUNT];
extern const char* gCPanelHelp[KB_CONTROL_PANEL_HELP_COUNT];
extern const char* gCSPanelHelp[KB_COMBAT_SPELL_PANEL_HELP_COUNT];
extern const char* gAPanelHelp[KB_ADVENTURE_PANEL_HELP_COUNT];
extern const char* gInitMenuHelp[KB_INIT_MENU_HELP_COUNT];
extern const char* gAdvMenuHelp[KB_ADVENTURE_MENU_HELP_COUNT];
extern const char* gLuckText[KB_LUCK_TEXT_COUNT];
extern const char* gMoraleText[KB_MORALE_TEXT_COUNT];
extern const char* onOffText[KB_ON_OFF_TEXT_COUNT];
extern const char* walkSpeedText[KB_WALK_SPEED_TEXT_COUNT];
extern const char* gColors[(FACTION_COUNT)];
extern const char* gMonthNames[KB_MONTH_NAME_COUNT];
extern const char* gWeekNames[KB_WEEK_NAME_COUNT];
extern const char* cHeroScreen[KB_HERO_SCREEN_TEXT_COUNT];
extern const char* cCastleInfo[KB_CASTLE_INFO_TEXT_COUNT];
extern const char* cLuckInfo[KB_LUCK_INFO_TEXT_COUNT];
extern const char* IQnames[KB_IQ_NAME_COUNT];
extern const char* cSpellHelp[KB_SPELL_HELP_TEXT_COUNT];
extern const char* speedText[KB_SPEED_TEXT_COUNT];
extern const char* cArmyDetail[KB_ARMY_DETAIL_TEXT_COUNT];
extern const char* cWellDetail[KB_WELL_DETAIL_TEXT_COUNT];
extern const char* cKingdomOverview[KB_KINGDOM_OVERVIEW_TEXT_COUNT];
extern const char* cNewTurn[KB_NEW_TURN_TEXT_COUNT];
extern const char* cViewGeneralLabels[KB_VIEW_GENERAL_LABEL_COUNT];
extern const char* cViewGeneralHelp[KB_VIEW_GENERAL_HELP_COUNT];
extern const char* cViewGeneralLongHelp[KB_VIEW_GENERAL_LONG_HELP_COUNT];
extern const char* cCombatMessage[KB_COMBAT_MESSAGE_COUNT];
extern const char* cHeroLevel[KB_HERO_LEVEL_TEXT_COUNT];
extern const char* cCombatHelp[KB_COMBAT_HELP_COUNT];
extern const char* cLongCombatHelp[KB_LONG_COMBAT_HELP_COUNT];
extern const char* cTownCommand[KB_TOWN_COMMAND_COUNT];
extern const char* gHeroDefaultNames[GAME_HERO_COUNT];
extern const char* gNewGameHelp[KB_NEW_GAME_HELP_COUNT];
extern const char* gSetupBaudHelp[KB_SETUP_BAUD_HELP_COUNT];
extern const char* gSetupComPortHelp[KB_SETUP_COM_PORT_HELP_COUNT];
extern const char* gSetupDCBaudHelp[KB_SETUP_DC_BAUD_HELP_COUNT];
extern const char* gSetupDCComPortHelp[KB_SETUP_DC_COM_PORT_HELP_COUNT];
extern const char* gSetupHotSeatGameHelp[KB_SETUP_HOT_SEAT_HELP_COUNT];
extern const char* gSetupModemGameHelp[KB_SETUP_MODEM_HELP_COUNT];
extern const char* gSetupDCGameHelp[KB_SETUP_DIRECT_CONNECT_HELP_COUNT];
extern const char* gSetupMultiPlayerGameHelp[KB_SETUP_MULTIPLAYER_HELP_COUNT];
extern const char* gSetupNetworkGameHelp[KB_SETUP_NETWORK_HELP_COUNT];
extern const char* gSetupNetworkGame2Help[KB_SETUP_NETWORK_SECOND_HELP_COUNT];
extern const char* gSetupGameHelp[KB_SETUP_GAME_HELP_COUNT];
extern const char* cBattleResults[KB_BATTLE_RESULT_TEXT_COUNT];
extern const char* cMoraleInfo[KB_MORALE_INFO_TEXT_COUNT];
extern const char* cMapSize[KB_MAP_SIZE_TEXT_COUNT];
extern const char* cDifficulty[(DIFFICULTY_COUNT)];
extern const char* cStartDifficulty[KB_START_DIFFICULTY_TEXT_COUNT];
extern const char* cCampaignLeaders[KB_CAMPAIGN_LEADER_TEXT_COUNT];
extern const char* cWinText[KB_WIN_TEXT_COUNT];
extern const char* cHumanDifficulty[(DIFFICULTY_COUNT)];
extern const char* cHumanInfoDifficulty[(DIFFICULTY_COUNT)];
extern const char* musicQualityText[KB_MUSIC_QUALITY_TEXT_COUNT];
extern const char* gSpellDesc[(SPELL_COUNT)];
extern const char* gSpellNames[(SPELL_COUNT)];
extern const char* gSecondarySkillLevels[KB_SECONDARY_SKILL_LEVEL_TEXT_COUNT];
extern const char* gSecondarySkills[(HERO_SKILL_COUNT)];
extern const char* cSecSkillDesc[(HERO_SKILL_COUNT)][SECONDARY_SKILL_VALUE_LEVEL_COUNT];
extern const char* cDirections[KB_DIRECTION_TEXT_COUNT];
extern const char* cRumourTerrainDescriptions[KB_RUMOUR_TERRAIN_DESCRIPTION_COUNT];
extern const char* gInterfaceTypeText[KB_INTERFACE_TYPE_TEXT_COUNT];
extern const char* cBWMouseText[KB_BW_MOUSE_TEXT_COUNT];
extern const char* combatSpeedText[KB_COMBAT_SPEED_COUNT];
extern const char* combatMiniInfoText[KB_COMBAT_MINI_INFO_TEXT_COUNT];
extern const char* gcCommandLineHelp[KB_COMMAND_LINE_HELP_COUNT];
extern const char* cOverviewText[KB_OVERVIEW_TEXT_COUNT];
extern const char* cWinComError[KB_WIN_COM_ERROR_TEXT_COUNT];
extern const char* cMiniViewText[KB_MINI_VIEW_TEXT_COUNT];
extern const char* gFileRequestHelp[KB_FILE_REQUEST_HELP_COUNT];
extern const char* cPersonality[KB_PERSONALITY_TEXT_COUNT];
extern const char* gArmySizeNames[KB_ARMY_SIZE_NAME_COUNT][KB_ARMY_SIZE_NAME_VARIANT_COUNT];
extern const char* cRandomTavernText[KB_RANDOM_TAVERN_TEXT_COUNT];
extern const char* cRandomSignText[KB_RANDOM_SIGN_TEXT_COUNT];
extern const char* cCampaignAwards[KB_CAMPAIGN_AWARD_TEXT_COUNT];
extern const char* cSlowVideoLevelText[KB_SLOW_VIDEO_LEVEL_TEXT_COUNT];
extern const char* gSPanelHelp[KB_SETTINGS_PANEL_HELP_COUNT];
extern const char* xBarrierColor[KB_BARRIER_COLOR_NAME_COUNT];
extern const char* xGenericSiteNames[KB_GENERIC_SITE_NAME_COUNT];
extern const char* xRecruitEmpty[X_GLOBAL_RECRUIT_EMPTY_COUNT];
extern const char* xRecruitBuy[X_GLOBAL_RECRUIT_BUY_COUNT];
extern const char* xRecruitmentSiteNames[KB_RECRUITMENT_SITE_NAME_COUNT];
extern b32 gbHeroWindShowing;
extern b32 gbFullCombatScreenDrawn;
extern b32 gbLimitedCombatUpdatePalette;
extern b32 gbFirstTimeThrough;
extern b32 gbSkipIntro;
extern b32 gbDoMemCheck;
extern b32 gbInCombat;
extern b32 gbDirectConnect;
extern i32 giForceSwitchMusic;
extern b32 gbComputeExtent;
extern b32 gbSaveBiggestExtent;
extern b32 gbLimitToExtent;
extern b32 gbCurrArmyDrawn;
extern AdventureDisposeLevel gAdvDisposeLevel;
extern i32 giHighScoreRank;
extern HighScoreType giHighScoreType;
extern b32 gbShowHighScore;
extern b32 gbLowMemory;
extern void* gLowPage;
extern b32 bKBDone;
extern struct _REDBOOK* hRedbookz;
extern b32 bForceCheckTimeEvent;
extern u16 IMHotSpots[KB_INIT_MENU_HOTSPOT_COUNT][(INIT_MENU_HOTSPOT_FIELD_COUNT)];
extern i32 lastIMHoverID;
extern b32 bInCheckEndGame;
extern b32 gbHitEvent;
extern i32 giMaxExtentX;
extern i32 giMaxExtentY;
extern i32 giRandomClouds;
extern MultiplayerBaseType iMPBaseType;
extern char cOverrideDigitalDriver[GLOBAL_DRIVER_NAME_SIZE];
extern b8 gArmyEffected[COMBAT_SIDE_COUNT][KB_ARMY_EFFECT_COUNT];
extern CalendarPeriodType giWeekType;
extern b32 gbInCampaign;
extern b32 gbIAmGreatest;
extern i32 giResExtra1;
extern i8 puzzlePiecesRemoved[PUZZLE_PIECE_STORAGE_SIZE];
extern b32 giSeedingValid;
extern i32 giLimitPlayer;
extern i32 giResExtra2;
extern i32 giShowClouds;
extern char gcBotViewText[GLOBAL_BOTTOM_VIEW_TEXT_SIZE];
extern class palette* gpBufferPalette;
extern i32 giResType1;
extern char gMapName[GLOBAL_SHORT_MAP_NAME_SIZE];
extern b32 gbBlackoutPlayer;
extern i32 giResType2;
extern i32 gIndex;
extern i32 giWeekTypeExtra;
extern i32 giTCPType;
extern i32 gCurLoadedSpellEffect;
extern i32 giCurTempMobility;
extern OverviewReturnAction giOverviewReturnAction;
extern char cOverrideMIDIDriver[GLOBAL_DRIVER_NAME_SIZE];
extern i32 giMinExtentX;
extern i32 giMinExtentY;
extern b32 gbTCPFirstTime;
extern i32 giHeroScreenSrcIndex;
extern i32 gbCampaignSideChoice;
extern i32 giMonthTypeExtra;
extern i32 iMPExtendedType;
extern i8 gcColorToSetupPos[RADAR_OWNER_COLOR_COUNT];
extern char gFullMapName[GLOBAL_MAP_NAME_SIZE];
extern char gcTCPName[GLOBAL_TCP_TEXT_SIZE];
extern i32 giShowIntro;
extern i32 giScore;
extern char gcRegAppPath[GLOBAL_AGGREGATE_PATH_SIZE];
extern u32l gTimeMark;
extern char* EXPANSION_AGGREGATE_NAME;
extern b8 xNetHasOldPlayers;
extern SMapChange sMapChangeQueue[CURSOR_MAP_CHANGE_QUEUE_COUNT];
extern char cPlayerNames[GAME_PLAYER_COUNT][GLOBAL_PLAYER_NAME_SIZE];
extern class icon* gCurLoadedSpellIcon;
extern u8 bSaveMusicPosition[KB_MUSIC_TRACK_COUNT];
extern char gcTCPAddress[GLOBAL_TCP_TEXT_SIZE];
extern char gLastFilename[GLOBAL_LAST_FILENAME_SIZE];
extern b32 giFullySeeded;
extern i32 iCombatControlNetPos[COMBAT_SIDE_COUNT];
extern char cExpAggPathName[GLOBAL_AGGREGATE_PATH_SIZE];
extern b32 gbMoveShown;
extern char gcWinText[GLOBAL_WINDOW_TEXT_SIZE];
extern i8 gbSetupGamePosToRealGamePos[RADAR_OWNER_COLOR_COUNT];
extern char gcRegCDRomPath[GLOBAL_AGGREGATE_PATH_SIZE];
extern class heroWindow* heroWin;
extern i32 giOverviewReturnActionExtra;
extern i32 giCurGeneral;
extern b32 gbIconClipOn;
extern b32 gbRemoteGameOpen;
extern i32 giTCPNumPlayers;
extern void* hMainWindow;
extern i32 giCurWindowsStyleFlags;
extern CalendarPeriodType giMonthType;
extern char* DEFAULT_AGGREGATE_NAME;
extern i32 gCurSpellEffectFrame;
extern char cAggPathName[GLOBAL_AGGREGATE_PATH_SIZE];
extern class highScoreManager* gpHighScoreManager;
extern i32 gMapX;
extern i32 gMapY;
extern b32 bFreshSave;
extern i32 gLowPageScreenSelector;
extern i32 giCurWatchPlayer;
extern char gcCommandLine[GLOBAL_COMMAND_LINE_SIZE];
extern i32 giMapChangeCtr;
extern b32 gbWaitForRemoteReceive;
extern u8 bMusicIsLooping[KB_MUSIC_TRACK_COUNT];

#endif
