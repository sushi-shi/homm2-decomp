#ifndef HOMM2_PLATFORM_WIN32_NETBIOSTRANSPORT_H
#define HOMM2_PLATFORM_WIN32_NETBIOSTRANSPORT_H
...
using enum NetbiosCommand;
ENABLE_ENUM_FLAGS(NetbiosCommand)

enum class NetbiosResult : u8 {
...
using enum NetbiosResult;

enum class NetbiosSessionOperation : i32 {
...
using enum NetbiosSessionOperation;

enum class NetbiosSessionStatus : u8 {
    NETBIOS_SESSION_ACTIVE          = 1,
    NETBIOS_SESSION_NAME_REGISTERED = 2,
    NETBIOS_SESSION_CONNECTED       = 8,
    NETBIOS_SESSION_ERROR           = 0x80
};
using enum NetbiosSessionStatus;
ENABLE_ENUM_FLAGS(NetbiosSessionStatus)
...
struct NetbiosControlBlock {
    H2EnumStorage<NetbiosCommand, u8> command;
    H2EnumStorage<NetbiosResult, u8> returnCode;
    u8 sessionNumber;
    u8 nameNumber;
    void* buffer;
    u16 length;
    ...
    u8 adapterNumber;
    H2EnumStorage<NetbiosResult, u8> commandComplete;
