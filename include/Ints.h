...
#define ENABLE_ENUM_FLAGS(name)                                                                    \
    [[maybe_unused]] inline constexpr name operator|(name a, name b) {                             \
    ...
    [[maybe_unused]] inline constexpr name operator~(name a) {                                     \
        return static_cast<name>(~static_cast<i32>(a));                                            \
    }                                                                                              \
    [[maybe_unused]] inline constexpr bool operator!(name a) { return !static_cast<i32>(a); }      \
    [[maybe_unused]] inline name& operator|=(name& a, name b) { return a = a | b; }                \
    [[maybe_unused]] inline name& operator&=(name& a, name b) { return a = a & b; }                \
    [[maybe_unused]] inline name& operator^=(name& a, name b) { return a = a ^ b; }                \
    [[maybe_unused]] inline constexpr name operator+(name a, name b) {                             \
        return static_cast<name>(static_cast<i32>(a) + static_cast<i32>(b));                       \
    ...
    [[maybe_unused]] inline name& operator+=(name& a, name b) { return a = a + b; }                \
    [[maybe_unused]] inline name& operator-=(name& a, name b) { return a = a - b; }                \
    [[maybe_unused]] inline constexpr name operator&(name a, i32 mask) {                           \
        return static_cast<name>(static_cast<i32>(a) & mask);                                      \
    }                                                                                              \
    [[maybe_unused]] inline constexpr name operator&(i32 mask, name a) { return a & mask; }        \
    [[maybe_unused]] inline constexpr name operator|(name a, i32 mask) {                           \
        return static_cast<name>(static_cast<i32>(a) | mask);                                      \
    }                                                                                              \
    [[maybe_unused]] inline constexpr name operator|(i32 mask, name a) { return a | mask; }        \
    [[maybe_unused]] inline name& operator&=(name& a, i32 mask) { return a = a & mask; }           \
    [[maybe_unused]] inline name& operator|=(name& a, i32 mask) { return a = a | mask; }

#define ENABLE_ENUM_STEPS(name)                                                                    \
    [[maybe_unused]] inline constexpr name operator+(name a, i32 amount) {                         \
        return static_cast<name>(static_cast<i32>(a) + amount);                                    \
    }                                                                                              \
    [[maybe_unused]] inline constexpr name operator-(name a, i32 amount) {                         \
        return static_cast<name>(static_cast<i32>(a) - amount);                                    \
    }                                                                                              \
    [[maybe_unused]] inline constexpr i32 operator-(name a, name b) {                              \
        return static_cast<i32>(a) - static_cast<i32>(b);                                          \
    }                                                                                              \
    [[maybe_unused]] inline constexpr name operator%(name a, i32 modulus) {                        \
        return static_cast<name>(static_cast<i32>(a) % modulus);                                   \
    }                                                                                              \
    [[maybe_unused]] inline constexpr name operator%(name a, name modulus) {                       \
        return static_cast<name>(static_cast<i32>(a) % static_cast<i32>(modulus));                 \
    }                                                                                              \
    [[maybe_unused]] inline constexpr name operator&(name a, i32 mask) {                           \
        return static_cast<name>(static_cast<i32>(a) & mask);                                      \
    }                                                                                              \
    [[maybe_unused]] inline name& operator+=(name& a, i32 amount) { return a = a + amount; }       \
    [[maybe_unused]] inline name& operator-=(name& a, i32 amount) { return a = a - amount; }       \
    [[maybe_unused]] inline name& operator+=(name& a, name b) { return a = a + static_cast<i32>(b); } \
    [[maybe_unused]] inline name& operator-=(name& a, name b) { return a = a - static_cast<i32>(b); } \
    [[maybe_unused]] inline name& operator%=(name& a, i32 modulus) { return a = a % modulus; }     \
    [[maybe_unused]] inline name& operator%=(name& a, name modulus) { return a = a % modulus; }    \
    [[maybe_unused]] inline name& operator++(name& a) { return a = a + 1; }                        \
    [[maybe_unused]] inline name operator++(name& a, int) { name old = a; a = a + 1; return old; } \
    [[maybe_unused]] inline name& operator--(name& a) { return a = a - 1; }                        \
    [[maybe_unused]] inline name operator--(name& a, int) { name old = a; a = a - 1; return old; }

#define ENABLE_ENUM_INDEX_OFFSETS(name)                                                            \
    [[maybe_unused]] inline constexpr i32 operator+(name value, i32 offset) {                      \
