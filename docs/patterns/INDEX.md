...
| od-signed-cast-breaks-memory-rmw | topic:od cpp:cast topic:flags | An explicit signed value cast can prevent VC4.2 from folding `field = field - bit` into memory RMW; use it when retail materializes load/sub/store |
...
| od-goto-stubs-and-inline-continuations | topic:od topic:control-flow cpp:goto cpp:inline flag:Ob1 | retail-extra `jmp` decoder: forward gotos own end stubs + dead structural jmps, `default: break` adds a case-exit jmp, and a bare `jmp $+0` before a statement is an inlined accessor read (or the empty DebugCheck hook); islands cannot move any of these |
