...
| msvc42-tu-state-noise-search | topic:tu-state topic:last-mile topic:tooling | Bounded preprocessor-only TU-state probes; generated noise is never retained, while guarded `--record-max` can preserve an eligible score for the unchanged source hash |
