...
| [flat-index-grouping-is-not-a-trick](flat-index-grouping-is-not-a-trick.md) | `*(base + x + row)` is a GROUPING fact, not a hack: `p[x+row]` regroups to `p + (x+row)` and regressed 19 exact functions; `(p + x)[row]` keeps `(p+x) + row` and is byte-identical |
