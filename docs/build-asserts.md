...
- Types come from the recovered class headers; globals from their owner-TU headers (a plain
  ...
  `include/win/windows.h`; CRT from real `<io.h>`/`<string.h>`; owner-less / cross-module synthetics
  from `include/_globals_model.h` (single-module synthetics live as `static` defs in their module).
...
- **`homm2 relocs`** (`assert_relocs.py`) — **opt-in reloc-target audit.** objdiff **masks every
  ...
  targets (from `symbol_names.csv` + header `DATA()` VAs — REL32→symbol RVA, DIR32→symbol+addend,
