...
## Source paths / file names (29)
    ...
    I:\Projects\Heroes\Prog\EDITOR\EDITMGR.CPP
    I:\Projects\Heroes\Prog\EDITOR\EDITOR.CPP
    I:\Projects\Heroes\Prog\EDITOR\Mapcell.cpp
    I:\Projects\Heroes\Prog\EDITOR\OVERLAY.CPP
    I:\Projects\Heroes\Prog\EDITOR\RANDOM.CPP
    I:\Projects\Heroes\Prog\EDITOR\TERRAIN.CPP
    I:\Projects\Heroes\Prog\EDITOR\evntedit.cpp
    I:\Projects\Heroes\Prog\EDITOR\line.cpp
    I:\Projects\Heroes\Prog\EDITOR\ridledit.cpp
    I:\Projects\Heroes\Prog\EDITOR\rumredit.cpp
    I:\Projects\Heroes\Prog\EDITOR\signedit.cpp
    I:\Projects\Heroes\Prog\EDITOR\specedit.cpp
    I:\Projects\Heroes\Prog\EDITOR\wingraph.cpp

## Assert / expression strings (identifier hints) (9)
...
## Format strings (124)
    ...
    %d days
    %d months
    %d obelisks on map.  There should be no more than 48 obelisks.
    %d weeks
    ...
    Generating map - Drawing ground type - %s
    Head %d   Tail %d
    ...
    Map Name: %s
    Map_%04d.%s
    Max map events (%d) has been reached.
    Max mines, sawmills and alchemists on map (%d) has been reached.
    Max towns on map (%d) has been reached.
    Mem Left %dK
    Memory Leak,  File '%13s'  Line % 4d, ptr %12d   size %6d
    Mine %02d: (%02d,%02d) type: %02d
    Move %s
    Move %s here.
    No Name %d
...
## Other strings (2682)
    ...
    *-UVWWW
    ...
    Allows you to draw roads by clicking and dragging.
    Allows you to draw streams by clicking and dragging.
    ...
    CSPANBTE.ICN
    CSPANBTN.ICN
    CToggle object grid on/off.  The object grid shows which squares an object occupies, and how each square is classified.
    Cactus
    Campfire
    Can't add manager!
    Can't init system
    ...
    Chain Lightning
    Chandler
    Change between zoom and normal view.
    ...
    Cold Ring
    Coliseum
    Colors allowed to get event
    Combat Army Info Level
    Combat Shade Level
    Combat Speed
    Comp. also wins via Special VC
    Computer Walk Speed
    Condor
    Continue the Hero's movement along the current path.
    Controls the area of the map being viewed.
    Corackston
    Corlagon defeated
    Corlagon's Defense
    Costs 1.25 times normal movement for all heroes.  (Pathfinding eliminates the penalty.)
    Costs 1.5 times normal movement for all heroes.  (Pathfinding reduces or eliminates the penalty.)
    Costs 1.75 times normal movement for all heroes.  (Pathfinding reduces or eliminates the penalty.)
    Costs 2 times normal movement for all heroes.  (Pathfinding reduces or eliminates the penalty.)
    Cottage
    Couldn't grab object!
    Country Lords
    Cracked
    Cracked Objs
    Crag Hack
    Crater
    Crawling
    Create a map that is 108 squares wide by 108 squares high.  (For reference, all the maps in Heroes 1 were 72 x 72)
    Create a map that is 144 squares wide by 144 squares high.  (For reference, all the maps in Heroes 1 were 72 x 72)
    Create a map that is 36 squares wide by 36 squares high.  (For reference, all the maps in Heroes 1 were 72 x 72)
    Create a map that is 72 squares wide by 72 squares high.  (For reference, all the maps in Heroes 1 were 72 x 72)
    ...
    DrawMenuBar
    Draws objects in 1 square increments.
    Draws objects in 2 by 2 square increments.
    Draws objects in 4 by 4 square increments.
    Draws terrain in 1 square increments.
    Draws terrain in 2 by 2 square increments.
    Draws terrain in 4 by 4 square increments.
    Dungeon
    ...
    Earthquake
    Earthworm
    Edit map title, description, and other general information.
    Editor
    ...
    Gazebo
    GdiSetBatchLimit
    Generating map - Analyzing Land Mass...
    Generating map - Cleaning up ground...
    Generating map - Connecting Castles...
    Generating map - Placing Castles...
    Generating map - Placing mountains...
    Generating map - Placing objects...
    Generating map - Placing treasures...
    Generating map - Placing trees...
    Genius
    GetACP
    GetActiveWindow
    GetAsyncKeyState
    ...
    Heroes of Might and Magic 2 Editor
    Hideous Mask
    Hill Fort
    Hillstone
    Hilltop
    ...
    No Name
    No heroes on map
    No maps exist for that size.
    No more events can be added unless some are deleted.
    No more rumors can be added unless some are deleted.
    No shots left!
    ...
    Only 1 whirlpool on map.  Another whirlpool is needed so there is someplace to travel to.
    Open the file options menu, where you can save or load maps, or quit out of the editor.
    Oracle
    Orchard
    Out of memory.
    ...
    ResetDisplayMode
    Resource
    Resources to give (or take if a negative qty.)
    ...
    ScreenToClient
    Scroll Button
    Scrolls the screen in the indicated direction.
    Sea Chest
    Secondary Skills
    See Rock City
    Seeing-eye Pendant
    ...
    Sorceress guild
    Sorpigal
    Sorry, you have already entered the maximum number of answers.
    Sound Quality
    Sound Volume
    ...
    Standard
    Standing Stones
    Start a new map from scratch.
    Start a new map.
    Start a single or multi-player game.
    Start from scratch with a blank map.
    Start with hero in each player's main castle
    ...
    The ultimate artifact has already been placed.
    The ultimate artifact must be placed at least 9 squares in from map edge.
    ...
    Toggle color cycling on/off.  Color cycling may hurt performance if you are running in Windows 95, non full screen, and your system is set to display more than 256 colors.
    Toggle foreground sounds level.
    Toggle object animation on/off.  Object animation may hurt performance, especially if you are working in the zoomed out modes.
    ...
    Toggles color mouse cursor on/off.
    Tomb robber -1
    Tortoise
    Total Memory Leaks
    Town Detail
    ...
    Unable to edit sign - created under old editor
    Unable to edit town - created under old editor
    ...
    Unable to place castles
    Undo your last action.  Press again to redo the action.
    ...
    Used for special editing of monsters, heroes and towns.
    Used to click and drag for clearing large areas.
    Used to click and drag for filling in large areas.
    Used to draw the underlying grass, dirt, water, etc. on the map.
    Used to erase objects off the map.
    Used to move the screen along the slide axis.
    Used to place a hero.
    Used to place a monster group.
    Used to place a resource or treasure.
    Used to place a town or castle.
    Used to place an artifact.
    Used to place objects (mountains, trees, treasure, etc.) on the map.
    Used to select objects most appropriate for use on beach.
    Used to select objects most appropriate for use on desert.
    Used to select objects most appropriate for use on dirt.
    Used to select objects most appropriate for use on grass.
    Used to select objects most appropriate for use on lava.
    Used to select objects most appropriate for use on snow.
    Used to select objects most appropriate for use on swamp.
    Used to select objects most appropriate for use on wasteland.
    Used to select objects most appropriate for use on water.
    VDf9V>u
    VGENBKG.ICN
    VGENBKGE.ICN
    VIEWARME.ICN
    ...
    View the editor system options, which let you customize the editor.
    ...
    You have at least one travel gate on the map without a partner.  Each of the 3 different travel gates only goes to others of the same type and needs at least one other to work properly.
