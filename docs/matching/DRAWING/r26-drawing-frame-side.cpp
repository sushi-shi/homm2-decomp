// VC6 SP5 source family r26-drawing-frame-side; parent aab81af5.
...
// build/source-variant-batch/r26-drawing-frame-side/results.json; RVA 0x395b8
// Complete 2 arms; elapsed 1.735001s; source restored.
...
// 1 / {'source_family': 'side_role', 'candidate': 'baseline'} / 5412 / ab9e6c7638823e78 / cbeb569835f98eab / 206:206 / 205,1,0,0 / True
...
// |     PollSound();
// | }
// ALTERNATIVE side_role (unified delta from BEFORE):
// | --- before
// | +++ side_role
// | @@ -9,6 +9,7 @@
// |  ) {
...
// |          extentChanged1 = false;
// | -        for (state = 0; IDX(state) < COMBAT_SIDE_COUNT; state++) {
// | +        for (extentSide = COMBAT_ATTACKER_SIDE; IDX(extentSide) < COMBAT_SIDE_COUNT; extentSide++) {
// |              for (armyIndex7 = 0; armyIndex7 < COMBAT_ARMY_SLOT_COUNT_DRAWING; armyIndex7++) {
// | -                if (m_limitCreatureCount[IDX(state)][armyIndex7] > 0) {
// | +                if (m_limitCreatureCount[IDX(extentSide)][armyIndex7] > 0) {
// |                      extentChanged1 = true;
// |                      gbComputeExtent = true;
// |                      gbSaveBiggestExtent = true;
// |                      gbReturnAfterComputeExtent = true;
...
// | -                        m_hexCells[m_armies[IDX(state)][armyIndex7].m_hex].m_x,
// | -                        m_hexCells[m_armies[IDX(state)][armyIndex7].m_hex].m_y,
// | +                    m_armies[IDX(extentSide)][armyIndex7].DrawToBuffer(
// | +                        m_hexCells[m_armies[IDX(extentSide)][armyIndex7].m_hex].m_x,
// | +                        m_hexCells[m_armies[IDX(extentSide)][armyIndex7].m_hex].m_y,
// |                          0
// |                      );
// |                      gbReturnAfterComputeExtent = false;
// | @@ -54,18 +55,18 @@
...
// | -            if (m_drawHero[IDX(state)] != 0) {
// | +        for (extentSide = COMBAT_ATTACKER_SIDE; IDX(extentSide) < COMBAT_SIDE_COUNT; extentSide++) {
// | +            if (m_drawHero[IDX(extentSide)] != 0) {
// |                  extentChanged1 = true;
// |                  gbComputeExtent = true;
// |                  gbSaveBiggestExtent = true;
// |                  gbReturnAfterComputeExtent = true;
...
// | -                        .animationFrames[m_heroAnimationState[IDX(state)]][m_heroAnimationFrame[IDX(state)]],
// | -                    &m_heroLimits[IDX(state)],
// | +                m_heroIcons[IDX(extentSide)]->CombatClipDrawToBuffer(
// | +                    IDX(extentSide) == 0 ? COMBAT_HERO_LEFT_X : COMBAT_HERO_RIGHT_X,
// | +                    IDX(extentSide) == 0 ? COMBAT_HERO_LEFT_Y : COMBAT_HERO_RIGHT_Y,
// | +                    sCmbtHero[m_heroSpriteIndex[IDX(extentSide)]]
// | +                        .animationFrames[m_heroAnimationState[IDX(extentSide)]][m_heroAnimationFrame[IDX(extentSide)]],
// | +                    &m_heroLimits[IDX(extentSide)],
// |                      ICON_DRAW_FLIPPED,
// |                      0,
// |                      NULL,
// | @@ -76,22 +77,22 @@
...
// |
// | -            if (m_drawHeroOverlay[IDX(state)] != 0) {
// | +            if (m_drawHeroOverlay[IDX(extentSide)] != 0) {
// |                  extentChanged1 = true;
// |                  gbComputeExtent = true;
// |                  gbSaveBiggestExtent = true;
// |                  gbReturnAfterComputeExtent = true;
...
// | -                    IDX(state) == 0 ? COMBAT_HERO_LEFT_X
// | -                               : ((m_heroes[IDX(state)]->m_isCaptain ? -1 : 0)
// | +                m_heroOverlayIcons[IDX(extentSide)]->CombatClipDrawToBuffer(
// | +                    IDX(extentSide) == 0 ? COMBAT_HERO_LEFT_X
// | +                               : ((m_heroes[IDX(extentSide)]->m_isCaptain ? -1 : 0)
// |                                    & (COMBAT_HERO_RIGHT_ALT_X - COMBAT_HERO_RIGHT_X))
// |                                       + COMBAT_HERO_RIGHT_X,
// | -                    IDX(state) == 0 ? COMBAT_HERO_LEFT_Y
// | -                               : ((m_heroes[IDX(state)]->m_isCaptain ? -1 : 0)
// | +                    IDX(extentSide) == 0 ? COMBAT_HERO_LEFT_Y
// | +                               : ((m_heroes[IDX(extentSide)]->m_isCaptain ? -1 : 0)
// |                                    & (COMBAT_HERO_RIGHT_ALT_Y - COMBAT_HERO_RIGHT_Y))
// |                                       + COMBAT_HERO_RIGHT_Y,
// | -                    m_heroOverlayFrame[IDX(state)],
// | -                    &m_heroOverlayLimits[IDX(state)],
// | +                    m_heroOverlayFrame[IDX(extentSide)],
// | +                    &m_heroOverlayLimits[IDX(extentSide)],
// |                      ICON_DRAW_FLIPPED,
// |                      0,
// |                      NULL,
