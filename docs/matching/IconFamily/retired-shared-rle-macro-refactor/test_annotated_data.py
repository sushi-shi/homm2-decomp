...
from homm2.build.annotated_data import (
    ...
    _clang_args,
    _icon_rle_state_definitions,
    source_definitions,
)


class AnnotatedDataTest(unittest.TestCase):
    def test_icon_rle_state_csv_supplies_macro_owned_definitions(self):
        with tempfile.TemporaryDirectory() as directory:
            repo = Path(directory)
            path = repo / "docs/icon-rle-state.csv"
            path.parent.mkdir()
            path.write_text(
                "unit,function,role,rva\n"
                "BASE/Icon2b,IconToBitmap,row,0x00134c20\n"
                "BASE/Icon2b,IconToBitmap,color,0x00134c28\n"
            )

            rows = _icon_rle_state_definitions(repo)

        self.assertEqual(
            [(row.name, row.rva, row.size) for row in rows],
            [("s_row", 0x00134C20, 4), ("s_color", 0x00134C28, 1)],
        )
