...
ICON_RLE_STATE_CSV = Path("docs/icon-rle-state.csv")
ICON_RLE_STATE_ROLES = {
    "row": ("s_row", 4),
    "pitch": ("s_pitch", 4),
    "color": ("s_color", 1),
    "dim_palette": ("s_dimPal", 4),
    "run": ("s_run", 4),
    "signed_run": ("s_run", 4),
    "loop_count": ("s_loopCount", 4),
    "loop_index": ("s_loopIndex", 4),
    "source": ("s_src", 4),
    "literal_source": ("s_literalSource", 4),
    "source_skip": ("s_srcSkip", 4),
    "destination": ("s_dst", 4),
    "dim_destination": ("s_dimDst", 4),
    "clip_right": ("s_clipR", 4),
    "clip_bottom": ("s_clipB", 4),
    "left": ("s_left", 4),
    "right": ("s_right", 4),
    "dim_length": ("s_dimLen", 4),
    "y": ("s_y", 4),
    "x": ("s_x", 4),
    "entry": ("s_entry", 4),
    "span_count": ("s_spanCount", 4),
    "signed_span_count": ("s_spanCount", 4),
}
...
def _icon_rle_state_definitions(repo: Path) -> list[AnnotatedDataDefinition]:
    path = repo / ICON_RLE_STATE_CSV
    if not path.is_file():
        return []
    rows = []
    seen = set()
    with path.open(newline="", encoding="utf-8") as stream:
        for line, value in enumerate(csv.DictReader(stream), 2):
            unit = value.get("unit", "").strip()
            function = value.get("function", "").strip()
            role = value.get("role", "").strip()
            rva_text = value.get("rva", "").strip()
            if not unit or not function or not role or not rva_text:
                raise ValueError(f"{path}:{line}: incomplete icon RLE state row")
            try:
                name, size = ICON_RLE_STATE_ROLES[role]
            except KeyError as error:
                raise ValueError(
                    f"{path}:{line}: unknown icon RLE state role {role!r}"
                ) from error
            identity = unit, name
            if identity in seen:
                raise ValueError(
                    f"{path}:{line}: duplicate icon RLE state {unit}:{name}"
                )
            seen.add(identity)
            rows.append(AnnotatedDataDefinition(
                ...
                rva=int(rva_text, 0),
                size=size,
                location=f"{ICON_RLE_STATE_CSV.as_posix()}:{line}",
                is_static=True,
            ))
    return rows
...
def source_definitions(source_root: Path, repo: Path, object_root: Path | None = None,
    ...
    rows.extend(_icon_rle_state_definitions(repo))
