// VC6 SP5 Castle source family r22-castle-handler-roles; parent f8b6c956.
// Source record only. All alternatives below are relative to the stated BEFORE,
// not cumulative diffs. No cross-family/TU-state census is claimed.
// build/source-variant-batch/r22-castle-handler-roles/results.json; RVA 0x25230
// Complete 2 arms; elapsed 1.600902s; input restored.
...
// 1 / {'source_family': 'separate_info_and_object_index', 'candidate': 'baseline'} / 2925 / 58e5604f1bb59139 / 767a6e0538f36594 / 127:130 / 123,0,0,4 / False
...
// |     return MESSAGE_DISPATCH_CONSUME;
// | }
// ALTERNATIVE separate_info_and_object_index (unified source delta from BEFORE):
// | --- before
// | +++ separate_info_and_object_index
// | @@ -1,5 +1,6 @@
// |  MessageDispatchResult CastleHandler(tag_message& message) {
// |      i32 objIndex;
// | +    i32 infoTextIndex;
...
// |                      if (!(gpTownManager->m_town->m_buildings & 1L))
// | -                        objIndex = IDX(INFO_BUILD_MAGE_GUILD);
// | +                        infoTextIndex = IDX(INFO_BUILD_MAGE_GUILD);
// |                      else if (gpTownManager->m_town->m_buildState == TOWN_MAGE_GUILD_MAX_LEVEL)
// | -                        objIndex = IDX(INFO_MAGE_GUILD_MAX_LEVEL);
// | +                        infoTextIndex = IDX(INFO_MAGE_GUILD_MAX_LEVEL);
// |                      else if (!CanBuy(gpTownManager->m_town, CASTLE_MAGE_GUILD))
// | -                        objIndex = IDX(INFO_CANNOT_AFFORD_MAGE_LEVEL);
// | +                        infoTextIndex = IDX(INFO_CANNOT_AFFORD_MAGE_LEVEL);
// |                      else
// | -                        objIndex = IDX(INFO_ADD_MAGE_GUILD_LEVEL);
// | -                    strcpy(gText, cCastleInfo[objIndex]);
// | +                        infoTextIndex = IDX(INFO_ADD_MAGE_GUILD_LEVEL);
// | +                    strcpy(gText, cCastleInfo[infoTextIndex]);
// |                  }
// |                  break;
// |
