// VC6 SP5 source family r27-newgame-update-counter; parent f84e0b7c plus U18 incoming packet header.
...
// build/source-variant-batch/r27-newgame-update-counter/results.json; RVA 0x76e3b
// Complete 2 arms; elapsed 1.757448s; source restored.
...
// 1 / {'family': 'separate_roles', 'candidate': 'baseline'} / 1295 / 13e3b238a744c693 / dee21ed9ddd159bb / 35:35 / 35,0,0,0 / True
...
// |     }
// |
// ALTERNATIVE separate_roles (unified delta from BEFORE):
// | --- before
// | +++ separate_roles
// | @@ -3,6 +3,8 @@
// |          b32 playerLockedValue;
...
// | -        for (playerIndex = 0; playerIndex < IDX(DIFFICULTY_COUNT); ++playerIndex) {
// | -            message.payload.widget.id = NEW_GAME_DIFFICULTY_FIRST + playerIndex;
// | +        for (difficultyIndex = 0; difficultyIndex < IDX(DIFFICULTY_COUNT); ++difficultyIndex) {
// | +            message.payload.widget.id = NEW_GAME_DIFFICULTY_FIRST + difficultyIndex;
// |              m_newGameWindow->BroadcastMessage(message);
// |          }
// |          message.payload.widget.command = NEW_GAME_WIDGET_ENABLE;
// | @@ -23,10 +25,10 @@
...
// | -            for (playerIndex = 0; playerIndex < GAME_CHAT_LINE_COUNT; ++playerIndex) {
// | -                sprintf(gText, cTextReceivedBuffer[playerIndex]);
// | +            for (chatLine = 0; chatLine < GAME_CHAT_LINE_COUNT; ++chatLine) {
// | +                sprintf(gText, cTextReceivedBuffer[chatLine]);
// |                  message.payload.widget.command = NEW_GAME_WIDGET_SET_TEXT;
// | -                message.payload.widget.id = NEW_GAME_CHAT_FIRST + playerIndex;
// | +                message.payload.widget.id = NEW_GAME_CHAT_FIRST + chatLine;
// |                  message.payload.widget.data.text = gText;
// |                  m_newGameWindow->BroadcastMessage(message);
// |              }
