# Strings — HEROES2W.EXE (Price of Loyalty)

Extracted by `scripts/extract_strings.py` (printable runs >= 5 bytes, deduped).
Source paths recover original file names; assert/expression strings can recover
argument / local / member identifiers (MSVC `assert()` embeds the expression).
**Hints, not authoritative** — cross-check against the disassembly.

## Source paths / file names (54)

    %c:\heroes2\anim\voy24.smk
    .\HELP\HEROES2.HLP
    C:\MSDEV\LIB\LIBCMT.lib
    C:\MSDEV\LIB\advapi32.lib
    C:\MSDEV\LIB\comdlg32.lib
    C:\MSDEV\LIB\gdi32.lib
    C:\MSDEV\LIB\kernel32.lib
    C:\MSDEV\LIB\oldnames.lib
    C:\MSDEV\LIB\ole32.lib
    C:\MSDEV\LIB\oleaut32.lib
    C:\MSDEV\LIB\shell32.lib
    C:\MSDEV\LIB\user32.lib
    C:\MSDEV\LIB\uuid.lib
    C:\MSDEV\LIB\winmm.lib
    C:\MSDEV\LIB\winspool.lib
    C:\MSDEV\LIB\wsock32.lib
    C:\TEMP\lnk2
    I:\Projects\Heroes\Prog\BASE\BITMAP.CPP
    I:\Projects\Heroes\Prog\BASE\Blur.cpp
    I:\Projects\Heroes\Prog\BASE\Bzip.cpp
    I:\Projects\Heroes\Prog\BASE\ICON.CPP
    I:\Projects\Heroes\Prog\BASE\MOUSEMGR.CPP
    I:\Projects\Heroes\Prog\BASE\Misc.cpp
    I:\Projects\Heroes\Prog\BASE\PALETTE.CPP
    I:\Projects\Heroes\Prog\BASE\RESMGR.CPP
    I:\Projects\Heroes\Prog\BASE\SAMPLE.CPP
    I:\Projects\Heroes\Prog\BASE\TEXTWDGT.CPP
    I:\Projects\Heroes\Prog\BASE\TILESET.CPP
    I:\Projects\Heroes\Prog\BASE\Textntry.cpp
    I:\Projects\Heroes\Prog\BASE\WINMGR.CPP
    I:\Projects\Heroes\Prog\BASE\droplist.cpp
    I:\Projects\Heroes\Prog\BASE\listbox.cpp
    I:\Projects\Heroes\Prog\BASE\soundmgr.cpp
    I:\Projects\Heroes\Prog\SOURCE\ADVMGR.CPP
    I:\Projects\Heroes\Prog\SOURCE\COMMAND.CPP
    I:\Projects\Heroes\Prog\SOURCE\CURSOR.CPP
    I:\Projects\Heroes\Prog\SOURCE\EVENTS.CPP
    I:\Projects\Heroes\Prog\SOURCE\FINDPATH.CPP
    I:\Projects\Heroes\Prog\SOURCE\GAME.CPP
    I:\Projects\Heroes\Prog\SOURCE\KB.CPP
    I:\Projects\Heroes\Prog\SOURCE\Newgame.cpp
    I:\Projects\Heroes\Prog\SOURCE\Overview.cpp
    I:\Projects\Heroes\Prog\SOURCE\PHILAI.CPP
    I:\Projects\Heroes\Prog\SOURCE\REMOTE.CPP
    I:\Projects\Heroes\Prog\SOURCE\SPELLS.CPP
    I:\Projects\Heroes\Prog\SOURCE\TOWNMGR.CPP
    I:\Projects\Heroes\Prog\SOURCE\Wsnetwin.cpp
    I:\Projects\Heroes\Prog\SOURCE\comwin.cpp
    I:\Projects\Heroes\Prog\SOURCE\dpnetwin.cpp
    I:\Projects\Heroes\Prog\SOURCE\netwin.cpp
    I:\Projects\Heroes\Prog\SOURCE\wingraph.cpp
    c:\TEMP\Z.DIF
    handler.cpp
    i:\projects\heroes\art\fin3d\

## Assert / expression strings (identifier hints) (13)

    /pqXY[\]]
    ===========New Log==========
    ?@AABCDEFFGHHIIJJKKLLLUUVWXYZ[[\]]
    NULL POINTER
    PhT[Q
    ResMgr::PointToFile failure!  ThisFileId:%d  LastFileId:%d  LastFileName:%s
    UUVWWXXYZZ[[[\]]]
    UUVWXXYZ[[\]]^__```
    UUVWXXZZ[[
    build\intel\mt_obj\assert.obj
    new_p == 0
    pp+/pWYZ[[
    sCDTest_Track_Data cdTestTrackData[CDTEST_NUMTRACKS] =

## Format strings (353)

                                                  Name - %s
                                            total time - %8d
                                Total Frames displayed - %8d
                              Total time spent reading - %8d
                             Average size of the frame - %8d
                             Slowest single frame time - %8d
                             Total time spent blitting - %8d
                           Slowest single frame number - %8d
                         Total io speed (sbytes/second) - %8d
                        Total number of skipped frames - %8d
                        Total time spent decompressing - %8d
                      Second slowest single frame time - %8d
                    Highest amount of memory allocated - %8d
                    Second slowest single frame number - %8d
            Time to open and prepare for decompression - %8d
       %d -> %d:  cand %5d,   freq = %6d,   notdone = %6d
      File: '%s'
      File:'%s'
      Line: %d
     %d levels.
     + %d / week
     All of the %s, %s and %s you have in your army have been trained by the battle masters of the fort.  Your army now contains %s, %s, and %s.
     Line# %d
     damage to the %s.
     scalings %d
    "I will accept your surrender and grant you and your troops safe passage for the price of %d gold."
    "Vqcobj%04d.icn
    #define CDTEST_NUMTRACKS %d
    %6.3f:1, %6.3f bits/byte, %5.2f%% saved, %d in, %d out.
    %c:\heroes2\anim\voy24.smk
    %d %s rise from the dead!
    %d %s rises from the dead!
    %d Unit
    %d Units
    %d of the %d %s will join your army, and the rest will leave you alone, for the sum of %d gold.  Do you accept?
    %d of the creatures will join us...
    %d/day
    %dK of Extended Memory (XMS) and
    %s %s half the enemy troops!
    %s Barrier
    %s Necromancy (+%d) allows %d percent of the creatures killed in combat to be brought back from the dead as Skeletons.
    %s Traveller's Tent
    %s casts '%s' on the %s.
    %s casts '%s'.
    %s currently has %d spell points out of a maximum of %d.  The maximum number of spell points is 10 times your knowledge.  It is occasionally possible to have more than your maximum spell points via special events.
    %s earns the most gold.
    %s growth +5.
    %s has been timed out of the game.  The current game has been saved as 'PLYREXIT'.  Do you wish to continue playing with a computer player filling in for %s?
    %s has been vanquished!
    %s has fallen!  All is lost.
    %s has found the most artifacts.
    %s has found the most obelisks.
    %s has gained
    %s has the most powerful forces.
    %s is a level %d %s with %d artifacts.
    %s is already built
    %s is exiting the game.  The current game has been saved as 'PLYREXIT'.  Do you wish to continue playing with a computer player filling in for %s?
    %s is near.
    %s left the game, and the %s decided to terminate the entire game.  The game has been saved as 'PLYREXIT'.  Do you wish to play on alone with the computer taking over for all human players?
    %s meets %s
    %s must defeat %s
    %s player
    %s population doubles!
    %s states:
    %s surrenders to the enemy, and departs in shame.
    %s the %s
    %s to attack again.
    %s to freeze in panic.
    %s%s.SMK
    %s's %s is under attack!
    %s's computer is not responding.  Do you wish to keep waiting for a response?
    %s's heroes have abandoned him, and he is banished from this land.
    %s's turn.
    %s, this is your last day to capture a town, or you will be banished from this land.
    %s, you have lost your last town.  If you do not conquer another town in the next week, you will be eliminated.
    %s, you only have %d days left to capture a town, or you will be banished from this land.
    %s, your heroes abandon you, and you are banished from this land.
    %s.icn
    %s: Can't allocate enough memory for compression.
    %s: Can't allocate enough memory for decompression.
    %s: Caught a SIGSEGV or SIGBUS whilst compressing,
    %s: Caught a SIGSEGV or SIGBUS whilst decompressing,
    %s: Compressed file ends unexpectedly;
    %s: Control-C (or similar) caught, quitting.
    %s: Data integrity error when decompressing.
    %s: I/O or other error, bailing out.  Possible reason follows.
    %s: block overrun during decompression,
    %s: compressed file didn't unblock correctly,
    %s: read past the end of compressed data,
    %sCAMPAIGN.HS
    %sSTANDARD.HS
    %s\heroes2\anim\voy24.smk
    %sattk.82M
    %sexpl.82M
    %sext1.82M
    %sext2.82M
    %skill.82M
    %smove.82M
    %sshot.82M
    %swnce.82M
    // FILE : %s
    A group of %s with a desire for greater glory wish to join you. Do you accept? 
    A leprechaun offers you the %s for the small price of 2000 gold.  Do you wish to buy this artifact?
    A leprechaun offers you the %s for the small price of 2500 gold and 3 %s.  Do you wish to buy this artifact?
    A leprechaun offers you the %s for the small price of 3000 gold and 5 %s.  Do you wish to buy this artifact?
    A magical %s barrier stands tall before you, blocking your way.  Runes on the arch read, "Speak the key and you may pass."
    ADVMBW%02d.BMP
    AI encountered object type %d and doesn't know how to deal with it.   Tell Phil
    ATDT%s
    Accumulate %d gold
    Add Name Error %02x
    After scouring the area, you fall upon a hidden chest, containing the ancient artifact '%s'
    After spending hours trying to fish the chest out of the sea, you open it and find 1000 gold and the %s
    All %d of the %s will join your army for the sum of %d gold.  Do you accept?
    All %s troops +1
    All of the %s and %s you have in your army have been trained by the battle masters of the fort.  Your army now contains %s and %s.
    All of the %s you have in your army have been trained by the battle masters of the fort.  Your army now contains %s.
    All of your  %s, %s and %s have been upgraded into %s, %s, and %s.
    All of your %s and %s have been upgraded into %s and %s.
    All of your %s have been upgraded into %s.
    An ancient and immortal witch living in a hut with bird's legs for stilts teaches you %s for her own inscrutable purposes.
    An eerie wailing song emanates from the sirens perched upon the rocks.  Many of your crew fall under its spell, and dive into the water where they drown.  You are now wiser for the visit, and gain %d experience.
    As you enter the Alchemist's Tower, a hobbled, graying man in a brown cloak makes his way towards you.  He checks your pack, and sees that you have %d cursed items.  For 750 gold,  the alchemist will remove them for you.  Do you pay?
    As you reach for the %s, it mysteriously disappears.
    Assert statement failed in module %s, line %d.  Do you wish to abort the program?
    Assertion failed: %s, file %s, line %d
    Astrologers proclaim month of the %s.
    Astrologers proclaim week of the %s.
    Attack %s
    BEGIN block %d
    Bad Delete,  File '%13s'  Line % 4d, ptr %12d
    Bad luck descends on the %s
    Base Score: %d
    Battle cowardice %d
    Build %s
    CAMP%c%02d.H2C
    CAMP%c%02dB.H2C
    CAMP%d_%02d.HXC
    CCYCLE%02d.BIN
    CMSEBW%02d.BMP
    COM%d
    CRC = 0x%x
    CRC Check Failed CRC 1 %d CRC 2 %d
    CRCs: stored = 0x%x, computed = 0x%x
    CSTLCAP%c.ICN
    Can't open file: %s
    Cannot afford %s
    Cannot build %s
    Cannot recruit - you already have %d Heroes.
    Capture the %s '%s'
    Cast %s
    Cast %s on %s
    Combine %s
    Combine %s armies
    Command '%s'
    Communications error on function '%s'
    Current experience %d
    Data miscommunication in hero position, first %d, %d, second %d, %d.  Please give Phil a copy of  your Autosave and, if possible, instructions to recreate this error
    Days: %d
    Defeat the hero '%s'
    Description '%s'
    Difficulty: %d
    Dismiss %s the %s
    Enter player %d's name.
    Error During WSAStartup(): %d
    Error During bind(): %d
    Error During gethostname(): %d
    Error During ioctlsocket(): %d
    Error During sendto(): %d
    Error During socket(): %d
    Error opening file %s!
    Event ID %d
    Exchange %s with %s
    FUN U :% 15s
    Fail to win by the end of month %d, week %d, day %d.
    File Error %s
    File error - bytes read %d, bytes requested %d, errno %d, last file '%s'
    Find the %s
    Fly %s here.
    For valor in combat, %s receives %d experience
    For valor in combat, %s receives %d experience, and gains %d level(s).
    Good luck shines on the %s
    Head %d   Tail %d
    Host player %s reports that player %s has been timed out of the game.  The game will continue with a computer player filling in for %s.
    Host player %s reports that player %s has exited the game.  The game will continue with a computer player filling in for %s.
    Hosting game at %s
    Hosting game at %s.
    I can offer you %d %s of %s for %d %s of %s.
    KBAlloc    Size %d   Ptr %d   File %s  Line %d
    KBFree    Size %d   Ptr %d   File %s  Line %d
    Kingdom Overview     Month %d, Week %d, Day %d
    Lose the %s '%s'.
    Lose the hero '%s'.
    MIDI%04d.XMI
    MS*100 per frame (100000/MS100PerFrame=Frames/Sec) - %8d
    Mage Guild, Level %d
    Manager %20s  this %d   prev %d  next %d
    Mem Left %dK
    Memory Leak,  File '%13s'  Line % 4d, ptr %12d   size %6d
    Move %s
    Move %s here.
    Move how many %s troops from %s to %s?
    Next level %d
    No maps exist for %d human player at that size.
    No maps exist for %d human players at that size.
    Only %d .AGG files can be used at once.
    Only one copy of %s may run at a time
    Opening COM%d
    Our victory was achieved in %d days!
    Player %d
    Player %d built %d %s in town %d.
    Player %d built %s in town %d.
    Player %d built hero in town %d.
    Practicing the dark arts of necromancy, you are able to raise %d of the enemy's dead to return under your service as Skeletons.
    RES - %15s  %d  %d
    RMT%sRC.BIN
    RMT%sRD.BIN
    RMT%sRL.BIN
    RMT%sSC.BIN
    RMT%sSD.BIN
    RMT%sSL.BIN
    Recruit %s
    Recruit %s the %s
    Redistribute %s army
    Requested %d bytes for a block size of %d.
    ResMgr::PointToFile failure!  ThisFileId:%d  LastFileId:%d  LastFileName:%s
    SHOT%04d.PCX
    SPELBW%02d.BMP
    Score: %d
    Select %s
    Shoot %s(%d shot(s) left)
    Split %s
    Stored CRC = 0x%x, computed CRC = 0x%x
    TCP/IP Error During command 'sendto()' # %d
    Test # %d
    That spell costs %d mana.  You only have %d mana, so you can't cast the spell.
    The %s %s the spell!
    The %s are paralyzed by the Cyclopes!
    The %s fires with the strength of %d Archers, each with a +%d bonus to their attack skill.
    The %s fires with the strength of %d Archers.
    The %s increases production of %s by 8 per week.
    The %s is destroyed.
    The %s is swayed by your diplomatic tongue, and offers to join your army for the sum of %d gold.  Do you accept?
    The %s produces %s.
    The %s spell does %d damage
    The %s, awed by the power of your forces, begin to scatter.  Do you wish to pursue and engage them?
    The Broach of Shielding provides 50% protection from Armageddon and Elemental Storm, but decreases spell power by 2.
    The Death spell does %d damage
    The Demon screams its challenge and attacks!  After a short, desperate battle, you slay the monster and find the %s in the back of the cave.
    The Golden Bow eliminates the 50% penalty for your troops shooting past obstacles. (e.g. castle walls)
    The Heart of Fire provides 50% protection from fire, but doubles the damage taken from cold.
    The Heart of Ice provides 50% protection from cold, but doubles the damage taken from fire.
    The Medusas' gaze turns the %s to stone!
    The Mummies' curse falls upon the %s!
    The Statesman's Quill reduces the cost of surrender to 10% of the total cost of troops you have in your army.
    The Unicorns' attack blinds the %s!
    The armaggedon does %d damage.
    The captain casts '%s' on the %s.
    The captain casts '%s'.
    The cold ray does %d
    The cold ring does %d damage.
    The cowardly %s flees from battle.
    The disrupting ray reduces defense by %d.
    The elemental storm does %d damage.
    The enemy has built up over %d gold in his treasury.  You must bow done in defeat before his wealth and power.
    The enemy has captured %s!  They are triumphant.
    The enemy has found the %s.  Your quest is a failure.
    The fireball does %d damage.
    The game you have chosen only has slots for %d human(s).  You need one with room for at least %d humans.
    The game you have chosen was being played with %d humans. Is it OK if the computer takes the place of the last %d human(s)?
    The lightning bolt does %d
    The magic arrow does %d
    The meteor shower does %d damage.
    The ultimate artifact is really the %s.
    The ultimate artifact may be found %s.
    The ultimate artifact may be found in the %s regions of the world.
    This Spell Scroll gives your hero the ability to cast the '%s' spell.
    This lets you change the handicap of a particular player.  Only humans may be handicapped.  Handicapped players start with fewer resources and earn 15 or 30% fewer resources per turn for mild and severe handicaps, respectively.
    Through a clearing you observe an ancient artifact.  Unfortunately, it's guarded by a nearby %s.  Do you want to fight the %s for the artifact?
    Through eagle-eyed observation, %s is able to learn the magic spell '%s'.
    Town:%2d  Building: % 18s   Raw BC = %8.2f,  RandBC = %8.2f.
    Town:%2d  Creature: % 18s   Raw BC = %8.2f,  RandBC = %8.2f.
    Town:%2d  Hero    : % 15i   Raw BC = %8.2f,  RandBC = %8.2f.
    Unknown message: %d
    Victorious, you take your prize, the %s
    View %s
    View %s %s Info
    View %s Info
    View %s info.
    WFHS %d
    Win95 Error Code: %d
    Win95 Error Meaning: %s
    You already have %d creatures groups in combat and cannot add any more.
    You approach the hut and observe a witch inside studying an ancient tome on %s.  As you approach, she turns and focuses her one glass eye on you.  "You already know everything you deserve to learn!" the witch screeches. "NOW GET OUT OF MY HOUSE!"
    You approach the hut and observe a witch inside studying an ancient tome on %s.  As you approach, she turns and speaks.  "You already know that which I would teach you. I can help you no further."
    You captured %s!  You are victorious.
    You come across an old wagon left by a trader who didn't quite make it to safe terrain.  Searching inside, you find the '%s'.
    You enter the tent and see an old woman gazing into a magic gem.  She looks up and says, "In my travels, I have learned much in the way of arcane magic.  A great oracle taught me his skill.  I have the answer you seek.  The %s keyword is '%s'."
    You find a small quantity of %s.
    You have %d guest(s) now logged in.  Click 'OK' to move on, or wait for additional guests.
    You have %d guest(s) out of an expected total of %d guest(s) now logged in.  Click 'CANCEL' to move on without waiting for additional guests.
    You have built up over %d gold in your treasury.  All enemies bow before your wealth and power.
    You have captured the enemy hero %s!  Your quest is complete.
    You have found the %s.  Your quest is complete.
    You have learned %s %s.
    You have lost the hero %s.  Your quest is over.
    You may learn either %s %s or %s %s.
    You've come across the spartan quarters of a retired soldier.  The soldier tells you that he is willing to pass on the %s to the first true leader he meets.
    You've found the humble dwelling of a withered hermit.  The hermit tells you that he is willing to give the %s to the first wise person he meets.
    You've pulled a shipwreck survivor from certain death in an unforgiving ocean.  Grateful, he rewards you for your act of kindness by giving you the %s.
    Your forces suffer a bitter defeat, and %s abandons your cause.
    Your hero has %d spell points remaining
    [%d: ac+mtf 
    bad MTF value %d
    buybuil%d.bin
    castbkg%c.icn
    castle%c.icn
    catsnd%02d.82M
    cmbtcap%c.icn
    cmbthro%c.icn
    covr%04d.icn
    cstl%s.icn
    ctrack%02d.icn
    evntwin%d.bin
    for a fee of %d gold.
    frng%04d.icn
    gi%d obtile%d obi%d ot%d ei%d bl%d %s X%d Y%d
    ground%d.icn
    guarded by %s %s
    herofl%02d.icn
    letter%d.icn
    loop%04d.82M
    magegld%c.icn
    misc%d.icn
    model %s:
    monh%04d.icn
    not mine %d
    on your %s!
    open %c: type cdaudio alias CD
    open %c: type cdaudio alias CD shareable
    pickup%02d.82M
    play CD from %d %s
    play CD from %d to %d%s
    play CD from %s %s
    play CD from %s to %d%s
    port%04d.icn
    townbkg%d.icn
    view%s.icn
    vwflag%d.icn
    wsnd%1d%1d.82M
    x_track%d.icn
    {%s Necromancy (+%d)}
    {Level %d}

## Other strings (7428)

      1) Make sure all cables are firmly connected.
      2) Reboot computer.
      3) Check to make sure you have the correct COM port setting in 'CONFIG'. (The 3rd button down on the screen where you choose Host or Guest.)
      4) Consider lowering the BAUD rate in 'CONFIG' to 19200 or 9600.
      Press 'CANCEL' to abort.
      Unfortunately, you do not have the wisdom to understand the spell, and you are unable to learn it.  
      Unfortunately, you have no Magic Book to record the spell with.
     !"#QEFGHIJW
     !"#QEFGHIJW@
     "I'm sorry sir," The leader of the soldiers says, "but you already know everything we have to teach."
     ..\build\intel\mt_obj\87disp.obj
     ..\build\intel\mt_obj\87sqrt.obj
     ..\build\intel\mt_obj\87tran.obj
     ..\build\intel\mt_obj\87trig.obj
     ..\build\intel\mt_obj\fpctrl.obj
     ..\build\intel\mt_obj\fpinit.obj
     ..\build\intel\mt_obj\ieee87.obj
     ..\build\intel\mt_obj\powhlp.obj
     ..\build\intel\mt_obj\tenpow.obj
     ??0fileRequester@@QAE@HHHPAD00@Z
     ??0swapManager@@QAE@PAVhero@@0@Z
     ??_C@_0N@FIAN@Domain?5error?$AA@
     ??_C@_0N@NGPA@Invalid?5seek?$AA@
     ?BuyBuild@townManager@@QAEHHHH@Z
     ?CDPlay@soundManager@@QAEXHHHH@Z
     ?CDShutdown@soundManager@@QAEXXZ
     ?CheckDimHero@advManager@@QAEXXZ
     ?ClearCellExtra@fullMap@@QAEXH@Z
     ?Close@heroWindowManager@@UAEXXZ
     ?ComboDraw@advManager@@QAEHHHH@Z
     ?ControlPanel@advManager@@QAEHXZ
     ?ConvertSmackerPalette@@YIXPAE@Z
     ?CreatureTypeCount@hero@@QAEHH@Z
     ?CreaturesToBuy@philAI@@QAEHHH@Z
     ?DataEntryWin@@3PAVheroWindow@@A
     ?DoBlast@combatManager@@QAEXHH@Z
     ?DoCompAI@combatManager@@QAEXH@Z
     ?DrawSwapWin@swapManager@@QAEHXZ
     ?EXPANSION_AGGREGATE_NAME@@3PADA
     ?Expunge@resourceManager@@QAEXXZ
     ?FindStringInString@@YIPADPAD0@Z
     ?FreeDecompressStructures@@YIXXZ
     ?FreeIcons@combatManager@@QAEXXZ
     ?GetCharacterWidth@font@@QAEHE@Z
     ?GetSoundId@advManager@@QAEHHH@Z
     ?GiveArtifact@@YIHPAVhero@@HHC@Z
     ?GrantAward@ExpCampaign@@QAEXH@Z
     ?HasSomeUndead@armyGroup@@QAEHXZ
     ?HideRoute@advManager@@QAEXHHH@Z
     ?InCastle@combatManager@@QAEHH@Z
     ?IsCompleted@ExpCampaign@@QAEEXZ
     ?IsSpecialUA@ExpCampaign@@QAECXZ
     ?IsWinner@combatManager@@QAEHH@Z
     ?LoadIcons@combatManager@@QAEXXZ
     ?LowerDoor@combatManager@@QAEXXZ
     ?MoveOrigin@advManager@@QAEXHH@Z
     ?MoveWindow@heroWindow@@QAEXHH@Z
     ?NewUpdate@mouseManager@@QAEXH@Z
     ?NormalDialog@@YIXPADHHHHHHHHH@Z
     ?OVScrollKnob@@3PAViconWidget@@A
     ?Open@heroWindowManager@@UAEHH@Z
     ?ProcessOnMapHeroes@game@@QAEXXZ
     ?RaiseDoor@combatManager@@QAEXXZ
     ?ResetAwards@ExpCampaign@@QAEXXZ
     ?ResetStrips@townManager@@QAEXXZ
     ?SCS@@3PAUSampleChannelStruct@@A
     ?SVSearchArray@@3VsearchArray@@A
     ?ShowCampaignInfo@game@@QAEXHH@Z
     ?ShowComputerScreen@game@@QAEXXZ
     ?ShowRoute@advManager@@QAEXHHH@Z
     ?ShowText@townManager@@QAEXPAD@Z
     ?StartCursor@advManager@@QAEXH@Z
     ?Update@highScoreManager@@QAEXXZ
     ?UpdateInfo@ExpCampaign@@QAEXH@Z
     ?UpdateSpellWidgets@game@@QAEXXZ
     ?UpgradeCreatures@hero@@QAEXHH@Z
     ?ValidAttack@army@@QAEHHHHHPAH@Z
     ?ViewWorld@advManager@@QAEXHHH@Z
     ?WaitEndSample@@YIXUSAMPLE2@@H@Z
     ?_query_new_handler@@YAP6AHI@ZXZ
     ?bsGetUChar@@YIEPAUBitStream@@@Z
     ?dumpModelStats@@YIXPAUModel@@@Z
     ?gNeutralBuildingCosts@@3PAY06HA
     ?gSetupNetworkGame2Help@@3PAPADA
     ?gSpecialBuildingCosts@@3PAY06HA
     ?gaiHeroEventStratRVOfPos@@3PAFA
     ?gbReturnAfterComputeExtent@@3HA
     ?gfSpellCastableCombatMod@@3PAMA
     ?gpAdvManager@@3PAVadvManager@@A
     ?gpBufferPalette@@3PAVpalette@@A
     ?iDPWaitForFirstGuestStatus@@3HA
     ?putUInt32@@YIXPAUBitStream@@I@Z
     ?unRLEandDump@@YIXPAU_iobuf@@H@Z
     ?xSetupCampaignGameHelp@@3PAPADA
     ?xSetupStandardGameHelp@@3PAPADA
     @ffF@
     Setup 1
     Setup 2
     Setup 3
     The soldiers living in the fort teach you a few new defensive tricks.
     Visions predicts the likely outcome of an encounter with a neutral army camp.
     __imp__AIL_redbook_track_info@16
     __imp__AIL_set_sample_address@12
     __imp__FreeEnvironmentStringsA@4
     __imp__FreeEnvironmentStringsW@4
     __imp__SetEnvironmentVariableA@8
     __imp__WaitForMultipleObjects@16
     a level.
     notify
    !..\build\intel\mt_obj\87cdisp.obj
    !..\build\intel\mt_obj\87csqrt.obj
    !..\build\intel\mt_obj\87ctran.obj
    !..\build\intel\mt_obj\87ctrig.obj
    !..\build\intel\mt_obj\87triga.obj
    !..\build\intel\mt_obj\mantold.obj
    !..\build\intel\mt_obj\matherr.obj
    !..\build\intel\mt_obj\x10fout.obj
    !.:HUcp
    !??0iconWidget@@QAE@FFFFPADFCFFF@Z
    !??0recruitUnit@@QAE@PAVtown@@HH@Z
    !??0textWidget@@QAE@FFFFPAD0FFFF@Z
    !??_C@_0N@NOAB@Expression?3?5?$AA@
    !??_C@_0O@KHHM@Improper?5link?$AA@
    !??_C@_0O@NOFB@Unknown?5error?$AA@
    !?ApplyBattleWinTemps@hero@@QAEXXZ
    !?Armageddon@combatManager@@QAEXXZ
    !?BitmapToScreen@@YIXPAVbitmap@@@Z
    !?CDIsPlaying@soundManager@@QAEHXZ
    !?CampaignInfoUpdate@game@@QAEXH@Z
    !?CanBuyBHC@philAI@@QAEHAAUBHC@@@Z
    !?CatAttack@combatManager@@QAEXH@Z
    !?CheckAnduranPieces@hero@@QAEXH@Z
    !?CheckValidAvailableHeroes@@YIXXZ
    !?CompleteDraw@advManager@@QAEXH@Z
    !?ComputeTradeRatios@@YIXHHPAH00@Z
    !?CopyTo@bitmap@@QAEXPAV1@HHHHHH@Z
    !?DimensionDoor@advManager@@QAEXXZ
    !?DimensionDoorTo@philAI@@QAEXHH@Z
    !?DoCommand@combatManager@@QAEXH@Z
    !?DoTradeKnob@@YIXUtag_message@@@Z
    !?DoVictory@combatManager@@QAEXH@Z
    !?DrawSelector@swapManager@@QAEXXZ
    !?DrawWindow@heroWindow@@QAEXHHH@Z
    !?Earthquake@combatManager@@QAEXXZ
    !?EnableButtons@advManager@@QAEXXZ
    !?Fireball@combatManager@@QAEXHH@Z
    !?FizzleCenter@advManager@@QAEXH@Z
    !?ForceNewHover@advManager@@QAEXXZ
    !?FreeArmies@combatManager@@QAEXXZ
    !?FutureDeflator@philAI@@QAEMQAH@Z
    !?GetArmyStrengths@hero@@QAEXQAK@Z
    !?GetBestDirection@army@@QAEHHHH@Z
    !?GetControl@combatManager@@QAEXXZ
    !?GetOddsOfWinning@philAI@@QAEMH@Z
    !?GetRandomNumTroops@game@@QAEHH@Z
    !?GiveSpells@town@@QAEXPAVhero@@@Z
    !?GrabBitmap@bitmap@@QAEXPAV1@FF@Z
    !?HasMobileHero@playerData@@QAEHXZ
    !?InitEntireCampaign@game@@QAEXH@Z
    !?InitRandomArtifacts@game@@QAEXXZ
    !?InitializePasswords@game@@QAEXXZ
    !?IsHomogeneous@armyGroup@@QAEHH@Z
    !?JosephName@ExpCampaign@@QAEPADXZ
    !?LoadArmies@combatManager@@QAEXXZ
    !?MIDIStartup@soundManager@@QAEXXZ
    !?MouseMessageHandler@@YIHPAXIIJ@Z
    !?NextPos@combatManager@@QAEXPAH@Z
    !?OtherArmyAdjacent@army@@QAEHHH@Z
    !?ProcessIconSelect@game@@QAEHHH@Z
    !?ReadByte@resourceManager@@QAECXZ
    !?ReadLong@resourceManager@@QAEJXZ
    !?ReadWord@resourceManager@@QAEFXZ
    !?ReceiveSaveGame@game@@QAEHHHHH@Z
    !?RemoveAward@ExpCampaign@@QAEXH@Z
    !?ResetMouse@combatManager@@QAEXXZ
    !?ResetRound@combatManager@@QAEXXZ
    !?SaveAndDraw@mouseManager@@QAEXXZ
    !?SetPointer@mouseManager@@QAEXH@Z
    !?SetSpellInfluence@army@@QAEHHH@Z
    !?SetupFiles@fileRequester@@QAEXXZ
    !?SetupNetPlayerNames@game@@QAEXXZ
    !?SetupPuzzlePieces@game@@QAEHHH@Z
    !?ShutDownSystem@executive@@QAEXXZ
    !?StopOnTrigger@@YIEPAVmapCell@@@Z
    !?SystemOptions@advManager@@QAEXXZ
    !?TransmitSaveGame@game@@QAEHHHH@Z
    !?TurnCostResource@philAI@@QAEXH@Z
    !?UpdateAppSpecificMenus@@YIXPAX@Z
    !?UpdateNewGameWindow@game@@QAEXXZ
    !?UpdateRadar@advManager@@QAEXHH@Z
    !?aBitStreamBuffer@@3UBitStream@@A
    !?bsPutUChar@@YIXPAUBitStream@@E@Z
    !?gCurLoadedSpellIcon@@3PAVicon@@A
    !?gInitResourcesComputer@@3PAY06HA
    !?giBottomViewOverrideEndTime@@3HA
    !?giOverviewReturnActionExtra@@3HA
    !?initModel@@YIXPAUModel@@PADHHH@Z
    !?sendMTFVal@@YIXPAUBitStream@@H@Z
    !?sendZeroes@@YIXPAUBitStream@@H@Z
    !?skillWidget@@3PAPAViconWidget@@A
    !?townPortalWin@@3PAVheroWindow@@A
    !?xIsPlayingExpansionCampaign@@3EA
    !?xScenarioDescription@@3PAY07PADA
    !This program cannot be run in DOS mode.
    !__imp__GetSystemPaletteEntries@16
    !__imp__SmackSoundUseDirectSound@4
    !__imp__UnhandledExceptionFilter@4
    "..\build\intel\mt_obj\87ctriga.obj
    "..\build\intel\mt_obj\87except.obj
    "..\build\intel\mt_obj\adj_fdiv.obj
    "..\build\intel\mt_obj\constpow.obj
    "..\build\intel\mt_obj\fpexcept.obj
    "..\build\intel\mt_obj\ieeemisc.obj
    "..\build\intel\mt_obj\intrncvt.obj
    "..\build\intel\mt_obj\strgtold.obj
    "..\build\intel\mt_obj\testfdiv.obj
    "0FSab
    "?AddScoreToHighScore@@YIHHHHHPAD@Z
    "?ApplyBattleLossTemps@hero@@QAEXXZ
    "?CalcDifficultyRating@game@@QAEHXZ
    "?CancelComputerScreen@game@@QAEXXZ
    "?CheckCastSpell@advManager@@QAEXXZ
    "?CheckHeroConsistency@game@@QAEXXZ
    "?CleanUpData@fileRequester@@QAEXXZ
    "?CleanUpNewGameWindow@game@@QAEXXZ
    "?DecrementSpellRounds@army@@QAEXXZ
    "?DeleteItem@listBoxWidget@@QAEXH@Z
    "?DisableButtons@advManager@@QAEXXZ
    "?DoAdvance@@YIXPAUSmackTag@@HHHH@Z
    "?DoNetCombat@advManager@@QAEHPAD@Z
    "?DoSpellAI@combatManager@@QAEHHH@Z
    "?DoSurrender@combatManager@@QAEHXZ
    "?DrawCell@advManager@@QAEXHHHHHH@Z
    "?GenerateMap@combatManager@@QAEXXZ
    "?GetAllMask@combatManager@@QAEHH@Z
    "?GetCommand@combatManager@@QAEHH@Z
    "?GetNumThievesGuilds@game@@QAEHH@Z
    "?HandleVictory@ExpCampaign@@QAEHXZ
    "?IncrementHourGlass@philAI@@QAEXXZ
    "?KeepAttack@combatManager@@QAEXH@Z
    "?MIDIShutdown@soundManager@@QAEXXZ
    "?MakeAllWaterVisible@game@@QAEXH@Z
    "?MusicPlaying@soundManager@@QAEHXZ
    "?PointInRect@@YICHHPAUtag_rect@@@Z
    "?ProcessHover@advManager@@QAEHHH@Z
    "?ProcessRandomObjects@game@@QAEXXZ
    "?PuzzleDraw@advManager@@QAEXHHHH@Z
    "?QuickViewRecruit@@YIXPAVtown@@H@Z
    "?Read13@resourceManager@@QAEXPAC@Z
    "?RecruitHero@townManager@@QAEHHH@Z
    "?ReplaySmacker@ExpCampaign@@QAEXXZ
    "?RightClick@combatManager@@QAEHH@Z
    "?SaveBackground@heroWindow@@QAEHXZ
    "?ScreenScroll@advManager@@QAEXHH@Z
    "?ServiceSound@soundManager@@QAEXXZ
    "?SetColorIndex@textWidget@@QAEXF@Z
    "?SetWinText@@YIXPAVheroWindow@@H@Z
    "?SetupDynamicStuff@game@@QAEXHHH@Z
    "?SetupMultiPlayerGame@game@@QAEHXZ
    "?SpellCastWorkChance@army@@QAEMH@Z
    "?SwapArtifacts@swapManager@@QAEXXZ
    "?UpdateCombatSystemOptions@@YIXH@Z
    "?UpdateScreen@advManager@@QAEXHH@Z
    "?VWCompleteDraw@advManager@@QAEXXZ
    "?ViewSpells@combatManager@@QAEHH@Z
    "?allocateCompressStructures@@YIXXZ
    "?cCampaignDescription@@3PAY0M@PADA
    "?compressStream@@YIXPAU_iobuf@@0@Z
    "?doReversibleTransformation@@YIXXZ
    "?gNeutralBaseResourceValues@@3PAHA
    "?gbLimitedCombatUpdatePalette@@3HA
    "?gbThisNetGotAdventureControl@@3HA
    "?gpSearchArray@@3PAVsearchArray@@A
    "?gpTownManager@@3PAVtownManager@@A
    "?set_new_handler@@YAP6AXXZP6AXXZ@Z
    "Go 'way!", the witch doctor barks, "you know all I know."
    "I have a riddle for you," the Sphinx says.  "Answer correctly, and you shall be rewarded.  Answer incorrectly, and you shall be eaten.  Do you accept the challenge?"
    "The Dwarfbane!!!!, run for your lives."
    "You guessed incorrectly," the Sphinx says, smiling.  The Sphinx swipes at you with a paw, knocking you to the ground.  Another blow makes the world go black, and you know no more.
    "__imp__AIL_set_sample_loop_count@8
    "__imp__InitializeCriticalSection@4
    #??0bankBox@@QAE@HHPAVplayerData@@@Z"
    #??_C@_0BA@GILI@GetActiveWindow?$AA@
    #??_C@_0P@GFAI@No?5such?5device?$AA@
    #??_C@_0P@GGKG@runtime?5error?5?$AA@2
    #??_C@_0P@NDNC@Is?5a?5directory?$AA@
    #??_C@_0P@OFDM@Too?5many?5links?$AA@
    #??_C@_0P@OPNK@File?5too?5large?$AA@"
    #?BuildHero@philAI@@QAEXPAVtown@@H@Z"
    #?BuildPath@searchArray@@QAEHHHHHH@Z
    #?CDSetVolume@soundManager@@QAEXHH@Z*
    #?ClearBottomView@advManager@@QAEXXZ"
    #?ClearEffects@combatManager@@QAEXXZ2
    #?DeleteItem@dropListWidget@@QAEXH@Z
    #?DoLoseWindow@combatManager@@QAEXXZ
    #?DrawLBStuff@listBoxWidget@@QAEXH@Z
    #?FindIndex@@YIHPAUindexArray@@HHH@Z6
    #?GetBestBHC@philAI@@QAEXHAAUBHC@@@Z&
    #?GetMapEvent@@YIPAUEventExtra@@HH@Z*
    #?GetNextArmy@combatManager@@QAEHH@Z
    #?GetPointer@combatManager@@QAEHHH@Z
    #?GetRandomArtifactId@game@@QAEHHH@Z
    #?HandleRemoteDeadPlayerExit@@YIXH@Z"
    #?HandleVictory1@ExpCampaign@@QAEXXZ
    #?HandleVictory2@ExpCampaign@@QAEXXZ
    #?HandleVictory3@ExpCampaign@@QAEXXZ
    #?HandleVictory4@ExpCampaign@@QAEXXZ
    #?HeroHandler@@YIHAAUtag_message@@@Z&
    #?InsertSound@advManager@@QAEXHHHH@Z"
    #?MIDIIsPlaying@soundManager@@QAEHXZ*
    #?MIDISetVolume@soundManager@@QAEXXZ2
    #?MakeId@resourceManager@@QAEKPADH@Z
    #?MirrorImage@combatManager@@QAEXH@Z&
    #?NullHandler@@YIHAAUtag_message@@@Z.
    #?PrintManagerList@executive@@QAEXXZ"
    #?ProcessSearch@advManager@@QAEHHH@Z&
    #?ReplaySmacker1@ExpCampaign@@QAEXXZ
    #?ReplaySmacker2@ExpCampaign@@QAEXXZ2
    #?ReplaySmacker3@ExpCampaign@@QAEXXZ:
    #?ReplaySmacker4@ExpCampaign@@QAEXXZ.
    #?Resurrect@combatManager@@QAEXHHH@Z&
    #?SetColorMice@mouseManager@@QAEXH@Z
    #?SetRandomHeroArmies@game@@QAEXHH@Z*
    #?SetTownContext@advManager@@QAEXH@ZJ
    #?SmackOptions@@3PAUSSmackOptions@@A
    #?TransmitAndWait@@YIHPADHHCCPAPAD@Z
    #?TransmitRemoteData@@YIHPADHHCCCC@Z.
    #?TurnValueOfObelisk@philAI@@QAEMH@Z"
    #?UpdateGrid@combatManager@@QAEHHH@Z
    #?WaitHandler@@YIHAAUtag_message@@@Z6
    #?WriteDiffHeaderInfo@@YIXEHPAEPAH@Z"
    #?gSetupMultiPlayerGameHelp@@3PAPADA
    #?gbSetupGamePosToRealGamePos@@3PACA
    #?loadAndRLEsource@@YIHPAU_iobuf@@@Z*
    #?lpDDPal@@3PAUIDirectDrawPalette@@A
    #?mySIGSEGVorSIGBUScatcher@@YIXPAH@Z
    #?sMapChangeQueue@@3PAUSMapChange@@A
    #__imp__AIL_allocate_sample_handle@4>
    $??_C@_0BA@LPOH@Resource?5device?$AA@
    $??_C@_0BB@JPOE@GetCurrentThread?$AA@
    $??_C@_0N@OMLL@SING?5error?$AN?6?$AA@
    $?AddArmy@combatManager@@QAEXHHHHHH@Z
    $?BlitBitmap@@YIXPAVbitmap@@HHHH0HH@Z
    $?CancelIndividualSpell@army@@QAEXH@Z
    $?CastSpell@combatManager@@QAEXHHHH@Z
    $?CheckHandleNet@advManager@@QAEPADXZ
    $?CheckLoadSample@advManager@@QAEXH@Z
    $?CompleteDraw@advManager@@QAEXHHHH@Z
    $?DamageEnemy@army@@QAEXPAV1@PAH1HH@Z
    $?DefaultSpell@combatManager@@QAEXH@Z
    $?DrawCursorShadow@advManager@@QAEXXZ
    $?DrawNGKPDisplayString@game@@QAEXH@Z
    $?ForceMouseMove@inputManager@@QAEXXZ
    $?ForcePollSound@soundManager@@QAEXXZ
    $?GenerateStandardFileName@@YIXPAD0@Z
    $?GetAdjacentCellIndex@army@@QAEHHH@Z
    $?GetAdjacentCellIndexNoArmy@@YIHHH@Z
    $?GetBestArmy@combatManager@@QAEHHH@Z
    $?GetCloudLookup@advManager@@QAEHHH@Z
    $?GetFilename@fileRequester@@QAEPADXZ
    $?GetFlyerMask@combatManager@@QAEHH@Z
    $?GetStrength@combatManager@@QAEKHH@Z
    $?KeyboardMessageHandler@@YIHPAXIIJ@Z
    $?LoadPlaySample@@YI?AUSAMPLE2@@PAD@Z
    $?Main@border@@UAEHAAUtag_message@@@Z
    $?Main@button@@UAEHAAUtag_message@@@Z
    $?Main@widget@@UAEHAAUtag_message@@@Z
    $?MaxBuyableCreatures@philAI@@QAEHH@Z
    $?MeteorShower@combatManager@@QAEXH@Z
    $?MoreTreesNear@combatManager@@QAEHXZ
    $?Open@widget@@QAEHHPAVheroWindow@@@Z
    $?ResetMapChoices@ExpCampaign@@QAEXXZ
    $?ResetMapsPlayed@ExpCampaign@@QAEXXZ
    $?SetArmyCommand@townManager@@QAEXH@Z
    $?SetHeroContext@advManager@@QAEXHH@Z
    $?SetMapWasPlayed@ExpCampaign@@QAEXXZ
    $?SetupExtraStuff@townManager@@QAEXXZ
    $?SetupNewOverviewType@game@@QAEXHH@Z
    $?ShiftQualChange@townManager@@QAEXXZ
    $?ShowLuckInfo@game@@QAEXPAVhero@@H@Z
    $?TestRaiseDoor@combatManager@@QAEXXZ
    $?UpdBottomView@advManager@@QAEXHHH@Z
    $?ValueOfTown@philAI@@QAEHPAVtown@@@Z
    $?ViewBallista@combatManager@@QAEXH@Z
    $?cRumourTerrainDescriptions@@3PAPADA
    $?gpInputManager@@3PAVinputManager@@A
    $?gpMouseManager@@3PAVmouseManager@@A
    $?gpSoundManager@@3PAVsoundManager@@A
    $?init_anchor@@YIXPAUtag_Anchor@@HH@Z
    $?lpDDSOne@@3PAUIDirectDrawSurface@@A
    $?setDecompressStructureSizes@@YIXH@Z
    $?uncompressStream@@YIHPAU_iobuf@@0@Z
    $?undoReversibleTransformation@@YIXXZ
    $PQj%j
    $__imp__AIL_release_sequence_handle@4
    $__imp__AIL_set_XMIDI_master_volume@8
    $__imp__AIL_set_sequence_loop_count@8
    %??_C@_0BA@BPKN@No?5such?5process?$AA@
    %??_C@_0BA@JMMO@Not?5a?5directory?$AA@
    %??_C@_0BB@IHAB@Invalid?5argument?$AA@
    %??_C@_0BC@CFOG@GetCurrentProcess?$AA@
    %??_C@_0L@ODMK@new_p?5?$DN?$DN?50?$AA@
    %??_C@_0O@DELO@TLOSS?5error?$AN?6?$AA@
    %?AICheckRetreat@combatManager@@QAEHXZ
    %?APanelHandler@@YIHAAUtag_message@@@Z
    %?BuildingsOwned@playerData@@QAEHHHH@Z
    %?CPanelHandler@@YIHAAUtag_message@@@Z
    %?CalcNumLevelArchers@town@@QAEXPAH0@Z
    %?CastleHandler@@YIHAAUtag_message@@@Z
    %?CheckGetAIMove@combatManager@@QAEXXZ
    %?CheckScreenScroll@advManager@@QAEXXZ
    %?CombatMessage@combatManager@@QAEXH@Z
    %?ComputeUpgradeValue@philAI@@QAEHHH@Z
    %?ConvertVolume@soundManager@@QAEHHH@Z
    %?DimBitmapArea@@YIXPAVbitmap@@HHHHH@Z
    %?DoAllHeroInteractions@philAI@@QAEXXZ
    %?DrawBackground@combatManager@@QAEXXZ
    %?DrawDropStuff@dropListWidget@@QAEXXZ
    %?DrawToBufferCareful@bitmap@@QAEXFF@Z
    %?ElementalStorm@combatManager@@QAEXXZ
    %?FirstArmy@combatManager@@QAEHHHPAH@Z
    %?GetFileSize@resourceManager@@QAEKK@Z
    %?GetGridIndex@combatManager@@QAEHHH@Z
    %?GetLossConditionText@game@@QAEXPAD@Z
    %?GetNewCellExtraIndex@fullMap@@QAEHXZ
    %?GetOccupiedTown@hero@@QAEPAVtown@@XZ
    %?GetWalkerMask@combatManager@@QAEHH@Z
    %?GetWorstArmy@combatManager@@QAEHHH@Z
    %?HeroQuickView@advManager@@QAEXHHHH@Z
    %?InitNewCampaign@ExpCampaign@@QAEXH@Z
    %?MapExtraPosAndAdjacentsSet@@YIHHHE@Z
    %?MobilizeCurrHero@advManager@@QAEXH@Z
    %?MouseCoords@mouseManager@@QAEXAAH0@Z
    %?MouseInScrollZone@advManager@@QAEHXZ
    %?PointToFile@resourceManager@@QAEXK@Z
    %?RedrawAdvScreen@advManager@@QAEXHH@Z
    %?RedrawTownScreen@townManager@@QAEXXZ
    %?RestoreBackground@heroWindow@@QAEXXZ
    %?SavePosition@resourceManager@@QAEXXZ
    %?ScreenShot@heroWindowManager@@QAEXXZ
    %?SetKeyCodeType@inputManager@@QAEXH@Z
    %?SetPointer@mouseManager@@QAEXPADHH@Z
    %?SetupSmallView@combatManager@@QAEXXZ
    %?SpellMessage@combatManager@@QAEXHH@Z
    %?StopAllSamples@soundManager@@QAEXH@Z
    %?TavernHandler@@YIHAAUtag_message@@@Z
    %?TownQuickView@advManager@@QAEXHHHH@Z
    %?UpdBottomViewHero@advManager@@QAEHXZ
    %?ViewGeneral@combatManager@@QAEHHHH@Z
    %?_set_new_handler@@YAP6AHI@ZP6AHI@Z@Z
    %?lpClipper@@3PAUIDirectDrawClipper@@A
    %?normalDirTable@@3PAUtag_tilePoint@@A
    %?sCmbtObstacles@@3PAUSCmbtObstacle@@A
    %?sMapChangeLastFew@@3PAUSMapChange@@A
    %?textWidgetTitle@@3PAPAVtextWidget@@A
    %__imp__AIL_allocate_sequence_handle@4
    %__imp__AIL_set_sample_playback_rate@8
    %l&mmn
    %lmmnnoo
    %mmnoopqpp
    &??0strip@@QAE@HHHKHPAVarmyGroup@@HHH@Z
    &??_C@_0BB@EALI@Result?5too?5large?$AA@
    &??_C@_0BB@KDDC@Not?5enough?5space?$AA@
    &??_C@_0BC@DJON@Permission?5denied?$AA@
    &??_C@_0BD@NJFP@GetLastActivePopup?$AA@
    &??_C@_0P@OJAK@DOMAIN?5error?$AN?6?$AA@
    &?CastMassSpell@combatManager@@QAEXHH@Z
    &?ConvertObject@game@@QAEXHHHHHHHHHHH@Z
    &?DeactivateCurrHero@advManager@@QAEXXZ
    &?DeactivateCurrTown@advManager@@QAEXXZ
    &?DemobilizeCurrHero@advManager@@QAEXXZ
    &?DoVisions@advManager@@QAEHPAVhero@@@Z
    &?DrawSmallView@combatManager@@QAEXHH@Z
    &?DrawUpperDeadOccupant@hexcell@@QAEXXZ
    &?FillBitmapArea@@YIXPAVbitmap@@HHHHH@Z
    &?GetGameAttentionValue@philAI@@QAEXH@Z
    &?GetMapHeader@@YIHPADPAUSMapHeader@@@Z
    &?GetOutOfItMask@combatManager@@QAEHH@Z
    &?GetShooterMask@combatManager@@QAEHH@Z
    &?GetTraitorMask@combatManager@@QAEHH@Z
    &?GetTurnAttentionValue@philAI@@QAEXH@Z
    &?GiveTroopsToNeutralTown@game@@QAEXH@Z
    &?GiveTroopsToNeutralTowns@game@@QAEXXZ
    &?HandleAppSpecificMenuCommands@@YIHH@Z
    &?HeroLoses@advManager@@QAEXPAVhero@@@Z
    &?HeroSwap@advManager@@QAEXPAVhero@@0@Z
    &?HideColorPointer@mouseManager@@QAEXXZ
    &?NetValueOfArtifact@philAI@@QAEHHHHH@Z
    &?NewGameHandler@@YIHAAUtag_message@@@Z
    &?PlayPreScenarioSmacker@game@@QAEXHH@Z
    &?QuickDistance@searchArray@@QAEHHHHH@Z
    &?RVOfPosition@philAI@@QAEHHHHHHHHHHH@Z
    &?ReadBlock@resourceManager@@QAEXPACK@Z
    &?ResetBonusChoices@ExpCampaign@@QAEXXZ
    &?Select@button@@QAEFAAUtag_message@@@Z
    &?SetMouseCoords@inputManager@@QAEXHH@Z
    &?SetMusicQuality@soundManager@@QAEXH@Z
    &?ShowColorPointer@mouseManager@@QAEXXZ
    &?ShowMoraleInfo@game@@QAEXPAVhero@@H@Z
    &?TrimLoopingSounds@advManager@@QAEXH@Z
    &?gpCombatManager@@3PAVcombatManager@@A
    &?outputHandleJustInCase@@3PAU_iobuf@@A
    '??_C@_0BC@BICL@Exec?5format?5error?$AA@2
    '??_C@_0BC@GADJ@Filename?5too?5long?$AA@"
    '?BuildBuilding@philAI@@QAEXPAVtown@@H@Z*
    '?CampaignHandler@@YIHAAUtag_message@@@Z
    '?ChainLightning@combatManager@@QAEXHH@Z*
    '?CheckAdjacentMon@advManager@@QAEXPAH@Z"
    '?CheckDimNextHeroBut@advManager@@QAEXXZ&
    '?ClipFillToBuffer@icon@@QAEXHHHHHHHHH@Z.
    '?ConvertAllToLateOverlay@game@@QAEXHH@Z
    '?DrawAdventureBorder@advManager@@QAEXXZ:
    '?DrawCombatBorder@combatManager@@QAEXXZR
    '?DrawFrame@combatManager@@QAEXHHHHHHH@Z
    '?DrawLowerDeadOccupants@hexcell@@QAEXXZ.
    '?EvaluateArtifactEvent@philAI@@QAEHHH@Z
    '?EvaluateMineEvent@philAI@@QAEHHHHPAH@Z
    '?GetArmySizeName@advManager@@QAEPADHH@Z"
    '?GetBuildingBaseResourceValue@@YIHHHH@Z
    '?GetCursorBaseFrame@advManager@@QAEHH@Z"
    '?GetCursorSampleSet@advManager@@QAEXH@Z"
    '?GetNumberCDDrives@soundManager@@QAEXXZ2
    '?GiveArmy@game@@QAEXPAVarmyGroup@@HHH@Z
    '?GiveTakeArtifactStat@@YIXPAVhero@@HH@Z
    '?HandleCastSpell@@YIHAAUtag_message@@@Z
    '?HasObjectTilesetIndex@game@@QAEHHHHH@Z:
    '?InitMenuHandler@@YIHAAUtag_message@@@Z
    '?IsThisMapCompleted@ExpCampaign@@QAECXZ&
    '?MakeScanCodeTable@inputManager@@QAEXXZ
    '?NumOfGivenArtifact@playerData@@QAEHH@Z
    '?OverviewHandler@@YIHAAUtag_message@@@Z
    '?PurgeMapChangeQueue@advManager@@QAEXXZ&
    '?ReallyHidePointer@mouseManager@@QAEXXZF
    '?ReallyShowPointer@mouseManager@@QAEXXZ&
    '?RedrawScreen@heroWindowManager@@QAEXXZ.
    '?ResetCycleTimers@combatManager@@QAEXXZ>
    '?RestoreUnderlying@mouseManager@@QAEXXZ
    '?SaveAdventureBorder@advManager@@QAEXXZ"
    '?SaveCombatBorder@combatManager@@QAEXXZ&
    '?SetCombatGrid@combatManager@@QAEXHHH@Z.
    '?SetInitialMapOrigin@advManager@@QAEXXZ2
    '?UpdBottomViewResMsg@advManager@@QAEHXZ*
    '?UpdateArmyGroup@combatManager@@QAEXH@Z2
    '?UpdateCombatArea@combatManager@@QAEXXZ6
    '?UpdateScreen@heroWindowManager@@QAEXXZ&
    '?ViewArmyHandler@@YIHAAUtag_message@@@Z*
    '?gDwellingBaseResourceValues@@3PAY0M@HA
    '?gsNetPlayerInfo@@3PAUSNetPlayerInfo@@A&
    '?iconWidgetDynamic@@3PAPAViconWidget@@A&
    '?pNormalDialogWindow@@3PAVheroWindow@@A
    '?textWidgetDynamic@@3PAPAVtextWidget@@A*
    'Grouped' combat formation bunches your army together in the center of your side of the battlefield.
    'Resurrects' creatures from a damaged or dead undead unit permanently.
    'Spread' combat formation spreads your armies from the top to the bottom of the battlefield, with at least one empty space between each army.
    (+1 Attack)}
    (+1 Defense)}
    (+12 Attack)}
    (+12 Defense)}
    (+12 Knowledge)}
    (+12 Spell Power)}
    (+2 Attack)}
    (+2 Defense)}
    (+2 Knowledge)}
    (+2 Spell Power)}
    (+3 Attack)}
    (+3 Defense)}
    (+3 Knowledge)}
    (+3 Spell Power)}
    (+4 Knowledge)}
    (+4 Spell Power)}
    (+5 Knowledge)}
    (??0recruitUnit@@QAE@PAVarmyGroup@@HPAF@Z
    (??_C@_0BC@IGMC@Arg?5list?5too?5long?$AA@
    (??_C@_0BD@CEML@No?5locks?5available?$AA@
    (??_C@_0BD@MPKK@Input?1output?5error?$AA@
    (??_C@_0BD@ODIH@No?5child?5processes?$AA@
    (?AdjustMusicVolumes@soundManager@@QAEXXZ
    (?AdjustSoundVolumes@soundManager@@QAEXXZ
    (?BaseSetupHandler@@YIHAAUtag_message@@@Z
    (?BuildCreature@philAI@@QAEXPAVtown@@HH@Z
    (?CheckCastleAttack@combatManager@@QAEXXZ
    (?CheckForCreatureUpgrades@philAI@@QAEXXZ
    (?ChooseGoldOrExperience@philAI@@QAEHHH@Z
    (?ChooseToPayRansomOnHero@philAI@@QAEHH@Z
    (?ConvertFlagToLateOverlay@game@@QAEXHH@Z
    (?CopyToCareful@bitmap@@QAEXPAV1@HHHHHH@Z
    (?CreaturesToBuy@philAI@@QAEHPAVtown@@H@Z
    (?CycleCombatScreen@combatManager@@QAEXXZ
    (?Deselect@button@@QAEFAAUtag_message@@@Z
    (?DoAnywhereDDoorTownGate@philAI@@QAEHH@Z
    (?DoDimensionDoor@philAI@@QAEHPAVhero@@@Z
    (?DoWhirlpool@advManager@@QAEXPAVhero@@@Z
    (?DrawBoundedString@font@@QAEXPADHHHHHH@Z
    (?EvaluateHeroEvent@philAI@@QAEHHHHHPAH@Z
    (?EvaluateJail@philAI@@QAEHPAVmapCell@@@Z
    (?EvaluateTownEvent@philAI@@QAEHHHHHPAH@Z
    (?GetCell@advManager@@QAEPAVmapCell@@HH@Z
    (?GetVictoryConditionText@game@@QAEXPAD@Z
    (?GetWorldMapData@game@@QAEPAVfullMap@@XZ
    (?GrabBitmapCareful@bitmap@@QAEXPAV1@FF@Z
    (?InitNewGame@game@@QAEXPAUSMapHeader@@@Z
    (?InitNonVisualVars@combatManager@@QAEXXZ
    (?MageGuildHandler@@YIHAAUtag_message@@@Z
    (?Main@advManager@@UAEHAAUtag_message@@@Z
    (?Main@iconWidget@@UAEHAAUtag_message@@@Z
    (?Main@textWidget@@UAEHAAUtag_message@@@Z
    (?NGKPSetupDisplayString@game@@QAEXPADG@Z
    (?PrintSummaryInfo@@YIXPAUSmackSumTag@@@Z
    (?PushCombatPoint@searchArray@@QAEXHHHH@Z
    (?RestorePosition@resourceManager@@QAEXXZ
    (?RippleCreature@combatManager@@QAEXHHH@Z
    (?SetupBaudHandler@@YIHAAUtag_message@@@Z
    (?SetupGameHandler@@YIHAAUtag_message@@@Z
    (?SplitArmyHandler@@YIHAAUtag_message@@@Z
    (?SummonElemental@combatManager@@QAEXHH@Z
    (?UpdBottomViewKingdom@advManager@@QAEHXZ
    (?UpdBottomViewNewTurn@advManager@@QAEHXZ
    (?UpdateHeroLocator@advManager@@QAEXHHH@Z
    (?UpdateHeroLocators@advManager@@QAEXHH@Z
    (?UpdateMouseGrid@combatManager@@QAEXHH@Z
    (?UpdateTownLocators@advManager@@QAEXHH@Z
    (?WinCombatHandler@@YIHAAUtag_message@@@Z
    (?lpDDSPrimary@@3PAUIDirectDrawSurface@@A
    (Cheater)
    (Note: Some older computers do not handle CD Stereo music well.  If you have a computer that leaves the music playing when you quit or crashes when you switch music tracks, try MIDI music.)
    (Note: When using CD Stereo music in DOS, the music may only be turned on or off - the level has no effect.)
    (Press Retry to debug the application - JIT must be enabled)
    (already visited)
    (not visited)
    (null)
    )??_C@_0BC@OIMC@Assertion?5failed?$CB?$AA@
    )??_C@_0BE@CDDH@Bad?5file?5descriptor?$AA@
    )??_C@_0BE@FLBN@Directory?5not?5empty?$AA@
    )??_C@_0BG@PMJE@SetThreadAffinityMask?$AA@
    )?AddWidget@heroWindow@@QAEXPAVwidget@@H@Z
    )?CheckUpdateMousePos@mouseManager@@QAEXXZ
    )?DoEvent@advManager@@QAEXPAVmapCell@@HH@Z
    )?DrawStringExecute@font@@IAEXPADHHHHHHH@Z
    )?EvaluateMonsterEvent@philAI@@QAEHHHPAH@Z
    )?EventWindow@advManager@@QAEXHHPADHHHHH@Z
    )?ExpStdGameHandler@@YIHAAUtag_message@@@Z
    )?GetIcon@resourceManager@@QAEPAVicon@@K@Z
    )?HandleViewGeneral@@YIHAAUtag_message@@@Z
    )?IsSpecialGoldenBow@ExpCampaign@@QAECHH@Z
    )?Main@recruitUnit@@UAEHAAUtag_message@@@Z
    )?Main@swapManager@@UAEHAAUtag_message@@@Z
    )?Main@townManager@@UAEHAAUtag_message@@@Z
    )?NoShowCombatLog@combatManager@@QAEXPAD@Z
    )?PlayAmbientMusic@soundManager@@QAEXHJH@Z
    )?ResetHitByCreature@combatManager@@QAEXXZ
    )?ResetLimitCreature@combatManager@@QAEXXZ
    )?SwitchAmbientMusic@soundManager@@QAEXH@Z
    )?TownPortalHandler@@YIHAAUtag_message@@@Z
    )?ValidHexToStandOn@combatManager@@QAEHH@Z
    )?ValidSpellTarget@combatManager@@QAEHHH@Z
    )?VaporizeCreature@combatManager@@QAEXHH@Z
    )?ViewArmy@combatManager@@QAEXPAVarmy@@H@Z
    )?ViewSpellsHandler@@YIHAAUtag_message@@@Z
    )?gMonsterDatabase@@3PAUtag_monsterInfo@@A
    )?getSymbol@@YIHPAUModel@@PAUBitStream@@@Z
    )?gsThisNetPlayerInfo@@3USNetPlayerInfo@@A
    )?sBuildingInfo@@3PAY0CA@USBuildingInfo@@A
    ***Command Line Help***
    *-UVWWW
    *??_C@_0BE@COOK@Too?5many?5open?5files?$AA@
    *??_C@_0BH@KAJB@GetProcessAffinityMask?$AA@
    *?ArenaWindowHandler@@YIHAAUtag_message@@@Z
    *?CheckChangeSelector@combatManager@@QAEXXZ
    *?CombatMessage@combatManager@@QAEXPADHHH@Z
    *?CombatSystemOptions@combatManager@@QAEXXZ
    *?DoLichShot@combatManager@@QAEXPAVarmy@@@Z
    *?DrawBolt@combatManager@@QAEXPAUSBolt@@H@Z
    *?EraseObj@advManager@@QAEXPAVmapCell@@HH@Z
    *?EventWindowHandler@@YIHAAUtag_message@@@Z
    *?GetBackgroundName@combatManager@@QAEPADXZ
    *?GetMirrorImageMask@combatManager@@QAEHH@Z
    *?Main@dimmerWidget@@UAEHAAUtag_message@@@Z
    *?Main@inputManager@@UAEHAAUtag_message@@@Z
    *?Main@mouseManager@@UAEHAAUtag_message@@@Z
    *?Main@soundManager@@UAEHAAUtag_message@@@Z
    *?MakeCreaturesVanish@combatManager@@QAEXXZ
    *?ManaRefreshValue@philAI@@QAEHPAVhero@@H@Z
    *?MapExistsForFilter@fileRequester@@QAEHH@Z
    *?ModifyFrameInfo@@YIXPAUSMonFrameInfo@@H@Z
    *?ProcessNewMap@game@@QAEXPAUSMapHeader@@@Z
    *?PushPoint@searchArray@@QAEXHHHHHHHHHHHH@Z
    *?RandomizeBarrier@game@@QAEXPAVmapCell@@@Z
    *?RecruitHeroHandler@@YIHAAUtag_message@@@Z
    *?RemoveAndDeleteWidget@heroWindow@@QAEXH@Z
    *?RestoreCell@game@@QAEXHHHHPAVmapCell@@H@Z
    *?SaveDropBackground@dropListWidget@@QAEXXZ
    *?SetupAdjacencyArray@combatManager@@QAEXXZ
    *?SetupRelativeHeroStrengths@philAI@@QAEXXZ
    *?TradingPostHandler@@YIHAAUtag_message@@@Z
    *?UnwindMapChangeQueue@advManager@@QAEXHH@Z
    *?UpdBottomViewEnemyTurn@advManager@@QAEHXZ
    *?ViewSpecialHandler@@YIHAAUtag_message@@@Z
    *?gSpecialBuildingBaseResourceValues@@3PAHA
    *?gpResourceManager@@3PAVresourceManager@@A
    *?gpWindowManager@@3PAVheroWindowManager@@A
    *?putSymbol@@YIXPAUModel@@HPAUBitStream@@@Z
    *?sViewArmyMonFrameInfo@@3USMonFrameInfo@@A
    +??0textEntryWidget@@QAE@FFFFFPAD0F0FFFFHH@Z
    +??_C@_0BG@JPLP@Illegal?5byte?5sequence?$AA@J
    +?AllocateSampleHandles@soundManager@@QAEXXZ"
    +?BlitBitmapToScreen@@YIXPAVbitmap@@HHHHHH@Z&
    +?CheckSetEvilInterface@advManager@@QAEXHH@Z.
    +?ChooseToFightForArtifact@philAI@@QAEHHHH@Z.
    +?ClearCombatMessages@combatManager@@QAEXH@Z6
    +?ComputeValueOfSS@philAI@@QAEHPAVhero@@HH@Z.
    +?DoAdvCommand@advManager@@QAEPAVmapCell@@XZ2
    +?DoDialog@executive@@QAEHPAVbaseManager@@@Z&
    +?EvaluateBarrier@philAI@@QAEHPAVmapCell@@@Z*
    +?GetFont@resourceManager@@QAEPAVfont@@PAD@Z
    +?GetIcon@resourceManager@@QAEPAVicon@@PAD@Z*
    +?GetIconEntry@@YIPAUIconEntry@@PAVicon@@H@Z"
    +?GetMoveShowIt@advManager@@QAEHPAVhero@@H@Z"
    +?GiveResource@advManager@@QAEXPAVhero@@HH@Z"
    +?HasValidSpellTarget@combatManager@@QAEHH@Z&
    +?InitializeFiles@fileRequester@@QAEHPAD0H@Z*
    +?Main@combatManager@@UAEHAAUtag_message@@@Z"
    +?Main@fileRequester@@UAEHAAUtag_message@@@Z&
    +?Main@listBoxWidget@@UAEHAAUtag_message@@@Z&
    +?ProcessSelectDialog@dropListWidget@@QAEXXZ
    +?Query@resourceManager@@QAEPAVresource@@K@Z.
    +?RandomizePassword@game@@QAEXPAVmapCell@@@Z6
    +?RemoveWidget@heroWindow@@QAEXPAVwidget@@@Z"
    +?SetCombatDirections@combatManager@@QAEXH@Z&
    +?SetEnvironmentOrigin@advManager@@QAEXHHH@Z*
    +?SetupComPortHandler@@YIHAAUtag_message@@@Z"
    +?SetupRecruitWin@@YIXPAVheroWindow@@HHHHH@Z&
    +?ShotIsThroughWall@combatManager@@QAEHHHH@Z"
    +?TeleportTo@advManager@@QAEXPAVhero@@HHHH@Z"
    +?TownEvent@advManager@@QAEXPAVmapCell@@HH@Z
    +?TurnToStone@combatManager@@QAEXPAVarmy@@@Z"
    +?WAVE_init_driver@@YIPAU_DIG_DRIVER@@KGGG@Z"
    +?WeeklyGenericSite@game@@QAEXPAVmapCell@@@Z&
    +?WeeklyRecruitSite@game@@QAEXPAVmapCell@@@Z"
    +?arithCodeDoneDecoding@@YIXPAUBitStream@@@Z6
    +?arithCodeDoneEncoding@@YIXPAUBitStream@@@Z&
    +?cdTestTrackData@@3PAUsCDTest_Track_Data@@A&
    +?pop_node@@YIPAUtag_Node@@PAUtag_Anchor@@@Z"
    , or you may win by defeating all enemy heroes and capturing all enemy towns and castles.
    ,??_C@_0BG@OHDM@Read?9only?5file?5system?$AA@
    ,?BuildTempWalkSeq@@YIXPAUSMonFrameInfo@@HH@Z
    ,?CheckApplyBadMorale@combatManager@@QAEHHH@Z
    ,?CombatSpecialHandler@@YIHAAUtag_message@@@Z
    ,?DetermineHeroToMove@philAI@@QAEPAVhero@@H@Z
    ,?DimensionDoorHandler@@YIHAAUtag_message@@@Z
    ,?EffectSpellCure@combatManager@@QAEXPAHHHH@Z
    ,?EvaluatePassword@philAI@@QAEHPAVmapCell@@@Z
    ,?EventSound@advManager@@QAEXHHPAUSAMPLE2@@@Z
    ,?IsCrystalBallInEffect@advManager@@QAEHHHH@Z
    ,?IsSpecialLossCondition@ExpCampaign@@QAECH@Z
    ,?Main@dropListWidget@@UAEHAAUtag_message@@@Z
    ,?MeanRVOfUnexploredTerritory@philAI@@QAEHH@Z
    ,?ResetCyclingCreatures@combatManager@@QAEXXZ
    ,?SetupAndLoadObstacles@combatManager@@QAEXXZ
    ,?StopSample@soundManager@@QAEXPAU_SAMPLE@@@Z
    ,?SystemOptionsHandler@@YIHAAUtag_message@@@Z
    ,?ValueOfEventAtPosition@philAI@@QAEHHHHPAH@Z
    ,?add_node@@YIXPAUtag_Anchor@@PAUtag_Node@@@Z
    ,?arithCodeStartDecoding@@YIXPAUBitStream@@@Z
    ,?arithCodeStartEncoding@@YIXPAUBitStream@@@Z
    ,?gpHighScoreManager@@3PAVhighScoreManager@@A
    ,?sElevationOverlay@@3PAUSElevationOverlay@@A
    - floating point not loaded
    - not enough space for _onexit/atexit table
    - not enough space for arguments
    - not enough space for environment
    - not enough space for lowio initialization
    - not enough space for stdio initialization
    - not enough space for thread data
    - pure virtual function call
    - unable to open console device
    - unexpected heap error
    - unexpected multithread lock error
    --*--Manager List Stop --*--
    -----Manager List Start-----
    -??_C@_0BI@IIAE@Operation?5not?5permitted?$AA@
    -?CheckApplyGoodMorale@combatManager@@QAEXHH@Z
    -?DetermineTargetPosition@philAI@@QAEHAAH0H0@Z
    -?Dispose@resourceManager@@QAEXPAVresource@@@Z
    -?EffectSpellDamage@combatManager@@QAEXPAHHH@Z
    -?ExpNewCampaignHandler@@YIHAAUtag_message@@@Z
    -?GetEvent@inputManager@@QAE?AUtag_message@@XZ
    -?GetMouse@resourceManager@@QAEPAVmouse@@PAD@Z
    -?GiveExperience@advManager@@QAEHPAVhero@@HH@Z
    -?HeroIDToHeroPos@game@@QAEHPAVplayerData@@H@Z
    -?Main@resourceManager@@UAEHAAUtag_message@@@Z
    -?Main@textEntryWidget@@UAEHAAUtag_message@@@Z
    -?ReceiveRemotePlayerExit@@YIXUSPlayerExit@@@Z
    -?RestoreDropBackground@dropListWidget@@QAEXXZ
    -?SeedPosition@searchArray@@QAEXHHHHHHHHHHHH@Z
    -?SetupMage@townManager@@QAEXPAVheroWindow@@@Z
    -?SetupModemGameHandler@@YIHAAUtag_message@@@Z
    -?SetupWell@townManager@@QAEXPAVheroWindow@@@Z
    -?TownIDToTownPos@game@@QAEHPAVplayerData@@H@Z
    -?arithCodeBitPlusFollow@@YIXPAUBitStream@@I@Z
    -?campaignChoices@@3PAY1M@2USCampaignChoice@@A
    -?getAndMoveToFrontDecode@@YIHPAUBitStream@@@Z
    -?gsMenuEnableStatus@@3PAUSMenuEnableStatus@@A
    -?moveToFrontCodeAndSend@@YIXPAUBitStream@@H@Z
    -?xCampaignChoices@@3PAY172USCampaignChoice@@A
    -ooWXXXX
    ..\build\intel\mt_obj\cfout.obj
    ..\build\intel\mt_obj\cvt.obj
    ..\build\intel\mt_obj\frnd.obj
    ..\build\intel\mt_obj\ftol.obj
    ..\build\intel\mt_obj\ldexp.obj
    ..\build\intel\mt_obj\util.obj
    .??_C@_0BJ@KAFL@Function?5not?5implemented?$AA@
    .?AddManager@executive@@QAEHPAVbaseManager@@H@Z
    .?CallManager@executive@@QAEXPAVbaseManager@@@Z
    .?ComputeValueOfFreeSS@philAI@@QAEHPAVhero@@H@Z
    .?DataEntryWindowHandler@@YIHAAUtag_message@@@Z
    .?DoBolt@combatManager@@QAEXHHHHHHHHHHHHHHHHH@Z
    .?ExpLoadCampaignHandler@@YIHAAUtag_message@@@Z
    .?ExperienceValueOfStack@combatManager@@QAEHH@Z
    .?FillBitmapAreaClip@@YIXPAVbitmap@@HHHHHHHHH@Z
    .?FillToBuffer@icon@@QAEXHHHHHPAUSLimitData@@@Z
    .?FirstResurrectable@combatManager@@QAEHHPAHH@Z
    .?Main@highScoreManager@@UAEHAAUtag_message@@@Z
    .?PeekEvent@inputManager@@QAE?AUtag_message@@XZ
    .?ReleaseFizzleSource@heroWindowManager@@QAEXXZ
    .?ShowMassSpell@combatManager@@QAEXQAY0BE@CHH@Z
    .?SpaceForElementalExists@combatManager@@QAEHXZ
    .?TrueFalseDialogHandler@@YIHAAUtag_message@@@Z
    .?ViewWorldDialogHandler@@YIHAAUtag_message@@@Z
    .?dpEnumSession@@YGHPAUDPSESSIONDESC@@PAXPAKK@Z
    .\DATA\
    .\DATA\HEROES2.AGG
    .\GAMES\
    .\MAPS\
    .\Mss32.lib
    .\Smkw32ms.lib
    .\Win32_RE\BASEMGR.OBJ
    .\Win32_RE\BITMAP.OBJ
    .\Win32_RE\BORDER.OBJ
    .\Win32_RE\BUTTON.OBJ
    .\Win32_RE\Blur.obj
    .\Win32_RE\Bzip.obj
    .\Win32_RE\DIMMER.OBJ
    .\Win32_RE\EXEC.OBJ
    .\Win32_RE\FONT.OBJ
    .\Win32_RE\ICON.OBJ
    .\Win32_RE\ICONWDGT.OBJ
    .\Win32_RE\INPUTMGR.OBJ
    .\Win32_RE\Icon2b.obj
    .\Win32_RE\Icond2b.obj
    .\Win32_RE\Icondf2b.obj
    .\Win32_RE\Iconf2b.obj
    .\Win32_RE\Iconm2b.obj
    .\Win32_RE\Iconmf2b.obj
    .\Win32_RE\MOUSEMGR.OBJ
    .\Win32_RE\Midi.obj
    .\Win32_RE\Misc.obj
    .\Win32_RE\PALETTE.OBJ
    .\Win32_RE\RESMGR.OBJ
    .\Win32_RE\RESOURCE.OBJ
    .\Win32_RE\Ripple.obj
    .\Win32_RE\SAMPLE.OBJ
    .\Win32_RE\TEXTWDGT.OBJ
    .\Win32_RE\TILESET.OBJ
    .\Win32_RE\Textntry.obj
    .\Win32_RE\WIDGET.OBJ
    .\Win32_RE\WINDOW.OBJ
    .\Win32_RE\WINMGR.OBJ
    .\Win32_RE\bmap2.obj
    .\Win32_RE\droplist.obj
    .\Win32_RE\icon2bc.obj
    .\Win32_RE\icon2bs.obj
    .\Win32_RE\icon2by.obj
    .\Win32_RE\iconf2bc.obj
    .\Win32_RE\iconf2by.obj
    .\Win32_RE\listbox.obj
    .\Win32_RE\miscwin.obj
    .\Win32_RE\soundmgr.obj
    .\Win32_Re\ADVMGR.OBJ
    .\Win32_Re\AI.OBJ
    .\Win32_Re\ARMY.OBJ
    .\Win32_Re\ARMYGRP.OBJ
    .\Win32_Re\CMBTMGR.OBJ
    .\Win32_Re\COMMAND.OBJ
    .\Win32_Re\CURSOR.OBJ
    .\Win32_Re\Campaign.obj
    .\Win32_Re\Castle.obj
    .\Win32_Re\DRAWING.OBJ
    .\Win32_Re\EVENTS.OBJ
    .\Win32_Re\FINDPATH.OBJ
    .\Win32_Re\FLY.OBJ
    .\Win32_Re\GAME.OBJ
    .\Win32_Re\HERO.OBJ
    .\Win32_Re\HEXCELL.OBJ
    .\Win32_Re\HISCORE.OBJ
    .\Win32_Re\KB.OBJ
    .\Win32_Re\Modem.obj
    .\Win32_Re\NOOPT.OBJ
    .\Win32_Re\Netbios.obj
    .\Win32_Re\Newgame.obj
    .\Win32_Re\Overview.obj
    .\Win32_Re\PATH.OBJ
    .\Win32_Re\PHILAI.OBJ
    .\Win32_Re\RECRUIT.OBJ
    .\Win32_Re\REMOTE.OBJ
    .\Win32_Re\REQUEST.OBJ
    .\Win32_Re\SEARCH.OBJ
    .\Win32_Re\SETUP.OBJ
    .\Win32_Re\SMACKMGR.OBJ
    .\Win32_Re\SPELLAI.OBJ
    .\Win32_Re\SPELLS.OBJ
    .\Win32_Re\STRIP.OBJ
    .\Win32_Re\SWAPMGR.OBJ
    .\Win32_Re\TOWN.OBJ
    .\Win32_Re\TOWNMGR.OBJ
    .\Win32_Re\VIEW.OBJ
    .\Win32_Re\Viewwrld.obj
    .\Win32_Re\Wsnetwin.obj
    .\Win32_Re\X_CAMPGN.OBJ
    .\Win32_Re\X_GLOBAL.OBJ
    .\Win32_Re\comwin.obj
    .\Win32_Re\dpnetwin.obj
    .\Win32_Re\heroes.res
    .\Win32_Re\kbwin.obj
    .\Win32_Re\mapcell.obj
    .\Win32_Re\netwin.obj
    .\Win32_Re\tradpost.obj
    .\Win32_Re\wingraph.obj
    .\Win32_Re\x_arena.obj
    .\Win32_Re\x_cdtest.obj
    .\Wing32.lib
    .\basewin.lib
    .\netapi32.lib
    .\winextra\BITS.obj
    .\winextra\TILE.obj
    .idata
    // DO NOT EDIT.
    // THIS FILE WAS AUTOMATICALLY GENERATED BY CDTest_GenerateTable.
    /??_C@_0BI@DMFE@No?5space?5left?5on?5device?$AA@"
    /??_C@_0BK@FNFN@Resource?5deadlock?5avoided?$AA@&
    /??_C@_0BK@ODCM@Interrupted?5function?5call?$AA@
    /?AttemptAttack@combatManager@@QAEHPAVarmy@@HH@Z.
    /?BroadcastMessage@heroWindowManager@@QAEHHHHH@Z"
    /?CheckWin@combatManager@@QAEHPAUtag_message@@@Z
    /?EvaluateGenericSite@philAI@@QAEHPAVmapCell@@@Z
    /?EvaluateRecruitSite@philAI@@QAEHPAVmapCell@@@Z&
    /?FindAdjacentMonster@advManager@@QAEHHHPAH0HH@Z
    /?GetBestHero@philAI@@QAEXPAVtown@@AAUBHC@@AAM@Z"
    /?GetBitmap@resourceManager@@QAEPAVbitmap@@PAD@Z"
    /?GetSample@resourceManager@@QAEPAVsample@@PAD@Z"
    /?GiveRandomArtifact@advManager@@QAEHPAVhero@@@Z6
    /?IsNegationSphereInEffect@combatManager@@QAEHXZ.
    /?LoadAggregateHeader@resourceManager@@QAEHPAD@Z
    /?Main@heroWindowManager@@UAEHAAUtag_message@@@Z
    /?ProcessNGKeyPress@game@@QAEHAAUtag_message@@@Z2
    /?ResetBoltAngle@combatManager@@QAEXPAUSBolt@@@Z
    /?SaveFizzleSource@heroWindowManager@@QAEXHHHH@ZF
    /?SetupDisplayString@textEntryWidget@@QAEXPADG@Z&
    /?SetupHotSeatGameHandler@@YIHAAUtag_message@@@Z&
    /?SetupNetworkGameHandler@@YIHAAUtag_message@@@Z.
    /?TransferArtifacts@advManager@@QAEXPAVhero@@0@Z
    /?ValidatePreviousPosition@soundManager@@QAEXH@ZF
    /?dpEnumServiceProvider@@YGHPAU_GUID@@PADKKPAX@Z2
    /D0 - disable digital sound
    /I0 - skip the intro
    /M0 - disable MIDI music
    /R0 - disable Redbook music
    0??_C@_0BK@HLB@No?5such?5device?5or?5address?$AA@
    0?BloodLustEffect@combatManager@@QAEXPAVarmy@@H@Z
    0?CheckSetMouseDirection@combatManager@@QAEXHHH@Z
    0?CheckUpdateCombatMessages@combatManager@@QAEXXZ
    0?DDCreateSurface@@YIPAUIDirectDrawSurface@@KKH@Z
    0?DigitalReport@soundManager@@QAEJPAU_SAMPLE@@F@Z
    0?EffectSpellResurrect@combatManager@@QAEXPAHHH@Z
    0?FightEvent@philAI@@QAEHPAVhero@@PAVmapCell@@H@Z
    0?FindCombatPath@searchArray@@QAEHHHPAVarmy@@HH@Z
    0?FindResurrectArmyIndex@combatManager@@QAEHHHH@Z
    0?GetClosestArmy@combatManager@@QAEHPAVarmy@@HH@Z
    0?GetLuck@game@@QAEHPAVhero@@PAVarmy@@PAVtown@@@Z
    0?ModifySample@soundManager@@QAEXPAU_SAMPLE@@FJ@Z
    0?RemoveManager@executive@@QAEXPAVbaseManager@@@Z
    0?SeedCombatPosition@searchArray@@QAEXPAVarmy@@@Z
    0?SetupCampaignGameHandler@@YIHAAUtag_message@@@Z
    0?SetupCastle@townManager@@QAEXPAVheroWindow@@H@Z
    0?SetupGridForArmy@combatManager@@QAEXPAVarmy@@@Z
    0?SetupNetworkGame2Handler@@YIHAAUtag_message@@@Z
    0?StrategicValueOfPosition@philAI@@QAEHHHHHPAHH@Z
    0?TownEvent@philAI@@QAEXPAVmapCell@@PAVhero@@HH@Z
    0?ValidMoveWithEvent@advManager@@QAEHPAVhero@@H@Z
    0?WalkTowardArmy@combatManager@@QAEHPAVarmy@@HH@Z
    0?arithCodeSymbol@@YIXPAUBitStream@@PAUModel@@H@Z
    0?bsOpenReadStream@@YIPAUBitStream@@PAU_iobuf@@@Z
    1#QNAN
    1#SNAN
    1??_C@_0BK@IEMO@No?5such?5file?5or?5directory?$AA@
    1?AddResource@resourceManager@@QAEXPAVresource@@@Z
    1?FizzleForward@heroWindowManager@@QAEXHHHHHPAC0@Z
    1?GetModLichDamage@combatManager@@QAEMPAVarmy@@M@Z
    1?GetPalette@resourceManager@@QAEPAVpalette@@PAD@Z
    1?GetTileset@resourceManager@@QAEPAVtileset@@PAD@Z
    1?IconToBitmap@@YIXPAVicon@@PAVbitmap@@HHHHHHHHH@Z
    1?MoveHero@advManager@@QAEPAVmapCell@@HHPAH00H0H@Z
    1?ProcessMapChange@advManager@@QAEXUSMapChange@@@Z
    1?UpdateHeroScreenStatusBar@@YIXAAUtag_message@@@Z
    1?UpdateScreenRegion@heroWindowManager@@QAEXHHHH@Z
    1?arithCodeRenormalise_Encode@@YIXPAUBitStream@@@Z
    1?arithDecodeSymbol@@YIHPAUBitStream@@PAUModel@@@Z
    1?bsOpenWriteStream@@YIPAUBitStream@@PAU_iobuf@@@Z
    1st Lvl Growth
    2??_C@_0BH@NNCD@?$DMprogram?5name?5unknown?$DO?$AA@
    2??_C@_0CB@BBBH@Resource?5temporarily?5unavailable@
    2?AsciiConvert@inputManager@@QAEXAAUtag_message@@@Z
    2?CombatSystemOptionsHandler@@YIHAAUtag_message@@@Z
    2?DetermineEffectOfSpell@combatManager@@QAEXHPAH0@Z
    2?EffectSpellCreateCreature@combatManager@@QAEHHH@Z
    2?MessageHandler@ExpCampaign@@CIHAAUtag_message@@@Z
    2?ShowSpellMessage@combatManager@@QAEXHHPAVarmy@@@Z
    3?CombineGroups@combatManager@@QAEXPAVarmyGroup@@0@Z*
    3?FadeScreen@heroWindowManager@@QAEXHHPAVpalette@@@Z"
    3?GetBackdrop@resourceManager@@QAEXPADPAVbitmap@@H@Z*
    3?GetBestBuilding@philAI@@QAEXPAVtown@@AAUBHC@@AAM@Z
    3?GetBestCreature@philAI@@QAEXPAVtown@@AAUBHC@@AAM@Z*
    3?GetMIDIWrap@resourceManager@@QAEPAVMIDIWrap@@PAD@Z
    3?GetMorale@armyGroup@@QAEHPAVhero@@PAVtown@@PAV1@@Z"
    3?HeroInteractionAtHero@philAI@@QAEXPAVhero@@0HPAH@Z
    3?HouseEvent@advManager@@QAEXPAVhero@@PAVmapCell@@@Z"
    3?SetCombatViewArmySmallLevel@combatManager@@QAEXH@Z&
    3?SetupMultiPlayerGameHandler@@YIHAAUtag_message@@@Z&
    3?ShootMissile@combatManager@@QAEXHHHHPAMPAVicon@@@Z
    3?ShowSkeletons@combatManager@@QAEXPAVheroWindow@@@Z
    3?TestPossibleDirections@searchArray@@QAEXHHQAC0HH@ZF
    3HPj`j`h
    480K of Conventional Memory
    4??_C@_0CE@BADJ@Inappropriate?5I?1O?5control?5operat@
    4?AddBolt@combatManager@@QAEXPAUSBolt@@HHHHHHHHHHHH@Z
    4?BroadcastMessage@heroWindow@@QAEHAAUtag_message@@@Z
    4?DamageGroup@philAI@@QAEHPAVarmyGroup@@PAVhero@@1M@Z
    4?DimIconToBitmap@@YIXPAVicon@@PAVbitmap@@HHHHHHHHH@Z
    4?DoAIEvent@advManager@@QAEXPAVmapCell@@PAVhero@@HH@Z
    4?JailEvent@advManager@@QAEXPAVmapCell@@PAVhero@@HH@Z
    4?ProcessIncomingGroupMapChange@advManager@@QAEXPAD@Z
    4?ReceiveHostReportsPlayerExit@@YIXHUSPlayerExit@@H@Z
    4?RemoveResource@resourceManager@@QAEXPAVresource@@@Z
    4?ShowDeadArmies@combatManager@@QAEXPAVheroWindow@@@Z
    5??_C@_0BK@DEOK@Runtime?5Error?$CB?6?6Program?3?5?$AA@
    5??_C@_0HA@EJLJ@For?5information?5on?5how?5your?5prog@
    5?AddWindow@heroWindowManager@@QAEXPAVheroWindow@@HH@Z
    5?AttemptAdjacentAttack@combatManager@@QAEHPAVarmy@@@Z
    5?BarrierEvent@advManager@@QAEHPAVmapCell@@PAVhero@@@Z
    5?ChangeTilesetIndex@fullMap@@QAEXPAVmapCell@@HHHHHH@Z
    5?FlipIconToBitmap@@YIXPAVicon@@PAVbitmap@@HHHHHHHHH@Z
    5?MonoIconToBitmap@@YIXPAVicon@@PAVbitmap@@HHHHHHHHH@Z
    5?ShowSpellCastFailure@combatManager@@QAEXPAVarmy@@H@Z
    5?ValueOfBuyingBuilding@philAI@@QAEXPAVtown@@HAAHAAM@Z
    5?WalkTowardArmyFront@combatManager@@QAEHPAVarmy@@HH@Z
    6??_C@_0BO@ELHF@Too?5many?5open?5files?5in?5system?$AA@
    6?ExpansionRecruitEvent@advManager@@QAEXPAVhero@@HPAF@Z
    6?IconToBitmapScale@@YIXPAVicon@@PAVbitmap@@HHHHHHHHH@Z
    6?JailAIEvent@advManager@@QAEXPAVmapCell@@PAVhero@@HH@Z
    6?PasswordEvent@advManager@@QAEXPAVmapCell@@PAVhero@@@Z
    6?RecruitEvent@advManager@@QAEXPAVhero@@HPAVmapCell@@@Z
    6?RedistributeTroops@philAI@@QAEXPAVarmyGroup@@0HHHHH@Z
    6?RemoveWindow@heroWindowManager@@QAEXPAVheroWindow@@@Z
    6?SetCommandAndText@townManager@@QAEXAAUtag_message@@@Z
    6?SetupThievesGuild@townManager@@QAEXPAVheroWindow@@H@Z
    6?ValueOfBuyingCreature@philAI@@QAEXPAVtown@@HAAHHAAM@Z
    6herologo.icn
    7?BarrierAIEvent@advManager@@QAEHPAVmapCell@@PAVhero@@@Z
    7?BlitBitmapToScreenNoMouseCheck@@YIXPAVbitmap@@HHHHHH@Z2
    7?EvaluateOneTimeCreaturePurchase@philAI@@QAEXHHHAAH00@Z:
    7?ProcessCombatMsg@combatManager@@QAEHAAUtag_message@@@Z&
    7?ShowEagleEyeSpell@combatManager@@QAEXPAVheroWindow@@@Z.
    8??_C@_0CN@FPEG@R6017?$AN?6?9?5unexpected?5multithread?5@
    8??_C@_0DN@MHHJ@?$CIPress?5Retry?5to?5debug?5the?5applic@
    8?ClearWinLoseBottom@combatManager@@QAEXPAVheroWindow@@@Z
    8?FlipDimIconToBitmap@@YIXPAVicon@@PAVbitmap@@HHHHHHHHH@Z
    8?GetNewCellExtraObject@fullMap@@QAEPAUmapCellExtra@@HH@Z
    8?GhostEvent@advManager@@QAEHPAVhero@@PAVmapCell@@PADHH@Z
    8?MemorySample@soundManager@@QAEPAU_SAMPLE@@PAVsample@@@Z
    8?PasswordAIEvent@advManager@@QAEXPAVmapCell@@PAVhero@@@Z
    8?ProcessNextAction@combatManager@@QAEHAAUtag_message@@@Z
    8?RawEffectSpellInfluence@combatManager@@QAEHPAVarmy@@H@Z
    9??_C@_0CF@LKPB@R6002?$AN?6?9?5floating?5point?5not?5load@
    9??_C@_0CG@DPMN@R6025?$AN?6?9?5pure?5virtual?5function?5c@
    9?ConvertToHover@heroWindowManager@@QAEHAAUtag_message@@@Z
    9?FlipMonoIconToBitmap@@YIXPAVicon@@PAVbitmap@@HHHHHHHHH@Z
    9?GenericSiteEvent@advManager@@QAEXPAVmapCell@@PAVhero@@@Z
    9?GetNewCellExtraOverlay@fullMap@@QAEPAUmapCellExtra@@HH@Z
    9?RecruitSiteEvent@advManager@@QAEXPAVmapCell@@PAVhero@@@Z
    9?StartSample@soundManager@@QAEPAU_SAMPLE@@PADPAPADFFHHJ@Z
    9?ValueOfBuyingHero@philAI@@QAEXPAVtown@@PAVhero@@AAHAAM@Z
    9?ViewSpells@game@@QAEHPAVhero@@HP6IHAAUtag_message@@@ZH@Z
    9?ZombieEvent@advManager@@QAEHPAVhero@@PAVmapCell@@PADHH@Z
    :??_C@_0CB@HPAL@?$AN?6abnormal?5program?5termination?$AN?6@
    :??_C@_0CF@JPDF@Microsoft?5Visual?5C?$CL?$CL?5Runtime?5Lib@
    :??_C@_0CJ@GGOE@R6019?$AN?6?9?5unable?5to?5open?5console?5@
    :??_C@_0CK@OIBL@R6008?$AN?6?9?5not?5enough?5space?5for?5ar@
    :??_C@_0CM@JOOB@R6009?$AN?6?9?5not?5enough?5space?5for?5en@
    :??_C@_0CM@OBIC@R6016?$AN?6?9?5not?5enough?5space?5for?5th@
    :??_C@_0DF@CKIP@R6024?$AN?6?9?5not?5enough?5space?5for?5_o@
    :??_C@_0DF@ECGN@R6027?$AN?6?9?5not?5enough?5space?5for?5lo@
    :??_C@_0DF@FKAC@R6026?$AN?6?9?5not?5enough?5space?5for?5st@
    :?GetBackdropAtLoc@resourceManager@@QAEXPADPAVbitmap@@HHH@Z
    :?ProcessMouseMessage@listBoxWidget@@QAEHAAUtag_message@@@Z
    :?QuickCombat@philAI@@QAEHPAVarmyGroup@@PAVhero@@01HHAAM2@Z
    :?ShowWinLoseArtifact@combatManager@@QAEXPAVheroWindow@@H@Z
    ;?CombatMonsterEvent@philAI@@QAEHPAVhero@@HPAHPAVmapCell@@@Z
    ;?ExperienceValueOfStack@game@@QAEHPAVarmyGroup@@PAVhero@@@Z:
    ;?GenericSiteAIEvent@advManager@@QAEXPAVmapCell@@PAVhero@@@Z
    ;?HeroInteractionAtTown@philAI@@QAEXPAVhero@@PAVtown@@HPAH@Z&
    ;?IconToBitmapYModify@@YIXPAVicon@@PAVbitmap@@HHHHHHHHHPAC@Z"
    ;?RecruitSiteAIEvent@advManager@@QAEXPAVmapCell@@PAVhero@@@Z"
    ;?SetupDynamicWindow@@YIXHHHHHHHPAH00000PAPAVheroWindow@@H@Z
    ;?SkeletonEvent@advManager@@QAEHPAVhero@@PAVmapCell@@PADHH@Z2
    <??_C@_0CB@LBOB@R6018?$AN?6?9?5unexpected?5heap?5error?$AN?6@
    <?FightValueOfStack@philAI@@QAEHPAVarmyGroup@@PAVhero@@HHHH@Z
    <?GetNextChainLightningTarget@combatManager@@QAEHPAVarmy@@H@Z
    <program name unknown>
    =?CheckHandleNetPlayerWait@advManager@@QAEHAAUtag_message@@H@Z
    =?ModifyDamageForArtifacts@combatManager@@QAEXPAJHPAVhero@@1@Z
    >?CombatClipDrawToBuffer@icon@@QAEHHHHPAUSLimitData@@HHPAEPAC@Z
    ??0ExpCampaign@@QAE@XZ
    ??0MIDIWrap@@QAE@PAD@Z
    ??0advManager@@QAE@XZ
    ??0army@@QAE@XZ.
    ??0armyGroup@@QAE@XZ
    ??0baseManager@@QAE@XZ
    ??0bitmap@@QAE@FFF@Z
    ??0bitmap@@QAE@K@Z
    ??0bitmap@@QAE@XZ
    ??0border@@QAE@FFFFFFFPAD@Z
    ??0border@@QAE@XZ
    ??0button@@QAE@FFFFKFFFFFF@Z
    ??0button@@QAE@FFFFPADFFFFFF@Z
    ??0button@@QAE@XZ
    ??0combatManager@@QAE@XZ
    ??0dimmerWidget@@QAE@FFFFFF@Z
    ??0dimmerWidget@@QAE@XZ"
    ??0dropListWidget@@QAE@XZ
    ??0executive@@QAE@XZ
    ??0font@@QAE@K@Z
    ??0fullMap@@QAE@XZ
    ??0hero@@QAE@XZ2
    ??0heroWindow@@QAE@HHHHH@Z
    ??0heroWindow@@QAE@HHPAD@Z
    ??0heroWindow@@QAE@XZ
    ??0heroWindowManager@@QAE@XZ
    ??0hexcell@@QAE@XZ
    ??0highScoreManager@@QAE@XZ.
    ??0icon@@QAE@K@Z
    ??0iconWidget@@QAE@FFFFKFCFFF@Z:
    ??0iconWidget@@QAE@XZ
    ??0inputManager@@QAE@XZ
    ??0listBoxWidget@@QAE@XZ
    ??0mouseManager@@QAE@XZ
    ??0palette@@QAE@K@Z
    ??0palette@@QAE@XZ
    ??0philAI@@QAE@XZ
    ??0resource@@QAE@FKFPAV0@@Z.
    ??0resource@@QAE@XZ&
    ??0resourceManager@@QAE@XZ
    ??0sample@@QAE@PADJJJ@Z
    ??0searchArray@@QAE@XZ
    ??0soundManager@@QAE@XZ"
    ??0swapManager@@QAE@XZ
    ??0textEntryWidget@@QAE@XZ
    ??0textWidget@@QAE@XZ
    ??0tileset@@QAE@K@Z
    ??0town@@QAE@XZ"
    ??0townManager@@QAE@XZ
    ??0townObject@@QAE@HHPAD@Z
    ??0widget@@QAE@FFFFFF@Z&
    ??0widget@@QAE@XZ
    ??1ExpCampaign@@QAE@XZ
    ??1MIDIWrap@@UAE@XZ&
    ??1bankBox@@QAE@XZ
    ??1bitmap@@UAE@XZ
    ??1border@@UAE@XZ
    ??1button@@UAE@XZ
    ??1dropListWidget@@UAE@XZ
    ??1font@@UAE@XZ
    ??1fullMap@@QAE@XZ
    ??1icon@@UAE@XZ.
    ??1iconWidget@@UAE@XZ
    ??1listBoxWidget@@UAE@XZ
    ??1palette@@UAE@XZ
    ??1resource@@UAE@XZ
    ??1sample@@UAE@XZ
    ??1searchArray@@QAE@XZ
    ??1strip@@QAE@XZ
    ??1textEntryWidget@@UAE@XZ
    ??1textWidget@@UAE@XZ
    ??1tileset@@UAE@XZ
    ??1townObject@@QAE@XZ
    ??1widget@@UAE@XZ
    ??2@YAPAXI@Z
    ??3@YAXPAX@Z
    ???@@@AAAAA
    ??@@AABB
    ??@@AABBCCDDDEEEEEEFFF
    ??@@ABCCDDEEFFFFFGGGGG
    ??@AABCCDEEEFFF
    ??@ABBCDEEFFGGHHHHHIII
    ??FlipIconToBitmapYModify@@YIXPAVicon@@PAVbitmap@@HHHHHHHHHPAC@Z
    ??IconToBitmapColorTable@@YIXPAVicon@@PAVbitmap@@HHHHHHHHHPAEH@Z>
    ??ProcessSelect@advManager@@QAEHPAUtag_message@@PAPAVmapCell@@@Z
    ??_7MIDIWrap@@6B@
    ??_7advManager@@6B@
    ??_7baseManager@@6B@
    ??_7bitmap@@6B@
    ??_7border@@6B@.
    ??_7button@@6B@"
    ??_7combatManager@@6B@
    ??_7dimmerWidget@@6B@
    ??_7dropListWidget@@6B@.
    ??_7fileRequester@@6B@
    ??_7font@@6B@
    ??_7heroWindowManager@@6B@
    ??_7highScoreManager@@6B@
    ??_7icon@@6B@
    ??_7iconWidget@@6B@
    ??_7inputManager@@6B@
    ??_7listBoxWidget@@6B@
    ??_7mouseManager@@6B@
    ??_7palette@@6B@
    ??_7recruitUnit@@6B@
    ??_7resource@@6B@
    ??_7resourceManager@@6B@
    ??_7sample@@6B@&
    ??_7soundManager@@6B@
    ??_7swapManager@@6B@
    ??_7textEntryWidget@@6B@
    ??_7textWidget@@6B@6
    ??_7tileset@@6B@
    ??_7townManager@@6B@
    ??_7widget@@6B@*
    ??_C@_01BJG@?6?$AA@*
    ??_C@_02HFBK@?3?5?$AA@
    ??_C@_02JJJH@?6?6?$AA@
    ??_C@_02PIMC@?$AN?6?$AA@
    ??_C@_03BGKD@log?$AA@
    ??_C@_03CDGJ@exp?$AA@
    ??_C@_03DLFL@cos?$AA@
    ??_C@_03EGFG@pow?$AA@
    ??_C@_03GAFO@_yn?$AA@
    ??_C@_03GNLD@_y1?$AA@
    ??_C@_03JAMN@sin?$AA@
    ??_C@_03JLPI@tan?$AA@
    ??_C@_03MHCE@_y0?$AA@
    ??_C@_03NAME@?4?4?4?$AA@
    ??_C@_04EAGN@atan?$AA@
    ??_C@_04EHAJ@fabs?$AA@
    ??_C@_04ELFI@asin?$AA@
    ??_C@_04JJMP@ceil?$AA@
    ??_C@_04JKCH@fmod?$AA@
    ??_C@_04KAOJ@modf?$AA@
    ??_C@_04LFAD@tanh?$AA@
    ??_C@_04LMBE@sqrt?$AA@
    ??_C@_04NPAI@sinh?$AA@
    ??_C@_04OAMO@acos?$AA@
    ??_C@_04PFKD@cosh?$AA@
    ??_C@_05BFPO@atan2?$AA@*
    ??_C@_05BGNL@1?$CDINF?$AA@
    ??_C@_05EDPF@1?$CDIND?$AA@
    ??_C@_05IJHH@log10?$AA@
    ??_C@_05JNMC@_cabs?$AA@"
    ??_C@_05MEKH@ldexp?$AA@*
    ??_C@_05MFBN@floor?$AA@"
    ??_C@_05OFLO@e?$CL000?$AA@
    ??_C@_05PAK@frexp?$AA@
    ??_C@_06FAAO@File?3?5?$AA@
    ??_C@_06GGHF@_hypot?$AA@
    ??_C@_06JMOL@Line?3?5?$AA@
    ??_C@_06LKFM@1?$CDSNAN?$AA@6
    ??_C@_06ONKE@?$CInull?$CJ?$AA@
    ??_C@_06PAPI@1?$CDQNAN?$AA@6
    ??_C@_08KKBH@No?5error?$AA@6
    ??_C@_08OBID@KERNEL32?$AA@
    ??_C@_09PIFG@Program?3?5?$AA@
    ??_C@_0L@HKL@user32?4dll?$AA@
    ??_C@_0M@CLKF@Broken?5pipe?$AA@2
    ??_C@_0M@EPHJ@File?5exists?$AA@"
    ??_C@_0M@IHMC@handler?4cpp?$AA@.
    ??_C@_0M@MLJM@Bad?5address?$AA@
    ??_C@_0M@PKCK@MessageBoxA?$AA@
    ??_EMIDIWrap@@UAEPAXI@Z"
    ??_Ebitmap@@UAEPAXI@Z
    ??_Eborder@@UAEPAXI@Z
    ??_Ebutton@@UAEPAXI@Z
    ??_EdimmerWidget@@UAEPAXI@Z&
    ??_EdropListWidget@@UAEPAXI@Z
    ??_Efont@@UAEPAXI@Z2
    ??_Eicon@@UAEPAXI@Z*
    ??_EiconWidget@@UAEPAXI@Z
    ??_ElistBoxWidget@@UAEPAXI@Z
    ??_Epalette@@UAEPAXI@Z
    ??_Esample@@UAEPAXI@Z
    ??_EtextEntryWidget@@UAEPAXI@Z
    ??_EtextWidget@@UAEPAXI@Z
    ??_Etileset@@UAEPAXI@Z
    ??_GMIDIWrap@@UAEPAXI@Z&
    ??_Gbitmap@@UAEPAXI@Z
    ??_Gborder@@UAEPAXI@Z
    ??_Gbutton@@UAEPAXI@Z
    ??_GdimmerWidget@@UAEPAXI@Z"
    ??_GdropListWidget@@UAEPAXI@Z
    ??_Gfont@@UAEPAXI@Z
    ??_Gicon@@UAEPAXI@Z>
    ??_GiconWidget@@UAEPAXI@Z
    ??_GlistBoxWidget@@UAEPAXI@Z
    ??_Gpalette@@UAEPAXI@Z
    ??_Gsample@@UAEPAXI@Z
    ??_GtextEntryWidget@@UAEPAXI@Z
    ??_GtextWidget@@UAEPAXI@Z
    ??_Gtileset@@UAEPAXI@Z
    ??_H@YGXPAXIHP6EX0@Z@Z
    ?@@ABCDEEFGGGHHHIII
    ?AbsAiPrint@@YIXPAD@Z
    ?Add@armyGroup@@QAEHHHH@Z
    ?AddNetBoxLine@@YIXPADD@Z
    ?AddSpell@hero@@QAEXHH@Z
    ?AdvPanel@advManager@@QAEXXZ
    ?AiPrint@@YIXPAD@Z
    ?AppAbout@@YGHPAXIIJ@Z
    ?AppCommand@@YIJPAXIIJ@Z
    ?AppExit@@YIXXZ
    ?AppIdle@@YIHXZ"
    ?AppInit@@YIHPAX0HPAD@Z6
    ?AppPaint@@YIHPAX0@Z
    ?AppWndProc@@YGJPAXIIJ@Z
    ?AttackTo@army@@QAEHH@Z"
    ?AttackTo@army@@QAEHXZ
    ?AutoInitSVGA@@YIFXZ
    ?Autosave@ExpCampaign@@QAEXXZ
    ?BVResMsg@@YIXPADHH@Z
    ?BaseAlloc@@YIPAXIPADH@Z
    ?BaseFree@@YIXPAXPADH@Z"
    ?Blur@combatManager@@QAEXHHH@Z
    ?BuildBuilding@town@@QAEXH@Z
    ?BuildObj@townManager@@QAEXH@Z
    ?CDMsg@@3Utag_message@@A
    ?CDPlaying@@3HAN
    ?CDPoll@soundManager@@QAEXXZ
    ?CDPreviousPosition@@3PAY0P@DA
    ?CDStartup@soundManager@@QAEXXZB
    ?CDStop@soundManager@@QAEXXZ
    ?CDTest_Cleanup@@YIXXZ
    ?CDTest_GenerateTable@@YIHPAD@Z
    ?CDTest_Init@@YIHXZ"
    ?CDTest_VerifyCDQuick@@YIHXZ
    ?CDTest_VerifyCDThorough@@YIHXZ
    ?CDTest_VerifyTrack@@YIHH@Z"
    ?CDWaiting@@3HA
    ?CSPanel@@3PAVheroWindow@@A.
    ?CalcBaseScore@@YIHH@Z
    ?CalcFileCRC@@YIHPAD@Z
    ?CalcMobility@hero@@QAEHXZ
    ?CalcTerrainCost@@YIHHHHHHH@Z
    ?CampaignID@ExpCampaign@@QAEHXZ
    ?CanBuild@@YIHPAVtown@@H@Z
    ?CanBuildDock@town@@QAEHXZ
    ?CanBuy@@YIHPAVtown@@H@Z
    ?CanFit@army@@QAEHHHPAH@Z
    ?CanJoin@armyGroup@@QAEHH@ZB
    ?CancelSpellType@army@@QAEXH@Z
    ?CastSpell@advManager@@QAEXH@Z
    ?ChangeTown@townManager@@QAEXXZ"
    ?CheckBerserk@philAI@@QAEXXZ
    ?CheckBuyStuff@philAI@@QAEXXZ
    ?CheckChangeCursor@@YIXHHH@Z
    ?CheckDoMain@@YIXHH@Z
    ?CheckEndGame@@YIXHH@Z
    ?CheckForTimeEvent@game@@QAEXXZ*
    ?CheckLevel@hero@@QAEXXZ
    ?CheckLuck@army@@QAEXXZ
    ?CheckMem@@YIHXZ
    ?CheckReload@philAI@@QAEXXZ"
    ?CheckShingleUpdate@@YIXXZ
    ?CheckSumIsDemoOK@@YIHPAD@Z"
    ?Choose@ExpCampaign@@QAEHXZ2
    ?ClaimMine@game@@QAEXHH@Z
    ?ClaimTown@game@@QAEXHHH@Z
    ?CleanUpMenus@@YIXXZ
    ?CleanUpWinGraphics@@YIXXZ
    ?CleanupDPVars@@YIXXZ
    ?Clear@searchArray@@QAEXXZ
    ?ClearMapExtra@@YIXXZ
    ?Close@advManager@@UAEXXZ
    ?Close@combatManager@@UAEXXZ
    ?Close@fileRequester@@UAEXXZ
    ?Close@fullMap@@QAEXXZ
    ?Close@heroWindow@@QAEXXZ
    ?Close@highScoreManager@@UAEXXZ
    ?Close@inputManager@@UAEXXZ
    ?Close@mouseManager@@UAEXXZ"
    ?Close@recruitUnit@@UAEXXZ
    ?Close@resourceManager@@UAEXXZ
    ?Close@searchArray@@QAEXXZ
    ?Close@soundManager@@UAEXXZ
    ?Close@swapManager@@UAEXXZ
    ?Close@townManager@@UAEXXZ
    ?Close@widget@@QAEXXZ
    ?CloseAIMapVars@@YIXXZ
    ?ComboDraw@advManager@@QAEHH@Z
    ?CommandString@@3PADA
    ?CompressTest2@@YIXXZ
    ?CompressTest3@@YIXXZ
    ?CompressTest@@YIXXZ
    ?ComputeAdvNetControl@@YIXXZ
    ?ComputeDailyGold@game@@QAEHH@Z&
    ?ComputeUALoc@@YIXH@Z
    ?CongratsWait@@YIXXZ
    ?Connect@@YIXXZ&
    ?ConnectToDLLs@@YIXXZ
    ?CountShrines@game@@QAEHH@Z&
    ?CreateBoat@game@@QAEHHHH@Z&
    ?CreateColorLookupTables@@YIXXZ
    ?CreateColorTables@@YIXXZ
    ?CreateDiffFile@@YIXPAD00HH@Z
    ?CreateFizzleTables@@YIXXZ
    ?CreateJoinFile@@YIXPAD00@Z
    ?CreatePCXFile@@YIXPADPAEHH1@Z
    ?CreatePrimary@@YIXXZ
    ?Cure@army@@QAEXH@Z6
    ?CurrentMidiFile@@3HA
    ?CycleColors@@YIXH@Z
    ?DDAppPaint@@YIHPAX0@Z
    ?DDCleanUpWinGraphics@@YIXXZ
    ?DDInitGraphics@@YIXXZ
    ?DDInitializePalette@@YIXXZ"
    ?DDQueryNewPalette@@YIHXZ
    ?DDRestoreDisplayMode@@YIXXZ
    ?DDSD@@YIXHPADH@Z
    ?DDSetFullScreenStatus@@YIXH@Z
    ?DDSetPalette@@YIHXZ
    ?DDUpdatePalette@@YIXPAC@Z
    ?DEFAULT_AGGREGATE_NAME@@3PADA
    ?DPSD@@YIXHPADH@Z
    ?Damage@army@@QAEHJH@Z
    ?DamageGroup@armyGroup@@QAEXM@Z"
    ?Data@palette@@QAEPACXZ&
    ?Days@ExpCampaign@@QAEFXZ
    ?Deallocate@hero@@QAEXH@Z
    ?Deallocate@town@@QAEXXZ
    ?DecodeData@@YIJPAD0K@Z*
    ?DecodePacket@@YIHPAEH@Z
    ?DelayMilli@@YIXJ@Z"
    ?DelayTil@@YIXPAH@Z*
    ?DelayTilMilli@@YIXJ@Z
    ?DeleteMainClasses@@YIXXZ
    ?Dial@@YIJXZ
    ?Dim@widget@@QAEXXZ.
    ?DimToBuffer@icon@@QAEXHHHH@Z
    ?DirDoAttack@army@@QAEXH@Z
    ?DisconnectDLLs@@YIXXZ
    ?Dismiss@armyGroup@@QAEXH@Z6
    ?Dismiss@hero@@QAEHXZ
    ?DispelGood@army@@QAEXXZ
    ?DoAI@philAI@@QAEXH@Z
    ?DoArenaDialog@@YIHXZ
    ?DoAttack@army@@QAEXH@Z6
    ?DoBlur@@YIXPAVbitmap@@0HHHH@Z
    ?DoCommand@townManager@@QAEXH@Z&
    ?DoHeroKnob@advManager@@QAEXXZ
    ?DoHeroSplit@@YIXHH@Z
    ?DoHydraAttack@army@@QAEXH@Z
    ?DoKnob@fileRequester@@QAEXXZ
    ?DoKnob@game@@QAEXXZ
    ?DoLuck@combatManager@@QAEXHH@Z>
    ?DoNewTurn@game@@QAEXXZ&
    ?DoRipple@@YIXPAVbitmap@@0HH@Z
    ?DoSSLevelDialog@hero@@QAEXHH@Z"
    ?DoTavern@townManager@@QAEXXZ
    ?DoTownKnob@advManager@@QAEXXZ
    ?DoTradingPost@@YIXHM@Z
    ?Draw@border@@UAEXXZ
    ?Draw@button@@UAEXXZ
    ?Draw@dimmerWidget@@UAEXXZ
    ?Draw@dropListWidget@@UAEXXZ
    ?Draw@iconWidget@@UAEXXZ
    ?Draw@listBoxWidget@@UAEXXZ
    ?Draw@strip@@QAEXXZ&
    ?Draw@textEntryWidget@@UAEXXZ
    ?Draw@textWidget@@UAEXXZ
    ?Draw@townObject@@QAEXH@Z
    ?DrawClouds@hexcell@@QAEXXZ
    ?DrawCursor@advManager@@QAEXXZ
    ?DrawFrame@strip@@QAEXXZ
    ?DrawGround@hexcell@@QAEXXZ.
    ?DrawIcons@strip@@QAEXH@Z
    ?DrawObstacle@hexcell@@QAEXXZ
    ?DrawOccupant@hexcell@@QAEXHH@Z*
    ?DrawString@font@@QAEXPADHHH@Z
    ?DrawToBuffer@army@@QAEXHHH@Z
    ?DrawToBuffer@bitmap@@QAEXFF@Z
    ?DrawToBuffer@icon@@QAEXHHHH@Z
    ?DrawToScreen@bitmap@@QAEXFF@Z
    ?DrawTower@hexcell@@QAEXH@Z
    ?DrawTown@townManager@@QAEXHH@Z&
    ?DrawWindow@heroWindow@@QAEXH@Z*
    ?DrawWindow@heroWindow@@QAEXXZ
    ?DropDownToOnePlayer@@YIXXZ"
    ?EarlyResizeWindow@@YIXHHHH@Z
    ?EarlySetup@@YIHXZ
    ?EarlyShutDownSystem@@YIXXZ"
    ?EarlyShutdown@@YIXPAD0@Z
    ?EncodeData@@YIJPAD0K@Z*
    ?EncodePacket@@YIHPAEDDH@Z
    ?EveryOther@@3HA
    ?ExpansionCampaignRect@@YIHHH@Z&
    ?FadeIn@@YIXH@Z"
    ?FadeOut@@YIXH@Z
    ?FadeTo@@YIXPAE0H@Z2
    ?FadeToColorTable@@YIXPAEH@Z
    ?FileError@@YIXPAD@Z
    ?FileSize@@YIJPAD@Z"
    ?FindLastToken@@YIPADPADD@Z2
    ?FindPath@army@@QAEHHHHHH@Z
    ?FindToken@@YIPADPADD@Z*
    ?Flush@inputManager@@QAEXXZ"
    ?FlyTo@army@@QAEHH@Z
    ?FlyTo@army@@QAEHXZ"
    ?ForcePollSound@@YIXXZ
    ?FreeCompressStructures@@YIXXZ
    ?FreeResources@army@@QAEXXZ2
    ?GETALL@@YIIH@Z
    ?GOut@@YIXPAD@Z"
    ?GUIMRc@@3HA
    ?GUIMRresp@@3PADA
    ?GUIMRresponse@@3PADA
    ?GUIMRrespptr@@3HA
    ?GUIModemCommand@@YIXPAD0@Z.
    ?GUIModemCommandExec@@YICXZ
    ?GUIModemResponse@@YICPAD0@Z
    ?GUIModemResponseExec@@YICXZ
    ?GameMode@@3EA
    ?GameUnsaved@@YIHXZ
    ?GetAttackMask@army@@QAEHHHH@Z
    ?GetBoatsBuilt@game@@QAEHXZ
    ?GetBuildingCost@@YIXHHQAHH@Z
    ?GetBuildingInfo@@YIPADHHH@Z
    ?GetBuildingName@@YIPADHH@Z"
    ?GetCategoryStats@@YIXHQAJQAC@Z6
    ?GetDataEntry@@YIXPAD0H0HH@Z
    ?GetExperience@hero@@QAEHH@Z
    ?GetGameAIVars@philAI@@QAEXXZ
    ?GetGraphicsInfo@@YIXXZ"
    ?GetLevel@hero@@QAEHH@Z.
    ?GetManaCost@@YIHHPAVhero@@@Z
    ?GetManaFrame@@YIHH@Z
    ?GetMap@game@@QAEXXZ
    ?GetMineId@game@@QAEHHH@Z
    ?GetMobilityFrame@@YIHH@Z
    ?GetMonType@@YIHHH@Z
    ?GetMonsterCost@@YIXHQAH@Z
    ?GetMonsterName@@YIPADH@Z
    ?GetMoveMask@army@@QAEHH@Z
    ?GetNewHeroId@game@@QAEHHHH@Z
    ?GetNthSS@hero@@QAEHH@Z
    ?GetNthSpell@hero@@QAEHHH@Z.
    ?GetNumArmies@armyGroup@@QAEHXZ.
    ?GetNumObelisks@@YIHH@Z.
    ?GetNumSpells@hero@@QAEHH@Z*
    ?GetPowBaseY@army@@QAEHXZ
    ?GetRemoteData@@YIPADC@Z
    ?GetSSLevel@hero@@QAECH@Z
    ?GetSideDesc@game@@QAEHPADHH@Z
    ?GetSkipCopyLen@@YIHPAEPAH@Z
    ?GetTownId@game@@QAEHHH@Z
    ?GetTownName@@YIPADH@Z
    ?GetTurnAIVars@philAI@@QAEXH@Z
    ?GiveSS@hero@@QAEHHH@Z
    ?GoBerserk@army@@QAEXXZ
    ?GoodAdjacent@philAI@@QAEHPAH@Z
    ?GrabScreen@bitmap@@QAEXFF@Z
    ?HandleCampaignWin@game@@QAEHXZ&
    ?HandleMCIError@@YIXHPAD@Z
    ?HandleRemoteSuddenExit@@YIXXZ
    ?HasAllUndead@armyGroup@@QAEHXZ"
    ?HasArtifact@hero@@QAEHH@Z
    ?HasAward@ExpCampaign@@QAECH@Z
    ?HasGarrison@town@@QAEHXZ
    ?HasLateOverlay@game@@QAEHHH@Z
    ?HasSpell@hero@@QAEHH@Z
    ?HeroMessageUpdate@@YIXPAD@Z
    ?HeroScreenUpdate@hero@@QAEXXZ
    ?HeroView@@YIHHHH@Z>
    ?IMHotSpots@@3PAY03GA
    ?IPXGuid@@3PAU_GUID@@A
    ?IQnames@@3PAPADA
    ?IconInfo@@3PAU_ICONINFO@@A*
    ?InCombatArea@@YIHHH@Z
    ?InMapArea@@YIHHH@Z"
    ?Init@army@@QAEXHHHHHH@Z
    ?Init@fullMap@@QAEXHH@Z
    ?Init@searchArray@@QAEXXZ
    ?InitAIMapVars@@YIXXZ
    ?InitCampaignMap@game@@QAEXXZ
    ?InitClean@army@@QAEXXZ&
    ?InitGraphics@@YIXXZ
    ?InitMainClasses@@YIXXZ"
    ?InitMap@ExpCampaign@@QAEXXZ
    ?InitMemEntry@@YIXXZ
    ?InitNetGuest@@YICXZ
    ?InitNetHost@@YICXZ
    ?InitNewGameWindow@game@@QAEXXZ*
    ?InitSystem@executive@@QAEHXZ
    ?InitVars@@YIXXZ
    ?InitVideo@@YIXXZ
    ?InitializePalette@@YIXXZ
    ?InterpretCommandLine@@YIHXZ
    ?IsCDDrive@@YIHH@Z
    ?IsCursedItem@@YIHH@Z
    ?IsCycleColor@@YIHH@Z
    ?IsMember@armyGroup@@QAEHH@Z
    ?IsMobile@game@@QAEHH@Z
    ?IsVis@mouseManager@@QAEHXZ.
    ?IvanName@ExpCampaign@@QAEPADXZ2
    ?KBChangeMenu@@YIXPAX@Z&
    ?KBTickCount@@YIJXZ.
    ?LeaveNoBody@army@@QAEHXZ
    ?LeftX@army@@QAEHXZ
    ?LineLength@font@@QAEHPADH@Z
    ?LineWidth@font@@QAEHPAD@Z
    ?LoadGame@game@@QAEXPADHH@Z
    ?LoadMap@game@@QAEHPAD@Z
    ?LoadRemote@advManager@@QAEXXZ
    ?LoadResources@army@@QAEXXZ"
    ?LoadSystemwideIcons@@YIXXZ
    ?LogInt@@YIXPADHHHHHHH@Z
    ?LogStr@@YIXPAD@Z
    ?LogTruncate@@YIXXZ
    ?LogicalPalette@@3U_PALETTE@@A
    ?MAKEFILEID@@YIKPAD@Z
    ?MAP_HEIGHT@@3HA
    ?MAP_WIDTH@@3HA2
    ?MIDIPlay@soundManager@@QAEXH@Z*
    ?MIDIPoll@soundManager@@QAEXXZ
    ?MIDIStop@soundManager@@QAEXXZ
    ?MainLoop@executive@@QAEXXZB
    ?MemError@@YIXXZ
    ?MemSize@@YIHH@Z
    ?MidX@army@@QAEHXZ
    ?MidY@army@@QAEHXZ
    ?MineTypesOwned@game@@QAEHHH@Z
    ?ModemCommand@@YIXPAD@Z
    ?ModemSetup@@YIXH@Z&
    ?MoveAttack@army@@QAEXHH@Z
    ?NGKPBkg@@3PAVicon@@A
    ?NGKPcursorFlashOn@@3HA*
    ?NGKPcursorIndex@@3HA
    ?NORMALISE@@YIHH@Z
    ?NORMALISEHI@@YIHH@Z
    ?NORMALISELO@@YIHH@Z
    ?NULL_SAMPLE2@@3USAMPLE2@@A
    ?NetPosToGamePos@@YIHH@Z
    ?NewGame@game@@QAEHXZ
    ?NewMap@game@@QAEXPAD@Z.
    ?NextHero@playerData@@QAEHH@Z
    ?NextPlayer@game@@QAEXXZ
    ?NumArtifacts@hero@@QAEHXZ
    ?OnMySide@@YIHH@Z
    ?Open@advManager@@UAEHH@Z
    ?Open@combatManager@@UAEHH@Z
    ?Open@fileRequester@@UAEHH@Z
    ?Open@heroWindow@@QAEHHH@Z
    ?Open@highScoreManager@@UAEHH@Z>
    ?Open@inputManager@@UAEHH@Z6
    ?Open@mouseManager@@UAEHH@Z
    ?Open@recruitUnit@@UAEHH@Z
    ?Open@resourceManager@@UAEHH@Z
    ?Open@soundManager@@UAEHH@Z"
    ?Open@swapManager@@UAEHH@Z
    ?Open@townManager@@UAEHH@Z
    ?OppositeDirection@@YIHH@Z
    ?Orientation@@3JA
    ?Overview@game@@QAEXXZ
    ?PacketSend@@3PADA
    ?PerDay@game@@QAEXXZ
    ?PerMonth@game@@QAEXXZ
    ?PerWeek@game@@QAEXXZ
    ?PickLoadGame@game@@QAEHXZ
    ?PlaySmacker@@YIHH@Z
    ?PlayerDead@@YIXH@Z"
    ?PollRemote@@YIXXZ
    ?PollSound@soundManager@@QAEXXZ
    ?PopNetBox@@YIXPADH@Z
    ?PowEffect@army@@QAEXHHHH@Z
    ?PrintMemoryLeaks@@YIXXZ
    ?Process1WindowsMessage@@YIXXZ
    ?ProcessAssert@@YIXHPADH@Z
    ?ProcessDeath@army@@QAEXH@Z
    ?ProcessMapExtra@game@@QAEXXZ
    ?QueryNewPalette@@YIHXZ"
    ?QuickInfo@advManager@@QAEXHH@Z"
    ?QuickViewWait@@YIXXZ
    ?RVConversion@philAI@@QAEHQAH@Z&
    ?RandMineQty@@3PAFAF
    ?Random@@YIHHH@Z
    ?RandomScan@game@@QAEHPACHHHC@Z2
    ?RandomizeEvents@game@@QAEXXZ
    ?RandomizeHeroPool@game@@QAEXXZ.
    ?RandomizeMine@game@@QAEXHH@Z
    ?RandomizeTown@game@@QAEXHHH@Z
    ?Read@border@@QAEXXZ
    ?Read@button@@QAEXXZ
    ?Read@dimmerWidget@@QAEXXZ
    ?Read@dropListWidget@@QAEXXZ
    ?Read@fullMap@@QAEXHH@Z
    ?Read@hero@@QAEXHC@Z
    ?Read@iconWidget@@QAEXXZ
    ?Read@listBoxWidget@@QAEXXZ"
    ?Read@playerData@@QAEXH@Z
    ?Read@textEntryWidget@@QAEXH@Z
    ?Read@textWidget@@QAEXXZ
    ?ReadPacket@@YIDXZ
    ?ReadPrefs@@YIXXZ
    ?ReadPrefsFromFile@@YIXXZ
    ?ReadPrefsFromRegistry@@YIXXZ
    ?ReceiveRemoteData@@YIHPAE0H@Z
    ?RedrawHeroScreen@@YIXXZ
    ?RemoteCleanup@@YIXXZ
    ?RemoteMain@@YIXH@Z
    ?Reseed@advManager@@QAEXHH@Z
    ?Reset@swapManager@@QAEXXZ
    ?ResetHeroRVs@@YIXHHH@Z
    ?ResetPath@army@@QAEXXZB
    ?ResizeWindow@@YIXHHHH@Z
    ?RestoreDisplayMode@@YIXXZ
    ?RiddleStringsEqual@@YIHPAD0@Z
    ?RightX@army@@QAEHXZ
    ?Ripple@combatManager@@QAEXH@Z
    ?S1cursorBaseFrame@@3HA"
    ?S1cursorCycle@@3HA:
    ?S1cursorDirection@@3HA
    ?S1cursorFrameCount@@3HA
    ?S1cursorTurning@@3HA
    ?SETFIRST16@@YIXHI@Z
    ?SETFIRST@@YIXHE@Z
    ?SETFOURTH@@YIXHE@Z&
    ?SETREST16@@YIXHI@Z&
    ?SETREST@@YIXHI@Z
    ?SETSECOND@@YIXHE@Z
    ?SETTHIRD@@YIXHE@Z
    ?SGenRand@@YIHXZ
    ?SIncRandomize@@YIXHH@Z*
    ?SRand@@YIXH@Z
    ?SRandom@@YIHHH@Z
    ?STRONG_NORMALISE@@YIHH@Z
    ?SaveGame@@YIHXZ
    ?SaveGame@game@@QAEHPADHC@Z"
    ?Scan@game@@QAEHPACHH@Z6
    ?SeedTo@advManager@@QAEXHH@Z
    ?SendMapChange@@YIXHCEEHEE@Z
    ?SendRemoteData@@YIHPAE0HH@Z
    ?SetFullScreenStatus@@YIXH@Z
    ?SetGameDefaults@@YIXXZ&
    ?SetGraphicsType@@YIHH@Z
    ?SetInstallDefaults@@YIXXZ
    ?SetMapSize@game@@QAEXHH@Z
    ?SetMenuStatus@@YIXH@Z
    ?SetMenus@@YIXPAXH@Z
    ?SetNoDialogMenus@@YIXH@Z
    ?SetOK@fileRequester@@QAEXH@Z
    ?SetPalette@@YIHXZ
    ?SetPalette@@YIXPACH@Z
    ?SetReady2Poll@@YGXK@Z
    ?SetSS@hero@@QAEXHH@Z
    ?SetText@textWidget@@QAEXPAD@Z
    ?SetVisibility@game@@QAEXHHHH@Z.
    ?SetupAdjacentMons@game@@QAEXXZ*
    ?SetupBaud@game@@QAEHXZ&
    ?SetupCDDrive@@YIHXZ
    ?SetupCDRom@@YIXXZ
    ?SetupCampaignGame@game@@QAEHXZ"
    ?SetupClipper@@YIXXZ
    ?SetupComPort@game@@QAEHXZ
    ?SetupGame@game@@QAEHXZ&
    ?SetupHeroView@@YIXXZ
    ?SetupHotSeatGame@game@@QAEHXZ
    ?SetupModemGame@game@@QAEHXZ
    ?SetupNetworkGame2@game@@QAEHXZ
    ?SetupNetworkGame@game@@QAEHXZ
    ?SetupNewRumour@game@@QAEXXZ
    ?SetupNewTrade@@YIXXZ
    ?SetupOrigData@game@@QAEXXZ>
    ?SetupResources@game@@QAEXXZ
    ?SetupTown@townManager@@QAEXXZ
    ?SetupTowns@game@@QAEXXZ
    ?ShowCongrats@@YIXH@Z
    ?ShowHeroesLogo@game@@QAEXXZ
    ?ShowInfo@ExpCampaign@@QAEXHH@Z2
    ?ShowMemoryStatus@@YIXXZ
    ?ShowScenInfo@game@@QAEXXZ
    ?ShowStatus@@YIXXZ
    ?ShowThisMap@@YIHPAD@Z
    ?ShowThisMapGame@@YIHPAD@Z
    ?ShutDown@@YIXPAD@Z"
    ?ShutDownSmacker@@YIXXZ
    ?ShutdownComError@@YIXPAD@Z*
    ?SmackFade@@YIXPAE0@Z
    ?SmackManagerMain@@YIXXZ
    ?SortStats@@YIXQAJQAC@Z
    ?SpecialAttack@army@@QAEXXZ*
    ?SpellCastWorks@army@@QAEHH@Z
    ?SpellEffect@army@@QAEXHHH@Z
    ?SplitArmy@townManager@@QAEXXZ
    ?SplitMons@swapManager@@QAEXXZ
    ?Stats@hero@@QAECH@Z
    ?StopCursor@advManager@@QAEXH@Z.
    ?StrEqNoCase@@YICPAD0@Z"
    ?Strength@army@@QAEKXZ
    ?SummonBoat@advManager@@QAEXXZ
    ?Swap@armyGroup@@QAEXHPAV1@H@Z
    ?SwapMons@swapManager@@QAEXXZ
    ?TCPGuid@@3PAU_GUID@@A
    ?TakeSS@hero@@QAEHHH@Z
    ?Terminate@executive@@QAEXXZ
    ?TestDynamicWindow@@YIXHH@Z
    ?Toggle@townManager@@QAEXH@Z
    ?TopY@army@@QAEHXZ
    ?TownGate@advManager@@QAEXH@Z
    ?TrigX@@3HA
    ?TrigY@@3HA
    ?TurnOffAIMusic@game@@QAEXXZ
    ?TurnOnAIMusic@game@@QAEXXZ.
    ?TurnTo@advManager@@QAEXH@Z.
    ?TurnsToBuy@philAI@@QAEMQAH@Z
    ?USMsg@@3Utag_message@@A
    ?UnloadRemoteDriver@@YIXF@Z2
    ?UnloadSystemwideIcons@@YIXXZ
    ?UnloadTown@townManager@@QAEXXZ"
    ?Update@bankBox@@QAEXH@Z
    ?Update@fileRequester@@QAEXH@Z
    ?Update@recruitUnit@@QAEXXZ:
    ?Update@swapManager@@QAEXXZ
    ?UpdateArenaIcons@@YIXXZ
    ?UpdateArmies@hero@@QAEXXZ
    ?UpdateDfltMenu@@YIXPAX@Z
    ?UpdateNormalDialog@@YIXPAD@Z
    ?UpdatePalette@@YIXPAC@Z
    ?UpdateSystemOptions@@YIXH@Z
    ?UpdateSystemOptionsMenu@@YIXXZ*
    ?UpdateTimers@@YGXK@Z
    ?UpdateTradingPost@@YIXH@Z
    ?UseSpell@hero@@QAEXH@Z"
    ?VWCleanup@advManager@@QAEXXZ
    ?VWInit@advManager@@QAEXHH@Z
    ?ValidFlight@army@@QAEHHH@Z*
    ?ValidHex@@YIHH@Z
    ?ValidMove@advManager@@QAEHHH@Z*
    ?ValidMove@army@@QAEHH@Z
    ?ValidMove@army@@QAEHHH@Z
    ?ValidPath@army@@QAEHHH@Z
    ?ValidRange@army@@QAEHH@Z
    ?ValidateHero@@YIXPAVhero@@@Z
    ?View@armyGroup@@QAEXH@Z
    ?View@town@@QAEXH@Z
    ?ViewArtifact@hero@@QAEXHHH@Z
    ?ViewMon@swapManager@@QAEXXZ
    ?ViewPuzzle@advManager@@QAEXXZ
    ?ViewStat@hero@@QAEXHH@Z
    ?WFDCStage@@3HA"
    ?WGAppPaint@@YIHPAX0@Z
    ?WGCleanUpWinGraphics@@YIXXZ
    ?WGInitGraphics@@YIXXZ
    ?WGInitializePalette@@YIXXZ
    ?WGQueryNewPalette@@YIHXZ
    ?WGUpdatePalette@@YIXPAC@Z
    ?Wait@@YIJXZ
    ?WaitForDirectConnect@@YIHXZ
    ?WaitForGuest@@YICXZ
    ?WaitForHost@@YICXZ"
    ?WaitForOtherPlayer@@YIHXZ
    ?WaitForPlayer@game@@QAEXPADH@Z*
    ?WaitSample@army@@QAEXH@Z
    ?Walk@army@@QAEXHHH@Z
    ?WalkTo@army@@QAEHH@Z
    ?WalkTo@army@@QAEHXZ
    ?Wince@army@@QAEXXZ:
    ?Write@fullMap@@QAEXH@Z
    ?Write@hero@@QAEXHC@Z
    ?Write@playerData@@QAEXH@Z
    ?WriteModemPacket@@YIXPADH@Z
    ?WritePrefs@@YIXXZ
    ?WritePrefsToFile@@YIXXZ
    ?WritePrefsToRegistry@@YIXXZ
    ?XformToCastle@town@@QAEXXZ
    ?_pnhHeap@@3P6AHI@ZA
    ?arenaWinPtr@@3PAVheroWindow@@A&
    ?bCPrefsChanged@@3HA
    ?bCampaignViewOnly@@3HA:
    ?bComboDraw@@3PAY0BC@CA"
    ?bDataEntryTime@@3HA
    ?bDoColorCycle@@3HA
    ?bEarlySetupDone@@3HA
    ?bEnteringTown@@3HA&
    ?bEvaluatingTravelGates@@3HA
    ?bForceCheckTimeEvent@@3HA
    ?bFreshSave@@3HA
    ?bGotGameType@@3HA
    ?bGotMidi@@3PAEA
    ?bGridWasShowing@@3HA
    ?bHeroBuiltThisTurn@@3HA
    ?bHostFound@@3HA
    ?bInCheckChangeCursor@@3HA
    ?bInCheckEndGame@@3HA
    ?bInDDSD@@3HA
    ?bInDPSD@@3HA
    ?bInHighMoraleBonus@@3HA
    ?bInNewMouseUpdate@@3HA*
    ?bInShutDown@@3HA
    ?bInTeleportGetDest@@3HA
    ?bInTimeoutFail@@3CA
    ?bIsMarketPlace@@3HA
    ?bIsMoatSlowed@@3PAEA
    ?bKBDone@@3HA
    ?bLastMouseOffscreen@@3HA
    ?bLastOnscreenMouseColor@@3HA
    ?bLeftDenominated@@3HA
    ?bMainDone@@3CA*
    ?bMapInitialized@@3DA
    ?bMouseWasVis@@3HA
    ?bMoveSoundMade@@3HA
    ?bMusicIsLooping@@3PAEA
    ?bPaletteInitialized@@3HA
    ?bPrefsChanged@@3HAZ
    ?bProcessMessage@@3PAEA
    ?bPuzzleDraw@@3PAEA*
    ?bRestartMusic@@3HA.
    ?bSVSearchArrayInUse@@3HA
    ?bSaveMusicPosition@@3PAEA
    ?bSecondAttack@@3HA*
    ?bShowIt@@3HA
    ?bSkeletonsShown@@3HA
    ?bSmackNum@@3CA>
    ?bSmackSound@@3HA
    ?bSpecialHideCursor@@3HA
    ?bStartUpInfoReceived@@3HA
    ?bStopOnTrigger@@3PAEA
    ?bTesting@@3HA
    ?bTradeMade@@3HA
    ?bUseDirectPlay@@3HA
    ?bUseWinsock@@3HA
    ?backImage@@3PAVicon@@A"
    ?bigD@@3IA
    ?bigFont@@3PAVfont@@A
    ?bigL@@3IA
    ?bigR@@3IA
    ?bitStreamEOF@@YIXXZ
    ?bitsOutstanding@@3IA
    ?block@@3PAEA
    ?blockOverrun@@YIXXZ
    ?blockSize100k@@3HA"
    ?bmpAndMask@@3PAUtagBITMAP@@A
    ?boatFrameFlip@@3PAFA
    ?bogusModel@@3UModel@@A
    ?brotherIcon@@3PAVicon@@A
    ?bsClose@@YIXPAUBitStream@@@Z
    ?bsGetBit@@YIHPAUBitStream@@@Z
    ?bsInUse@@3HA
    ?bsPutBit@@YIXPAUBitStream@@H@Z
    ?bytesIn@@3HA
    ?bytesOut@@3HA
    ?cAggPathName@@3PADA
    ?cAndBits@@3PAPAXA
    ?cArmyDetail@@3PAPADA
    ?cArmyFrameFileNames@@3PAPADA
    ?cArmySizeName@@3PADA
    ?cBWMouseText@@3PAPADA
    ?cBattleResults@@3PAPADA
    ?cBuildingInfoNeutral@@3PAPADA
    ?cCampaignAwards@@3PAPADA
    ?cCampaignLeaders@@3PAPADA
    ?cCampaignName@@3PAY0M@PADA"
    ?cCastleInfo@@3PAPADA
    ?cColorBits@@3PAPAXA
    ?cCombatBkgNames@@3PAPADA
    ?cCombatHelp@@3PAPADA
    ?cCombatMessage@@3PAPADA
    ?cDEDest@@3PADA&
    ?cDifficulty@@3PAPADA
    ?cDirections@@3PAPADA
    ?cEvilTranslate@@3PAY01PADA
    ?cExpAggPathName@@3PADA
    ?cFRDummy@@3PADA
    ?cHeroLevel@@3PAPADA
    ?cHeroScreen@@3PAPADA
    ?cHeroTypeInitial@@3PADA
    ?cHeroTypeShortName@@3PAPADA
    ?cHumanDifficulty@@3PAPADA
    ?cHumanInfoDifficulty@@3PAPADA
    ?cKingdomOverview@@3PAPADA
    ?cLongCombatHelp@@3PAPADA
    ?cLuckInfo@@3PAPADA&
    ?cMapSize@@3PAPADA
    ?cMiniViewText@@3PAPADA*
    ?cModemCommand@@3PADA
    ?cMonFilename@@3PAPADA
    ?cMoraleInfo@@3PAPADA
    ?cNGKPCore@@3PADA
    ?cNGKPDisplay@@3PADA
    ?cNetBoxColor@@3PADA
    ?cNetBoxLine@@3PAY0IM@DA
    ?cNewTurn@@3PAPADA
    ?cOutOfMemory@@3PADA
    ?cOverrideDigitalDriver@@3PADA
    ?cOverrideMIDIDriver@@3PADA"
    ?cOverviewText@@3PAPADA"
    ?cPanel@@3PAVheroWindow@@A
    ?cPersonality@@3PAPADA
    ?cPlayerNames@@3PAY0BF@DA
    ?cRandomSignText@@3PAPADA
    ?cRandomTavernText@@3PAPADA6
    ?cSecSkillDesc@@3PAY02PADA
    ?cSlowVideoLevelText@@3PAPADA
    ?cSpellHelp@@3PAPADA
    ?cStartDifficulty@@3PAPADA
    ?cTextReceivedBuffer@@3PAPADA
    ?cTownCommand@@3PAPADA
    ?cViewGeneralHelp@@3PAPADA
    ?cViewGeneralLabels@@3PAPADA
    ?cViewGeneralLongHelp@@3PAPADA
    ?cWSTextBuffer@@3PADA
    ?cWellDetail@@3PAPADA
    ?cWinComError@@3PAPADA
    ?cWinText@@3PAPADA
    ?calc_crc@@YIXPAGPAEH@Z.
    ?calc_crc_long@@YIHPAEH@Z
    ?campWin@@3PAVheroWindow@@A"
    ?captainStats@@3PAY03CA*
    ?casWin@@3PAVheroWindow@@A
    ?castX@@3HA
    ?castY@@3HA
    ?castleSlotsBase@@3PAEA:
    ?castleSlotsUse@@3PAEA
    ?choice@@3HA
    ?cleanUpAndFail@@YIXXZ
    ?com_init@@YIFEHH@Z"
    ?com_rcv@@YIFFGPAX@Z
    ?com_sess@@YAFHHZZ
    ?com_snd@@YIFFGGPAXH@Z
    ?com_stat@@YIEFG@Z
    ?com_term@@YIXF@Z
    ?combatMiniInfoText@@3PAPADA
    ?combatSpeedText@@3PAPADA
    ?comm_wrt_task@@YIXXZ
    ?compress@@YIXPAD@Z
    ?compressOutOfMemory@@YIXHH@Z
    ?compressedStreamEOF@@YIXXZ
    ?compressing@@3HA
    ?congratsText@@3PADA
    ?copyOffsetWords@@YIXXZ>
    ?coreRatio@@3PAGA
    ?costTemp@@3PAHA
    ?crc32Table@@3PAIA
    ?crcError@@YIXII@Z
    ?dcoID@@3KA2
    ?digitalDriverNames@@3PAPADA
    ?doorPos@@3PAY01GA
    ?dpEvaluateMessage@@YIXKH@Z"
    ?dpProcessMessages@@YIXXZ
    ?dpSendMessage@@YIXHEGPAX@Z*
    ?dpWaitForExtraGuests@@YIHXZ
    ?dpWaitForFirstGuest@@YIHXZ
    ?dpWaitForHost@@YIHXZ
    ?dphEvent@@3PAXA
    ?dpnet_init@@YIFXZ
    ?dpnet_rcv@@YIFFGPAX@Z
    ?dpnet_sess@@YAFHHZZ
    ?dpnet_snd@@YIHHHPAX@Z
    ?dpnet_stat@@YIEFG@Z
    ?dpnet_term@@YIXXZ
    ?dumpAllModelStats@@YIXXZ
    ?endsInBz@@YIHPAD@Z.
    ?fBerserkFactor@@3MA
    ?fFirstWeekTownFV@@3MA
    ?fGutterMinY@@3MA
    ?fGutterTravelLength@@3MA
    ?fReduceFactor@@3MA*
    ?fTradingPostEfficency@@3PAMA
    ?fTradingPostEfficiency@@3MA
    ?ffffff
    ?ftab@@3PAHA
    ?fullGt@@YIHHH@Z
    ?gAPanelHelp@@3PAPADA
    ?gAdvDisposeLevel@@3HA
    ?gAdvMenuHelp@@3PAPADA
    ?gAlignmentNames@@3PAPADA
    ?gArmyEffected@@3PAY0BE@CA
    ?gArmyNames@@3PAPADA
    ?gArmyNamesPlural@@3PAPADA
    ?gArmyShortNames@@3PAPADA
    ?gArmySizeNames@@3PAY02PADA"
    ?gArtifactBaseRV@@3PAHA"
    ?gArtifactDesc@@3PAPADA
    ?gArtifactEvent@@3PAPADA
    ?gArtifactLevel@@3PAEA
    ?gArtifactNames@@3PAPADA
    ?gBuildingInfoSpecial@@3PAPADA
    ?gBuyBuildIcons@@3PAVicon@@A
    ?gCPanelHelp@@3PAPADA
    ?gCSPanelHelp@@3PAPADA
    ?gCastleAmounts@@3PAFA
    ?gCastleResources@@3PACA
    ?gColorTableDarkBrown@@3PAEA
    ?gColorTableDarkGray@@3PAEA.
    ?gColorTableGray@@3PAEA.
    ?gColorTableLighten@@3PAEA
    ?gColorTableNoCycle@@3PAEA
    ?gColorTableRed@@3PAEA
    ?gColorTableScenWin@@3PAEA
    ?gColorTableTan@@3PAEA
    ?gColorTableYellow@@3PAEA
    ?gColors@@3PAPADA
    ?gCombatFxNames@@3PAPADA
    ?gConfig@@3UconfigStruct@@A*
    ?gCurLoadedSpellEffect@@3HA"
    ?gCurSpellEffectFrame@@3HA
    ?gCyclePal@@3PACA
    ?gDwellingCosts@@3PAY1M@6HA&
    ?gDwellingNames@@3PAY0M@PADA
    ?gDwellingType@@3PAY0M@CA
    ?gEstatesGoldLevel@@3PAHA
    ?gEventHandle@@3PAXA
    ?gEventText@@3PAPADA
    ?gFileRequestHelp@@3PAPADA
    ?gFullMapName@@3PADA
    ?gGameCommand@@3HA
    ?gHeroDefaultNames@@3PAPADA"
    ?gHeroGoldCost@@3HA*
    ?gHeroSkillBonus@@3PAY113CA
    ?gHierarchyMask@@3PAY0M@KA
    ?gIn_addrIP@@3Uin_addr@@A
    ?gIndex@@3HA
    ?gInitMenuHelp@@3PAPADA"
    ?gInitResourcesHuman@@3PAY06HA
    ?gInterfaceTypeText@@3PAPADA
    ?gLastFilename@@3PADA
    ?gLowPage@@3PAXA
    ?gLowPageScreenSelector@@3HA
    ?gLuckText@@3PAPADA
    ?gMageBaseResourceValues@@3PAHA.
    ?gMageBuildingCosts@@3PAY06HA
    ?gMapColors@@3PAEA
    ?gMapName@@3PADA
    ?gMapX@@3HA
    ?gMapY@@3HA&
    ?gMinExpForLevel@@3PAFA6
    ?gMineCharacteristics@@3PAHA
    ?gMonthNames@@3PAPADA
    ?gMoraleText@@3PAPADA
    ?gNeutralBuildingNames@@3PAPADA2
    ?gNewGameHelp@@3PAPADA
    ?gObjectColors@@3PAEA
    ?gOwnerColors@@3PAEA
    ?gPalette@@3PAVpalette@@A
    ?gQuickViewText@@3PAPADA
    ?gResourceBaseValue@@3PAHA
    ?gResourceNames@@3PAPADA
    ?gSPanelHelp@@3PAPADA
    ?gSSValues@@3PAY02HA
    ?gSecondarySkillLevels@@3PAPADA*
    ?gSecondarySkills@@3PAPADA
    ?gSetupBaudHelp@@3PAPADA
    ?gSetupComPortHelp@@3PAPADA
    ?gSetupDCBaudHelp@@3PAPADA
    ?gSetupDCComPortHelp@@3PAPADA
    ?gSetupDCGameHelp@@3PAPADA
    ?gSetupGameHelp@@3PAPADA
    ?gSetupHotSeatGameHelp@@3PAPADA.
    ?gSetupModemGameHelp@@3PAPADA
    ?gSetupNetworkGameHelp@@3PAPADA&
    ?gShingleAnim@@3PAVicon@@A
    ?gSpecialBuildingNames@@3PAPADA&
    ?gSpellDesc@@3PAPADA
    ?gSpellLimits@@3PACA
    ?gSpellNames@@3PAPADA
    ?gStartingHeroStats@@3PAY04EA
    ?gStatDesc@@3PAPADA"
    ?gStatNames@@3PAPADA
    ?gSystemIcons@@3PAVicon@@A
    ?gTerrainNames@@3PAPADA2
    ?gText@@3PADA
    ?gTilesetFiles@@3PAPADA&
    ?gTimeMark@@3KA
    ?gTownEligibleBuildMask@@3PAKA
    ?gTownObjNames@@3PAPADA6
    ?gTownPrefixNames@@3PAPADA
    ?gUltArtifactAvgValue@@3HA
    ?gVesaMode@@3PAFA
    ?gWeekNames@@3PAPADA
    ?gWellExtraNames@@3PAPADA
    ?gWinSetup@@3PAUSWinSetup@@A
    ?g_lpGuid@@3PAU_GUID@@A
    ?gafAITurnCostResource@@3PAMA
    ?gaiEnemyHeroReachable@@3PACA
    ?gaiHeroLiveChance@@3PAFA
    ?gaiHeroStrategicRVOfPos@@3PAFA&
    ?gaiLiveChanceOfPos@@3PAFA
    ?gaiTurnValueOfMine@@3PACA
    ?gbActualBoatFound@@3HA&
    ?gbActualShipyardFound@@3HA2
    ?gbAllBlack@@3HA
    ?gbAllowTextEntryEscape@@3HA
    ?gbAllowUpgrade@@3HA
    ?gbArrow@@3PAY07CA
    ?gbBerserk@@3HA
    ?gbBlackoutPlayer@@3HA
    ?gbBothMachinesWin95@@3HA
    ?gbCampaignSideChoice@@3HA
    ?gbCheatMenus@@3HA
    ?gbClosingApp@@3HA
    ?gbColorMice@@3HA
    ?gbCombatSurrender@@3CA"
    ?gbComputeExtent@@3HA
    ?gbCurrArmyDrawn@@3HA
    ?gbDDrawAttached@@3HA
    ?gbDirectConnect@@3HA
    ?gbDismissArmy@@3HA
    ?gbDoMemCheck@@3HA
    ?gbDoModemConfig@@3HA
    ?gbDontTryDigital@@3HA
    ?gbDontTryMIDI@@3HA
    ?gbDontTryRedbook@@3HA
    ?gbDrawSavedCursor@@3HA&
    ?gbDrawWindowBackground@@3HA
    ?gbDrawingPuzzle@@3HA
    ?gbEnlargeScreenBlit@@3HA
    ?gbEveryOtherCycle@@3HA.
    ?gbFirstTimeThrough@@3HA
    ?gbForceUpdate@@3HA
    ?gbForegroundApp@@3HA
    ?gbFullCombatScreenDrawn@@3HA
    ?gbFunctionComplete@@3HA
    ?gbGameInitialized@@3HA.
    ?gbGameOver@@3HA
    ?gbGamePosToNetPos@@3PACA
    ?gbGenieHalf@@3HA
    ?gbGotFirstHeartbeat@@3HA
    ?gbHeroMoving@@3HA
    ?gbHeroWindShowing@@3HA"
    ?gbHitEvent@@3HA
    ?gbHumanPlayer@@3PAHA
    ?gbIAmGreatest@@3HA6
    ?gbIconClipOn@@3HA
    ?gbInCampaign@@3HA
    ?gbInCombat@@3HA
    ?gbInDialog@@3HA
    ?gbInDrawSmallView@@3HAB
    ?gbInMemError@@3HA
    ?gbInNetSetup@@3CA
    ?gbInNewGameSetup@@3HA
    ?gbInPollSound@@3HA>
    ?gbInRemoteCleanup@@3HA
    ?gbInRemoteMain@@3HA
    ?gbInSetPointer@@3HA
    ?gbInSetupDialog@@3HA
    ?gbInSmackMgr@@3HA
    ?gbInViewWorld@@3HA2
    ?gbLastFramePlayed@@3HA&
    ?gbLeaveNetBoxAlone@@3HA
    ?gbLimitToExtent@@3HA
    ?gbLoadingMonoIcon@@3HA*
    ?gbLowMemory@@3HA
    ?gbLowPageGrabbed@@3HA
    ?gbMinimized@@3HA
    ?gbMoveShown@@3HA
    ?gbNGColor@@3PACA
    ?gbNGDifficulty@@3PACA
    ?gbNGHeroType@@3PACA
    ?gbNGHuman@@3PACA
    ?gbNGPlayerPos@@3PACA
    ?gbNewGameDialogOver@@3HA
    ?gbNewGameShadowHidden@@3HA*
    ?gbNoBorder@@3HA
    ?gbNoCDRom@@3HA&
    ?gbNoDialogMenusOn@@3HA.
    ?gbNoDismiss@@3HA
    ?gbNoShowCombat@@3HA
    ?gbNoSound@@3HA
    ?gbPlayedThrough@@3HA
    ?gbPossibleShipyardFound@@3HA
    ?gbProcessingCombatAction@@3HA
    ?gbPutzingWithMouseCtr@@3HA"
    ?gbReduceByBerserk@@3HA&
    ?gbReduceByReload@@3HA
    ?gbRemoteGameOpen@@3HA
    ?gbRemoteOn@@3HA
    ?gbRetreatWin@@3CA
    ?gbSaveBiggestExtent@@3HA
    ?gbSendMouseMoveMessages@@3HA
    ?gbShowAllMaps@@3HA"
    ?gbShowHighScore@@3HA
    ?gbShowingLoseWindow@@3HA
    ?gbSkipIntro@@3HA
    ?gbTCPFirstTime@@3HA
    ?gbTextEntryEscaped@@3HA
    ?gbThisNetHasControl@@3HA
    ?gbThisNetHumanPlayer@@3PACA
    ?gbTroopReload@@3HA
    ?gbUpgradeArmy@@3HA*
    ?gbUseDiffCompression@@3DA
    ?gbUseEvilInterface@@3HA
    ?gbUseRegularCompression@@3DA
    ?gbUseWaveout@@3HA
    ?gbWaitForRemoteReceive@@3HA
    ?gbWhichAnimationPlaying@@3HA
    ?gbWinGAttached@@3HA
    ?gbWinGraphBusy@@3HA
    ?gbmOldMonoBitmap@@3PAXA
    ?gcAnimPath@@3PADA
    ?gcBotViewText@@3PADA
    ?gcBottomViewText@@3PADA
    ?gcColorToPlayerPos@@3PACA
    ?gcColorToSetupPos@@3PACA
    ?gcCommandLine@@3PADA
    ?gcCommandLineHelp@@3PAPADA
    ?gcCurMapName@@3PADA
    ?gcGamePath@@3PADA
    ?gcMapPath@@3PADA
    ?gcRegAppPath@@3PADA
    ?gcRegCDRomPath@@3PADA
    ?gcSpellInfluenceIcons@@3PAEA
    ?gcTCPAddress@@3PADA
    ?gcTCPName@@3PADA
    ?gcThisNetName@@3PADA
    ?gcWinText@@3PADA
    ?getFinalCRC@@YIIXZ
    ?getGlobalCRC@@YIIXZ
    ?getInt32@@YIHPAUBitStream@@@Z
    ?getMTFVal@@YIHPAUBitStream@@@Z2
    ?getRLEpair@@YIHPAU_iobuf@@@Z
    ?getUChar@@YIEPAUBitStream@@@Z
    ?getUInt32@@YIIPAUBitStream@@@Z*
    ?gfAttackComputerBonus@@3MA
    ?gfAttackHumanBonus@@3MA
    ?gfBattleStat@@3PAMA
    ?gfCancelDurationMods@@3PAMA
    ?gfCombatSpeedMod@@3PAMA
    ?gfDurationMods@@3PAMA
    ?gfHeroInteractionBonus@@3PAMA
    ?gfPhilAIDurationMod@@3PAMA*
    ?gfPhilAISpellPowerMod@@3PAMA
    ?gfSSAIArcheryMod@@3PAMA
    ?gfSSArcheryMod@@3PAMA
    ?gfSSLogisticsMod@@3PAMA
    ?gfSSNavigationMod@@3PAMA
    ?gfSpellCastNumMod@@3PAMA
    ?gfSpellTypeNumMod@@3PAMA
    ?gfStatPower@@3PAMA"
    ?gheroWin@@3PAVheroWindow@@A
    ?giBestShipyardDist@@3HA
    ?giBestShipyardId@@3HA
    ?giBottomViewOverride@@3HA
    ?giBottomViewResource@@3HA
    ?giBottomViewResourceQty@@3HA
    ?giBuildBoat@@3PACA
    ?giBuildBoatStuffTurn@@3PACA
    ?giBuildShipyard@@3PACA*
    ?giChangeThreshold@@3PAEA
    ?giCheatSeq@@3HA
    ?giCloudType@@3PAEA*
    ?giCurAIHeroLuck@@3HA
    ?giCurAIHeroMorale@@3HA"
    ?giCurExe@@3HA
    ?giCurGeneral@@3HA
    ?giCurPlayer@@3HA
    ?giCurPlayerBit@@3EA
    ?giCurTempMobility@@3HA&
    ?giCurTurn@@3HA"
    ?giCurWatchPlayer@@3HA
    ?giCurWatchPlayerBit@@3EA
    ?giCurWindowsStyleFlags@@3HA
    ?giCurrSpellGroup@@3HA
    ?giCycleType@@3HA
    ?giDebugBuildingToBuild@@3HA
    ?giDebugLevel@@3HA
    ?giDeferObjDrawX@@3HA
    ?giDeferObjDrawY@@3HA
    ?giDialogTimeout@@3HA
    ?giEndSequence@@3HA&
    ?giForceSwitchMusic@@3HA
    ?giFrameCount@@3HA
    ?giFrameStep@@3HA
    ?giFullySeeded@@3HA
    ?giGraphicsType@@3HA
    ?giGroundShape@@3PAEA
    ?giGroundToTerrain@@3PAEA
    ?giHeroScreenSrcIndex@@3HA
    ?giHighMemBuffer@@3HA
    ?giHighScoreRank@@3HA
    ?giHighScoreType@@3HA
    ?giHostAcceptStatus@@3HA
    ?giHumanTownConquered@@3HA
    ?giLastConfirm@@3HA"
    ?giLastHourGlassUpdateTime@@3HA
    ?giLimitPlayer@@3HA&
    ?giLimitUpdMaxX@@3HA
    ?giLimitUpdMaxY@@3HA
    ?giLimitUpdMinX@@3HA
    ?giLimitUpdMinY@@3HA
    ?giMainVideoModeColorDepth@@3HA
    ?giMainVideoModeHeight@@3HA2
    ?giMainVideoModeWidth@@3HA
    ?giMapChangeCtr@@3HA
    ?giMapSizeFilter@@3HA
    ?giMapSizes@@3PAEA
    ?giMaxExtentX@@3HA
    ?giMaxExtentY@@3HA
    ?giMaxHeroesForThisPlayer@@3HA
    ?giMenuCommand@@3HA.
    ?giMinExtentX@@3HA
    ?giMinExtentY@@3HA
    ?giMonType@@3PACA
    ?giMonoIconSkip@@3HA
    ?giMonthType@@3HA
    ?giMonthTypeExtra@@3HA
    ?giNetPosToDCOPos@@3PAHA
    ?giNewMonsterCycleFrame@@3HA
    ?giNextAction@@3HA
    ?giNextActionExtra@@3HA
    ?giNextActionGridIndex2@@3HA
    ?giNextActionGridIndex@@3HA6
    ?giNumHumanPlayers@@3HA&
    ?giNumPowFrames@@3PAEA
    ?giOverviewItems@@3PAHA
    ?giOverviewReturnAction@@3HA
    ?giOverviewTop@@3PAHA
    ?giOverviewType@@3HA
    ?giPixelsPerStep@@3PAHA
    ?giRandomClouds@@3HA
    ?giResExtra1@@3HA
    ?giResExtra2@@3HA
    ?giResType1@@3HA
    ?giResType2@@3HA
    ?giScore@@3HA
    ?giScoreCampaignMon@@3PAY01FA
    ?giScoreMon@@3PAY01FA
    ?giScreenScroll@@3HA
    ?giScrollX@@3HA
    ?giScrollY@@3HA
    ?giSeed@@3HA
    ?giSeedingValid@@3HA
    ?giSetupGameType@@3EA
    ?giShowClouds@@3HA
    ?giShowComputerRoute@@3HA
    ?giShowIntro@@3HA
    ?giSkeletonsCreated@@3HA
    ?giSpellEffectShowType@@3HA"
    ?giSpellInfluenceToSpell@@3PAEA
    ?giStepDelay@@3PAHA
    ?giSurrenderCost@@3HA
    ?giTCPHostStatus@@3HA
    ?giTCPNumPlayers@@3HA
    ?giTCPType@@3HA>
    ?giTerrainCost@@3PAY131HA
    ?giTerrainToMusicTrack@@3PAEA
    ?giThisGamePos@@3HA&
    ?giThisNetPos@@3HA
    ?giTotalHighMem@@3HA
    ?giTotalMemAllocated@@3HA
    ?giTownPortalChoice@@3HA
    ?giTtlBlts@@3HA"
    ?giUABaseX@@3FA
    ?giUABaseY@@3FA&
    ?giUARadius@@3FA
    ?giViewWorldScale@@3HA
    ?giViewWorldScaleLookup@@3HA
    ?giVisRange@@3PACA
    ?giVisRangeTown@@3CA
    ?giWaitType@@3HA
    ?giWalkingFrom2@@3HA
    ?giWalkingFrom@@3HA
    ?giWalkingTo2@@3HA
    ?giWalkingTo@@3HA
    ?giWalkingYMod@@3HA"
    ?giWeekType@@3HA
    ?giWeekTypeExtra@@3HA
    ?giWinCmbtFrame@@3HA
    ?glBottomRefresh@@3HA
    ?glCurTicks@@3JA
    ?glLastStartTick@@3JA
    ?glMilliCounter@@3KA
    ?glTimers@@3PAHA
    ?glTotalTicks@@3JA
    ?globalCrc@@3IA2
    ?gpCurAIHero@@3PAVhero@@A
    ?gpCurPlayer@@3PAVplayerData@@A&
    ?gpExec@@3PAVexecutive@@A
    ?gpGame@@3PAVgame@@A
    ?gpHVHero@@3PAVhero@@A
    ?gpInitWin@@3PAVheroWindow@@A
    ?gpMemEntry@@3PAUMemEntry@@A
    ?gpMonGroup@@3PAVarmyGroup@@A
    ?gpPhilAI@@3PAVphilAI@@A
    ?gsSpellInfo@@3PAUSSpellInfo@@A"
    ?gyModify@@3PACA
    ?hDDrawLibrary@@3PAXA
    ?hInstApp@@3PAXA
    ?hMDI@@3PAU_MDI_DRIVER@@A
    ?hMainWindow@@3PAXA&
    ?hMouseCursor@@3PAPAXA
    ?hNewWalkSample@@3PAU_SAMPLE@@A*
    ?hOldWalkSample@@3PAU_SAMPLE@@A&
    ?hRedbookz@@3PAU_REDBOOK@@A"
    ?hSequence@@3PAPAU_SEQUENCE@@A
    ?hWinGLibrary@@3PAXA
    ?hbmpAndMask@@3PAPAXA
    ?hdcImage@@3PAXA
    ?heroWin@@3PAVheroWindow@@A.
    ?hinstDplayx@@3PAXA"
    ?hmnuAdv@@3PAXA"
    ?hmnuApp@@3PAXA*
    ?hmnuCmbt@@3PAXA
    ?hmnuCurrent@@3PAXA*
    ?hmnuDflt@@3PAXA
    ?hmnuRecruitSave@@3PAXA"
    ?hmnuTown@@3PAXA
    ?horseFrameFlip@@3PAFA
    ?hpalApp@@3PAXA
    ?hsbkg.icn
    ?hwndApp@@3PAXA
    ?iAddrLen@@3HA
    ?iAlphaMale@@3HA
    ?iBaud@@3PAHA
    ?iBaudBits@@3HA"
    ?iBusyRetry@@3HA
    ?iCDRomErr@@3HA&
    ?iCalibrateLoop@@3HA
    ?iCampaignTrackType@@3HA
    ?iCombatControlNetPos@@3PAHA
    ?iCombatCycleFrame@@3HA"
    ?iCurBottomView@@3HA
    ?iCurBottomViewEnemy@@3HA
    ?iCurHourGlassPhase@@3HA
    ?iCurLastID@@3HA
    ?iCurPlaceToVisit@@3HA
    ?iCurSwapPalette@@3HA
    ?iCurTransferArtifact@@3HA
    ?iCurViewMap@@3HA
    ?iCurViewSide@@3HA
    ?iCycle1Count@@3HA
    ?iCycle2Count@@3HA
    ?iCycle3Count@@3HA
    ?iDEMaxLen@@3HA&
    ?iDPRcvBufferHead@@3HA
    ?iDPRcvBufferTail@@3HA
    ?iDPWaitForHostStatus@@3HA
    ?iDialogNestCount@@3HA
    ?iDummy@@3HA
    ?iEnumCount@@3HA
    ?iGUIDCount@@3HA
    ?iGetSSByAlignment@@3PAY05EA
    ?iHotSpot@@3PAY01CA
    ?iIDCtr@@3HA
    ?iIRQ@@3PAHA
    ?iInOrder@@3PAHA
    ?iInOrderCtr@@3HA
    ?iInitNetGuestStatus@@3CA
    ?iInitNetHostStatus@@3CA
    ?iLastActionTime@@3HA
    ?iLastAnimFrame@@3HA
    ?iLastBWOnScreenCheck@@3HA
    ?iLastBroadcastTime@@3HA
    ?iLastDialPos@@3HA
    ?iLastDiffSendTo@@3HA
    ?iLastDynamicTop@@3HA
    ?iLastDynamicType@@3HA
    ?iLastFrameRateTimer@@3HA
    ?iLastHereIAmTickCount@@3HA
    ?iLastHourGlassPhase@@3HA
    ?iLastIds@@3PAHA
    ?iLastMsgNumHumanPlayers@@3HA
    ?iLastMusic@@3HA
    ?iLastNewSandAnimTime@@3HA
    ?iLastSandAnimTime@@3HA
    ?iLastScrollTime@@3HA
    ?iLastSeed@@3HA&
    ?iLastVolume@@3PAFA
    ?iLeftRightSave@@3HA
    ?iMPBaseType@@3HA
    ?iMPExtendedType@@3HA
    ?iMPNetProtocol@@3HA
    ?iMainWinScreenHeight@@3HA
    ?iMainWinScreenWidth@@3HA
    ?iMaxListSize@@3HA
    ?iMaxMapExtra@@3HA
    ?iMaxSession@@3HA
    ?iMaxTransferArtifacts@@3HA&
    ?iMaxUnitsToTrade@@3HA
    ?iMemEntries@@3HA
    ?iModemCommandPos@@3HA
    ?iMouseOffset@@3PAHA
    ?iMouseSize@@3PAY01CA
    ?iNameRetryCount@@3HA
    ?iNextShingleAnim@@3HA
    ?iOrigHeroViewID@@3HA
    ?iOverviewItems@@3HA
    ?iPlacesVisited@@3PAY01HA
    ?iQWE@@3HA
    ?iRc@@3HA
    ?iSandAnim@@3HA2
    ?iSaveCtr@@3HA
    ?iSessionToTry@@3HA&
    ?iShingleAnimFrame@@3HA"
    ?iTempX@@3HA
    ?iTempY@@3HA
    ?iThisMaxY@@3HA
    ?iThisMinY@@3HA&
    ?iTimesDropped@@3HA&
    ?iTotalVepHits@@3HA"
    ?iTowerToHexCell@@3PAEA6
    ?iTradeRatio@@3HA
    ?iTransferArtifacts@@3PACA
    ?iTransferArtifactsInfo@@3PACA
    ?iVWCenterOffset@@3HA
    ?iVWDrawAllObjs@@3HA
    ?iVWDrawAllTerrains@@3HA
    ?iVWHalf@@3PAY151CAR
    ?iVWMapOriginX@@3HA
    ?iVWMapOriginY@@3HA.
    ?iVWViewableCells@@3HA
    ?iVWWhatToDraw@@3HA&
    ?iVWXPixelOffset@@3HA
    ?iVWYPixelOffset@@3HA
    ?iVepCacheHits@@3HA
    ?iViewArmyFrame@@3HA
    ?iViewArmyNumTroops@@3HA
    ?iViewArmyType@@3HA&
    ?iViewArmyUpgradeToType@@3HA
    ?iViewGeneralWhichSide@@3HAR
    ?iWSAttempts@@3HA
    ?iWSLastMsgNumHumanPlayers@@3HA
    ?iWSNextTickCount@@3HA
    ?iWSWaitForHostStatus@@3HA
    ?iWaitForGuestStatus@@3CA
    ?iWaitForHostStatus@@3CA
    ?iWaitForHostWaitCount@@3HA&
    ?iWallToHexCell@@3PAEA
    ?idstr@@3PADA
    ?inBoxX@@3HA
    ?inBoxY@@3HA
    ?inName@@3PADA
    ?indexToCastOn@@3HA
    ?inescape@@3HA
    ?initBogusModel@@YIXXZ
    ?initModels@@YIXXZ
    ?initialiseCRC@@YIXXZ
    ?inque@@3Uinque_t@@A
    ?ioError@@YIXXZ:
    ?is_netbios_avail@@YIHXZ
    ?keepInputFiles@@3HA
    ?lDelayRefresh@@3JA&
    ?lLastAilServe@@3JA"
    ?lLastCycleColorsTickCount@@3JA2
    ?lLastGTimerTickCount@@3JA
    ?lLastGetMessage@@3JA
    ?lLastHeartbeatReceive@@3PAJA
    ?lLastHeartbeatSend@@3JA
    ?lLastMIDIPollTickCount@@3JA
    ?lPaintStart@@3JA
    ?lSessions@@3PAJA
    ?lTemp@@3JA&
    ?last@@3HA
    ?lastAggZ@@3PAHA
    ?lastIMHoverID@@3HA
    ?lastPP@@3HA
    ?lastPositionZ@@3PAJA
    ?leftResource@@3HA
    ?ll@@3PAEA
    ?localstage@@3HA
    ?longestFileName@@3HA
    ?lpDD@@3PAUIDirectDraw@@A
    ?lpIDC@@3PAUIDirectPlay@@A
    ?lpInitWin@@3PAXA
    ?lpszReturnString@@3PADA
    ?mapExtra@@3PAEA
    ?maxUnitsToTrade@@3HA
    ?memSelector@@3FA
    ?minUInt32@@YIIII@Z&
    ?moatCell@@3PAEA
    ?models@@3PAUModel@@A
    ?monAnimDrawFrame@@3PAEA
    ?musicQualityText@@3PAPADA
    ?mySignalCatcher@@YAXPAH@Z
    ?nMCIError@@3KA&
    ?nb_thr_ctl@@YIXXZ
    ?nbnet_init@@YIHXZ
    ?newpacket@@3HA&
    ?numbuf@@3PADA
    ?oldmain@@YIHXZ
    ?oldsec@@3HA
    ?onOffText@@3PAPADA
    ?opMode@@3HA
    ?origPtr@@3HA
    ?outName@@3PADA
    ?outque@@3Uoutque_t@@A
    ?overWin@@3PAVheroWindow@@A6
    ?pMIDIWrap@@3PAPAVMIDIWrap@@A
    ?pVWFlags@@3PAVicon@@A
    ?pVWGround@@3PAVicon@@A:
    ?pVWLetters@@3PAVicon@@A
    ?pVWMisc@@3PAVicon@@A
    ?packet@@3PADA
    ?packetlen@@3HAF
    ?panic@@YIXPAD@Z
    ?piDPRcvBufferSize@@3PAHA
    ?ppDPRcvBuffer@@3PAPAEA*
    ?ppMapExtra@@3PAPAXA
    ?progName@@3PADA
    ?progNameReally@@3PADA
    ?ps@@3UtagPAINTSTRUCT@@A
    ?putInt32@@YIXPAUBitStream@@H@Z
    ?putUChar@@YIXPAUBitStream@@E@Z*
    ?puzzlePiecesRemoved@@3PACA"
    ?pwSizeOfMapExtra@@3PAFA
    ?qsortFull@@YIXHH@Z:
    ?qtyToTrade@@3HA
    ?rcTemp@@3UtagRECT@@A
    ?rcvBuf@@3PAPADA
    ?rcvBufIn@@3PADA
    ?rcvBufOut@@3PADA
    ?read_byte@@YIHXZ
    ?remoteidstr@@3PADA*
    ?remotestage@@3HA
    ?rightResource@@3HA*
    ?sCmbtHero@@3PAUSCmbtHero@@A
    ?saddr_loc@@3Usockaddr_in@@A
    ?saddr_remote@@3Usockaddr_in@@A2
    ?screenImage@@3U_IMAGE@@A
    ?setGlobalCRC@@YIXI@Z
    ?shellTrivial@@YIXXZ
    ?showFileNames@@YIXXZ
    ?smallFont@@3PAVfont@@A
    ?smk1@@3PAUSmackTag@@A
    ?smk2@@3PAUSmackTag@@A
    ?smksum@@3USmackSumTag@@A
    ?sndBuf@@3PADA
    ?sortIt@@YIXXZ
    ?speedText@@3PAPADAB
    ?spotBlock@@YIXH@Z
    ?startVals@@3PAHA
    ?stime@@3HA&
    ?stripe@@YIXXZ
    ?szAppName@@3PADA
    ?szTitle@@3PADA
    ?toupper@@YIDD@Z
    ?towerPos@@3PAY01GA&
    ?townTheme@@3PACA
    ?tpWindow@@3PAVheroWindow@@A
    ?tpX@@3HA
    ?tpY@@3HA
    ?trackXY@@3PAY1N@1FA
    ?tradeKnob@@3PAViconWidget@@A
    ?trivialGt@@YIHHH@Z
    ?uDimPal@@3PAY13BAA@EA
    ?uRainbow@@3PAEA
    ?uRedBeam@@3PAEA
    ?unblockError@@YIXXZ
    ?uncompress@@YIXPAD@Z
    ?uncompressOutOfMemory@@YIXHH@Z"
    ?updateModel@@YIXPAUModel@@H@Z
    ?verbose@@3HA
    ?veryVerbose@@3HA
    ?viewArmyBaseX@@3HA
    ?viewArmyBaseY@@3HA6
    ?viewArmyFacingWIPXMod@@3HA*
    ?viewSpellsHero@@3PAVhero@@A
    ?wVer@@3GA
    ?walkSpeedText@@3PAPADA&
    ?wallHex@@3PAEA*
    ?wallPos@@3PAY01GA
    ?words@@3PAIA
    ?write_buffer@@YIHPADH@Z
    ?write_byte@@YIXH@Z*
    ?wsEvaluateMessage@@YIXKH@Z2
    ?wsProcessMessages@@YIXXZ
    ?wsSendMessage@@YIXHEGPAX@Z2
    ?wsWaitForExtraGuests@@YIHXZ
    ?wsWaitForFirstGuest@@YIHXZ.
    ?wsWaitForHost@@YIHXZ
    ?wsadata@@3UWSAData@@A
    ?wsnet_init@@YIFXZ
    ?wsnet_rcv@@YIFFGPAX@Z
    ?wsnet_snd@@YIHHHPAX@Z
    ?wsnet_term@@YIXXZ
    ?xBarrierColor@@3PAPADA*
    ?xCampaign@@3VExpCampaign@@A
    ?xCampaignAwards@@3PAPADA
    ?xGenericSiteNames@@3PAPADA
    ?xHSCampaignNames@@3PAPADA
    ?xIsExpansionMap@@3CA
    ?xJosephName@@3PAPADA
    ?xLastChoice@@3HA
    ?xNecromancerShrine@@3PADA
    ?xNecromancerShrineDesc@@3PADA
    ?xNetHasOldPlayers@@3CA&
    ?xNewHeroAlignment@@3PAEA
    ?xPasswordStrings@@3PAPADA
    ?xPasswordStringsIndex@@3PAHA
    ?xRecruitBuy@@3PAPADA
    ?xRecruitEmpty@@3PAPADA&
    ?xRecruitmentSiteNames@@3PAPADA"
    ?xScenarioName@@3PAY07PADA
    ?xShortCampaignNames@@3PAPADA
    ?xShortSSLevelNames@@3PAPADA
    ?xShrineBuildingCost@@3PAHA
    ?xSmackFromNetwork@@3CA*
    ?xStableText@@3PAPADA
    ?xTheSpell@@3HA&
    ?xUncleIvanName@@3PAPADA
    ?zptr@@3PAHA
    @.data
    @.reloc
    @@ffffff
    @BlitBitmapToScreenVesa@28
    @FPhL\Q
    @FPhT]Q
    @FPht\Q
    @FPh|oO
    @PollSound@0
    @SVWj@j
    @ff6@ffV@
    @nb_init@8
    @nb_rcv@8
    @nb_snd@12
    @nb_stat@4
    @nb_term@0
    A Black Dragon will take out a Titan any day of the week.
    A Horde of
    A Legion of
    A Nomad trader seeks protection from a tribe of Goblins.  For your assistance, he gives you a finely crafted pair of boots made from the softest leather.  Looking closely, you see fascinating ancient carvings engraved on the leather.
    A Swarm of
    A Throng of
    A Troll stops you and says: "Pay me 5,000 gold, or the Sword of Anduran will slay you where you stand."  You refuse.  The troll grabs the sword hanging from its belt, screams in pain, and runs away.  Picking up the fabled sword, you give thanks that half-witted Trolls tend to grab the wrong end of sharp objects.
    A blacksmith working at the foundry offers to convert all Pikemen and Swordsmen's weapons brought to him from iron to steel. He also says that he knows a process that will convert Iron Golems into Steel Golems.  Unfortunately, you have none of these troops in your army, so he can't help you.
    A brief roadside encounter with a small caravan and a game of knucklebones wins a magic pendant.  Its former owner says that it protects from Necromancers' death spells.
    A brief stop at a hastily abandoned Wizard's tower turns up a magical vial of mercury that always has a little left on the bottom.  Recognizing a treasure when you see one, you cap it and slip it in your pocket.
    A brief stop at an improbable rural inn yields an exchange of money, tales, and accidentally, luggage.  You find a magical skullcap in your new backpack.
    A chance meeting with a famous Archer finds you in a game of knucklebones pitting his bow against your horse.  You win.
    A clumsy Giant has killed himself with his own flail.  Knowing your superior skill with this weapon, you confidently remove the spectacular flail from the fallen Giant.
    A colorful Rogues' wagon stands empty here.  Perhaps more Rogues will be here later.
    A cry of pain leads you to a Centaur, caught in a trap.  Upon setting the creature free, he hands you a small pouch.  Emptying the contents, you find a dazzling jeweled ring.
    A dirty shovel has been thrust into a dirt mound nearby. Upon investigation, you discover it to be the enchanted shovel of the Gravediggers, long thought lost by mortals.
    A drink at the oasis fills your troops with strength and lifts their spirits.  You can travel a bit further today.
    A drink at the spring is supposed to give you twice your normal spell points, but you are already at that level.
    A drink at the watering hole fills your troops with strength and lifts their spirits.  You can travel a bit further today.
    A drink at the well is supposed to restore your spell points, but you are already at maximum.
    A drink from the spring fills your blood with magic!  You have twice your normal spell points in reserve.
    A drink from the well has restored your spell points to maximum.
    A face forms in the water for a moment, and then is gone.
    A few Trolls remain, cowering under the bridge.  They approach you and offer to join your forces as mercenaries.  Do you want to buy any Trolls?
    A fierce windstorm reveals the entrance to a buried tomb.  Your investigation reveals that the tomb has already been looted, but the thieves overlooked an ankh on a silver chain in the dark.
    A former Captain of the Guard admires your quest and gives you the enchanted Sword Breaker that he relied on during his tour of duty.
    A gang of Rogues is sifting through the possessions of dead warriors.  Scaring off the scavengers, you note the Rogues had overlooked a beautiful breastplate.
    A generous tip for the barkeep yields the following rumor:
    A glint of golden light catches your eye.  Upon further investigation, you find a golden helm hidden under a bush.
    A glorious victory!
    A group of Archers with a desire for greater glory wish to join you. Do you accept? 
    A group of Centaurs with a desire for greater glory wish to join you. Do you accept? 
    A group of Dwarves with a desire for greater glory wish to join you. Do you accept? 
    A group of Goblins with a desire for greater glory wish to join you. Do you accept?
    A group of Halflings with a desire for greater glory wish to join you. Do you accept? 
    A group of Orcs with a desire for greater glory wish to join you. Do you accept? 
    A group of Peasants with a desire for greater glory wish to join you. Do you accept? 
    A group of Skeletons with a desire for greater glory wish to join you. Do you accept? 
    A group of Sprites with a desire for greater glory wish to join you. Do you accept? 
    A group of tattered tents, billowing in the sandy wind, beckons you.  Do you wish to have any Nomads join you during your travels?
    A group of tattered tents, billowing in the sandy wind, beckons you.  The tents are unoccupied.  Perhaps more Nomads will be here later.
    A kindly Sorceress thinks that your army's defenses could use a magical boost.  She offers to enchant the Broach that you wear on your cloak, and you accept.
    A little leprechaun dances gleefully around a magic sack.  Seeing you approach, he stops in mid-stride.  The little man screams and stamps his foot ferociously, vanishing into thin air.  Remembering the old leprechaun saying 'Finders Keepers', you grab the sack and leave.
    A merchant from far away lands trades you a new invention of his people for traveling supplies.  It makes distant objects appear closer, and he calls it...
    A miniature view of the known world.  Left click to move viewing area.
    A multi-player game, with several human players competing against each other on a single map.
    A noblewoman, separated from her traveling companions, asks for your help.  After escorting her home, she rewards you with a bag filled with gold.
    A pack of
    A quick and deadly battle with a Necromancer wins you his magical pendant.  Later, a Wizard tells you that the pendant protects undead under your control from holy word spells.
    A rain of rocks strikes an area of the battlefield, damaging all nearby creatures.
    A retired captain living on this refurbished fishing platform offers to sell you maps of the sea he made in his younger days for 1,000 gold.  Do you wish to buy the maps?
    A second drink at the well in one day will not help you.
    A short rainstorm brings forth a rainbow...and you can see the end of it.  Riding quickly, you seize the pot of gold you find there.  The leprechaun who owns it, unable to stop you from taking it, offers an endless pouch of gems for the return of his gold.  You accept.
    A signpost reads:}
    A single player game playing out a single map.
    A single player game playing through a series of maps.
    A stone statue of a warrior holds a silver shield.  As you remove the shield, the statue crumbles into dust.
    A surprise turn in the landscape finds you in the midst of a grisly scene:  Vultures picking at the aftermath of a terrible battle.  Your cursory search of the remains turns up an enchanted hourglass.
    A traveling merchant offers you a rabbit's foot, made of gleaming silver fur, for safe passage.  The merchant explains the charm will increase your luck in combat.
    A traveling tinker in need of supplies offers you a helm with a thunderbolt design on its top in exchange for food and water.  Curious, you accept, and later find out that the helm is magical.
    A visit and a prayer at the temple raises the morale of your troops.
    A walk across a dry saltwater lake bed yields an unlikely prize:  A white pearl amidst shattered shells and debris.
    A whirlpool engulfs your ship.  Some of your army has fallen overboard.
    A?ProcessIncomingSingleMapChange@advManager@@QAEXPAUSMapChange@@@Z
    AAABABA
    ADVAPI32.dll
    ADVAPI32_NULL_THUNK_DATA
    ADVMCO.ICN
    APANBKG.ICN
    APANBKGE.ICN
    APANEL.ICN
    APANELE.ICN
    AUTOSAVE
    Aardvark
    Abandoned Mine
    About to T
    About to initiate TCP/IP connection.  Heroes II will now drop from full screen mode to windowed mode, so that any Windows 95 generated dialog boxes can be seen.
    Accept the choice made.
    Add another level to Mage Guild
    Additional hero characteristics
    Adjust Music Volumes 1
    Adjust Music Volumes 2
    Adjust Sound Volumes 1
    Adjust Sound Volumes 2
    AdjustWindowRect
    Advance
    Advanced
    Advanced Archery increases the damage done by range attacking creatures by 25 percent.
    Advanced Ballistics gives your hero's catapult an extra shot, and each shot has a greater chance to hit and do damage to castle walls.
    Advanced Diplomacy allows you to negotiate with monsters who are weaker than your group.  Approximately 1/2 of the creatures may offer to join you.
    Advanced Eagle Eye gives your hero a 30 percent chance to learn any given 3rd level spell (or below) used against him in combat.
    Advanced Leadership +2
    Advanced Leadership increases your hero's troops' morale by 2.
    Advanced Logistics increases your hero's movement points by 20 percent.
    Advanced Luck +2
    Advanced Luck increases your hero's luck by 2.
    Advanced Mysticism regenerates three of your hero's spell points per day.
    Advanced Navigation increases your hero's movement points over water by 2/3.
    Advanced Necromancy allows 20 percent of the creatures killed in combat to be brought back from the dead as Skeletons.
    Advanced Pathfinding reduces the movement penalty for rough terrain by 50 percent.
    Advanced Scouting increases your hero's viewable area by 2 squares.
    Advanced Wisdom allows your hero to learn fourth level spells.
    Advent
    Adventure
    After freeing a princess of a neighboring kingdom from the evil clutches of despicable slavers, she awards you with a Medal of Honor.
    After rescuing a Sorceress from a cursed tomb, she rewards your heroism with an exquisite jeweled necklace.
    After saving a young boy from a vicious pack of Wolves, you return him to his father's manor.  The grateful nobleman awards you with a Medal of Courage.
    After scouring the area, you fall upon a hidden treasure cache.  You may take the gold or distribute the gold to the peasants for experience.  Do you wish to keep the gold?
    After spending hours trying to fish the chest out of the sea, you open it and find 1500 gold pieces.
    After spending hours trying to fish the chest out of the sea, you open it, only to find it empty.
    Ahh, an adventurer! I will be happy to teach you a little of what I have learned over the ages for a mere 10 gems."  (Just bury it around my roots.)
    Air Summoning Altar
    Alamar
    Alchemist Lab
    Alchemist's Tower
    All dwellings increase population.
    All populations are halved.
    All the creatures will join us...
    All the dwarf towns have fallen.  This is a disastrous defeat!  You have lost.
    Allows the caster to magically transport to a nearby location.
    Allows the caster to view detailed information on enemy Heroes.
    Allows the computer to fight out the battle for you.
    Allows you to customize the combat screen.
    Alongside the remains of a burnt witch lies a beautiful broach, intricately designed.  Approaching the corpse with caution, you add the broach to your inventory.
    Ambrose
    Ammo Cart
    An Orcish witch doctor living in the hut deepens your knowledge of magic by showing you how to cast stones, read portents, and decipher the intricacies of chicken entrails.
    An ammunition cart in the middle of an old battlefield catches your eye.  Inspection shows it to be in good working order, so  you take it along.
    An earthquake will do you no good unless there are town walls to damage.
    An ensnared Unicorn whinnies in fright.  Murmuring soothing words, you set her free.  Snorting and stamping her front hoof once, she gallops off.  Looking down you see a golden horseshoe.
    An icicle withstanding the full heat of the noonday sun attracts your attention.  Intrigued, you break it off, and find that it does not melt in your hand.
    An improved version of fireball, fireblast affects two hexes around the center point of the spell, rather than one.
    An old Knight appears on the steps of the gazebo. "I am sorry, my liege, I have taught you all I can."
    An old Knight appears on the steps of the gazebo. "My liege, I will teach you all that I know to aid you in your travels."
    An old man claiming to be an inventor asks you to try his latest invention.  He then hands you a compass.
    An old sea captain is being tortured by Ogres.  You save him, and in return he rewards you with a wondrous instrument to measure the distance of a star.
    An old seaman tells you a tale of an enchanted masthead that he used in his youth to rally his crew during times of trouble.  He then hands you a faded map that shows where he hid it.  After much exploring, you find it stashed underneath a nearby dock.
    An unusual alliance of Orcs, Ogres, and Dwarves offer to train (upgrade) any such troops brought to them.  Unfortunately, you have none with you.
    Ancient Lamp
    Animate
    Animate Dead
    AnimatePalette
    Annexation
    Anti-Magic
    Apocalypse
    AppAbout
    AppPath
    AppWndProc
    Arcane Necklace of Magic
    Archer
    Archer's House
    Archery
    Archery Range
    Archibald
    Are you sure you want to dismiss this Hero?
    Are you sure you want to dismiss this army?
    Are you sure you want to load a new game?  (Your current game will be lost)
    Are you sure you want to quit?
    Are you sure you want to restart this scenario?
    Are you sure you want to restart?  (Your current game will be lost)
    Are you sure you want to retreat?
    Arg list too long
    Arm of the Martyr
    Armageddon
    Armored Gauntlets of Protection
    Armory
    Army Camp
    Arrow's Flight
    Artesian Spring
    Artifact
    Artifact 82.
    Artifact 83.
    Artifact 84.
    Artifact 85.
    Artifacts
    Artifacts:
    As the sirens sing their eerie song, your small, determined army manages to overcome the urge to dive headlong into the sea.
    As you approach the Centaurs' cave you notice that there is no one here.
    As you approach the Orcs' watch tower you notice that there is no one here.
    As you approach the Peasant dwelling you notice that there is no one here.
    As you approach the Skeletons' excavation you notice that there is no one here.
    As you approach the Sprites' tree house you notice that there is no one here.
    As you approach the bubbling pit of mud, creatures begin to climb out and position themselves around it.  In unison they say: "Mother Earth would like to offer you a few of her troops.  Do you want to recruit Earth Elementals?"
    As you approach the dwelling, you notice that there is no one here.
    As you approach the group of elves, their leader calls them all to attention.  He shouts to them, "Who of you is brave enough to join this fearless ally of ours?"  The group explodes with cheers as they run to join your ranks.
    As you approach the habitat you notice that there is no one here.
    As you approach the stables, the head groom appears, leading a fine looking war horse. "This steed will help speed you in your travels. Alas, he will grow tired in a week. You must also let me give better horses to your mounted soldiers, their horses look shoddy and weak."
    As you approach the stables, the head groom appears, leading a fine looking war horse. "This steed will help speed you in your travels. Alas, his endurance will wane with a lot of heavy riding, and you must return for a fresh mount in a week. We also have many fine war horses which could benefit mounted soldiers, but you have none we can help."
    As you are walking along a narrow path, a nearby bush suddenly bursts into flames.  Before your eyes the flames become the image of a beautiful woman.  She holds out a magnificent sword to you.
    As you drink the sweet water, you gain luck for your next battle.
    As you enter the Alchemist's Tower, a hobbled, graying man in a brown cloak makes his way towards you.  He checks your pack, and sees that you have 1 cursed item.  For 750 gold, the alchemist will remove it for you.  Do you pay?
    As you speak the magic word, the glowing barrier dissolves into nothingness.
    Assert Failure
    Assertion failed!
    Astrologers proclaim month of the PLAGUE!
    At last, you have the opportunity and the facilities to rid the Empire of the necromancer's evil. Eradicate them completely, and you will be sung as a hero for all time.
    At least one player does not have the Heroes II Expansion set.  You will only be able to choose from original Heroes II games.
    Attack
    Attack Skill
    Attack Skill: 
    Attack:
    Attack: 
    Attacker
    Attraction
    Aurora Borealis
    Auto Combat
    Auto Combat Use Spells
    Auto combat
    Autosave
    Available
    Available:
    Available: 
    Average
    B?FlipIconToBitmapColorTable@@YIXPAVicon@@PAVbitmap@@HHHHHHHHHPAE@Z
    BADMRLE.82M
    BBCDCDD
    BBDDCDD
    BCDEDEE
    BDEEDEE
    BROKENA
    Backstab
    Bad address
    Bad file descriptor
    Bad luck sometimes falls on your armies in combat, causing their attacks to only do half damage.
    Bad morale may cause your armies to freeze in combat.
    Bad return on Send Data
    Badger
    Ballista
    Ballista of Quickness
    Ballistics
    Barbarian
    Barbarian Coliseum +2
    Barbarian Wars
    Barrow Mounds
    Base Score:
    Basic Archery increases the damage done by range attacking creatures by 10 percent.
    Basic Ballistics gives your hero's catapult shots a greater chance to hit and do damage to castle walls.
    Basic Diplomacy allows you to negotiate with monsters who are weaker than your group.  Approximately 1/4 of the creatures may offer to join you.
    Basic Eagle Eye gives your hero a 20 percent chance to learn any given 1st or 2nd level enemy spell used against him in a combat.
    Basic Leadership +1
    Basic Leadership increases your hero's troops' morale by 1.
    Basic Logistics increases your hero's movement points by 10 percent.
    Basic Luck +1
    Basic Luck increases your hero's luck by 1.
    Basic Mysticism regenerates two of your hero's spell points per day.
    Basic Navigation increases your hero's movement points over water by 1/3.
    Basic Necromancy allows 10 percent of the creatures killed in combat to be brought back from the dead as Skeletons.
    Basic Pathfinding reduces the movement penalty for rough terrain by 25 percent.
    Basic Scouting increases your hero's viewable area by 1 square.
    Basic Wisdom allows your hero to learn third level spells.
    Battle
    Battle Garb
    Battle Garb of Anduran
    Battle Garb of Anduran gives you maximum luck.
    Battle Garb of Anduran gives you maximum morale.
    Battlefield Casualties
    Because of this problem running with CD stereo music, Heroes II has been configured to run with MIDI music in the future.  You can always manually change this setting in the control panel within the game.
    Beetle
    Before you appears a levitating glass case with a scroll, perched upon a bed of crimson velvet.  At your touch, the lid opens and the scroll floats into your awaiting hands.
    Begin Transmit Init Confirm
    BeginPaint
    Beneath a structure that serves to hold in heat, Fire Elementals move about in a fiery pool of molten lava.  A group of them approach you and offer their services.  Would you like to recruit Fire Elementals?
    Berserk
    Berzerker
    Best Hero Stats:
    Best Hero:
    Best Monster:
    BestBHC 
    Betrayal
    Betrayal's End
    Black & White
    Black Pearl
    Black Tower
    Blackout Computer
    Blacksmith
    Blazing
    Blind 
    Blood Lust
    Blood is Thicker
    Blood!
    Book of Elements
    Border
    Border Towns
    Bottle
    Breastplate
    Breastplate of Anduran
    Bridge
    Bring up the adventure options menu.
    Bring up the file options menu, allowing you to load, save etc.
    Bring up the system options menu, allowing you to customize your game.
    Brings a single enemy unit under your control for one combat round if its hits are less than 25 times the caster's spell power.
    Broach of Shielding
    Broken pipe
    Brother Brax
    Buckler
    Build Mage Guild
    Build a new ship:
    Build improvement:
    Builder
    Bumblebee
    Buoy visited +1
    Burma shave
    Butterfly
    C?ComputerMonsterInteract@advManager@@QAEXPAVmapCell@@PAVhero@@PAH@Z"
    CBKGBEAC.icn
    CBKGCRCK.icn
    CBKGDIMT.icn
    CBKGDITR.icn
    CBKGDSRT.icn
    CBKGGRAV.icn
    CBKGGRMT.icn
    CBKGGRTR.icn
    CBKGLAVA.icn
    CBKGSNMT.icn
    CBKGSNTR.icn
    CBKGSWMP.icn
    CBKGWATR.icn
    CBf9C@~
    CD MUSIC ERROR
    CD Stereo w/o Opera
    CD Stereo with Opera
    CDDrive
    CDEFEFF
    CDFGEGF
    CDFGEGG
    CDROM drive error.  Exiting.
    CDf9CF~
    CHOOSE
    CHOOSEW
    CMSECO.ICN
    CONNECT
    CPANBKG.ICN
    CPANBKGE.ICN
    CPANEL.ICN
    CPANELE.ICN
    CREDITS
    CSPANBKE.ICN
    CSPANBKG.ICN
    CSPANBTE.ICN
    CSPANBTN.ICN
    CV$build\intel\st_obj\oldnames\open.obj
    CV$build\intel\st_obj\oldnames\read.obj
    CV$build\intel\st_obj\oldnames\tell.obj
    CV%build\intel\st_obj\oldnames\close.obj
    CV%build\intel\st_obj\oldnames\lseek.obj
    CV%build\intel\st_obj\oldnames\write.obj
    CV&build\intel\st_obj\oldnames\access.obj
    CV&build\intel\st_obj\oldnames\strrev.obj
    CV'build\intel\st_obj\oldnames\strcmpi.obj
    CV'build\intel\st_obj\oldnames\stricmp.obj
    CV(build\intel\st_obj\oldnames\strnicmp.obj
    CYBCREDS
    CYLOGO
    Cactus
    Campfire
    Can't add manager!
    Can't create boat!
    Can't load 'DPLAYX.DLL'
    Cancel
    Cancel back to main menu.
    Cancel back to the main menu.
    Cancel without making a choice.
    Cannot afford a Hero.
    Cannot afford next level.
    Cannot build another boat.
    Cannot build.  Already built here this turn.
    Cannot combine hero's last army
    Cannot move last army to garrison.
    Cannot recruit - you already have a Hero in this town.
    Canter
    Captain
    Captain's Options
    Captain's Quarters
    Capture the town on the island off the southeast shore in order to construct a boat and travel back towards the mainland. Do not lose the hero Gallavant.
    Carator Mines
    Carlawn
    Carry over forces
    Cast Spell
    Cast a magical spell.  You may only cast one spell per combat round.  The round is reset when every creature has had a turn.
    Cast an adventure spell.
    Caster's Bracelet of Magic
    Castle
    Castle Options
    Catapult
    Cathedral
    Cauldron
    Causes a bolt of electrical energy to strike a selected creature, then strike the nearest creature with half damage, then strike the NEXT nearest creature with half again damage, and so on, until it becomes too weak to be harmful.  Warning:  This spell can hit your own creatures!
    Causes a bolt of electrical energy to strike the selected creature.
    Causes a creature to attack its nearest neighbor.
    Causes a giant fireball to strike the selected area, damaging all nearby creatures.
    Causes a magic arrow to strike the selected target.
    Causes all Heroes across the land to become visible.
    Causes all artifacts across the land to become visible.
    Causes all enemy troops to inflict minimum damage.
    Causes all mines across the land to become visible.
    Causes all of your units to inflict maximum damage.
    Causes all resources across the land to become visible.
    Causes all towns and castles across the land to become visible.
    Causes the entire land to become visible.
    Causes the selected creatures to inflict maximum damage.
    Causes the selected creatures to inflict minimum damage.
    Cavalry
    Centaur
    Chain Lightning
    Champion
    Change the speed at which your heroes move on the main screen.
    Change the type of music.  MIDI music does not usually sound as good, but hurts performance less than CD Stereo.  If you use CD Stereo, you can select whether or not to have the operatic town themes play or not.
    Change your direct connect port configuration.
    Change your modem configuration.
    Chaos.mp2
    Charity
    Cheater!!!
    CheckBuy End  
    CheckBuy Start
    CheckMenuItem
    City of the Dead
    Class:
    Clear communications break
    Clear communications error queue
    ClearCommBreak
    ClearCommError
    Click here to select which scenario to play.
    Click to accept these settings and start a new game.
    Click to return to the main menu.
    ClientToScreen
    Cliff Nest
    Close Spellbook
    CloseHandle
    Cloud Castle
    Clouds the affected creatures' eyes, preventing them from moving.
    Coffin
    Cold Ray
    Cold Ring
    Coliseum
    Combat Army Info Level
    Combat Shade Level
    Combat Speed
    Command
    Common Modem 1
    Common Modem 2
    Common Modem 3
    Common Modem 4
    Common Modem 5
    Common Modem 6
    Common Modem 7
    Computer Walk Speed
    Conditions
    Condor
    Configure communications device
    Congratulations!
    Congratulations! After spending many hours digging here, you have uncovered the 
    Conquer and Unify
    Conquer and unite all the enemy tribes. Don't lose the hero Jarkonas, the forefather of all descendants.
    Continue the Hero's movement along the current path.
    Control
    Coordinates at top left corner of view:
    Corlagon defeated
    Corlagon's Defense
    Corruption's Heart
    Cottage
    Country Lords
    Crag Hack
    Crater
    Crawling
    Crazy Uncle Ivan
    Create
    CreateBitmapIndirect
    CreateEventA
    CreateFileA
    CreateIconIndirect
    CreatePalette
    CreateWindowExA
    Creates an illusionary unit that duplicates one of your existing units.  This illusionary unit does the same damages as the original, but will vanish if it takes any damage.
    Crossbow
    Crystal
    Crystal Ball
    Crystal Garden
    Crystalline structures cast shadows over a small reflective pool of water.  You peer into the pool, and a face that is not your own peers back.  It asks: "Would you like to call upon the powers of water?"
    Current Luck Modifiers:
    Current Map Offset
    Current Morale Modifiers:
    Cursed
    Cursor
    Customize system options.
    Cyclops
    D$ RPj&
    D$$QRPj&j
    D$(RPj&j
    D$4PQj
    D$8jdPW
    D?ChooseEvaluateBattle@philAI@@QAEXPAVarmyGroup@@PAVhero@@01HHHAAH2@Z
    D?DoCombat@advManager@@QAEHHHPAVhero@@PAVarmyGroup@@PAVtown@@01HHHH@Z
    D?ProcessDeSelect@advManager@@QAEHPAUtag_message@@PAHPAPAVmapCell@@@Z
    DAjPhPcO
    DDERR_CLIPPERISUSINGHWND 
    DDERR_GENERIC            
    DDERR_IMPLICITLYCREATED  
    DDERR_INCOMPATIBLEPRIMARY
    DDERR_INVALIDCLIPLIST
    DDERR_INVALIDOBJECT      
    DDERR_INVALIDPARAMS
    DDERR_INVALIDRECT
    DDERR_LOCKEDSURFACES     
    DDERR_NOALPHAHW          
    DDERR_NOBLTHW
    DDERR_NOCLIPLIST         
    DDERR_NOCLIPPERATTACHED  
    DDERR_NODDROPSHW
    DDERR_NOEXCLUSIVEMODE
    DDERR_NOMIRRORHW         
    DDERR_NOPALETTEATTACHED  
    DDERR_NOPALETTEHW
    DDERR_NORASTEROPHW
    DDERR_NOROTATIONHW       
    DDERR_NOSTRETCHHW
    DDERR_NOT8BITCOLOR       
    DDERR_NOZBUFFERHW        
    DDERR_OUTOFMEMORY
    DDERR_SURFACEBUSY        
    DDERR_SURFACELOST        
    DDERR_UNSUPPORTED
    DDERR_WRONGMODE          
    DDRAW.DLL
    DEFEAT
    DHjPhPWO
    DHjPhTWO
    DIGSOUND.82M
    DOMAIN error
    DPERR_ACCESSDENIED       
    DPERR_ACTIVEPLAYERS      
    DPERR_ALREADYINITIALIZED 
    DPERR_BUFFERTOOSMALL     
    DPERR_BUSY               
    DPERR_CANTADDPLAYER      
    DPERR_CANTCREATEGROUP    
    DPERR_CANTCREATEPLAYER   
    DPERR_CANTCREATESESSION  
    DPERR_CAPSNOTAVAILABLEYET
    DPERR_EXCEPTION          
    DPERR_GENERIC            
    DPERR_INVALIDFLAGS       
    DPERR_INVALIDOBJECT      
    DPERR_INVALIDPARAMS      
    DPERR_INVALIDPLAYER      
    DPERR_NOCAPS             
    DPERR_NOCONNECTION       
    DPERR_NOMESSAGES         
    DPERR_NONAMESERVERFOUND  
    DPERR_NOPLAYERS          
    DPERR_NOSESSIONS         
    DPERR_OUTOFMEMORY        
    DPERR_SENDTOOBIG         
    DPERR_TIMEOUT            
    DPERR_UNAVAILABLE        
    DPERR_UNSUPPORTED        
    DPERR_USERCANCEL         
    DPLAYX.DLL
    DPMSGSTARTUP
    DSVWUh
    Daemon Cave
    Dagger
    Damage
    Damage: 
    Damages all living (non-undead) units in the battle.
    Damages all living (non-undead) units in the battle.  This spell is an improved version of Death Ripple.
    Damages all undead in the battle.
    Damages all undead in the battle.  This is an improved version of Holy Word.
    Damages castle walls.
    Damg: 
    Darkness
    Darlana
    Days Spent:
    Dead Player
    Dead Tree
    Death Ripple
    Death Wave
    Decimate
    Decisive
    DefWindowProcA
    Default Construct
    Defeat
    Defeat all enemy heroes and capture all enemy towns and castles.
    Defender
    Defender Helm
    Defender Helm of Protection
    Defenders:
    Defense
    Defense Skill
    Defense Skill: 
    Defense:
    Defense: 
    DeleteCriticalSection
    DeleteDC
    DeleteFileA
    DeleteObject
    Derelict Ship
    Derelict ship robber -1
    Descendants
    Desert
    Desert Tent
    Destroy
    Destroy the barbarians who are attacking the southern border of your kingdom! Recover your fallen towns, and then invade the jungle kingdom. Leave no enemy standing.
    DestroyIcon
    DestroyMenu
    DestroyWindow
    Determines if the video sequences play normally or in interlaced mode.  Interlaced mode runs better on slower machines, or machines with double-speed CD drives.
    Devastation
    Deviate
    Dialing...
    DialogBoxParamA
    Difficulty
    Difficulty Rating:
    Dig for the Ultimate Artifact.
    Digging for artifacts requires a whole day, try again tomorrow.
    Digital sound is not currently available on this system.
    Dimension
    Dimension Door
    Dimension Door failed!!!
    Dimitri
    Diplomacy
    Direct Connect Baud Rate
    Direct Connect Com Port
    DirectDraw Error:
    DirectDrawCreate
    DirectPlay Error:
    DirectPlayCreate
    DirectPlayEnumerateA
    Directory not empty
    Discover
    Discovering a pair of beautifully beaded boots made from the finest and softest leather, you thank the anonymous donor and add the boots to your inventory.
    Discretion is the better part of valor, and you decide to avoid this fight for today.
    DispatchMessageA
    Dispel Magic
    Disrupting Ray
    Disruption
    Distance
    Distant sounds of music and laughter draw you to a colorful wagon housing Rogues.  Do you wish to have any Rogues join your army?
    Divine Breastplate of Protection
    Do you wish to enter each player's name?
    Do-gooder wizards have taken the Necromancers' castle.  You must retake it to achieve victory.  Remember that while you start with a powerful army, you have no castle and must take one within 7 days, or lose this battle.  (Hint: The nearest castle is to the southeast.)
    Dock with boat
    Domain error
    Don't Show
    Dragon
    Dragon City
    Dragon Master
    Dragon Slayer
    Dragon Sword
    Dragon Sword of Dominion
    Dragon alliance
    Dragon city has fallen!  You are now the Master of the Dragons.
    Dragonfly
    Drains body heat from a single enemy unit.
    Drains body heat from all units surrounding the center point, but not including the center point.
    DrawMenuBar
    Dungeon
    Duplicate ID Strings!
    During a sudden storm, a bolt of lightning strikes a tree, splitting it.  Inside the tree you find a mysterious mace.
    Dwarf Cabin
    Dwarf Cottage
    DwarfBane
    Dwarven
    Dwarven alliance
    Dwelling
    Dynamic
    Dynamic Construct
    E @Pj]jRj
    E Pj]jej
    EEEEEEEEEEE
    EFGHI<0
    ERROR : Artifact 82
    ERROR : Artifact 83
    ERROR : Artifact 84
    ERROR : Artifact 85
    ERROR : Artifact event 82.
    ERROR : Artifact event 83.
    ERROR : Artifact event 84.
    ERROR : Artifact event 85.
    ERROR_ACCESS_DENIED       
    ERROR_ALREADY_EXISTS      
    ERROR_ARENA_TRASHED       
    ERROR_BAD_ENVIRONMENT     
    ERROR_BAD_FORMAT          
    ERROR_CURRENT_DIRECTORY   
    ERROR_FILE_NOT_FOUND      
    ERROR_INVALID_ACCESS      
    ERROR_INVALID_BLOCK       
    ERROR_INVALID_DATA        
    ERROR_INVALID_DRIVE       
    ERROR_INVALID_FUNCTION    
    ERROR_INVALID_HANDLE      
    ERROR_NOT_ENOUGH_MEMORY   
    ERROR_NOT_SAME_DEVICE     
    ERROR_NO_MORE_FILES       
    ERROR_PATH_NOT_FOUND      
    ERROR_TOO_MANY_OPEN_FILES 
    EVIL01
    EVIL01V
    EVIL02
    EVIL02W
    EVIL03
    EVIL03QW
    EVIL05
    EVIL05AV
    EVIL05BV
    EVIL06
    EVIL06AW
    EVIL06BW
    EVIL07
    EVIL07W
    EVIL08
    EVIL09
    EVIL09W
    EVIL10
    EVIL11W
    EVIWDDOR.ICN
    EVIWHROS.ICN
    EVIWMINE.ICN
    EVIWPUZL.ICN
    EVIWRSRC.ICN
    EVIWRTFX.ICN
    EVIWTWNS.ICN
    EVIWWRLD.ICN
    EVIW_ALL.ICN
    Eagle Eye
    Earth Summoning Altar
    Earthquake
    Earthworm
    Editor Color Mouse Cursor
    Editor Full Screen
    Editor Height
    Editor Palette Cycling
    Editor Screen Animation
    Editor Show Menu
    Editor Width
    Editor X
    Editor Y
    Effects
    Either Roland's or Archibald's campaign from the original Heroes of Might and Magic II.
    Elemental
    Elemental Ring
    Elemental Storm
    Eliminate all oposition in this area. Then the first piece of the artifact will be yours.
    Eliminate all the other forces who oppose the rule of Lord Alberon. Gallavant must not die.
    Elixir
    Elven alliance
    Emblem
    Empire Too 
    EnableMenuItem
    Enchanted Hourglass
    Encode 1
    End Transmit Init Confirm
    End your turn and let the computer take its turn.
    EndDialog
    EndPaint
    Endless Bag of Gold
    Endless Cart of Ore
    Endless Cord of Wood
    Endless Pouch of Crystal
    Endless Pouch of Gems
    Endless Pouch of Sulfur
    Endless Purse of Gold
    Endless Sack of Gold
    Endless Vial of Mercury
    Enemy Speed
    Enemy heroes are now fully identifiable.
    Engage
    Enrage
    Enter the host IP address.
    Enter the name of the file you wish to save.
    EnterCriticalSection
    Entertain
    Entire unit is undead, so morale does not apply.
    Error in IP Address, please try again.
    Error reading the Heroes 2 Expansion CD.  Retry?
    Error receiving data.  Keep trying?
    Error sending data.  Keep trying??
    Error type unknown
    Establishing connection...
    Estates
    Evercold Icicle
    Everhot Lava Rock
    Evil Eye
    Evil Interface Usage
    Example:
    Excavation
    Except for evidence of a terrible battle, the cave is empty.
    Exec format error
    Exit Castle
    Exit Hero Screen
    Exit this menu without doing anything.
    Exit this menu.
    Exit town
    ExitProcess
    Experience
    Experience allows your heroes to go up levels, increasing their primary and secondary skills.
    Expert
    Expert Archery increases the damage done by range attacking creatures by 50 percent.
    Expert Ballistics gives your hero's catapult an extra shot, and each shot automatically destroys any wall, except a fortified wall in a Knight town.
    Expert Diplomacy allows you to negotiate with monsters who are weaker than your group.  All of the creatures may offer to join you.
    Expert Eagle Eye gives your hero a 40 percent chance to learn any given 4th level spell (or below) used against him in combat.
    Expert Leadership +3
    Expert Leadership increases your hero's troops' morale by 3.
    Expert Logistics increases your hero's movement points by 30 percent.
    Expert Luck +3
    Expert Luck increases your hero's luck by 3.
    Expert Mysticism regenerates four of your hero's spell points per day.
    Expert Navigation doubles your hero's movement points over water.
    Expert Necromancy allows 30 percent of the creatures killed in combat to be brought back from the dead as Skeletons.
    Expert Pathfinding eliminates the movement penalty for rough terrain.
    Expert Scouting increases your hero's viewable area by 3 squares.
    Expert Wisdom allows your hero to learn fifth level spells.
    Explorer
    Expression: 
    Eye of the Magi
    F?LikelihoodOfEnemyAttacking@philAI@@QAEXPAVtown@@PAVhero@@AAM2AAH332@Z
    F?lpDirectDrawCreate@@3P6GJPAU_GUID@@PAPAUIDirectDraw@@PAUIUnknown@@@ZA
    Faerie Ring
    Faerie ring visited +1
    Falagar
    FatalAppExitA
    Fenced Meadow
    File exists
    File to Load:
    File to Save:
    File too large
    File: 
    Filename too long
    Final Justice
    Final Score:
    Find a machine with more memory, perhaps?
    Find and defeat Martine, the pirate leader, who resides in Pirates Cove. Do not lose Gallavant or your quest will be over.
    Find the Crown before Archibald's heroes find it.  Roland will need the Crown for the final battle.
    Find the Crown before Roland's heroes find it.  Archibald will need the Crown for the final battle with Roland.
    Find the Orb of negation, which is said to be buried in this land. There are clues inscribed on stone obelisks which will help lead you to your prize. Find the Orb before the first day of the sixth month, or your rivals will surely have gotten to the fount before you.
    Find the ultimate artifact
    Find your wayward son Joseph who is rumored to be living in the desolate lands. Do it before the first day of the third month or it will be of no help to your family.
    FindClose
    FindFirstFileA
    FindNextFileA
    Fineous
    Fire Cloak
    Fire Summoning Altar
    Fireball
    Fireblast
    First Blood
    First Map Offset
    Fizbin Medal
    Fizbin of Misfortune
    Fizbin of Misfortune -2
    Flotsam
    Flowers
    FlushFileBuffers
    Follower
    For information on how your program can cause an assertion
    For the good of the Empire, eliminate Kraeger.
    Force of Arms
    Foremost Scroll
    Foremost Scroll of Knowledge
    Fortifications
    Fortunate
    Fortune
    Foundry
    Fount of Wizardry
    Fountain
    Fountain visited +1
    Four-Leaf Clover
    Four-Leaf Clover +1
    FreeEnvironmentStringsA
    FreeEnvironmentStringsW
    FreeLibrary
    Freeing a virtuous maiden from the clutches of an evil overlord, you are granted a Medal of Valor by the King's herald.
    Freeman's Foundry
    From the observation tower, you are able to see distant lands.
    Function not implemented
    Fvf+D$
    G?PlayerMonsterInteract@advManager@@QAEXPAVmapCell@@0PAVhero@@PAHHHHHH@Z&
    G?ViewArmy@game@@QAEXHHHHPAVtown@@HHHPAVhero@@PAVarmy@@PAVarmyGroup@@H@Z6
    GDI32.dll
    GDI32_NULL_THUNK_DATA
    GOOD01
    GOOD01V
    GOOD02
    GOOD02W
    GOOD03
    GOOD03QW
    GOOD04
    GOOD04W
    GOOD05
    GOOD05V
    GOOD06
    GOOD06AV
    GOOD06BV
    GOOD07
    GOOD07QW
    GOOD09
    GOOD09W
    GOOD10
    GOOD10W
    Gain the favor of the elves. They will not allow trees to be chopped down, so we will send you wood every 2 weeks. You must complete your mission before the first day of the seventh month, or the kingdom will surely fall.
    Gallop
    Gambler's Lucky Coin
    Gambler's Lucky Coin +1
    Game Difficulty:
    Game canceled.
    Game saved successfully.
    Garbage Heap
    Gargoyle
    Garrison
    Garrison does
    Gather as large an army as possible and capture the enemy castle within 8 weeks.  You are opposed by only one enemy, but must travel a long way to get to the enemy castle.  Any troops you have in your army at the end of this scenario will be with you in the final battle.
    Gather as large an army as possible and capture the enemy castle within 8 weeks.  You are opposed by only one enemy, but must travel a long way to get to the enemy castle.  Any troops you have in your army at the end of this scenario will be with you in the next.
    Gauntlets
    Gazebo
    GdiSetBatchLimit
    Gems, Cr, Slf & Mer:
    Genius
    GetACP
    GetActiveWindow
    GetAsyncKeyState
    GetCPInfo
    GetClientRect
    GetCommModemStatus
    GetCommState
    GetCommTimeouts
    GetCommandLineA
    GetCurrentDirectoryA
    GetCurrentProcess
    GetCurrentThread
    GetCurrentThreadId
    GetCursorPos
    GetDeviceCaps
    GetDriveTypeA
    GetEnvironmentStrings
    GetEnvironmentStringsW
    GetFileAttributesA
    GetFileType
    GetLastActivePopup
    GetLastError
    GetLogicalDrives
    GetMenuItemCount
    GetMenuItemID
    GetMessageA
    GetModuleFileNameA
    GetModuleHandleA
    GetOEMCP
    GetProcAddress
    GetProcessAffinityMask
    GetStartupInfoA
    GetStdHandle
    GetStringTypeA
    GetStringTypeW
    GetSubMenu
    GetSystemPaletteEntries
    GetTickCount
    GetVersion
    GetVersionExA
    GetWindowLongA
    GetWindowRect
    Giant Flail of Dominion
    Giants
    Gloves
    Goblin
    Goblin Hut
    Gold Per Day:
    Gold Watch
    Gold in Treasury:
    Golden Bow
    Golden Goose
    Golden Horseshoe
    Golden Horseshoe +1
    Good luck sometimes lets your armies get lucky attacks (double strength) in combat.
    Good morale may give your armies extra attacks in combat.
    Gopher
    Got HereIAm from 
    Grasshopper
    Graveyard
    Graveyard robber -1
    Gravity
    Greater Glory
    Greatly increases a unit's attack skill vs. Dragons.
    Green Tower
    Growth
    Guardian Spell
    Gwenneth
    H2INTRO
    H2XINTRO
    H?CombatMonsterEvent@advManager@@QAEHPAVhero@@HHPAVmapCell@@HHHHHHHHHHH@Z
    HEROES
    HEROES2.CFG
    HEROES2D /R0 /I0
    HEROES2W.EXE
    HEROES2W.exp
    Habitat
    Halberd
    Halfling Hole
    Halves damage received from ranged attacks for a single unit.
    Halves damage received from ranged attacks for all of your units.
    Hammer
    Haunts a mine you control with Ghosts.  This mine stops producing resources.  (If I can't keep it, nobody will!)
    Have the current creature wait to take its turn until all the other creatures have gone.  The creature does not lose its turn.
    Having defeated the Dragon champions, the city's leaders agree to supply some Dragons to your army for a price.  Do you wish to recruit Dragons?
    Having prepared for your arrival, Kraeger has arranged for a force of necromancers to thwart your quest. You must capture the castle of Scabsdale before the first day of the third week, or the Necromancers will be too strong for you.
    He told her "Yada yada yada"...  and then she said "Blah, blah, blah..."
    He's innocent.
    HeapAlloc
    HeapCreate
    HeapDestroy
    HeapFree
    HeapReAlloc
    HeapSize
    Heart of Fire
    Heart of Ice
    Hedgehog
    Helmet
    Helmet of Anduran
    Hero Screen
    Hero's Army
    Hero's Options
    Hero, Best RV target XY  current XY
    Hero/Stats
    Heroes
    Heroes 2
    Heroes II
    Heroes II requires 256 color mode or higher.
    Heroes II requires a minimum of
    Heroes Player
    Heroes of Might and Magic 2
    Heroes of Might and Magic II
    Heroic
    Hideous Mask
    High morale enables the 
    Hill Fort
    Hit Points Left: 
    Hit Points: 
    Holy Hammer
    Holy Pendant
    Holy Shout
    Holy Word
    Holy terror strikes the battlefield, causing severe damage to all creatures.
    Hornet
    Horseshoe
    Host Reports Player Exit
    Human-
    Human-Easy
    Human-Expert
    Human-Hard
    Human-Normal
    Hut of the Magi
    Hypnotize
    I fear these creatures are in the mood for a fight.
    I?DoDialog@heroWindowManager@@QAEHPAVheroWindow@@P6IHAAUtag_message@@@ZH@Z
    I?SetupCombat@combatManager@@QAEXHHPAVhero@@PAVarmyGroup@@PAVtown@@01HHH@Z
    IKMOQLNJLNP
    IPX networking is the most commonly used form of network in Windows 95.  Up to 6 players can play using IPX.  Only the Windows 95 version of Heroes II supports IPX networking.
    IVYDES
    IVYPOL
    IVYVOY
    IVYWIZ
    Ice Cloak
    Identify
    Identify Hero
    Idol visited +1
    Ignite
    Illegal byte sequence
    Illusion
    Impossible
    Improper link
    In Remote Main
    In a dazzling display of daring, you break into the local jail and free the hero imprisoned there, who, in return, pledges loyalty to your cause.
    In reward for helping his cart out of a ditch, a traveling potion salesman gives you a "magic" gold watch.  Unbeknownst to him, the watch really is magical.
    In the middle of a patch of dead and dry vegetation, to your surprise you find a healthy green four-leaf clover.
    In your travels, you find a leather purse filled with gold that once belonged to a great warrior king who had the ability to transform any inanimate object into gold.
    In your wanderings you come across a hermit living in a small, tidy hut.  Impressed with your mission, he takes time out from his meditations to bless and give you a charm against curses.
    Inappropriate I/O control operation
    Income:
    Incomplete TCP/IP command line information
    Increases a unit's attack skill.
    Increases the defense skill of the targeted creatures.  This is an improved version of Stoneskin.
    Increases the speed of all of your creatures by two.
    Increases the speed of any creature by two.
    Indicates how many players total are in the scenario.  Any positions not occupied by humans will be occupied by computer players.
    Indicates whether the map is small (36 x 36), medium (72 x 72), large (108 x 108), or extra large (144 x 144).
    Initialization failed!
    Initialize communications paramaters
    InitializeCriticalSection
    Initializing modem...
    Initializing network.
    Input/output error
    Instant
    Insulted by your refusal of his generous offer, the leprechaun stamps his foot and ignores you.
    Insulted by your refusal of their offer, the monsters attack!
    Interface
    Interlaced
    Interrupted function call
    Invalid Teleport Destination
    Invalid argument
    Invalid seek
    InvalidateRect
    InvalidateRect Failed
    Invisible
    Is a directory
    Island of Chaos
    It doesn't help to pray twice before a battle.  Come back after you've fought.
    Ivan II
    Ivan III
    Ivan IV
    Ivan V
    Ivan VI
    Ivory Gates
    Ivory Tower
    Jaclyn
    Jezebel
    Jojosh
    Joseph
    Joseph II
    Joseph III
    Joseph IV
    Joseph V
    Joseph VI
    Jousting Arena
    KB.LOG
    KERNEL32
    KERNEL32.dll
    KERNEL32_NULL_THUNK_DATA
    KKKKKKKKKKK
    Kalindra
    Kastore
    Kinetic Pendant
    King Archibald requires you to defeat the three enemies in this region.  They are not allied with one another, so they will spend most of their energy fighting amongst themselves.  You will win when you own all of the enemy castles and there are no more heroes left to fight.
    King and Country
    Kingdom
    Kingdom Overview
    Knight
    Knight bonus +1
    Knighthood
    Knowl.
    Knowledge
    Knowledge:
    Knowledge: 
    Kraeger Defeated
    L$ RPQj
    L$ VPQj
    L$0SWPj
    L$LPQWV
    L$TVWU
    L$XPWQ
    L$|RVQWP
    LCMapStringA
    LCMapStringW
    LGNDXTRA.ICN
    LGNDXTRE.ICN
    LIBRARY
    LIBRARYW
    LLLLLLLLLLL
    LLLLLLLLLLN
    Laboratory
    Lantern
    Lava Pool
    Leadership
    Lean-To
    Leather
    LeaveCriticalSection
    Left Turret
    Legendary Scepter
    Legendary Sceptre
    Legion
    Level 2 Mage Guild
    Library
    Lighthouse
    Lightning
    Lightning Bolt
    Lightning Helm
    Lightning Rod 
    Line: 
    Lizard
    Load Game 1
    Load Game 2
    Load a previously saved game.
    LoadCursorA
    LoadIconA
    LoadLibraryA
    LoadMenuA
    Locust
    Log Cabin
    Logistics
    Looking somewhat disappointed, the Sphinx sighs.  You've answered my riddle so here's your reward.  Now begone.
    Lord Alamar
    Lord Ironfist
    Lord Kilburn
    Lord Slayer
    Lose all your heroes, towns and castles.
    Lots of
    Low morale causes the 
    Luck: 
    Lucky Rabbit's Foot
    Lucky Rabbit's Foot +1
    M?AutoResolveCombat@advManager@@QAEHHHPAVhero@@PAVarmyGroup@@PAVtown@@01HHHH@Z
    MC BuildBoat
    MC ClaimMine
    MC ClaimTown
    MC DeadHero
    MC Erase Object
    MC Move Hero
    MC My Turn
    MC RecruitHero
    MC Teleport Hero
    MIXDES10
    MIXDES11
    MIXDES12
    MIXDES13
    MIXDES14
    MIXDES15
    MIXDES9
    MIXPOL1
    MIXPOL2
    MIXPOL3
    MIXPOL4
    MIXPOL5
    MIXPOL6
    MIXPOL7
    MIXPOL8
    MIXVOY21
    MIXVOY22
    MIXVOY23
    MIXVOY24
    MIXVOY25
    MIXWIZ16
    MIXWIZ17
    MIXWIZ18
    MIXWIZ19
    MIXWIZ20
    Mage Guild
    Mage Guild is at highest level.
    Mage's Ring
    Mage's Ring of Power
    Magellan's Maps
    Magic Arrow
    Magic Book
    Magic Garden
    Magic Well
    Magical elements pour down on the battlefield, damaging all creatures.
    Magically increases the defense skill of the selected creatures.
    Main Game Color Mouse Cursor
    Main Game Full Screen
    Main Game Height
    Main Game Show Menu
    Main Game Width
    Main Game X
    Main Game Y
    Majesty
    Major Scroll
    Major Scroll of Knowledge
    Malevolent
    Mandigal
    Mandrake
    Mangled bones, yellowed with age, peer from the ragged folds of a dead Druid's robe.  Searching the robe, you discover a scroll hidden within.
    Mansion
    Map Size
    Marketplace
    Mass Bless
    Mass Cure
    Mass Curse
    Mass Dispel
    Mass Haste
    Mass Shield
    Mass Slow
    Masthead
    Masthead bonus at sea +1
    Mausoleum
    Maximus
    Medal of Courage
    Medal of Courage +1
    Medal of Distinction
    Medal of Distinction +1
    Medal of Honor
    Medal of Honor +1
    Medal of Valor
    Medal of Valor +1
    Medium
    Medusa
    MemEntries Below 0
    Memory Sample 1
    Memory Sample 2a
    Memory Sample 2b
    Mercenary Camp
    Mercury
    Mermaid
    Mermaid visited +1
    MessageBeep
    MessageBoxA
    Meteor
    Meteor Shower
    Microsoft CVTRES 4.00
    Microsoft LINK 2.60.5112 (NT)
    Microsoft Visual C++ Runtime Library
    Minor Scroll
    Minor Scroll of Knowledge
    Minotaur
    Minutemen
    Mirror Image
    Mirror Image spell failed!
    Missile
    Modem Baud Rate
    Modem Com Port
    Modem Init String
    Modify Sample 1
    Modify Sample 2
    Mongoose
    Monster
    Monster Info
    Morale: 
    Mountains
    Mouse Cursor
    Move how many troops?
    MoveWindow
    Movement
    MultiByteToWideChar
    Multiple
    Music Type
    Music Volume
    My heroes, our foes have been scattered, their castles broken and laid bare.  The great campaign is now complete, and I stand before you as the undisputed High King!
    Myrini
    Mysticism
    N?ProbableOutcomeOfBattle@philAI@@QAEXPAVarmyGroup@@PAVhero@@010HHHAAMAAH3333@Z
    NETAPI32.dll
    NETAPI32_NULL_THUNK_DATA
    NETBIOS is not loaded.
    NETBios networking is the only form of networking available for DOS machines, and is also available for Windows 95.  Only two players may connect with NetBIOS.  If all machines you wish to connect are using the Windows 95 version of Heroes II, then IPX networking is recommended.
    NEWGAME
    NULL_IMPORT_DESCRIPTOR
    NWCLOGO
    Natasha
    Navigation
    Nearest town occupied.  Spell Failed!!!
    Necromancer
    Necromancer guild
    Necromancers
    Necromancy
    Neither MIDI nor Redbook music is currently available on this system.
    Nestled among the trees sits a blind seer. After explaining the intent of your journey, the seer activates his crystal ball, allowing you to see the strengths and weaknesses of your opponents.
    Netbios
    Network Default Name
    Network initialization failed
    Neutral luck means your armies will never get lucky or unlucky attacks on the enemy.
    Neutral morale means your armies will never be blessed with extra attacks or freeze in combat.
    New Game 1
    New Game 2
    New Join CRC
    Next sign 50 miles
    No Fire Elementals approach you from the lava pool.
    No available town.  Spell Failed!!!
    No child processes
    No error
    No locks available
    No shots left!
    No space left on device
    No spells to cast.
    No such device
    No such device or address
    No such file or directory
    No such process
    Nomad Boots
    Nomad Boots of Mobility
    Normal
    Not a directory
    Not enough space
    Note: For a 14400 baud modem, use the 19200 baud speed.  For a 28800 baud modem, use the 38400 baud speed.
    Note: In general, computers with the older UART 8250 chip should use 19200 baud, and computers with the newer UART 16550 chip should use 38400 baud.  When in doubt, try slower speeds first, and if they work, then try faster speeds.  Most computers made in 1994 or later have a UART 16550 chip.
    Nothing Special
    Nothing here.
    Number of Castles:
    Number of Heroes:
    Number of Towns:
    O?SendHeroTownData@advManager@@QAEXHHPAVhero@@PAVarmyGroup@@PAVtown@@01HHHHHHH@Z2
    Oasis visited +1
    Obelisk
    Obelisks Found:
    Observation Tower
    OffsetRect
    Ogre alliance
    Ointment
    One of the four new campaigns from the Price of Loyalty expansion set.
    One of the less intelligent members of your party picks up an arm off of the ground.  Despite its missing a body, it is still moving.  Your troops find the dismembered arm repulsive, but you cannot bring yourself to drop it: it seems to hold some sort of magical power that influences your decision making.
    One or more heroes may still move, are you sure you want to end your turn?
    Operation not permitted
    Opponents
    Opponents:
    Oracle
    Orchard
    Orig Join CRC
    Out Remote Main
    Out of Memory
    Out of memory.
    Out of pity for a poor peasant, you purchase a chest of old junk they are hawking for too much gold. Later, as you search through it, you find it contains the 3 pieces of the legendary battle garb of Anduran!
    Outcropping
    OutputDebugStringA
    OverFlow1
    OverFlow2
    Overthrow the entrenched monarchy of Lord Alberon, and claim all the land in your name. Gallavant must not die.
    PLYREXIT
    PREBATTL.82M
    PRISON
    Paladin
    Paralyze
    Partake
    PatBlt
    Pathfinding
    Pausing to rest and light a cook fire, you pull wood out of a nearby pile of dead wood.  As you keep pulling wood from the pile, you notice that it doesn't shrink.  You realize to your delight that the wood is enchanted, so you take it along.
    Peasant Hut
    PeekMessageA
    Pendant of Death
    Pendant of Free Will
    Pendant of Life
    Permission denied
    Personality:
    Phoenix
    Pirate Isles
    Play a Hot Seat game, where 2 to 4 players play around the same computer, switching into the 'Hot Seat' when it is their turn.
    Play a direct connect game, where 2 players use their own computers directly connected through their serial port by a null modem.
    Play a modem game, where 2 players use their own computers connected over the phone lines using modems.
    Play a network game, where 2 players use their own computers connected through a LAN (Local Area Network).
    Play with 2 human players, and optionally, up to 4 additional computer players.
    Play with 3 human players, and optionally, up to 3 additional computer players.
    Play with 4 human players, and optionally, up to 2 additional computer players.
    Play with 5 human players, and optionally 1 computer player.
    Play with 6 human players.
    Player with HeroTOMOVE
    Please enter a 'handle' by which you will be known.
    Please enter any special initialization string required by your modem, or hit 'ENTER' to accept the default.
    Please enter the telephone number.
    Please enter your name for the high score list.
    Please inspect our fine wares.  If you feel like offering a trade, click on the items you wish to trade with and for.
    Please make a selection from the list, or press cancel.
    Poll Sound 1
    Poll Sound 1a
    Poll Sound 1b
    Poll Sound 1c
    Poll Sound 1d
    Poll Sound 2
    Portal
    Post T
    PostDiffFileSize
    PostMessageA
    PostQuitMessage
    PostWait
    Potion
    Powder
    Power Axe
    Power Axe of Dominion
    Power Ring
    Power's End
    Practicing the dark arts of necromancy, you are able to raise one of the enemy's dead to return under your service as a Skeleton.
    PreWait
    Press 'CANCEL' to abort.
    Prevents harmful magic against the selected creatures.
    Price of Loyalty
    Process Act
    Program: 
    Puzzle Pieces
    Pyramid
    Pyramid raided -2
    Q_`abc
    Queen Lamanda
    Quick Combat Level
    Quit Heroes of Might and Magic and return to the operating system.
    Quit out of Heroes of Might and Magic II.
    RBETRAYV
    RGBLOOKP.BIN
    Rabbit
    Rainbow
    RaiseException
    Random
    Random Artifact
    Random Artifact - Major
    Random Artifact - Minor
    Random Artifact - Treasure
    Random Castle
    Random Hero
    Random Monster
    Random Monster - medium
    Random Monster - strong
    Random Monster - very strong
    Random Monster - weak
    Random Resource
    Random Town
    Ranger
    Ranking:
    Ranloo
    Ransacking an enemy camp, you discover a hidden cache of treasures.
    Rating
    Rating:
    Read communications data
    Read-only file system
    ReadFile
    RealizePalette
    Rebecca
    Rebellion
    Receive
    Receive End
    Receive Remote Player Exit
    Receive Start - Decompressing Data
    Receive Start - Getting Data
    Receiving Data
    Recognizing you, the butler refuses to admit you.  "The master," he says, "will not see the same student twice."
    Recruit
    Recruit Hero
    Red Tower
    Reduce the block size, and/or use the -e flag.
    Reduces the defense rating of an enemy unit by three.
    RegCloseKey
    RegCreateKeyA
    RegOpenKeyExA
    RegQueryValueExA
    RegSetValueExA
    RegisterClassA
    ReleaseCapture
    ReleaseCapture Failed
    ReleaseDC
    Removes all magic spells from a single target.
    Removes all magic spells from all creatures.
    Removes all negative spells cast upon one of your units, and restores up to 5 HP per level of spell power.
    Removes all negative spells cast upon your forces, and restores up to 5 HP per level of spell power, per creature.
    Requires:
    Rescue
    Rescue your crazy uncle Ivan. Find him before the first day of the fourth month or it will be no help to your kingdom.
    ResetDisplayMode
    ResetEvent
    Resource
    Resource cost:
    Resource deadlock avoided
    Resource device
    Resource temporarily unavailable
    Responding to cries for help, you find river Sprites making a sport of dunking an old man.  Feeling vengeful, you rescue the man and drag a Sprite onto dry land for awhile.  The Sprite, uncomfortable in the air, gives you a magic pendant to let him go.
    Responding to the panicked cries of a damsel in distress, you discover a young woman fleeing from a hungry bear.  You slay the beast in the nick of time, and the grateful Sorceress weaves a magic cloak from the bear's hide.
    Restless spirits of long dead warriors seeking their final resting place offer to join you in hopes of finding peace.  Do you wish to recruit ghosts?
    Result too large
    Resurrect
    Resurrect True
    Resurrects creatures from a damaged or dead unit permanently.
    Resurrects creatures from a damaged or dead unit until end of combat.
    Retake the castle of Ivory Gates, which has fallen due to treachery.
    Retreat
    Retreat your hero, abandoning your creatures.  Your hero will be available for you to recruit again, however, the hero will have only a novice hero's forces.
    Return to the battle.
    Returns the caster to any town or castle currently owned.
    Returns the hero to the town or castle of choice, provided it is controlled by you.
    Rialdo
    Ridding the countryside of the hideous Minotaur who made a sport of eating noblemen's Knights, you are honored with the Medal of Distinction.
    Right Turret
    Righteous
    Roland
    Roland has been captured!  All is lost.
    Roland needs you to defeat the lords near his castle to begin his war of rebellion against his brother.  They are not allied with each other, so they will spend most of their time fighting with one another.  Victory is yours when you have defeated all of their castles and heroes.
    Roland strengthened
    Roxana
    RtlUnwind
    Rumors of a Griffin of unusual size preying upon the countryside lead you to its cave lair.  A quick, brutal fight dispatches the beast, and a search of its foul nest turns up a huge black pearl.
    Runtime Error!
    SBETRAYV
    SCHOOSE
    SCYBCRED
    SCYLOGO
    SDEFEAT
    SDES10
    SDES11
    SDES12
    SDES13
    SDES14
    SDES15
    SEVIL01
    SEVIL01V
    SEVIL02
    SEVIL03
    SEVIL05
    SEVIL06
    SEVIL07
    SEVIL08
    SEVIL09
    SEVIL10
    SEVIL5AV
    SEVIL5BV
    SGOOD01
    SGOOD01V
    SGOOD02
    SGOOD03
    SGOOD04
    SGOOD05
    SGOOD05V
    SGOOD06
    SGOOD07
    SGOOD09
    SGOOD10
    SGOOD6AV
    SGOOD6BV
    SH2INTRO
    SH2XINTR
    SING error
    SINTRO
    SLIBRARY
    SNWCLOGO
    SOFTWARE\New World Computing\Heroes of Might and Magic 2\1.0
    SPANBKG.ICN
    SPANBKGE.ICN
    SPANBTN.ICN
    SPANBTNE.ICN
    SPELCO.ICN
    SPRISON
    SSSS 1
    SSSS 2
    SSSSSS
    SURDRBKE.ICN
    SURDRBKG.ICN
    SURRENDE.ICN
    SURRENDR.ICN
    SVOY21
    SVOY22
    SVOY23
    SVOY24
    SVOY25
    SVPQh 
    SVWUt6
    SVWUtB
    SVWUu@
    SVWUuL
    SVWUuV
    SVWh4hQ
    SWIZ16
    SWIZ17
    SWIZ18
    SWIZ19
    SWIZ20
    Sailor's Astrolabe of Mobility
    Sandro
    Sarakin
    Save the Dwarves
    Save the current game.
    Sawmill
    Scabbard
    Scarab
    Scimitar
    Scouting
    ScreenToClient
    Scroll
    Sea Chest
    Searching for host.
    Searching for the Ultimate Artifact is fruitless.  Your hero could not carry it even if he found it - all his artifact slots are full.
    See Rock City
    Seeing that you do not have 2500 gold, the demon slashes you with its claws, and the last thing you see is a red haze.
    Seeing-eye Pendant
    Select Spell
    Select Spell Target
    Select teleport destination.
    Select the next Hero.
    Select town to port to.
    SelectObject
    SelectPalette
    Selects a game from the Price of Loyalty expansion set.
    Selects a game from the original Heroes of Might and Magic II.
    Send Map Change
    Sending Data
    Serenity Pendant
    ServiceProvider:
    Sessions:
    Set Air Guardian
    Set Earth Guardian
    Set Fire Guardian
    Set Water Guardian
    Set army combat formation to 'Grouped'
    Set army combat formation to 'Spread'
    Set communications break
    Set communications timeouts
    Set garrison combat formation to 'Grouped'
    Set garrison combat formation to 'Spread'
    Set the speed of combat actions and animations.
    SetCapture
    SetCommBreak
    SetCommState
    SetCommTimeouts
    SetConsoleCtrlHandler
    SetCurrentDirectoryA
    SetCursor
    SetEndOfFile
    SetEnvironmentVariableA
    SetEvent
    SetFilePointer
    SetHandleCount
    SetLastError
    SetMenu
    SetStdHandle
    SetThreadAffinityMask
    SetTimer
    SetWindowLongA
    Sets Air Elementals to guard a mine against enemy armies.
    Sets Earth Elementals to guard a mine against enemy armies.
    Sets Fire Elementals to guard a mine against enemy armies.
    Sets Water Elementals to guard a mine against enemy armies.
    Sets the speed that A.I. heroes move at.  You can also elect not to view A.I. movement at all.
    Sets what type of interface you want to use.  The default selection is a dynamic interface, which uses Evil graphics for the 3 evil heroes (Barbarian, Warlock, and Necromancer).
    Setup 0
    Setup 0a
    SetupComm
    Several
    Shadow
    Shield
    Shipwreck
    Shipwreck Survivor
    Shipwreck robber -1
    Shipyard
    Shots left: 
    Shots: 
    Show Combat Grid
    Show Combat Mouse Hex
    Show Object Boxes
    Show Path
    Show Route
    ShowCursor
    ShowWindow
    Shows the results of individual monster's actions.
    Shrine
    Shrine - Player Rankings
    Shrine of the First Circle
    Shrine of the Second Circle
    Shrine of the Third Circle
    Shutdown
    Sir Gallant
    Sirens
    Sister Eliza
    Skeleton
    Skills
    Skip this unit
    Skips the current creature.  The current creature loses its turn and does not get to go again until the next round.
    Skull Pile
    Skullcap
    Slay the Dwarves
    Slayer
    Slow Video
    Slows all enemies to half movement rate.
    Slows target to half movement rate.
    Snake-Ring
    Some Liches living here are willing to join your army for a price.  Do you want to recruit Liches?
    Some Trolls living under a bridge are willing to join your army, but for a price.  Do you want to recruit Trolls?
    Some of the Sprites living in the tree city are willing to join your army for a price.  Do you want to recruit Sprites?
    Some of the surviving Liches are impressed by your victory over their fellows, and offer to join you for a price.  Do you want to recruit Liches?
    Some undead in group -1
    Sorceress
    Sorceress Rainbow +2
    Sorceress guild
    Sorry Please Try Again
    Sound Quality
    Sound Volume
    Sound initialization error!
    Sound initialization error!  No wave devices found.
    Spade of Necromancy
    Special
    Speed:
    Speed: 
    Spell Casting
    Spell Casting}
    Spell Failed!!!  You must be on land for this spell to work.
    Spell Points:
    Spell Points: 
    Spell Power
    Spell Power:
    Spell Power: 
    Spell Scroll
    Spells Only
    Sphere of Negation
    Sphinx
    Spider
    Spiked Helm
    Spiked Shield
    Squirrel
    Stables
    Staff of Wizardry
    Standing Stones
    Start a single or multi-player game.
    Starts the DOS version of Heroes2 with redbook
    Startup Error
    Statesman's Quill
    Statue
    Stealth Shield
    Stealth Shield of Protection
    Steelskin
    Stick Hut
    Stone Liths
    Stonehenge
    Stoneskin
    Stop Catapult
    Stop Sample 1
    Stop Sample 2
    Stranded
    Stream
    Subdue the unruly local lords in order to provide the Empire with facilities to operate in this region.
    Suddenly, a biting coldness engulfs your body.  You seize up, falling from your horse.  The pain subsides, but you still feel as if your chest is frozen.  As you pick yourself up off of the ground, you hear hearty laughter.  You turn around just in time to see a Frost Giant run off into the woods and disappear.
    Suggested solutions:
    Sulfur
    Summon
    Summon Air Elemental
    Summon Boat
    Summon Boat failed!!!
    Summon Earth
    Summon Earth Elemental
    Summon Fire Elemental
    Summon Water Elemental
    Summons Air Elementals to fight for your army.
    Summons Earth Elementals to fight for your army.
    Summons Fire Elementals to fight for your army.
    Summons Water Elementals to fight for your army.
    Summons the nearest unoccupied, friendly boat to an adjacent shore location.  A friendly boat is one which you just built or were the most recent player to occupy.
    Superior Scroll of Knowledge
    Surrender
    Surrendering costs gold.  However, if you pay the ransom, the hero and all of his or her surviving creatures will be available to recruit again.
    Switch Ambient Music 1
    Switch Ambient Music 2
    Switching sides leaves you with three castles against the enemy's one.  This battle will be the easiest one you will face for the rest of the war...traitor.
    Sword Breaker
    Sword of Anduran
    T$ PQRj 
    T$ PQRj0
    T$4WUj
    T$<PQR
    T$LPQR
    TCP/IP is most commonly used as a way of connecting machines over the internet.  Up to 6 players can play using IPX.  To connect over the internet, you must have an account with an ISP (Independent Service Provider).  Only the Windows 95 version of Heroes II supports TCP/IP networking.
    TLOSS error
    TOO LONG
    TRADPOSE.ICN
    TRADPOST.ICN
    TTTTS3
    Taking shelter from a storm in a small cave,  you notice a small patch of crystal in one corner.  Curious, you break a piece off and notice that the original crystal grows the lost piece back.  You decide to stuff the entire patch into a pouch and take it with you.
    Tar Pit
    Tavern
    Tavern +1
    Tax Lien
    Tears brim in the eyes of the tree.  "I need 10 gems." it whispers. (sniff)  "Well, come back when you can pay me."
    Tears brim in the eyes of the tree.  "I need 2000 gold." it whispers. (sniff)  "Well, come back when you can pay me."
    Telepathy
    Teleport
    Teleport Here
    Teleports the creature you select to any open position on the battlefield.
    Telescope
    Temple
    Temple visited +2
    Terminated
    Terror
    That spell will affect no one!
    Thatched Hut
    The Abyss
    The Ammo Cart provides endless ammunition for all your troops that shoot.
    The Ankh doubles the effectiveness of all your resurrect and animate spells.
    The Arcane Necklace of Magic increases your spell power by 4.
    The Archmages dispel all good spells
    The Arena guards turn you away.
    The Arm of the Martyr increases your spell power by 3 but adds the undead morale penalty.
    The Armored Gauntlets of Protection increase your defense skill by 1.
    The Ballista of Quickness lets your catapult fire twice per combat round.
    The Battle Garb of Anduran combines the powers of the three Anduran artifacts.  It provides maximum luck and morale for your troops and gives you the Town Portal spell.
    The Black Pearl increases your spell power and knowledge by 2 each.
    The Book of Elements doubles the effectiveness of all your summoning spells.
    The Breastplate increases your defense by 5.
    The Captain's Quarters provides a captain to assist in the castle's defense when no hero is present.
    The Caster's Bracelet of Magic increases your spell power by 2.
    The Castle improves town defense and increases income to 1000 gold per day.
    The City of the Dead is empty of life, and empty of unlife as well.  Perhaps some undead will move in next week.
    The Coliseum provides inspiring spectacles to defending troops, raising their morale by two during combat.
    The Crown
    The Crystal Ball lets you get more specific information about monsters, enemy heroes, and castles nearby the hero who holds it.
    The Defender Helm of Protection increases your defense skill by 1.
    The Demon leaps upon you and has its claws at your throat before you can even draw your sword.  "Your life is mine," it says.  "I will sell it back to you for 2,500 gold."
    The Demon screams its challenge and attacks!  After a short, desperate battle, you slay the monster and receive 1,000 experience points and 2,500 gold.
    The Demon screams its challenge and attacks!  After a short, desperate battle, you slay the monster and receive 1,000 experience points.
    The Divine Breastplate of Protection increases your defense skill by 3.
    The Dragon Sword of Dominion increases your attack skill by 3.
    The Dragon city has no Dragons willing to join you this week.  Perhaps a Dragon will become available next week.
    The Dragon city is willing to offer some Dragons for your army for a price.  Do you wish to recruit Dragons?
    The Dungeon increases the income of the town by 500 / day.
    The Dwarves need conquering before they can interfere in King Archibald's plans.  Roland's forces have more than one hero and many towns to start with, so be ready for attack from multiple directions.  You must capture all of the enemy towns and castles to claim victory.
    The Elemental Ring halves the casting cost of all summoning spells.
    The Elven Lands
    The Empire is weak in this region. You will be unable to completely subdue all forces in this area, so take what you can before reprisal strikes. Remember, your true goal is to claim the Helmet of Anduran.
    The Enchanted Hourglass extends the duration of all your spells by 2 turns.
    The Endless Bag of Gold provides you with 750 gold per day.
    The Endless Cart of Ore provides 1 unit of ore per day.
    The Endless Cord of Wood provides 1 unit of wood per day.
    The Endless Pouch of Crystal provides 1 unit of crystal/day.
    The Endless Pouch of Gems provides 1 unit of gems per day.
    The Endless Pouch of Sulfur provides 1 unit of sulfur per day.
    The Endless Purse of Gold provides you with 500 gold per day.
    The Endless Sack of Gold provides you with 1000 gold per day.
    The Endless Vial of Mercury provides 1 unit of mercury per day.
    The Epic Battle
    The Eternal Scrolls
    The Evercold Icicle causes your cold spells to do 50% more damage to enemy troops.
    The Everhot Lava Rock causes your fire spells to do 50% more damage to enemy troops.
    The Evil Eye reduces the casting cost of curse spells by half.
    The Fire Cloak halves all damage your troops take from fire spells.
    The Fizbin of Misfortune greatly decreases your morale.
    The Foremost Scroll of Knowledge increases your knowledge by 5.
    The Fortifications increase the toughness of the walls, increasing the number of turns it takes to knock them down.
    The Four-Leaf Clover increases your luck in combat.
    The Gambler's Lucky Coin increases your luck in combat.
    The Gauntlet
    The Giant Flail of Dominion increases your attack skill by 1.
    The Giant's Pass
    The Gold Watch doubles the effectiveness of your hypnotize spells.
    The Golden Goose brings in an income of 10,000 gold per turn.
    The Golden Horseshoe increases your luck in combat.
    The Helmet increases your spell power by 5.
    The Heroes 2 Expansion CD-ROM is not in the drive.  Without a Heroes 2 Expansion CD-ROM you will only be able to play as the guest in a multi-player game.  If you have the CD, then exit the program, put the CD in, and try again.
    The Hideous Mask prevents all 'wandering' armies from joining your hero.
    The Holy Hammer increases your attack skill by 5.
    The Holy Pendant makes all your troops immune to curse spells.
    The Host already has a game in progress and is not accepting new players.
    The Host is not responding.  Keep waiting?
    The Ice Cloak halves all damage your troops take from cold spells.
    The Kinetic Pendant makes all your troops immune to paralyze spells.
    The Left Turret provides extra firepower during castle combat.
    The Legendary Scepter adds 2 points to all attributes.
    The Library increases the number of spells in the Guild by one for each level of the guild.
    The Lightning Helm halves all damage your troops take from lightning spells.
    The Lightning Rod causes your lightning spells to do 50% more damage to enemy troops.
    The Lucky Rabbit's Foot increases your luck in combat.
    The Mage Guild allows heroes to learn spells and replenish their spell points.
    The Mage's Ring of Power increases your spell power by 2.
    The Magic Book enables you to cast spells.
    The Major Scroll of Knowledge increases your knowledge by 3.
    The Marketplace can be used to convert one type of resource into another.  The more marketplaces you control, the better the exchange rate.
    The Masthead boosts your luck and morale by 1 each in sea combat.
    The Medal of Courage increases your morale.
    The Medal of Distinction increases your morale.
    The Medal of Honor increases your morale.
    The Medal of Valor increases your morale.
    The Minor Scroll of Knowledge increases your knowledge by 2.
    The Moat slows attacking units.  Any unit entering the moat must end its turn there and becomes more vulnerable to attack.
    The Nomad Boots of Mobility increase your movement on land.
    The Pendant of Death makes all your troops immune to holy spells.
    The Pendant of Free Will makes all your troops immune to hypnotize spells.
    The Pendant of Life makes all your troops immune to death spells.
    The Power Axe of Dominion increases your attack skill by 2.
    The Power Ring returns 2 extra spell points/turn to your hero.
    The Rainbow increases the luck of the defending units by two.
    The Right Turret provides extra firepower during castle combat.
    The Sailors' Astrolabe of Mobility increases your movement on sea.
    The Seeing-eye Pendant makes all your troops immune to blindness spells.
    The Serenity Pendant makes all your troops immune to berserk spells.
    The Shipyard allows ships to be built.
    The Shrine increases the necromancy skill of all your necromancers by 10 percent.
    The Shrouded Isles
    The Skullcap halves the casting cost of all mind influencing spells.
    The Snake Ring halves the casting cost of all your bless spells.
    The Sorceress' guild of Noraston has requested Roland's aid against an attack from Archibald's allies.  Capture all of the enemy castles to win, and don't lose Noraston, or you'll lose the scenario.  (Hint:  There is an enemy castle on an island in the ocean.)
    The Southern War
    The Spade gives you increased necromancy skill.
    The Sphere of Negation artifact is in effect for this battle, disabling all combat spells.
    The Sphere of Negation disables all spell casting, for both sides, in combat.
    The Sphinx asks you the following riddle:
    The Spiked Helm increases your attack and defense skills by 1 each.
    The Spiked Shield increases your attack and defense skills by 2 each.
    The Staff of Wizardry boosts your spell power by 5.
    The Statue increases your town's income by 250 per day.
    The Stealth Shield of Protection increases your defense skill by 2.
    The Storm adds +2 to the power of spells of a defending spell caster.
    The Superior Scroll of Knowledge increases your knowledge by 4.
    The Sword Breaker increases your defense by 4 and attack by 1.
    The Sword increases your attack skill by 5.
    The Tavern increases morale for troops defending the castle.
    The Tax Lien costs you 250 gold pieces/turn.
    The Telescope increases the amount of terrain your hero reveals when adventuring by 1 extra square.
    The Tent provides workers to build a castle, provided the materials and the gold are available.
    The Thieves' Guild provides information on enemy players.  Thieves' Guilds can also provide scouting information on enemy towns.  Additional Guilds provide more information.
    The Thunder Mace of Dominion increases your attack skill by 1.
    The Traveler's Boots of Mobility increase your movement on land.
    The True Compass of Mobility increases your movement on land and sea.
    The Ultimate Book of Knowledge increases your knowledge by 12.
    The Ultimate Cloak of Protection increases your defense skill by 12.
    The Ultimate Crown increases each of your basic skills by 4 points.
    The Ultimate Shield increases your attack and defense skills by 6 each.
    The Ultimate Staff increases your spell power and knowledge by 6 each.
    The Ultimate Sword of Dominion increases your attack skill by 12.
    The Ultimate Wand of Magic increases your spell power by 12.
    The Unknown Hero
    The Wand of Negation protects your troops from the Dispel Magic spell.
    The Wayward Son
    The Well increases the growth rate of all dwellings by 2 creatures per week.
    The White Pearl increases your spell power and knowledge by 1 each.
    The Witch's Broach of Magic increases your spell power by 3.
    The Wizard's Hat increases the duration of your spells by 10 turns!
    The above spells are available here.
    The above spells have been added to your book.
    The barbarian despot in this area is, as yet, ignorant of your presence. Quickly, build up your forces before you are discovered and attacked! Secure the region by subduing all enemy forces.
    The bones of Lord Slayer are buried in the foundation of the arena.
    The butler admits you to see the master of the house.  He trains you in the four skills a hero should know.
    The butler opens the door and looks you up and down.  "You are neither famous nor diplomatic enough to be admitted to see my master," he sniffs.  "Come back when you think yourself worthy."
    The captain sighs.  "You don't have enough money, eh?  You can't expect me to give my maps away for free!"
    The creatures are swayed by your diplomatic tongue, and make you an offer:
    The creatures are willing to join us!
    The current game has been saved as 'PLYREXIT'. Do you wish to keep playing with the computer filling in for the other humans?
    The currently selected map is not a valid choice for your next scenario.
    The dark side is stronger.
    The description of the currently selected map.
    The difficulty rating reflects a combination of various settings for your game.  This number will be applied to your final score.
    The dragons see you and call out.  "Our alliance with Archibald compels us to join you.  Unfortunately you have no room.  A pity!"  They quickly scatter.
    The dragons, snarling and growling, agree to join forces with you, their 'Ally'.
    The drink at the oasis is refreshing, but offers no further benefit.  The oasis might help again if you fought a battle first.
    The drink at the watering hole is refreshing, but offers no further benefit.  The watering hole might help again if you fought a battle first.
    The dwarves hail you, "Any friend of Roland is a friend of ours.  You may pass."
    The dwarves recognize their allies and gladly join your forces.
    The elves stand at attention as you approach.  Their leader calls to you and says, "Let us not impede your progress, ally!  Move on, and may victory be yours."
    The end of the world is near.
    The enemy has fled!
    The enemy has surrendered!
    The enemy is beaten.  Your side has triumphed!
    The entrance to the cave is dark, and a foul, sulfurous smell issues from the cave mouth.  Will you enter?
    The fabled golden bow of the elves lies here in the dust.  You take it and journey back to the elven towns.  They shower you with their graciousness and the king promises that his people will aid you whenever you seek help.
    The guest waits for the host to call and set up the game.
    The guest waits for the host to set up the game, then is automatically added in.  There can be multiple guests for TCP/IP and IPX games, but only 1 guest for NetBIOS games.
    The guest waits for the host to set up the game.
    The head groom approaches you and speaks, "You already have a fine horse, and have no inexperienced cavalry which might make use of our trained war horses."
    The head groom speaks to you, "That is a fine looking horse you have. I am afraid we can give you no better, but the horses your cavalry are riding look to be of poor breeding stock. We have many trained war horses which would aid your riders greatly. I insist you take them."
    The host has canceled the game.
    The host sets up the game options, chooses the number to dial, and places the call.
    The host sets up the game options.
    The host sets up the game options.  There can only be one host per network game.
    The jailer tells you that the hero who was imprisoned here has been released by the king who imprisoned him.
    The keeper of the mill announces: "Milord, I am sorry, there are no resources currently available. Please try again next week."
    The keeper of the mill announces: "Milord, I am sorry, there is no gold currently available.  Please try again next week."
    The keeper of the mill announces: "Milord, I have been working very hard to provide you with these resources, come back next week for more."
    The keeper of the mill announces: "Milord, I have been working very hard to provide you with this gold, come back next week for more."
    The lean-to is long abandoned.  There is nothing of value here.
    The lighthouse is now under your control, and all of your ships will now move further each turn.
    The local lords refuse to swear allegiance to Roland, and must be subdued.  They are wealthy and powerful, so be prepared for a tough fight.  Capture all enemy castles to win.
    The location of the great library has been discovered! You must make your way to it, and reclaim the city of Chronos in which it lies.
    The magical, soothing beauty of the Mermaids reaches you and your crew.  Just for a moment, you forget your worries and bask in the beauty of the moment.  The mermaids charms bless you with increased luck for your next combat.
    The map difficulty of the currently selected map.  The map difficulty is determined by the scenario designer.  More difficult maps might include more or stronger enemies, fewer resources, or other special conditions making things tougher for the human player.
    The mermaids silently entice you to return later and be blessed again.
    The mirror image is destroyed!
    The name of the currently selected map.
    The ogres give you a grunt of recognition, "Archibald's allies may pass."
    The ogres recognize you as the Dwarfbane and lumber over to join you.
    The pit of mud bubbles for a minute and then lies still.
    The rotting hulk of a great pirate ship creaks eerily as it is pushed against the rocks.  Do you wish to search the ship?
    The rotting hulk of a great pirate ship creaks eerily as it is pushed against the rocks.  Do you wish to search the shipwreck?
    The sorceresses to the northeast are rebelling! For the good of the empire you must quash their feeble uprising on your way to the mountains.
    The sounds of combat draw you to the scene of a fight between an old Barbarian and an eight-headed Hydra.  Your timely intervention swings the battle in favor of the man, and he rewards you with a pendant he used to use to calm his mind for battle.
    The spring only refills once a week, and someone's already been here this week.
    The targeted creatures are paralyzed, unable to move or retaliate.
    The three Anduran artifacts magically combine into one.
    The truth is out there.
    The two positions selected can not be swapped.
    There are 4 possible loss conditions, as indicated by the following icons:
    There are 6 possibilities:
    There are seven resources in Heroes 2, used to build and improves castles, purchase troops and recruit heroes.  Gold is the most common, required for virtually everything.  Wood and ore are used for most buildings.  Gems, Mercury, Sulfur and Crystal are rare magical resources used for the most powerful creatures and buildings.
    There are two enemies allied against you in this mission.  Both are well armed and seek to evict you from their island.  Avoid them and capture Dragon City to win.
    There is no open space adjacent to your hero to summon an Elemental to.
    There is no room in the garrison for this army.
    There once was a man from Nantucket...
    These weak creatures will surely flee before us.
    Thieves' Guild
    Thieves' Guild: Player Rankings
    This burial site is deathly still.
    This could be a bug -- please report it to me at:
    This eye seems to be intently studying its surroundings.
    This is the final battle against your rival kingdom of Harondale. Eliminate everyone, and don't lose the hero Jarkonas VI.
    This is the final battle.  Both you and your enemy are armed to the teeth, and all are allied against you.  Capture Archibald to end the war!
    This is the final battle.  Both you and your enemy are armed to the teeth, and all are allied against you.  Capture Roland to win the war, and be sure not to lose Archibald in the fight!
    This item can't be traded.
    This lets you change player starting positions and colors.  A particular color will always start in a particular location.  Some positions may only be played by a computer player or only by a human player.
    This lets you change the class of a player.  Classes are not always changeable.  Depending on the scenario, a player may receive additional towns and/or heroes not of their primary alignment.
    This lets you change the starting difficulty at which you will play.  Higher difficulty levels start you off with fewer resources, and at the higher settings, give extra resources to the computer.
    This space for rent
    This town may not be upgraded to a castle.
    This window provides information on the status of your hero or kingdom, and shows the date.  Left click here to cycle through these windows.
    Three allied enemies stand before you and victory, including Lord Corlagon.  Roland is in a castle to the northwest, and you will lose if he falls to the enemy.  Remember that capturing Lord Corlagon will ensure that he will not fight against you in the final scenario.
    Throng
    Thundax
    Thunder
    Thunder Mace
    Thunder Mace of Dominion
    TlsAlloc
    TlsFree
    TlsGetValue
    TlsSetValue
    To cast spells, you must first buy a spell book for 500 gold.  Do you wish to buy one?
    To cast spells, you must first buy a spell book for 500 gold.  Unfortunately, you seem to be a little short of cash at the moment.
    To change color mode, right click in an open area on the Windows 95 background, choose 'Properties', then the 'Settings' tab, then change the entry in the 'Color Palette Box'.
    Toggle 'Show Path' on/off.  If 'Show Path' is on, your first click on a map location will show the path to get there, your second will start you moving. If this option is off, one click starts you moving immediately.
    Toggle ambient music level.
    Toggle color cursors on/off.  Color cursors look nicer, but sometimes don't move as smoothly as black and white ones.
    Toggle foreground sounds level.
    Toggle on or off a shadowing system showing where the hex grid that the mouse cursor is currently over is shadowed.  When this is turned on, there is a slight performance cost, most noticeable on 486 computers.
    Toggle on or off a shadowing system showing where your creatures can move and attack.  When this is turned on, there is a slight performance cost, most noticeable on 486 computers.
    Toggle the hex grid on or off.  The hex grid always underlies movement, even if turned off.  This switch only determines if the grid is visible.
    Toggle the monster info window, which shows information on the active and targeted monsters.
    Toggle whether or not the computer will cast spells for you when auto combat is on.  (Note: This does not affect spell casting for computer players in any way, nor does it affect quick combat.)
    Tomb robber -1
    Too many links
    Too many open files
    Too many open files in system
    Tortoise
    Total Army Strength:
    Total Memory Leaks
    Tower does
    Town Gate
    Town Portal
    Town Screen
    Town/Castle
    Trading Post
    TranslateMessage
    Transmit End
    Transmit Game Start
    Transmit Start - Compressing
    Transmit Start - Sending
    Traveler's Boots
    Traveler's Boots of Mobility
    Travesty
    Treason
    Treasure
    Treasure Chest
    Treaty
    Tree City
    Tree House
    Tree of Knowledge
    Treehouse
    Trident
    Troll Bridge
    Trolls living under the bridge challenge you.  Will you fight them?
    Troops of 3 alignments -1
    Troops of 4 alignments -2
    Troops of 5 alignments -3
    Troyan
    True Compass of Mobility
    Try looking on land!!!
    Try searching on clear ground.
    Turning Point
    Turns Owned
    UNKNOWN_ERROR             
    USER32.dll
    USER32_NULL_THUNK_DATAB
    USVPh 
    UUUUVVWWXXXXY
    Ultimate Artifact
    Ultimate Book of Knowledge
    Ultimate Cloak of Protection
    Ultimate Crown
    Ultimate Shield
    Ultimate Staff
    Ultimate Sword of Dominion
    Ultimate Wand of Magic
    Ultimate crown
    Ultra Fast
    Unable to access CD-ROM Drive.  Without a CD-ROM drive and a Heroes 2 Expansion CD-ROM you will only be able to play as the guest in a multi-player game.
    Unable to change to the Heroes II directory.  Please run the installation program.
    Unable to find the Heroes II data files.  Please run the installation program.
    Unable to initialize input devices - possible problem with mouse or keyboard.
    Unable to initialize mouse.
    Unable to initialize resources - possible disk problem.
    Unable to initialize sound.
    Unable to initialize windows - possible memory or disk error.
    Unable to set up CD stereo music.  Your CD player might be in use by another program, or your sound driver might not support CD stereo.
    Uncharted Territory
    Uncle Ivan
    Unexpected Program Termination
    Unfortunately, you do not have the wisdom to understand the spell, and you are unable to learn it.  
    Unfortunately, you have no Magic Book to record the spell with.
    UnhandledExceptionFilter
    Unicorn
    Unique System ID
    Unknown
    Unknown error
    UpdateWindow
    UpdateWindow Failed
    Upg. Adobe
    Upg. Archery Range
    Upg. Armory
    Upg. Blacksmith
    Upg. Bridge
    Upg. Cathedral
    Upg. Cloud Castle
    Upg. Cottage
    Upg. Foundry
    Upg. Graveyard
    Upg. Ivory Tower
    Upg. Jousting Arena
    Upg. Mansion
    Upg. Mausoleum
    Upg. Maze
    Upg. Pyramid
    Upg. Stick Hut
    Upg. Stonehenge
    Upon cresting a small hill, you come upon a ridiculous looking sight.  A Sprite is attempting to carry a Scepter that is almost as big as it is.  Trying not to laugh, you ask, "Need help?"  The Sprite glares at you and answers: "You think this is funny?  Fine.  You can carry it.  I much prefer flying anyway."
    Upon defeating the Ghosts you sift through the debris and find something!
    Upon defeating the Ghosts you spend several hours sifting through the debris and find nothing.  Such a despicable act reduces your army's morale.
    Upon defeating the Skeletons you sift through the debris and find something!
    Upon defeating the Skeletons you spend several hours sifting through the debris and find nothing.  Such a despicable act reduces your army's morale.
    Upon defeating the Zombies you spend several hours searching the graves and find nothing.  Such a despicable act reduces your army's morale.
    Upon defeating the daemon's servants, you find a hidden cache with 2500 gold.
    Upon defeating the monsters, you decipher an ancient glyph on the wall, telling the secret of the spell - 
    Upon defeating the zomies you search the graves and find something!
    Upon entering the mystical faerie ring, your army gains luck for its next battle.
    Upon your approach, the tree opens its eyes in delight.  "Ahh, an adventurer!  Allow me to teach you a little of what I have learned over the ages."
    Upon your approach, the tree opens its eyes in delight.  "Ahh, an adventurer! I will be happy to teach you a little of what I have learned over the ages for a mere 2000 gold."  (Just bury it around my roots.)
    Upon your approach, the tree opens its eyes in delight.  "It is good to see you, my student.  I hope my teachings have helped you."
    Uprising
    Use COM Port 1 for the direct connection.
    Use COM Port 1 for the modem connection.
    Use COM Port 2 for the direct connection.
    Use COM Port 2 for the modem connection.
    Use COM Port 3 for the direct connection.
    Use COM Port 3 for the modem connection.
    Use COM Port 4 for the direct connection.
    Use COM Port 4 for the modem connection.
    Use Opera
    Use a 19200 baud connection speed.
    Use a 2400 baud connection speed. 
    Use a 38400 baud connection speed.
    Use a 9600 baud connection speed. 
    VDf9V>u
    VGENBKG.ICN
    VGENBKGE.ICN
    VIEWARME.ICN
    VIEWARMY.ICN
    VIEWDDOR.ICN
    VIEWHROS.ICN
    VIEWMINE.ICN
    VIEWPUZL.ICN
    VIEWRSRC.ICN
    VIEWRTFX.ICN
    VIEWTWNS.ICN
    VIEWWRLD.ICN
    VIEW_ALL.ICN
    VQh()R
    VWUuWh
    Vampire
    Vatawna
    Version
    Very Fast
    Very slow
    Vesper
    Victory
    View All
    View Artifacts
    View Bad Luck Info
    View Bad Morale Info
    View Ballista Info
    View Combat Spells
    View Experience Info
    View Good Luck Info
    View Good Morale Info
    View Hero
    View Heroes
    View Mines
    View Neutral Luck Info
    View Neutral Morale Info
    View Opposing Captain
    View Opposing Hero
    View Resources
    View Spell Points Info
    View Spells
    View Towns
    View a summary of your kingdom.
    View adventure Spells
    View all maps, regardless of size.
    View information on the scenario you are currently playing.
    View next page
    View only maps of size extra large (144 x 144).
    View only maps of size large (108 x 108).
    View only maps of size medium (72 x 72).
    View only maps of size small (36 x 36).
    View previous page
    View the credits screen.
    View the entire world.
    View the high score screen.
    View the obelisk puzzle.
    Visions
    Visiting a local wiseman, you explain the intent of your journey.  He reaches into a sack and withdraws a yellowed scroll and hands it to you.
    Volcano
    Volume 1
    Volume 2
    Volume 3
    Volume 4
    Volume 5
    Volume 6
    Volume 7
    Volume 8
    Volume 9
    Voyage Home
    Vulnerable
    WINCMBBE.ICN
    WINCMBTB.ICN
    WING32.dll
    WING32_IMPORT_DESCRIPTOR
    WING32_NULL_THUNK_DATA*
    WINLOSE.ICN
    WINLOSEE.ICN
    WINMM.dll
    WINMM_NULL_THUNK_DATA
    WPhDaQ
    WSACleanup
    WSAGetLastError
    WSAStartup
    WSMSGSTARTUP
    WSOCK32.dll
    WSOCK32.dll&
    WSOCK32_NULL_THUNK_DATA
    WWWWWW
    Wagon Camp
    Wait for other units to go.
    WaitForMultipleObjects
    WaitForSingleObject
    WaitMessage
    Waiting On Guest(s).
    Waiting On Guest.
    Waiting On Host.
    Waiting for other computer to log in to direct connection.
    Waiting for other remote player to set up game.
    Waiting for ring...
    Waiting to receive game data.
    Walk Speed
    Walking through the ruins of an ancient walled city, you find the instrument of the city's destruction, an elaborately crafted ballista.
    Wand of Negation
    Warfare
    Warlock
    Warrior
    Wasteland
    Watch Tower
    Water Summoning Altar
    Water Wheel
    Waterfall
    Watering Hole
    Watering hole visited +1
    Wayward son
    Weapon
    Weasel
    When the connection is successfully made, you can return to full screen mode by pressing 'F4'.
    Where could it be?
    While out scaring up game, your troops find a mysterious staff levitating about three feet off of the ground.  They hand it to you, and you notice an inscription.  It reads: "Brains best brawn and magic beats might.  Heed my words, and you'll win every fight."
    While pausing to rest, you notice a bobcat climbing a short tree to get at a crow's nest.  On impulse, you climb the tree yourself and scare off the cat.  When you look in the nest, you find a collection of shiny stones and a ring.
    While searching through the rubble of a caved-in mine, you free a group of trapped Dwarves.  Grateful, the leader gives you a golden bracelet.
    While venturing into a decrepit hut you find the Skeleton of a long dead witch.  Investigation of the remains reveals a glass eye rolling around inside an empty skull.
    While waiting out a storm, a lighting bolt strikes a nearby cottage's lightning rod, which melts and falls to the ground.  The tip of the rod, however, survives intact and makes your hair stand on end when you touch it.  Hmm...
    Whirlpool
    White Pearl
    White stone pillars support a roof that rises up to the sky.  As you enter the structure, the dead air of the outside gives way to a whirling gust that almost pushes you back out.  The air current materializes into a barely visible form.  The creature asks, in what can only be described as a loud whisper: "Why have you come?  Are you here to call upon the forces of the air?"
    WideCharToMultiByte
    Wilfrey
    WinGBitBlt
    WinGCreateBitmap
    WinGCreateDC
    WinGRecommendDIBFormat
    WinGSetDIBColorTable
    WinGStretchBlt
    WinHelpA
    Windmill
    Wisdom
    Witch Doctor's Hut
    Witch's Broach of Magic
    Witch's Hut
    Wizard
    Wizard's Hat
    Wizard's Isle
    Wizardry
    Wood & Ore:
    Wood bonus
    Wrathmont
    Write communications data
    WriteFile
    Xanadu
    Y?fff?33s?
    You already have 8 heroes, and regretfully must leave the prisoner in this jail to languish in agony for untold days.
    You and your allies 
    You and your ally 
    You are unable to recruit at this time, your ranks are full.
    You beat the Ghosts and are able to restore the mine to production.
    You can't afford to upgrade your troops!
    You cannot pick up this artifact, you already have a full load!
    You catch a leprechaun foolishly sleeping amidst a cluster of magic mushrooms.  In exchange for his freedom, he guides you to a small pot filled with precious things.
    You come across a conjurer who begs to accompany you and your army awhile for safety.  You agree, and he offers as payment a copy of the book of the elements.
    You come across a giant Sphinx.  The Sphinx remains strangely quiet.
    You come across a golem wearing a glowing pendant and blocking your way.   Acting on a hunch, you cut the pendant from its neck.  Deprived of its power source, the golem breaks down, leaving you with the magical pendant.
    You come across a lavish shrine attended by a group of high priests.  In exchange for your protection, they agree to teach you a sophisticated spell - 
    You come across a small shrine attended by a group of novice acolytes.  In exchange for your protection, they agree to teach you a simple spell - 
    You come across an old wagon left by a trader who didn't quite make it to safe terrain.  Inside, you find some of the wagon's cargo still intact.
    You come across an old wagon left by a trader who didn't quite make it to safe terrain.  Unfortunately, others have found it first, and the wagon is empty.
    You come across an ornate shrine attended by a group of rotund friars.  In exchange for your protection, they agree to teach you a spell - 
    You come across the remains of an ancient Druid.  Bones, yellowed with age, peer from the ragged folds of her robe.  Searching the robe, you discover a scroll hidden in the folds.
    You come upon a battle where a Paladin has been mortally wounded by a group of Zombies.  He asks you to take his hammer and finish what he started.  As you pick it up, it begins to hum, and then everything becomes a blur.  The Zombies lie dead, the hammer dripping with blood.  You strap it to your belt.
    You come upon a bridge spanning a dry gully.  Before you can cross, a Troll steps out from under the bridge and demands payment before it will permit you to pass.  You refuse, and the Troll charges, forcing you to slay it.  You take its spiked shield as a trophy.
    You come upon a caravan of gypsies who are feasting and fortifying their bodies with mead.  They call you forward and say "If you prove that you can dance the Rama-Buta, we will reward you."  You don't know it, but try anyway.  They laugh hysterically, but admire your bravery, giving you a Crystal Ball.
    You come upon a sign.  It reads: "Here lies the body of Anduran.  Bow and swear fealty, and you shall be rewarded."  You decide to do as it says.  As you stand up, you feel a coldness against your skin.  Looking down, you find that you are suddenly wearing a gleaming, ornate breastplate.
    You come upon a very old woman, long blind from cataracts and dying alone.  You tend to her final needs and promise a proper burial.  Grateful, she gives you a magic pendant emblazoned with a stylized eye.  It lets you see with your eyes closed.
    You come upon an abandoned gold mine.  The mine appears to be haunted.  Do you wish to enter?
    You come upon an ancient artifact.  As you reach for it, a pack of Rogues leap out of the brush to guard their stolen loot.
    You come upon an obelisk made from a type of stone you have never seen before.  Staring at it intensely, the smooth surface suddenly changes to an inscription.  The inscription is a piece of a lost ancient map.  Quickly you copy down the piece and the inscription vanishes as abruptly as it had appeared.
    You come upon the pyramid of a great and ancient king.  Routine exploration reveals that the pyramid is completely empty.
    You come upon the pyramid of a great and ancient king.  You are tempted to search it for treasure, but all the old stories warn of fearful curses and undead guardians.  Will you search?
    You come upon the remains of an unfortunate adventurer.  Searching through the tattered clothing, you find 
    You come upon the remains of an unfortunate adventurer.  Searching through the tattered clothing, you find nothing.
    You don't have enough gold!
    You don't have enough memory for non-interlaced video.
    You drink from the enchanted fountain, but nothing happens.
    You encounter the infamous Black Knight!  After a grueling duel ending in a draw, the Knight, out of respect, offers you a pair of armored gauntlets.
    You enter a recently burned glade and come upon a Fire Elemental sitting atop a rock.  It looks up, its flaming face contorted in a look of severe pain.  It then tosses a glowing object at you.  You put up your hands to block it, but it passes right through them and sears itself into your chest.
    You enter a rickety hut and talk to the magician who lives there.  He tells you of places near and far which may aid you in your journeys.
    You enter the arena and face a pack of vicious lions.  You handily defeat them, to the wild cheers of the crowd.  Impressed by your skill, the aged trainer of gladiators agrees to train you in a skill of your choice.
    You enter the faerie ring, but nothing happens.
    You enter the structure of white stone pillars, and find nothing.
    You find a powerful and grotesque Demon in the cave.  "Today," it rasps, "you will fight and surely die.  But I will give you a choice of deaths.  You may fight me, or you may fight my servants.  Do you prefer to fight my servants?"
    You find a small tree that closely resembles the great Warlock Carnauth with a ring around one of its twigs.  Scraps of clothing and rotting leather lead you to suspect that it IS Carnauth, transformed.  Since you can't help him, you take the magic ring.
    You find an elaborate container which houses an old vellum scroll. The runes on the container are very old, and the artistry with which it was put together is stunning. As you pull the scroll out, you feel imbued with magical power.
    You gain control of a crystal mine. It will provide you with one unit of crystal per day.
    You gain control of a gem mine. It will provide you with one unit of gems per day.
    You gain control of a gold mine. It will provide you with 1000 gold per day.
    You gain control of a sawmill. It will provide you with two units of wood per day.
    You gain control of a sulfur mine. It will provide you with one unit of sulfur per day.
    You gain control of an ore mine. It will provide you with two units of ore per day.
    You have already been to this obelisk.
    You have already cast a spell this round.
    You have been eliminated from the game!!!
    You have captured a mischievous imp who has been terrorizing the region.  In exchange for his release, he rewards you with a magical coin.
    You have captured an enemy artifact!
    You have failed to complete your quest in time.  All is lost.
    You have no hero to cast a spell.
    You have no room to carry another artifact!
    You have received quite a bargain.  I expect to make no profit on the deal.  Can I interest you in any of my other wares?
    You have taken control of the local Alchemist shop. It will provide you with one unit of Mercury per day.
    You have your crew stop up their ears with wax before the sirens' eerie song has any chance of luring them to a watery grave.
    You hear a voice from behind the locked door, "You don't have enough gold to pay for my services."
    You hear a voice from high above in the tower, "Go away! I can't help you!"
    You may only summon one type of elemental per combat.
    You meet an old Wizard friend of yours traveling in the opposite direction.  He presents  you with a gift:  A wand that prevents the use of the dispel magic spell on your allies.
    You must be standing on the entrance to a mine (sawmills and alchemists don't count) to cast this spell.
    You must be within 3 spaces of a monster for the Visions spell to work.
    You must purchase a spell book to use the mage guild, but you currently have no room for a spell book.  Try giving one of your artifacts to another hero.
    You must put down a peasant revolt led by Roland's forces.  All are allied against you, but you have Lord Corlagon, an experienced hero, to help you.  Capture all enemy castles to win.
    You must take control of the castle of Magic, where the fount of wizardry lies. Do this and your victory will be supreme.
    You must unify the barbarian tribes of the north by conquering them.  As in the previous mission, the enemy is not allied against you, but they have more resources at their disposal.  You will win when you own all of the enemy castles and there are no more heroes left to fight.
    You own Dragon City.
    You own the Lighthouse.
    You pause to help a diplomat with a broken axle fix his problem.  In gratitude, he gives you a writing quill with magical properties which he says will "help people see things your way".
    You search the ruins, but the Medusas that used to live here are gone.  Perhaps there will be more next week.
    You search through the flotsam and find some wood and some gold.
    You search through the flotsam, and find some wood and some gold.
    You search through the flotsam, and find some wood.
    You search through the flotsam, but find nothing.
    You see a Wizard fleeing from a Griffin and riding like the wind.  The Wizard opens a portal and rides through, getting his hat knocked off by the edge of the gate.  The Griffin follows; the gate closes.  You pick the hat up, dust it off, and put it on.
    You see a silver axe embedded deeply in the ground.  After several unsuccessful attempts by your army to remove the axe, you tightly grip the handle of the axe and effortlessly pull it free.
    You speak, and nothing happens.
    You spy a gleaming object poking up out of the ground.   You send a member of your party over to investigate.  He comes back with a golden helmet in his hands.  You realize that it must be the helmet of the legendary Anduran, the only man who was known to wear solid gold armor.
    You stand before the Dragon City, a place off-limits to mere humans.  Do you wish to violate this rule and challenge the Dragons to a fight?
    You stop to help a Peasant catch a runaway mare.  To show his gratitude, he hands you a tiny sphere.  As soon as you grasp it, you feel the magical energy drain from your limbs...
    You stumble upon a dented and tarnished lamp lodged deep in the earth. Do you wish to rub the lamp?
    You stumble upon a medal lying alongside the empty road.  Adding the medal to your inventory, you become aware that you have acquired the undesirable Fizbin of Misfortune, greatly decreasing your army's morale.
    You tentatively approach the burial ground of ancient warriors.  Do you want to search the graves?
    You try to pay the leprechaun, but realize that you can't afford it.  The leprechaun stamps his foot and ignores you.
    You visit an alchemist who, upon seeing your army, is swayed by the righteousness of your cause.  The newly loyal subject gives you his endless pouch of sulfur to help with the war effort.
    You will face four allied enemies in a straightforward fight for resources and treasure.  Capture all of the enemy castles for victory.
    You've come upon a fight between a Necromancer and a Paladin.  The Necromancer blasts the Paladin with a fire bolt, bringing him to his knees.  Acting quickly, you slay the evil one before the final blow.  The grateful Paladin gives you the fire cloak that saved him.
    You've come upon a mercenary camp practicing their tactics.  "You're too advanced for us," the mercenary captain says.  "We can teach nothing more."
    You've come upon a mercenary camp practicing their tactics.  The mercenaries welcome you and your troops and invite you to train with them.
    You've found a Goblin weapon smithy making weapons for use against humans.  With a tremendous yell you and your army descend upon their camp and drive them away.  A search finds a magic ore cart that never runs out of iron.
    You've found a Sprite Tree City.  Unfortunately, none of the Sprites living there wish to join an army.  Maybe next week.
    You've found a group of Druids worshipping at one of their strange stone edifices.  Silently, the Druids turn you away, indicating they have nothing new to teach you.
    You've found a group of Druids worshipping at one of their strange stone edifices.  Silently, they teach you new ways to cast spells.
    You've found a magic garden, the kind of place that leprechauns and faeries like to cavort in, but there is no one here today.  Perhaps you should try again next week.
    You've found an abandoned lean-to.  Poking about, you discover some resources hidden nearby.
    You've found an ancient and weathered stone idol.  It is supposed to grant luck to visitors, but since the stars are already smiling upon you, it does nothing.
    You've found an ancient and weathered stone idol.  Kissing it is supposed to be lucky, so you do.  The stone is very cold to the touch.
    You've found an oddly shaped ring on the finger of a long dead traveler.  The ring looks like a snake biting its own tail.
    You've found one of those bridges that Trolls are so fond of living under, but there are none here.  Perhaps there will be some next week.
    You've found some Medusas living in the ruins.  They are willing to join your army for a price.  Do you want to recruit Medusas?
    You've found the ruins of an ancient city, now inhabited solely by the undead.  Will you search?
    You've pulled a shipwreck survivor from certain death in an unforgiving ocean.  Grateful, he says, "I would give you an artifact as a reward, but you're all full."
    Your answer?
    Your army is ambushed by a small tribe of wild (and none too bright) Orcs.  You fend them off easily and the survivors flee in all directions.  One of the Orcs was wearing a polished spiked helm.  Figuring it will make a good souvenir, you take it.
    Your attack skill is a bonus added to each creature's attack skill.
    Your big spending habits have earned you a massive tax bill that you can't hope to pay.  The tax man takes pity and agrees to only take 250 gold a day from your account for life.  Check here if you want one dollar to go to the presidential campaign election fund.
    Your cowardly forces flee from battle.
    Your defense skill is a bonus added to each creature's defense skill.
    Your enemies are allied against you and start close by, so be ready to come out fighting.  You will need to own all four castles in this small valley to win.
    Your forces suffer a bitter defeat.
    Your forces surrender to the enemy, and depart in shame.
    Your hero is too tired to cast this spell today.  Try again tomorrow.
    Your hero produces 100 gold pieces per turn as tax revenue from estates.
    Your hero produces 250 gold pieces per turn as tax revenue from estates.
    Your hero produces 500 gold pieces per turn as tax revenue from estates.
    Your knowledge determines how many spell points your hero may have.  Under normal cirumstances, a hero is limited to 10 spell points per level of knowledge.
    Your looting of the grave of Sinfilas Gardolad, the famous shapeshifting Warlock, unearths his fabled mask.  Trembling, you put it on and it twists your visage into an awful grimace!  Oh no!  It's actually the hideous mask of Gromluck Greene, and you are stuck with it.
    Your men spot a navigational buoy, confirming that you are on course and increasing their morale.
    Your men spot a navigational buoy, confirming that you are on course.
    Your mission is to vanquish the warring mages in the magical Shrouded Isles. The completion of this task will give you a fighting chance against your rivals.
    Your orders are to conquer the country lords that have sworn to serve Roland.  All of the enemy castles are unified against you.  Since you start without a castle, you must hurry to capture one before the end of the week.  Capture all enemy castles for victory.
    Your remote connection to the other players appears to be broken.  Do you wish to keep waiting for a response?
    Your rival, the kingdom of Harondale, is attacking weak towns on your border! Recover from their first strike and crush them completely!
    Your spell power determines the length or power of a spell.
    Your task is to defend the Dwarves against Archibald's forces.  Capture all of the enemy towns and castles to win, and be sure not to lose all of the dwarf towns at once, or the enemy will have won.
    Your troops can be upgraded, but it will cost you dearly.  Do you wish to upgrade them?
    Your wanderings bring you into contact with a tribe of ape-like beings using a magical lava rock that never cools to light their fires.  You take pity on them and teach them to make fire with sticks.  Believing you to be a god, the apes give you their rock.
    Zombie
    Zounds
    Zounds...
    [ Incoming data, must exit... ]
    \$$PUS
    \$$SRQP
    \HEROES2\ANIM\
    _$I10_OUTPUT
    _?ReceiveHeroTownData@advManager@@QAEXPADPAH11PAPAVhero@@PAPAVarmyGroup@@PAPAVtown@@23111PAC55@Z
    _AIL_allocate_sample_handle@4
    _AIL_allocate_sequence_handle@4
    _AIL_allocate_sequence_handle@4*
    _AIL_end_sample@4
    _AIL_get_preference@4
    _AIL_init_sample@4
    _AIL_init_sequence@12
    _AIL_last_error@0
    _AIL_midiOutClose@4
    _AIL_midiOutClose@46
    _AIL_midiOutOpen@12
    _AIL_midiOutOpen@12&
    _AIL_redbook_close@4
    _AIL_redbook_open@4
    _AIL_redbook_open@4"
    _AIL_redbook_track_info@16
    _AIL_redbook_tracks@4
    _AIL_release_sequence_handle@4
    _AIL_resume_sequence@4
    _AIL_sample_status@4
    _AIL_sample_volume@4
    _AIL_sequence_status@4
    _AIL_serve@0
    _AIL_set_XMIDI_master_volume@8
    _AIL_set_preference@8
    _AIL_set_sample_address@12
    _AIL_set_sample_loop_count@8
    _AIL_set_sample_playback_rate@8
    _AIL_set_sample_playback_rate@8"
    _AIL_set_sample_type@12
    _AIL_set_sample_type@12&
    _AIL_set_sample_volume@8
    _AIL_set_sequence_loop_count@8
    _AIL_shutdown@0
    _AIL_shutdown@0B
    _AIL_start_sample@4
    _AIL_start_sequence@4
    _AIL_startup@0
    _AIL_stop_sequence@4
    _AIL_waveOutOpen@16
    _AdjustWindowRect@12
    _AnimatePalette@16
    _BeginPaint@8
    _BitClear
    _BitSet&
    _BitTest
    _CheckMenuItem@12
    _ClearCommBreak@4
    _ClearCommError@12
    _ClientToScreen@8
    _CloseHandle@4
    _CreateBitmapIndirect@4"
    _CreateEventA@16
    _CreateFileA@282
    _CreateIconIndirect@4
    _CreatePalette@4
    _CreateWindowExA@48.
    _DefWindowProcA@16
    _DeleteCriticalSection@4
    _DeleteDC@4
    _DeleteFileA@4
    _DeleteObject@4"
    _DestroyIcon@4
    _DestroyMenu@4
    _DestroyWindow@4
    _DialogBoxParamA@20&
    _DispatchMessageA@4&
    _DrawMenuBar@4
    _EnableMenuItem@12
    _EndDialog@8
    _EndPaint@8*
    _EnterCriticalSection@4"
    _ExitProcess@4
    _FatalAppExitA@8
    _FindClose@4
    _FindFirstFileA@8
    _FindNextFileA@8
    _FlushFileBuffers@4.
    _FreeEnvironmentStringsA@4
    _FreeEnvironmentStringsW@4
    _FreeLibrary@4
    _GdiSetBatchLimit@42
    _GetACP@0
    _GetAsyncKeyState@4
    _GetCPInfo@8
    _GetClientRect@8
    _GetCommModemStatus@8
    _GetCommState@86
    _GetCommTimeouts@8
    _GetCommandLineA@0
    _GetCurrentDirectoryA@8*
    _GetCurrentThread@0
    _GetCurrentThreadId@0
    _GetCursorPos@4"
    _GetDC@4
    _GetDeviceCaps@8
    _GetDriveTypeA@4
    _GetEnvironmentStrings@0
    _GetEnvironmentStringsW@0
    _GetFileAttributesA@4
    _GetFileType@4
    _GetLastError@0"
    _GetLogicalDrives@0.
    _GetMenuItemCount@4.
    _GetMenuItemID@8
    _GetMessageA@16.
    _GetModuleFileNameA@12
    _GetModuleHandleA@4>
    _GetOEMCP@0"
    _GetProcAddress@8
    _GetStartupInfoA@4
    _GetStdHandle@4
    _GetStringTypeA@20
    _GetStringTypeW@16
    _GetSubMenu@8
    _GetSystemPaletteEntries@166
    _GetTickCount@0*
    _GetVersion@0
    _GetVersionExA@4
    _GetWindowLongA@8
    _GetWindowRect@8
    _HeapAlloc@12
    _HeapCreate@12
    _HeapDestroy@4
    _HeapFree@12
    _HeapReAlloc@16*
    _HeapSize@12
    _InitializeCriticalSection@4
    _InvalidateRect@12
    _LCMapStringA@24
    _LCMapStringW@24
    _LeaveCriticalSection@4:
    _LoadCursorA@8
    _LoadIconA@8
    _LoadLibraryA@4B
    _LoadMenuA@8
    _MessageBeep@4
    _MessageBoxA@16
    _MoveWindow@24
    _MultiByteToWideChar@24
    _Netbios@4
    _OffsetRect@12
    _OutputDebugStringA@4
    _PatBlt@24
    _PeekMessageA@20
    _PostMessageA@16
    _PostQuitMessage@4
    _RaiseException@16
    _ReadFile@20
    _RealizePalette@4
    _RegCloseKey@4
    _RegCreateKeyA@12
    _RegOpenKeyExA@20
    _RegQueryValueExA@24
    _RegSetValueExA@24
    _RegisterClassA@4
    _ReleaseCapture@0
    _ReleaseDC@8
    _ResetEvent@4
    _RtlUnwind@16
    _ScreenToClient@8
    _SelectObject@8
    _SelectPalette@12
    _SetCapture@4
    _SetCommBreak@4
    _SetCommState@8
    _SetCommTimeouts@8
    _SetConsoleCtrlHandler@8
    _SetCurrentDirectoryA@4
    _SetCursor@4
    _SetEndOfFile@4*
    _SetEnvironmentVariableA@8
    _SetEvent@4.
    _SetFilePointer@16
    _SetHandleCount@4
    _SetLastError@4*
    _SetMenu@8
    _SetStdHandle@8
    _SetTimer@16
    _SetWindowLongA@12
    _SetupComm@12
    _ShowCursor@4
    _ShowWindow@8
    _Sleep@4
    _SmackClose@4
    _SmackDoFrame@4
    _SmackNextFrame@4
    _SmackOpen@12
    _SmackSoundUseDirectSound@4&
    _SmackSoundUseMSS@4"
    _SmackSummary@8
    _SmackToBuffer@28
    _SmackToBufferRect@8
    _SmackWait@4
    _TileToBitmap
    _TlsAlloc@0
    _TlsFree@4
    _TlsGetValue@4
    _TlsSetValue@8
    _TranslateMessage@4"
    _UnhandledExceptionFilter@4:
    _UpdateWindow@42
    _WSACleanup@0
    _WSAGetLastError@0
    _WSAStartup@8
    _WaitForMultipleObjects@16
    _WaitForSingleObject@8
    _WaitMessage@0
    _WideCharToMultiByte@32"
    _WinGBitBlt@32
    _WinGCreateBitmap@12
    _WinGCreateDC@0*
    _WinGRecommendDIBFormat@4
    _WinGSetDIBColorTable@16
    _WinGStretchBlt@40
    _WinHelpA@16
    _WinMain@16"
    _WinMainCRTStartup
    _WriteFile@20
    __87except
    __CIacos
    __CIasin
    __CIatan
    __CIatan2
    __CIcos
    __CIexp
    __CIlog
    __CIlog10
    __CIpow
    __CIsin
    __CIsqrt
    __CItan
    __C_Termination_Done
    __CopyMan
    __FF_MSGBANNER
    __FPinit
    __FPmtinit
    __FPmtterm
    __FillZeroMan
    __First_FPE_Indx
    __GET_RTERRMSG
    __IMPORT_DESCRIPTOR_ADVAPI32
    __IMPORT_DESCRIPTOR_GDI32
    __IMPORT_DESCRIPTOR_KERNEL32
    __IMPORT_DESCRIPTOR_NETAPI32
    __IMPORT_DESCRIPTOR_USER32
    __IMPORT_DESCRIPTOR_WINMM
    __IMPORT_DESCRIPTOR_WSOCK32>
    __IMPORT_DESCRIPTOR_mss32
    __IMPORT_DESCRIPTOR_smackw32
    __IncMan
    __IsZeroMan>
    __NLG_Destination
    __NLG_Dispatch
    __NLG_Notify
    __NLG_Notify1
    __NLG_Return2
    __NMSG_WRITE
    __NULL_IMPORT_DESCRIPTOR
    __Num_FPE
    __OP_ACOSjmptab.
    __OP_ASINjmptab
    __OP_ATAN2jmptab
    __OP_ATANjmptab&
    __OP_COSjmptab
    __OP_EXPjmptab
    __OP_LOG10jmptab
    __OP_LOGjmptab
    __OP_POWjmptab
    __OP_SINjmptab
    __OP_SQRTjmptab
    __OP_TANjmptab
    __RoundMan
    __ShrMan
    __XcptActTab
    __XcptActTabCount
    __XcptActTabSize
    __XcptFilter
    __ZeroTail
    ___STRINGTOLD
    ___add_12
    ___addl
    ___app_type
    ___argc
    ___argv
    ___badioinfo
    ___crtGetEnvironmentStringsA
    ___crtGetEnvironmentStringsW
    ___crtGetStringTypeA
    ___crtGetStringTypeW
    ___crtLCMapStringA
    ___crtLCMapStringW
    ___crtMessageBoxA
    ___decimal_point
    ___decimal_point_length
    ___doserrno2
    ___dtold
    ___endstdio:
    ___error_mode
    ___fastflag
    ___fpecode
    ___initenv
    ___initmbctable&
    ___initstdio
    ___lc_codepage
    ___lc_handle
    ___ld12mul
    ___lookuptable
    ___mb_cur_max
    ___mbcodepage
    ___mblcid
    ___mbulinfo"
    ___mtold12
    ___multtenpow122
    ___nullstring
    ___onexitbegin
    ___onexitend
    ___onexitinit
    ___piob
    ___pioinfo
    ___pxcptinfoptrs
    ___setfflag
    ___shl_12
    ___shr_12
    ___strgtold12
    ___threadhandle
    ___threadid.
    ___tlsindex
    ___wargv
    ___winitenv6
    ___wnullstring
    __abnormal_termination
    __access
    __acmdln
    __adbgmsg
    __add_exp
    __adj_fdiv_m16i.
    __adj_fdiv_m32
    __adj_fdiv_m32i"
    __adj_fdiv_m64
    __adj_fdiv_r
    __adj_fdivr_m16i
    __adj_fdivr_m32
    __adj_fdivr_m32i
    __adj_fdivr_m64"
    __adj_fpatan
    __adj_fprem"
    __adj_fprem1
    __adj_fptan
    __adjust_fdiv
    __aenvptr
    __aexit_rtn"
    __alloc_osfhnd
    __alloca_probe
    __amblksiz
    __amsg_exit.
    __assert
    __atodbl
    __atoflt
    __atoldbl
    __aulldiv
    __aullrem
    __bufin
    __c_exit
    __callnewh
    __cexit
    __cfltcvt
    __cfltcvt_init
    __cfltcvt_tab
    __cflush
    __cftoe
    __cftof
    __cftog
    __chdir
    __chgsign
    __chkstk
    __chsize
    __chsize_lk*
    __cinit
    __cintrindisp1
    __cintrindisp2
    __clearfp
    __close
    __close_lk
    __clrfp
    __commit
    __commode
    __control87
    __controlfp"
    __copysign
    __cropzeros
    __crtheap
    __ctrandisp1
    __ctrandisp2
    __ctrlfp
    __ctype
    __d_ind
    __d_inf2
    __d_inttype&
    __d_max
    __d_min"
    __d_mzero
    __decomp
    __dosmaperr
    __environ
    __errcode
    __errno
    __except1
    __except2
    __except_handler3
    __exit
    __exitflag
    __fFATN2
    __fFCOS
    __fFEXP
    __fFLN
    __fFSIN
    __fassign
    __fclose_lk&
    __fcloseall*
    __fdivp_sti_st
    __fdivrp_sti_st
    __ffexpm1
    __fflush_lk&
    __filbuf
    __finite
    __fload
    __flsbuf
    __fltout2
    __fltused
    __flush
    __flushall
    __fmode6
    __forcdecpt*
    __fpclass
    __fpclear
    __fpmath
    __fprem1_common
    __fprem_common
    __fpreset
    __fptostr
    __fptrap
    __fread_lk
    __free_osfhnd
    __freebuf
    __freeptd
    __frnd
    __fseek_lk
    __fsopen
    __ftbuf
    __ftell_lk
    __ftol
    __fwrite_lk2
    __get_exp
    __get_osfhandle
    __getbuf
    __getmbcp
    __getptd
    __getstream
    __global_unwind2
    __handle_exc
    __handle_qnan1
    __handle_qnan2
    __heap_alloc
    __heap_init*
    __heap_term
    __imp__AIL_end_sample@4
    __imp__AIL_get_preference@4"
    __imp__AIL_init_sample@4
    __imp__AIL_init_sequence@12"
    __imp__AIL_last_error@0
    __imp__AIL_midiOutClose@4
    __imp__AIL_midiOutOpen@12
    __imp__AIL_redbook_close@4
    __imp__AIL_redbook_open@4
    __imp__AIL_redbook_tracks@4
    __imp__AIL_resume_sequence@4
    __imp__AIL_sample_status@4
    __imp__AIL_sample_volume@4
    __imp__AIL_sequence_status@4
    __imp__AIL_serve@0
    __imp__AIL_set_preference@8
    __imp__AIL_set_sample_type@12
    __imp__AIL_set_sample_volume@8
    __imp__AIL_shutdown@0
    __imp__AIL_start_sample@4
    __imp__AIL_start_sequence@42
    __imp__AIL_startup@0
    __imp__AIL_stop_sequence@4
    __imp__AIL_waveOutOpen@16
    __imp__AdjustWindowRect@12
    __imp__AnimatePalette@16
    __imp__BeginPaint@8"
    __imp__CheckMenuItem@12*
    __imp__ClearCommBreak@4
    __imp__ClearCommError@12
    __imp__ClientToScreen@8
    __imp__CloseHandle@4
    __imp__CreateBitmapIndirect@4
    __imp__CreateEventA@16
    __imp__CreateFileA@28
    __imp__CreateIconIndirect@4"
    __imp__CreatePalette@4
    __imp__CreateWindowExA@48
    __imp__DefWindowProcA@16
    __imp__DeleteCriticalSection@4
    __imp__DeleteDC@4
    __imp__DeleteFileA@4
    __imp__DeleteObject@4
    __imp__DestroyIcon@4
    __imp__DestroyMenu@4
    __imp__DestroyWindow@4
    __imp__DialogBoxParamA@20
    __imp__DispatchMessageA@4
    __imp__DrawMenuBar@4
    __imp__EnableMenuItem@12
    __imp__EndDialog@8
    __imp__EndPaint@8
    __imp__EnterCriticalSection@4
    __imp__ExitProcess@4
    __imp__FatalAppExitA@8
    __imp__FindClose@4
    __imp__FindFirstFileA@8
    __imp__FindNextFileA@8
    __imp__FlushFileBuffers@4
    __imp__FreeLibrary@4
    __imp__GdiSetBatchLimit@4
    __imp__GetACP@0"
    __imp__GetAsyncKeyState@4
    __imp__GetCPInfo@8
    __imp__GetClientRect@8
    __imp__GetCommModemStatus@8"
    __imp__GetCommState@8
    __imp__GetCommTimeouts@8
    __imp__GetCommandLineA@0
    __imp__GetCurrentDirectoryA@8
    __imp__GetCurrentThread@0
    __imp__GetCurrentThreadId@0
    __imp__GetCursorPos@4
    __imp__GetDC@4
    __imp__GetDeviceCaps@8
    __imp__GetDriveTypeA@4
    __imp__GetEnvironmentStrings@0
    __imp__GetEnvironmentStringsW@0"
    __imp__GetFileAttributesA@4
    __imp__GetFileType@4
    __imp__GetLastError@0
    __imp__GetLogicalDrives@0
    __imp__GetMenuItemCount@4
    __imp__GetMenuItemID@8
    __imp__GetMessageA@16
    __imp__GetModuleFileNameA@12
    __imp__GetModuleHandleA@4
    __imp__GetOEMCP@0
    __imp__GetProcAddress@8"
    __imp__GetStartupInfoA@4
    __imp__GetStdHandle@4
    __imp__GetStringTypeA@20
    __imp__GetStringTypeW@16
    __imp__GetSubMenu@8.
    __imp__GetTickCount@0
    __imp__GetVersion@0&
    __imp__GetVersionExA@4
    __imp__GetWindowLongA@8
    __imp__GetWindowRect@8
    __imp__HeapAlloc@12.
    __imp__HeapCreate@12
    __imp__HeapDestroy@4
    __imp__HeapFree@12
    __imp__HeapReAlloc@16
    __imp__HeapSize@12
    __imp__InvalidateRect@12
    __imp__LCMapStringA@24
    __imp__LCMapStringW@24
    __imp__LeaveCriticalSection@4
    __imp__LoadCursorA@8
    __imp__LoadIconA@8
    __imp__LoadLibraryA@4
    __imp__LoadMenuA@8
    __imp__MessageBeep@4
    __imp__MessageBoxA@16
    __imp__MoveWindow@24
    __imp__MultiByteToWideChar@24
    __imp__Netbios@4
    __imp__OffsetRect@12
    __imp__OutputDebugStringA@4&
    __imp__PatBlt@24
    __imp__PeekMessageA@20
    __imp__PostMessageA@16
    __imp__PostQuitMessage@4
    __imp__RaiseException@16
    __imp__ReadFile@20
    __imp__RealizePalette@4"
    __imp__RegCloseKey@4
    __imp__RegCreateKeyA@12"
    __imp__RegOpenKeyExA@20
    __imp__RegQueryValueExA@24
    __imp__RegSetValueExA@24
    __imp__RegisterClassA@4&
    __imp__ReleaseCapture@0"
    __imp__ReleaseDC@8
    __imp__ResetEvent@4&
    __imp__RtlUnwind@16"
    __imp__ScreenToClient@8"
    __imp__SelectObject@8
    __imp__SelectPalette@12.
    __imp__SetCapture@46
    __imp__SetCommBreak@4
    __imp__SetCommState@8
    __imp__SetCommTimeouts@8
    __imp__SetConsoleCtrlHandler@8
    __imp__SetCurrentDirectoryA@4
    __imp__SetCursor@4
    __imp__SetEndOfFile@4
    __imp__SetEvent@4
    __imp__SetFilePointer@16
    __imp__SetHandleCount@4*
    __imp__SetLastError@4
    __imp__SetMenu@8
    __imp__SetStdHandle@8
    __imp__SetTimer@16
    __imp__SetWindowLongA@12
    __imp__SetupComm@12
    __imp__ShowCursor@4>
    __imp__ShowWindow@8"
    __imp__Sleep@4
    __imp__SmackClose@4*
    __imp__SmackDoFrame@4
    __imp__SmackNextFrame@4"
    __imp__SmackOpen@126
    __imp__SmackSoundUseMSS@4
    __imp__SmackSummary@8
    __imp__SmackToBuffer@28
    __imp__SmackToBufferRect@8
    __imp__SmackWait@4
    __imp__TlsAlloc@0
    __imp__TlsFree@4
    __imp__TlsGetValue@4
    __imp__TlsSetValue@8
    __imp__TranslateMessage@4
    __imp__UpdateWindow@4
    __imp__WSACleanup@0.
    __imp__WSAGetLastError@0
    __imp__WSAStartup@8*
    __imp__WaitForSingleObject@8
    __imp__WaitMessage@0
    __imp__WideCharToMultiByte@32
    __imp__WinGBitBlt@32
    __imp__WinGCreateBitmap@12
    __imp__WinGCreateDC@0
    __imp__WinGRecommendDIBFormat@4
    __imp__WinGSetDIBColorTable@16
    __imp__WinGStretchBlt@40
    __imp__WinHelpA@16
    __imp__WriteFile@20&
    __imp__auxSetVolume@8
    __imp__bind@12
    __imp__closesocket@4
    __imp__gethostbyname@4
    __imp__gethostname@8
    __imp__htonl@4
    __imp__htons@4
    __imp__inet_addr@4
    __imp__inet_ntoa@4
    __imp__ioctlsocket@12
    __imp__lstrlenA@4
    __imp__mciGetErrorStringA@12
    __imp__mciSendStringA@16
    __imp__recvfrom@24
    __imp__sendto@24
    __imp__socket@12
    __imp__waveOutGetDevCapsA@12
    __imp__waveOutGetNumDevs@0
    __imp__wsprintfA
    __indefinite
    __infinity
    __initptd
    __ioinit
    __ioterm
    __isatty
    __isctype
    __ismbbalnum
    __ismbbalpha
    __ismbbgraph
    __ismbbkalnum
    __ismbbkana
    __ismbbkprint
    __ismbbkpunct
    __ismbblead"
    __ismbbprint
    __ismbbpunct
    __ismbbtrail
    __isnan
    __itoa
    __ld12cvt
    __ld12tod
    __ld12tof
    __ld12told
    __ldused
    __local_unwind2*
    __lock
    __lock_fhandle
    __lock_file"
    __lock_file2
    __lockerr_exit
    __lockexit
    __locktable
    __logb
    __logemax
    __lseek
    __lseek_lk
    __ltoa
    __matherr
    __matherr_flag
    __mbctoupper
    __mbctype
    __minfinity"
    __ms_p5_mp_test_fdiv
    __ms_p5_test_fdiv
    __msize
    __mtdeletelocks
    __mtinit
    __mtinitlocks
    __mtterm
    __nan2
    __newmode
    __nextafter&
    __nh_malloc
    __nhandle
    __nosnan2
    __nstream
    __onexit
    __open
    __open_osfhandle
    __openfile
    __osver
    __output
    __pctype
    __pgmptr
    __piby2
    __positive
    __pow10neg
    __pow10pos
    __powhlp
    __purecall
    __pwctype
    __raise_exc6
    __read
    __read_lk
    __rtchsifneg
    __rtindfnpop
    __rtindfpop
    __rtinfnpop
    __rtinfnpopse
    __rtinfpop
    __rtinfpopse
    __rtnospop
    __rtnospopde
    __rtonenpop
    __rtonepop
    __rtpiby2
    __rttosnpop
    __rttosnpopde
    __rttospop
    __rttospopde
    __rtzeronpop
    __rtzeropop*
    __safe_fdiv
    __safe_fdivr
    __safe_fprem
    __safe_fprem1
    __scalb"
    __seh_longjmp_unwind@4
    __set_bexp
    __set_errno&
    __set_exp
    __set_osfhnd
    __set_statfp
    __setargv
    __setdefaultprecision
    __setenvp
    __setmbcp
    __setmode
    __setmode_lk
    __sopen
    __sptype
    __statfp
    __statusfp
    __stbuf
    __stdbuf
    __strcmpi
    __stricmp
    __strnicmp
    __strrev
    __strupr
    __sys_errlist
    __sys_nerr
    __tell
    __tolower
    __tolower_lk
    __tosnan1
    __tosnan2
    __toupper
    __toupper_lk
    __trandisp1
    __trandisp2
    __ultoa
    __umaskval
    __umatherr
    __ungetc_lk.
    __unlink
    __unlock
    __unlock_fhandle
    __unlock_file
    __unlock_file2
    __unlockexit
    __wctomb_lk:
    __wenviron
    __wenvptr
    __winmajor
    __winminor
    __winver
    __wpgmptr
    __write
    __write_lk
    _abort
    _access
    _atan2
    _atexit
    _auxSetVolume@8
    _bind@12
    _calloc
    _close
    _closesocket@4
    _fclose
    _fdiv_main_routine
    _fflush
    _fgetc
    _fopen
    _fprintf
    _fputc
    _fputs
    _fread
    _fseek
    _ftell
    _fwrite
    _gethostbyname@4
    _gethostname@8
    _htonl@4
    _htons@4
    _hypot
    _inet_addr@4
    _inet_ntoa@4
    _ioctlsocket@12
    _ldexp
    _log10
    _lseek
    _lstrlenA@46
    _malloc
    _mciGetErrorStringA@12
    _mciSendStringA@16
    _memcmp
    _memcpy
    _memmove
    _memset
    _nb_sess
    _perror
    _raise
    _realloc
    _recvfrom@24
    _remove
    _sendto@24
    _setvbuf
    _signal"
    _socket@12
    _sprintf
    _srand
    _strcat
    _strcmp
    _strcmpi
    _strcpy
    _strerror
    _stricmp
    _strlen
    _strncat
    _strncmp
    _strncpy
    _strnicmp
    _strrev2
    _tolower
    _toupper
    _ungetc
    _waveOutGetDevCapsA@12
    _waveOutGetNumDevs@0
    _wcslen"
    _wctomb
    _write
    _wsprintfA
    `.rdata
    a horde of
    a legion of
    a pack of
    a swarm of
    a telescope.
    a throng of
    abnormal program termination
    advManager
    adv_wind.bin
    advbord.icn
    advborde.icn
    advbtns.icn
    advebtns.icn
    advmice.mse
    aelem.icn
    air elemental
    air elementals
    antimagk
    apanel.bin
    arch_msl.icn
    archer
    archer.icn
    archer2.icn
    archers
    archmage
    archmagi
    archrfrm.bin
    arithCodeSymbol: too many bits outstanding
    armgedn
    armywin.bin
    arrax.mx2
    art32.icn
    artfx.icn
    artifact.icn
    audiodrv.dig
    auxSetVolume
    b-bflg32.icn
    b-flag32.icn
    badluck.82m
    bankbox.bin
    barb32.icn
    battle dwarf
    battle dwarves
    berzerk
    berzerk.icn
    bigfont.fnt
    black dragon
    black dragons
    bless.icn
    blind.icn
    bloodlus
    bluefire.icn
    boar.icn
    boar_frm.bin
    boat32.icn
    boatshad.icn
    bone dragon
    bone dragons
    boulder.icn
    brcrest.icn
    brokena.mp2
    brothers.icn
    bsClose
    bsOpenReadStream
    bsOpenWriteStream
    bucket sorting ...
    build\intel\mt_obj\_filbuf.obj
    build\intel\mt_obj\_file.obj
    build\intel\mt_obj\_flsbuf.obj
    build\intel\mt_obj\_fptostr.obj
    build\intel\mt_obj\_freebuf.obj
    build\intel\mt_obj\_getbuf.obj
    build\intel\mt_obj\_newmode.obj
    build\intel\mt_obj\_open.obj
    build\intel\mt_obj\_sftbuf.obj
    build\intel\mt_obj\abort.obj
    build\intel\mt_obj\abs.obj
    build\intel\mt_obj\access.obj
    build\intel\mt_obj\atox.obj
    build\intel\mt_obj\aw_env.obj
    build\intel\mt_obj\aw_map.obj
    build\intel\mt_obj\aw_str.obj
    build\intel\mt_obj\calloc.obj
    build\intel\mt_obj\chdir.obj
    build\intel\mt_obj\chkstk.obj
    build\intel\mt_obj\chsize.obj
    build\intel\mt_obj\close.obj
    build\intel\mt_obj\closeall.obj
    build\intel\mt_obj\cmiscdat.obj
    build\intel\mt_obj\commit.obj
    build\intel\mt_obj\crt0dat.obj
    build\intel\mt_obj\crt0fp.obj
    build\intel\mt_obj\crt0init.obj
    build\intel\mt_obj\crt0msg.obj
    build\intel\mt_obj\crtmbox.obj
    build\intel\mt_obj\ctype.obj
    build\intel\mt_obj\delete.obj
    build\intel\mt_obj\dosmap.obj
    build\intel\mt_obj\exsup.obj
    build\intel\mt_obj\exsup3.obj
    build\intel\mt_obj\fclose.obj
    build\intel\mt_obj\fflush.obj
    build\intel\mt_obj\fgetc.obj
    build\intel\mt_obj\fopen.obj
    build\intel\mt_obj\fp8.obj
    build\intel\mt_obj\fprintf.obj
    build\intel\mt_obj\fputc.obj
    build\intel\mt_obj\fputs.obj
    build\intel\mt_obj\fread.obj
    build\intel\mt_obj\free.obj
    build\intel\mt_obj\fseek.obj
    build\intel\mt_obj\ftell.obj
    build\intel\mt_obj\fwrite.obj
    build\intel\mt_obj\handler.obj
    build\intel\mt_obj\heapinit.obj
    build\intel\mt_obj\ioinit.obj
    build\intel\mt_obj\isatty.obj
    build\intel\mt_obj\isctype.obj
    build\intel\mt_obj\ismbbyte.obj
    build\intel\mt_obj\lseek.obj
    build\intel\mt_obj\malloc.obj
    build\intel\mt_obj\mbctype.obj
    build\intel\mt_obj\mbtoupr.obj
    build\intel\mt_obj\memcmp.obj
    build\intel\mt_obj\memcpy.obj
    build\intel\mt_obj\memmove.obj
    build\intel\mt_obj\memset.obj
    build\intel\mt_obj\mlock.obj
    build\intel\mt_obj\msize.obj
    build\intel\mt_obj\ncommode.obj
    build\intel\mt_obj\new.obj
    build\intel\mt_obj\nlsdata1.obj
    build\intel\mt_obj\nlsdata2.obj
    build\intel\mt_obj\onexit.obj
    build\intel\mt_obj\open.obj
    build\intel\mt_obj\osfinfo.obj
    build\intel\mt_obj\output.obj
    build\intel\mt_obj\perror.obj
    build\intel\mt_obj\purevirt.obj
    build\intel\mt_obj\rand.obj
    build\intel\mt_obj\read.obj
    build\intel\mt_obj\realloc.obj
    build\intel\mt_obj\setmode.obj
    build\intel\mt_obj\setvbuf.obj
    build\intel\mt_obj\sprintf.obj
    build\intel\mt_obj\stdargv.obj
    build\intel\mt_obj\stdenvp.obj
    build\intel\mt_obj\strcat.obj
    build\intel\mt_obj\strcmp.obj
    build\intel\mt_obj\stream.obj
    build\intel\mt_obj\strerror.obj
    build\intel\mt_obj\stricmp.obj
    build\intel\mt_obj\strlen.obj
    build\intel\mt_obj\strncat.obj
    build\intel\mt_obj\strncmp.obj
    build\intel\mt_obj\strncpy.obj
    build\intel\mt_obj\strnicmp.obj
    build\intel\mt_obj\strrev.obj
    build\intel\mt_obj\strupr.obj
    build\intel\mt_obj\syserr.obj
    build\intel\mt_obj\tell.obj
    build\intel\mt_obj\tidtable.obj
    build\intel\mt_obj\tolower.obj
    build\intel\mt_obj\toupper.obj
    build\intel\mt_obj\txtmode.obj
    build\intel\mt_obj\ulldiv.obj
    build\intel\mt_obj\ullrem.obj
    build\intel\mt_obj\ungetc.obj
    build\intel\mt_obj\unlink.obj
    build\intel\mt_obj\wcslen.obj
    build\intel\mt_obj\wctomb.obj
    build\intel\mt_obj\wincrt0.obj
    build\intel\mt_obj\winsig.obj
    build\intel\mt_obj\winxfltr.obj
    build\intel\mt_obj\write.obj
    build\intel\mt_obj\xtoa.obj
    buildtwn.82M
    buried in barren lava.
    buried on a beach.
    buybuild.icn
    buybuile.icn
    campaign.bin
    campbkge.ICN
    campbkgg.ICN
    campxtre.ICN
    campxtrg.ICN
    campxtrg.icn
    caslwind.bin
    caslxtra.icn
    castle
    catapult.icn
    cavalries
    cavalry
    cavalryb.icn
    cavalryr.icn
    centaur
    centaur.icn
    centaurs
    central
    centrfrm.bin
    chainlte
    champion
    champions
    clof32.til
    clop32.icn
    close CD
    closesocket
    cloudluk.icn
    cmbtfle1.icn
    cmbtfle2.icn
    cmbtfle3.icn
    cmbtlos1.icn
    cmbtlos2.icn
    cmbtlos3.icn
    cmbtmisc.icn
    cmbtmous.mse
    cmbtsurr.icn
    cmbtwin.bin
    coldray
    coldray.icn
    coldring
    coldring.icn
    combatManager
    cpanel.bin
    creature
    creatures
    crest.icn
    crusader
    crusaders
    cspanel.bin
    curse.icn
    cvlr2frm.bin
    cvlryfrm.bin
    cyclofrm.bin
    cyclopes
    cyclops
    cyclops.icn
    damage
    destroy
    destroys
    dimdoor.bin
    dipmagk
    disrray.icn
    disruptr
    doReversibleTransformation
    drabnfrm.bin
    dragbfrm.bin
    dragblak.icn
    dragbone.icn
    draggfrm.bin
    draggree.icn
    dragred.icn
    dragrfrm.bin
    dragslay.icn
    drawbrg.82m
    drgnslay
    druid.icn
    druid2.icn
    druidfrm.bin
    druidmsl.icn
    druids
    dummy.icn
    dwarf.icn
    dwarf2.icn
    dwarffrm.bin
    dwarves
    earth elemental
    earth elementals
    eastern
    eelem.icn
    electric.icn
    elf.icn
    elf2.icn
    elf__frm.bin
    elf__msl.icn
    erthquak
    expernce.82m
    expmrl.icn
    extraovr.icn
    failure, see the Visual C++ documentation on asserts
    felem.icn
    felemfrm.bin
    ffffff
    fileRequester
    fire elemental
    fire elementals
    firebal2.icn
    fireball
    fireball.icn
    flag32.icn
    froth.icn
    fullhse.mx2
    g-bflg32.icn
    g-flag32.icn
    garglfrm.bin
    gargoyle
    gargoyle.icn
    gargoyles
    genie.icn
    geniefrm.bin
    genies
    getAndMoveToFrontDecode
    getRLEpair: ungetc failed
    gethostbyname
    gethostname
    ghost.icn
    ghostfrm.bin
    ghosts
    giants
    goblin
    goblin.icn
    goblins
    goblnfrm.bin
    golem.icn
    golem2.icn
    golemfrm.bin
    goodluck.82m
    goodmrle.82M
    goodmrle.82m
    gradedSort
    grand elf
    grand elves
    greater druid
    greater druids
    green dragon
    green dragons
    grifffrm.bin
    griffin
    griffin.icn
    griffins
    ground32.til
    h2mine.82m
    hInstApp
    halflfrm.bin
    halfling
    halfling.icn
    halflings
    halflmsl.icn
    haste.icn
    heroWindowManager
    herobkg.icn
    heroes.icn
    heroes2.agg
    heroes2x.agg
    heroexte.icn
    heroextg.icn
    herologe.icn
    herologo.icn
    herowind.bin
    highScoreManager
    hiscore.bin
    hourglas.icn
    hsbkg.icn
    hydra.icn
    hydrafrm.bin
    hydras
    hypnotiz
    hypnotiz.icn
    icecloud.icn
    in a murky swamp.
    in a parched wasteland.
    in the dirt.
    in the driven snow.
    in the murky depths of the ocean.
    inet_addr
    inet_ntoa
    info CD UPC wait
    inputManager
    ioctlsocket
    iron golem
    iron golems
    is corrupted.
    iwav.dig
    j@jKj j
    j@jPj j
    jOh(~R
    jPjPh6
    jammer.dig
    kb.pal
    kbswzn
    keep.icn
    keepshot.82M
    killfade.82M
    killfade.82m
    kngt32.icn
    l$$URQP
    lich.icn
    lich2.icn
    lich_frm.bin
    lich_msl.icn
    lichclod.icn
    liches
    lightblt
    llmmmnnnn
    lmm'())**++-./
    locatore.icn
    locators.icn
    lots of
    lstrlenA
    magcarow
    mage1.icn
    mage1frm.bin
    mage2.icn
    magewind.bin
    magic01.icn
    magic02.icn
    magic03.icn
    magic04.icn
    magic05.icn
    magic06.icn
    magic07.icn
    magic08.icn
    mana.icn
    massbles
    masscure
    masscurs
    masshast
    massslow
    master swordsman
    master swordsmen
    mciGetErrorStringA
    mciSendStringA
    medusa
    medusa.icn
    medusas
    medusfrm.bin
    meteor.icn
    meteor~1
    minicapt.icn
    minihero.icn
    minilkmr.icn
    minimon.icn
    miniport.icn
    miniss.icn
    minitown.icn
    minotau2.icn
    minotaur
    minotaur king
    minotaur kings
    minotaur.icn
    minotaurs
    minotfrm.bin
    mirrorim
    mmm(()*++,-./1
    mnrdeath
    mnuAdv
    mnuAdvD
    mnuCmbt
    mnuDflt
    mnuTown
    mnuTownD
    moatpart.icn
    moatwhol.icn
    mobility.icn
    mons32.icn
    moraleb.icn
    moraleg.icn
    mouseManager
    mss32.dll
    mss32_NULL_THUNK_DATA
    mtncrck.icn
    mtndirt.icn
    mtndsrt.icn
    mtngras.icn
    mtnlava.icn
    mtnmult.icn
    mtnsnow.icn
    mtnswmp.icn
    mummies
    mummy2.icn
    mummyfrm.bin
    mummyw.icn
    mutant zombie
    mutant zombies
    necr32.icn
    netbox.bin
    newmonth.82m
    newweek.82m
    ngextra.icn
    nghs.bin
    ngmp.bin
    ngsp.bin
    nn(*nnVVVV
    nomad.icn
    nomadfrm.bin
    nomads
    nopqqqtuvwxy/0223344
    northeastern
    northern
    northwestern
    nvdig.dig
    nwherolv.82m
    o-bflg32.icn
    o-flag32.icn
    objnarti.icn
    objncrck.icn
    objndirt.icn
    objndsrt.icn
    objngra2.icn
    objngras.icn
    objnhaun.icn
    objnlav2.icn
    objnlav3.icn
    objnlava.icn
    objnmul2.icn
    objnmult.icn
    objnrsrc.icn
    objnsnow.icn
    objnswmp.icn
    objntown.icn
    objntwba.icn
    objntwrd.icn
    objntwsh.icn
    objnwat2.icn
    objnwatr.icn
    objnxtra.icn
    ogre lord
    ogre lords
    ogre.icn
    ogre2.icn
    ogre_frm.bin
    on a grassy plain.
    oo*.oWXXX
    orange
    orc chief
    orc chiefs
    orc.icn
    orc2.icn
    orc__frm.bin
    orc__msl.icn
    origdata.bin
    overback.icn
    overview.icn
    overwind.bin
    p-bflg32.icn
    p-flag32.icn
    p??_C@_1O@POHA@?$AA?$CI?$AAn?$AAu?$AAl?$AAl?$AA?$CJ?$AA?$AA?$AA?$AA?$AA?$AD?$AA?$AA?$AA?$AA?$AA?$AA?$;E?$AA?$ADm@
    paladfrm.bin
    paladin
    paladin.icn
    paladin2.icn
    paladins
    paralize
    paralyze.icn
    peas_frm.bin
    peasant
    peasant.icn
    peasants
    perhaps it is corrupted?  *Possible* reason follows.
    perish
    perishes
    phoenfrm.bin
    phoenix
    phoenix.icn
    physical.icn
    pickup01.82m
    pikeman
    pikeman.icn
    pikeman2.icn
    pikemen
    pikmnfrm.bin
    playing
    portcflg.icn
    portxtra.icn
    power lich
    power liches
    ppxxxx
    primskil.icn
    proaudio.dig
    purple
    puzzle.icn
    qhero0.bin
    qhero1.bin
    qtown1.bin
    qwikinfo.bin
    r-bflg32.icn
    r-flag32.icn
    radar.icn
    rainbluk.icn
    ranger
    rangers
    rap10.dig
    rcrthero.bin
    recruiq0.bin
    recruiq1.bin
    recruit0.bin
    recruit1.bin
    recruitManager
    recvfrom
    red dragon
    red dragons
    reddeath.icn
    redfire.icn
    report it to me at: sewardj@cs.man.ac.uk
    request.bin
    requests.bin
    resist
    resists
    resource.icn
    resourceManager
    ressmall.icn
    resurect
    resurtru
    road.icn
    roc.icn
    roc__frm.bin
    rogue.icn
    roguefrm.bin
    rogues
    route.icn
    royal mummies
    royal mummy
    rsbryfzl.82m
    rsstuuvwwxyyz{{|}}~
    runtime error 
    sb16.dig
    sblaster.dig
    sbpro.dig
    sceninfo.bin
    scroll.icn
    scrollcn.icn
    scrolle.icn
    secskill.icn
    sendMTFVal: bad value!
    sendto
    set CD time format tmsf
    several
    sewardj@cs.man.ac.uk.
    shadow32.icn
    shield
    shield.icn
    shipwind.bin
    shnganim.icn
    skel_frm.bin
    skeleton
    skeleton.icn
    skeletons
    slugfest.mp2
    smackw32.DLL
    smackw32_NULL_THUNK_DATA
    smalclod.icn
    smalfont.fnt
    sndscape.dig
    sndsys.dig
    socket
    sorc32.icn
    sound disabled and the intro skipped.
    soundManager
    southeastern
    southern
    southwestern
    spanel.bin
    sparks.icn
    spellinf.icn
    spellinl.icn
    spells.icn
    spellwin.bin
    spelmous.mse
    splitwin.bin
    spotBlock
    sprite
    sprite.icn
    sprites
    spritfrm.bin
    status CD mode
    status CD position
    steel golem
    steel golems
    stelskin
    stelskin.icn
    ston.til
    stonback.icn
    stonbake.icn
    stonebk2.icn
    stonskin
    stonskin.icn
    stop CD
    stop CD wait
    stopped
    storm.icn
    stpbaud.bin
    stpcom.bin
    stpdc.bin
    stpdccfg.bin
    stphotst.bin
    stpmain.bin
    stpmcfg.bin
    stpmodem.bin
    stpmp.bin
    stpnet.bin
    stpnet2.bin
    stpnewgm.bin
    stream.icn
    strip.icn
    sumnelm
    sunmoon.icn
    sunmoone.icn
    surrendr.bin
    surrendr.icn
    swapManager
    swapbtn.icn
    swapwin.bin
    swordsm2.icn
    swordsman
    swordsmen
    swordsmn.icn
    swrdsfrm.bin
    system.icn
    systeme.icn
    t$dPQV
    tVhH;S
    tXhH;S
    tavwin.bin
    telptin.82m
    telptout
    textbar.icn
    the enemy - 
    the enemy alliance of 
    thiefwin.bin
    tita2frm.bin
    titanbla.icn
    titanblu.icn
    titanfrm.bin
    titanmsl.icn
    titans
    to all living creatures.
    to all undead creatures.
    townManager
    townport.bin
    townwind.bin
    townwind.icn
    tradpost.bin
    tradpost.icn
    treasure.82m
    tredeci.icn
    treevil.icn
    trefall.icn
    trefir.icn
    trejngl.icn
    tresnow.icn
    trivialSort ...
    trivialSort done.
    troll.icn
    troll2.icn
    trollfrm.bin
    trollmsl.icn
    trolls
    ultra.dig
    under a desert drift.
    unicofrm.bin
    unicorn
    unicorn.icn
    unicorns
    user32.dll
    vampifrm.bin
    vampire
    vampire lord
    vampire lords
    vampire.icn
    vampire2.icn
    vampires
    veteran pikeman
    veteran pikemen
    vgenwin.bin
    viewarsm.icn
    viewlgnd.bin
    viewpuzl.bin
    wTT3?wx
    war troll
    water elemental
    water elementals
    waveOutGetDevCapsA
    waveOutGetNumDevs
    welem.icn
    wellwind.bin
    western
    which probably indicates a bug in BZIP.  Please
    which probably indicates that the compressed data
    which probably means it is corrupted.
    which probably means the compressed file
    wincmbt.bin
    wing32.def
    winloseb.icn
    wolf.icn
    wolf_frm.bin
    wolves
    wrlk32.icn
    wsprintfA
    wzrd32.icn
    x_camp.bin
    x_cmpext.icn
    x_ivy.icn
    x_loadcm.bin
    x_loc1.icn
    x_loc2.icn
    x_loc3.icn
    x_mapmnu.bin
    xprimary.icn
    xq|wpO
    y-bflg32.icn
    y-flag32.icn
    yellow
    yinyang.icn
    zomb_frm.bin
    zombie
    zombie.icn
    zombie2.icn
    zombies
    zounds of
    {19200 Baud}
    {2 Players}
    {2400 Baud}
    {3 Players}
    {38400 Baud}
    {4 Players}
    {5 Players}
    {6 Players}
    {9600 Baud}
    {Advanced Archery}
    {Advanced Ballistics}
    {Advanced Diplomacy}
    {Advanced Eagle Eye}
    {Advanced Leadership}
    {Advanced Logistics}
    {Advanced Luck}
    {Advanced Mysticism}
    {Advanced Navigation}
    {Advanced Necromancy}
    {Advanced Pathfinding}
    {Advanced Scouting}
    {Advanced Wisdom}
    {Adventure Options}
    {Alchemist}
    {All Maps}
    {Ammo cart}
    {Animate Dead}
    {Ankh}
    {Anti-Magic}
    {Arcane Necklace
    {Archer's House}
    {Arm of the Martyr}
    {Armageddon}
    {Armored Gauntlets
    {Artesian Spring}
    {Artifact}
    {Astrolabe}
    {Attack Skill}
    {Auto Combat
    {Auto Combat}
    {Bad Luck}
    {Bad Morale}
    {Ballista}
    {Basic Archery}
    {Basic Ballistics}
    {Basic Diplomacy}
    {Basic Eagle Eye}
    {Basic Leadership}
    {Basic Logistics}
    {Basic Luck}
    {Basic Mysticism}
    {Basic Navigation}
    {Basic Necromancy}
    {Basic Pathfinding}
    {Basic Scouting}
    {Basic Wisdom}
    {Battle Garb}
    {Berzerker}
    {Black pearl}
    {Bless}
    {Blind }
    {Blood Lust}
    {Book of Elements}
    {Breastplate of Anduran}
    {Broach of Shielding}
    {Buoy}
    {COM 1}
    {COM 2}
    {COM 3}
    {COM 4}
    {Campaign Game}
    {Campfire}
    {Cancel}
    {Cast Spell}
    {Caster's Bracelet
    {Cave}
    {Chain Lightning}
    {Chest}
    {City of the Dead}
    {Class}
    {Clover}
    {Coins} - Accumulate a large amount of gold.
    {Coin}
    {Cold Ray}
    {Cold Ring}
    {Com 4}
    {Compass}
    {Config}
    {Continue Movement}
    {Credits}
    {Crystal Ball}
    {Crystal Mine}
    {Cure}
    {Curse}
    {Daemon Cave}
    {Death Ripple}
    {Death Wave}
    {Defender Helm
    {Defense Skill}
    {Derelict Ship}
    {Desert Tent}
    {Difficulty Rating}
    {Dimension Door}
    {Direct Connect}
    {Dispel Magic}
    {Disrupting Ray}
    {Divine Breastplate
    {Dragon City}
    {Dragon Slayer}
    {Dragon Sword
    {Dwarf Cottage}
    {ERROR}
    {Earthquake}
    {Effects}
    {Elemental Storm}
    {Elemental ring}
    {Enchanted hourglass}
    {End Turn}
    {Endless Bag}
    {Endless Purse}
    {Endless Sack}
    {Endless cart of ore}
    {Endless cord of wood}
    {Endless pouch of crystal}
    {Endless pouch of gems}
    {Endless pouch of sulfur}
    {Endless vial of mercury}
    {Enemy Speed}
    {Enter Name}
    {Evercold icicle}
    {Everhot lava rock}
    {Evil Eye}
    {Excavation}
    {Expansion Campaign}
    {Expansion Map}
    {Experience}
    {Expert Archery}
    {Expert Ballistics}
    {Expert Diplomacy}
    {Expert Eagle Eye}
    {Expert Leadership}
    {Expert Logistics}
    {Expert Luck}
    {Expert Mysticism}
    {Expert Navigation}
    {Expert Necromancy}
    {Expert Pathfinding}
    {Expert Scouting}
    {Expert Wisdom}
    {Extra Large Maps}
    {Faerie Ring}
    {File Options}
    {Fire cloak}
    {Fireball}
    {Fireblast}
    {Fizbin}
    {Flotsam}
    {Followers}
    {Foremost Scroll
    {Fort}
    {Fountain}
    {Freeman's Foundry}
    {Game Difficulty}
    {Gazebo}
    {Gem Mine}
    {Genie Lamp}
    {Giant Flail
    {Goblin Hut}
    {Gold Mine}
    {Gold watch}
    {Golden Goose}
    {Golden bow}
    {Good Luck}
    {Good Morale}
    {Grand Estates}
    {Graveyard}
    {Grid}
    {Grouped Formation}
    {Guest}
    {Halfling Hole}
    {Handicap}
    {Handshake} - Your side defeats the opposing side.
    {Haste}
    {Haunt}
    {Heart of Fire}
    {Heart of Ice}
    {Helmet of Anduran}
    {Hero Face} - Defeat a specific hero.
    {Hero Face} - Lose a specific hero.
    {Hideous mask}
    {High Scores}
    {Hill Fort}
    {Holy Hammer}
    {Holy Pendant}
    {Holy Shout}
    {Holy Word}
    {Horseshoe}
    {Host}
    {Hot Seat}
    {Hourglass} - Run out of time. (Fail to win by a certain point.)
    {Hypnotize}
    {Ice cloak}
    {Identify Hero}
    {Idol}
    {Interface}
    {Kinetic pendant}
    {Kingdom Summary}
    {Knowledge}
    {Large Maps}
    {Lean To}
    {Legendary Scepter}
    {Lighthouse}
    {Lightning Bolt}
    {Lightning helm}
    {Lightning rod }
    {Load Game}
    {Loss Condition Icon}
    {Mage's Ring
    {Magellan's Maps}
    {Magic Arrow}
    {Magic Book}
    {Magic Garden}
    {Magic Well}
    {Major Scroll
    {Mass Bless}
    {Mass Cure}
    {Mass Curse}
    {Mass Dispel}
    {Mass Haste}
    {Mass Shield}
    {Mass Slow}
    {Masthead}
    {Medal}
    {Medal} - Find a specific artifact.
    {Medium Maps}
    {Mercenary Camp}
    {Message Bar}
    {Meteor Shower}
    {Minor Scroll
    {Mirror Image}
    {Modem}
    {Moderate Estates}
    {Monster Info}
    {Mouse Cursor}
    {Multi-Player Game}
    {Music Type}
    {Music}
    {NETBios}
    {Network}
    {Neutral Luck}
    {Neutral Morale}
    {New Game}
    {Next Hero}
    {Nomad Boots}
    {Oasis}
    {Obelisk}
    {Observation Tower}
    {Opponents}
    {Oracle}
    {Ore Mine}
    {Original Campaign}
    {Original Map}
    {Paralyze}
    {Peasant Hut}
    {Pendant of death}
    {Pendant of free will}
    {Pendant of life}
    {Players Icon}
    {Power Axe
    {Power ring}
    {Quit}
    {Rabbit's Foot}
    {Resources}
    {Resurrect True}
    {Resurrect}
    {Retreat}
    {Ruins}
    {Sawmill}
    {Scenario}
    {Seeing-eye pendant}
    {Selected Description}
    {Selected Map Difficulty}
    {Selected Name}
    {Serenity pendant}
    {Set Air Guardian}
    {Set Earth Guardian}
    {Set Fire Guardian}
    {Set Water Guardian}
    {Shadow Cursor}
    {Shadow Movement}
    {Shield}
    {Shipwreck Survivor}
    {Shipwreck}
    {Show Path}
    {Shrine of the 1st Circle}
    {Shrine of the 2nd Circle}
    {Shrine of the 3rd Circle}
    {Signpost}
    {Size Icon}
    {Skeleton}
    {Skip}
    {Skullcap}
    {Slow}
    {Small Estates}
    {Small Maps}
    {Snake-ring}
    {Snow Cave}
    {Spade of Necromancy}
    {Speed}
    {Spell Points}
    {Spell Power}
    {Spell Scroll}
    {Sphere of Negation}
    {Sphinx}
    {Spiked helm}
    {Spiked shield}
    {Spread Formation}
    {Staff of Wizardry}
    {Standard Game}
    {Standing Stones}
    {Statesman's quill}
    {Status Window}
    {Stealth Shield
    {Steelskin}
    {Stoneskin}
    {Sulfur Mine}
    {Summon Air Elemental}
    {Summon Boat}
    {Summon Earth Elemental}
    {Summon Fire Elemental}
    {Summon Water Elemental}
    {Superior Scroll
    {Surrender}
    {Sword Breaker}
    {Sword of Anduran}
    {System Options}
    {TCP/IP}
    {Tax lien}
    {Teleport}
    {Telescope}
    {Temple}
    {Thatched Hut}
    {Thunder Mace
    {Tombstone} - Defeat all enemy heroes and towns.
    {Tombstone} - Lose all your heroes and towns.
    {Town Gate}
    {Town Portal}
    {Town} - Capture a specific town.
    {Town} - Lose a specific town.
    {Traveler's Boots}
    {Tree City}
    {Tree House}
    {Tree of Knowledge}
    {Troll Bridge}
    {Ultimate Book
    {Ultimate Cloak
    {Ultimate Crown}
    {Ultimate Shield}
    {Ultimate Staff}
    {Ultimate Sword
    {Ultimate Wand
    {Victory Condition Icon}
    {Video}
    {View All}
    {View Artifacts}
    {View Heroes}
    {View Mines}
    {View Resources}
    {View Towns}
    {Visions}
    {Wagon}
    {Wait}
    {Wand of negation}
    {Watch Tower}
    {Water Wheel}
    {Watering Hole}
    {Whirlpool}
    {White pearl}
    {Windmill}
    {Witch Doctor's Hut}
    {Witch's Hut}
    {Witches Broach
    {Wizard's hat}
    {World Map}
    {Xanadu}
