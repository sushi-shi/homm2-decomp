...
static H2EnumStorage<NetbiosSessionStatus, u8> gNetStatus[NETBIOS_STATUS_COUNT] = {0};
...
extern "C" void __fastcall nb_term(void) {
    ...
    if ((H2EnumIndex((gNetStatus[gNbMaxSess]) & (NETBIOS_SESSION_NAME_REGISTERED)))) {
        memset(&localNcb, 0, sizeof(localNcb));
...
extern "C" u16 __fastcall nb_snd(i16 session, i16 len, void* data) {
    ...
    if (!(H2EnumIndex((gNetStatus[session]) & (NETBIOS_SESSION_ACTIVE))))
        return H2EnumIndex(NETBIOS_RESULT_SESSION_OUT_OF_RANGE);
...
nb_sess(NetbiosSessionOperation operation, ...) {
    ...
    char* callName;
    H2EnumStorage<NetbiosResult, i16> returnCode;
    va_list argList;
...
void nb_thr_ctl(void) {
    ...
    tag_Node* node;
    NetbiosControlBlock localNcb;
    H2EnumStorage<NetbiosResult, u8> rc;
    i32 sendComplete;
...
static void __fastcall nb_arm_recv(i32 session) {
    ...
    H2EnumStorage<NetbiosResult, u8> result;
    for (;;) {
        ProcessAssert(
