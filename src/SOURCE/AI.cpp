...
i32 combatManager::AICheckRetreat(void) {
    ...
    if (force[(m_currentSide)] > COMBAT_AI_STRENGTH_40000)
        chance -= static_cast<float>(force[(m_currentSide)] / COMBAT_AI_STRENGTH_20000);
    else if (force[(m_currentSide)] > COMBAT_AI_STRENGTH_30000)
        chance =
            static_cast<float>(chance - COMBAT_AI_STRENGTH_30000_RETREAT_PENALTY);
    else if (force[(m_currentSide)] > COMBAT_AI_STRENGTH_15000)
        chance =
            static_cast<float>(chance - COMBAT_AI_STRENGTH_15000_RETREAT_PENALTY);
    else if (force[(m_currentSide)] > COMBAT_AI_STRENGTH_5000)
        chance =
            static_cast<float>(chance - COMBAT_AI_STRENGTH_5000_RETREAT_PENALTY);
    else if (force[(m_currentSide)] > COMBAT_AI_STRENGTH_2500)
        chance =
...
void combatManager::DoCompAI(CombatSide) {
    ...
    if ((((thisArmy->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_SHOOTER))) != 0) {
        if (thisArmy->m_monster.shots > 0)
            plan = COMBAT_AI_ATTACK_SHOOT;
        else
            plan = COMBAT_AI_ATTACK_WALK;
    } else if ((((thisArmy->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_FLYING))) != 0) {
...
finish:
    if (giNextAction == ACTION_MOVE && giNextActionGridIndex > 0
        ...
        for (dirIndex = COMBAT_DIRECTION_NORTHEAST;
             (dirIndex) < COMBAT_AI_ADJACENT_DIRECTION_COUNT;
             dirIndex++) {
...
void combatManager::DoLichShot(class army* lich) {
    ...
    for (armyIndex = 0; armyIndex < m_armyCount[(OppositeCombatSide(m_currentSide))];
        ...
        if (targetArmy == NULL
            || (((targetArmy->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED))) != 0
        ...
        *(marked + (targetArmy->m_side) * COMBAT_AI_ARMY_SLOT_COUNT + targetArmy->m_index) =
        ...
        for (iDir = COMBAT_DIRECTION_NORTHEAST;
            ...
            if (sideHex >= 0 && sideHex < COMBAT_HEX_COUNT
                ...
                && *(marked + (m_hexCells[sideHex].m_occupantSide) * COMBAT_AI_ARMY_SLOT_COUNT
    ...
    for (armyIndex = 0; armyIndex < m_armyCount[(side)]; armyIndex++) {
        ...
        if (currentArmy != NULL
            && (((currentArmy->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                   == 0
            && (((currentArmy->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_SHOOTER))) != 0
            ...
            && currentArmy->m_spellInfluence[(ARMY_SPELL_INFLUENCE_BERSERK)] == 0
            && currentArmy->m_spellInfluence[(ARMY_SPELL_INFLUENCE_HYPNOTIZE)] == 0)
            bits |= armyBit;
        armyBit <<= 1;
    }
    return bits;
...
i32 combatManager::GetMirrorImageMask(CombatSide side) {
    ...
    for (armyIndex = 0; armyIndex < m_armyCount[(side)]; armyIndex++) {
        ...
        if (currentArmy != NULL
            && (((currentArmy->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                   == 0
            && (((currentArmy->m_monster.flags.all) & (MONSTER_FLAGS_MIRROR_IMAGE))) != 0)
            bits |= armyBit;
        armyBit <<= 1;
    }
    return bits;
...
i32 combatManager::GetFlyerMask(CombatSide side) {
    ...
    for (armyIndex = 0; armyIndex < m_armyCount[(side)]; armyIndex++) {
        ...
        if (currentArmy != NULL
            && (((currentArmy->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                   == 0
            && (((currentArmy->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_FLYING))) != 0
            && currentArmy->m_spellInfluence[(ARMY_SPELL_INFLUENCE_BLIND)] == 0
            && currentArmy->m_spellInfluence[(ARMY_SPELL_INFLUENCE_PARALYZE)] == 0
            && currentArmy->m_spellInfluence[(ARMY_SPELL_INFLUENCE_PETRIFIED)] == 0
            && currentArmy->m_spellInfluence[(ARMY_SPELL_INFLUENCE_BERSERK)] == 0
...
i32 combatManager::AttemptAttack(
    ...
    while (mask != 0) {
        ...
        if ((((targetArmy[m_armies[(side)]].m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_WIDE)))
            != 0) {
            if (targetArmy[m_armies[(side)]].m_facing == ARMY_FACING_LEFT)
                targetHex--;
            else
                targetHex++;
            currentArmy->m_moveTargetHex = targetHex;
...
i32 combatManager::AttemptAdjacentAttack(class army* currentArmy) {
    ...
    if (enemyArmy != COMBAT_AI_NO_ARMY) {
        giNextAction = ACTION_MOVE;
        giNextActionGridIndex = m_armies[(OppositeCombatSide(m_currentSide))][enemyArmy].m_hex;
        return 1;
    } else {
        return 0;
    }
...
i32 combatManager::WalkTowardArmyFront(
    ...
    frontHex = armyIndex[m_armies[(side)]].m_hex;
    if ((((armyIndex[m_armies[(side)]].m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_WIDE)))
        != 0)
