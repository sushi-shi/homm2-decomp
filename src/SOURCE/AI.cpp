...
i32 combatManager::AICheckRetreat(void) {
    ...
    if (gpGame->m_mapHeader.victoryCondition == MAP_VICTORY_DEFEAT_HERO
        && m_heroes[H2EnumIndex(m_currentSide)]->m_id
    ...
    for (side9 = 0; side9 < COMBAT_SIDE_COUNT; side9++) {
        ...
        for (armyIndex36 = 0; armyIndex36 < COMBAT_AI_ARMY_SLOT_COUNT; armyIndex36++) {
            if ((m_armies[0] + side9 * COMBAT_ARMY_STORAGE_SLOT_COUNT + armyIndex36)->IsAlive()) {
                ...
                if ((H2EnumIndex(((m_armies[0] + side9 * COMBAT_ARMY_STORAGE_SLOT_COUNT + armyIndex36)
        ...
        if (heroPtr9 != NULL) {
            for (armyIndex36 = 0; armyIndex36 < AI_BATTLE_ARTIFACT_SLOT_COUNT; armyIndex36++) {
                if (heroPtr9->m_artifacts[armyIndex36] != ARTIFACT_NONE
                    && H2EnumIndex(heroPtr9->m_artifacts[armyIndex36])
                           < AI_BATTLE_BASE_ARTIFACT_LIMIT) {
                    artifactValues[side9] +=
                        gArtifactBaseRV[H2EnumIndex(heroPtr9->m_artifacts[armyIndex36])];
                }
            }
        }
    }

    strengths1[H2EnumIndex(OppositeCombatSide(m_currentSide))] = static_cast<i32>(
        strengths1[H2EnumIndex(OppositeCombatSide(m_currentSide))] * COMBAT_AI_TOWN_STRENGTH_MODIFIER
    );
    artifactValue15 = artifactValues[H2EnumIndex(m_currentSide)];
    if (artifactValues[H2EnumIndex(m_currentSide)] < COMBAT_AI_MIN_ARTIFACT_VALUE
    ...
    if (strengths1[H2EnumIndex(m_currentSide)] > COMBAT_AI_STRENGTH_40000)
        retreatChance7 -= static_cast<float>(strengths1[H2EnumIndex(m_currentSide)] / COMBAT_AI_STRENGTH_20000);
    else if (strengths1[H2EnumIndex(m_currentSide)] > COMBAT_AI_STRENGTH_30000)
        retreatChance7 =
            static_cast<float>(retreatChance7 - COMBAT_AI_STRENGTH_30000_RETREAT_PENALTY);
    else if (strengths1[H2EnumIndex(m_currentSide)] > COMBAT_AI_STRENGTH_15000)
        retreatChance7 =
            static_cast<float>(retreatChance7 - COMBAT_AI_STRENGTH_15000_RETREAT_PENALTY);
    else if (strengths1[H2EnumIndex(m_currentSide)] > COMBAT_AI_STRENGTH_5000)
        retreatChance7 =
            static_cast<float>(retreatChance7 - COMBAT_AI_STRENGTH_5000_RETREAT_PENALTY);
    else if (strengths1[H2EnumIndex(m_currentSide)] > COMBAT_AI_STRENGTH_2500)
        retreatChance7 =
    ...
    if (retreatChance7
        > (retreatThreshold7 =
               static_cast<float>(strengths1[H2EnumIndex(m_currentSide)])
               / (strengths1[H2EnumIndex(COMBAT_ATTACKER_SIDE)] + strengths1[H2EnumIndex(COMBAT_DEFENDER_SIDE)])))
        return 1;
    return 0;
}
...
void combatManager::DoCompAI(CombatSide) {
    ...
    m_limitCreature = 0;
    currentArmy9 = m_currentArmyIndex + m_armies[H2EnumIndex(m_currentArmySide)];
    attackType3 = COMBAT_AI_ATTACK_NONE;
    enemySide12 = OppositeCombatSide(m_currentSide);

    mirrorMasks7[H2EnumIndex(m_currentSide)] = GetMirrorImageMask(m_currentSide);
    mirrorMasks7[H2EnumIndex(enemySide12)] = GetMirrorImageMask(enemySide12);
    shooterMasks6[H2EnumIndex(m_currentSide)] = GetShooterMask(m_currentSide);
    shooterMasks6[H2EnumIndex(enemySide12)] = GetShooterMask(enemySide12);
    flyerMasks14[H2EnumIndex(m_currentSide)] = GetFlyerMask(m_currentSide);
    flyerMasks14[H2EnumIndex(enemySide12)] = GetFlyerMask(enemySide12);
    walkerMasks15[H2EnumIndex(m_currentSide)] = GetWalkerMask(m_currentSide);
    walkerMasks15[H2EnumIndex(enemySide12)] = GetWalkerMask(enemySide12);
    outOfItMasks15[H2EnumIndex(m_currentSide)] = GetOutOfItMask(m_currentSide);
    outOfItMasks15[H2EnumIndex(enemySide12)] = GetOutOfItMask(enemySide12);
    traitorMasks13[H2EnumIndex(m_currentSide)] = GetTraitorMask(m_currentSide);
    traitorMasks13[H2EnumIndex(enemySide12)] = GetTraitorMask(enemySide12);
    shooterStrengths37[H2EnumIndex(m_currentSide)] = GetStrength(m_currentSide, shooterMasks6[H2EnumIndex(m_currentSide)]);
    shooterStrengths37[H2EnumIndex(enemySide12)] = GetStrength(enemySide12, shooterMasks6[H2EnumIndex(enemySide12)]);
    totalArmyStrength11 = GetStrength(
        m_currentSide,
        shooterMasks6[H2EnumIndex(m_currentSide)]
            | (flyerMasks14[H2EnumIndex(m_currentSide)]
               | (walkerMasks15[H2EnumIndex(m_currentSide)]
                  | (outOfItMasks15[H2EnumIndex(m_currentSide)] | traitorMasks13[H2EnumIndex(m_currentSide)])))
    );
    fifthArmyStrength5 = static_cast<i32>(totalArmyStrength11 + COMBAT_AI_STRENGTH_ROUNDING)
                         / COMBAT_AI_STRENGTH_FRACTION;
    strongEnough16 = 0;
    enemyStronger3 = 0;
    currentShooterStrength5 = GetStrength(m_currentSide, shooterMasks6[H2EnumIndex(m_currentSide)]);
    enemyShooterStrength18 = GetStrength(enemySide12, shooterMasks6[H2EnumIndex(enemySide12)]);
    ...
    if (m_inCastleCombat != 0) {
        ...
        if (m_wallStates[H2EnumIndex(COMBAT_WALL_SLOT_KEEP)] == COMBAT_WALL_STATE_KEEP_STANDING) {
            castle2 = m_combatTowns[H2EnumIndex(COMBAT_DEFENDER_SIDE)];
            m_combatTowns[H2EnumIndex(COMBAT_DEFENDER_SIDE)]->CalcNumLevelArchers(&numArchers6, &archerLevel18);
            extraArchers29 = 0;
            ...
            if (m_heroes[H2EnumIndex(COMBAT_ATTACKER_SIDE)] != NULL
                && (m_heroes[H2EnumIndex(COMBAT_ATTACKER_SIDE)]->HasArtifact(ARTIFACT_GOLDEN_BOW)
                    || m_heroes[H2EnumIndex(COMBAT_ATTACKER_SIDE)]->m_secondarySkills[H2EnumIndex(HERO_SKILL_ARCHERY)]
                           != HERO_SKILL_LEVEL_NONE))
                goldenBowBonus = 0;
            else
                goldenBowBonus = 1;
    ...
    if ((H2EnumIndex((currentArmy9->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_SHOOTER))) != 0) {
        if (currentArmy9->m_monster.shots > 0)
            attackType3 = COMBAT_AI_ATTACK_SHOOT;
        else
            attackType3 = COMBAT_AI_ATTACK_WALK;
    } else if ((H2EnumIndex((currentArmy9->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_FLYING))) != 0) {
    ...
    switch (attackType3) {
        case COMBAT_AI_ATTACK_SHOOT:
            ...
            targetArmy16 = GetBestArmy(enemySide12, mirrorMasks7[H2EnumIndex(enemySide12)]);
            if (targetArmy16 != -1) {
                giNextAction = ACTION_MOVE;
                giNextActionGridIndex =
                    (m_armies[0] + targetArmy16 + H2EnumIndex(enemySide12) * COMBAT_ARMY_STORAGE_SLOT_COUNT)
                        ->m_hex;
                goto finish;
            }
            targetArmy16 = GetBestArmy(enemySide12, shooterMasks6[H2EnumIndex(enemySide12)]);
            if (targetArmy16 != -1) {
                giNextAction = ACTION_MOVE;
                giNextActionGridIndex =
                    (m_armies[0] + targetArmy16 + H2EnumIndex(enemySide12) * COMBAT_ARMY_STORAGE_SLOT_COUNT)
            ...
            targetArmy16 = GetBestArmy(enemySide12, flyerMasks14[H2EnumIndex(enemySide12)]);
            if (targetArmy16 != -1) {
                giNextAction = ACTION_MOVE;
                giNextActionGridIndex =
                    (m_armies[0] + targetArmy16 + H2EnumIndex(enemySide12) * COMBAT_ARMY_STORAGE_SLOT_COUNT)
            ...
            if (walkerMasks15[H2EnumIndex(enemySide12)] != 0) {
                targetArmy16 =
                    GetClosestArmy(currentArmy9, enemySide12, walkerMasks15[H2EnumIndex(enemySide12)]);
                if (targetArmy16 != -1) {
                    giNextAction = ACTION_MOVE;
                    giNextActionGridIndex =
                        (m_armies[0] + targetArmy16 + H2EnumIndex(enemySide12) * COMBAT_ARMY_STORAGE_SLOT_COUNT)
            ...
            targetArmy16 = GetBestArmy(enemySide12, outOfItMasks15[H2EnumIndex(enemySide12)]);
            if (targetArmy16 != -1) {
                giNextAction = ACTION_MOVE;
                giNextActionGridIndex =
                    (m_armies[0] + targetArmy16 + H2EnumIndex(enemySide12) * COMBAT_ARMY_STORAGE_SLOT_COUNT)
            ...
            targetArmy16 = GetBestArmy(enemySide12, traitorMasks13[H2EnumIndex(enemySide12)]);
            if (targetArmy16 != -1) {
                giNextAction = ACTION_MOVE;
                giNextActionGridIndex =
                    (m_armies[0] + targetArmy16 + H2EnumIndex(enemySide12) * COMBAT_ARMY_STORAGE_SLOT_COUNT)
        ...
        case COMBAT_AI_ATTACK_FLY:
            if (strongEnough16 != 0 && enemyStronger3 == 0) {
                if (AttemptAttack(currentArmy9, enemySide12, mirrorMasks7[H2EnumIndex(enemySide12)]))
                    goto finish;
                if (AttemptAttack(currentArmy9, enemySide12, shooterMasks6[H2EnumIndex(enemySide12)]))
                    goto finish;
                if (AttemptAttack(currentArmy9, enemySide12, flyerMasks14[H2EnumIndex(enemySide12)]))
                    goto finish;
                if (AttemptAttack(currentArmy9, enemySide12, walkerMasks15[H2EnumIndex(enemySide12)]))
                    goto finish;
                if (AttemptAttack(currentArmy9, enemySide12, outOfItMasks15[H2EnumIndex(enemySide12)]))
                    goto finish;
                if (AttemptAttack(currentArmy9, enemySide12, traitorMasks13[H2EnumIndex(enemySide12)]))
                    goto finish;
            } else {
                if (AttemptAttack(currentArmy9, enemySide12, mirrorMasks7[H2EnumIndex(enemySide12)]))
                    goto finish;
        ...
        case COMBAT_AI_ATTACK_WALK:
            if (strongEnough16 != 0 && enemyStronger3 == 0) {
                ...
                if (AttemptAttack(currentArmy9, enemySide12, traitorMasks13[H2EnumIndex(enemySide12)]))
                    goto finish;
                if (WalkTowardArmyFront(currentArmy9, m_currentSide, shooterMasks6[H2EnumIndex(m_currentSide)]))
                    goto finish;
            ...
            if (AttemptAttack(currentArmy9, enemySide12, traitorMasks13[H2EnumIndex(enemySide12)]))
                goto finish;
            if (WalkTowardArmy(currentArmy9, enemySide12, shooterMasks6[H2EnumIndex(enemySide12)]))
                goto finish;
            if (WalkTowardArmy(currentArmy9, enemySide12, walkerMasks15[H2EnumIndex(enemySide12)]))
                goto finish;
            if (WalkTowardArmy(currentArmy9, enemySide12, flyerMasks14[H2EnumIndex(enemySide12)]))
                goto finish;
            if (WalkTowardArmy(currentArmy9, enemySide12, outOfItMasks15[H2EnumIndex(enemySide12)]))
                goto finish;
            if (WalkTowardArmy(currentArmy9, enemySide12, traitorMasks13[H2EnumIndex(enemySide12)]))
                goto finish;
...
finish:
    if (giNextAction == ACTION_MOVE && giNextActionGridIndex > 0
        ...
        for (adjacentDirection6 = COMBAT_DIRECTION_NORTHEAST;
             H2EnumIndex(adjacentDirection6) < COMBAT_AI_ADJACENT_DIRECTION_COUNT;
             adjacentDirection6++) {
...
void combatManager::DoLichShot(class army* lich) {
    ...
    i32 armyIndex37;
    u8 damaged19[H2EnumIndex(COMBAT_SIDE_COUNT) * COMBAT_AI_ARMY_SLOT_COUNT];
    float damageValue10;
    ...
    for (armyIndex37 = 0; armyIndex37 < m_armyCount[H2EnumIndex(OppositeCombatSide(m_currentSide))];
         armyIndex37++) {
        memset(damaged19, 0, sizeof(damaged19));
        damageValue10 = 0;
        target17 = &m_armies[H2EnumIndex(OppositeCombatSide(m_currentSide))][armyIndex37];
        if (target17 == NULL
            || (H2EnumIndex((target17->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED))) != 0
            || target17->m_quantity <= 0)
            continue;
        damageValue10 = GetModLichDamage(target17, lichDamage5);
        damaged19[H2EnumIndex(target17->m_side) * COMBAT_AI_ARMY_SLOT_COUNT + target17->m_index] = 1;
        targetHex36 = target17->m_hex;
        for (direction37 = COMBAT_DIRECTION_NORTHEAST;
             H2EnumIndex(direction37) < COMBAT_AI_ADJACENT_DIRECTION_COUNT;
             direction37++) {
            ...
            if (adjacentHex13 >= 0 && adjacentHex13 < COMBAT_HEX_COUNT
                ...
                && damaged19
                           [H2EnumIndex(m_hexCells[adjacentHex13].m_occupantSide) * COMBAT_AI_ARMY_SLOT_COUNT
                            + m_hexCells[adjacentHex13].m_occupantIndex]
                       == 0) {
                adjacentDamage6 = GetModLichDamage(
                    &m_armies[H2EnumIndex(m_hexCells[adjacentHex13].m_occupantSide)]
                             [m_hexCells[adjacentHex13].m_occupantIndex],
                    lichDamage5
                );
                damaged19
...
i32 combatManager::GetShooterMask(CombatSide side) {
    ...
    for (armyIndex2 = 0; armyIndex2 < m_armyCount[H2EnumIndex(side)]; armyIndex2++) {
        currentArmy10 = armyIndex2 + m_armies[H2EnumIndex(side)];
        if (currentArmy10 != NULL
            && (H2EnumIndex((currentArmy10->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                   == 0
            && (H2EnumIndex((currentArmy10->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_SHOOTER))) != 0
            && currentArmy10->m_monster.shots > 0
            && currentArmy10->m_spellInfluence[H2EnumIndex(ARMY_SPELL_INFLUENCE_BLIND)] == 0
            && currentArmy10->m_spellInfluence[H2EnumIndex(ARMY_SPELL_INFLUENCE_PARALYZE)] == 0
            && currentArmy10->m_spellInfluence[H2EnumIndex(ARMY_SPELL_INFLUENCE_PETRIFIED)] == 0
            && currentArmy10->m_spellInfluence[H2EnumIndex(ARMY_SPELL_INFLUENCE_BERSERK)] == 0
            && currentArmy10->m_spellInfluence[H2EnumIndex(ARMY_SPELL_INFLUENCE_HYPNOTIZE)] == 0)
            mask5 |= bit1;
        bit1 <<= 1;
    }
    return mask5;
...
i32 combatManager::GetMirrorImageMask(CombatSide side) {
    ...
    for (armyIndex2 = 0; armyIndex2 < m_armyCount[H2EnumIndex(side)]; armyIndex2++) {
        currentArmy10 = m_armies[H2EnumIndex(side)] + armyIndex2;
        if (currentArmy10 != NULL
            && (H2EnumIndex((currentArmy10->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                   == 0
            && (H2EnumIndex((currentArmy10->m_monster.flags.all) & (MONSTER_FLAGS_MIRROR_IMAGE))) != 0)
            mask5 |= bit1;
        bit1 <<= 1;
    }
    return mask5;
...
i32 combatManager::GetFlyerMask(CombatSide side) {
    ...
    for (armyIndex2 = 0; armyIndex2 < m_armyCount[H2EnumIndex(side)]; armyIndex2++) {
        ...
        if (currentArmy10 != NULL
            && (H2EnumIndex((currentArmy10->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                   == 0
            && (H2EnumIndex((currentArmy10->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_FLYING))) != 0
            && currentArmy10->m_spellInfluence[H2EnumIndex(ARMY_SPELL_INFLUENCE_BLIND)] == 0
            && currentArmy10->m_spellInfluence[H2EnumIndex(ARMY_SPELL_INFLUENCE_PARALYZE)] == 0
            && currentArmy10->m_spellInfluence[H2EnumIndex(ARMY_SPELL_INFLUENCE_PETRIFIED)] == 0
            && currentArmy10->m_spellInfluence[H2EnumIndex(ARMY_SPELL_INFLUENCE_BERSERK)] == 0
...
i32 combatManager::GetAllMask(CombatSide side) {
    ...
    for (armyIndex11 = 0; armyIndex11 < m_armyCount[H2EnumIndex(side)]; armyIndex11++) {
        currentArmy38 = &m_armies[H2EnumIndex(side)][armyIndex11];
        if (currentArmy38 != NULL
            && (H2EnumIndex((currentArmy38->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                   == 0
...
i32 combatManager::GetOutOfItMask(CombatSide side) {
    ...
    for (armyIndex2 = 0; armyIndex2 < m_armyCount[H2EnumIndex(side)]; armyIndex2++) {
        currentArmy10 =
            m_armies[0] + H2EnumIndex(side) * COMBAT_ARMY_STORAGE_SLOT_COUNT + armyIndex2;
        if (currentArmy10 != NULL
            && (H2EnumIndex((currentArmy10->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                   == 0
            && (currentArmy10->m_spellInfluence[H2EnumIndex(ARMY_SPELL_INFLUENCE_BLIND)] != 0
    ...
    for (armyIndex2 = 0; armyIndex2 < m_armyCount[H2EnumIndex(side)]; armyIndex2++) {
        if ((mask & bit1) != 0) {
            strength8 =
                (m_armies[0] + H2EnumIndex(side) * COMBAT_ARMY_STORAGE_SLOT_COUNT + armyIndex2)
                    ->Strength();
            if ((m_armies[0] + H2EnumIndex(side) * COMBAT_ARMY_STORAGE_SLOT_COUNT + armyIndex2)
...
i32 combatManager::GetWorstArmy(CombatSide side, i32 mask) {
    ...
    for (armyIndex2 = 0; armyIndex2 < m_armyCount[H2EnumIndex(side)]; armyIndex2++) {
        if ((mask & bit1) != 0) {
            strength8 = m_armies[H2EnumIndex(side)][armyIndex2].Strength();
...
i32 combatManager::GetClosestArmy(
    ...
    for (armyIndex2 = 0; armyIndex2 < m_armyCount[H2EnumIndex(side)]; armyIndex2++) {
        if ((bit1 & mask) != 0) {
            target26 = &m_armies[H2EnumIndex(side)][armyIndex2];
...
u32l combatManager::GetStrength(CombatSide side, i32 mask) {
    ...
    for (armyIndex4 = 0; armyIndex4 < m_armyCount[H2EnumIndex(side)]; armyIndex4++) {
        if ((bit36 & mask) != 0) {
            currentArmy8 = &m_armies[H2EnumIndex(side)][armyIndex4];
            if (currentArmy8 != NULL
                && (H2EnumIndex((currentArmy8->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                       == 0)
...
i32 combatManager::AttemptAdjacentAttack(class army* currentArmy) {
    ...
    for (direction36 = COMBAT_DIRECTION_NORTHEAST;
         H2EnumIndex(direction36) < COMBAT_AI_ATTACK_DIRECTION_COUNT;
         direction36++) {
    ...
    if (targetArmy15 != COMBAT_AI_NO_ARMY) {
        giNextAction = ACTION_MOVE;
        giNextActionGridIndex = m_armies[H2EnumIndex(OppositeCombatSide(m_currentSide))][targetArmy15].m_hex;
        return 1;
    } else {
        return 0;
    }
...
i32 combatManager::WalkTowardArmyFront(
    ...
    targetHex7 = targetArmy6[m_armies[H2EnumIndex(side)]].m_hex;
    if ((H2EnumIndex((targetArmy6[m_armies[H2EnumIndex(side)]].m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_WIDE)))
        != 0)
...
i32 combatManager::WalkTowardArmy(
    ...
    targetPtr9 = &m_armies[H2EnumIndex(side)][targetArmy6];
    ...
    if (pathFound5 == 0
        && (H2EnumIndex((targetPtr9->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_WIDE))) != 0) {
