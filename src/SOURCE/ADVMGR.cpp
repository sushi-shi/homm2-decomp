...
i32 advManager::Open(i32 id) {
    ...
    if (m_heroIcons[(FACTION_KNIGHT)] == NULL) {
        m_heroIcons[(FACTION_KNIGHT)] = gpResourceManager->GetIcon("kngt32.icn");
    }
    if (m_heroIcons[(FACTION_BARBARIAN)] == NULL) {
        m_heroIcons[(FACTION_BARBARIAN)] = gpResourceManager->GetIcon("barb32.icn");
    }
    if (m_heroIcons[(FACTION_SORCERESS)] == NULL) {
        m_heroIcons[(FACTION_SORCERESS)] = gpResourceManager->GetIcon("sorc32.icn");
    }
    if (m_heroIcons[(FACTION_WARLOCK)] == NULL) {
        m_heroIcons[(FACTION_WARLOCK)] = gpResourceManager->GetIcon("wrlk32.icn");
    }
    if (m_heroIcons[(FACTION_WIZARD)] == NULL) {
        m_heroIcons[(FACTION_WIZARD)] = gpResourceManager->GetIcon("wzrd32.icn");
    }
    if (m_heroIcons[(FACTION_NECROMANCER)] == NULL) {
        m_heroIcons[(FACTION_NECROMANCER)] = gpResourceManager->GetIcon("necr32.icn");
    }
    if (m_heroIcons[(HERO_TYPE_BOAT)] == NULL) {
        m_heroIcons[(HERO_TYPE_BOAT)] = gpResourceManager->GetIcon("boat32.icn");
    }
            ...
            case MESSAGE_KEY_DOWN:
                ...
                switch (message.payload.keyboard.keyCode) {
                    ...
                    case INPUT_SCAN_F8:
                        break;
                    case INPUT_SCAN_F9:
                        for (cheatDigitLocal = 0; cheatDigitLocal < (RES_COUNT);
                             ++cheatDigitLocal) {
            ...
            switch (currentCell->m_triggerType & MAP_TRIGGER_TYPE_MASK) {
                ...
                case MAP_OBJECT_MOSSY_ROCK:
                    sprintf(
                        gText,
                        "%s",
                        gTerrainNames[(giGroundToTerrain
                ...
                case MAP_OBJECT_MINE:
                    ...
                    quick_info_guarded:
                        sprintf(
                            ...
                            gArmyNamesPlural[(gpGame->m_mines[currentCell->m_objectMetadata].guardianType)]
                        );
...
void advManager::UpdateTownLocators(i32 drawWindow, i32 updateScreen) {
    ...
    for (locatorSlot = 0; locatorSlot < LOCATOR_VISIBLE_COUNT; ++locatorSlot) {
        ...
        if (townId37 == TOWN_ID_NONE || gbAllBlack) {
            ...
            if (H2BitTest(gpGame->m_knownTowns, townId37)) {
                locatorMessage14.payload.widget.command = ADVMGR_LOCATOR_COMMAND_SET_FLAGS;
            } else {
                locatorMessage14.payload.widget.command = ADVMGR_LOCATOR_COMMAND_CLEAR_FLAGS;
            }
    ...
    if (gpCurPlayer->m_townCount < LOCATOR_PAGE_THRESHOLD) {
        ...
        m_scrollRightButton->m_y = static_cast<i16>(
            gpCurPlayer->m_townLocatorPage * scrollStep + (LOCATOR_SCROLL_BASE_Y)
        );
    }
    if (drawWindow) {
        m_adventureWindow->DrawWindow(updateScreen);
...
i32 advManager::UpdBottomViewKingdom(void) {
    ...
    textY5[(RES_WOOD)] = KINGDOM_VIEW_RESOURCE_TEXT_Y;
    textY5[(RES_MERCURY)] = KINGDOM_VIEW_RESOURCE_TEXT_Y;
    textY5[(RES_ORE)] = KINGDOM_VIEW_RESOURCE_TEXT_Y;
    textY5[(RES_SULFUR)] = KINGDOM_VIEW_RESOURCE_TEXT_Y;
    textY5[(RES_CRYSTAL)] = KINGDOM_VIEW_RESOURCE_TEXT_Y;
    textY5[(RES_GEMS)] = KINGDOM_VIEW_RESOURCE_TEXT_Y;
    textY5[(RES_GOLD)] = KINGDOM_VIEW_TOWN_TEXT_Y;
...
void advManager::TownQuickView(i32 townId, i32 locatorSlot, i32 windowX, i32 windowY) {
    ...
    messageLocal.payload.widget.data.value =
        (quickTownLocal->m_type) + TOWN_QUICK_TYPE_FRAME_BASE;
    ...
    if (informationLevel != TOWN_QUICK_INFORMATION_EXACT
        || H2BitTest(gpGame->m_knownTowns, static_cast<i8>(quickTownLocal->m_id)) == 0) {
...
void advManager::DemobilizeCurrHero(void) {
    ...
    if (m_cursorType == HERO_TYPE_BOAT) {
        currentHero->m_eventFlags = HeroEventFlag(
            static_cast<i32>(currentHero->m_eventFlags) | (HERO_EVENT_EMBARKED)
        );
    }
...
void advManager::SetTownContext(i32 townId) {
    ...
    contextValue7 = (giGroundToTerrain[GetCell(currentTownValue->m_x, currentTownValue->m_y)
                              ->m_terrainImageIndex]);
    ...
    if (bShowIt != 0) {
        ...
        if (m_cursorType == HERO_TYPE_BOAT) {
            mapHero->m_eventFlags = HeroEventFlag(
                static_cast<i32>(mapHero->m_eventFlags) | (HERO_EVENT_EMBARKED)
            );
        }
        m_cursorActive = 0;
    }
...
void advManager::ViewPuzzle(void) {
    ...
    for (pieceIndexPosition = 0; pieceIndexPosition < PUZZLE_PIECE_COUNT;
         ++pieceIndexPosition) {
        if (!H2BitTest(puzzlePiecesRemoved, pieceIndexPosition)) {
...
void UpdateSystemOptions(i32 initialDraw) {
    ...
    cPanel->BroadcastMessage(message);
    message.payload.widget.id =
        (SYSTEM_OPTION_INTERFACE) + ADVMGR_SYSTEM_OPTIONS_TEXT_ID_OFFSET;
    message.payload.widget.data.text = gInterfaceTypeText[gConfig.evilInterfaceUsage];
    cPanel->BroadcastMessage(message);
    message.payload.widget.id =
        (SYSTEM_OPTION_VIDEO) + ADVMGR_SYSTEM_OPTIONS_TEXT_ID_OFFSET;
...
i32 advManager::DoVisions(hero* visionHero) {
    ...
    if (visionHero->m_army.CanJoin(creatureData)
        ...
        if (forcedJoinState) {
            ...
            joiningCostIndex = gMonsterDatabase[(creatureData)].cost * monsterCountIndex
