...
i32 advManager::Open(i32 id) {
    ...
    GetCursorSampleSet(gConfig.walkSpeeds[(CONFIG_WALK_SPEED_HUMAN)]);
    if (!gbThisNetHumanPlayer[giCurPlayer]) {
    ...
    if (InMapArea(mouseX, mouseY)) {
        ...
        if (m_lastHoverCell != mouseCellX || m_hoverCellY != mouseCellY) {
            ...
            if (gpCurPlayer->m_currentHero == INVALID_HERO) {
                ...
                if (hoverCell->m_flags & (MAP_CELL_OCCUPIED)) {
                    if ((hoverCell->m_triggerType & MAP_TRIGGER_TYPE_MASK)
                        == MAP_OBJECT_CASTLE) {
                    ...
                    if (mapX > 0) {
                        ...
                        if (s_drawAdjacentCell->m_triggerType
                            ...
                            if (s_drawMine->guardianType == CREATURE_GHOST) {
                                ...
                                DRAW_ADVENTURE_ICON(
                                    m_objectIcons[(TILESET_OBJNXTRA)],
...
void advManager::TrimLoopingSounds(i32 maxSamples) {
    ...
    i32 loaded = 0;
    char keep[(ADVMGR_ENVIRONMENT_SOUND_COUNT)];
...
void advManager::SystemOptions(void) {
    ...
    prevWalkSpeed = gConfig.walkSpeeds[(CONFIG_WALK_SPEED_HUMAN)];
...
void UpdateSystemOptions(i32 initialDraw) {
    ...
    message.payload.widget.data.value =
        (gConfig.walkSpeeds[(CONFIG_WALK_SPEED_HUMAN)]) + ADVMGR_SYSTEM_OPTIONS_SPEED_FRAME_BASE;
    cPanel->BroadcastMessage(message);
    ...
    if (gConfig.blackoutComputer != 0) {
        ...
        message.payload.widget.data.value =
            (gConfig.walkSpeeds[(CONFIG_WALK_SPEED_COMPUTER)]) + ADVMGR_SYSTEM_OPTIONS_SPEED_FRAME_BASE;
    }
    cPanel->BroadcastMessage(message);
    ...
    message.payload.widget.data.text = walkSpeedText[(gConfig.walkSpeeds[(CONFIG_WALK_SPEED_HUMAN)])];
    cPanel->BroadcastMessage(message);
    message.payload.widget.id =
            ...
            switch (message.payload.widget.command) {
                ...
                case WIDGET_NOTIFY_SELECT: {
                    switch (
                        ...
                        case (SYSTEM_OPTION_HERO_SPEED):
                            ++gConfig.walkSpeeds[(CONFIG_WALK_SPEED_HUMAN)];
                            gConfig.walkSpeeds[(CONFIG_WALK_SPEED_HUMAN)] %= CONFIG_WALK_SPEED_COUNT;
                            preferencesChanged = true;
                            bPrefsChanged = true;
                            break;
                        ...
                        case (SYSTEM_OPTION_COMPUTER_SPEED):
                            if (gConfig.blackoutComputer) {
                                ...
                                gConfig.walkSpeeds[(CONFIG_WALK_SPEED_COMPUTER)] = CONFIG_WALK_SPEED_NORMAL;
                            } else if (gConfig.walkSpeeds[(CONFIG_WALK_SPEED_COMPUTER)] < CONFIG_WALK_SPEED_INSTANT) {
                                ++gConfig.walkSpeeds[(CONFIG_WALK_SPEED_COMPUTER)];
                            } else {
...
i32 advManager::DoVisions(hero* visionHero) {
    ...
    count = spot->m_objectMetadata & (MAP_MONSTER_COUNT_MASK);
    sprintf(
        gText,
        "{%d %s}\n\n",
        count,
