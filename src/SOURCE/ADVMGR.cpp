...
void advManager::TownQuickView(i32 townId, i32, i32 windowX, i32 windowY) {
    ...
    if (gpGame->GetTown(townId)->m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_CASTLE)))
        message.payload.widget.data.value = castleIconFrames[H2EnumIndex(townPointer->m_type)];
    else
...
i32 SaveGame(void) {
    ...
    utf8::Format(suffix, sizeof(suffix), "%s", GetSaveFileExtension(false).c_str());
    utf8::Format(pattern, sizeof(pattern), "*%s", suffix);
