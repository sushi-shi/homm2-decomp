            ...
            if (((H2EnumIndex((drawMask) & (ADVMGR_DRAW_HERO))) || (H2EnumIndex((drawMask) & (ADVMGR_DRAW_HERO_SHADOW))))
                ...
                if (s_drawCell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_BOAT))) {
                    ...
                    if (s_drawCell->m_triggerType
                        ...
                        s_drawPlayerColor = gpGame->m_players[H2EnumIndex(s_drawHero->m_owner)].m_color;
                        s_drawHeroType =
                ...
                if ((cell->m_flags & CURSOR_MAP_VISIBLE_FLAG) != 0
                    ...
                    if ((cell->m_triggerType & MAP_TRIGGER_TYPE_MASK)
                        ...
                        if (setId == TILESET_X_LOC2
                            ...
                            color = gMapColors[H2EnumIndex(CELL_TERRAIN(cell))] + RADAR_TERRAIN_SHADE;
                        } else {
                            switch (setId) {
            ...
            switch (currentCell->m_triggerType & MAP_TRIGGER_TYPE_MASK) {
                ...
                case MAP_OBJECT_TRAVELER_TENT:
                    if ((H2EnumIndex((currentCell->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG)))) {
                        ...
                        sprintf(gText, "%s", gTerrainNames[H2EnumIndex(CELL_TERRAIN(currentCell))]);
                    }
                    break;
...
void advManager::HeroQuickView(i32 heroId, i32 locatorSlot, i32 windowX, i32 windowY) {
    ...
    message.payload.widget.data.value =
        gpGame->m_players[H2EnumIndex(targetHero->m_owner)].m_color * HERO_QUICK_PLAYER_COLOR_STRIDE;
    window->BroadcastMessage(message);
...
void advManager::TownQuickView(
    ...
    if (townPointer->m_owner == -1) {
        ...
        message.payload.widget.id = TOWN_QUICK_PLAYER_COLOR_WIDGET;
        message.payload.widget.data.value =
            gpGame->m_players[H2EnumIndex(townPointer->m_owner)].m_color * HERO_QUICK_PLAYER_COLOR_STRIDE;
        window->BroadcastMessage(message);
        ++message.payload.widget.id;
        ++message.payload.widget.data.value;
        window->BroadcastMessage(message);
...
void advManager::SetTownContext(i32 townId) {
    ...
    townNo = H2EnumIndex(CELL_TERRAIN(GetCell(townPointer->m_x, townPointer->m_y)));
...
i32 advManager::DoVisions(hero* visionHero) {
    ...
    if (visionHero->m_army.CanJoin(type) && fRatio > MONSTER_STRENGTH_JOIN
        ...
        if (isForced) {
            ...
            if (joinFee
                > gpGame->m_players[H2EnumIndex(visionHero->m_owner)].m_resources[H2EnumIndex(RES_GOLD)]) {
