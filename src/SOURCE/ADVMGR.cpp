...
static const i32 environmentVolumes[ADVMGR_ENVIRONMENT_VOLUME_COUNT] = {64, 57, 40, 21, 7, 5, 3, 0};
...
i32 advManager::Open(i32 id) {
    ...
    if (m_adventureWindow == 0) {
        ...
        m_scrollLeftButton = new iconWidget(
            ADVMGR_SCROLL_LEFT_X,
            ADVMGR_SCROLL_Y,
            ADVMGR_SCROLL_WIDTH,
            ADVMGR_SCROLL_HEIGHT,
            "scroll.icn",
            4,
            0,
            ADVMGR_SCROLL_LEFT_FRAME,
            ADVMGR_SCROLL_HOTKEY,
            1
        );
    ...
    for (resourceIndex = 0; resourceIndex < ADVMGR_OBJECT_ICON_COUNT; ++resourceIndex) {
        if (strlen(gTilesetFiles[resourceIndex]) > 1 && m_objectIcons[resourceIndex] == 0
    ...
    for (resourceIndex = 0; resourceIndex < ADVMGR_SOUND_CELL_COUNT; ++resourceIndex) {
        m_activeSounds[resourceIndex].soundId = ADVMGR_ENVIRONMENT_SOUND_NONE;
        m_activeSounds[resourceIndex].volume = ADVMGR_ENVIRONMENT_SOUND_DEFAULT_VOLUME;
        m_activeSoundMask = 0;
    }
...
void advManager::GetCursorSampleSet(i32 sampleSet) {
    ...
    for (i32 index = 0; index < ADVMGR_CURSOR_SAMPLE_COUNT; ++index) {
        ...
        m_cursorSamples[index]->m_playbackData.volume = ADVMGR_CURSOR_SAMPLE_VOLUME;
        m_cursorSamples[index]->m_playbackData.channelType = ADVMGR_CURSOR_SAMPLE_CHANNEL;
    }
}
...
void advManager::CheckSetEvilInterface(i32 redraw, i32 player) {
    ...
    if (shouldChange) {
        ...
        if (redraw) {
            ...
            for (translationIndex = 0; translationIndex < ADVMGR_INTERFACE_TRANSLATION_COUNT;
                 ++translationIndex) {
...
i32 advManager::ProcessHover(i32 mouseX, i32 mouseY) {
    ...
    if (InMapArea(mouseX, mouseY)) {
        ...
        if (m_lastHoverCell != cellXPosition || m_hoverCellY != cellYCurrent) {
            ...
            if (gpCurPlayer->m_currentHero == ADVMGR_INVALID_HERO) {
                if ((hoverCellLocal->m_triggerType & MAP_TRIGGER_TYPE_MASK) == ADVMGR_HOVER_TOWN
                ...
                if (!((m_cursorType == ADVMGR_CURSOR_ROUTE
                       ...
                       || hoverCellLocal->m_triggerType == (MAP_TRIGGER_ACTION_FLAG | MAP_OBJECT_HERO_INTERACTION)
                       || hoverCellLocal->m_triggerType == (MAP_TRIGGER_ACTION_FLAG | MAP_OBJECT_BOAT)
...
void advManager::DrawCell(
    ...
    if (gbAllBlack == 0 && (mapX < 0 || mapY < 0 || mapX >= MAP_WIDTH || mapY >= MAP_HEIGHT)) {
        ...
        if (s_drawStoneTile == -1) {
            s_drawStoneTile =
                ((mapY + 16) % ADVMGR_CLOUD_VARIANTS) * 4 + (mapX + 16) % ADVMGR_CLOUD_VARIANTS;
        }
        TileToBitmap(
            m_stoneTiles,
            s_drawStoneTile,
            ...
            if (((drawMask & ADVMGR_DRAW_HERO) || (drawMask & ADVMGR_DRAW_HERO_SHADOW))
                ...
                if (drawMask & ADVMGR_DRAW_HERO) {
                    if (mapX > 0) {
                        ...
                        if (s_drawAdjacentCell->m_triggerType == (MAP_TRIGGER_ACTION_FLAG | MAP_OBJECT_MINE)) {
                            s_drawMine = &gpGame->m_mines[s_drawAdjacentCell->m_objectMetadata];
                            if (s_drawMine->guardianType == 59) {
                                IconToBitmap(
                                    m_objectIcons[ADVMGR_MINE_GUARDIAN_ICON_SLOT],
                                    gpWindowManager->m_screen,
                                    s_drawPixelX - 16,
                                    s_drawPixelY,
                                ...
                                IconToBitmap(
                                    m_objectIcons[ADVMGR_TILESET_BOAT],
                                    gpWindowManager->m_screen,
                                    s_drawPixelX - 32,
                                    s_drawPixelY,
                    ...
                    if (s_drawCell->m_objectTileset == ADVMGR_TILESET_MINE) {
                        if (m_lastQuickViewX == mapX && m_lastQuickViewY == mapY) {
                            IconToBitmap(
                                m_objectIcons[ADVMGR_TILESET_MONSTER],
                                gpWindowManager->m_screen,
                                s_drawPixelX + 16,
                                s_drawPixelY + 30,
                ...
                if (s_drawCell->m_triggerType == (MAP_TRIGGER_ACTION_FLAG | MAP_OBJECT_BOAT)) {
                    ...
                    s_drawHeroYOffset = 0;
                    if (s_drawCell->m_triggerType == (MAP_TRIGGER_ACTION_FLAG | MAP_OBJECT_HERO_INTERACTION)) {
        ...
        case ADVMGR_RADAR_MAP_SMALL:
            screenRowOffset = gpWindowManager->m_screen->m_pixels
                              + (minYOffset * 4 + ADVMGR_RADAR_TOP) * ADVMGR_RADAR_ROW_GROUPS
                                    * ADVMGR_RADAR_GROUP_BYTES
                              + ADVMGR_RADAR_LEFT;
            screenColumnIndex = minXSlot * 4;
            break;
        case ADVMGR_RADAR_MAP_MEDIUM:
            screenRowOffset = gpWindowManager->m_screen->m_pixels
                              + (minYOffset * 2 + ADVMGR_RADAR_TOP) * ADVMGR_RADAR_ROW_GROUPS
                                    * ADVMGR_RADAR_GROUP_BYTES
                              + ADVMGR_RADAR_LEFT;
...
void advManager::UpdateHeroLocator(i32 locatorSlot, i32 drawWindow, i32 updateScreen) {
    ...
    widgetBase7 = locatorSlot * ADVMGR_LOCATOR_HERO_WIDGET_STRIDE + ADVMGR_LOCATOR_HERO_WIDGET_BASE;
    ...
    locatorMessage15.payload.widget.id = widgetBase7 + ADVMGR_LOCATOR_HERO_BORDER_OFFSET;
    if (gpCurPlayer->m_currentHero == heroId9 && gpCurPlayer->m_currentHero != ADVMGR_INVALID_HERO
        && !gbAllBlack) {
    ...
    if (heroId9 == ADVMGR_INVALID_HERO || gbAllBlack) {
        locatorMessage15.payload.widget.id = widgetBase7 + ADVMGR_LOCATOR_HERO_IMAGE_OFFSET;
        ...
        for (index3 = 0; index3 <= ADVMGR_LOCATOR_HERO_WIDGET_STRIDE - 1; ++index3) {
            locatorMessage15.payload.widget.id = widgetBase7 + index3;
            m_adventureWindow->BroadcastMessage(locatorMessage15);
        }

        mobilityFrame18 = GetMobilityFrame(locatorHero8->m_remainingMobility);
        locatorMessage15.payload.widget.id = widgetBase7 + ADVMGR_LOCATOR_HERO_MOBILITY_OFFSET;
        locatorMessage15.payload.widget.command = ADVMGR_LOCATOR_COMMAND_SET_FRAME;
        locatorMessage15.payload.widget.data.value = mobilityFrame18;
        m_adventureWindow->BroadcastMessage(locatorMessage15);

        manaFrame36 = GetManaFrame(locatorHero8->m_spellPoints);
        locatorMessage15.payload.widget.id = widgetBase7 + ADVMGR_LOCATOR_HERO_MANA_OFFSET;
        locatorMessage15.payload.widget.command = ADVMGR_LOCATOR_COMMAND_SET_FRAME;
        locatorMessage15.payload.widget.data.value = manaFrame36;
        m_adventureWindow->BroadcastMessage(locatorMessage15);

        locatorMessage15.payload.widget.id = widgetBase7 + ADVMGR_LOCATOR_HERO_PORTRAIT_OFFSET;
        locatorMessage15.payload.widget.command = ADVMGR_LOCATOR_COMMAND_SET_FRAME;
    ...
    if (drawWindow) {
        ...
        if (updateScreen) {
            gpWindowManager->UpdateScreenRegion(
                ...
                locatorSlot * ADVMGR_LOCATOR_SCREEN_ROW_HEIGHT + ADVMGR_LOCATOR_SCREEN_TOP,
                ADVMGR_LOCATOR_SCREEN_WIDTH,
                ADVMGR_LOCATOR_SCREEN_HEIGHT
            );
        }
    }
}
...
void advManager::UpdateHeroLocators(i32 drawWindow, i32 updateScreen) {
    ...
    for (locatorSlot = 0; locatorSlot < ADVMGR_LOCATOR_VISIBLE_COUNT; ++locatorSlot) {
...
void advManager::UpdateTownLocators(i32 drawWindow, i32 updateScreen) {
    ...
    for (locatorSlot = 0; locatorSlot < ADVMGR_LOCATOR_VISIBLE_COUNT; ++locatorSlot) {
        ...
        locatorMessage14.payload.widget.id = locatorSlot + ADVMGR_LOCATOR_TOWN_BORDER_BASE;
        ...
        locatorMessage14.payload.widget.id = locatorSlot + ADVMGR_LOCATOR_TOWN_IMAGE_BASE;
        if (townId37 == ADVMGR_INVALID_CELL || gbAllBlack) {
            ...
            locatorMessage14.payload.widget.data.value = ADVMGR_LOCATOR_TOWN_ENABLE_FLAGS;
            m_adventureWindow->BroadcastMessage(locatorMessage14);
            locatorMessage14.payload.widget.command = ADVMGR_LOCATOR_COMMAND_CLEAR_FLAGS;
            locatorMessage14.payload.widget.data.value = ADVMGR_LOCATOR_TOWN_DISABLE_FLAGS;
            locatorMessage14.payload.widget.id = locatorSlot + ADVMGR_LOCATOR_TOWN_FLAG_BASE;
            m_adventureWindow->BroadcastMessage(locatorMessage14);
        } else {
...
update_bottom_view:
    if (updated && drawWindow) {
        m_adventureWindow->DrawWindow(
            ADVMGR_BOTTOM_VIEW_DRAW_LEFT,
            ADVMGR_BOTTOM_VIEW_DRAW_TOP,
            ADVMGR_BOTTOM_VIEW_DRAW_BOTTOM
        );
        if (updateScreen) {
            gpWindowManager->UpdateScreenRegion(
                ADVMGR_BOTTOM_VIEW_PANEL_X,
                ADVMGR_BOTTOM_VIEW_PANEL_Y,
...
void advManager::ClearBottomView(void) {
    ...
    for (widgetIndex = 0; widgetIndex < ADVMGR_BOTTOM_VIEW_WIDGET_CAPACITY; ++widgetIndex) {
...
i32 advManager::UpdBottomViewEnemyTurn(void) {
    ...
    if (iCurBottomView != ADVMGR_ENEMY_TURN_VIEW_ID) {
        ...
        m_bottomViewBackground = new iconWidget(
            ADVMGR_ENEMY_TURN_BACKGROUND_X,
            ADVMGR_ENEMY_TURN_BACKGROUND_Y,
            ADVMGR_ENEMY_TURN_BACKGROUND_WIDTH,
            ADVMGR_ENEMY_TURN_BACKGROUND_HEIGHT,
            "stonback.icn",
            0,
            0,
            ADVMGR_ENEMY_TURN_BACKGROUND_ID,
            ADVMGR_ENEMY_TURN_WIDGET_FLAGS,
            1
        );
        if (m_bottomViewBackground == 0) {
            MemError();
        }
        m_adventureWindow->AddWidget(m_bottomViewBackground, ADVMGR_ENEMY_TURN_BACKGROUND_Z);

        m_bottomViewHourglassBackground = new iconWidget(
            ADVMGR_ENEMY_TURN_HOURGLASS_X,
            ADVMGR_ENEMY_TURN_HOURGLASS_Y,
            ADVMGR_ENEMY_TURN_HOURGLASS_WIDTH,
            ADVMGR_ENEMY_TURN_HOURGLASS_HEIGHT,
            "hourglas.icn",
            0,
            0,
            ADVMGR_ENEMY_TURN_HOURGLASS_ID,
            ADVMGR_ENEMY_TURN_WIDGET_FLAGS,
            1
        );
        ...
        m_adventureWindow->AddWidget(
            ...
            ADVMGR_ENEMY_TURN_HOURGLASS_Z
        );
    }

    if (gbForceUpdate || KBTickCount() - iLastSandAnimTime > ADVMGR_ENEMY_TURN_ANIMATION_DELAY) {
        iLastSandAnimTime = KBTickCount();
        iLastAnimFrame = m_updateMaxX;
        if (KBTickCount() - iLastNewSandAnimTime > ADVMGR_ENEMY_TURN_ANIMATION_DELAY) {
            ...
            if (m_bottomViewIcons[1] != 0) {
                ...
                m_bottomViewIcons[1] = new iconWidget(
                    ADVMGR_ENEMY_TURN_ANIMATION_X,
                    ADVMGR_ENEMY_TURN_ANIMATION_Y,
                    ADVMGR_ENEMY_TURN_ANIMATION_WIDTH,
                    ADVMGR_ENEMY_TURN_ANIMATION_HEIGHT,
                    "hourglas.icn",
                    iSandAnim + ADVMGR_ENEMY_TURN_SAND_FRAME_OFFSET,
                    0,
                    ADVMGR_ENEMY_TURN_SAND_ID,
                    ADVMGR_ENEMY_TURN_WIDGET_FLAGS,
                    1
                );
                if (m_bottomViewIcons[1] == 0) {
                ...
                m_adventureWindow->AddWidget(m_bottomViewIcons[1], ADVMGR_ENEMY_TURN_SAND_Z);
            }
        }
    }
    ...
    if (gbForceUpdate || iCurBottomViewEnemy != giCurPlayer) {
        ...
        if (m_bottomViewIcons[0] != 0) {
            ...
            m_adventureWindow->AddWidget(m_bottomViewIcons[0], ADVMGR_ENEMY_TURN_CREST_Z);
        }
    }

    if (gbForceUpdate || iLastHourGlassPhase > iCurHourGlassPhase || iLastHourGlassPhase < 0
        || (iCurHourGlassPhase > iLastHourGlassPhase
            && KBTickCount() - giLastHourGlassUpdateTime >= ADVMGR_ENEMY_TURN_PHASE_DELAY)) {
        updated = 1;
        ...
        if (m_bottomViewIcons[2] != 0) {
            ...
            m_adventureWindow->AddWidget(m_bottomViewIcons[2], ADVMGR_ENEMY_TURN_PHASE_Z);
        }
    }
    return updated;
}
...
i32 advManager::UpdBottomViewNewTurn(void) {
    ...
    m_bottomViewBackground = new iconWidget(
        ADVMGR_BOTTOM_VIEW_PANEL_X,
        ADVMGR_BOTTOM_VIEW_PANEL_Y,
        ADVMGR_BOTTOM_VIEW_BACKGROUND_WIDTH,
        ADVMGR_BOTTOM_VIEW_PANEL_HEIGHT,
        "stonback.icn",
        0,
        0,
        ADVMGR_BOTTOM_VIEW_BACKGROUND_ID,
        ADVMGR_BOTTOM_VIEW_WIDGET_FLAGS,
        1
    );
    if (m_bottomViewBackground == 0) {
        MemError();
    ...
    m_bottomViewHourglassBackground = new iconWidget(
        ADVMGR_NEW_TURN_DATE_ICON_X,
        ADVMGR_NEW_TURN_DATE_ICON_Y,
        ADVMGR_NEW_TURN_DATE_ICON_WIDTH,
        ADVMGR_NEW_TURN_DATE_ICON_HEIGHT,
        "sunmoon.icn",
        dateIconFrame,
        0,
    ...
    m_bottomViewAllTexts[0] = new textWidget(
        ...
        ADVMGR_NEW_TURN_WEEK_TEXT_Y,
        ADVMGR_NEW_TURN_DATE_TEXT_WIDTH,
        ADVMGR_NEW_TURN_WEEK_TEXT_HEIGHT,
        weekText,
        "smalfont.fnt",
        1,
        ADVMGR_BOTTOM_VIEW_TEXT_ID,
        ADVMGR_BOTTOM_VIEW_TEXT_FLAGS,
        1
    );
    ...
    m_bottomViewAllTexts[0] = new textWidget(
        ADVMGR_NEW_TURN_DATE_TEXT_X,
        ADVMGR_NEW_TURN_DAY_TEXT_Y,
        ADVMGR_NEW_TURN_DATE_TEXT_WIDTH,
        ADVMGR_NEW_TURN_DAY_TEXT_HEIGHT,
        dayText,
        "bigfont.fnt",
        1,
        ADVMGR_BOTTOM_VIEW_TEXT_ID,
...
i32 advManager::UpdBottomViewResMsg(void) {
    ...
    m_bottomViewAllTexts[0] = new textWidget(
        ...
        ADVMGR_BOTTOM_VIEW_PANEL_WIDTH,
        ADVMGR_RESOURCE_VIEW_TEXT_HEIGHT,
        messageText2,
        "smalfont.fnt",
        1,
        ADVMGR_BOTTOM_VIEW_TEXT_ID,
    ...
    if (giBottomViewResource >= 0) {
        ...
        m_bottomViewHourglassBackground = new iconWidget(
            (ADVMGR_BOTTOM_VIEW_PANEL_WIDTH - iconWidth6) / 2 + ADVMGR_BOTTOM_VIEW_PANEL_X,
        ...
        m_bottomViewAllTexts[1] = new textWidget(
            ...
            ADVMGR_RESOURCE_VIEW_COUNT_Y,
            ADVMGR_RESOURCE_VIEW_COUNT_WIDTH,
            ADVMGR_RESOURCE_VIEW_COUNT_HEIGHT,
            resourceCountText6,
            "smalfont.fnt",
            1,
            ADVMGR_BOTTOM_VIEW_TEXT_ID_2,
            ADVMGR_BOTTOM_VIEW_TEXT_FLAGS,
            1
        );
...
i32 advManager::UpdBottomViewKingdom(void) {
    ...
    m_bottomViewHourglassBackground = new iconWidget(
        ADVMGR_KINGDOM_VIEW_ICON_X,
        ADVMGR_KINGDOM_VIEW_ICON_Y,
        ADVMGR_BOTTOM_VIEW_PANEL_WIDTH,
        ADVMGR_BOTTOM_VIEW_PANEL_HEIGHT,
        "ressmall.icn",
        0,
    ...
    for (index11 = 0; index11 < ADVMGR_KINGDOM_VIEW_ENTRY_COUNT; ++index11) {
        ...
        m_bottomViewAllTexts[index11] = new textWidget(
            ...
            ADVMGR_KINGDOM_VIEW_TEXT_HEIGHT,
            countText14[index11],
            "smalfont.fnt",
            1,
            index11 + ADVMGR_BOTTOM_VIEW_TEXT_ID,
            ADVMGR_BOTTOM_VIEW_TEXT_FLAGS,
            1
        );
...
i32 advManager::UpdBottomViewHero(void) {
    ...
    m_bottomViewBackground = new iconWidget(
        ADVMGR_BOTTOM_HERO_PANEL_X,
        ADVMGR_BOTTOM_HERO_PANEL_Y,
        ADVMGR_BOTTOM_HERO_PANEL_WIDTH,
        ADVMGR_BOTTOM_HERO_PANEL_HEIGHT,
        "stonback.icn",
        0,
        0,
        ADVMGR_BOTTOM_VIEW_FIRST_MESSAGE,
        ADVMGR_BOTTOM_HERO_WIDGET_FLAGS,
        1
    );
    if (m_bottomViewBackground == 0) {
        MemError();
    ...
    if (occupiedSlotsLocal != 0) {
        ...
        for (armySlot = 0; armySlot < ADVMGR_BOTTOM_HERO_ARMY_SLOTS; ++armySlot) {
            ...
            if (creature != ADVMGR_BOTTOM_HERO_EMPTY_SLOT) {
                ...
                i8 armyLayouts[ADVMGR_BOTTOM_HERO_ARMY_SLOTS][ADVMGR_BOTTOM_HERO_ARMY_SLOTS] = {
                    {3, -1, -1, -1, -1},
                    {2, 4, -1, -1, -1},
                    {0, 1, 6, -1, -1},
                    {0, 1, 5, 6, -1},
                ...
                m_bottomViewIcons[displayIndexData] = new iconWidget(
                    ...
                    ADVMGR_BOTTOM_HERO_ICON_WIDTH,
                    ADVMGR_BOTTOM_HERO_ICON_HEIGHT,
                    "mons32.icn",
                    creature,
                    0,
                ...
                m_bottomViewTexts[displayIndexData] = new textWidget(
                    ...
                    ADVMGR_BOTTOM_HERO_LABEL_HEIGHT,
                    armyCountLabelsResult[displayIndexData],
                    "smalfont.fnt",
                    1,
                    displayIndexData + ADVMGR_BOTTOM_HERO_FIRST_TEXT_ID,
                    ADVMGR_BOTTOM_HERO_TEXT_ALIGNMENT,
                    1
                );
...
void advManager::RedrawAdvScreen(i32 update, i32 freeBorder) {
    ...
    if (update) {
        gpWindowManager->UpdateScreenRegion(0, 0, ADVMGR_SCREEN_WIDTH, ADVMGR_SCREEN_HEIGHT);
    }
...
void advManager::SetTownContext(i32 townId) {
    ...
    } else if (gpCurPlayer->m_townLocatorPage + ADVMGR_LOCATOR_VISIBLE_COUNT - 1 < selectedIndex7) {
        gpCurPlayer->m_townLocatorPage =
            static_cast<i8>(selectedIndex7 - (ADVMGR_LOCATOR_VISIBLE_COUNT - 1));
    }
    ...
    SetEnvironmentOrigin(
        ...
        m_mapOriginY + ADVMGR_VIEW_CENTER_OFFSET,
        1
    );
...
void advManager::SetHeroContext(i32 heroId, i32 update) {
    ...
    } else if (gpCurPlayer->m_heroLocatorPage + ADVMGR_LOCATOR_VISIBLE_COUNT - 1 < selectedIndex7) {
        gpCurPlayer->m_heroLocatorPage =
            static_cast<i8>(selectedIndex7 - (ADVMGR_LOCATOR_VISIBLE_COUNT - 1));
    }
        ...
        if (message.type == MESSAGE_MOUSE_MOVE) {
            ...
            if (locatorCount29 > ADVMGR_LOCATOR_VISIBLE_COUNT) {
                newPageState = static_cast<i32>(
                    (m_scrollLeftButton->m_y - ADVMGR_LOCATOR_SCROLL_BASE_Y) / pageHeight7
                );
                if (newPageState != previousPageSlot) {
                    gpCurPlayer->m_heroLocatorPage = static_cast<i8>(newPageState);
                    if (newPageState > locatorCount29 - (ADVMGR_LOCATOR_VISIBLE_COUNT - 1)) {
                        newPageState = locatorCount29 - (ADVMGR_LOCATOR_VISIBLE_COUNT - 1);
                    }
...
void advManager::DoTownKnob(void) {
    ...
    while (message.type != MESSAGE_LEFT_BUTTON_UP && message.type != MESSAGE_RIGHT_BUTTON_UP) {
        if (message.type == MESSAGE_MOUSE_MOVE) {
            ...
            if (locatorCount29 > ADVMGR_LOCATOR_VISIBLE_COUNT) {
                newPageState = static_cast<i32>(
                    (m_scrollRightButton->m_y - ADVMGR_LOCATOR_SCROLL_BASE_Y) / pageHeight7
                );
                if (newPageState != previousPageSlot) {
    ...
    switch (spell) {
        ...
        setMineGuardian:
            ...
            gpGame->m_mines[currentCell->m_objectMetadata].guardianType =
                static_cast<i8>(guardianTypes1[ADVMGR_MINE_GUARDIAN_TYPE_INDEX]);
            ...
            gpGame->m_mines[currentCell->m_objectMetadata].guardianCount =
                static_cast<u8>(spellPowerValue * ADVMGR_MINE_GUARDIANS_PER_POWER);
...
i32 SaveGame(void) {
    ...
    fileRequester* requester2 = new fileRequester(
        ADVMGR_SAVE_REQUESTER_X,
        ADVMGR_SAVE_REQUESTER_Y,
...
i32 advManager::ComboDraw(i32 originX, i32 originY, i32 animate) {
    ...
    if (m_heroContextLocked != 0) {
        for (mapRow = ADVMGR_COMBO_HERO_PANEL_TOP; mapRow <= ADVMGR_COMBO_HERO_PANEL_BOTTOM - 1;
             ++mapRow) {
            for (column = ADVMGR_COMBO_HERO_PANEL_LEFT; column <= ADVMGR_COMBO_HERO_PANEL_RIGHT - 1;
                 ++column) {
                ++bComboDraw[column][mapRow];
            }
        }
...
void advManager::SetEnvironmentOrigin(i32 originX, i32 originY, i32 stopSounds) {
    ...
    for (edgeOffset = 0; edgeOffset < ADVMGR_SOUND_CELL_COUNT; ++edgeOffset) {
        if (m_activeSounds[edgeOffset].soundId != ADVMGR_ENVIRONMENT_SOUND_NONE) {
            ...
            if (stopSounds != 0) {
                ...
                m_activeSounds[edgeOffset].volume = ADVMGR_ENVIRONMENT_SOUND_DEFAULT_VOLUME;
            } else {
                m_activeSounds[edgeOffset].volume = ADVMGR_ENVIRONMENT_SOUND_DEFAULT_VOLUME;
            }
        }
    ...
    if (gConfig.soundVolume != 0) {
        ...
        for (soundLayer = ADVMGR_ENVIRONMENT_SOUND_FIRST_LAYER;
             soundLayer <= ADVMGR_ENVIRONMENT_SOUND_LAYER_COUNT;
             ++soundLayer) {
            InsertSound(originX, originY, 0, soundLayer);
            for (soundRadius = 0; soundRadius < ADVMGR_SOUND_CELL_COUNT; ++soundRadius) {
...
void advManager::TeleportTo(
    ...
    if (skipMapChange == 0) {
        SendMapChange(
            ADVMGR_TELEPORT_MAP_CHANGE,
            mapHero->m_id,
            static_cast<u8>(destinationX),
            static_cast<u8>(destinationY),
            ADVMGR_TELEPORT_MAP_CHANGE_VALUE,
            0,
            0
        );
    }
    ...
    if (bShowIt != 0) {
        ...
        if (oldCellFlag26 != 0) {
            ...
            destinationCell29->m_triggerType = (MAP_TRIGGER_ACTION_FLAG | MAP_OBJECT_HERO_INTERACTION);
...
void advManager::TownGate(i32 spellId) {
    ...
    if (spellId == SPELL_TOWN_PORTAL) {
        ...
        if (gpWindowManager->m_dialogResult == ADVMGR_TOWN_PORTAL_FIRST_CHOICE) {
            return;
        }
    } else {
        for (townListIndex = 0; townListIndex < gpCurPlayer->m_townCount; ++townListIndex) {
        ...
        for (pathIndex = gpSearchArray->m_pathLength - 1; pathIndex >= 0; --pathIndex) {
            ...
            if (pathIndex == 0) {
                ...
                m_visibilityMap[routeY1 * (MAP_WIDTH | 0) + routeX1] = static_cast<u16>(
                    ...
                    + routeFrame * ADVMGR_ROUTE_ARROW_FRAME_STRIDE + ADVMGR_ROUTE_ARROW_FRAME_OFFSET
                );
            }

            if (remainingMobility2 < 0) {
                m_visibilityMap[routeY1 * (MAP_WIDTH | 0) + routeX1] += ADVMGR_ROUTE_DAY_MASK;
            } else {
                routeReachable8 = 1;
            }
        }
...
void advManager::HideRoute(i32 redraw, i32 clearDestination, i32 updateButton) {
    ...
    if (clearDestination && gpCurPlayer->m_currentHero != ADVMGR_INVALID_HERO) {
...
void advManager::CheckDimHero(void) {
    if (!gbThisNetHumanPlayer[giCurPlayer] || gpCurPlayer->CurrentHero() == ADVMGR_INVALID_HERO) {
        return;
    }
...
void advManager::CheckScreenScroll(void) {
    ...
    if (KBTickCount() - iLastScrollTime > ADVMGR_SCROLL_TICK_INTERVAL) {
        ...
        if (mouseX6 >= 0 && mouseX6 < ADVMGR_SCREEN_WIDTH && mouseY1 >= 0
...
void advManager::SetInitialMapOrigin(void) {
    ...
    m_currentTerrain = giGroundToTerrain
        [GetCell(m_mapOriginX + ADVMGR_VIEW_CENTER_OFFSET, m_mapOriginY + ADVMGR_VIEW_CENTER_OFFSET)
             ->m_terrainImageIndex];
    gpSoundManager->SwitchAmbientMusic(giTerrainToMusicTrack[m_currentTerrain]);
    SetEnvironmentOrigin(
        m_mapOriginX + ADVMGR_VIEW_CENTER_OFFSET,
...
void advManager::TrimLoopingSounds(i32 maxSamples) {
    ...
    if (loadedSampleCount < maxSamples) {
        for (soundIndex = 0; soundIndex < ADVMGR_LOOPING_SAMPLE_COUNT; ++soundIndex) {
            if (retainedSamples[soundIndex] == 0 && m_loopingSamples[soundIndex] != 0) {
    ...
    for (soundIndex = 0; soundIndex < ADVMGR_LOOPING_SAMPLE_COUNT; ++soundIndex) {
        if (m_loopingSamples[soundIndex] != 0 && retainedSamples[soundIndex] == 0) {
...
void advManager::DisableButtons(void) {
    ...
    msg.payload.widget.id = ADVMGR_BUTTON_SLOT_1;
    m_adventureWindow->BroadcastMessage(msg);
    msg.payload.widget.id = ADVMGR_BUTTON_SLOT_2;
    m_adventureWindow->BroadcastMessage(msg);
    msg.payload.widget.id = ADVMGR_BUTTON_SLOT_3;
    m_adventureWindow->BroadcastMessage(msg);
    msg.payload.widget.id = ADVMGR_BUTTON_SLOT_4;
    m_adventureWindow->BroadcastMessage(msg);
    msg.payload.widget.id = ADVMGR_BUTTON_SLOT_5;
    m_adventureWindow->BroadcastMessage(msg);
    msg.payload.widget.id = ADVMGR_BUTTON_SLOT_6;
    m_adventureWindow->BroadcastMessage(msg);
}
...
void advManager::SaveAdventureBorder(void) {
    ...
    for (row = ADVMGR_BORDER_EDGE_SIZE; row < ADVMGR_BORDER_MIDDLE_END; ++row) {
        ...
        for (s_adjacentMonsterX = originX - ADVMGR_ADJACENT_MONSTER_RADIUS;
            ...
            for (s_adjacentMonsterY = originY - ADVMGR_ADJACENT_MONSTER_RADIUS;
                ...
                if (m_mapData->Row(s_adjacentMonsterY)[s_adjacentMonsterX].m_triggerType
                    == ADVMGR_ADJACENT_MONSTER_TRIGGER) {
                    if (originY > s_adjacentMonsterY) {
                        if ((GetCell(originX, originY)->m_objectIndex
                             ...
                             || (GetCell(originX, originY)->m_flags & ADVMGR_HOVER_UNREACHABLE))
...
void advManager::ViewPuzzle(void) {
    ...
    u8 puzzleOrderLocal[ADVMGR_PUZZLE_PIECE_COUNT] = {23, 7,  44, 5,  24, 47, 1,  39, 16, 36,
    ...
    heroWindow* puzzleWindowObject =
        new heroWindow(ADVMGR_PUZZLE_WINDOW_X, ADVMGR_PUZZLE_WINDOW_Y, "viewpuzl.bin");
    if (puzzleWindowObject == 0) {
        MemError();
    }
    gpWindowManager->AddWindow(puzzleWindowObject, -1, 1);

    i32 puzzleXViewCurrent = gpGame->m_ultimateArtifactX - ADVMGR_PUZZLE_COORDINATE_OFFSET;
    i32 puzzleYTop = gpGame->m_ultimateArtifactY - ADVMGR_PUZZLE_COORDINATE_OFFSET;
    ...
    u8* pixelIterator;
    u8* rowLimitAddress;
    for (rowCounterNumber = ADVMGR_PUZZLE_VIEW_ORIGIN; rowCounterNumber < ADVMGR_PUZZLE_VIEW_END;
         ++rowCounterNumber) {
        pixelIterator = gpWindowManager->m_screen->m_pixels + rowCounterNumber * ADVMGR_SCREEN_WIDTH
                        + ADVMGR_PUZZLE_VIEW_ORIGIN;
...
void advManager::PuzzleDraw(i32 left, i32 top, i32 right, i32 bottom) {
    ...
    IconToBitmap(
        ...
        (bottom - top) * ADVMGR_PUZZLE_TILE_SIZE,
        0,
        1,
        0,
        0,
...
void advManager::AdvPanel(void) {
    heroWindow* adventurePanel;
    {
        ...
        adventurePanel = new heroWindow(ADVMGR_PANEL_WINDOW_X, ADVMGR_PANEL_WINDOW_Y, "apanel.bin");
        if (adventurePanel == 0) {
            MemError();
        }
        if (gpCurPlayer->CurrentHero() == ADVMGR_INVALID_HERO) {
...
i32 advManager::ControlPanel(void) {
    ...
    heroWindow* controlPanel =
        new heroWindow(ADVMGR_PANEL_WINDOW_X, ADVMGR_PANEL_WINDOW_Y, "cpanel.bin");
    if (controlPanel == 0) {
        MemError();
    }
    if (gbRemoteOn) {
...
i32 advManager::DoVisions(hero* visionHero) {
    ...
    for (scanXType = visionHero->m_x - ADVMGR_VISIONS_RADIUS;
         scanXType <= visionHero->m_x + ADVMGR_VISIONS_RADIUS;
         ++scanXType) {
        for (scanYLocal = visionHero->m_y - ADVMGR_VISIONS_RADIUS;
             scanYLocal <= visionHero->m_y + ADVMGR_VISIONS_RADIUS;
             ++scanYLocal) {
