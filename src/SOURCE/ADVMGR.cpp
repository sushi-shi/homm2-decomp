...
DATA(0x00523e1c) static b32 s_drawFlipCloud = false;
...
advManager::advManager(void) {
    ...
    for (index = 0; index < IDX(ADVMGR_ENVIRONMENT_SOUND_COUNT); ++index) {
...
i32 advManager::Open(i32 id) {
    ...
    for (i = 0; i < IDX(ADVMGR_ENVIRONMENT_SOUND_COUNT); ++i) {
...
#define commandValue c
#define currentHero hero2
#define unusedMouseX mx
#define unusedMouseY my
#endif
VA(0x004027cf, 0xe03)
MessageDispatchResult advManager::Main(struct tag_message& message) {
    ...
    i32 quit;
    i32 commandValue;
    i32 H2_UNUSED(unusedMouseX);
    i32 H2_UNUSED(unusedMouseY);
    i32 town;
    mapCell* evtCell;
            ...
            case MESSAGE_KEY_DOWN:
                ...
                switch (message.payload.keyboard.keyCode) {
                    ...
                    case INPUT_SCAN_F8:
                        break;
                    case INPUT_SCAN_F9:
                        for (commandValue = 0; commandValue < IDX(RES_COUNT); ++commandValue) {
                            gpCurPlayer->m_resources[commandValue] +=
                                (commandValue == IDX(RES_GOLD) ? CHEAT_GOLD_AMOUNT : CHEAT_RESOURCE_AMOUNT);
                        }
                        break;
                    case INPUT_SCAN_0:
                        commandValue = 0;
                    ...
                    case INPUT_SCAN_NUMPAD_8:
                        if (HAS(message.payload.keyboard.modifiers,
            ...
            if (HAS(message->payload.widget.modifiers, MESSAGE_MODIFIER_RIGHT_BUTTON)) {
                if (!visible) {
                    ...
                    if (m_lastHoverCell == VIEW_CENTER_CELL && m_hoverCellY == VIEW_CENTER_CELL
                        ...
                        objectTypeState = IDX(theCell->m_triggerType & MAP_TRIGGER_TYPE_MASK);
            ...
            if (gbHumanPlayer[giCurPlayer]) {
                ...
                if (special) {
                    sprintf(
                        gText,
                        "%s%s",
                        localization::Tr("adventure.search.found_artifact.prefix"),
    ...
    if (InMapArea(mouseX, mouseY)) {
        ...
        if (m_lastHoverCell != mouseCellX || m_hoverCellY != mouseCellY) {
            ...
            if (gpCurPlayer->m_currentHero == INVALID_HERO) {
                ...
                if (hoverCell->m_flags & IDX(MAP_CELL_OCCUPIED)) {
                    if ((hoverCell->m_triggerType & MAP_TRIGGER_TYPE_MASK)
                        == MAP_OBJECT_CASTLE) {
                ...
                if (HAS(drawMask, ADVMGR_DRAW_HERO)) {
                    if (mapX > 0) {
                        ...
                        if (s_drawAdjacentCell->m_triggerType
                            ...
                            if (s_drawMine->guardianType == CREATURE_GHOST) {
                                ...
                                DRAW_ADVENTURE_ICON(
                                    m_objectIcons[IDX(TILESET_OBJNXTRA)],
                    ...
                    if (s_drawCell->m_objectTileset == TILESET_MONS32) {
                        if (mapX == m_lastQuickViewX && mapY == m_lastQuickViewY) {
                            DRAW_ADVENTURE_ICON(
                                m_objectIcons[IDX(TILESET_MINIMON)],
                                s_drawPixelX + CELL_CENTER_PIXEL,
            ...
            if (HAS(drawMask, ADVMGR_DRAW_OVERLAY) || HAS(drawMask, ADVMGR_DRAW_OVERLAY_TOP)) {
                if (HAS(drawMask, ADVMGR_DRAW_OVERLAY)
                    ...
                    if (s_drawCell->m_objectDrawnAsOverlay
                        && (gbDrawingPuzzle == 0
                            || bPuzzleDraw[IDX(s_drawCell->m_objectTileset)])) {
                        DRAW_ADVENTURE_ICON(
                            m_objectIcons[IDX(s_drawCell->m_objectTileset)],
                            s_drawPixelX,
                            s_drawPixelY,
                        ...
                        if (s_drawCell->m_animatedObject) {
                            s_drawAnimationLength = GetIconEntry(
                                                        m_objectIcons
                                                            [IDX(s_drawCell->m_objectTileset)],
                                                        s_drawCell->m_objectIndex
                            )
                                                        ->flags;
                            DRAW_ADVENTURE_ICON(
                    ...
                    while (s_drawExtra != NULL) {
                        if (s_drawExtra->objectDrawnAsOverlay
                            && (gbDrawingPuzzle == 0
                                || bPuzzleDraw[IDX(s_drawExtra->objectTileset)])) {
                            DRAW_ADVENTURE_ICON(
                                m_objectIcons[IDX(s_drawExtra->objectTileset)],
                                s_drawPixelX,
                                s_drawPixelY,
...
#define minX minx
#define minY miny
#define pixelPointer pixPtr
#define unusedWidth w
#define unusedX cx
#endif
VA(0x004084ac, 0xeb7)
void advManager::UpdateRadar(i32 updateScreen, i32 partial) {
    ...
    i32 maxY;
    mapCell* cell;
    H2_ENUM_STORAGE(TilesetId, i32) setId;
    i32 owner;
            ...
            switch (currentCell->m_triggerType & MAP_TRIGGER_TYPE_MASK) {
                ...
                case MAP_OBJECT_MOSSY_ROCK:
                    ...
                    sprintf(
                        ...
                        blocked ? localization::Tr("adventure.quick_info.cannot_dig")
                ...
                case MAP_OBJECT_MINE:
                    ...
                    quick_info_guarded:
                        sprintf(
                            ...
                            gArmyNamesPlural[IDX(gpGame->m_mines[currentCell->m_objectMetadata]
                                                     .guardianType)]
                        );
...
void advManager::UpdateHeroLocator(i32 locatorSlot, i32 drawWindow, i32 updateScreen) {
    ...
    if (whichHero == INVALID_HERO || gbAllBlack) {
        ...
        message.payload.widget.command = WIDGET_COMMAND_SET_FRAME;
        message.payload.widget.data.value = IDX(heroPointer->m_portrait);
        m_adventureWindow->BroadcastMessage(message);
    }
...
#if H2_RETAIL_COMPILER
#define layoutPosition layoutPos
#endif
VA(0x0040bd33, 0x5ef)
...
#if H2_RETAIL_COMPILER
#undef layoutPosition
#endif

#if H2_RETAIL_COMPILER
#define currentIndex idx
#define index ii
#define message msg
    ...
    if (targetHero->m_owner == giCurPlayer || m_identifyHeroActive == 1
        ...
        if (creatureCount != 0) {
            ...
            for (index = 0; index < creatureCount; ++index) {
                ...
                monster = IDX(targetHero->m_army.m_creatureTypes[currentIndex]);
...
#if H2_RETAIL_COMPILER
#define basePosition basePos
#define rowCreatureCount row2
#define townPointer townPtr
#define unusedEnabled on
#endif
VA(0x0040d1f9, 0xcd1)
void advManager::TownQuickView(
    ...
    message.payload.widget.data.value =
        IDX(townPointer->m_type) + TOWN_QUICK_TYPE_FRAME_BASE;
    ...
    if (townPointer->m_owner == -1) {
        ...
        message.payload.widget.id = TOWN_QUICK_PLAYER_COLOR_WIDGET;
        message.payload.widget.data.value =
            gpGame->m_players[IDX(townPointer->m_owner)].m_color * HERO_QUICK_PLAYER_COLOR_STRIDE;
        window->BroadcastMessage(message);
        ++message.payload.widget.id;
        ++message.payload.widget.data.value;
        window->BroadcastMessage(message);
    ...
    if (scouting == TOWN_QUICK_INFORMATION_UNKNOWN || creatureCount == 0) {
        ...
        for (armyIndex = 0; armyIndex < topRow; ++armyIndex) {
            ...
            monster = IDX(townPointer->m_army.m_creatureTypes[creatureSlot]);
...
void advManager::DemobilizeCurrHero(void) {
    ...
    if (m_cursorType == HERO_TYPE_BOAT) {
        heroPointer->m_eventFlags =
            HeroEventFlag(static_cast<i32>(heroPointer->m_eventFlags) | IDX(HERO_EVENT_EMBARKED));
    }
...
void advManager::SetTownContext(i32 townId) {
    ...
    townNo = IDX(CELL_TERRAIN(GetCell(townPointer->m_x, townPointer->m_y)));
...
#if H2_RETAIL_COMPILER
#define currentSuccess ok
#define requester req
#endif
VA(0x0040edaf, 0x24b)
i32 SaveGame(void) {
...
void advManager::InsertSound(i32 x, i32 mapY, i32 distance, i32 soundLayer) {
    ...
    if (soundSlot != -1) {
        if (m_activeSounds[soundSlot].soundId != ADVMGR_ENVIRONMENT_SOUND_NONE) {
            gpSoundManager->StopSample(m_loopingSamples[IDX(m_activeSounds[soundSlot].soundId)]);
        }
...
void advManager::SummonBoat(void) {
    ...
    for (iDirection = 0; iDirection < IDX(MAP_DIRECTION_COUNT); ++iDirection) {
...
void advManager::LoadRemote(void) {
    ...
    if (giDebugLevel > 0) {
        DATA(0x00524000) static b32 debugWarned = false;
        if (!debugWarned) {
            debugWarned = true;
            sprintf(
                gText,
...
void advManager::TrimLoopingSounds(i32 maxSamples) {
    ...
    if (maxSamples >= IDX(ADVMGR_ENVIRONMENT_SOUND_COUNT)) {
        return;
    }

    i32 loaded = 0;
    char keep[IDX(ADVMGR_ENVIRONMENT_SOUND_COUNT)];
            ...
            switch (message.payload.widget.command) {
                ...
                case WIDGET_NOTIFY_SELECT: {
                    switch (
                        ...
                        case IDX(SYSTEM_OPTION_SOUND_VOLUME):
                            if (gConfig.soundVolume == CONFIG_VOLUME_MUTED
                                // VC6 lowers this explicit Boolean materialization
                                // differently from the bare short-circuit expression.
