        ...
        if (m_lastHoverCell != cellXPosition || m_hoverCellY != cellYCurrent) {
            ...
            if (gpCurPlayer->m_currentHero == ADVMGR_INVALID_HERO) {
                if ((hoverCellLocal->m_triggerType & MAP_TRIGGER_TYPE_MASK) == ADVMGR_HOVER_TOWN
                    ...
                    if ((hoverCellLocal->m_triggerType & MAP_TRIGGER_TYPE_MASK) == ADVMGR_HOVER_HERO
    ...
    if (gbAllBlack == 0 && (mapX < 0 || mapY < 0 || mapX >= MAP_WIDTH || mapY >= MAP_HEIGHT)) {
        ...
        if (HAS(drawMask, ADVMGR_DRAW_CLOUD) && !gbDrawingPuzzle) {
            if (s_drawCovered != 0) {
                ...
                if ((m_visibilityMap[mapY * MAP_WIDTH + mapX] & ADVMGR_ROUTE_BEYOND_MOBILITY_FLAG)
                    != 0) {
                            ...
                            if (s_drawPlayerColor != -1) {
                                if (s_drawHeroType == ADVMGR_HERO_TYPE_BOAT) {
                                    ...
                                    FlipIconToBitmap(
                                        ...
                                        s_drawPixelY + 31,
                                        m_updateMaxY % 8
                                            + (s_drawHeroFrame & ADVMGR_HERO_FRAME_INDEX_MASK) + 56,
                                        1,
                                        0,
                                        0,
                                        ADVMGR_DRAW_CLIP_WIDTH,
