...
i32 swapManager::Open(i32 id) {
    ...
    for (SwapManagerSide side_6 = SWAP_SIDE_LEFT; side_6 < SWAP_SIDE_COUNT; ++side_6) {
        for (i32 skillSlot = 0; skillSlot < SECONDARY_SKILL_WIDGET_COUNT; ++skillSlot) {
            if (skillSlot < m_heroes[H2EnumIndex(side_6)]->m_secondarySkillCount) {
                message.payload.widget.command = WIDGET_COMMAND_SET_FRAME;
                message.payload.widget.id = H2EnumIndex(side_6) * SECONDARY_SKILL_WIDGET_COUNT + skillSlot
                                            + CONTROL_LEFT_SKILL_FIRST;
                message.payload.widget.data.value = H2EnumIndex(m_heroes[H2EnumIndex(side_6)]->GetNthSS(skillSlot));
                m_window->BroadcastMessage(message);
                ...
                sprintf(
                    gText,
                    "%d",
                    m_heroes[H2EnumIndex(side_6)]->GetSSLevel(m_heroes[H2EnumIndex(side_6)]->GetNthSS(skillSlot))
                );
                m_window->BroadcastMessage(message);
            } else {
...
MessageDispatchResult swapManager::Main(tag_message& message) {
    ...
    i32 quickView = ((H2EnumIndex((message.payload.widget.modifiers) & (MESSAGE_MODIFIER_RIGHT_BUTTON)))) != 0;
            ...
            switch (message.payload.widget.command) {
                ...
                case WIDGET_COMMAND_ALTERNATE_SELECT:
                    switch (message.payload.widget.id) {
                        ...
                        showSecondarySkill:
                            if (slotIndex_8 >= m_heroes[H2EnumIndex(side)]->m_secondarySkillCount)
                                break;
                            secondarySkill_1 = m_heroes[H2EnumIndex(side)]->GetNthSS(slotIndex_8);
                            m_heroes[H2EnumIndex(side)]->DoSSLevelDialog(secondarySkill_1, quickView);
                            break;
                        ...
                        case CONTROL_LEFT_ARTIFACT_LAST:
                            ...
                            if (quickView) {
                                if (m_heroes[H2EnumIndex(SWAP_SIDE_LEFT)]->m_artifacts[artifactSlot_2]
                                    == ARTIFACT_NONE)
                                    break;
                                m_heroes[H2EnumIndex(SWAP_SIDE_LEFT)]->ViewArtifact(
                                    m_heroes[H2EnumIndex(SWAP_SIDE_LEFT)]->m_artifacts[artifactSlot_2],
                                    VIEW_QUICK,
                                    m_heroes[H2EnumIndex(SWAP_SIDE_LEFT)]->m_artifactExtra[artifactSlot_2]
                                );
                                break;
                            }
                        ...
                        case CONTROL_RIGHT_ARTIFACT_LAST:
                            ...
                            if (quickView) {
                                if (m_heroes[H2EnumIndex(SWAP_SIDE_RIGHT)]->m_artifacts[artifactSlot_2]
                                    == ARTIFACT_NONE)
                                    break;
                                m_heroes[H2EnumIndex(SWAP_SIDE_RIGHT)]->ViewArtifact(
                                    m_heroes[H2EnumIndex(SWAP_SIDE_RIGHT)]->m_artifacts[artifactSlot_2],
                                    VIEW_QUICK,
                                    m_heroes[H2EnumIndex(SWAP_SIDE_RIGHT)]->m_artifactExtra[artifactSlot_2]
                                );
                                break;
                            }
                            if (m_itemType != SWAP_ITEM_ARTIFACT) {
...
void swapManager::SwapArtifacts(void) {
    ...
    if (selectedArtifact == ARTIFACT_SPADE_NECROMANCY
        ...
        for (SwapManagerSide side = SWAP_SIDE_LEFT; side < SWAP_SIDE_COUNT; ++side) {
            for (i32 skillSlot = 0; skillSlot < SECONDARY_SKILL_WIDGET_COUNT; ++skillSlot) {
                if (skillSlot < m_heroes[H2EnumIndex(side)]->m_secondarySkillCount) {
                    message_1.payload.widget.command = WIDGET_COMMAND_SET_TEXT;
                    message_1.payload.widget.id = H2EnumIndex(side) * SECONDARY_SKILL_WIDGET_COUNT
                                                  + skillSlot + CONTROL_LEFT_SKILL_LEVEL_FIRST;
                    message_1.payload.widget.data.text = gText;
                    sprintf(
                        gText,
                        "%d",
                        m_heroes[H2EnumIndex(side)]->GetSSLevel(m_heroes[H2EnumIndex(side)]->GetNthSS(skillSlot))
                    );
...
void swapManager::SplitMons(void) {
    ...
    if (m_selectedSide == m_targetSide) {
        ...
        sprintf(
            ...
            gArmyNames[H2EnumIndex(selectedArmy->m_creatureTypes[m_selectedSlot])],
