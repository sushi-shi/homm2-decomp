...
void swapManager::SwapArtifacts(void) {
    ...
    GiveTakeArtifactStat(m_heroes[H2EnumIndex(m_targetSide)], targetArtifact, true);
...
void swapManager::SwapMons(void) {
    ...
    for (i32 slot = 0; slot < ARMY_GROUP_SLOT_COUNT; ++slot) {
        if (ARMY_GROUP_HAS_POSITIVE_STACK(m_heroes[H2EnumIndex(m_selectedSide)]->m_army, slot))
            ++selectedArmyCount;
    }
