...
void townManager::SetupCastle(heroWindow* window, i32 updateOnly) {
    ...
    casWin = window;
    builtFrame = (FRAME_BUILT);
    noBuildFrame = (FRAME_CANNOT_BUILD);
    cannotAffordIcon = (FRAME_CANNOT_AFFORD);
    ...
    for (slotNum = 0; slotNum < CASTLE_SLOT_COUNT; ++slotNum) {
        if (CanBuy(m_town, castleSlotsUse[slotNum]))
            m_affordableBuildings |= 1L << (castleSlotsUse[slotNum]);
        if (CanBuild(m_town, castleSlotsUse[slotNum]))
            m_buildableBuildings |= 1L << (castleSlotsUse[slotNum]);
    }
    ...
    for (slotNum = 0; slotNum < CASTLE_SLOT_COUNT; ++slotNum) {
        ...
        message.payload.widget.data.value = (castleSlotsUse[slotNum]);
    ...
    sprintf(
        icnName,
        "cstl%s.icn",
        cHeroTypeShortName[(m_town->m_type)]
    );
    ...
    for (slotNum = 0; slotNum < CASTLE_SLOT_COUNT; ++slotNum) {
        ...
        if (TOWN_BUILDING_COMPLETE(*m_town, castleSlotsUse[slotNum])) {
            ...
            else if (!(m_affordableBuildings & (1L << (castleSlotsUse[slotNum]))))
        ...
        if (stateFrame == FRAME_BUILT) {
            ...
            message.payload.widget.data.value =
                stateFrame == FRAME_NONE ? (FRAME_AVAILABLE) : FRAME_UNAVAILABLE;
            casWin->BroadcastMessage(message);
        }
    }

    captainQuarters = m_town->m_buildings & (TOWN_BUILDING_CAPTAIN_QUARTERS);
    message.payload.widget.command =
        captainQuarters != 0 ? WIDGET_COMMAND_CLEAR_FLAGS : WIDGET_COMMAND_SET_FLAGS;
    ...
    sprintf(
        gText,
        "CSTLCAP%c.ICN",
        cHeroTypeInitial[(m_town->m_type)]
    );
    message.payload.widget.command = WIDGET_COMMAND_SET_ICON;
    message.payload.widget.id = CONTROL_CAPTAIN_ICON;
    message.payload.widget.data.text = gText;
    casWin->BroadcastMessage(message);
    message.payload.widget.command =
        captainQuarters != 0 ? WIDGET_COMMAND_SET_FLAGS : WIDGET_COMMAND_CLEAR_FLAGS;
    ...
    if (captainQuarters != 0) {
        ...
        for (slotNum = 0; slotNum < HERO_PRIMARY_STAT_COUNT; ++slotNum) {
            sprintf(
                captainStatLine,
                "%d\n",
                captainStats[(m_town->m_type)][slotNum]
            );
            strcat(gText, captainStatLine);
        }
        ...
        if (!CanBuild(m_town, TOWN_OBJECT_CAPTAIN_QUARTERS))
            ...
            stateFrame = FRAME_CANNOT_AFFORD;
        if (CanBuild(m_town, TOWN_OBJECT_CAPTAIN_QUARTERS))
            m_buildableBuildings |= (TOWN_BUILDING_CAPTAIN_QUARTERS);
        if (CanBuy(m_town, TOWN_OBJECT_CAPTAIN_QUARTERS))
            m_affordableBuildings |= (TOWN_BUILDING_CAPTAIN_QUARTERS);
    }

    message.payload.widget.command =
        stateFrame == FRAME_NONE ? WIDGET_COMMAND_CLEAR_FLAGS : WIDGET_COMMAND_SET_FLAGS;
    ...
    for (slotNum = 0; slotNum < HERO_AVAILABLE_SLOT_COUNT; ++slotNum) {
        ...
        sprintf(
            icnName,
            "port%04d.icn",
            (gpGame->m_heroRecs[gpCurPlayer->AvailableHeroId(slotNum)].m_portrait)
        );
        message.payload.widget.data.text = icnName;
...
MessageDispatchResult CastleHandler(tag_message& message) {
    ...
    if (message.type == MESSAGE_MOUSE_MOVE || message.type == MESSAGE_WIDGET) {
        ...
        if (message.payload.widget.id == CONTROL_CAPTAIN_ICON)
            whichBuilding = (TOWN_OBJECT_CAPTAIN_QUARTERS);
        ...
        switch (whichBuilding) {
            ...
            case (TOWN_OBJECT_MAGE_GUILD):
                if (!(gpTownManager->m_buildableBuildings & (1 << (whichBuilding)))) {
                    ...
                    if (!(gpTownManager->m_town->m_buildings & 1L))
                        ...
                        objIndex = (INFO_ADD_MAGE_GUILD_LEVEL);
            ...
            case (BUILDING_SLOT_NONE):
                switch (message.payload.widget.id) {
                    ...
                    hero_hover_text:
                        if (gpCurPlayer->m_resources[(RES_GOLD)] < gHeroGoldCost) {
                            strcpy(gText, cCastleInfo[(INFO_CANNOT_AFFORD_HERO)]);
                        } else if (gpCurPlayer->m_heroCount == PLAYER_HERO_CAPACITY) {
                            sprintf(
                                gText,
                                cCastleInfo[(INFO_TOO_MANY_HEROES)],
                                PLAYER_HERO_CAPACITY
                            );
                        } else if (gpTownManager->m_town->m_occupyingHeroId != -1) {
                            strcpy(gText, cCastleInfo[(INFO_TOWN_OCCUPIED)]);
                        } else {
                            sprintf(
                                gText,
                                cCastleInfo[(INFO_RECRUIT_HERO)],
                        ...
                        break;
                    case CONTROL_CLOSE:
                        strcpy(gText, cCastleInfo[(INFO_EXIT)]);
                        break;
                    default:
                        strcpy(gText, cCastleInfo[(INFO_OPTIONS)]);
                        break;
                }
                break;
        }
...
u8 castleSlotsBase[CASTLE_SLOT_COUNT] =
    {19, 20, 21, 22, 23, 24, 0, 2, 1, 3, 7, 10, 4, 11, 13, 8, 9, 12};
heroWindow* casWin;
u8 castleSlotsUse[CASTLE_SLOT_COUNT];
