...
void townManager::SetupCastle(heroWindow* window, i32 updateOnly) {
    ...
    casWin = window;
    i16 builtFrame7 = (FRAME_BUILT);
    i16 cannotBuildFrame17 = (FRAME_CANNOT_BUILD);
    i16 cannotAffordFrame6 = (FRAME_CANNOT_AFFORD);
    ...
    for (slot7 = 0; slot7 < CASTLE_SLOT_COUNT; ++slot7) {
        ...
        if (widgetFrame12 == FRAME_BUILT) {
            ...
            message3.payload.widget.data.value =
                widgetFrame12 == FRAME_NONE ? (FRAME_AVAILABLE) : FRAME_UNAVAILABLE;
            casWin->BroadcastMessage(message3);
        }
    }

    captainBuilt = m_town->m_buildings & (TOWN_BUILDING_CAPTAIN_QUARTERS);
    ...
    terrainIconFrame27 = ((giGroundToTerrain
...
MessageDispatchResult CastleHandler(tag_message& message) {
    ...
    textControl = (CONTROL_STATUS_TEXT);
    ...
    if (message.type == MESSAGE_MOUSE_MOVE || message.type == MESSAGE_WIDGET) {
        ...
        if (message.payload.widget.id == CONTROL_CAPTAIN_ICON)
            ...
            if (buildSlot != BUILDING_SLOT_NONE)
                buildSlot = castleSlotsUse[(buildSlot)];
        }
    }
    ...
    if (hoverMessage) {
        ...
        switch (buildSlot) {
            ...
            case TOWN_OBJECT_MAGE_GUILD:
                if (!(gpTownManager->m_buildableBuildings & (1 << (buildSlot)))) {
                    sprintf(
                        gText,
                ...
                } else if (!(gpTownManager->m_affordableBuildings & (1 << (buildSlot)))) {
                    sprintf(
                        gText,
                    ...
                    if (!(gpTownManager->m_town->m_buildings & 1L))
                        ...
                        loopIndex = (INFO_ADD_MAGE_GUILD_LEVEL);
            ...
            case BUILDING_SLOT_SPECIAL_THIRTY:
                ...
                } else if (gpTownManager->m_town->m_buildings & (1 << (buildSlot))) {
                    sprintf(
                        gText,
    ...
    if (message.type == MESSAGE_WIDGET) {
        switch (message.payload.widget.command) {
            ...
            case WIDGET_COMMAND_ALTERNATE_SELECT:
                quickFlag = ((((message.payload.widget.modifiers) & (MESSAGE_MODIFIER_LEFT_SHIFT)))) != 0;
                switch (buildSlot) {
                    ...
                    case BUILDING_SLOT_SPECIAL_THIRTY:
                        if (quickFlag
                            || (!(gpTownManager->m_town->m_buildings & (1 << (buildSlot)))
                                && (gpTownManager->m_buildableBuildings & (1 << (buildSlot))))) {
                        buy_building:
                            for (loopIndex = 0;
                            ...
                            result = gpTownManager->BuyBuild(
                                buildSlot,
                                (gpTownManager->m_affordableBuildings & (1 << (buildSlot))) == 0,
                                quickFlag
                            );
                        }
                        break;
