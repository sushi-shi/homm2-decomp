...
game::ViewSpells(
    ...
    if (spellHero->GetNumSpells(spellType) == 0) {
        NormalDialog(
            localization::Tr("spell.none_to_cast"),
            1
        );
    } else {
...
void game::ViewArmy(
    ...
    strcpy(armyName, gArmyNames[H2EnumIndex(monsterType)]);
        ...
        for (innerIndex = H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST); innerIndex <= H2EnumIndex(BUILDING_SLOT_DWELLING_LAST);
            ...
            if ((H2EnumIndex((castle->m_buildings) & ((1 << innerIndex))))) {
                ...
                if (castle->m_owner >= 0
                    ...
                    && !gbHumanPlayer[H2EnumIndex(castle->m_owner)]) {
                    if (gpGame->m_difficulty == DIFFICULTY_HARD)
...
void game::RandomizeTown(i32 x, i32 y, i32) {
    ...
    if (townExtra->color == RANDOM_TOWN_UNOWNED_COLOR)
        race = static_cast<FactionType>(Random(H2EnumIndex(FACTION_KNIGHT), H2EnumIndex(FACTION_NECROMANCER)));
    else
...
void game::SetupTowns(void) {
    ...
    for (townIndex = 0; townIndex < GAME_TOWN_COUNT; townIndex++) {
        ...
        if (extra->hasCustomBuildings) {
            ...
            if (!gbHumanPlayer[H2EnumIndex(castle->m_owner)] && dwellingCount == 1 && Random(1, 10) < 4)
                dwellingCount++;
            if (--dwellingCount != 0) {
            ...
            for (spellIndex = 0; spellIndex < TOWN_MAGE_GUILD_SPELLS_PER_LEVEL; spellIndex++) {
                ...
                }
                {
                    ...
                    do {
                        ...
                        if (castle->m_owner != -1 && !gbHumanPlayer[H2EnumIndex(castle->m_owner)])
                            spellValue =
                    ...
                    if (pass == MAP_HERO_PLACEMENT_PASS) {
                        ...
                        if (isJail) {
                            ...
                            m_players[H2EnumIndex(mapHero->m_owner)]
                                ...
                                mapHero->m_id;
                            m_players[H2EnumIndex(mapHero->m_owner)].m_heroCount++;
                        }
        ...
        ultimateRumour:
            ...
            if (selectionRoll < 33) {
                ...
                sprintf(
                    ...
                    cRumourTerrainDescriptions[H2EnumIndex(CELL_TERRAIN(
