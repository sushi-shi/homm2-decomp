        ...
        for (innerIndex = WEEKLY_FIRST_DWELLING; innerIndex <= WEEKLY_LAST_DWELLING;
            ...
            if ((H2EnumIndex((castle5->m_buildings) & ((1 << innerIndex))))) {
                ...
                if (castle5->m_owner >= 0
                    ...
                    && !gbHumanPlayer[H2EnumIndex(castle5->m_owner)]) {
                    if (gpGame->m_difficulty == DIFFICULTY_HARD)
...
void game::SetupTowns(void) {
    ...
    for (townIndex1 = 0; townIndex1 < GAME_TOWN_COUNT; townIndex1++) {
        ...
        if (extra0->hasCustomBuildings) {
            ...
            if (!gbHumanPlayer[H2EnumIndex(castle8->m_owner)] && dwellingCount1 == 1 && Random(1, 10) < 4)
                dwellingCount1++;
            if (--dwellingCount1 != 0) {
        ...
        if (castle8->m_type == FACTION_NECROMANCER && castle8->m_owner != -1
            && !gbHumanPlayer[H2EnumIndex(castle8->m_owner)]) {
            if (Random(0, 100) < 50)
            ...
            for (spellIndex3 = 0; spellIndex3 < TOWN_MAGE_GUILD_SPELLS_PER_LEVEL; spellIndex3++) {
                ...
                }
                {
                    ...
                    do {
                        ...
                        if (castle8->m_owner != -1 && !gbHumanPlayer[H2EnumIndex(castle8->m_owner)])
                            spellValue =
                ...
                if ((cell9->m_triggerType & MAP_TRIGGER_TYPE_MASK) == MAP_OBJECT_HERO
                    ...
                    if (pass27 == MAP_HERO_PLACEMENT_PASS) {
                        ...
                        if (isJail4) {
                            ...
                            m_players[H2EnumIndex(mapHero14->m_owner)]
                                .m_heroIds[m_players[H2EnumIndex(mapHero14->m_owner)].m_heroCount] =
                                mapHero14->m_id;
                            m_players[H2EnumIndex(mapHero14->m_owner)].m_heroCount++;
                        }
