...
i32 game::SaveGame(const char* filename, i32 generateName, i8 expansionFormat) {
    ...
    WriteGameData(outFile, workBuffer, SAVE_LEGACY_SERIALIZED_SIZE);
    ...
    WriteGameData(outFile, m_townOwners, sizeof(m_townOwners));
...
void game::ViewArmy(
    ...
    if (castle && (gpAdvManager->m_active == 1 || gpTownManager->m_active == 1)) {
        ...
        if ((monsterType == CREATURE_GREEN_DRAGON || monsterType == CREATURE_RED_DRAGON)
            && (H2EnumIndex((castle->m_buildings) & (H2EnumIndex(TOWN_BUILDING_ALTERNATE_UPGRADED_DWELLING_6))))) {
            gbAllowUpgrade = true;
...
void game::PerWeek(void) {
    ...
    for (outerIndex = 0; outerIndex < GAME_TOWN_COUNT; outerIndex++) {
        ...
        for (innerIndex = H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST); innerIndex <= H2EnumIndex(BUILDING_SLOT_DWELLING_LAST);
             innerIndex++) {
            if ((H2EnumIndex((castle->m_buildings) & ((1 << innerIndex))))) {
                creatureGrowth = gMonsterDatabase[H2EnumIndex(gDwellingType[H2EnumIndex(castle->m_type)]
                               ...
                               .growth;
                if ((H2EnumIndex((castle->m_buildings) & ((1 << H2EnumIndex(BUILDING_SLOT_SPECIAL_FOUR))))))
                    creatureGrowth += TOWN_WELL_BASE_GROWTH_BONUS;
                if (innerIndex == H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST)
                    && (H2EnumIndex((castle->m_buildings) & ((1 << H2EnumIndex(BUILDING_SLOT_WELL_EXTRA))))))
...
void game::PerMonth(void) {
    ...
    for (i = 0; i < GAME_TOWN_COUNT; i++) {
        for (j = H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST); j <= H2EnumIndex(BUILDING_SLOT_DWELLING_LAST); j++) {
            ...
            if ((H2EnumIndex((townPointer->m_buildings) & ((1 << j))))) {
...
void game::RandomizeTown(i32 x, i32 y, i32) {
    ...
    if (townExtra->color == RANDOM_TOWN_UNOWNED_COLOR)
        race = FactionTypeFromOrdinal(Random(H2EnumIndex(FACTION_KNIGHT), H2EnumIndex(FACTION_NECROMANCER)));
    else
...
void game::SetupTowns(void) {
    ...
    for (townIndex = 0; townIndex < GAME_TOWN_COUNT; townIndex++) {
        ...
        if (extra->hasCustomBuildings) {
            castle->m_buildings =
                (H2EnumIndex((castle->m_buildings) & ((H2EnumIndex(TOWN_BUILDING_CASTLE) | H2EnumIndex(TOWN_BUILDING_TENT)))))
        ...
        for (slot = H2EnumIndex(BUILDING_SLOT_UPGRADE_FIRST); slot <= H2EnumIndex(BUILDING_SLOT_DWELLING_LAST);
             slot++) {
            if ((H2EnumIndex((castle->m_buildings) & ((1 << slot))))) {
        ...
        if ((H2EnumIndex((castle->m_buildings) & (H2EnumIndex(TOWN_BUILDING_MAGE_GUILD))))) {
            for (slot = 1; slot <= castle->m_buildState; slot++) {
                ...
                if (castle->m_type == FACTION_WIZARD
                    && (H2EnumIndex((castle->m_buildings) & ((1 << H2EnumIndex(BUILDING_SLOT_SPECIAL))))))
                ...
                if ((cell->m_triggerType & MAP_TRIGGER_TYPE_MASK) == MAP_OBJECT_HERO
                    ...
                    if (pass == MAP_HERO_CLASS_PASS) {
                        if (isJail) {
                            ...
                            heroFaction = FactionTypeFromCode(
                    ...
                    if (pass == MAP_HERO_PLACEMENT_PASS) {
                        ...
                        if (!isJail) {
                            SetVisibility(
                                ...
                                mapHero->m_y,
                                mapHero->m_owner,
                                giVisRange[H2EnumIndex(mapHero->m_secondarySkills
                                                   [H2EnumIndex(HERO_SKILL_SCOUTING)])]
                            );
                        }
                        H2_FREE(ppMapExtra[extraIndex]);
                        ppMapExtra[extraIndex] = NULL;
