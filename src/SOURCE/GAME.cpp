...
typedef enum GameNextPlayerConstant {
    NEXT_PLAYER_SCRATCH_SIZE = 20
} GameNextPlayerConstant;
...
i32 game::SetupPuzzlePieces(i32 player, i32 justCount) {
    ...
    for (i = 0; (&i)[0] < pieceCountTotal; i++) {
        for (pieceValue = 0;
            ...
            if (!H2BitTest(puzzlePiecesRemoved, pieceValue))
                break;
        }
        ...
        if (pieceValue < PUZZLE_PIECE_COUNT)
            H2BitSet(puzzlePiecesRemoved, pieceValue);
        else
...
i32 game::GetNewHeroId(i32, FactionType heroClass, i32 requireExperienced) {
    ...
    while (attempts < HERO_SELECTION_RETRY_LIMIT) {
        attempts++;
        heroId = Random(0, H2EnumIndex(GAME_HERO_COUNT) - 1);
...
i32 game::SaveGame(char* filename, i32 generateName, i8 expansionFormat) {
    ...
    i32 handle = platform::FileOpen(savePathValue, platform::FileMode::Write);
    if (handle == -1)
        FileError(savePathValue);

    i32 legacyMarkerTemp = -1;
    if (!expansionFormat)
        platform::FileWrite(handle, &legacyMarkerTemp, sizeof(legacyMarkerTemp));
    platform::FileWrite(handle, &m_worldMap.width, sizeof(m_worldMap.width));
    platform::FileWrite(handle, &m_worldMap.height, sizeof(m_worldMap.height));
    platform::FileWrite(handle, &m_mapHeader, sizeof(m_mapHeader));
    platform::FileWrite(handle, m_setupPlayerColor, CAMPAIGN_SETUP_RESET_SIZE);
    platform::FileWrite(handle, &gbIAmGreatest, SAVE_TRUNCATED_SCALAR_SIZE);
    platform::FileWrite(handle, this, sizeof(m_difficultyRating));
    platform::FileWrite(handle, &giMonthType, SAVE_TRUNCATED_SCALAR_SIZE);
    platform::FileWrite(handle, &giMonthTypeExtra, SAVE_TRUNCATED_SCALAR_SIZE);
    platform::FileWrite(handle, &giWeekType, SAVE_TRUNCATED_SCALAR_SIZE);
    platform::FileWrite(handle, &giWeekTypeExtra, SAVE_TRUNCATED_SCALAR_SIZE);
    platform::FileWrite(handle, cPlayerNames, sizeof(cPlayerNames));

    char legacyData[SAVE_LEGACY_SCRATCH_SIZE];
    memset(legacyData, 0, SAVE_LEGACY_CLEAR_SIZE);
    platform::FileWrite(handle, legacyData, SAVE_LEGACY_SERIALIZED_SIZE);
    if (xIsPlayingExpansionCampaign) {
        i32 campaignTypeInfo = SAVE_EXPANSION_CAMPAIGN_FORMAT_TAG;
        platform::FileWrite(handle, &campaignTypeInfo, sizeof(campaignTypeInfo));
        platform::FileWrite(handle, &xCampaign, CAMPAIGN_SAVE_PREFIX_SIZE);
    } else {
        platform::FileWrite(handle, &gbInCampaign, sizeof(gbInCampaign));
        if (gbInCampaign)
            platform::FileWrite(handle, &m_campaignType, CAMPAIGN_STATE_RESET_SIZE);
    }
    if (!expansionFormat)
        platform::FileWrite(handle, &xIsExpansionMap, sizeof(xIsExpansionMap));

    gpAdvManager->PurgeMapChangeQueue();
    platform::FileWrite(handle, &giMapChangeCtr, sizeof(giMapChangeCtr));
    GenerateStandardFileName(m_saveName, legacyData);
    platform::FileWrite(handle, legacyData, SAVE_STANDARD_FILENAME_SIZE);
    platform::FileWrite(handle, &m_playerCount, sizeof(m_playerCount));
    char currentPlayerInfo[SAVE_CURRENT_PLAYER_SCRATCH_SIZE];
    currentPlayerInfo[0] = static_cast<char>(giCurPlayer);
    platform::FileWrite(handle, currentPlayerInfo, sizeof(currentPlayerInfo[0]));
    platform::FileWrite(handle, &m_deadPlayerCount, sizeof(m_deadPlayerCount));
    platform::FileWrite(handle, m_playerDead, sizeof(m_playerDead));
    ...
    platform::FileWrite(handle, humanFlagsLocal, GAME_PLAYER_COUNT);
    platform::FileWrite(handle, &m_day, sizeof(m_day));
    platform::FileWrite(handle, &m_week, sizeof(m_week));
    platform::FileWrite(handle, &m_month, sizeof(m_month));
    for (indexFile = 0; indexFile < GAME_PLAYER_COUNT; indexFile++)
        m_players[indexFile].Write(handle);

    platform::FileWrite(handle, &m_obeliskCount, sizeof(m_obeliskCount));
    for (indexFile = 0; indexFile < GAME_HERO_COUNT; indexFile++)
        m_heroRecs[indexFile].Write(handle, !expansionFormat);
    platform::FileWrite(handle, m_availableHeroes, sizeof(m_availableHeroes));
    platform::FileWrite(handle, m_castleRecs, sizeof(m_castleRecs));
    platform::FileWrite(handle, m_castleOwners, sizeof(m_castleOwners));
    platform::FileWrite(handle, m_dailyEventFlags, sizeof(m_dailyEventFlags));
    platform::FileWrite(handle, m_mines, sizeof(m_mines));
    platform::FileWrite(handle, m_mineOwners, sizeof(m_mineOwners));
    if (!expansionFormat)
        platform::FileWrite(handle, m_randomArtifacts, H2EnumIndex(ARTIFACT_COUNT));
    else
        platform::FileWrite(handle, m_randomArtifacts, ARTIFACT_BASE_TABLE_SIZE);
    platform::FileWrite(handle, m_boats, sizeof(m_boats));
    platform::FileWrite(handle, m_boatSlots, sizeof(m_boatSlots));
    platform::FileWrite(handle, m_obeliskVisitors, sizeof(m_obeliskVisitors));
    platform::FileWrite(handle, &m_ultimateArtifactX, sizeof(m_ultimateArtifactX));
    platform::FileWrite(handle, &m_ultimateArtifactY, sizeof(m_ultimateArtifactY));
    platform::FileWrite(handle, &m_ultimateArtifactId, sizeof(m_ultimateArtifactId));
    platform::FileWrite(handle, m_rumour, sizeof(m_rumour));
    platform::FileWrite(handle, m_defaultPlayerNames, sizeof(m_defaultPlayerNames));

    platform::FileWrite(handle, &m_rumourEventCount, SAVE_EVENT_HEADER_SIZE);
    platform::FileWrite(
        handle,
        m_rumourEventIndices,
        m_rumourEventCount * sizeof(m_rumourEventIndices[0])
    );
    platform::FileWrite(handle, &m_timeEventCount, SAVE_EVENT_HEADER_SIZE);
    platform::FileWrite(handle, m_timeEventIndices, m_timeEventCount * sizeof(m_timeEventIndices[0]));
    platform::FileWrite(handle, &m_mapEventCount, SAVE_EVENT_HEADER_SIZE);
    platform::FileWrite(handle, m_mapEventIndices, m_mapEventCount * sizeof(m_mapEventIndices[0]));

    i32 markerBuffer[SAVE_MARKER_SCRATCH_COUNT];
    markerBuffer[0] = GAME_FILE_MARKER;
    i32 unusedMarkerInfo = GAME_UNUSED_FILE_MARKER;
    platform::FileWrite(handle, markerBuffer, sizeof(markerBuffer[0]));
    platform::FileWrite(handle, &iMaxMapExtra, sizeof(iMaxMapExtra));
    platform::FileWrite(handle, markerBuffer, sizeof(markerBuffer[0]));
    for (indexFile = 1; indexFile < iMaxMapExtra; indexFile++) {
        platform::FileWrite(handle, markerBuffer, sizeof(markerBuffer[0]));
        platform::FileWrite(handle, pwSizeOfMapExtra + indexFile, sizeof(pwSizeOfMapExtra[indexFile]));
        if (ppMapExtra[indexFile] != NULL)
            platform::FileWrite(handle, ppMapExtra[indexFile], pwSizeOfMapExtra[indexFile]);
        else
            platform::FileWrite(handle, emptyPayload, pwSizeOfMapExtra[indexFile]);
    }
    platform::FileWrite(handle, markerBuffer, sizeof(markerBuffer[0]));
    platform::FileWrite(handle, mapExtra, MAP_WIDTH * MAP_HEIGHT);
    platform::FileWrite(handle, markerBuffer, sizeof(markerBuffer[0]));
...
void game::LoadGame(char* filename, i32 loadFromFile, i32) {
    ...
    i32 file0 = platform::FileOpen(path28, platform::FileMode::Read);
    if (file0 == -1)
        FileError(path28);
    ClearMapExtra();
    ...
    platform::FileRead(file0, &width8, sizeof(width8));
    if (width8 == -1) {
        expansionMarker0 = 1;
        platform::FileRead(file0, &width8, sizeof(width8));
    }
    platform::FileRead(file0, height9, sizeof(height9[0]));
    SetMapSize(width8, height9[0]);
    platform::FileRead(file0, &m_mapHeader, sizeof(m_mapHeader));
    platform::FileRead(file0, m_setupPlayerColor, CAMPAIGN_SETUP_RESET_SIZE);
    platform::FileRead(file0, &gbIAmGreatest, SAVE_TRUNCATED_SCALAR_SIZE);
    platform::FileRead(file0, this, sizeof(m_difficultyRating));
    platform::FileRead(file0, &giMonthType, SAVE_TRUNCATED_SCALAR_SIZE);
    platform::FileRead(file0, &giMonthTypeExtra, SAVE_TRUNCATED_SCALAR_SIZE);
    platform::FileRead(file0, &giWeekType, SAVE_TRUNCATED_SCALAR_SIZE);
    platform::FileRead(file0, &giWeekTypeExtra, SAVE_TRUNCATED_SCALAR_SIZE);
    platform::FileRead(file0, cPlayerNames, sizeof(cPlayerNames));

    char oldData3[SAVE_LEGACY_CLEAR_SIZE];
    platform::FileRead(file0, oldData3, SAVE_LEGACY_SERIALIZED_SIZE);
    platform::FileRead(file0, &gbInCampaign, sizeof(gbInCampaign));
    if (gbInCampaign == 1) {
        platform::FileRead(file0, &m_campaignType, CAMPAIGN_STATE_RESET_SIZE);
    } else if (gbInCampaign == SAVE_EXPANSION_CAMPAIGN_FORMAT_TAG) {
        xIsPlayingExpansionCampaign = 1;
        gbInCampaign = false;
        platform::FileRead(file0, &xCampaign, CAMPAIGN_SAVE_PREFIX_SIZE);
    }
    if (expansionMarker0)
        platform::FileRead(file0, &xIsExpansionMap, sizeof(xIsExpansionMap));
    ...
    platform::FileRead(file0, currentPlayer6, sizeof(currentPlayer6[0]));
    giCurPlayer = currentPlayer6[0];
    platform::FileRead(file0, &m_deadPlayerCount, sizeof(m_deadPlayerCount));
    platform::FileRead(file0, m_playerDead, sizeof(m_playerDead));

    char humanFlags1[SAVE_PLAYER_FLAGS_SCRATCH_SIZE];
    platform::FileRead(file0, humanFlags1, GAME_PLAYER_COUNT);
    i32 i29;
    ...
    platform::FileRead(file0, &m_day, sizeof(m_day));
    platform::FileRead(file0, &m_week, sizeof(m_week));
    platform::FileRead(file0, &m_month, sizeof(m_month));
    giCurTurn = (m_week - 1) * EVENT_DAYS_PER_WEEK
                + (m_month - 1) * EVENT_DAYS_PER_MONTH + m_day;
    for (i29 = 0; i29 < GAME_PLAYER_COUNT; i29++)
        m_players[i29].Read(file0);

    platform::FileRead(file0, &m_obeliskCount, sizeof(m_obeliskCount));
    for (i29 = 0; i29 < GAME_HERO_COUNT; i29++)
        m_heroRecs[i29].Read(file0, expansionMarker0);
    platform::FileRead(file0, m_availableHeroes, sizeof(m_availableHeroes));
    platform::FileRead(file0, m_castleRecs, sizeof(m_castleRecs));
    platform::FileRead(file0, m_castleOwners, sizeof(m_castleOwners));
    platform::FileRead(file0, m_dailyEventFlags, sizeof(m_dailyEventFlags));
    platform::FileRead(file0, m_mines, sizeof(m_mines));
    platform::FileRead(file0, m_mineOwners, sizeof(m_mineOwners));
    if (expansionMarker0)
        platform::FileRead(file0, m_randomArtifacts, H2EnumIndex(ARTIFACT_COUNT));
    else
        platform::FileRead(file0, m_randomArtifacts, ARTIFACT_BASE_TABLE_SIZE);
    platform::FileRead(file0, m_boats, sizeof(m_boats));
    platform::FileRead(file0, m_boatSlots, sizeof(m_boatSlots));
    platform::FileRead(file0, m_obeliskVisitors, sizeof(m_obeliskVisitors));
    platform::FileRead(file0, &m_ultimateArtifactX, sizeof(m_ultimateArtifactX));
    platform::FileRead(file0, &m_ultimateArtifactY, sizeof(m_ultimateArtifactY));
    platform::FileRead(file0, &m_ultimateArtifactId, sizeof(m_ultimateArtifactId));
    platform::FileRead(file0, m_rumour, sizeof(m_rumour));
    platform::FileRead(file0, m_defaultPlayerNames, sizeof(m_defaultPlayerNames));
    platform::FileRead(file0, &m_rumourEventCount, SAVE_EVENT_HEADER_SIZE);
    platform::FileRead(
        file0,
        m_rumourEventIndices,
        m_rumourEventCount * sizeof(m_rumourEventIndices[0])
    );
    platform::FileRead(file0, &m_timeEventCount, SAVE_EVENT_HEADER_SIZE);
    platform::FileRead(file0, m_timeEventIndices, m_timeEventCount * sizeof(m_timeEventIndices[0]));
    platform::FileRead(file0, &m_mapEventCount, SAVE_EVENT_HEADER_SIZE);
    platform::FileRead(file0, m_mapEventIndices, m_mapEventCount * sizeof(m_mapEventIndices[0]));

    char marker0[LOAD_MARKER_SCRATCH_SIZE];
    platform::FileRead(file0, marker0, sizeof(i32));
    platform::FileRead(file0, &iMaxMapExtra, sizeof(iMaxMapExtra));
    platform::FileRead(file0, marker0, sizeof(i32));
    ...
    for (i29 = 1; (&i29)[0] < iMaxMapExtra; i29++) {
        platform::FileRead(file0, marker0, sizeof(i32));
        platform::FileRead(file0, pwSizeOfMapExtra + i29, sizeof(pwSizeOfMapExtra[i29]));
        ppMapExtra[i29] = H2_ALLOC(pwSizeOfMapExtra[i29]);
        platform::FileRead(file0, ppMapExtra[i29], pwSizeOfMapExtra[i29]);
    }
    platform::FileRead(file0, marker0, sizeof(i32));
    platform::FileRead(file0, mapExtra, MAP_WIDTH * MAP_HEIGHT);
    platform::FileRead(file0, marker0, sizeof(i32));
    m_worldMap.Read(file0, 0);
    platform::FileRead(file0, marker0, sizeof(i32));
    platform::FileClose(file0);
...
void game::GiveTroopsToNeutralTown(i32 townId) {
    ...
    if ((m_castleRecs[townId].m_x > 0 || m_castleRecs[townId].m_y > 0)
        ...
        switch (H2EnumIndex(m_castleRecs[townId].m_type) + tierKey36) {
...
void game::NewMap(char* filename) {
    ...
    for (player2 = 0; player2 < m_playerCount; player2++) {
        if (player2 == 0 && gbInCampaign
            && (m_campaignAwards[H2EnumIndex(CAMPAIGN_AWARD_SORCERESS_GUILD)] != 0
                || m_campaignAwards[H2EnumIndex(CAMPAIGN_AWARD_DWARFBANE)] != 0)) {
        ...
        }
        {
            ...
            heroClass5 = static_cast<FactionType>(Random(0, H2EnumIndex(FACTION_COUNT) - 1));
    ...
    secondHero:
        heroClass5 = (heroClass5 + Random(1, H2EnumIndex(FACTION_COUNT) - 1)) % H2EnumIndex(FACTION_COUNT);
    ...
    for (player2 = 0; player2 < m_playerCount; player2++) {
        if (gbHumanPlayer[player2]) {
            ...
            if (m_playerHandicap[player2] != PLAYER_HANDICAP_NONE) {
                for (townIndex9 = 0; townIndex9 < H2EnumIndex(RES_COUNT); townIndex9++) {
...
void game::RandomizeEvents(void) {
        ...
        for (xPos2 = 0; xPos2 < MAP_WIDTH; xPos2++) {
            ...
            switch (cell2->m_triggerType) {
                ...
                case MAP_TRIGGER_ACTION_FLAG | MAP_OBJECT_RESOURCE: {
                    ...
                    cell2->m_objectMetadata = H2EnumIndex(resourceType);
                    switch (resourceType) {
                        case RES_WOOD:
                        case RES_ORE:
                            cell2->m_objectMetadata =
                ...
                case MAP_TRIGGER_ACTION_FLAG | MAP_OBJECT_ARTIFACT:
                    ...
                    if (value26 != ARTIFACT_SPELL_SCROLL) {
                        if (randomValue7 < ARTIFACT_EVENT_UNCONDITIONAL_CUTOFF) {
                            ...
                            if (gArtifactLevel[H2EnumIndex(value26)] == ARTIFACT_LEVEL_TREASURE)
                                cell2->m_objectMetadata = ARTIFACT_EVENT_MODE_GOLD;
                            else if (gArtifactLevel[H2EnumIndex(value26)] == ARTIFACT_LEVEL_MINOR)
                                cell2->m_objectMetadata =
                                    (Random(H2EnumIndex(RES_WOOD), H2EnumIndex(RES_GEMS))
                                     << ARTIFACT_EVENT_RESOURCE_SHIFT)
                                    | ARTIFACT_EVENT_MODE_RESOURCE_3;
                            else if (gArtifactLevel[H2EnumIndex(value26)] == ARTIFACT_LEVEL_MAJOR)
                                cell2->m_objectMetadata =
                                    (Random(H2EnumIndex(RES_WOOD), H2EnumIndex(RES_GEMS))
                                     << ARTIFACT_EVENT_RESOURCE_SHIFT)
                            ...
                            if (gArtifactLevel[H2EnumIndex(value26)] == ARTIFACT_LEVEL_TREASURE)
                                ...
                                cell2->m_objectMetadata |= H2EnumIndex(artifactGuardianChoices[Random(
                                    EVENT_ROLL_MIN,
    ...
    for (yPos19 = 0; yPos19 < MAP_HEIGHT; yPos19++) {
        for (xPos2 = 0; xPos2 < MAP_WIDTH; xPos2++) {
            ...
            upperCount = 0;
            lowerCount16 = 0;
            if (!(cell2->m_flags & H2EnumIndex(MAP_CELL_OCCUPIED)) && yPos19 < MAP_HEIGHT - 1
                && cell2->m_objectIndex != MAPCELL_SPRITE_NONE
                && !(cell2->m_triggerType & MAP_TRIGGER_ACTION_FLAG)
                && !(cell2->m_flags & H2EnumIndex(MAP_CELL_OBJECT_SHADOW_ONLY))) {
                mapCell* below0;
...
i32 game::LoadMap(char* filename) {
    ...
    file2 = platform::FileOpen(gText, platform::FileMode::Read);
    if (file2 == -1)
        FileError(gText);
    platform::FileRead(file2, &m_mapHeader, sizeof(m_mapHeader));
    m_worldMap.Read(file2, 1);
    SetMapSize(m_worldMap.width, m_worldMap.height);

    for (i37 = 0; i37 < GAME_TOWN_COUNT; i37++) {
        platform::FileRead(file2, column5, sizeof(column5[0]));
        platform::FileRead(file2, row9, sizeof(row9[0]));
        platform::FileRead(file2, type5, sizeof(type5[0]));
        if (static_cast<u8>(column5[0]) != SAVED_TOWN_OFF_MAP) {
            ...
            if (type5[0] < 0)
                m_castleRecs[i37].m_buildings |= H2EnumIndex(TOWN_BUILDING_CASTLE);
            else
                m_castleRecs[i37].m_buildings |= H2EnumIndex(TOWN_BUILDING_TENT);
        }
    }
    ...
    platform::FileRead(file2, &m_obeliskCount, sizeof(m_obeliskCount));
    platform::FileRead(
        file2,
        m_rumourEventIndices,
    ...
    platform::FileRead(file2, &iMaxMapExtra, sizeof(iMaxMapExtra));
    ppMapExtra = reinterpret_cast<void**>(
    ...
    for (i37 = 1; (&i37)[0] < iMaxMapExtra; i37++) {
        platform::FileRead(file2, pwSizeOfMapExtra + i37, sizeof(pwSizeOfMapExtra[0]));
        ppMapExtra[i37] = H2_ALLOC(pwSizeOfMapExtra[i37]);
        platform::FileRead(file2, ppMapExtra[i37], pwSizeOfMapExtra[i37]);
    }
    platform::FileRead(file2, trailer15, sizeof(u16));
    platform::FileClose(file2);
    return 0;
}
...
void game::UpdateSpellWidgets(void) {
    ...
    message9.payload.widget.data.value = H2EnumIndex(WIDGET_FLAG_ENABLED | WIDGET_FLAG_DRAW);
    ...
    for (spellSlot6 = 0; spellSlot6 < VIEW_SPELL_PAGE_SIZE; spellSlot6++) {
        if (m_viewSpellsTop[H2EnumIndex(m_viewSpellsType)] + spellSlot6
            ...
            spell2 = m_viewSpellsHero->GetNthSpell(
                m_viewSpellsType,
                m_viewSpellsTop[H2EnumIndex(m_viewSpellsType)] + spellSlot6 + 1
            );
            ...
            if (m_viewSpellsReadOnly) {
                message9.payload.widget.command = WIDGET_COMMAND_SET_FLAGS;
                message9.payload.widget.data.value = H2EnumIndex(WIDGET_FLAG_ENABLED);
                m_viewSpellsWindow->BroadcastMessage(message9);
            }
            message9.payload.widget.command = WIDGET_COMMAND_SET_FRAME;
            message9.payload.widget.data.value = gsSpellInfo[H2EnumIndex(spell2)].iconIndex;
            m_viewSpellsWindow->BroadcastMessage(message9);
            lineLength0 = smallFont->LineLength(gSpellNames[H2EnumIndex(spell2)], VIEW_SPELL_NAME_WIDTH);
            if (lineLength0 == 1) {
                sprintf(
                    gText,
                    "%s\n[%d]",
                    gSpellNames[H2EnumIndex(spell2)],
...
void game::ViewArmy(
    ...
    if (castle && (gpAdvManager->m_active == 1 || gpTownManager->m_active == 1)) {
        for (loopIndex0 = H2EnumIndex(BUILDING_SLOT_DWELLING_SECOND);
             loopIndex0 <= H2EnumIndex(BUILDING_SLOT_DWELLING_SIXTH);
             loopIndex0++) {
            if (gDwellingType[H2EnumIndex(castle->m_type)]
                             [loopIndex0 - H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST)]
                    == monsterType
                && (castle->m_buildings
    ...
    tag_monsterInfo* monster8 = &gMonsterDatabase[H2EnumIndex(monsterType)];
    tag_monsterInfo* armyMonster11;
    if (theArmy)
        armyMonster11 = &theArmy->m_monster;
    else
        armyMonster11 = &gMonsterDatabase[H2EnumIndex(monsterType)];
    ...
    if (gbLowMemory)
        sprintf(filename4, "monh%04d.icn", H2EnumIndex(monsterType));
    else
        strcpy(filename4, cMonFilename[H2EnumIndex(monsterType)]);

    icon* monsterIcon5 = gpResourceManager->GetIcon(filename4);
    i32 iconFrame15 = sViewArmyMonFrameInfo.animationFrames[H2EnumIndex(ARMY_ANIMATION_WALK)][0];
    ...
    iconWidget* monsterWidget7 = new iconWidget(
        ...
        filename4,
        gbLowMemory ? 0 : sViewArmyMonFrameInfo.animationFrames[H2EnumIndex(ARMY_ANIMATION_WALK)][0],
    ...
    strcpy(armyName8, gArmyNames[H2EnumIndex(monsterType)]);
    ...
    if ((H2EnumIndex((monster8->flags.all) & (MONSTER_FLAGS_NO_MORALE))))
        morale2 = 0;
    ...
    if ((H2EnumIndex((monster8->flags.all) & (MONSTER_FLAGS_SHOOTER)))) {
    ...
    if (!gbAllowUpgrade) {
        ...
        message6.payload.widget.data.value = H2EnumIndex(WIDGET_FLAG_ENABLED | WIDGET_FLAG_DRAW);
    ...
    if (theArmy) {
        ...
        H2SteppedEnumStorage<ArmySpellInfluence, i32> spellIndex9 =
        ...
        for (loopIndex0 = 0;
            ...
            for (; spellIndex9 < ARMY_SPELL_INFLUENCE_COUNT; spellIndex9++) {
                if (theArmy->m_spellInfluence[H2EnumIndex(spellIndex9)])
                    break;
            }
            iconWidget* spellWidget = new iconWidget(
                ...
                static_cast<i16>(H2EnumIndex(spellIndex9)),
                ICON_DRAW_NORMAL,
            ...
            case WIDGET_COMMAND_DESELECT:
                switch (msg.payload.widget.id) {
                    ...
                    case VIEW_ARMY_UPGRADE_ACTION_ID:
                        goldCost6 = (gMonsterDatabase[H2EnumIndex(iViewArmyUpgradeToType)].cost
                        ...
                        if (gpCurPlayer->m_resources[H2EnumIndex(RES_GOLD)] >= goldCost6
                            && (resourceType0 == RES_NONE
                                || gpCurPlayer->m_resources[H2EnumIndex(resourceType0)] >= resourceCost5)) {
                            NormalDialog(
                                const_cast<char*>(
                                ...
                                goldCost6,
                                H2EnumIndex(resourceType0),
                                resourceCost5,
                                -1,
                                0
                            );
                            if (gpWindowManager->m_dialogResult == NORMAL_DIALOG_BUTTON_FIVE) {
                                gpCurPlayer->m_resources[H2EnumIndex(RES_GOLD)] -= goldCost6;
...
i32 game::ComputeDailyGold(i32 player) {
    ...
    for (index = 0; index < GAME_TOWN_COUNT; index++) {
        if (m_castleRecs[index].m_owner == player) {
            if (m_castleRecs[index].m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_UPGRADE_CASTLE)))
...
void game::PerDay(void) {
    ...
    for (player = 0; player < GAME_HERO_COUNT; player++) {
        ...
        restoredSpellPoints13 +=
            H2EnumIndex(currentHero6->m_secondarySkills[H2EnumIndex(HERO_SKILL_MYSTICISM)]) + 1;
        ...
        if ((H2EnumIndex((currentHero6->m_eventFlags) & (HERO_EVENT_MAGIC_WELL))))
            ((currentHero6->m_eventFlags) &= ~(HERO_EVENT_MAGIC_WELL));
    }

    for (player = 0; player < GAME_TOWN_COUNT; player++) {
        currentTown4 = GetTown(player);
        if (!(currentTown4->m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_MAGE_GUILD))))
            continue;
...
void game::PerWeek(void) {
    ...
    for (outerIndex5 = 0; outerIndex5 < GAME_TOWN_COUNT; outerIndex5++) {
        ...
        for (innerIndex3 = WEEKLY_FIRST_DWELLING; innerIndex3 <= WEEKLY_LAST_DWELLING;
            ...
            if (castle37->m_buildings & (1 << innerIndex3)) {
                growth13 = gMonsterDatabase[H2EnumIndex(gDwellingType[H2EnumIndex(castle37->m_type)]
                                                             [innerIndex3 - WEEKLY_FIRST_DWELLING])]
                               .growth;
                if (castle37->m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_SPECIAL_FOUR)))
                    growth13 += CASTLE_GROWTH_SPECIAL_BONUS;
                if (innerIndex3 == WEEKLY_FIRST_DWELLING
                    && (castle37->m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_WELL_EXTRA))))
                ...
                if (giWeekType == CALENDAR_PERIOD_CREATURE
                    && H2EnumIndex(gDwellingType[H2EnumIndex(castle37->m_type)]
                                        [innerIndex3 - WEEKLY_FIRST_DWELLING])
                           == giWeekTypeExtra)
    ...
    for (outerIndex5 = 0; outerIndex5 < GAME_PLAYER_COUNT; outerIndex5++) {
        for (innerIndex3 = 0; innerIndex3 < AVAILABLE_HERO_SLOTS; innerIndex3++) {
            ...
            heroClass18 =
                (heroClass18 + Random(1, H2EnumIndex(FACTION_COUNT) - 1)) % H2EnumIndex(FACTION_COUNT);
    ...
    for (outerIndex5 = 0; outerIndex5 < GAME_HERO_COUNT; outerIndex5++) {
        ...
        if ((H2EnumIndex((weeklyHero4->m_eventFlags) & (WEEKLY_HERO_VISIT_FLAG))))
            ((weeklyHero4->m_eventFlags) &= ~(WEEKLY_HERO_VISIT_FLAG));
    }
    ...
    for (townIndex0 = 0; townIndex0 < GAME_TOWN_COUNT; townIndex0++) {
        for (building4 = WEEKLY_FIRST_DWELLING; building4 <= WEEKLY_LAST_DWELLING; building4++) {
            ...
            if (castle10->m_buildings & (1 << building4)) {
                growth9 = gMonsterDatabase[H2EnumIndex(gDwellingType[H2EnumIndex(castle10->m_type)]
                ...
                if (giMonthType == CALENDAR_PERIOD_CREATURE
                    && H2EnumIndex(gDwellingType[H2EnumIndex(castle10->m_type)]
                                        [building4 - WEEKLY_FIRST_DWELLING])
                           == giMonthTypeExtra)
                ...
                if (cell0->m_triggerType == MAP_OBJECT_NONE && !cell0->m_objectLayerBit1
                    ...
                    if (Random(MONSTER_SPAWN_MIN, MONSTER_SPAWN_MAX)
                        ...
                        if (Random(MONSTER_GUARD_ROLL_MIN, MONSTER_GUARD_ROLL_MAX)
                            < MONSTER_GUARD_CUTOFF)
                            cell0->m_objectMetadata |= H2EnumIndex(MAP_MONSTER_GUARD_FLAG);
                    }
                }
            }
        }
...
void game::RandomizeTown(i32 x, i32 y, i32) {
    ...
    ConvertObject(
        ...
        H2EnumIndex(race0) << RANDOM_TOWN_RACE_FRAME_SHIFT,
...
void game::RandomizeMine(i32 x, i32 y) {
    ...
    i32 mineId;
    MineType mineType29;
    H2EnumStorage<TerrainType, i32> terrain3;
    i32 columnOffset4;
    i32 retry4;
    i32 rowOffset0;
    u8 mineFrame36;
    ...
    for (retry4 = 0; retry4 < RANDOM_MINE_RETRY_LIMIT; retry4++) {
        switch (terrain3) {
            case TERRAIN_GRASS:
            case TERRAIN_DIRT:
                mineType29 = static_cast<MineType>(Random(H2EnumIndex(MINE_TYPE_MERCURY), H2EnumIndex(MINE_TYPE_GOLD)));
                if (mineType29 == MINE_TYPE_MERCURY)
                    mineType29 = MINE_TYPE_WOOD;
                break;
            case TERRAIN_SNOW:
                mineType29 = static_cast<MineType>(Random(H2EnumIndex(MINE_TYPE_ORE), H2EnumIndex(MINE_TYPE_GOLD)));
                break;
            case TERRAIN_SWAMP:
                mineType29 = static_cast<MineType>(Random(H2EnumIndex(MINE_TYPE_WOOD), H2EnumIndex(MINE_TYPE_GOLD)));
                break;
            case TERRAIN_LAVA:
        ...
        if (RandMineQty[H2EnumIndex(mineType29)] == 0)
            retry4 = RANDOM_MINE_RETRY_LIMIT;
    }
    RandMineQty[H2EnumIndex(mineType29)]++;
    ...
    if (mineType29 == MINE_TYPE_MERCURY) {
        ...
        m_worldMap.ChangeTilesetIndex(
            ...
            y,
            TILESET_EXTRAOVR,
            H2EnumIndex(mineType29) - H2EnumIndex(MINE_TYPE_ORE),
            0,
            -1
        );
    ...
    for (rowOffset0 = 0; rowOffset0 < RANDOM_MINE_FOOTPRINT_WIDTH; rowOffset0++) {
        for (columnOffset4 = 0; columnOffset4 < RANDOM_MINE_FOOTPRINT_WIDTH; columnOffset4++) {
            if ((WORLDMAP->GetCell(columnOffset4 + x, y - rowOffset0)->m_triggerType
                 & MAP_TRIGGER_TYPE_MASK)
                > MAP_OBJECT_NONE)
                if (H2EnumIndex(WORLDMAP->GetCell(columnOffset4 + x, y - rowOffset0)->m_triggerType
                        & MAP_TRIGGER_TYPE_MASK)
...
i32 game::GetRandomArtifactId(
    ...
    while (1) {
        ...
        if (!(H2EnumIndex((levelMask) & (gArtifactLevel[H2EnumIndex(artifact)]))))
            continue;
        if (artifact == ARTIFACT_EDITOR_ANY_ULTIMATE || artifact == ARTIFACT_EDITOR_UNUSED_84
            ...
            continue;
        if (attempts++ < ARTIFACT_UNIQUE_RETRIES && m_randomArtifacts[H2EnumIndex(artifact)])
            continue;
...
void game::SetRandomHeroArmies(i32 heroId, i32 strongArmy) {
    ...
    RandomHeroArmyRange armyTable7[H2EnumIndex(FACTION_COUNT)][RANDOM_HERO_ARMY_OPTION_COUNT] = {
    ...
    for (index9 = 0; index9 < RANDOM_HERO_ARMY_SELECTION_COUNT; index9++) {
        if (selected9[index9]) {
            army2->m_creatureTypes[armySlot7] =
                static_cast<i8>(armyTable7[H2EnumIndex(m_heroRecs[heroId].m_cursorType)][index9].creature);
            minimum5 = armyTable7[H2EnumIndex(m_heroRecs[heroId].m_cursorType)][index9].minimum
                       * RANDOM_HERO_COUNT_SCALE;
            maximum5 = armyTable7[H2EnumIndex(m_heroRecs[heroId].m_cursorType)][index9].maximum
                           * RANDOM_HERO_COUNT_SCALE
...
i32 game::ExperienceValueOfStack(armyGroup* group, hero* h) {
    ...
    for (i = 0; i < ARMY_GROUP_SLOT_COUNT; i++) {
        if (group->m_quantities[i] > 0) {
            exp += gMonsterDatabase[H2EnumIndex(group->m_creatureTypes[i])].hitPoints
        ...
        for (x = 0; x < MAP_WIDTH; x++) {
            for (y = 0; y < MAP_HEIGHT; y++) {
                if (gpAdvManager->FindAdjacentMonster(x, y, &col, &row, -1, -1))
                    mapExtra[y * MAP_WIDTH + x] |= H2EnumIndex(MAP_EXTRA_ADJACENT_MONSTER);
                else
                    mapExtra[y * MAP_WIDTH + x] &= H2EnumIndex(mask);
            }
        }
    }
}
...
void game::SetupTowns(void) {
    ...
    for (townIndex = 0; townIndex < GAME_TOWN_COUNT; townIndex++) {
        ...
        if (extra->hasCustomBuildings) {
            castle->m_buildings =
                (gTownEligibleBuildMask[H2EnumIndex(castle->m_type)] & extra->buildings)
            ...
            castle->m_buildings |= H2EnumIndex(TOWN_BUILDING_DWELLING_1);
        ...
        if (castle->m_type == FACTION_NECROMANCER && castle->m_owner != -1
            ...
            spellLevel = H2EnumIndex(gsSpellInfo[H2EnumIndex(spell)].level) - 1;
        ...
        for (spellLevel = 0; spellLevel < TOWN_MAGE_GUILD_LEVEL_COUNT; spellLevel++) {
            ...
            for (spellSlot = 0; spellSlot < TOWN_MAGE_GUILD_SPELLS_PER_LEVEL; spellSlot++) {
                if (castle->m_spellSlots[spellLevel * TOWN_MAGE_GUILD_SPELLS_PER_LEVEL + spellSlot]
                    != SPELL_NONE) {
                    usedSpells[H2EnumIndex(castle->m_spellSlots
                    ...
                    do {
                        ...
                        while (H2EnumIndex(gsSpellInfo[H2EnumIndex(spell)].level) - 1 != spellLevel)
                            spell = SpellType(
                                Random(H2EnumIndex(SPELL_FIREBALL), H2EnumIndex(SPELL_SET_WATER_GUARDIAN))
                            );
                        if (castle->m_owner != -1 && !gbHumanPlayer[castle->m_owner])
                            spellValue =
                                ((H2EnumIndex((gsSpellInfo[H2EnumIndex(spell)].attributes) & (SPELL_INFO_ATTRIBUTE_POWER)))
                                     ? 4
                                     : 1)
                                    * gsSpellInfo[H2EnumIndex(spell)].aiValue
                                + 50;
                        else
                            spellValue = 1500;
                        ...
                        || gsSpellInfo[H2EnumIndex(spell)].raceChance[H2EnumIndex(castle->m_type)] < Random(0, 10)
                        || attempts++ > 500 || usedSpells[H2EnumIndex(spell)]
                    ...
                    castle
                        ...
                        spell;
                    usedSpells[H2EnumIndex(spell)] = 1;
                }
            }
        }
                ...
                if ((cell5->m_triggerType & MAP_TRIGGER_TYPE_MASK) == MAP_OBJECT_HERO
                    ...
                    if (pass19 == MAP_HERO_CLASS_PASS) {
                        ...
                        if (extra0->hasAssignedHero) {
                            ...
                            heroId1 = RandomScan(
                                usedHeroes4,
                                H2EnumIndex(heroClass6) * MAP_HEROES_PER_FACTION,
                    ...
                    if (pass19 == MAP_HERO_PLACEMENT_PASS) {
                        ...
                        if (extra0->hasCustomSkills) {
                            ...
                            for (recordPosition = 0;
                                 recordPosition < H2EnumIndex(HERO_SKILL_COUNT);
                                 recordPosition++) {
                        ...
                        if (!isJail6) {
                            SetVisibility(
                                ...
                                mapHero0->m_y,
                                mapHero0->m_owner,
                                giVisRange[H2EnumIndex(mapHero0->m_secondarySkills
...
void CreateDiffFile(
    ...
    sprintf(gText, "%s%s", ".\\DATA\\", joinName);
    joinFile1 = platform::FileOpen(gText, platform::FileMode::Read);
    if (joinFile1 == -1)
        FileError(gText);
    platform::FileRead(joinFile1, joinData29, joinSize36);
    platform::FileClose(joinFile1);
    LogInt(
    ...
    if (!forceWhole) {
        ...
        sprintf(gText, "%s%s", ".\\DATA\\", oldName);
        oldFile17 = platform::FileOpen(gText, platform::FileMode::Read);
        if (oldFile17 == -1)
            FileError(gText);
        platform::FileRead(oldFile17, oldData13, oldSize37);
        platform::FileClose(oldFile17);
    }
    ...
    joinFile1 = platform::FileOpen(gText, platform::FileMode::Write);
    if (joinFile1 == -1)
        FileError(gText);
    platform::FileWrite(joinFile1, diffData6, diffSize29);
    platform::FileClose(joinFile1);
    ...
    if (joinFile1 == -1)
        FileError(gText);
    platform::FileWrite(joinFile1, joinData29, joinSize36);
    platform::FileClose(joinFile1);
...
void CreateJoinFile(char* oldName, char* diffName, char* joinName) {
    ...
    sprintf(gText, "%s%s", ".\\DATA\\", diffName);
    diffFile2 = platform::FileOpen(gText, platform::FileMode::Read);
    if (diffFile2 == -1)
        FileError(gText);
    platform::FileRead(diffFile2, diffData5, diffSize1);
    platform::FileClose(diffFile2);
    ...
    if (diffData5[0] == 0) {
        ...
        if (diffFile2 == -1)
            FileError(gText);
        platform::FileRead(diffFile2, oldData13, oldSize10);
        platform::FileClose(diffFile2);
    ...
    sprintf(gText, "%s%s", ".\\DATA\\", joinName);
    joinFile0 = platform::FileOpen(gText, platform::FileMode::Write);
    if (joinFile0 == -1)
        FileError(gText);
    platform::FileWrite(joinFile0, joinData9, joinSize37);
    platform::FileClose(joinFile0);
    LogInt(
...
void game::SetupNewRumour(void) {
    ...
    if (Random(0, 100) < 30) {
        ...
        if (roll2 < 80 && giCurTurn > 1) {
            ...
            while (attempts13++ < 200) {
                ...
                if (categoryStats7[1] != categoryStats7[0]) {
                    if (selectionRoll == H2EnumIndex(THIEVES_CATEGORY_OBELISKS))
                        sprintf(
                            m_rumour,
                            "%s has found the most obelisks.",
                            cPlayerNames[categoryOrder2[0]]
                        );
                    else if (selectionRoll == H2EnumIndex(THIEVES_CATEGORY_ARTIFACTS))
                        sprintf(
                            m_rumour,
                            "%s has found the most artifacts.",
                            cPlayerNames[categoryOrder2[0]]
                        );
                    else if (selectionRoll == H2EnumIndex(THIEVES_CATEGORY_ARMY_STRENGTH))
                        sprintf(
                            m_rumour,
        ...
        ultimateRumour:
            ...
            if (selectionRoll < 33) {
                if (!(H2EnumIndex(m_mapHeader.width) * 0.33 <= m_ultimateArtifactX
                      || H2EnumIndex(m_mapHeader.height) * 0.33 <= m_ultimateArtifactX)) {
                    direction9 = 7;
                } else if (!(H2EnumIndex(m_mapHeader.width) * 0.33 <= m_ultimateArtifactX
                             || m_ultimateArtifactX <= H2EnumIndex(m_mapHeader.height) * 0.66)) {
                    direction9 = 5;
                } else if (!(H2EnumIndex(m_mapHeader.width) * 0.33 <= m_ultimateArtifactX)) {
                    direction9 = 6;
                } else if (!(m_ultimateArtifactX <= H2EnumIndex(m_mapHeader.width) * 0.66
                             || H2EnumIndex(m_mapHeader.height) * 0.33 <= m_ultimateArtifactX)) {
                    direction9 = 1;
                } else if (!(m_ultimateArtifactX <= H2EnumIndex(m_mapHeader.width) * 0.66
                             || m_ultimateArtifactX <= H2EnumIndex(m_mapHeader.height) * 0.66)) {
                    ...
                    direction9 = 2;
                } else if (!(H2EnumIndex(m_mapHeader.height) * 0.33 <= m_ultimateArtifactX)) {
                    direction9 = 0;
                } else if (!(m_ultimateArtifactX <= H2EnumIndex(m_mapHeader.height) * 0.66)) {
                    direction9 = 4;
                } else {
                    direction9 = 8;
                }
...
i32 CalcFileCRC(char* filename) {
    ...
    i32 hand = platform::FileOpen(filename, platform::FileMode::Read);
    if (hand == -1)
        FileError(filename);
    platform::FileRead(hand, block, size);
    i32 crc = calc_crc_long(reinterpret_cast<u8*>(block), size);
    platform::FileClose(hand);
    H2_FREE(block);
    return crc;
}
...
void CompressTest(void) {
    ...
    fileHandle4 = platform::FileOpen(filename3, platform::FileMode::Read);
    if (fileHandle4 == -1)
        FileError(filename3);
    platform::FileRead(fileHandle4, sourceData6, fileSize7);
    LogStr(const_cast<char*>("C3"));
    sourceCrc0 = calc_crc_long(reinterpret_cast<u8*>(sourceData6), fileSize7);
    LogStr(const_cast<char*>("C4"));
    platform::FileClose(fileHandle4);
