...
i32 game::GetNewHeroId(i32, FactionType heroClass, i32 requireExperienced) {
    ...
    while (attempts < HERO_SELECTION_RETRY_LIMIT) {
        attempts++;
        heroId = Random(0, (GAME_HERO_COUNT) - 1);
...
i32 game::SaveGame(char* filename, i32 generateName, i8 expansionFormat) {
    ...
    if (!expansionFormat)
        write(handle, m_randomArtifacts, (ARTIFACT_COUNT));
    else
...
void game::GiveTroopsToNeutralTown(i32 townId) {
    ...
    if ((m_castleRecs[townId].m_x > 0 || m_castleRecs[townId].m_y > 0)
        ...
        switch ((m_castleRecs[townId].m_type) + tierKey36) {
...
void game::NewMap(char* filename) {
    ...
    for (player2 = 0; player2 < m_playerCount; player2++) {
        ...
        }
        {
            ...
            heroClass5 = static_cast<FactionType>(Random(0, (FACTION_COUNT) - 1));
    ...
    secondHero:
        heroClass5 = (heroClass5 + Random(1, (FACTION_COUNT) - 1)) % (FACTION_COUNT);
    ...
    for (player2 = 0; player2 < m_playerCount; player2++) {
        if (gbHumanPlayer[player2]) {
            ...
            if (m_playerHandicap[player2] != PLAYER_HANDICAP_NONE) {
                for (townIndex9 = 0; townIndex9 < (RES_COUNT); townIndex9++) {
...
void game::RandomizeEvents(void) {
        ...
        for (xPos2 = 0; xPos2 < MAP_WIDTH; xPos2++) {
            ...
            switch (cell2->m_triggerType) {
                ...
                case MAP_TRIGGER_ACTION_FLAG | MAP_OBJECT_RESOURCE: {
                    ...
                    cell2->m_objectMetadata = (resourceType);
                    switch (resourceType) {
                        case RES_WOOD:
                        case RES_ORE:
                            cell2->m_objectMetadata =
                ...
                case MAP_TRIGGER_ACTION_FLAG | MAP_OBJECT_ARTIFACT:
                    ...
                    if (value26 != ARTIFACT_SPELL_SCROLL) {
                        if (randomValue7 < ARTIFACT_EVENT_UNCONDITIONAL_CUTOFF) {
                            ...
                            if (gArtifactLevel[(value26)] == ARTIFACT_LEVEL_TREASURE)
                                cell2->m_objectMetadata = ARTIFACT_EVENT_MODE_GOLD;
                            else if (gArtifactLevel[(value26)] == ARTIFACT_LEVEL_MINOR)
                                cell2->m_objectMetadata =
                                    (Random((RES_WOOD), (RES_GEMS))
                                     << ARTIFACT_EVENT_RESOURCE_SHIFT)
                                    | ARTIFACT_EVENT_MODE_RESOURCE_3;
                            else if (gArtifactLevel[(value26)] == ARTIFACT_LEVEL_MAJOR)
                                cell2->m_objectMetadata =
                                    (Random((RES_WOOD), (RES_GEMS))
                                     << ARTIFACT_EVENT_RESOURCE_SHIFT)
                            ...
                            if (gArtifactLevel[(value26)] == ARTIFACT_LEVEL_TREASURE)
                                ...
                                cell2->m_objectMetadata |= (artifactGuardianChoices[Random(
                                    EVENT_ROLL_MIN,
    ...
    for (yPos19 = 0; yPos19 < MAP_HEIGHT; yPos19++) {
        for (xPos2 = 0; xPos2 < MAP_WIDTH; xPos2++) {
            ...
            upperCount = 0;
            lowerCount16 = 0;
            if (!(cell2->m_flags & (MAP_CELL_OCCUPIED)) && yPos19 < MAP_HEIGHT - 1
                && cell2->m_objectIndex != MAPCELL_SPRITE_NONE
                && !(cell2->m_triggerType & MAP_TRIGGER_ACTION_FLAG)
                && !(cell2->m_flags & (MAP_CELL_OBJECT_SHADOW_ONLY))) {
                mapCell* below0;
...
void game::UpdateSpellWidgets(void) {
    ...
    message9.payload.widget.data.value = (WIDGET_FLAG_ENABLED | WIDGET_FLAG_DRAW);
    ...
    for (spellSlot6 = 0; spellSlot6 < VIEW_SPELL_PAGE_SIZE; spellSlot6++) {
        if (m_viewSpellsTop[(m_viewSpellsType)] + spellSlot6
            ...
            if (m_viewSpellsReadOnly) {
                message9.payload.widget.command = WIDGET_COMMAND_SET_FLAGS;
                message9.payload.widget.data.value = (WIDGET_FLAG_ENABLED);
                m_viewSpellsWindow->BroadcastMessage(message9);
            }
            message9.payload.widget.command = WIDGET_COMMAND_SET_FRAME;
            message9.payload.widget.data.value = gsSpellInfo[(spell2)].iconIndex;
            m_viewSpellsWindow->BroadcastMessage(message9);
            lineLength0 = smallFont->LineLength(gSpellNames[(spell2)], VIEW_SPELL_NAME_WIDTH);
            if (lineLength0 == 1) {
                sprintf(
                    gText,
                    "%s\n[%d]",
...
void game::ViewArmy(
    ...
    if (castle && (gpAdvManager->m_active == 1 || gpTownManager->m_active == 1)) {
        for (loopIndex0 = (BUILDING_SLOT_DWELLING_SECOND);
             loopIndex0 <= (BUILDING_SLOT_DWELLING_SIXTH);
             loopIndex0++) {
            if (gDwellingType[(castle->m_type)]
                             [loopIndex0 - (BUILDING_SLOT_DWELLING_FIRST)]
                    == monsterType
                && (castle->m_buildings
    ...
    if (!gbAllowUpgrade) {
        ...
        message6.payload.widget.data.value = (WIDGET_FLAG_ENABLED | WIDGET_FLAG_DRAW);
...
void game::PerDay(void) {
    ...
    for (player = 0; player < GAME_HERO_COUNT; player++) {
        ...
        restoredSpellPoints13 +=
            (currentHero6->m_secondarySkills[(HERO_SKILL_MYSTICISM)]) + 1;
    ...
    for (player = 0; player < GAME_TOWN_COUNT; player++) {
        ...
        if (!(currentTown4->m_buildings & (1 << (BUILDING_SLOT_MAGE_GUILD))))
            continue;
...
void game::PerWeek(void) {
    ...
    for (outerIndex5 = 0; outerIndex5 < GAME_PLAYER_COUNT; outerIndex5++) {
        for (innerIndex3 = 0; innerIndex3 < AVAILABLE_HERO_SLOTS; innerIndex3++) {
            ...
            heroClass18 =
                (heroClass18 + Random(1, (FACTION_COUNT) - 1)) % (FACTION_COUNT);
...
void game::RandomizeMine(i32 x, i32 y) {
    ...
    for (retry4 = 0; retry4 < RANDOM_MINE_RETRY_LIMIT; retry4++) {
        switch (terrain3) {
            case TERRAIN_GRASS:
            case TERRAIN_DIRT:
                mineType29 = static_cast<MineType>(Random((MINE_TYPE_MERCURY), (MINE_TYPE_GOLD)));
                if (mineType29 == MINE_TYPE_MERCURY)
                    mineType29 = MINE_TYPE_WOOD;
                break;
            case TERRAIN_SNOW:
                mineType29 = static_cast<MineType>(Random((MINE_TYPE_ORE), (MINE_TYPE_GOLD)));
                break;
            case TERRAIN_SWAMP:
                mineType29 = static_cast<MineType>(Random((MINE_TYPE_WOOD), (MINE_TYPE_GOLD)));
                break;
            case TERRAIN_LAVA:
                    ...
                    if (pass19 == MAP_HERO_PLACEMENT_PASS) {
                        ...
                        if (!isJail6) {
                            SetVisibility(
                                ...
                                mapHero0->m_y,
                                mapHero0->m_owner,
                                giVisRange[(mapHero0->m_secondarySkills
