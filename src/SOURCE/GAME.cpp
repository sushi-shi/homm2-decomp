...
typedef enum NewMapConstant {
    ...
    NECROMANCER_SHRINE_BUILD_MASK              = 1 << (BUILDING_SLOT_NECROMANCER_SHRINE),
...
typedef enum WeeklyRuntimeConstant {
    ...
    CREATURE_WEEK_LAST            = (CREATURE_BONE_DRAGON),
...
typedef enum GameRandomArtifactConstant {
    ARTIFACT_BASE_TABLE_SIZE      = (ARTIFACT_MAGIC_BOOK) + 1,
    ARTIFACT_FIRST                = (ARTIFACT_ULTIMATE_BOOK),
    ARTIFACT_BASE_LAST            = (ARTIFACT_MAGIC_BOOK),
    ARTIFACT_EXPANSION_LAST       = (ARTIFACT_SPADE_NECROMANCY),
...
inline MineType RandomMineType(MineType first, MineType last) {
    return static_cast<MineType>(Random((first), (last)));
}
...
typedef enum WitchHutConstant {
    WITCH_HUT_SKILL_FIRST = (HERO_SKILL_PATHFINDING),
    WITCH_HUT_SKILL_LAST  = (HERO_SKILL_ESTATES)
} WitchHutConstant;
...
i32 game::SetupPuzzlePieces(i32 player, i32 justCount) {
    ...
    float interp =
        ...
        / (PUZZLE_INTERPOLATION_TERM_COUNT)
        ;
...
i32 game::GetNewHeroId(i32, FactionType heroClass, i32 requireExperienced) {
    ...
    while (attempts < HERO_SELECTION_RETRY_LIMIT) {
        attempts++;
        heroIndex = Random(0, (GAME_HERO_COUNT) - 1);
...
i32 game::SaveGame(const char* filename, i32 generateName, i8 expansionFormat) {
    ...
    if (!expansionFormat)
        write(outFile, m_randomArtifacts, (ARTIFACT_COUNT));
    else
...
void game::LoadGame(const char* filename, i32 loadFromFile, i32) {
    ...
    if (expTag)
        read(fileDescriptor, m_randomArtifacts, (ARTIFACT_COUNT));
    else
...
void game::GiveTroopsToNeutralTown(i32 townId) {
    ...
    if ((m_castleRecs[townId].m_x > 0 || m_castleRecs[townId].m_y > 0)
        ...
        switch (tierBase + (m_castleRecs[townId].m_type)) {
            case REINFORCEMENT_TIER_ONE_KEY + (FACTION_KNIGHT):
                monster = CREATURE_PEASANT;
                break;
            case REINFORCEMENT_TIER_TWO_KEY + (FACTION_KNIGHT):
                monster = CREATURE_ARCHER;
                break;
            case REINFORCEMENT_TIER_THREE_KEY + (FACTION_KNIGHT):
                monster = CREATURE_PIKEMAN;
                break;
            case REINFORCEMENT_TIER_FOUR_KEY + (FACTION_KNIGHT):
                monster = CREATURE_SWORDSMAN;
                break;
            case REINFORCEMENT_TIER_FIVE_KEY + (FACTION_KNIGHT):
                monster = CREATURE_CAVALRY;
                break;
            case REINFORCEMENT_TIER_ONE_KEY + (FACTION_BARBARIAN):
                monster = CREATURE_GOBLIN;
                break;
            case REINFORCEMENT_TIER_TWO_KEY + (FACTION_BARBARIAN):
                monster = CREATURE_ORC;
                break;
            case REINFORCEMENT_TIER_THREE_KEY + (FACTION_BARBARIAN):
                monster = CREATURE_WOLF;
                break;
            case REINFORCEMENT_TIER_FOUR_KEY + (FACTION_BARBARIAN):
                monster = CREATURE_OGRE;
                break;
            case REINFORCEMENT_TIER_FIVE_KEY + (FACTION_BARBARIAN):
                monster = CREATURE_TROLL;
                break;
            case REINFORCEMENT_TIER_ONE_KEY + (FACTION_SORCERESS):
                monster = CREATURE_SPRITE;
                break;
            case REINFORCEMENT_TIER_TWO_KEY + (FACTION_SORCERESS):
                monster = CREATURE_DWARF;
                break;
            case REINFORCEMENT_TIER_THREE_KEY + (FACTION_SORCERESS):
                monster = CREATURE_ELF;
                break;
            case REINFORCEMENT_TIER_FOUR_KEY + (FACTION_SORCERESS):
                monster = CREATURE_DRUID;
                break;
            case REINFORCEMENT_TIER_FIVE_KEY + (FACTION_SORCERESS):
                monster = CREATURE_UNICORN;
                break;
            case REINFORCEMENT_TIER_ONE_KEY + (FACTION_WARLOCK):
                monster = CREATURE_CENTAUR;
                break;
            case REINFORCEMENT_TIER_TWO_KEY + (FACTION_WARLOCK):
                monster = CREATURE_GARGOYLE;
                break;
            case REINFORCEMENT_TIER_THREE_KEY + (FACTION_WARLOCK):
                monster = CREATURE_GRIFFIN;
                break;
            case REINFORCEMENT_TIER_FOUR_KEY + (FACTION_WARLOCK):
                monster = CREATURE_MINOTAUR;
                break;
            case REINFORCEMENT_TIER_FIVE_KEY + (FACTION_WARLOCK):
                monster = CREATURE_HYDRA;
                break;
            case REINFORCEMENT_TIER_ONE_KEY + (FACTION_WIZARD):
                monster = CREATURE_HALFLING;
                break;
            case REINFORCEMENT_TIER_TWO_KEY + (FACTION_WIZARD):
                monster = CREATURE_BOAR;
                break;
            case REINFORCEMENT_TIER_THREE_KEY + (FACTION_WIZARD):
                monster = CREATURE_IRON_GOLEM;
                break;
            case REINFORCEMENT_TIER_FOUR_KEY + (FACTION_WIZARD):
                monster = CREATURE_ROC;
                break;
            case REINFORCEMENT_TIER_FIVE_KEY + (FACTION_WIZARD):
                monster = CREATURE_MAGE;
                break;
            case REINFORCEMENT_TIER_ONE_KEY + (FACTION_NECROMANCER):
                monster = CREATURE_SKELETON;
                break;
            case REINFORCEMENT_TIER_TWO_KEY + (FACTION_NECROMANCER):
                monster = CREATURE_ZOMBIE;
                break;
            case REINFORCEMENT_TIER_THREE_KEY + (FACTION_NECROMANCER):
                monster = CREATURE_MUMMY;
                break;
            case REINFORCEMENT_TIER_FOUR_KEY + (FACTION_NECROMANCER):
                monster = CREATURE_VAMPIRE;
                break;
            case REINFORCEMENT_TIER_FIVE_KEY + (FACTION_NECROMANCER):
...
void game::NewMap(char* filename) {
    ...
    if (xIsExpansionMap)
        gTownEligibleBuildMask[(FACTION_NECROMANCER)] |= NECROMANCER_SHRINE_BUILD_MASK;
    else
        gTownEligibleBuildMask[(FACTION_NECROMANCER)] &= ~NECROMANCER_SHRINE_BUILD_MASK;
    ...
    for (player = 0; player < m_playerCount; player++) {
                ...
                for (nTown = 0; nTown < m_players[player].m_townCount; nTown++) {
                    if (selectedTown == -1
                        && m_castleRecs[(m_players + player)->m_townIds[nTown]].m_occupyingHeroId
                               == -1
                        && ((((m_castleRecs[(m_players + player)->m_townIds[nTown]].m_buildings) & ((TOWN_BUILDING_CASTLE))))
                                != 0
        ...
        if (selectedTown != -1) {
            ...
            SetVisibility(
                ...
                m_heroRecs[m_players[player].m_heroIds[m_players[player].m_heroCount]].m_y,
                player,
                giVisRange[(m_heroRecs[m_players[player].m_heroIds[0]]
                                   .m_secondarySkills[(HERO_SKILL_SCOUTING)])]
            );
    ...
    for (player = 0; player < m_playerCount; player++) {
        if (player == 0 && gbInCampaign
            && (m_campaignAwards[(CAMPAIGN_AWARD_SORCERESS_GUILD)] != 0
                || m_campaignAwards[(CAMPAIGN_AWARD_NECROMANCER_GUILD)] != 0)) {
            specClass = m_campaignAwards[(CAMPAIGN_AWARD_SORCERESS_GUILD)]
                                  ? FACTION_SORCERESS
                                  : FACTION_NECROMANCER;
        ...
        }
        {
            ...
            startClass = static_cast<FactionType>(Random(0, (FACTION_COUNT) - 1));
    ...
    secondHero:
        startClass = (startClass + Random(1, (FACTION_COUNT) - 1)) % (FACTION_COUNT);
    ...
    m_ultimateArtifactId =
        static_cast<ArtifactType>(Random((ARTIFACT_ULTIMATE_BOOK), (ARTIFACT_GOLDEN_GOOSE)));
    if (gbInCampaign
    ...
    for (player = 0; player < m_playerCount; player++) {
        if (gbHumanPlayer[player]) {
            ...
            if (m_playerHandicap[player] != PLAYER_HANDICAP_NONE) {
                for (nTown = 0; nTown < (RES_COUNT); nTown++) {
...
void game::RandomizeEvents(void) {
        ...
        for (xPosition = 0; xPosition < MAP_WIDTH; xPosition++) {
            ...
            switch (cell->m_triggerType) {
                case MAP_ACTION_TRIGGER(MAP_OBJECT_WITCH_HUT):
                    cell->m_objectMetadata = (HERO_SKILL_NECROMANCY);
                    while (cell->m_objectMetadata == (HERO_SKILL_NECROMANCY)
                           || cell->m_objectMetadata == (HERO_SKILL_LEADERSHIP)) {
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_FLOTSAM):
                    cell->m_objectMetadata =
                        Random((FLOTSAM_EMPTY), (FLOTSAM_LARGE_TREASURE));
                    break;
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_CAMPFIRE):
                    ...
                    cell->m_objectMetadata |= Random((RES_WOOD), (RES_GEMS));
                    break;
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_DERELICT_SHIP):
                    switch (Random(EVENT_ROLL_MIN, EVENT_BUCKET_ROLL_MAX) % EVENT_BUCKET_COUNT) {
                        ...
                        case 1:
                        case 2:
                            cell->m_objectMetadata = (EVENT_LEVEL_SMALL);
                            break;
                        case 3:
                        case 4:
                        case 5:
                            cell->m_objectMetadata = (EVENT_LEVEL_MEDIUM);
                            break;
                        case 6:
                        case 7:
                        case 8:
                            cell->m_objectMetadata = (EVENT_LEVEL_LARGE);
                            break;
                        case 9:
                            cell->m_objectMetadata = (EVENT_LEVEL_HUGE);
                            break;
                    }
                    break;
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_MAGIC_GARDEN):
                    cell->m_objectMetadata =
                        Random(EVENT_ROLL_MIN, EVENT_BINARY_ROLL_MAX) == EVENT_ROLL_MIN
                            ? (RES_GEMS) + MAP_EVENT_RESOURCE_OFFSET
                            : (RES_GOLD) + MAP_EVENT_RESOURCE_OFFSET;
                    break;
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_MONSTER):
                    if (cell->m_objectMetadata == MAP_EVENT_DATA_EMPTY) {
                        ...
                        if (cell->m_objectIndex != (CREATURE_GHOST)
                            && cell->m_objectIndex != (CREATURE_EARTH_ELEMENTAL)
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_SHRINE_FIRST_CIRCLE):
                    cell->m_objectMetadata =
                        Random((SPELL_FIREBALL), (SPELL_COUNT) - 1) + MAP_EVENT_SPELL_OFFSET;
                    while (gsSpellInfo[cell->m_objectMetadata - MAP_EVENT_SPELL_OFFSET].level
                           != SPELL_LEVEL_FIRST)
                        cell->m_objectMetadata = Random((SPELL_FIREBALL), (SPELL_COUNT) - 1)
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_ARTIFACT):
                    ...
                    if (randomValue < ARTIFACT_EVENT_UNCONDITIONAL_CUTOFF) {
                        ...
                        if (gArtifactLevel[(value)] == ARTIFACT_LEVEL_TREASURE)
                            cell->m_objectMetadata = ARTIFACT_EVENT_MODE_GOLD;
                        else if (gArtifactLevel[(value)] == ARTIFACT_LEVEL_MINOR)
                            cell->m_objectMetadata =
                                (Random((RES_WOOD), (RES_GEMS))
                                 << ARTIFACT_EVENT_RESOURCE_SHIFT)
                                | ARTIFACT_EVENT_MODE_RESOURCE_3;
                        else if (gArtifactLevel[(value)] == ARTIFACT_LEVEL_MAJOR)
                            cell->m_objectMetadata =
                                (Random((RES_WOOD), (RES_GEMS))
                                 << ARTIFACT_EVENT_RESOURCE_SHIFT)
                        ...
                        if (gArtifactLevel[(value)] == ARTIFACT_LEVEL_TREASURE)
                            cell->m_objectMetadata |= (CREATURE_ROGUE);
                        else if (gArtifactLevel[(value)] == ARTIFACT_LEVEL_MINOR)
                            cell->m_objectMetadata |= (artifactGuardianChoices[Random(
                                EVENT_ROLL_MIN,
    ...
    for (yPosition = 0; yPosition < MAP_HEIGHT; yPosition++) {
        for (xPosition = 0; xPosition < MAP_WIDTH; xPosition++) {
            ...
            upperCount = 0;
            lowerCount = 0;
            if (!(cell->m_flags & (MAP_CELL_OCCUPIED)) && yPosition < MAP_HEIGHT - 1
                && cell->m_objectIndex != MAPCELL_SPRITE_NONE
                && !(cell->m_triggerType & MAP_TRIGGER_ACTION_FLAG)
                && !(cell->m_flags & (MAP_CELL_OBJECT_SHADOW_ONLY))) {
                mapCell* below;
...
i32 game::LoadMap(char* filename) {
    ...
    for (i = 0; i < GAME_TOWN_COUNT; i++) {
        ...
        if (x[0] != SAVED_TOWN_OFF_MAP) {
            ...
            if (type[0] < 0)
                m_castleRecs[i].m_buildings |= (TOWN_BUILDING_CASTLE);
            else
                m_castleRecs[i].m_buildings |= (TOWN_BUILDING_TENT);
        }
    }
...
game::ViewSpells(
    ...
    if (spellHero->GetNumSpells(spellType) == 0) {
        ...
        m_viewSpellsTop[(SPELL_TYPE_COMBAT)] = 0;
        m_viewSpellsCount[(SPELL_TYPE_COMBAT)] =
            spellHero->GetNumSpells(SPELL_TYPE_COMBAT);
        m_viewSpellsTop[(SPELL_TYPE_ADVENTURE)] = 0;
        m_viewSpellsCount[(SPELL_TYPE_ADVENTURE)] =
...
void game::UpdateSpellWidgets(void) {
    ...
    message.type = MESSAGE_WIDGET;
    message.payload.widget.command = (spellPoints <= VIEW_SPELL_MANA_HUNDREDS_THRESHOLD)
    ...
    message.type = MESSAGE_WIDGET;
    message.payload.widget.command = (spellPoints <= VIEW_SPELL_MANA_TENS_THRESHOLD)
        ? WIDGET_COMMAND_CLEAR_FLAGS : WIDGET_COMMAND_SET_FLAGS;
    ...
    for (i = 0; i < VIEW_SPELL_PAGE_SIZE; i++) {
        if (m_viewSpellsTop[(m_viewSpellsType)] + i
            ...
            spell = m_viewSpellsHero->GetNthSpell(
                m_viewSpellsType,
                m_viewSpellsTop[(m_viewSpellsType)] + i + 1
            );
            ...
            if (GetManaCost(spell, m_viewSpellsHero) > m_viewSpellsHero->m_spellPoints)
                message.payload.widget.data.value = (FONT_DRAW_DIMMED);
            else
                message.payload.widget.data.value = (FONT_DRAW_DEFAULT);
            m_viewSpellsWindow->BroadcastMessage(message);

            lines = smallFont->LineLength(gSpellNames[(spell)], VIEW_SPELL_NAME_WIDTH);
            if (lines == 1) {
                sprintf(
                    gText,
                    "%s\n[%d]",
            ...
            message.payload.widget.data.value = gsSpellInfo[(spell)].iconIndex;
            m_viewSpellsWindow->BroadcastMessage(message);
        ...
        switch (message.payload.widget.command) {
            case WIDGET_NOTIFY_DESELECT:
                if (message.payload.widget.command == WIDGET_NOTIFY_RIGHT_CLICK
                    || ((((message.payload.widget.modifiers) & (MESSAGE_MODIFIER_RIGHT_BUTTON)))) != 0)
                    break;
                {
...
void game::ViewArmy(
    ...
    i16 titleMessage [[maybe_unused]];
    i16 detailMessage [[maybe_unused]];
    ...
    i16 baseX [[maybe_unused]];
    i32 loopIndex;
    i32 iconFrame;
    i16 numWidget [[maybe_unused]];
    i32 morale;
    i32 luck;
    ...
    if (castle && (gpAdvManager->m_active == 1 || gpTownManager->m_active == 1)) {
        for (loopIndex = (BUILDING_SLOT_DWELLING_SECOND);
             loopIndex <= (BUILDING_SLOT_DWELLING_SIXTH);
             loopIndex++) {
        ...
        if ((monsterType == CREATURE_GREEN_DRAGON || monsterType == CREATURE_RED_DRAGON)
            && (((castle->m_buildings) & ((TOWN_BUILDING_ALTERNATE_UPGRADED_DWELLING_6))))) {
            gbAllowUpgrade = true;
            iViewArmyUpgradeToType = CREATURE_BLACK_DRAGON;
        }
    }

    monster = &gMonsterDatabase[(monsterType)];
    if (theArmy)
        armyMonster = &theArmy->m_monster;
    else
        armyMonster = &gMonsterDatabase[(monsterType)];
    ...
    if (facing == ARMY_FACING_RIGHT)
        ...
        viewArmyFacingWIPXMod = 1;
    gpResourceManager
        ->PointToFile(gpResourceManager->MakeId(cArmyFrameFileNames[(monsterType)], 1));
    ...
    strcpy(filename, cMonFilename[(monsterType)]);

    monsterIcon = gpResourceManager->GetIcon(filename);
    iconFrame = sViewArmyMonFrameInfo.animationFrames[(ARMY_ANIMATION_WALK)][0];
    ...
    strcpy(armyName, gArmyNames[(monsterType)]);
    armyName[0] = ToUpperCp1251(armyName[0]);
    ...
    if (theGroup)
        ...
        morale = 0;
    if ((((monster->attributes) & (MONSTER_FLAGS_NO_MORALE))))
        morale = 0;
    ...
    if ((((monster->attributes) & (MONSTER_FLAGS_SHOOTER)))) {
    ...
    if (theArmy) {
        ...
        for (loopIndex = 0;
            ...
            for (; spellIndex < ARMY_SPELL_INFLUENCE_COUNT; spellIndex++) {
                if (theArmy->m_spellInfluence[(spellIndex)])
                    break;
            }
            ...
            case WIDGET_NOTIFY_DESELECT:
                switch (message.payload.widget.id) {
                    ...
                    case VIEW_ARMY_DISMISS_ID:
                        NormalDialog(
                            const_cast<char*>(
                                "Вы действительно хотите распустить этот отряд?"
                            ),
                    ...
                    case VIEW_ARMY_UPGRADE_ACTION_ID:
                        ...
                        if (gpCurPlayer->m_resources[(RES_GOLD)] >= goldCost
                            && (resourceType == RES_NONE
                                || gpCurPlayer->m_resources[(resourceType)] >= resourceCost)) {
                            NormalDialog(
                                const_cast<char*>(
                                    "Вы можете улучшить ваших воинов за некоторую сумму. Желаете улучшить их?"
                                ),
                                NORMAL_DIALOG_CONFIRM,
                                -1,
                                -1,
...
i32 game::ComputeDailyGold(i32 player) {
    ...
    for (index = 0; index < GAME_TOWN_COUNT; index++) {
        if (m_castleRecs[index].m_owner == player) {
            dailyGold += (((m_castleRecs[index].m_buildings) & ((1 << (BUILDING_SLOT_UPGRADE_CASTLE)))))
...
void game::PerDay(void) {
    ...
    for (player = 0; player < gpGame->m_playerCount; player++) {
        for (resource = RES_WOOD; resource < RES_COUNT; resource++) {
            gpGame->m_players[player].m_aiData.m_income[(resource)] =
                -m_players[player].m_resources[(resource)];
        }
    }
    ...
    for (player = 0; player < GAME_MINE_COUNT; player++) {
        if (m_mines[player].owner != -1) {
            ...
            if (mineType != MINE_TYPE_GOLD && mineType <= MINE_TYPE_GOLD)
                m_players[m_mines[player].owner].m_resources[(mineType)] += dailyIncome;
        }
    }
    ...
    if (xIsPlayingExpansionCampaign && xCampaign.HasAward(AWARD_WOOD_BONUS))
        m_players[0].m_resources[(RES_WOOD)] += DAILY_CAMPAIGN_WOOD_BONUS;
    ...
    for (player = 0; player < gpGame->m_playerCount; player++) {
        for (resource = RES_WOOD; resource < RES_GOLD; resource++) {
            if (m_playerHandicap[player] != PLAYER_HANDICAP_NONE) {
                m_players[player].m_resources[(resource)] -= static_cast<i32>(
                    (gpGame->m_players[player].m_aiData.m_income[(resource)]
                     + m_players[player].m_resources[(resource)])
    ...
    for (player = 0; player < GAME_HERO_COUNT; player++) {
        ...
        restoredSpellPoints +=
            (currentHero->m_secondarySkills[(HERO_SKILL_MYSTICISM)]) + 1;
        ...
        if ((((currentHero->m_eventFlags) & (HERO_EVENT_MAGIC_WELL))))
            ((currentHero->m_eventFlags) &= ~(HERO_EVENT_MAGIC_WELL));
    }

    for (player = 0; player < GAME_TOWN_COUNT; player++) {
        currentTown = GetTown(player);
        if (!(((currentTown->m_buildings) & ((1 << (BUILDING_SLOT_MAGE_GUILD))))))
            continue;
...
void game::PerWeek(void) {
    ...
    for (outerIndex = 0; outerIndex < GAME_TOWN_COUNT; outerIndex++) {
        ...
        for (innerIndex = (BUILDING_SLOT_DWELLING_FIRST); innerIndex <= (BUILDING_SLOT_DWELLING_LAST);
             innerIndex++) {
            if ((((castle->m_buildings) & ((1 << innerIndex))))) {
                creatureGrowth = gMonsterDatabase[(gDwellingType[(castle->m_type)]
                                                             [innerIndex - (BUILDING_SLOT_DWELLING_FIRST)])]
                               .growth;
                if ((((castle->m_buildings) & ((1 << (BUILDING_SLOT_SPECIAL_FOUR))))))
                    creatureGrowth += TOWN_WELL_BASE_GROWTH_BONUS;
                if (innerIndex == (BUILDING_SLOT_DWELLING_FIRST)
                    && (((castle->m_buildings) & ((1 << (BUILDING_SLOT_WELL_EXTRA))))))
    ...
    for (outerIndex = 0; outerIndex < GAME_HERO_COUNT; outerIndex++) {
        ...
        if ((((weeklyHero->m_eventFlags) & (HERO_EVENT_STABLES))))
...
void game::RandomizeTown(i32 x, i32 y, i32) {
    ...
    if (townExtra->color == RANDOM_TOWN_UNOWNED_COLOR)
        race = static_cast<FactionType>(Random((FACTION_KNIGHT), (FACTION_NECROMANCER)));
    else
    ...
    ConvertObject(
        ...
        TILESET_OBJNTOWN,
        (race) << RANDOM_TOWN_RACE_FRAME_SHIFT,
...
void game::RandomizeMine(i32 x, i32 y) {
    ...
    for (count = 0; count < RANDOM_MINE_RETRY_LIMIT; count++) {
        switch (terrain) {
            case TERRAIN_GRASS:
            case TERRAIN_DIRT:
                resType = static_cast<MineType>(Random((MINE_TYPE_MERCURY), (MINE_TYPE_GOLD)));
                if (resType == MINE_TYPE_MERCURY)
                    resType = MINE_TYPE_WOOD;
                break;
            case TERRAIN_SNOW:
                resType = static_cast<MineType>(Random((MINE_TYPE_ORE), (MINE_TYPE_GOLD)));
                break;
            case TERRAIN_SWAMP:
                resType = static_cast<MineType>(Random((MINE_TYPE_WOOD), (MINE_TYPE_GOLD)));
                break;
            case TERRAIN_LAVA:
...
i32 game::GetRandomArtifactId(
    ...
    for (;;) {
        ...
        if (!(((gArtifactLevel[(artifact)]) & (levelMask))))
            continue;
        ...
        if (tries++ < ARTIFACT_UNIQUE_RETRIES && m_randomArtifacts[(artifact)])
            continue;
...
void game::SetRandomHeroArmies(i32 heroId, i32 strongArmy) {
    ...
    i32 armySlot = 0;
    RandomHeroArmyRange armyTable[(FACTION_COUNT)][RANDOM_HERO_ARMY_OPTION_COUNT] = {
    ...
    for (index = 0; index < RANDOM_HERO_ARMY_SELECTION_COUNT; index++) {
        if (selected[index]) {
            ...
            army->m_creatureCounts[armySlot] =
                Random(minimumCount, maximum) / RANDOM_HERO_COUNT_SCALE;
            armySlot++;
        }
    }
}
...
void game::ProcessRandomObjects(void) {
    ...
    giUABaseY = -1;
    giUARadius = 0;
    for (mineIndex = 0; mineIndex < (RES_COUNT); mineIndex++)
    ...
    for (i = 0; i < ARMY_GROUP_SLOT_COUNT; i++) {
        if (group->m_creatureCounts[i] > 0) {
            experience += group->m_creatureCounts[i]
                * gMonsterDatabase[(group->m_creatureTypes[i])].hitPoints;
        }
    }
    if (heroPointer != NULL)
...
i32 game::GetLuck(hero* heroPointer, class army*, town* castle) {
    ...
    luck += (heroPointer->m_secondarySkills[(HERO_SKILL_LUCK)]);
    ...
    for (x = 0; x < MAP_WIDTH; x++) {
        for (y = 0; y < MAP_HEIGHT; y++) {
            if (gpAdvManager->FindAdjacentMonster(x, y, &monsterX, &monsterY, -1, -1))
                MAP_EXTRA_AT(x, y) |= (MAP_EXTRA_ADJACENT_MONSTER);
            else
                MAP_EXTRA_AT(x, y) &= (mask);
        }
    }
}
    ...
    for (townIndex = 0; townIndex < GAME_TOWN_COUNT; townIndex++) {
        ...
        for (slot = (BUILDING_SLOT_UPGRADE_FIRST); slot <= (BUILDING_SLOT_DWELLING_LAST);
            ...
            if ((((castle->m_buildings) & ((1 << slot))))) {
                if (slot == (BUILDING_SLOT_DWELLING_LAST))
                    castle->m_buildings &=
        ...
        if (castle->m_type == FACTION_NECROMANCER && castle->m_owner != -1
            && !gbHumanPlayer[(castle->m_owner)]) {
            if (Random(0, 100) < 50)
                spellIndex = (SPELL_DEATH_RIPPLE);
            else
                spellIndex = (SPELL_DEATH_WAVE);
            spellLevel = (gsSpellInfo[spellIndex].level) - 1;
        ...
        roll = Random(0, 100);
        if (roll < 25)
            spellIndex = (SPELL_DISPEL);
        else if (roll < 50)
            spellIndex = (SPELL_MASS_DISPEL);
        else if (roll < 75)
            spellIndex = (SPELL_ANTI_MAGIC);
        else
            spellIndex = (SPELL_CURE);
        spellLevel = (gsSpellInfo[spellIndex].level) - 1;
        castle->m_spells[spellLevel][spellsPerLevel[spellLevel]] = SpellType(spellIndex);
        spellsPerLevel[spellLevel]++;
        ...
        roll = Random(0, 100);
        if (roll < 20)
            spellIndex = (SPELL_MAGIC_ARROW);
        else if (roll < 40)
            spellIndex = (SPELL_LIGHTNING_BOLT);
        else if (roll < 60)
            spellIndex = (SPELL_FIREBALL);
        else if (roll < 80)
            spellIndex = (SPELL_COLD_RAY);
        else
            spellIndex = (SPELL_COLD_RING);
        spellLevel = (gsSpellInfo[spellIndex].level) - 1;
        castle->m_spells[spellLevel][spellsPerLevel[spellLevel]] = SpellType(spellIndex);
        spellsPerLevel[spellLevel]++;
        ...
        for (spellLevel = 0; spellLevel < TOWN_MAGE_GUILD_LEVEL_COUNT; spellLevel++) {
            ...
            for (spellIndex = 0; spellIndex < TOWN_MAGE_GUILD_SPELLS_PER_LEVEL; spellIndex++) {
                ...
                }
                {
                    ...
                    do {
                        spell =
                            SpellType(Random((SPELL_FIREBALL), (SPELL_SET_WATER_GUARDIAN)));
                        ...
                        if (castle->m_owner != -1 && !gbHumanPlayer[(castle->m_owner)])
                            spellValue =
                                gsSpellInfo[(spell)].aiValue
                                    * ((((gsSpellInfo[(spell)].attributes) & (SPELL_INFO_ATTRIBUTE_POWER)))
                                           ? 4
                                           : 1)
                                + 50;
                        else
                        ...
                        (combatSpells == 1
                         && (((gsSpellInfo[(spell)].attributes) & (SPELL_INFO_ATTRIBUTE_ADVENTURE))))
                        || Random(0, 10)
                               > gsSpellInfo[(spell)].raceChance[(castle->m_type)]
                        || attempts++ > 500 || usedSpells[(spell)]
                        || Random(1, 1500) > spellValue
                    );
                    if ((((gsSpellInfo[(spell)].attributes) & (SPELL_INFO_ATTRIBUTE_ADVENTURE))))
                        combatSpells++;
                    castle
                ...
                if ((cell->m_triggerType & MAP_TRIGGER_TYPE_MASK) == MAP_OBJECT_HERO
                    ...
                    if (pass == MAP_HERO_PLACEMENT_PASS) {
                        ...
                        if (!isJail && extra->hasPatrol) {
                            mapHero->m_patrolX = static_cast<u8>(mapX);
                            mapHero->m_patrolY = static_cast<u8>(mapY);
                        ...
                        if (extra->hasCustomArmy) {
                            for (armySlot = 0; armySlot < ARMY_GROUP_SLOT_COUNT;
                                ...
                                if (mapHero->m_army.m_creatureCounts[armySlot] > 0)
                                    mapHero->m_army.m_creatureTypes[armySlot] =
                                        extra->troopTypes[armySlot];
                                else
                                    mapHero->m_army.m_creatureTypes[armySlot] = CREATURE_NONE;
                            }
                        }
                        ...
                        if (extra->hasCustomSkills) {
                            ...
                            for (recordPosition = 0;
                                 recordPosition < (HERO_SKILL_COUNT);
                                 recordPosition++) {
                        ...
                        if (!isJail) {
                            SetVisibility(
                                ...
                                mapHero->m_y,
                                mapHero->m_owner,
                                giVisRange[(mapHero->m_secondarySkills
                                                   [(HERO_SKILL_SCOUTING)])]
                            );
                        }
                        H2_FREE(ppMapExtra[extraIndex]);
...
i32 game::TransmitSaveGame(i32 remotePlayer, i32 playerExited, i32 useCurrentSave) {
    ...
    if (gpAdvManager->m_active == 1)
        BVResMsg("Пересылка данных", RES_NONE, 0);
...
i32 game::ReceiveSaveGame(
    ...
    if (gpAdvManager->m_active == 1)
        BVResMsg("Получение данных", RES_NONE, 0);
        ...
        if (giWeekType != CALENDAR_PERIOD_NONE) {
            ...
            if (m_week == 1) {
                ...
                if (giMonthType == CALENDAR_PERIOD_NORMAL) {
                    ...
                    lowerName[0] = ToLowerCp1251(lowerName[0]);
                    sprintf(
                        gText,
...
i32 game::GetNumThievesGuilds(i32 color) {
    ...
    for (i = 0; i < m_players[color].m_townCount; i++) {
        if ((((gpGame->m_castleRecs[m_players[color].m_townIds[i]].m_buildings) & ((TOWN_BUILDING_THIEVES_GUILD)))))
            count++;
    }
    return count;
}
        ...
        if (roll < 80 && giCurTurn > 1) {
            ...
            while (attempts++ < 200) {
                ...
                if (categoryStats[0] != categoryStats[1]) {
                    if (selectionRoll == (THIEVES_CATEGORY_OBELISKS))
                        sprintf(
                            m_rumour,
                            "%s нашел больше всех обелисков.",
                            cPlayerNames[categoryOrder[0]]
                        );
                    else if (selectionRoll == (THIEVES_CATEGORY_ARTIFACTS))
                        sprintf(
                            m_rumour,
                            "%s нашел больше всех артефактов.",
                            cPlayerNames[categoryOrder[0]]
                        );
                    else if (selectionRoll == (THIEVES_CATEGORY_ARMY_STRENGTH))
                        sprintf(
                            m_rumour,
        ...
        ultimateRumour:
            ...
            if (selectionRoll < 33) {
                ...
                sprintf(
                    ...
                    cRumourTerrainDescriptions[(CELL_TERRAIN(
                ...
                sprintf(
                    m_rumour,
                    "Определенно, могущественный артефакт это %s.",
...
void game::CheckForTimeEvent(void) {
    ...
    for (eventIndex = 0; eventIndex < m_timeEventCount; eventIndex++) {
        ...
        if (((gbHumanPlayer[giCurPlayer] && event->appliesToHuman)
            ...
            secondaryType = -1;
            secondaryAmount = 0;
            for (resourceIndex = 0; resourceIndex < (RES_COUNT);
                 resourceIndex++) {
...
void CompressTest2(void) {
    ...
    char* unpackedData;
    i32l compSize;
    i32 sourceCrc [[maybe_unused]];
    i32 dataSz;
    i32 index;
    i32 unpackedCrc [[maybe_unused]];
    char* fromData;
    char* encoded;
    i32 sourceCrcCheck [[maybe_unused]];
