...
void game::LoadGame(const char* filename, i32 loadFromFile, i32) {
    ...
    fileDescriptor = platform::FileOpen(pathBuffer, platform::FileMode::Read);
    if (fileDescriptor == -1)
    ...
    ReadGameData(fileDescriptor, &giMonthType, SAVE_TRUNCATED_SCALAR_SIZE);
    ReadGameData(fileDescriptor, &giMonthTypeExtra, SAVE_TRUNCATED_SCALAR_SIZE);
    ReadGameData(fileDescriptor, &giWeekType, SAVE_TRUNCATED_SCALAR_SIZE);
    ...
    ReadGameData(fileDescriptor, playerBuffer, sizeof(playerBuffer[0]));
    ...
    ReadGameData(fileDescriptor, &m_deadPlayerCount, sizeof(m_deadPlayerCount));
    ReadGameData(fileDescriptor, m_playerDead, sizeof(m_playerDead));

    ReadGameData(fileDescriptor, isHuman, GAME_PLAYER_COUNT);
    for (index = 0; index < GAME_PLAYER_COUNT; index++) {
    ...
    ReadGameData(fileDescriptor, &m_day, sizeof(m_day));
    ReadGameData(fileDescriptor, &m_week, sizeof(m_week));
    ReadGameData(fileDescriptor, &m_month, sizeof(m_month));
    ...
    ReadGameData(fileDescriptor, &m_ultimateArtifactX, sizeof(m_ultimateArtifactX));
    ReadGameData(fileDescriptor, &m_ultimateArtifactY, sizeof(m_ultimateArtifactY));
    ...
    for (index = 1; index < iMaxMapExtra; index++) {
        ReadGameData(fileDescriptor, chunkTag, sizeof(i32));
        ReadGameData(fileDescriptor, pwSizeOfMapExtra + index, sizeof(pwSizeOfMapExtra[index]));
        RequireReadableBytes(fileDescriptor, pwSizeOfMapExtra[index]);
        ppMapExtra[index] = H2_ALLOC(pwSizeOfMapExtra[index]);
        ReadGameData(fileDescriptor, ppMapExtra[index], pwSizeOfMapExtra[index]);
    }
    ReadGameData(fileDescriptor, chunkTag, sizeof(i32));
    ...
    for (player = 0; player < GAME_PLAYER_COUNT; player++) {
        if (player >= gpGame->m_mapHeader.playerCount) {
            ...
            if (m_setupPlayerNetworkId[player] == GAME_COMPUTER_PLAYER)
                gbSetupGamePosToRealGamePos[player] = humanPosition++;
            else
        ...
        for (xPosition = 0; xPosition < MAP_WIDTH; xPosition++) {
            ...
            switch (cell->m_triggerType) {
                case MAP_ACTION_TRIGGER(MAP_OBJECT_WITCH_HUT):
                    cell->m_objectMetadata = H2EnumIndex(HERO_SKILL_NECROMANCY);
                    while (cell->m_objectMetadata == H2EnumIndex(HERO_SKILL_NECROMANCY)
                           || cell->m_objectMetadata == H2EnumIndex(HERO_SKILL_LEADERSHIP)) {
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_WAGON):
                    fightValue = Random(EVENT_ROLL_MIN, EVENT_ROLL_MAX);
                    if (fightValue < WAGON_EMPTY_CUTOFF)
                        cell->m_objectMetadata = MAP_EVENT_DATA_EMPTY;
                    else if (fightValue < WAGON_ARTIFACT_CUTOFF)
                        cell->m_objectMetadata =
                            GetRandomArtifactId(
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_TREASURE_CHEST):
                    if (CELL_TERRAIN(cell) == TERRAIN_WATER) {
                        ...
                        fightValue = Random(EVENT_ROLL_MIN, EVENT_ROLL_MAX);
                        if (fightValue < SEA_CHEST_EMPTY_CUTOFF)
                            cell->m_objectMetadata = SEA_CHEST_OUTCOME_EMPTY;
                        else if (fightValue < SEA_CHEST_GOLD_CUTOFF)
                        ...
                        fightValue = Random(EVENT_ROLL_MIN, EVENT_ROLL_MAX);
                        if (fightValue < LAND_CHEST_SMALL_CUTOFF)
                            cell->m_objectMetadata = CHEST_REWARD_SMALL;
                        else if (fightValue < LAND_CHEST_MEDIUM_CUTOFF)
                            cell->m_objectMetadata = CHEST_REWARD_MEDIUM;
                        else if (fightValue < LAND_CHEST_LARGE_CUTOFF)
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_CAMPFIRE):
                    ...
                    cell->m_objectMetadata |= Random(H2EnumIndex(RES_WOOD), H2EnumIndex(RES_GEMS));
                    break;
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_SHIPWRECK_SURVIVOR):
                    fightValue = Random(EVENT_ROLL_MIN, EVENT_ROLL_MAX);
                    if (fightValue < SHIPWRECK_SURVIVOR_TREASURE_CUTOFF)
                        cell->m_objectMetadata =
                            GetRandomArtifactId(ARTIFACT_LEVEL_TREASURE, true);
                    else if (fightValue < SHIPWRECK_SURVIVOR_MINOR_CUTOFF)
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_DERELICT_SHIP):
                    switch (Random(EVENT_ROLL_MIN, EVENT_BUCKET_ROLL_MAX) % EVENT_BUCKET_COUNT) {
                        ...
                        case 1:
                        case 2:
                            cell->m_objectMetadata = H2EnumIndex(EVENT_LEVEL_SMALL);
                            break;
                        case 3:
                        case 4:
                        case 5:
                            cell->m_objectMetadata = H2EnumIndex(EVENT_LEVEL_MEDIUM);
                            break;
                        case 6:
                        case 7:
                        case 8:
                            cell->m_objectMetadata = H2EnumIndex(EVENT_LEVEL_LARGE);
                            break;
                        case 9:
                            cell->m_objectMetadata = H2EnumIndex(EVENT_LEVEL_HUGE);
                            break;
                    }
                    break;
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_MONSTER):
                    if (cell->m_objectMetadata == MAP_EVENT_DATA_EMPTY) {
                        cell->m_objectMetadata = GetRandomNumTroops(CreatureTypeFromCode(cell->m_objectIndex));
                        if (cell->m_objectIndex != H2EnumIndex(CREATURE_GHOST)
                            && cell->m_objectIndex != H2EnumIndex(CREATURE_EARTH_ELEMENTAL)
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_SHRINE_FIRST_CIRCLE):
                    ...
                    while (gsSpellInfo[cell->m_objectMetadata - MAP_EVENT_SPELL_OFFSET].level
                           != SPELL_LEVEL_FIRST)
                        cell->m_objectMetadata = Random(H2EnumIndex(SPELL_FIREBALL), H2EnumIndex(SPELL_COUNT) - 1)
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_ARTIFACT):
                    fightValue = Random(EVENT_ROLL_MIN, EVENT_BUCKET_ROLL_MAX);
                    value = ArtifactTypeFromCode(cell->m_objectIndex / 2);
                    if (value == ARTIFACT_SPELL_SCROLL)
                        break;
                    if (fightValue < ARTIFACT_EVENT_UNCONDITIONAL_CUTOFF) {
                        if (fightValue % EVENT_BUCKET_COUNT
                            == ARTIFACT_EVENT_WISDOM_BUCKET)
                            cell->m_objectMetadata = ARTIFACT_EVENT_MODE_WISDOM;
                        else if (fightValue % EVENT_BUCKET_COUNT
                    ...
                    } else if (fightValue < ARTIFACT_EVENT_GUARD_CUTOFF) {
                ...
                case MAP_ACTION_TRIGGER(MAP_OBJECT_ABANDONED_MINE):
                    ...
                    m_mines[mineId].guardianCount = Random(
    ...
    for (yPosition = 0; yPosition < MAP_HEIGHT; yPosition++) {
        for (xPosition = 0; xPosition < MAP_WIDTH; xPosition++) {
            ...
            if ((cell->m_triggerType & MAP_TRIGGER_TYPE_MASK) == MAP_OBJECT_ROCK
                && cell->ObjectTileset() == TILESET_X_LOC2)
            ...
            upperCount = 0;
            lowerCount = 0;
            if (!(cell->m_flags & H2EnumIndex(MAP_CELL_OCCUPIED)) && yPosition < MAP_HEIGHT - 1
                && cell->m_objectIndex != MAPCELL_SPRITE_NONE
                && !(cell->m_triggerType & MAP_TRIGGER_ACTION_FLAG)
                && !(cell->m_flags & H2EnumIndex(MAP_CELL_OBJECT_SHADOW_ONLY))) {
                mapCell* below;
                if (m_worldMap.GetCell(xPosition, yPosition + 1)->m_objectIndex != MAPCELL_SPRITE_NONE
                    ...
                    for (fightValue = 0; fightValue < upperCount; fightValue++) {
                        for (lowerTileIndex = 0; lowerTileIndex < lowerCount; lowerTileIndex++) {
                            if (upperTilesets[fightValue] == lowerTilesets[lowerTileIndex]
                                || (upperTilesets[fightValue] >= TILESET_OBJNTOWN
                                    && upperTilesets[fightValue] <= TILESET_OBJNTWRD
...
void game::UpdateSpellWidgets(void) {
    ...
    message.payload.widget.command =
        WidgetFlagsCommand(spellPoints > VIEW_SPELL_MANA_HUNDREDS_THRESHOLD);
    ...
    message.type = MESSAGE_WIDGET;
    message.payload.widget.command =
        WidgetFlagsCommand(spellPoints > VIEW_SPELL_MANA_TENS_THRESHOLD);
    ...
    utf8::Format(gText, GLOBAL_TEXT_BUFFER_SIZE, "%d", spellPoints % VIEW_SPELL_MANA_DIGIT_BASE);
    ...
    for (i = 0; i < VIEW_SPELL_PAGE_SIZE; i++) {
        if (m_viewSpellsTop[H2EnumIndex(m_viewSpellsType)] + i
            ...
            if (GetManaCost(spell, m_viewSpellsHero) > m_viewSpellsHero->m_spellPoints)
                message.payload.widget.data.value = H2EnumIndex(FONT_DRAW_DIMMED);
            else
                message.payload.widget.data.value = H2EnumIndex(FONT_DRAW_DEFAULT);
            m_viewSpellsWindow->BroadcastMessage(message);

            lines = smallFont->LineLength(gSpellNames[H2EnumIndex(spell)], VIEW_SPELL_NAME_WIDTH);
            if (lines == 1) {
                utf8::Format(
                    gText, GLOBAL_TEXT_BUFFER_SIZE,
                    "%s\n[%d]",
            ...
            message.payload.widget.data.value = gsSpellInfo[H2EnumIndex(spell)].iconIndex;
            m_viewSpellsWindow->BroadcastMessage(message);
        ...
        switch (message.payload.widget.command) {
            case WIDGET_NOTIFY_DESELECT:
                if (message.payload.widget.command == WIDGET_NOTIFY_RIGHT_CLICK
                    || ((H2EnumIndex((message.payload.widget.modifiers) & (MESSAGE_MODIFIER_RIGHT_BUTTON)))) != 0)
                    break;
                {
                ...
                if (message.payload.widget.command == WIDGET_NOTIFY_RIGHT_CLICK
                    ...
                    switch (message.payload.widget.id) {
                        ...
                        case VIEW_SPELL_ICON_ID_11:
                            ...
                            break;
                        case VIEW_SPELL_PREVIOUS_ID:
                            NormalDialog(cSpellHelp[VIEW_SPELL_HELP_PREVIOUS], NORMAL_DIALOG_QUICK_VIEW);
                            break;
                        case VIEW_SPELL_NEXT_ID:
                            NormalDialog(cSpellHelp[VIEW_SPELL_HELP_NEXT], NORMAL_DIALOG_QUICK_VIEW);
                            break;
                        case VIEW_SPELL_COMBAT_TAB_ID:
                            NormalDialog(cSpellHelp[VIEW_SPELL_HELP_COMBAT], NORMAL_DIALOG_QUICK_VIEW);
                            break;
                        case VIEW_SPELL_ADVENTURE_TAB_ID:
                            NormalDialog(cSpellHelp[VIEW_SPELL_HELP_ADVENTURE], NORMAL_DIALOG_QUICK_VIEW);
                            break;
                        case VIEW_SPELL_MANA_LABEL_ID:
                        case VIEW_SPELL_MANA_HUNDREDS_ID:
                        case VIEW_SPELL_MANA_TENS_ID:
...
void game::ViewArmy(
    ...
    i32 morale;
    i32 luck;
    char armyName[VIEW_ARMY_NAME_SIZE];
    icon* monsterIcon;
    tag_message message;
    ...
    if (castle && (gpAdvManager->m_active == 1 || gpTownManager->m_active == 1)) {
        ...
        if ((monsterType == CREATURE_GREEN_DRAGON || monsterType == CREATURE_RED_DRAGON)
            && (castle->m_buildings & H2EnumIndex(TOWN_BUILDING_ALTERNATE_UPGRADED_DWELLING_6))) {
            gbAllowUpgrade = true;
    ...
    if (theArmy)
        ...
        armyMonster = &gMonsterDatabase[H2EnumIndex(monsterType)];
    ...
    iconFrame = sViewArmyMonFrameInfo.animationFrames[H2EnumIndex(ARMY_ANIMATION_WALK)][0];
    ...
    message.payload.widget.data.text = armyName;
    ...
    if (theGroup)
        ...
        morale = 0;
    if ((H2EnumIndex((monster->attributes) & (MONSTER_FLAGS_NO_MORALE))))
        morale = 0;
    ...
    if (modifier) {
        utf8::Format(gText, GLOBAL_TEXT_BUFFER_SIZE, " (%d)", monster->attack + modifier);
        strcat(details, gText);
    }
    ...
    if (modifier) {
        utf8::Format(gText, GLOBAL_TEXT_BUFFER_SIZE, " (%d)", monster->defense + modifier);
        strcat(details, gText);
    }
    ...
    if (monster->damageMin != monster->damageMax) {
        utf8::Format(gText, GLOBAL_TEXT_BUFFER_SIZE, "-%d", monster->damageMax);
        strcat(details, gText);
    }
    utf8::Format(
        gText, GLOBAL_TEXT_BUFFER_SIZE,
    ...
    if (theArmy) {
        ...
        H2SteppedEnumStorage<ArmySpellInfluence, i32> spellIndex =
        ...
        for (loopIndex = 0;
            ...
            for (; spellIndex < ARMY_SPELL_INFLUENCE_COUNT; spellIndex++) {
                if (theArmy->m_spellInfluence[H2EnumIndex(spellIndex)])
                    break;
            }
            spellWidget = new iconWidget(
                ...
                0,
                "spellinl.icn",
                static_cast<i16>(H2EnumIndex(spellIndex)),
                ICON_DRAW_NORMAL,
                ...
                switch (message.payload.widget.id) {
                    ...
                    case VIEW_ARMY_UPGRADE_ACTION_ID:
                        ...
                        if (gpCurPlayer->m_resources[H2EnumIndex(RES_GOLD)] >= goldCost
                            ...
                            NormalDialog(localization::Tr("army.upgrade.confirm"), NORMAL_DIALOG_CONFIRM, -1, -1, H2EnumIndex(RES_GOLD), goldCost, H2EnumIndex(resourceType), resourceCost);
                            if (gpWindowManager->m_dialogResult == DIALOG_BUTTON_5) {
                            ...
                            NormalDialog(localization::Tr("army.upgrade.cannot_afford"), NORMAL_DIALOG_INFO, -1, -1, H2EnumIndex(RES_GOLD), goldCost, H2EnumIndex(resourceType), resourceCost);
                        }
                        break;
                    default:
                        break;
...
void game::PerDay(void) {
    ...
    for (player = 0; player < gpGame->m_playerCount; player++) {
        for (resource = RES_WOOD; resource < RES_COUNT; resource++) {
            gpGame->m_players[player].m_aiData.m_income[H2EnumIndex(resource)] =
                -m_players[player].m_resources[H2EnumIndex(resource)];
        }
    }
    ...
    for (player = 0; player < GAME_MINE_COUNT; player++) {
        if (m_mines[player].owner != -1) {
            ...
            if (mineType != MINE_TYPE_GOLD && mineType <= MINE_TYPE_GOLD)
                m_players[m_mines[player].owner].m_resources[H2EnumIndex(mineType)] += dailyIncome;
        }
    }
    ...
    for (player = 0; player < gpGame->m_playerCount; player++) {
        for (resource = RES_WOOD; resource < RES_GOLD; resource++) {
            if (m_playerHandicap[player] != PLAYER_HANDICAP_NONE) {
                m_players[player].m_resources[H2EnumIndex(resource)] -= static_cast<i32>(
                    (gpGame->m_players[player].m_aiData.m_income[H2EnumIndex(resource)]
                     + m_players[player].m_resources[H2EnumIndex(resource)])
    ...
    for (player = 0; player < GAME_HERO_COUNT; player++) {
        ...
        maxSpellPoints =
            HERO_NORMAL_SPELL_POINTS(*currentHero);
        restoredSpellPoints +=
            H2EnumIndex(currentHero->m_secondarySkills[H2EnumIndex(HERO_SKILL_MYSTICISM)]) + 1;
        ...
        if (restoredSpellPoints > currentHero->m_spellPoints)
            currentHero->m_spellPoints = restoredSpellPoints;
        if ((H2EnumIndex((currentHero->m_eventFlags) & (HERO_EVENT_MAGIC_WELL))))
            ((currentHero->m_eventFlags) &= ~(HERO_EVENT_MAGIC_WELL));
    }

    for (player = 0; player < GAME_TOWN_COUNT; player++) {
        currentTown = GetTown(player);
        if (!(currentTown->m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_MAGE_GUILD))))
            continue;
        if (currentTown->m_occupyingHeroId != -1) {
            townHero = GetHero(currentTown->m_occupyingHeroId);
            maxSpellPoints =
                HERO_NORMAL_SPELL_POINTS(*townHero);
            if (maxSpellPoints > townHero->m_spellPoints)
                townHero->m_spellPoints = maxSpellPoints;
        }
    }
...
void game::PerWeek(void) {
    ...
    for (outerIndex = 0; outerIndex < GAME_TOWN_COUNT; outerIndex++) {
        ...
        for (innerIndex = H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST); innerIndex <= H2EnumIndex(BUILDING_SLOT_DWELLING_LAST);
             innerIndex++) {
            if (castle->m_buildings & (1 << innerIndex)) {
                creatureGrowth = gMonsterDatabase[H2EnumIndex(gDwellingType[H2EnumIndex(castle->m_type)]
                                                             [innerIndex - H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST)])]
                               .growth;
                if (castle->m_buildings
                    & (1 << H2EnumIndex(BUILDING_SLOT_SPECIAL_FOUR)))
                    creatureGrowth += TOWN_WELL_BASE_GROWTH_BONUS;
                if (innerIndex == H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST)
                    && (castle->m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_WELL_EXTRA))))
                ...
                if (giWeekType == CALENDAR_PERIOD_CREATURE
                    && H2EnumIndex(gDwellingType[H2EnumIndex(castle->m_type)]
                                        [innerIndex - H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST)])
                           == giWeekTypeExtra)
    ...
    for (outerIndex = 0; outerIndex < GAME_HERO_COUNT; outerIndex++) {
        ...
        if ((H2EnumIndex((weeklyHero->m_eventFlags) & (HERO_EVENT_STABLES))))
            ((weeklyHero->m_eventFlags) &= ~(HERO_EVENT_STABLES));
    }
...
void game::PerMonth(void) {
    ...
    for (i = 0; i < GAME_TOWN_COUNT; i++) {
        for (j = H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST); j <= H2EnumIndex(BUILDING_SLOT_DWELLING_LAST); j++) {
        ...
        for (x = 0; x < MAP_WIDTH; x++) {
            for (y = 0; y < MAP_HEIGHT; y++) {
                ...
                if (spot->m_triggerType == MAP_OBJECT_NONE && !spot->m_objectLayerBit1
                    ...
                    && CELL_TERRAIN(spot) != TERRAIN_WATER) {
    ...
    for (x = left; x <= right; x++) {
        for (y = top; y <= bottom; y++) {
            ...
            while (extra != NULL) {
                ...
                if (extra->nextIndex != 0
                    && WORLDMAP->Extra(extra->nextIndex)->objectIndex != MAPCELL_SPRITE_NONE)
            ...
            while (extra != NULL) {
                ...
                if (extra->nextIndex != 0
                    && WORLDMAP->Extra(extra->nextIndex)->overlayIndex != MAPCELL_SPRITE_NONE)
                    extra = WORLDMAP->Extra(extra->nextIndex);
                else
                    extra = NULL;
            }
...
i32 game::GetRandomArtifactId(
    ...
    for (i32 i = 0; i < numArtifacts; ++i) {
        ...
        if (!cursedAllowed
                && ironfist::IsCursedArtifact(ArtifactTypeFromOrdinal(i))) {
            continue;
        }
...
void game::SetRandomHeroArmies(i32 heroId, i32 strongArmy) {
    ...
    for (i32 slot = 0; slot < ARMY_GROUP_SLOT_COUNT; ++slot) {
        ...
        heroArmy->m_creatureCounts[slot] = 0;
    }
...
void game::ProcessRandomObjects(void) {
    ...
    i32 maxValue;
    mapCell* cell;
    MapTriggerCode randomObjectType;
    i32 randomType;

    giUABaseX = -1;
    giUABaseY = -1;
    giUARadius = 0;
    for (mineIndex = 0; mineIndex < H2EnumIndex(RES_COUNT); mineIndex++)
...
i32 game::ExperienceValueOfStack(armyGroup* group, hero* heroPointer) {
    ...
    for (i = 0; i < ARMY_GROUP_SLOT_COUNT; i++) {
        if (group->m_creatureCounts[i] > 0) {
            experience += group->m_creatureCounts[i]
...
i32 game::GetLuck(hero* heroPointer, class army* stack, town* castle) {
    i32 luck;
    if (heroPointer == NULL)
        return ironfist::hooks::ModifyLuck(heroPointer, stack, castle, NEUTRAL);
    luck = NEUTRAL;
    ...
    if (heroPointer->HasArtifact(ARTIFACT_FOUR_LEAF_CLOVER))
        luck++;
    if (heroPointer->HasArtifact(ARTIFACT_MASTHEAD) && (H2EnumIndex((heroPointer->m_eventFlags) & (HERO_EVENT_EMBARKED)))) {
        luck++;
    }
    luck += heroPointer->m_luck;
    luck += H2EnumIndex(heroPointer->m_secondarySkills[H2EnumIndex(HERO_SKILL_LUCK)]);
    ...
    if (heroPointer->HasArtifact(ARTIFACT_BATTLE_GARB))
        luck = MAXIMUM;
    return ironfist::hooks::ModifyLuck(heroPointer, stack, castle, luck);
}
    ...
    for (townIndex = 0; townIndex < GAME_TOWN_COUNT; townIndex++) {
        ...
        if (extra->hasCustomArmy) {
            for (slot = 0; slot < ARMY_GROUP_SLOT_COUNT; slot++) {
                castle->m_army.m_creatureCounts[slot] = extra->troopCounts[slot];
                if (castle->m_army.m_creatureCounts[slot] > 0)
                    castle->m_army.m_creatureTypes[slot] = extra->troopTypes[slot];
                else
                    castle->m_army.m_creatureTypes[slot] = CREATURE_NONE;
            }
        } else {
            for (slot = 0; slot < ARMY_GROUP_SLOT_COUNT; slot++) {
                castle->m_army.m_creatureCounts[slot] = 0;
                castle->m_army.m_creatureTypes[slot] = CREATURE_NONE;
            }
        ...
        for (slot = H2EnumIndex(BUILDING_SLOT_UPGRADE_FIRST); slot <= H2EnumIndex(BUILDING_SLOT_DWELLING_LAST);
            ...
            if (castle->m_buildings & (1 << slot)) {
                if (slot == H2EnumIndex(BUILDING_SLOT_DWELLING_LAST)
)                     castle->m_buildings &=
        ...
        if (castle->m_type == FACTION_CYBORG) {
            castle->m_spells[0][0] = SPELL_SHADOW_MARK;
            castle->m_spells[0][1] = SPELL_MARKSMAN_PIERCE;
            castle->m_spells[1][0] = SPELL_PLASMA_CONE;
            castle->m_spells[1][1] = SPELL_FORCE_SHIELD;
            castle->m_spells[2][0] = SPELL_MASS_FORCE_SHIELD;
            castle->m_spells[3][0] = SPELL_FIRE_BOMB;
            castle->m_spells[4][0] = SPELL_IMPLOSION_GRENADE;
        ...
        if (castle->m_type == FACTION_NECROMANCER && castle->m_owner != -1
            && !gbHumanPlayer[castle->m_owner]) {
            if (Random(0, 100) < 50)
                spellIndex = H2EnumIndex(SPELL_DEATH_RIPPLE);
            else
                spellIndex = H2EnumIndex(SPELL_DEATH_WAVE);
            spellLevel = H2EnumIndex(gsSpellInfo[spellIndex].level) - 1;
        ...
        roll = Random(0, 100);
        if (roll < 25)
            spellIndex = H2EnumIndex(SPELL_DISPEL);
        else if (roll < 50)
            spellIndex = H2EnumIndex(SPELL_MASS_DISPEL);
        else if (roll < 75)
            spellIndex = H2EnumIndex(SPELL_ANTI_MAGIC);
        else
            spellIndex = H2EnumIndex(SPELL_CURE);
        spellLevel = H2EnumIndex(gsSpellInfo[spellIndex].level) - 1;
        castle->m_spells[spellLevel][spellsPerLevel[spellLevel]] = SpellType(spellIndex);
        spellsPerLevel[spellLevel]++;
        ...
        roll = Random(0, 100);
        if (roll < 20)
            spellIndex = H2EnumIndex(SPELL_MAGIC_ARROW);
        else if (roll < 40)
            spellIndex = H2EnumIndex(SPELL_LIGHTNING_BOLT);
        else if (roll < 60)
            spellIndex = H2EnumIndex(SPELL_FIREBALL);
        else if (roll < 80)
            spellIndex = H2EnumIndex(SPELL_COLD_RAY);
        else
            spellIndex = H2EnumIndex(SPELL_COLD_RING);
        spellLevel = H2EnumIndex(gsSpellInfo[spellIndex].level) - 1;
        castle->m_spells[spellLevel][spellsPerLevel[spellLevel]] = SpellType(spellIndex);
        spellsPerLevel[spellLevel]++;
            ...
            for (spellIndex = 0; spellIndex < TOWN_MAGE_GUILD_SPELLS_PER_LEVEL; spellIndex++) {
                if (castle->m_spellSlots[spellLevel * TOWN_MAGE_GUILD_SPELLS_PER_LEVEL + spellIndex]
                    ...
                    usedSpells[H2EnumIndex(castle->m_spellSlots
                            [spellLevel * TOWN_MAGE_GUILD_SPELLS_PER_LEVEL + spellIndex])] = 1;
                    continue;
                }
                {
                    attempts = 0;
...
void game::ProcessOnMapHeroes(void) {
    ...
    for (i32 i = 0; i < ironfist::AVAILABLE_HERO_COUNT; i++) {
        ...
        if (i < H2EnumIndex(GAME_HERO_COUNT)) {
            ...
            data.faction = FactionTypeFromOrdinal(i / MAP_HEROES_PER_FACTION);
    ...
    for (i32 y = 0; y < MAP_HEIGHT; ++y) {
        for (i32 x = 0; x < MAP_WIDTH; ++x) {
            ...
            const MapObjectType locationType = loc->m_triggerType & MAP_TRIGGER_TYPE_MASK;
            if (locationType == MAP_OBJECT_HERO
                || loc->m_triggerType == MAP_ACTION_TRIGGER(MAP_OBJECT_JAIL)) {
                const bool isJail = locationType == MAP_OBJECT_JAIL;
                ...
                if (mapExtraHero->hasAssignedHero) {
                    ...
                    m_heroRecs[heroIndex].m_cursorType = faction;
                } else {
                    ...
                    m_heroRecs[randomHeroIdx].m_cursorType = faction;
                    if (mapExtraHero->hasCustomHero
                        && extendedHeroId >= H2EnumIndex(GAME_HERO_COUNT)) {
                        m_heroRecs[randomHeroIdx].m_portrait =
                            HeroPortraitFromCode(extendedHeroId);
                    }
                ...
                if (mapExtraHero->hasCustomArmy) {
                    for (i32 slot = 0; slot < ARMY_GROUP_SLOT_COUNT; ++slot) {
                        randomHero->m_army.m_creatureCounts[slot] =
                            static_cast<i16>(mapExtraHero->troopCounts[slot]);
                        if (randomHero->m_army.m_creatureCounts[slot] <= 0) {
                ...
                for (i32 i = 0; i < EVENT_RECORD_HERO_ARTIFACT_COUNT; ++i) {
                    if (mapExtraHero->artifacts[i] >= 0) {
                        GiveArtifact(
                        randomHero, ArtifactTypeFromCode(mapExtraHero->artifacts[i]),
                            true, -1
                        );
                    }
                }
                ...
                if (mapExtraHero->hasCustomSkills) {
                    ...
                    for (i32 i = 0; i < HERO_SECONDARY_SKILL_CAPACITY; ++i) {
                        if (mapExtraHero->skillTypes[i] != -1)
                            randomHero->GiveSS(
                            HeroSecondarySkillFromCode(mapExtraHero->skillTypes[i]),
                            HeroSkillLevelFromCode(mapExtraHero->skillLevels[i])
                            );
                    }
...
i32 game::TransmitSaveGame(i32 remotePlayer, i32 playerExited, i32 useCurrentSave) {
    ...
    }
    {
        ...
        header->init.dataSize = fileSize;
        header->init.crc = fileCrc;
        header->init.wireCrc = transmitCrc;
        header->init.playerExited = playerExited;
        ...
        for (batch = 0; batch < batchCount; batch++) {
            ...
            while (!done) {
                for (sendPacketIndex = batch * REMOTE_PACKET_BATCH_SIZE;
                    ...
                    if (!acknowledged[sendPacketIndex]) {
                        ...
                        RequireGameData(platform::binary::WriteU16(
                            reinterpret_cast<u8*>(header->bytes),
                            sizeof(header->bytes), 0,
                            static_cast<u16>(sendPacketIndex)
                        ));
                        memcpy(
                            header->chunk.data,
                ...
                if (!result)
                    ShutDown(NULL);
                for (sendPacketIndex = 0; sendPacketIndex < packetsInBatch; sendPacketIndex++) {
                    if (reinterpret_cast<RemoteMessage*>(reply)->payload[sendPacketIndex] > 0)
                        *(acknowledged + sendPacketIndex + batch * REMOTE_PACKET_BATCH_SIZE) = 1;
                }
                done = true;
                for (sendPacketIndex = batch * REMOTE_PACKET_BATCH_SIZE;
                     sendPacketIndex < batch * REMOTE_PACKET_BATCH_SIZE + packetsInBatch;
...
i32 game::ReceiveSaveGame(
    ...
    while (!finished) {
        ...
        receivedPacket = reinterpret_cast<RemoteSaveMessage*>(GetRemoteData(1));
        if (receivedPacket
                ...
                case REMOTE_SAVE_DATA_COMMAND: {
                    ...
                    RequireGameData(
                        ...
                        && platform::binary::ReadU16(
                            reinterpret_cast<const u8*>(receivedPacket->payload.bytes),
                            static_cast<std::size_t>(receivedPacket->payloadSize),
                            0,
                            encodedPacketStart
                        )
                    );
                    packetStart = encodedPacketStart;
                    const i32 payloadBytes = receivedPacket->payloadSize - REMOTE_PACKET_INDEX_SIZE;
                    RequireGameData(
...
void game::DoNewTurn(void) {
    ...
    char lowerName[NEW_TURN_LOWER_NAME_CAPACITY];
    i32 musicTrack;
...
void CreateDiffFile(
        ...
        while (1) {
            ...
            if (*(prevData + position + length) == *(fullData + position + length)) {
                ...
                if (matchLength <= DIFF_MAX_SHORT_MATCH) {
                    ...
                    RequireGameData(
                        WriteDiffHeaderInfo(0, matchLength, diffOut, diffCapacity, &diffTotal)
                    );
    ...
    destinationFile = platform::FileOpen(gText, platform::FileMode::Write);
    if (destinationFile == -1)
        FileError(gText);
    WriteGameData(destinationFile, diffOut, diffTotal);
    platform::FileClose(destinationFile);

    utf8::Format(gText, GLOBAL_TEXT_BUFFER_SIZE, "%s%s", ".\\DATA\\", oldName);
    destinationFile = platform::FileOpen(gText, platform::FileMode::Write);
    if (destinationFile == -1)
        FileError(gText);
    WriteGameData(destinationFile, fullData, joinSize);
    platform::FileClose(destinationFile);
...
void CreateJoinFile(char* oldName, char* diffName, char* joinName) {
    ...
    if (diffData[0] == 0) {
        ...
        if (diffFile == -1)
            FileError(gText);
        ReadGameData(diffFile, oldBuffer, oldSize);
        platform::FileClose(diffFile);
    ...
    if (Random(0, 100) < 30) {
        ...
        if (roll < 80 && giCurTurn > 1) {
            ...
            while (attempts++ < 200) {
                ...
                GetCategoryStats(
                    TownThievesGuildCategoryFromOrdinal(selectionRoll),
                    categoryStats,
                    categoryOrder
                );
...
void game::CheckForTimeEvent(void) {
    ...
    for (eventIndex = 0; eventIndex < m_timeEventCount; eventIndex++) {
        ...
        if (((gbHumanPlayer[giCurPlayer] && event->appliesToHuman)
            ...
            secondaryType = -1;
            secondaryAmount = 0;
            for (resourceIndex = 0; resourceIndex < H2EnumIndex(RES_COUNT);
                 resourceIndex++) {
...
i32 CalcFileCRC(char* file) {
    ...
    u8* block = static_cast<u8*>(H2_ALLOC(length));
    i32 fileDescriptor = platform::FileOpen(file, platform::FileMode::Read);
    if (fileDescriptor == -1)
        FileError(file);
    ReadGameData(fileDescriptor, block, length);
    i32 checksum = calc_crc_long(block, length);
    platform::FileClose(fileDescriptor);
    H2_FREE(block);
    return checksum;
}
