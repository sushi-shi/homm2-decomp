    ...
    if (!IsSoundBackendActive(gpSoundManager) || gConfig.soundVolume == CONFIG_VOLUME_MUTED
        ...
        if (bSmackSound) {
            ...
            AIL_set_digital_master_volume(
                digitalDriver5,
                smackMasterVolumes[(gConfig.soundVolume)]
            );
        }
    }
...
ExpansionCampaignId ExpansionCampaignRect(i32 x, i32 y) {
    ...
    for (; (campaign) < EXPANSION_RECT_COUNT; ++campaign) {
        if (PointInRect(x, y, &expansionCampaignRects[(campaign)]))
            return campaign;
    }
...
i32 smackMasterVolumes[(CONFIG_VOLUME_LEVEL_COUNT)] =
