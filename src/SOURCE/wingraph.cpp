...
i32 DDAppPaint(void* window, void* paintDC [[maybe_unused]]) {
    i32 sourceWidth;
    i32 sourceHeight5;
    i32 sourceTop8;
    i32 sourceLeft6;
...
i32 WGAppPaint(void* window, void* paintDC) {
    i32 padding8 [[maybe_unused]];
    char unusedByte0 [[maybe_unused]];
    RECT clientRect16;
    i32 destY0;
    i32 xSource;
    PAINTSTRUCT paint5;
...
void SetFullScreenStatus(b32 fullScreen) {
    ...
    if (giGraphicsType == WINGRAPH_GRAPHICS_WING) {
        if (gbDDrawAttached == 0)
            return;
        gConfig.gfx[H2EnumIndex(giCurExe)].fullScreen = true;
