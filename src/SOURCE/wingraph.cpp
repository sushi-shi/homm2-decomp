...
void DDInitGraphics(void) {
    ...
    if (CURRENT_GRAPHICS_CONFIG.fullScreen != 0) {
        ...
        result = lpDD->SetDisplayMode(LOGICAL_SCREEN_WIDTH, LOGICAL_SCREEN_HEIGHT, WINGRAPH_COLOR_DEPTH);
        if (result != DD_OK)
            DDSD(
                result,
                "wingraph.cpp",
...
i32 DDAppPaint(HWND window, HDC paintDC [[maybe_unused]]) {
    i32 sourceWidth;
    i32 sourceHeight;
    i32 sourceTop;
    i32 sourceLeft;
        ...
        return 1;
    {
        ...
        if (paint.rcPaint.right < LOGICAL_SCREEN_WIDTH)
            paint.rcPaint.right++;
        if (paint.rcPaint.bottom < LOGICAL_SCREEN_HEIGHT)
            paint.rcPaint.bottom++;

        gDDDestinationRect = paint.rcPaint;
        sourceWidth = ((gDDDestinationRect.right - gDDDestinationRect.left + 1) * LOGICAL_SCREEN_WIDTH)
                       / iMainWinScreenWidth;
        sourceHeight = ((gDDDestinationRect.bottom - gDDDestinationRect.top + 1) * LOGICAL_SCREEN_HEIGHT)
        ...
        while (1) {
            ...
            if (gDDResult == DDERR_SURFACELOST) {
                ...
                if (gDDResult == DDERR_WRONGMODE) {
                    ...
                    gDDResult =
                        lpDD->SetDisplayMode(LOGICAL_SCREEN_WIDTH, LOGICAL_SCREEN_HEIGHT, WINGRAPH_COLOR_DEPTH);
                    if (gDDResult != DD_OK)
                        DDSD(
                            gDDResult,
                            "wingraph.cpp",
...
void DDInitializePalette(void) {
        ...
        return;
    {
        ...
        GetSystemPaletteEntries(
            winDC,
            H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE,
            WINGRAPH_SYSTEM_PALETTE_SIZE,
            &LogicalPalette.entries[H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE]
        );
        ReleaseDC(NULL, winDC);
        for (entry = 0; entry < WINGRAPH_SYSTEM_PALETTE_SIZE; entry++) {
            LogicalPalette.entries[entry].peFlags = 0;
            LogicalPalette.entries[H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
                .peFlags = 0;
        }
...
void __cdecl DDUpdatePalette(i8* paletteData) {
    ...
    for (entry = WINGRAPH_SYSTEM_PALETTE_SIZE;
        ...
        LogicalPalette.entries[entry].peRed =
            paletteData[entry * H2EnumIndex(PALETTE_CHANNEL_COUNT) + H2EnumIndex(PALETTE_CHANNEL_RED)]
            << PALETTE_VALUE_SHIFT;
        LogicalPalette.entries[entry].peGreen =
            paletteData[entry * H2EnumIndex(PALETTE_CHANNEL_COUNT) + H2EnumIndex(PALETTE_CHANNEL_GREEN)]
            << PALETTE_VALUE_SHIFT;
        LogicalPalette.entries[entry].peBlue =
            paletteData[entry * H2EnumIndex(PALETTE_CHANNEL_COUNT) + H2EnumIndex(PALETTE_CHANNEL_BLUE)]
            << PALETTE_VALUE_SHIFT;
        LogicalPalette.entries[entry].peFlags = PC_NOCOLLAPSE;
    }
    ProcessAssert(
    ...
    result = lpDDPal->SetEntries(
        0,
        WINGRAPH_SYSTEM_PALETTE_SIZE,
        H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE * H2EnumIndex(SYSTEM_PALETTE_REGION_COUNT),
...
void DDCleanUpWinGraphics(void) {
    HRESULT restoreResult [[maybe_unused]];
    HRESULT result;
...
void __cdecl WGUpdatePalette(i8* paletteData) {
    ...
    for (index = WINGRAPH_SYSTEM_PALETTE_SIZE;
         index < H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE;
         index++) {
        LogicalPalette.entries[index].peRed =
            paletteData[index * H2EnumIndex(PALETTE_CHANNEL_COUNT) + H2EnumIndex(PALETTE_CHANNEL_RED)]
            << PALETTE_VALUE_SHIFT;
        screenImage.colors[index].rgbRed = LogicalPalette.entries[index].peRed;
        LogicalPalette.entries[index].peGreen =
            paletteData[index * H2EnumIndex(PALETTE_CHANNEL_COUNT) + H2EnumIndex(PALETTE_CHANNEL_GREEN)]
            << PALETTE_VALUE_SHIFT;
        screenImage.colors[index].rgbGreen = LogicalPalette.entries[index].peGreen;
        LogicalPalette.entries[index].peBlue =
            paletteData[index * H2EnumIndex(PALETTE_CHANNEL_COUNT) + H2EnumIndex(PALETTE_CHANNEL_BLUE)]
            << PALETTE_VALUE_SHIFT;
...
void WGInitializePalette(void) {
    ...
    for (entry = 0; entry < WINGRAPH_SYSTEM_PALETTE_SIZE; entry++) {
        ...
        LogicalPalette.entries[entry].peFlags = 0;
        screenImage.colors[H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry].rgbRed =
            LogicalPalette.entries[H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
                .peRed;
        screenImage.colors[H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry].rgbGreen =
            LogicalPalette.entries[H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
                .peGreen;
        screenImage.colors[H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry].rgbBlue =
            LogicalPalette.entries[H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
                .peBlue;
        screenImage.colors[H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
            .rgbReserved = 0;
        LogicalPalette.entries[H2EnumIndex(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
            .peFlags = 0;
    }
...
i32 WGAppPaint(HWND window, HDC paintDC) {
    i32 padding [[maybe_unused]];
    char unusedByte [[maybe_unused]];
    ...
    if (screenImage.bits != NULL) {
        ...
        if (iMainWinScreenWidth == LOGICAL_SCREEN_WIDTH && iMainWinScreenHeight == LOGICAL_SCREEN_HEIGHT) {
            ...
            WinGStretchBlt(
                ...
                sourceX,
                sourceY,
                (destinationWidth * LOGICAL_SCREEN_WIDTH) / iMainWinScreenWidth,
                (destinationHeight * LOGICAL_SCREEN_HEIGHT) / iMainWinScreenHeight
            );
        }
...
void GetGraphicsInfo(void) {
    ...
    if (screenDC != NULL) {
        ...
        if (giMainVideoModeColorDepth < WINGRAPH_COLOR_DEPTH)
            ShutDown(
                localization::Tr("system.display.palette_required")
...
i32 SetGraphicsType(WingraphGraphicsType graphicsType) {
    ...
    buffer = H2_ALLOC(LOGICAL_SCREEN_WIDTH * LOGICAL_SCREEN_HEIGHT);
...
i32l Orientation = 1;
struct _PALETTE LogicalPalette = {
    WINGRAPH_PALETTE_VERSION, PALETTE_COLOR_COUNT, {}
};
void* lpInitWin = NULL;
