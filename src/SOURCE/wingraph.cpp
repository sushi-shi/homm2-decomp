...
#endif
VA(0x004afe8f, 0x4da)
i32 DDAppPaint(HWND window, HDC H2_UNUSED(paintDC)) {
    i32 sourceWidth;
    i32 sourceHeight;
    i32 sourceTop;
    i32 sourceLeft;
...
void DDInitializePalette(void) {
        ...
        return;
    {
        ...
        GetSystemPaletteEntries(
            winDC,
            IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE,
            WINGRAPH_SYSTEM_PALETTE_SIZE,
            &LogicalPalette.entries[IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE]
        );
        ReleaseDC(NULL, winDC);
        for (entry = 0; entry < WINGRAPH_SYSTEM_PALETTE_SIZE; entry++) {
            LogicalPalette.entries[entry].peFlags = 0;
            LogicalPalette.entries[IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
                .peFlags = 0;
        }
...
void __cdecl DDUpdatePalette(i8* paletteData) {
    ...
    for (entry = WINGRAPH_SYSTEM_PALETTE_SIZE;
        ...
        LogicalPalette.entries[entry].peRed =
            paletteData[entry * IDX(PALETTE_CHANNEL_COUNT) + IDX(PALETTE_CHANNEL_RED)]
            << PALETTE_VALUE_SHIFT;
        LogicalPalette.entries[entry].peGreen =
            paletteData[entry * IDX(PALETTE_CHANNEL_COUNT) + IDX(PALETTE_CHANNEL_GREEN)]
            << PALETTE_VALUE_SHIFT;
        LogicalPalette.entries[entry].peBlue =
            paletteData[entry * IDX(PALETTE_CHANNEL_COUNT) + IDX(PALETTE_CHANNEL_BLUE)]
            << PALETTE_VALUE_SHIFT;
        LogicalPalette.entries[entry].peFlags = PC_NOCOLLAPSE;
    }
    ProcessAssert(
    ...
    result = lpDDPal->SetEntries(
        0,
        WINGRAPH_SYSTEM_PALETTE_SIZE,
        IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE * IDX(SYSTEM_PALETTE_REGION_COUNT),
...
void __cdecl WGUpdatePalette(i8* paletteData) {
    ...
    for (index = WINGRAPH_SYSTEM_PALETTE_SIZE;
         index < IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE;
         index++) {
        LogicalPalette.entries[index].peRed =
            paletteData[index * IDX(PALETTE_CHANNEL_COUNT) + IDX(PALETTE_CHANNEL_RED)]
            << PALETTE_VALUE_SHIFT;
        screenImage.colors[index].rgbRed = LogicalPalette.entries[index].peRed;
        LogicalPalette.entries[index].peGreen =
            paletteData[index * IDX(PALETTE_CHANNEL_COUNT) + IDX(PALETTE_CHANNEL_GREEN)]
            << PALETTE_VALUE_SHIFT;
        screenImage.colors[index].rgbGreen = LogicalPalette.entries[index].peGreen;
        LogicalPalette.entries[index].peBlue =
            paletteData[index * IDX(PALETTE_CHANNEL_COUNT) + IDX(PALETTE_CHANNEL_BLUE)]
            << PALETTE_VALUE_SHIFT;
...
void WGInitializePalette(void) {
    ...
    for (entry = 0; entry < WINGRAPH_SYSTEM_PALETTE_SIZE; entry++) {
        ...
        LogicalPalette.entries[entry].peFlags = 0;
        screenImage.colors[IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry].rgbRed =
            LogicalPalette.entries[IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
                .peRed;
        screenImage.colors[IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry].rgbGreen =
            LogicalPalette.entries[IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
                .peGreen;
        screenImage.colors[IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry].rgbBlue =
            LogicalPalette.entries[IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
                .peBlue;
        screenImage.colors[IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
            .rgbReserved = 0;
        LogicalPalette.entries[IDX(PALETTE_COLOR_COUNT) - WINGRAPH_SYSTEM_PALETTE_SIZE + entry]
            .peFlags = 0;
    }
...
void GetGraphicsInfo(void) {
    ...
    if (screenDC != NULL) {
        ...
        if (giMainVideoModeColorDepth < WINGRAPH_COLOR_DEPTH)
            ShutDown(
                localization::Tr("system.display.palette_required") /* "Для Героев II требуется режим в 256 цветов или больше.
...
DATA(0x00519bd0) struct _PALETTE LogicalPalette = {
    WINGRAPH_PALETTE_VERSION, PALETTE_COLOR_COUNT, H2_ZERO_INIT
};
