...
typedef enum ViewWorldConstant {
    ...
    WORLD_TILESET_COUNT        = (TILESET_COUNT),
...
void advManager::VWInit(i32 centerX, i32 centerY) {
    ...
    iVWViewableCells = WORLD_DRAW_SIZE / (giViewWorldScale);
    ...
    iVWXPixelOffset = (WORLD_WINDOW_X - (iVWViewableCells) * (giViewWorldScale)) >> 1;
...
void advManager::VWCompleteDraw(void) {
    ...
    drawTilesets[(TILESET_OBJNARTI)] = 0;
    drawTilesets[(TILESET_ART32)] = 0;
    drawTilesets[(TILESET_FLAG32)] = 0;
    ...
    for (cellY = iVWMapOriginY; cellY < iVWMapOriginY + iVWViewableCells; cellY++) {
        for (cellX = iVWMapOriginX; cellX < iVWMapOriginX + iVWViewableCells; cellX++) {
            ...
            if ((MAP_EXTRA_AT_WFIRST(cellX, cellY) & giCurPlayerBit) || iVWDrawAllTerrains
                ...
                orientation = ICON_DRAW_NORMAL;
                screenX = iVWXPixelOffset + (cellX - iVWMapOriginX) * (giViewWorldScale);
                drawY = iVWYPixelOffset + (cellY - iVWMapOriginY) * (giViewWorldScale);
                shape =
                ...
                switch (shape) {
                    ...
                    case 20:
                    case 21:
                        frame = GROUND_FRAME_ZERO;
                        break;
                    case 1:
                    case 12:
                    case 15:
                    case 16:
                        frame = GROUND_FRAME_ONE;
                        break;
                    case 2:
                    case 10:
                    case 11:
                        frame = GROUND_FRAME_TWO;
                        break;
                    case 3:
                    case 13:
                    case 14:
                    case 17:
                        frame = GROUND_FRAME_THREE;
                        break;
                    case 4:
                        frame = GROUND_FRAME_FOUR;
                        break;
                    case 5:
                        frame = GROUND_FRAME_FIVE;
                        break;
                    case 6:
                        frame = GROUND_FRAME_SIX;
                        break;
                    case 7:
                        frame = GROUND_FRAME_SEVEN;
                        break;
                    case 8:
                        frame = GROUND_FRAME_EIGHT;
                        break;
                    default:
                        break;
                }
                ...
                if (frame == 0)
                    ...
                    frame += GROUND_FRAME_OFFSET;
                frame += (CELL_TERRAIN(cell)) * WORLD_TERRAIN_FRAME_STRIDE;
                if ((MAP_EXTRA_AT_WFIRST(cellX, cellY) & giCurPlayerBit) || iVWDrawAllTerrains) {
                    pVWGround->DrawToBuffer(
                        screenX
                            + (orientation == ICON_DRAW_FLIPPED ? (giViewWorldScale) - 1 : 0),
                        drawY,
                        frame,
                        orientation
                    );
                ...
                if (cell->m_overlayIndex != WORLD_NO_SPRITE
                    && drawTilesets[(cell->m_overlayTileset)]) {
                    IconToBitmapScale(
                        m_objectIcons[(cell->m_overlayTileset)],
                        gpWindowManager->m_screen,
                        screenX,
                        drawY,
                        cell->m_overlayIndex,
                ...
                if (cell->m_extraIndex != 0
                    ...
                    extraCell = NULL;
                while (extraCell != NULL) {
                    if (drawTilesets[(extraCell->overlayTileset)]) {
                        IconToBitmapScale(
                            m_objectIcons[(extraCell->overlayTileset)],
                            gpWindowManager->m_screen,
                            screenX,
                            drawY,
        ...
        for (cellX = iVWMapOriginX; cellX < iVWMapOriginX + iVWViewableCells; cellX++) {
            ...
            if (cell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_ARTIFACT))
                ...
                IconToBitmap(
                    pVWMisc,
                    gpWindowManager->m_screen,
                    screenX - iVWHalf[giViewWorldScaleLookup][OFFSET_ARTIFACT][(COORDINATE_AXIS_X)],
                    drawY - iVWHalf[giViewWorldScaleLookup][OFFSET_ARTIFACT][(COORDINATE_AXIS_Y)],
                    ARTIFACT_ICON_FRAME,
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
            ...
            if ((cell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_CASTLE))
                ...
                if (color < 0)
                    ...
                    color = gpGame->m_players[color].m_color;
                townFlagX[SCALE_INDEX_FAR] = -2;
                townFlagX[SCALE_INDEX_MIDDLE] = -3;
                townFlagX[SCALE_INDEX_NEAR] = -6;
                iconX[SCALE_INDEX_FAR] = 5;
                iconX[SCALE_INDEX_MIDDLE] = 7;
                iconX[SCALE_INDEX_NEAR] = 10;
                FlipIconToBitmap(
                    pVWFlags,
                    gpWindowManager->m_screen,
                    screenX + townFlagX[giViewWorldScaleLookup]
                        - iVWHalf[giViewWorldScaleLookup][OFFSET_TOWN][(COORDINATE_AXIS_X)],
                    drawY - iVWHalf[giViewWorldScaleLookup][OFFSET_TOWN][(COORDINATE_AXIS_Y)],
                    color,
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
                ...
                IconToBitmap(
                    pVWFlags,
                    gpWindowManager->m_screen,
                    screenX + iconX[giViewWorldScaleLookup]
                        - iVWHalf[giViewWorldScaleLookup][OFFSET_TOWN][(COORDINATE_AXIS_X)],
                    drawY - iVWHalf[giViewWorldScaleLookup][OFFSET_TOWN][(COORDINATE_AXIS_Y)],
                    color,
                    ICON_DRAW_CLIP,
            ...
            if ((cell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_HERO_INTERACTION))
                ...
                if (color >= 0) {
                    ...
                    IconToBitmap(
                        pVWMisc,
                        gpWindowManager->m_screen,
                        screenX - iVWHalf[giViewWorldScaleLookup][OFFSET_HERO][(COORDINATE_AXIS_X)],
                        drawY - iVWHalf[giViewWorldScaleLookup][OFFSET_HERO][(COORDINATE_AXIS_Y)],
            ...
            if (cell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_RESOURCE))
                ...
                IconToBitmap(
                    pVWMisc,
                    gpWindowManager->m_screen,
                    screenX - iVWHalf[giViewWorldScaleLookup][OFFSET_RESOURCE][(COORDINATE_AXIS_X)],
                    drawY - iVWHalf[giViewWorldScaleLookup][OFFSET_RESOURCE][(COORDINATE_AXIS_Y)],
                    RESOURCE_ICON_FRAME,
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
                ...
                IconToBitmap(
                    pVWLetters,
                    gpWindowManager->m_screen,
                    screenX - iVWHalf[giViewWorldScaleLookup][OFFSET_LETTER][(COORDINATE_AXIS_X)],
                    drawY - iVWHalf[giViewWorldScaleLookup][OFFSET_LETTER][(COORDINATE_AXIS_Y)],
                    frame,
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
            ...
            if ((cell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_MINE))
                ...
                color = gpGame->m_mineOwners[cell->m_objectMetadata];
                frame = color < 0 ? WORLD_NO_OWNER_COLOR
                                                  : gpGame->m_players[color].m_color;
                IconToBitmap(
                    pVWMisc,
                    gpWindowManager->m_screen,
                    screenX - iVWHalf[giViewWorldScaleLookup][OFFSET_MINE][(COORDINATE_AXIS_X)],
                    drawY - iVWHalf[giViewWorldScaleLookup][OFFSET_MINE][(COORDINATE_AXIS_Y)],
                    frame,
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
                ...
                letterY[SCALE_INDEX_MIDDLE] = 0;
                letterY[SCALE_INDEX_NEAR] = 2;
                resource = gpGame->m_mines[cell->m_objectMetadata].resourceType;
                IconToBitmap(
                    pVWLetters,
                    gpWindowManager->m_screen,
                    screenX - iVWHalf[giViewWorldScaleLookup][OFFSET_LETTER][(COORDINATE_AXIS_X)],
                    drawY + letterY[giViewWorldScaleLookup]
                        - iVWHalf[giViewWorldScaleLookup][OFFSET_LETTER][(COORDINATE_AXIS_Y)],
                    (resource),
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
