    ...
    for (cellY = iVWMapOriginY; cellY < iVWMapOriginY + iVWViewableCells; cellY++) {
        for (cellX = iVWMapOriginX; cellX < iVWMapOriginX + iVWViewableCells; cellX++) {
            ...
            if ((MAP_EXTRA_AT_WFIRST(cellX, cellY) & giCurPlayerBit) || iVWDrawAllTerrains
                ...
                drawY1 = iVWYPixelOffset + (cellY - iVWMapOriginY) * (giViewWorldScale);
                shape =
                ...
                frame0 += (CELL_TERRAIN(cell)) * WORLD_TERRAIN_FRAME_STRIDE;
                if ((MAP_EXTRA_AT_WFIRST(cellX, cellY) & giCurPlayerBit) || iVWDrawAllTerrains) {
                    pVWGround->DrawToBuffer(
                        screenX
                            + (orientation1 == ICON_DRAW_FLIPPED ? (giViewWorldScale) - 1 : 0),
                        drawY1,
                        frame0,
                        orientation1
                    );
                ...
                if (cell->m_objectLayerBit0 && cell->m_objectIndex != WORLD_NO_SPRITE
                    && drawTilesets0[(cell->m_objectTileset)]) {
                    IconToBitmapScale(
                ...
                while (extraCell != NULL) {
                    if (extraCell->objectLayerBit0 && extraCell->objectIndex != WORLD_NO_SPRITE
                        && drawTilesets0[(extraCell->objectTileset)]) {
                        IconToBitmapScale(
                ...
                if (cell->m_overlayIndex != WORLD_NO_SPRITE
                    && drawTilesets0[(cell->m_overlayTileset)]) {
                    IconToBitmapScale(
                ...
                if (cell->m_extraIndex != 0
                    ...
                    extraCell = NULL;
                while (extraCell != NULL) {
                    if (drawTilesets0[(extraCell->overlayTileset)]) {
                        IconToBitmapScale(
        ...
        for (cellX = iVWMapOriginX; cellX < iVWMapOriginX + iVWViewableCells; cellX++) {
            ...
            if (cell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_ARTIFACT))
                ...
                IconToBitmap(
                    ...
                    drawY1 - iVWHalf[giViewWorldScaleLookup][OFFSET_ARTIFACT][(COORDINATE_AXIS_Y)],
                    ARTIFACT_ICON_FRAME,
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
            ...
            if ((cell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_CASTLE))
                ...
                FlipIconToBitmap(
                    ...
                    drawY1 - iVWHalf[giViewWorldScaleLookup][OFFSET_TOWN][(COORDINATE_AXIS_Y)],
                    color0,
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
            ...
            if ((cell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_HERO_INTERACTION))
                ...
                if (color0 >= 0) {
                    ...
                    IconToBitmap(
                        ...
                        drawY1 - iVWHalf[giViewWorldScaleLookup][OFFSET_HERO][(COORDINATE_AXIS_Y)],
            ...
            if (cell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_RESOURCE))
                ...
                IconToBitmap(
                    ...
                    drawY1 - iVWHalf[giViewWorldScaleLookup][OFFSET_RESOURCE][(COORDINATE_AXIS_Y)],
                    RESOURCE_ICON_FRAME,
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
                ...
                IconToBitmap(
                    ...
                    drawY1 - iVWHalf[giViewWorldScaleLookup][OFFSET_LETTER][(COORDINATE_AXIS_Y)],
                    frame0,
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
            ...
            if ((cell->m_triggerType == (MAP_ACTION_TRIGGER(MAP_OBJECT_MINE))
                ...
                IconToBitmap(
                    ...
                    drawY1 - iVWHalf[giViewWorldScaleLookup][OFFSET_MINE][(COORDINATE_AXIS_Y)],
                    frame0,
                    ICON_DRAW_CLIP,
                    WORLD_LEFT,
                    WORLD_TOP,
