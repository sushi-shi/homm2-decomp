...
i32 game::SetupComPort(void) {
    ...
    if (gbDirectConnect == 0) {
        ...
        GetDataEntry(
            "Пожалуйста, укажите значение строки инициализации вашего модема или нажмите 'ENTER', чтобы присвоить значения по умолчанию."
            ,
            initString,
...
i32 game::SetupNetworkGame2(void) {
    ...
    if (hLib == NULL) {
        ...
        dimMessage.payload.widget.data.value = (WIDGET_FLAGS_ARGUMENT_DIMMED);
