...
i32 combatManager::DoSpellAI(CombatSide side, i32 restricted) {
    ...
    for (spellIndex = (COMBAT_ATTACKER_SIDE); (spellIndex) < COMBAT_SIDE_COUNT; spellIndex++) {
    ...
    for (spellIndex = SPELL_FIREBALL; spellIndex < SPELL_COUNT; spellIndex++) {
        if (m_heroes[(side)]->HasSpell(spellIndex)
            && (((gsSpellInfo[(spellIndex)].attributes) & (SPELL_INFO_ATTRIBUTE_COMBAT)))
        ...
        switch (spell) {
            ...
            case SPELL_ANTI_MAGIC:
                if (targetCreature->m_spellInfluence[(ARMY_SPELL_INFLUENCE_ANTI_MAGIC)]) {
                    ...
                    if (spell == SPELL_ANTI_MAGIC && m_heroes[(OppositeCombatSide(m_currentSide))] == NULL) {
                        effect = 0;
                    } else {
                        effect = static_cast<i32>(
                            RawEffectSpellInfluence(
            ...
            case SPELL_EARTHQUAKE:
                if (m_currentSide == COMBAT_ATTACKER_SIDE && m_inCastleCombat != 0) {
                    ...
                    for (idx = 0; idx < COMBAT_WALL_SECTION_COUNT; idx++) {
                        if (m_wallStates[idx + (COMBAT_WALL_SLOT_SECTION_FIRST)]
...
i32 combatManager::EffectSpellCreateCreature(i32 hex, SpellType spell) {
    ...
    i32 creatureEffect = (creatureType) * gMonsterDatabase[(creatureType)].fightValue;
    if (spell == SPELL_MIRROR_IMAGE) {
        ...
        if ((((gMonsterDatabase[(creatureType)].flags.abilityFlags) & (MONSTER_ABILITY_FLAG_SHOOTER))))
            creatureEffect =
...
i32 combatManager::RawEffectSpellInfluence(army* target, ArmySpellInfluence influence) {
    ...
    switch (influence) {
        ...
        case ARMY_SPELL_INFLUENCE_DRAGON_SLAYER: {
            ...
            i32 i;
            for (i = 0; i < m_armyCount[(OppositeCombatSide(target->m_side))]; i++) {
            ...
            if (adjacent)
                ...
                dragonMod = static_cast<float>(dragonCount / m_armyCount[(OppositeCombatSide(target->m_side))]);
        ...
        case ARMY_SPELL_INFLUENCE_SHIELD: {
            ...
            float shooterMod = static_cast<float>(shooterCount / m_armyCount[(OppositeCombatSide(target->m_side))]);
            ...
            if (m_armies[sideWork][index].IsAlive()) {
                ...
                if (cure == 1) {
                    ...
                    positiveEffectResult = static_cast<i32>(
                        ...
                        + static_cast<float>(curePointsTotal)
...
void combatManager::EffectSpellResurrect(i32* effect, i32 hex, SpellType spell) {
    ...
    *effect = quantityResult[0] * gMonsterDatabase[(targetStack->m_monsterType)].fightValue;
