        ...
        for (direction = COMBAT_DIRECTION_NORTHEAST;
            ...
            if (fromHex % COMBAT_GRID_ROW_LENGTH == 0
                ...
                if (destinationHex % COMBAT_GRID_ROW_LENGTH == 0
                    ...
                    m_adjacency[fromHex][(direction)] = static_cast<i8>(destinationHex);
            }
        }
    }
}
    ...
    for (speedLoop = 0; speedLoop < COMBAT_SPEED_LEVEL_COUNT; speedLoop++) {
        for (sideLoop = 0; sideLoop < COMBAT_SIDE_COUNT; sideLoop++) {
            ...
            for (armyCounter = 0; armyCounter < m_armyCount[(stackSide)]; armyCounter++) {
                ...
                if ((((activeArmy->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED | MONSTER_ABILITY_FLAG_BAD_MORALE)))
...
void combatManager::CatAttack(CombatSide side) {
    ...
    i32 impactY0 = -1;
    if (wallIndex17 != -1) {
        impactX5 = wallPos[wallIndex17][(COORDINATE_AXIS_X)];
        impactY0 = wallPos[wallIndex17][(COORDINATE_AXIS_Y)];
        targetHex4 = CombatCastleHex(iWallToHexCell[wallIndex17]);
    }
    if (towerIndex27 != COMBAT_WALL_SLOT_NONE) {
        impactX5 = towerPos[(towerIndex27)][(COORDINATE_AXIS_X)];
        impactY0 = towerPos[(towerIndex27)][(COORDINATE_AXIS_Y)];
        targetHex4 = CombatCastleHex(iTowerToHexCell[(towerIndex27)]);
    }
    if (gateIndex2 != -1) {
        impactX5 = doorPos[0][(COORDINATE_AXIS_X)];
        impactY0 = doorPos[0][(COORDINATE_AXIS_Y)];
...
void combatManager::KeepAttack(CombatTowerSelector tower) {
    ...
    for (armyIndex3 = 0; armyIndex3 < COMBAT_ARMY_CAPACITY; armyIndex3++) {
        if (m_armies[(COMBAT_ATTACKER_SIDE)][armyIndex3].IsAlive()) {
            ...
            value26 =
                gMonsterDatabase[(target0->m_monsterType)].fightValue * target0->m_quantity;
    ...
    if (killed29 > 0) {
        sprintf(
            ...
            "damage",
            killed29,
            killed29 > 1 ? gArmyNamesPlural[(target0->m_monsterType)]
                         : gArmyNames[(target0->m_monsterType)],
...
void combatManager::ShootMissile(
    ...
    i8 reverseMissile7 = ICON_DRAW_NORMAL;
