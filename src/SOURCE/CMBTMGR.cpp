...
void combatManager::CatAttack(CombatSide side) {
    ...
    if (towerIndex != COMBAT_WALL_SLOT_NONE) {
        ...
        targetHex = CombatCastleHex(iTowerToHexCell[(towerIndex)]);
    }
    if (gateIndex != -1) {
...
void combatManager::KeepAttack(CombatTowerSelector tower) {
    ...
    for (armyIndex = 0; armyIndex < COMBAT_ARMY_CAPACITY; armyIndex++) {
        if (m_armies[(COMBAT_ATTACKER_SIDE)][armyIndex].IsAlive()) {
            ...
            } else if ((((target->m_monster.flags.all) & (MONSTER_FLAGS_SHOOTER)))) {
                priority = KEEP_PRIORITY_SHOOTER;
            } else if ((((target->m_monster.flags.all) & (MONSTER_FLAGS_FLYING)))) {
