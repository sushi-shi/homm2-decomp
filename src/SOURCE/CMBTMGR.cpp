...
void combatManager::InitNonVisualVars(void) {
    ...
    m_sideDefeated[(COMBAT_ATTACKER_SIDE)] = false;
    m_sideDefeated[(COMBAT_DEFENDER_SIDE)] = false;
        ...
        for (direction = COMBAT_DIRECTION_NORTHEAST;
            ...
            if (fromHex % COMBAT_GRID_ROW_LENGTH == 0
                ...
                if (toHex % COMBAT_GRID_ROW_LENGTH == 0
                    ...
                    m_adjacency[fromHex][(direction)] = toHex;
            }
        }
    }
}
...
void combatManager::UpdateArmyGroup(CombatSide side) {
    ...
    for (index = 0; index < m_armyCount[(side)]; index++) {
        if (!(((m_armies[(side)][index].m_monster.attributes) & (MONSTER_FLAGS_DEAD)))
            && m_armies[(side)][index].m_quantity > 0
            && (m_playerId[(side)] == -1
                || ((!IS_ELEMENTAL_CREATURE(m_armies[(side)][index].m_monsterType))
                    || !(((m_armies[(side)][index].m_monster.attributes) & (MONSTER_FLAGS_SUMMONED)))))
            && !(((m_armies[(side)][index].m_monster.attributes) & (MONSTER_FLAGS_MIRROR_IMAGE)))) {
...
i32 combatManager::MoreTreesNear(void) {
    ...
    i32 radius;
    i32 combatOriginX;
    NearbyFeature nearbyTypeTable[NEARBY_RADIUS_COUNT][(MAP_DIRECTION_COUNT)];
    u8 nearbyTileset;
    i32 nearbyDirection;
    i32 centerY;

    memset(nearbyTypeTable, (NEARBY_UNKNOWN), sizeof(nearbyTypeTable));
    combatOriginX = m_combatX;
    centerY = m_combatY;

    for (radius = 0; radius < NEARBY_RADIUS_COUNT; radius++) {
        for (nearbyDirection = 0; nearbyDirection < (MAP_DIRECTION_COUNT); nearbyDirection++) {
    ...
    for (speedIter = 0; speedIter < COMBAT_SPEED_LEVEL_COUNT; speedIter++) {
        for (sideLoop = 0; sideLoop < COMBAT_SIDE_COUNT; sideLoop++) {
            ...
            for (stackCounter = 0; stackCounter < m_armyCount[(stackSide)]; stackCounter++) {
                ...
                if ((((curArmy->m_monster.attributes) & (MONSTER_FLAGS_DEAD | MONSTER_FLAGS_TURN_SPENT)))
                ...
                if (((curArmy->m_monster.attributes) & (MONSTER_FLAGS_DEFERRED_TURN))) {
                    skipEnt = true;
                    hasPending = true;
                }
...
i32 combatManager::IsWinner(CombatSide side) {
    ...
    result = true;
    for (index = 0; index < m_armyCount[(side)]; index++) {
        if (!(m_armies[(side)][index].m_monster.attributes
              & MONSTER_FLAGS_DEAD))
            result = false;
    }
    return result;
...
void combatManager::CatAttack(CombatSide side) {
    ...
    i32 impactY = -1;
    if (wallIndex != -1) {
        impactX = wallPos[wallIndex][(COORDINATE_AXIS_X)];
        impactY = wallPos[wallIndex][(COORDINATE_AXIS_Y)];
        targetHex = iWallToHexCell[wallIndex];
    }
    if (towerIndex != COMBAT_WALL_SLOT_NONE) {
        impactX = towerPos[(towerIndex)][(COORDINATE_AXIS_X)];
        impactY = towerPos[(towerIndex)][(COORDINATE_AXIS_Y)];
        targetHex = iTowerToHexCell[(towerIndex)];
    }
    if (gateIndex != -1) {
        impactX = doorPos[0][(COORDINATE_AXIS_X)];
        impactY = doorPos[0][(COORDINATE_AXIS_Y)];
    ...
    for (frame = 0; frame < COMBAT_CATAPULT_PROJECTILE_FRAME_COUNT; frame++) {
        ...
        if (frame != 0) {
            ...
            giMinExtentY =
                static_cast<i32l>(
                    projectileY - (COMBAT_CATAPULT_PROJECTILE_EXTENT_RADIUS)
                );
            giMaxExtentY =
                static_cast<i32l>(
                    projectileY + (COMBAT_CATAPULT_PROJECTILE_EXTENT_RADIUS)
...
void combatManager::KeepAttack(CombatTowerSelector tower) {
    ...
    for (armyIndex = 0; armyIndex < COMBAT_ARMY_SLOT_COUNT; armyIndex++) {
        if (m_armies[(COMBAT_ATTACKER_SIDE)][armyIndex].IsAlive()) {
            target = &m_armies[(COMBAT_ATTACKER_SIDE)][armyIndex];
            if (ARMY_HAS_INCAPACITATING_SPELL(*target)
                || ARMY_HAS_BERSERK_OR_HYPNOTIZE(*target)) {
                priority = KEEP_PRIORITY_DISABLED;
            } else if ((((target->m_monster.attributes) & (MONSTER_FLAGS_SHOOTER)))) {
                priority = KEEP_PRIORITY_SHOOTER;
            } else if ((((target->m_monster.attributes) & (MONSTER_FLAGS_FLYING)))) {
            ...
            value =
                target->m_quantity * gMonsterDatabase[(target->m_monsterType)].fightValue;
    ...
    target = &gpCombatManager->m_armies[(COMBAT_ATTACKER_SIDE)][bestArmyIndex];
    sprintf(gText, "keepshot.82M");
    SAMPLE2 keepSample = LoadPlaySample(gText);

    CombatTowerOrigin towerOrigins[(FACTION_COUNT)][COMBAT_KEEP_TOWER_COUNT] = {
    ...
    i32 sourceX = towerOrigins[(m_combatTowns[(COMBAT_DEFENDER_SIDE)]->m_type)][(tower)].x;
    i32 sourceY = towerOrigins[(m_combatTowns[(COMBAT_DEFENDER_SIDE)]->m_type)][(tower)].y;
    ...
    i32 shotCount;
    i32 attackBonus;
    m_combatTowns[(COMBAT_DEFENDER_SIDE)]->CalcNumLevelArchers(&shotCount, &attackBonus);
    attackBonus += COMBAT_KEEP_TOWER_DAMAGE_BONUS;
    if (m_heroes[(COMBAT_DEFENDER_SIDE)])
        attackBonus += m_heroes[(COMBAT_DEFENDER_SIDE)]->Stats(HERO_PRIMARY_ATTACK);
    ...
    for (iSide = 0; iSide < COMBAT_SIDE_COUNT; iSide++) {
        for (armyIndex = 0; armyIndex < gpCombatManager->m_armyCount[iSide]; armyIndex++) {
            if (m_removedArmies[iSide][armyIndex]) {
                ...
                if ((((removedArmy->m_monster.attributes) & (MONSTER_FLAGS_WIDE)))) {
                    CLEAR_HEX_OCCUPANT(
                        m_hexCells
