...
u8 bIsMoatSlowed[COMBAT_HEX_COUNT] = {};
...
void searchArray::SeedCombatPosition(class army* unit) {
    ...
    for (index = 0;
         ...
         index++) {
        enemy = &gpCombatManager->m_armies[(OppositeCombatSide(unit->m_side))][index];
...
i32 searchArray::FindCombatPath(
    ...
    if (gpCombatManager->m_drawbridgeBackgroundVisible != 0) {
        ...
        if ((((unit->m_monster.attributes) & (MONSTER_FLAGS_WIDE))) != 0) {
            sourceWideHex =
    ...
    while (m_queueCount > 0) {
        ...
        if (unit->m_targetSide != COMBAT_SIDE_NONE) {
            ...
            if (attackMask != COMBAT_ALL_DIRECTIONS_BLOCKED) {
                for (searchDirection = 0; searchDirection < (MAP_DIRECTION_COUNT);
                     searchDirection++) {
        ...
        for (searchDirection = 0; searchDirection < (MAP_DIRECTION_COUNT); searchDirection++) {
            i32 nextHex;
