...
typedef i32 FileRequesterHelpIndex;
enum {
...
typedef i32 RequesterIconFrame;
            ...
            if (m_mode == FILE_REQUESTER_MAP_GAME) {
                ...
                if (header.minHumanPlayers > giNumHumanPlayers
                    ...
                    || (giMapSizeFilter != FILE_REQUESTER_MAP_SIZE_ALL
                        && header.width != giMapSizes[(giMapSizeFilter)]))
                    goto CountNextFile;
                    ...
                    if ((((message.payload.widget.modifiers) & (MESSAGE_MODIFIER_RIGHT_BUTTON)))) {
                        ...
                        switch (message.payload.widget.id) {
                            ...
                            case FILE_REQUESTER_SCROLL_GUTTER: {
                                ...
                                if (positions < 1)
                                    positions = 1;
                                stepScreen = ((fGutterTravelLength * (FILE_REQUESTER_GUTTER_SCALE)) / positions);
...
void fileRequester::Update(i32 drawWindow) {
    ...
    tag_message message;
    char localStorage [[maybe_unused]][FILE_REQUESTER_UPDATE_STORAGE_SIZE];
    i32 i;
    ...
    if (m_mode == FILE_REQUESTER_MAP_GAME || m_mode == FILE_REQUESTER_MAP) {
        for (i = 0; i < (FILE_REQUESTER_MAP_SIZE_COUNT); ++i) {
            ...
            message.payload.widget.data.value =
                (i == (giMapSizeFilter)) + i * FILTER_FRAME_STEP + FILTER_FRAME_BASE;
            m_window->BroadcastMessage(message);
        }
        ...
        if (m_mapHeaders[m_selectedIndex].width == MAP_DIMENSION_SMALL) {
            message.payload.widget.data.value = (MAP_SIZE_FRAME_SMALL);
        } else if (m_mapHeaders[m_selectedIndex].width == MAP_DIMENSION_MEDIUM) {
            message.payload.widget.data.value = (MAP_SIZE_FRAME_MEDIUM);
        } else if (m_mapHeaders[m_selectedIndex].width == MAP_DIMENSION_LARGE) {
            message.payload.widget.data.value = (MAP_SIZE_FRAME_LARGE);
        } else {
            message.payload.widget.data.value = (MAP_SIZE_FRAME_XLARGE);
        }
        m_window->BroadcastMessage(message);
    ...
    for (i = 0; i < iMaxListSize; ++i) {
        ...
        if (m_selectedIndex == m_topIndex + i) {
            message.payload.widget.data.value = (FONT_DRAW_YELLOW);
        } else {
    ...
    if (m_fileCount <= iMaxListSize) {
        m_scrollKnob->m_y =
            (fGutterTravelLength / (SCROLL_CENTER_DIVISOR) + fGutterMinY);
    } else {
