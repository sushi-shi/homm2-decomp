    ...
    switch (spell) {
        ...
        case SPELL_MASS_FORCE_SHIELD:
            ...
            if (m_hexCells[hex].m_occupantSide != m_currentSide)
                return 0;
            if (target
                && gIronfistExtra.combat.stack.forceShieldHP[target]
...
void combatManager::CastSpell(
    ...
    switch (spell) {
        case SPELL_TELEPORT:
            ...
            if ((H2EnumIndex((teleportArmy->m_monster.attributes) & (MONSTER_FLAGS_WIDE))) != 0) {
        ...
        case SPELL_MARKSMAN_PIERCE: {
            ...
            utf8::Format(
                ...
                target->m_quantity > 1 ? GetCreaturePluralName(H2EnumIndex(target->m_monsterType))
...
static std::vector<i32> GetPlasmaConeSpellMask(i32 fromHex, i32 direction) {
    static const i32 spellNeighbourDirections[COMBAT_DIRECTION_ADJACENT_COUNT][3] = {
        {5, 0, 1}, // right up
        {0, 1, 2}, // right
        {1, 2, 3}, // right down
        {2, 3, 4}, // left down
...
static bool IsOutOfBoundsHex(i32 hexIndex) {
    return !ValidHex(hexIndex)
           || !(hexIndex % COMBAT_GRID_ROW_LENGTH
...
std::vector<i32> combatManager::GetSpellMask(SpellType spell, i32 fromHex, i32 direction) {
    ...
    switch (spell) {
        ...
        case SPELL_METEOR_SHOWER:
            ...
            break;
        case SPELL_COLD_RING:
            for (i32 dir = 0; dir < COMBAT_DIRECTION_ADJACENT_COUNT; dir++) {
                i32 neighbor =
            ...
            for (i32 dir = 0; dir < COMBAT_DIRECTION_ADJACENT_COUNT; dir++) {
                ...
                if (ValidSpellTarget(spell, closestNeighbor)) {
                    i32 rightHex = GetAdjacentCellIndexNoArmy(
                        closestNeighbor,
                        CombatHexDirectionFromOrdinal((dir + 1) % COMBAT_DIRECTION_ADJACENT_COUNT)
                    );
                    ...
                    i32 leftHex = GetAdjacentCellIndexNoArmy(
                        ...
                        CombatHexDirectionFromOrdinal(
                            (dir + COMBAT_DIRECTION_ADJACENT_COUNT - 1)
...
void combatManager::ImplosionGrenade(i32 hexIndex) {
    ...
    i32 centerRow = hexIndex / COMBAT_GRID_ROW_LENGTH;
    i32 centerColumn = hexIndex % COMBAT_GRID_ROW_LENGTH;
    for (i32 affectedHex : affectedHexes) {
        i32 rowDiff = centerRow - affectedHex / COMBAT_GRID_ROW_LENGTH;
        i32 columnDiff = centerColumn - affectedHex % COMBAT_GRID_ROW_LENGTH;
        i32 pullDirection;
        if (rowDiff < 0) {
            if (columnDiff < 0) {
...
void combatManager::MeteorShower(i32 targetHex) {
    ...
    hexes[0] = targetHex;
    for (direction = H2EnumIndex(COMBAT_DIRECTION_NORTHEAST); direction < COMBAT_DIRECTION_ADJACENT_COUNT;
         ++direction) {
        hexes[direction + 1] =
...
void combatManager::Armageddon(void) {
    ...
    if (!gbNoShowCombat) {
        ...
        memcpy(
            ...
            H2EnumIndex(LOGICAL_SCREEN_WIDTH) * COMBAT_AREA_HEIGHT
        );
...
void combatManager::CastMassSpell(SpellType spell, i32 spellPower) {
    ...
    switch (spell) {
        ...
        case SPELL_HOLY_SHOUT: {
            ...
            for (side = COMBAT_ATTACKER_SIDE; H2EnumIndex(side) < COMBAT_SIDE_COUNT; ++side) {
                for (armyIndex = 0; armyIndex < m_armyCount[H2EnumIndex(side)]; ++armyIndex) {
                    if ((H2EnumIndex((m_armies[H2EnumIndex(side)][armyIndex].m_monster.attributes) & (MONSTER_FLAGS_UNDEAD)))
    ...
    while (distance < MIRROR_SEARCH_DISTANCE_LIMIT) {
        for (sourcePart = 0; sourcePart < MIRROR_SOURCE_PART_COUNT; ++sourcePart) {
            if (sourcePart == 0) {
                ...
                if ((H2EnumIndex((source->m_monster.attributes) & (MONSTER_FLAGS_WIDE)))) {
...
void combatManager::Earthquake(void) {
    ...
    for (pass = 0; pass < EARTHQUAKE_SHAKE_PASS_COUNT; ++pass) {
        for (frame = 0; frame < EARTHQUAKE_SHAKE_FRAME_COUNT; ++frame) {
            ...
            PollSound();
            width = LOGICAL_SCREEN_WIDTH - abs(shakeOffsets[frame][H2EnumIndex(COORDINATE_AXIS_X)]);
    ...
    for (index = 0; index < EARTHQUAKE_STRUCTURE_COUNT; ++index) {
        ...
        if (m_wallStates[index + H2EnumIndex(COMBAT_WALL_SLOT_SECTION_FIRST)]
            ...
            if (newWallStates[index] == COMBAT_WALL_STATE_DESTROYED
                ...
                m_hexCells[H2EnumIndex(iWallToHexCell[index])].m_blocked = 0;
        }
    ...
    if (castByCreature != 0) {
        if (spell == SPELL_PARALYZE)
            ...
            utf8::Format(
                ...
                localization::Tr("combat.ability.archmage_dispel"), targetName
            );
        else if (spell == SPELL_SHADOW_MARK)
            utf8::Format(
                message, sizeof(message),
                localization::Tr("combat.ability.shadow_mark"), targetName
            );
    } else {
        if (target != NULL) {
