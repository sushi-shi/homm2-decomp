        ...
        switch (m_selectedSpell) {
            ...
            check_elemental:
                ...
                if (m_armyCount[(m_currentSide)] >= ELEMENTAL_ARMY_LIMIT) {
                    sprintf(
                        gText,
                        "У вас уже есть %d групп "
                        "воинов в бою и больше "
                        "добавить нельзя."
                        ,
                ...
                if (!gpCombatManager->ValidSpellTarget(gpCombatManager->m_selectedSpell, hex)) {
                    ...
                    if (gpCombatManager->m_selectedSpell == SPELL_TELEPORT && bInTeleportGetDest) {
                        gpCombatManager->CombatMessage(
                              "Недопустимое\x20место\x20для\x20телепортации\x20",
                            1, 0, 0
                        );
                    } else {
                        gpCombatManager->CombatMessage(
                              "Выберите\x20цель\x20для\x20заклинания",
                            1, 0, 0
                        );
                    }
                } else {
            ...
            if (indexToCastOn != NO_SELECTION) {
                if (bInTeleportGetDest) {
                    ...
                    if (gpCombatManager->m_selectedSpell == SPELL_TELEPORT) {
                        ...
                        gpCombatManager->CombatMessage(
                              "Выберите\x20место\x20назначения\x20телепортации\x2e",
                            1, 0, 0
                        );
                        return MESSAGE_DISPATCH_CONSUME;
                    }
...
i32 combatManager::ValidSpellTarget(SpellType spell, i32 hex) {
    ...
    if (spell != SPELL_FIREBALL && spell != SPELL_FIREBLAST && spell != SPELL_METEOR_SHOWER
        ...
        if (target_j->m_spellInfluence[(ARMY_SPELL_INFLUENCE_ANTI_MAGIC)] != 0
...
void combatManager::SpellMessage(SpellType spell, i32 hex) {
    ...
    switch (spell) {
        ...
        format_target:
            sprintf(
                gText,
                  "Направить\x20\x27\x25\x73\x27\x20на\x20\x25\x73",
                gSpellNames[(spell)],
...
void combatManager::CastSpell(
    ...
    i32 unusedCastA8 [[maybe_unused]];
    i32 unusedCastB1 [[maybe_unused]];
    ...
    if (spell == SPELL_FIREBALL || spell == SPELL_FIREBLAST || spell == SPELL_COLD_RING
        ...
        target3 = &m_armies[(m_hexCells[targetHex].m_occupantSide)]
    ...
    if (castByCreature != 0) {
        ...
        spellPower6 = m_spellPower[(m_currentSide)];
    ...
    if (strlen(gsSpellInfo[(soundSpell18)].soundName) > 0)
        sprintf(sampleName10, "%s.82M", gsSpellInfo[(soundSpell18)].soundName);

    if (castByCreature == 0 && target3 != NULL && !target3->SpellCastWorks(spell)) {
        ShowSpellCastFailure(target3, (spell));
        goto cast_done;
    }
    ...
    switch (spell) {
        case SPELL_TELEPORT:
            ...
            if ((((teleportArmy6->m_monster.flags.all) & (MONSTER_FLAGS_WIDE))) != 0) {
        ...
        case SPELL_DISRUPTING_RAY:
            ...
            sprintf(
                gText,
                "Разрушительный\x20луч\x20уменьшает\x20защиту\x20на\x20\x25\x64\x20ед\x2e",
                oldDefense - target3->m_monster.defense
            );
        ...
        case SPELL_COLD_RAY:
            ...
            sprintf(
                gText,
                "Хладный\x20луч\x20наносит\x20\x25\x64\x0a\x20ед\x2e\x20урона\x20отряду\x20\x25\x73\x2e",
                static_cast<i32>(damage1),
                target3->m_quantity <= PLURAL_QUANTITY_MINIMUM - 1
                    ? gArmyNames[(target3->m_monsterType)]
                    : gArmyNamesPlural[(target3->m_monsterType)]
            );
            CombatMessage(gText, 1, 1, 0);
            DoBlast(targetHex, spell);
            target3->SpellEffect(gsSpellInfo[(SPELL_COLD_RAY)].combatEffect, 0, 0);
        ...
        case SPELL_MAGIC_ARROW:
            if (1) {
                ...
                sprintf(
                    gText,
                    "Волшебная\x20стрела\x20наносит\x20\x25\x64\x0a\x20ед\x2e\x20урона\x20отряду\x20\x25\x73\x2e",
                    static_cast<i32>(damage1),
                    target3->m_quantity <= PLURAL_QUANTITY_MINIMUM - 1
                        ? gArmyNames[(target3->m_monsterType)]
                        : gArmyNamesPlural[(target3->m_monsterType)]
        ...
        case SPELL_LIGHTNING_BOLT:
            ...
            sprintf(
                gText,
                "Разряд\x20молнии\x20наносит\x20\x25\x64\x0a\x20ед\x2e\x20урона\x20отряду\x20\x25\x73\x2e",
                static_cast<i32>(damage1),
                target3->m_quantity <= PLURAL_QUANTITY_MINIMUM - 1
                    ? gArmyNames[(target3->m_monsterType)]
                    : gArmyNamesPlural[(target3->m_monsterType)]
            ...
            target3->SpellEffect(gsSpellInfo[(SPELL_LIGHTNING_BOLT)].combatEffect, 0, 0);
            target3->Damage(damage1, SPELL_NONE);
            target3->PowEffect(COMBAT_EFFECT_INVALID, 1, -1, -1);
            break;
        case SPELL_MASS_CURE:
        ...
        case SPELL_CURE:
            ...
            target3->SpellEffect(gsSpellInfo[(SPELL_CURE)].combatEffect, 0, 0);
        ...
        case SPELL_SLOW:
            ...
            target3->SpellEffect(gsSpellInfo[(SPELL_SLOW)].combatEffect, 0, 0);
            break;
        case SPELL_HASTE:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->SetSpellInfluence(ARMY_SPELL_INFLUENCE_HASTE, spellPower6);
            target3->SpellEffect(gsSpellInfo[(SPELL_HASTE)].combatEffect, 0, 0);
            break;
        case SPELL_SHIELD:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->SetSpellInfluence(ARMY_SPELL_INFLUENCE_SHIELD, spellPower6);
            target3->SpellEffect(gsSpellInfo[(SPELL_SHIELD)].combatEffect, 0, 0);
            break;
        case SPELL_DRAGON_SLAYER:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->SetSpellInfluence(ARMY_SPELL_INFLUENCE_DRAGON_SLAYER, spellPower6);
            target3->SpellEffect(gsSpellInfo[(SPELL_DRAGON_SLAYER)].combatEffect, 0, 0);
            break;
        case SPELL_BLESS:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->SetSpellInfluence(ARMY_SPELL_INFLUENCE_BLESS, spellPower6);
            target3->SpellEffect(gsSpellInfo[(SPELL_BLESS)].combatEffect, 0, 0);
            break;
        case SPELL_STONE_SKIN:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->SetSpellInfluence(ARMY_SPELL_INFLUENCE_STONESKIN, spellPower6);
            target3->SpellEffect(gsSpellInfo[(SPELL_STONE_SKIN)].combatEffect, 0, 0);
            break;
        case SPELL_STEEL_SKIN:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->SetSpellInfluence(ARMY_SPELL_INFLUENCE_STEELSKIN, spellPower6);
            target3->SpellEffect(gsSpellInfo[(SPELL_STEEL_SKIN)].combatEffect, 0, 0);
            break;
        case SPELL_CURSE:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->SetSpellInfluence(ARMY_SPELL_INFLUENCE_CURSE, spellPower6);
            target3->SpellEffect(gsSpellInfo[(SPELL_CURSE)].combatEffect, 0, 0);
            break;
        case SPELL_BERSERKER:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->SetSpellInfluence(ARMY_SPELL_INFLUENCE_BERSERK, spellPower6);
            target3->SpellEffect(gsSpellInfo[(SPELL_BERSERKER)].combatEffect, 0, 0);
            break;
        case SPELL_HYPNOTIZE:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->SetSpellInfluence(ARMY_SPELL_INFLUENCE_HYPNOTIZE, spellPower6);
            target3->SpellEffect(gsSpellInfo[(SPELL_HYPNOTIZE)].combatEffect, 0, 0);
            break;
        case SPELL_PARALYZE:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->SetSpellInfluence(ARMY_SPELL_INFLUENCE_PARALYZE, spellPower6);
            target3->SpellEffect(gsSpellInfo[(SPELL_PARALYZE)].combatEffect, 0, 0);
            break;
        case CREATURE_SPELL_DISPEL:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->DispelGood();
            target3->SpellEffect(gsSpellInfo[(SPELL_DISPEL)].combatEffect, 0, 1);
            break;
        case SPELL_DISPEL:
            ShowSpellMessage(castByCreature, spell, target3);
            target3->DispelGood();
            target3->SpellEffect(gsSpellInfo[(SPELL_DISPEL)].combatEffect, 0, 0);
        ...
        case SPELL_BLIND:
            ...
            target3->SpellEffect(gsSpellInfo[(SPELL_BLIND)].combatEffect, 0, 0);
            break;
        case SPELL_BLOOD_LUST:
            ShowSpellMessage(castByCreature, spell, target3);
        ...
        case SPELL_ANTI_MAGIC:
            ...
            target3->SpellEffect(gsSpellInfo[(SPELL_ANTI_MAGIC)].combatEffect, 0, 0);
            break;
...
cast_done:
    ...
    for (side3 = 0; side3 < COMBAT_SIDE_COUNT; side3++) {
        for (armyIndex = 0; armyIndex < m_armyCount[(side3)]; armyIndex++) {
            army& combatArmy = m_armies[(side3)][armyIndex];
    ...
    for (direction = 0; direction < SPELL_METEOR_AFFECTED_HEX_COUNT; ++direction) {
        if (hexes[direction] != COMBAT_HEX_EMPTY
            ...
            if (target->SpellCastWorks(SPELL_METEOR_SHOWER)
                && !*(
                    gArmyEffected[0]
                    + (m_hexCells[hexes[direction]].m_occupantSide) * COMBAT_ARMY_SLOT_COUNT
                    + m_hexCells[hexes[direction]].m_occupantIndex
                )) {
                *(gArmyEffected[0]
                  + (m_hexCells[hexes[direction]].m_occupantSide) * COMBAT_ARMY_SLOT_COUNT
...
void combatManager::ElementalStorm(void) {
    ...
    if (hit) {
        sprintf(
            gText,
            "Буря\x20стихий\x20наносит\x20\x25\x64\x20ед\x2e\x20урона\x2e",
            baseDam
        );
        CombatMessage(gText, 1, 1, 0);
    ...
    if (!gbNoShowCombat) {
        ...
        for (pass1 = 0; pass1 < SPELL_ARMAGEDDON_BRIGHTEN_PASS_COUNT; ++pass1) {
            for (color9 = 0; color9 < SPELL_ARMAGEDDON_PALETTE_COLOR_COUNT; ++color9) {
                if (effectData28[color9 * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_RED)]
                    < SPELL_ARMAGEDDON_CHANNEL_MAX)
                    ++effectData28[color9 * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_RED)];
                if (effectData28[color9 * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_GREEN)]
                    < SPELL_ARMAGEDDON_CHANNEL_MAX)
                    ++effectData28[color9 * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_GREEN)];
                if (effectData28[color9 * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_BLUE)]
                    < SPELL_ARMAGEDDON_CHANNEL_MAX)
                    ++effectData28[color9 * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_BLUE)];
            }
            DelayTil(&glTimers[0]);
        ...
        i32 shakeOffsets15[SPELL_ARMAGEDDON_SHAKE_FRAME_COUNT][(COORDINATE_AXIS_COUNT)] = {
            {2, 2},
            {4, 1},
            {3, -2},
            {0, -6},
        ...
        for (shakePass5 = 0; shakePass5 < SPELL_ARMAGEDDON_SHAKE_PASS_COUNT; ++shakePass5) {
            ...
            for (frame8 = 0; frame8 < SPELL_ARMAGEDDON_SHAKE_FRAME_COUNT; ++frame8) {
                if (shakePass5 == 0) {
                    for (color9 = 0; color9 < SPELL_ARMAGEDDON_PALETTE_COLOR_COUNT; ++color9) {
                        if (effectData28
                                [color9 * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_GREEN)]
                            > SPELL_ARMAGEDDON_CHANNEL_STEP)
                            effectData28
                                [color9 * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_GREEN)] -=
                                SPELL_ARMAGEDDON_CHANNEL_STEP;
                        if (effectData28
                                [color9 * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_BLUE)]
                            > SPELL_ARMAGEDDON_CHANNEL_STEP)
                            effectData28
                                [color9 * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_BLUE)] -=
                                SPELL_ARMAGEDDON_CHANNEL_STEP;
                ...
                offsetX18 =
                    static_cast<i32>(shakeOffsets15[frame8][(COORDINATE_AXIS_X)] * scale1);
                offsetY9 = static_cast<i32>(shakeOffsets15[frame8][(COORDINATE_AXIS_Y)] * scale1);
                ...
                copyWidth =
                    COMBAT_SCREEN_WIDTH
                    - abs(static_cast<i32>(shakeOffsets15[frame8][(COORDINATE_AXIS_X)] * scale1));
                copyHeight1 =
                    COMBAT_AREA_HEIGHT
                    - abs(static_cast<i32>(shakeOffsets15[frame8][(COORDINATE_AXIS_Y)] * scale1))
                    - 1;
...
void combatManager::Ripple(i32 strength) {
    i32 unusedRippleWord1 [[maybe_unused]];
    i32 unusedRippleWord2 [[maybe_unused]];
...
void combatManager::ResetBoltAngle(SBolt* bolt) {
    i32 angleX7;
    float averageAngle0;
    i32 unusedBoltAngleWord8 [[maybe_unused]];
    i32 width6;
    i32 angleY9;
    i32 distance2;
    i32 widthDelta0;
...
void combatManager::DrawBolt(SBolt* bolt, i32 stepCount) {
    ...
    i32 distance15;
    i32 unusedBoltWord4 [[maybe_unused]];
    i32 unusedDrawWord1 [[maybe_unused]];
    i32 beamOffset0;
    i32 drawX6;
    i32 drawStep;
    i32 drawY;
...
void combatManager::ChainLightning(i32 targetHex, i32 spellPower) {
    ...
    army* target1;
    i32 deltaY5;
    i32 unusedChainWord64 [[maybe_unused]];
    i32 deltaX3;
    i32 targetY;
    i32 startY1;
    i32 targetX9;
    ...
    i32 strike18;
    i32 forceAngle4;
    i32 unusedChainWord5 [[maybe_unused]];
    i32 targetDamage9;
    i32 deadline4;
    i32 unusedChainWord8 [[maybe_unused]];
    i32 branchDistance6;
    ...
    for (strike18 = 0; strike18 < CHAIN_LIGHTNING_MAX_TARGETS; ++strike18) {
        target1 = &m_armies[(m_hexCells[targetHex].m_occupantSide)]
                            [m_hexCells[targetHex].m_occupantIndex];
        ...
        damage >>= 1;
        gArmyEffected[(target1->m_side)][target1->m_index] = true;
...
void combatManager::VaporizeCreature(CombatSide side, i32 armyIndex) {
    ...
    target_d = &m_armies[(side)][armyIndex];
    ResetLimitCreature();
...
void combatManager::RippleCreature(
    ...
    target2 = &m_armies[(side)][armyIndex];
    switch (mode) {
...
void combatManager::ShowMassSpell(
    ...
    i32 unusedMassSpellWord2 [[maybe_unused]];
    CombatSide side8;
    army* target0;
    i32 returnFrames4;
    b32 creatureDied0;
    ...
    effectFile4 = MAKEFILEID(gCombatFxNames[(effect)]);
    effectFrames4 = giNumPowFrames[(effect)] - 1;
    returnFrames4 = 0;
    ...
    for (side8 = COMBAT_ATTACKER_SIDE; (side8) < COMBAT_SIDE_COUNT; ++side8) {
        for (armyIndex = 0; armyIndex < m_armyCount[(side8)]; ++armyIndex) {
            target0 = &m_armies[(side8)][armyIndex];
            if (affected[(side8)][armyIndex] != 0)
                target0->m_drawSpellEffect = true;
            if (animateCreatures != 0 && affected[(side8)][armyIndex] != 0
                ...
                if (target0->m_quantity == 0) {
                    if (target0->m_frameInfo.animationFrameCount[(ARMY_ANIMATION_DEATH)]
                        > effectFrames4)
                        effectFrames4 =
                            target0->m_frameInfo.animationFrameCount[(ARMY_ANIMATION_DEATH)];
                    gpSoundManager->MemorySample(target0->m_samples[(ARMY_SAMPLE_KILL)]);
                } else {
                    if (target0->m_frameInfo.animationFrameCount[(ARMY_ANIMATION_WINCE)]
                        > effectFrames4)
                        effectFrames4 =
                            target0->m_frameInfo.animationFrameCount[(ARMY_ANIMATION_WINCE)];
                    if (target0->m_frameInfo.animationFrameCount[(ARMY_ANIMATION_WINCE_RETURN)]
                        > returnFrames4)
                        returnFrames4 = target0->m_frameInfo
                                            .animationFrameCount[(ARMY_ANIMATION_WINCE_RETURN)];
                    gpSoundManager->MemorySample(target0->m_samples[(ARMY_SAMPLE_WINCE)]);
                }
            }
        }
    }
    ...
    for (frame9 = 0; frame9 < effectFrames4; ++frame9) {
        for (side8 = COMBAT_ATTACKER_SIDE; (side8) < COMBAT_SIDE_COUNT; ++side8) {
            for (armyIndex = 0; armyIndex < m_armyCount[(side8)]; ++armyIndex) {
                    ...
                    if (target0->m_quantity == 0) {
                        if (target0->m_animationSequence != ARMY_ANIMATION_DEATH) {
                            ...
                            target0->m_animationFrame = 0;
                                   ...
                                   < target0->m_frameInfo
                                         .animationFrameCount[(target0->m_animationSequence)]) {
                ...
                if (frame9 + 1 < giNumPowFrames[(effect)])
    ...
    for (side8 = COMBAT_ATTACKER_SIDE; (side8) < COMBAT_SIDE_COUNT; ++side8) {
        for (armyIndex = 0; armyIndex < m_armyCount[(side8)]; ++armyIndex)
            m_armies[(side8)][armyIndex].m_drawSpellEffect = false;
    }
    ...
    for (side8 = COMBAT_ATTACKER_SIDE; (side8) < COMBAT_SIDE_COUNT; ++side8) {
        for (armyIndex = 0; armyIndex < m_armyCount[(side8)]; ++armyIndex) {
            target0 = &m_armies[(side8)][armyIndex];
            if (affected[(side8)][armyIndex] != 0 && target0->m_quantity == 0) {
...
void combatManager::CastMassSpell(SpellType spell, i32 spellPower) {
    ...
    switch (spell) {
        case SPELL_MASS_SLOW:
        case SPELL_MASS_CURSE:
            side2 = COMBAT_DEFENDER_SIDE - (m_currentSide);
            for (armyIndex = 0; armyIndex < m_armyCount[(side2)]; ++armyIndex) {
                if (m_armies[(side2)][armyIndex].SpellCastWorks(spell))
                    affected2[(side2)][armyIndex] = 1;
            }
            break;
        ...
        case SPELL_MASS_DISPEL:
            for (side2 = COMBAT_ATTACKER_SIDE; (side2) < COMBAT_SIDE_COUNT; ++side2) {
                for (armyIndex = 0; armyIndex < m_armyCount[(side2)]; ++armyIndex) {
                    if (m_armies[(side2)][armyIndex].SpellCastWorks(spell))
                        affected2[(side2)][armyIndex] = 1;
                }
        ...
        case SPELL_HOLY_SHOUT: {
            ...
            for (side2 = COMBAT_ATTACKER_SIDE; (side2) < COMBAT_SIDE_COUNT; ++side2) {
                for (armyIndex = 0; armyIndex < m_armyCount[(side2)]; ++armyIndex) {
                    if ((((m_armies[(side2)][armyIndex].m_monster.attributes) & (MONSTER_ATTRIBUTE_UNDEAD)))
                        && m_armies[(side2)][armyIndex].SpellCastWorks(spell))
                        affected2[(side2)][armyIndex] = 1;
                }
            }
            ...
            for (side2 = COMBAT_ATTACKER_SIDE; (side2) < COMBAT_SIDE_COUNT; ++side2) {
                for (armyIndex = 0; armyIndex < m_armyCount[(side2)]; ++armyIndex) {
                    if (affected2[(side2)][armyIndex] != 0)
                        m_armies[(side2)][armyIndex].Damage(damage_c, SPELL_NONE);
                }
            }
            sprintf(
                gText,
...
applySpellInfluence:
    for (side2 = COMBAT_ATTACKER_SIDE; (side2) < COMBAT_SIDE_COUNT; ++side2) {
        for (armyIndex = 0; armyIndex < m_armyCount[(side2)]; ++armyIndex) {
            ...
            target = &m_armies[(side2)][armyIndex];
            switch (spell) {
                case SPELL_MASS_CURSE:
    ...
    while (distance0 < MIRROR_SEARCH_DISTANCE_LIMIT) {
        for (sourcePart7 = 0; sourcePart7 < MIRROR_SOURCE_PART_COUNT; ++sourcePart7) {
            if (sourcePart7 == 0) {
                ...
                if ((((source2->m_monster.flags.all) & (MONSTER_FLAGS_WIDE)))) {
            ...
            for (direction = COMBAT_DIRECTION_NORTHEAST;
                ...
                if (source2->m_facing == ARMY_FACING_RIGHT)
                    ...
                    searchDirection9 = COMBAT_DIRECTION_NORTHWEST - (direction);
...
void combatManager::SummonElemental(CreatureType monsterType, i32 spellPower) {
    ...
    elementals = &m_armies[(m_hexCells[summonHex4].m_occupantSide)]
...
void combatManager::Resurrect(SpellType spell, i32 targetHex, i32 spellPower) {
    army* target;
    i32 otherHex;
    i32 unusedResurrectWord9 [[maybe_unused]];
    i32 deadHex_j;
    i32 unusedResurrectWord6 [[maybe_unused]];
    i32 index_o;
    i32 effectY;
    i32 effectX;
    i32 deadIndex;
    i32 unusedResurrectWord2 [[maybe_unused]];
    i32 oldQuantity_o;
    b32 keepSearching_d;
    i32 armyIndex;
    icon* resurrectIcon;
...
void combatManager::Earthquake(void) {
    ...
    i32 impactPositions6[EARTHQUAKE_MAX_IMPACTS][(COORDINATE_AXIS_COUNT)];
    i32 impactCount;
    i32 index7;
    ...
    i32 shakeOffsets17[EARTHQUAKE_SHAKE_FRAME_COUNT][(COORDINATE_AXIS_COUNT)] = {
        {2, 2},
        {4, 1},
        {3, -2},
        {0, -6},
    ...
    i32 unusedQuakeA10 [[maybe_unused]];
    i32 unusedQuakeB15 [[maybe_unused]];
    ...
    for (pass = 0; pass < EARTHQUAKE_SHAKE_PASS_COUNT; ++pass) {
        for (frame8 = 0; frame8 < EARTHQUAKE_SHAKE_FRAME_COUNT; ++frame8) {
            ...
            PollSound();
            width0 = COMBAT_SCREEN_WIDTH - abs(shakeOffsets17[frame8][(COORDINATE_AXIS_X)]);
            height9 = COMBAT_AREA_HEIGHT - abs(shakeOffsets17[frame8][(COORDINATE_AXIS_Y)]) - 1;
            source1 = m_backgroundBuffer->m_pixels
                         + (shakeOffsets17[frame8][(COORDINATE_AXIS_X)] < 0
                                ? 0
                                : shakeOffsets17[frame8][(COORDINATE_AXIS_X)])
                         + (shakeOffsets17[frame8][(COORDINATE_AXIS_Y)] < 0
                                ? 0
                                : shakeOffsets17[frame8][(COORDINATE_AXIS_Y)])
                               * COMBAT_SCREEN_WIDTH;
    ...
    for (index7 = 0; index7 < EARTHQUAKE_STRUCTURE_COUNT; ++index7) {
        newWallStates1[index7] = m_wallStates[index7 + (COMBAT_WALL_SLOT_SECTION_FIRST)];
        if (m_wallStates[index7 + (COMBAT_WALL_SLOT_SECTION_FIRST)]
                != COMBAT_WALL_STATE_DESTROYED
            && m_wallStates[index7 + (COMBAT_WALL_SLOT_SECTION_FIRST)]
            ...
            if (m_wallStates[index7 + (COMBAT_WALL_SLOT_SECTION_FIRST)]
                ...
                ++newWallStates1[index7];
            impactPositions6[impactCount][(COORDINATE_AXIS_X)] =
                wallPos[index7][(COORDINATE_AXIS_X)];
            impactPositions6[impactCount][(COORDINATE_AXIS_Y)] =
                wallPos[index7][(COORDINATE_AXIS_Y)] + EARTHQUAKE_CLOUD_Y_OFFSET;
            ++impactCount;
        ...
        if (m_wallStates[index7] != COMBAT_WALL_STATE_DESTROYED
            ...
            impactPositions6[impactCount][(COORDINATE_AXIS_X)] =
                towerPos[index7][(COORDINATE_AXIS_X)];
            impactPositions6[impactCount][(COORDINATE_AXIS_Y)] =
                towerPos[index7][(COORDINATE_AXIS_Y)] + EARTHQUAKE_CLOUD_Y_OFFSET;
            ++impactCount;
        }
    }
    ...
    if (m_drawbridgeState != COMBAT_CASTLE_GATE_HIDDEN
        ...
        impactPositions6[impactCount][(COORDINATE_AXIS_X)] = towerPos[0][(COORDINATE_AXIS_X)];
        impactPositions6[impactCount][(COORDINATE_AXIS_Y)] =
            towerPos[0][(COORDINATE_AXIS_Y)] + EARTHQUAKE_CLOUD_Y_OFFSET;
        ++impactCount;
        impactPositions6[impactCount][(COORDINATE_AXIS_X)] = towerPos[1][(COORDINATE_AXIS_X)];
        impactPositions6[impactCount][(COORDINATE_AXIS_Y)] =
            ...
            for (impact = 0; impact < impactCount; ++impact) {
                if (index7 >= impactDelay9[impact]
                    ...
                    IconToBitmap(
                        cloudIcon1,
                        gpWindowManager->m_screen,
                        impactPositions6[impact][(COORDINATE_AXIS_X)],
                        impactPositions6[impact][(COORDINATE_AXIS_Y)],
...
void combatManager::ShowSpellMessage(
    ...
    i32 unhandledSpell5 [[maybe_unused]];
    if (target != NULL)
        sprintf(targetName, gArmyNamesPlural[(target->m_monsterType)]);
    if (castByCreature != 0) {
        if (spell == SPELL_PARALYZE)
            sprintf(message, "\x25\x73\x20парализованы\x20циклопами\x21", targetName);
        else if (spell == SPELL_BLIND)
            sprintf(message, "\x25\x73\x20ослеплены\x20атакой\x20единорогов\x21", targetName);
        else if (spell == CREATURE_SPELL_PETRIFY)
            sprintf(message, "\x25\x73\x20обращены\x20взором\x20медузы\x20в\x20камень\x21", targetName);
        else if (spell == SPELL_CURSE)
            sprintf(message, "\x25\x73\x20прокляты\x20мумиями\x21", targetName);
        else if (spell == CREATURE_SPELL_DISPEL)
            sprintf(message, "\x25\x73\x20лишились\x20наложенных\x20на\x20них\x20чар\x0aиз\x2dза\x20архимагов\x21", targetName);
        else {
            unhandledSpell5 = 0;
            ++unhandledSpell5;
        }
        ...
        if (target != NULL) {
            if (m_heroes[(m_currentSide)]->m_isCaptain != 0)
                sprintf(
                    message,
                    "Капитан\x20направляет\x20\x0aзаклинание\x20\x27\x25\x73\x27\x20на\x20отряд\x20\x25\x73\x2e",
                    gSpellNames[(spell)],
                    targetName
                );
            else
                sprintf(
                    message,
                    "\x25\x73\x20направляет\x20\x27\x25\x73\x27\x20на\x20\x25\x73",
            ...
            if (m_heroes[(m_currentSide)]->m_isCaptain != 0)
                sprintf(message, "Капитан\x20направляет\x20заклинание\x20\x27\x25\x73\x27", gSpellNames[(spell)]);
            else
                sprintf(
                    message,
                    "\x25\x73\x20направляет\x20заклинание\x20\x27\x25\x73\x27",
                    m_heroes[(m_currentSide)]->m_name,
                    gSpellNames[(spell)]
                );
        }
