...
void combatManager::CastSpell(
    ...
    if (castByCreature == 0 && m_eagleEyeSpell[(OppositeCombatSide(m_currentSide))] == SPELL_NONE
        && m_heroes[(OppositeCombatSide(m_currentSide))] != NULL && !m_heroes[(OppositeCombatSide(m_currentSide))]->HasSpell(spell)
        && m_heroes[(OppositeCombatSide(m_currentSide))]->m_secondarySkills[(HERO_SKILL_EAGLE_EYE)]
               != HERO_SKILL_LEVEL_NONE
        && (m_heroes[(OppositeCombatSide(m_currentSide))]->m_secondarySkills[(HERO_SKILL_EAGLE_EYE)]) + 1
    ...
    switch (spell) {
        case SPELL_TELEPORT:
            ...
            if ((((teleportArmy_i->m_monster.flags.all) & (MONSTER_FLAGS_WIDE)))
                != 0) {
...
void combatManager::Fireball(i32 targetHex, SpellType spell) {
    ...
    DrawFrame(1, 0, 0, 0, COMBAT_DRAW_DELAY, 1, 1);
    target_n = m_armies[0] + (m_currentSide) * COMBAT_ARMY_STORAGE_SLOT_COUNT
               + m_currentArmyIndex;
    ...
    for (frame_i = (COMBAT_DIRECTION_NORTHEAST);
    ...
    if (!gbNoShowCombat) {
        ...
        for (pass = 0; pass < SPELL_ARMAGEDDON_BRIGHTEN_PASS_COUNT; ++pass) {
            ...
            for (color = 0; color < SPELL_ARMAGEDDON_PALETTE_COLOR_COUNT; ++color) {
                if (effectData[color * (PALETTE_CHANNEL_COUNT)
                               + (PALETTE_CHANNEL_RED)]
                    < SPELL_ARMAGEDDON_CHANNEL_MAX)
                    ++effectData[color * (PALETTE_CHANNEL_COUNT)
                                 + (PALETTE_CHANNEL_RED)];
                if (effectData[color * (PALETTE_CHANNEL_COUNT)
                ...
                if (pass == 0) {
                    ...
                    for (color = 0; color < SPELL_ARMAGEDDON_PALETTE_COLOR_COUNT; ++color) {
                        if (effectData[color * (PALETTE_CHANNEL_COUNT)
                            ...
                            effectData[color * (PALETTE_CHANNEL_COUNT)
                                       + (PALETTE_CHANNEL_GREEN)] -= SPELL_ARMAGEDDON_CHANNEL_STEP;
                        if (effectData[color * (PALETTE_CHANNEL_COUNT)
                                       + (PALETTE_CHANNEL_BLUE)]
                            > SPELL_ARMAGEDDON_CHANNEL_STEP)
                            effectData[color * (PALETTE_CHANNEL_COUNT)
                                       + (PALETTE_CHANNEL_BLUE)] -= SPELL_ARMAGEDDON_CHANNEL_STEP;
                    }
                    SetPalette(effectData, 1);
                }

                i32 offsetX = static_cast<i32>(
                    shakeOffsets[frame][(COORDINATE_AXIS_X)] * scale
                );
                i32 offsetY = static_cast<i32>(
                    shakeOffsets[frame][(COORDINATE_AXIS_Y)] * scale
                );
...
void combatManager::ChainLightning(i32 targetHex, i32 spellPower) {
    ...
    for (strike_i = 0; strike_i < CHAIN_LIGHTNING_MAX_TARGETS; ++strike_i) {
        ...
        damage_l >>= 1;
        gArmyEffected[(target_j->m_side)][target_j->m_index] = 1;
...
void combatManager::ShowMassSpell(
    ...
    effectFile3 = MAKEFILEID(gCombatFxNames[(effect)]);
    ...
    for (side8 = COMBAT_ATTACKER_SIDE; (side8) < COMBAT_SIDE_COUNT; ++side8) {
        for (armyIndex0 = 0; armyIndex0 < m_armyCount[(side8)]; ++armyIndex0) {
            target0 = &m_armies[(side8)][armyIndex0];
            if (affected[(side8)][armyIndex0] != 0)
                target0->m_drawSpellEffect = 1;
            if (animateCreatures != 0 && affected[(side8)][armyIndex0] != 0
    ...
    for (side8 = COMBAT_ATTACKER_SIDE; (side8) < COMBAT_SIDE_COUNT; ++side8) {
        for (armyIndex0 = 0; armyIndex0 < m_armyCount[(side8)]; ++armyIndex0)
            m_armies[(side8)][armyIndex0].m_drawSpellEffect = 0;
    }
    ...
    for (side8 = COMBAT_ATTACKER_SIDE; (side8) < COMBAT_SIDE_COUNT; ++side8) {
        for (armyIndex0 = 0; armyIndex0 < m_armyCount[(side8)]; ++armyIndex0) {
            target0 = &m_armies[(side8)][armyIndex0];
            if (affected[(side8)][armyIndex0] != 0 && target0->m_quantity == 0) {
...
void combatManager::CastMassSpell(SpellType spell, i32 spellPower) {
    army* target_i = NULL;
    CombatEffectType effect = gsSpellInfo[(spell)].combatEffect;
    ...
    switch (spell) {
        case SPELL_MASS_SLOW:
        case SPELL_MASS_CURSE:
            side_i = COMBAT_DEFENDER_SIDE - (m_currentSide);
        ...
        case SPELL_MASS_DISPEL:
            for (side_i = COMBAT_ATTACKER_SIDE; (side_i) < COMBAT_SIDE_COUNT; ++side_i) {
                for (armyIndex_k = 0; armyIndex_k < m_armyCount[(side_i)]; ++armyIndex_k) {
                    if (m_armies[(side_i)][armyIndex_k].SpellCastWorks(spell))
                        affected[(side_i)][armyIndex_k] = 1;
                }
        ...
        case SPELL_HOLY_SHOUT: {
            ...
            for (side_i = COMBAT_ATTACKER_SIDE; (side_i) < COMBAT_SIDE_COUNT; ++side_i) {
                for (armyIndex_k = 0; armyIndex_k < m_armyCount[(side_i)]; ++armyIndex_k) {
                    if ((((m_armies[(side_i)][armyIndex_k].m_monster.attributes) & (MONSTER_ATTRIBUTE_UNDEAD)))
            ...
            for (side_i = COMBAT_ATTACKER_SIDE; (side_i) < COMBAT_SIDE_COUNT; ++side_i) {
                for (armyIndex_k = 0; armyIndex_k < m_armyCount[(side_i)]; ++armyIndex_k) {
                    if (affected[(side_i)][armyIndex_k] != 0)
                        m_armies[(side_i)][armyIndex_k].Damage(damage_c, SPELL_NONE);
                }
            }
            sprintf(
                gText,
    ...
    while (distance6 < MIRROR_SEARCH_DISTANCE_LIMIT) {
        for (sourcePart9 = 0; sourcePart9 < MIRROR_SOURCE_PART_COUNT; ++sourcePart9) {
            if (sourcePart9 == 0) {
                ...
                if ((((source6->m_monster.flags.all) & (MONSTER_FLAGS_WIDE)))) {
            ...
            for (direction6 = COMBAT_DIRECTION_NORTHEAST;
                 (direction6) < SPELL_ADJACENT_DIRECTION_COUNT;
                 ++direction6) {
                if (source6->m_facing == ARMY_FACING_RIGHT)
                    searchDirection11 = direction6;
                else
                    searchDirection11 = COMBAT_DIRECTION_NORTHWEST - (direction6);
...
mirror_found:
    ...
    image3 =
        &m_armies[(m_hexCells[candidateHex1].m_occupantSide)][m_hexCells[candidateHex1].m_occupantIndex];
    image3->m_monster.flags.abilityFlags |= MONSTER_ABILITY_FLAG_SUMMONED;
    duration9 = m_spellPower[(m_currentSide)];
    if (m_heroes[(m_currentSide)]->HasArtifact(ARTIFACT_ENCHANTED_HOURGLASS))
    ...
    ResetLimitCreature();
    m_limitCreatureCount[(m_hexCells[candidateHex1].m_occupantSide)]
...
void combatManager::SummonElemental(
    ...
    for (offset = 0; offset < SUMMON_HEXES_PER_SIDE; ++offset) {
        if (m_hexCells[*(summonHexes_l + (m_currentSide) * SUMMON_HEXES_PER_SIDE
                         + (randomOffset_a + offset) % SUMMON_HEXES_PER_SIDE)]
                .m_occupantSide
            == COMBAT_SIDE_NONE)
            summonHex =
                *(summonHexes_l + (m_currentSide) * SUMMON_HEXES_PER_SIDE
                  + (randomOffset_a + offset) % SUMMON_HEXES_PER_SIDE);
    }
...
void combatManager::DoLuck(CombatSide side, i32 armyIndex) {
    ...
    targetY_l -= GetIconEntry(
                     ...
                     target_i->m_frameInfo
                         .animationFrames[(ARMY_ANIMATION_STAND)][LUCK_TARGET_FRAME_INDEX]
                 )
                     ->h
                 + LUCK_ICON_Y_PADDING;
                ...
                if (index_o >= RESURRECT_DEATH_REVERSE_FRAME) {
                    ...
                    target_i->m_animationFrame =
                        RESURRECT_DEATH_REVERSE_FRAME - 1 - index_o
                            ...
                            ? RESURRECT_DEATH_REVERSE_FRAME - 1 - index_o
                            : target_i->m_frameInfo.animationFrameCount[(ARMY_ANIMATION_DEATH)]
                                  - 1;
                }
            }
...
void combatManager::Earthquake(void) {
    ...
    i32 impactPositions2[EARTHQUAKE_MAX_IMPACTS][(COORDINATE_AXIS_COUNT)];
    i32 impactCount4;
    i32 structure7;
    ...
    for (pass = 0; pass < EARTHQUAKE_SHAKE_PASS_COUNT; ++pass) {
        for (frame5 = 0; frame5 < EARTHQUAKE_SHAKE_FRAME_COUNT; ++frame5) {
            ...
            width0 =
                COMBAT_SCREEN_WIDTH
                - abs(shakeOffsets[frame5][(COORDINATE_AXIS_X)]);
            height8 =
                COMBAT_AREA_HEIGHT
                - abs(shakeOffsets[frame5][(COORDINATE_AXIS_Y)])
                - 1;
            source9 =
                m_backgroundBuffer->m_pixels
                + (shakeOffsets[frame5][(COORDINATE_AXIS_Y)] > 0
                       ? shakeOffsets[frame5][(COORDINATE_AXIS_Y)]
                       : 0)
                      * COMBAT_SCREEN_WIDTH
                + (shakeOffsets[frame5][(COORDINATE_AXIS_X)] > 0
                       ? shakeOffsets[frame5][(COORDINATE_AXIS_X)]
                       : 0);
            destinationX37 =
                shakeOffsets[frame5][(COORDINATE_AXIS_X)] <= 0
                    ? -shakeOffsets[frame5][(COORDINATE_AXIS_X)]
    ...
    for (structure7 = 0; structure7 < EARTHQUAKE_STRUCTURE_COUNT; ++structure7) {
        newWallStates9[structure7] = m_wallStates[structure7 + (COMBAT_WALL_SLOT_SECTION_FIRST)];
        if (m_wallStates[structure7 + (COMBAT_WALL_SLOT_SECTION_FIRST)]
                != COMBAT_WALL_STATE_DESTROYED
            && m_wallStates[structure7 + (COMBAT_WALL_SLOT_SECTION_FIRST)]
            ...
            if (m_wallStates[structure7 + (COMBAT_WALL_SLOT_SECTION_FIRST)]
                ...
                ++newWallStates9[structure7];
            impactPositions2[impactCount4][(COORDINATE_AXIS_X)] =
                wallPos[structure7][(COORDINATE_AXIS_X)];
            impactPositions2[impactCount4][(COORDINATE_AXIS_Y)] =
                wallPos[structure7][(COORDINATE_AXIS_Y)] + EARTHQUAKE_CLOUD_Y_OFFSET;
            ++impactCount4;
        ...
        if (m_wallStates[structure7] != COMBAT_WALL_STATE_DESTROYED
            ...
            impactPositions2[impactCount4][(COORDINATE_AXIS_X)] =
                towerPos[structure7][(COORDINATE_AXIS_X)];
            impactPositions2[impactCount4][(COORDINATE_AXIS_Y)] =
                towerPos[structure7][(COORDINATE_AXIS_Y)] + EARTHQUAKE_CLOUD_Y_OFFSET;
            ++impactCount4;
        }
    }
    ...
    if (m_drawbridgeState != COMBAT_CASTLE_GATE_HIDDEN
        ...
        impactPositions2[impactCount4][(COORDINATE_AXIS_X)] =
            towerPos[0][(COORDINATE_AXIS_X)];
        impactPositions2[impactCount4][(COORDINATE_AXIS_Y)] =
            towerPos[0][(COORDINATE_AXIS_Y)] + EARTHQUAKE_CLOUD_Y_OFFSET;
        ++impactCount4;
        impactPositions2[impactCount4][(COORDINATE_AXIS_X)] =
            towerPos[1][(COORDINATE_AXIS_X)];
        impactPositions2[impactCount4][(COORDINATE_AXIS_Y)] =
            ...
            for (impact = 0; impact < impactCount4; ++impact) {
                if (impactDelay9[impact] <= structure7
                    ...
                    IconToBitmap(
                        cloudIcon,
                        gpWindowManager->m_screen,
                        impactPositions2[impact][(COORDINATE_AXIS_X)],
                        impactPositions2[impact][(COORDINATE_AXIS_Y)],
