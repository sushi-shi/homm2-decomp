    ...
    switch (eventType_g) {
        ...
        case MAP_OBJECT_XANADU:
            if (!(eventHero2->m_xanaduVisits & (1 << cell->m_objectMetadata))) {
                if (eventHero2->m_level + eventHero2->m_secondarySkills[HERO_SKILL_DIPLOMACY] * 2
...
void advManager::EraseObj(class mapCell* cell, i32 x, i32 y) {
    ...
    for (i_e = 0; i_e < ERASE_CELL_COUNT; i_e++) {
        ...
        currentCell_k->m_flags |= MAP_CELL_OBJECT_SHADOW_ONLY;
    cellDone:
        changed_i = 0;
    }
    ...
    switch (siteType2) {
        case GENERIC_SITE_ALCHEMIST_TOWER:
            ...
            if (cursedArtifactCount9 != 0) {
                ...
                NormalDialog(gText, 2, -1, -1, -1, 0, -1, 0, -1, 0);
                if (gpWindowManager->m_dialogResult == MONSTER_DIALOG_YES) {
                    if (gpCurPlayer->m_resources[RES_GOLD] >= GENERIC_SITE_ALCHEMIST_COST) {
                        for (index3 = 0; index3 < GENERIC_SITE_ARTIFACT_SLOT_COUNT; index3++) {
                            if (IsCursedItem(eventHero->m_artifacts[index3]))
                                eventHero->m_artifacts[index3] = ARTIFACT_NONE;
                        }
                        gpCurPlayer->m_resources[RES_GOLD] -= GENERIC_SITE_ALCHEMIST_COST;
                    } else {
                        NormalDialog(
...
void GiveTakeArtifactStat(hero* targetHero, i32 artifact, i32 take) {
    ...
    switch (artifact) {
        case ARTIFACT_ULTIMATE_BOOK:
            statChanges[HERO_PRIMARY_KNOWLEDGE] = 12;
            break;
        case ARTIFACT_ULTIMATE_SWORD:
            statChanges[HERO_PRIMARY_ATTACK] = 12;
            break;
        case ARTIFACT_ULTIMATE_CLOAK:
            statChanges[HERO_PRIMARY_DEFENSE] = 12;
            break;
        case ARTIFACT_ULTIMATE_WAND:
            statChanges[HERO_PRIMARY_SPELL_POWER] = 12;
            break;
        ...
        case ARTIFACT_WITCHES_BROACH:
            statChanges[HERO_PRIMARY_SPELL_POWER] = 3;
            break;
    ...
    switch (eventType_g) {
        ...
        case MAP_OBJECT_XANADU:
            if ((eventHero->m_xanaduVisits & (1U << cell->m_objectMetadata)) == 0
                && eventHero->m_level + eventHero->m_secondarySkills[HERO_SKILL_DIPLOMACY] * 2
                       >= XANADU_ADMISSION_LEVEL) {
            ...
            if ((eventHero->m_treeKnowledgeVisits & (1U << (cell->m_objectMetadata & 31))) == 0) {
                ...
                switch (cell->m_objectMetadata >> TREE_KNOWLEDGE_MODE_SHIFT) {
                    case TREE_KNOWLEDGE_FREE:
                        ...
                        break;
                    case TREE_KNOWLEDGE_GOLD:
                        if (gpCurPlayer->m_resources[RES_GOLD] > AI_EVENT_TREE_GOLD) {
                        ...
                        break;
                    case TREE_KNOWLEDGE_GEMS:
                        if (gpCurPlayer->m_resources[RES_GEMS] > AI_EVENT_TREE_GEMS) {
        ...
        case MAP_OBJECT_PYRAMID:
            if (cell->m_objectMetadata != 0
                ...
                if (battleWon_j == 0) {
                    ...
                    combatResult_d =
                        gpPhilAI
                            ->CombatMonsterEvent(eventHero, CREATURE_ROYAL_MUMMY, &index_h, cell);
                    if (combatResult_d == 0) {
                    } else {
                        eventHero->AddSpell(
                            cell->m_objectMetadata - 1,
    ...
    switch (siteType3) {
        case AI_GENERIC_SITE_CURSED_ARTIFACTS:
            ...
            if (cursedArtifactCount5 != 0
                && gpCurPlayer->m_resources[RES_GOLD] >= AI_EVENT_CURSED_ARTIFACT_GOLD_THRESHOLD) {
...
void advManager::RecruitSiteAIEvent(mapCell* cell, hero* eventHero) {
    ...
    if (availableCount1 != 0) {
        ...
        if (purchaseCount17 > 0) {
            gpGame
                ->GiveArmy(&eventHero->m_army, creatureType13, purchaseCount17, replacementSlot26);
    ...
    if (eventHero->m_army.CanJoin(monsterType) && !eventHero->HasArtifact(ARTIFACT_HIDEOUS_MASK)
        ...
        if (forcedJoin) {
            ...
            if (eventHero->m_secondarySkills[HERO_SKILL_DIPLOMACY]) {
                ...
                if (gpGame->m_players[eventHero->m_owner].m_resources[RES_GOLD] < joiningCost) {
        ...
        if (eventHero->GetSSLevel(HERO_SKILL_NECROMANCY)
            ...
            gpGame->GiveArmy(
                ...
                static_cast<i32>(
                    ...
                    * eventHero->GetSSLevel(HERO_SKILL_NECROMANCY) * MONSTER_NECROMANCY_FRACTION
                ),
                -1
            );
        }
...
void advManager::SendHeroTownData(
    ...
    if (firstHero) {
        buffer->firstGold = gpGame->m_players[firstHero->m_owner].m_resources[RES_GOLD];
    } else {
        buffer->firstGold = 0;
    }
    if (secondHero) {
    ...
    if (secondHero) {
        buffer->secondGold = gpGame->m_players[secondHero->m_owner].m_resources[RES_GOLD];
    } else {
        buffer->secondGold = 0;
    }
    ...
    *secondArmy = static_cast<armyGroup*>(BaseAlloc(sizeof(armyGroup), RETAIL_FILE, 6246));
    ...
    while (!gotFirstHeroFirst3 || !gotFirstHeroSecond9 || !gotSecondHeroFirst13
        ...
        if (packet && EVENTS_REMOTE_MESSAGE(packet)->type == REMOTE_MESSAGE_RELIABLE
            ...
            lastPacketTime36 = KBTickCount();
            if (EVENTS_REMOTE_HERO(packet)->fragment == COMBAT_REMOTE_FIRST_HERO_FIRST) {
                memcpy(*firstHero, EVENTS_REMOTE_HERO(packet)->data, COMBAT_REMOTE_HERO_FIRST_SIZE);
                gotFirstHeroFirst3 = 1;
            }
            if (EVENTS_REMOTE_HERO(packet)->fragment == COMBAT_REMOTE_FIRST_HERO_SECOND) {
                memcpy(
                    &(*firstHero)->m_spells[COMBAT_REMOTE_HERO_SECOND_SPELL_INDEX],
            ...
            if (EVENTS_REMOTE_HERO(packet)->fragment == COMBAT_REMOTE_SECOND_HERO_FIRST) {
                memcpy(
                    *secondHero,
                    EVENTS_REMOTE_HERO(packet)->data,
            ...
            if (EVENTS_REMOTE_HERO(packet)->fragment == COMBAT_REMOTE_SECOND_HERO_SECOND) {
                memcpy(
                    &(*secondHero)->m_spells[COMBAT_REMOTE_HERO_SECOND_SPELL_INDEX],
                    EVENTS_REMOTE_HERO(packet)->data,
                    COMBAT_REMOTE_HERO_SECOND_SIZE
