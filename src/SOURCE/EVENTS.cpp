        ...
        shrineSpell:
            if (!eventHero2->HasArtifact(ARTIFACT_MAGIC_BOOK)) {
                ...
                EventWindow(-1, NORMAL_DIALOG_INFO, gText, -1, 0, -1, 0, -1);
                           ...
                           eventHero2->m_secondarySkills[(HERO_SKILL_WISDOM)]
                       )
                           + SHRINE_WISDOM_BONUS
    ...
    switch (siteType2) {
        case GENERIC_SITE_ALCHEMIST_TOWER:
            ...
            if (cursedArtifactCount9 != 0) {
                EventSound(
                    cell->m_triggerType & MAP_TRIGGER_TYPE_MASK, (siteType2), &eventSample5
                );
    ...
    switch (eventType_g) {
        ...
        case MAP_OBJECT_PYRAMID:
            if (cell->m_objectMetadata != 0
                ...
                if ((((gsSpellInfo[index_h].attributes) & (SPELL_INFO_ATTRIBUTE_POWER)))) {
                    battleStatValue_o =
        ...
        if (purchaseCount17 > 0) {
            ...
            for (resource = 0; resource < RES_COUNT; resource++) {
                gpCurPlayer->m_resources[(resource)] -=
                    cost16[(resource)] * purchaseCount17;
            }
        }
    }
}
