...
typedef i32 AIArtifactEventMode;
...
typedef enum AIFightValueConstant {
    FIGHT_VALUE_SPELL_COUNT      = (SPELL_SUMMON_FIRE_ELEMENTAL) + 1,
    ...
    for (node = 0; node < GAME_HERO_COUNT; node++) {
        ...
        if (resetAll == 0
            || abs(y - gpGame->m_heroRecs[node].m_x) + abs(x - gpGame->m_heroRecs[node].m_x)
                   < NEARBY_RADIUS)
            gaiHeroLiveChance[node] = (RV_UNSET);
    }
}
        ...
        for (armyNo = 0; armyNo < CREATURE_UPGRADE_ARMY_COUNT; armyNo++) {
            ...
            for (slot = 0; slot < ARMY_GROUP_SLOT_COUNT; slot++) {
                ...
                for (dwelling = BUILDING_SLOT_DWELLING_SECOND;
                     ...
                     dwelling++) {
                    if (gDwellingType[(townRef->m_type)]
                ...
                if ((army->m_creatureTypes[slot] == CREATURE_GREEN_DRAGON
                    ...
                    && (townRef->m_buildings & (KB_DWELLING_UPGRADE_SIXTH_FLAG))) {
                    upgradable = 1;
                    newType = CREATURE_BLACK_DRAGON;
                }
                if (upgradable) {
                    goldAmount = (gMonsterDatabase[(newType)].cost
                                - gMonsterDatabase[(army->m_creatureTypes[slot])].cost)
                    ...
                    if (goldAmount <= gpCurPlayer->m_resources[(RES_GOLD)]
                        && (material == RES_NONE
                            || resourceAmount <= gpCurPlayer->m_resources[(material)])) {
                        gpCurPlayer->m_resources[(RES_GOLD)] -= goldAmount;
                        if (material != RES_NONE)
                            gpCurPlayer->m_resources[(material)] -= resourceAmount;
...
void philAI::CheckBuyStuff(void) {
    ...
    if (gpCurPlayer->m_resources[(RES_GOLD)] < PURCHASE_MINIMUM_GOLD
        || (gpCurPlayer->m_resources[(RES_GOLD)] < AI_HERO_PURCHASE_GOLD_FLOOR
    ...
    if (giBuildShipyard[giCurPlayer] >= 0) {
        if (CanBuy(idx, BUILDING_SLOT_DOCK) && CanBuild(idx, BUILDING_SLOT_DOCK)) {
            ...
            gpCurPlayer->m_resources[(RES_GOLD)] -= SHIPYARD_GOLD_COST;
            gpCurPlayer->m_resources[(RES_WOOD)] -= SHIPYARD_WOOD_COST;
        }
    }
    if (giBuildBoat[giCurPlayer] >= 0) {
        if ((idx->m_buildings & AI_BUILDING_SHIPYARD_MASK)
            && gpCurPlayer->m_resources[(RES_GOLD)] >= TOWN_BOAT_GOLD_COST
            && gpCurPlayer->m_resources[(RES_WOOD)] >= TOWN_BOAT_WOOD_COST) {
...
i32 philAI::GoodAdjacent(MapDirection* direction) {
    ...
    if ((gpAdvManager->GetCell(gpCurAIHero->m_x, gpCurAIHero->m_y)->m_triggerType
        ...
        return 0;
    for (dirIdx = MAP_DIRECTION_NORTH; (dirIdx) < NORMAL_DIRECTION_COUNT;
         dirIdx++) {
        if (gpAdvManager->ValidMoveWithEvent(gpCurAIHero, dirIdx)) {
            kn = normalDirTable[(dirIdx)].x + gpCurAIHero->m_x;
            nb = normalDirTable[(dirIdx)].y + gpCurAIHero->m_y;
            if ((((gpAdvManager->GetCell(kn, nb)->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG)))
                && !(mapExtra[kn + MAP_WIDTH * nb] & (MAP_EXTRA_ADJACENT_MONSTER))
...
void philAI::CheckReload(void) {
    ...
    gpSearchArray->SeedPosition(
        ...
        (gpCurAIHero->m_secondarySkills[(HERO_SKILL_PATHFINDING)]),
        -1,
        -1,
        0,
        0
                                ...
                                if (positionValue > bestValue) {
                                    for (adjacentIndex = 0;
                                        ...
                                        if (giGroundToTerrain[cell->m_terrainImageIndex]
                                            == TERRAIN_WATER)
                                            continue;
                                        if (cell->m_flags & (MAP_CELL_OCCUPIED))
                                            continue;
                                        if ((((cell->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG))))
                                            continue;
...
void philAI::DoAI(i32 player) {
    ...
    for (pathIndex3[0] = 0; pathIndex3[0] < (RES_COUNT); pathIndex3[0]++) {
        sprintf(
            gText,
            "RES - %15s  %d  %d",
    ...
    while ((currentHero15 = DetermineHeroToMove(player)) != NULL) {
        ...
        stepLimit36 =
            static_cast<i32>(stepLimit36 * (1.7 - (gpGame->m_difficulty) * 0.1));
        minimumValue9 = static_cast<i32>(
            minimumValue9 * (((gpGame->m_difficulty) - 1) * 0.06 + 0.8)
        );
...
firstWeekDone:
    ...
    gpCurPlayer->m_aiData.m_upgradeValueWeight =
        static_cast<float>(
            gpCurPlayer->m_aiData.m_income[(RES_GOLD)]
            + gpCurPlayer->m_resources[(RES_GOLD)]
        ) / (lastFightValue8 + 1000)
        + gpCurPlayer->m_aiData.m_attentionWeights.upgradeBase;

    artifactTotal8 = 0;
    for (generalIndex4 = (ARTIFACT_ULTIMATE_SHIELD);
         generalIndex4 < (ARTIFACT_GAMBLERS_COIN);
         generalIndex4++)
    ...
    for (playerIndex0 = 0; playerIndex0 < gpGame->m_playerCount; playerIndex0++) {
        ...
        for (innerIndex27 = 0; innerIndex27 < gpGame->m_players[playerIndex0].m_heroCount;
            ...
            gpSearchArray->SeedPosition(
                ...
                1,
                threatHeroPtr6->m_remainingMobility + 100,
                (threatHeroPtr6->m_secondarySkills[(HERO_SKILL_PATHFINDING)]),
                -1,
                -1,
                0,
                0
...
void philAI::GetBestBHC(i32 player, BHC& best) {
    i32 townStrength6[(GAME_TOWN_COUNT)];
    i32 desiredStrength5[(GAME_TOWN_COUNT)];
    BHC candidate10;
    float value19 = 1.0f;
    ...
    for (i36 = 0; i36 < gpCurPlayer->m_townCount; i36++) {
        ...
        if (townPtr7->m_buildings & (TOWN_BUILDING_CASTLE))
            totalWeight1 += 10;
        else
            totalWeight1 += 7;
    }
    ...
    for (i36 = 0; i36 < gpCurPlayer->m_townCount; i36++) {
        townPtr7 = &gpGame->m_castleRecs[gpCurPlayer->m_townIds[i36]];
        desiredStrength5[i36] = ((townPtr7->m_buildings & (TOWN_BUILDING_CASTLE)) ? 10 : 7)
                                     * averageStrength19
                                 + 400;
    }
    ...
    for (i36 = 0; i36 < gpCurPlayer->m_townCount; i36++) {
            ...
            continue;
        {
            ...
            if (gpCurPlayer->m_heroCount < giMaxHeroesForThisPlayer
                && (townPtr7->m_buildings & (TOWN_BUILDING_CASTLE))) {
...
i32 philAI::DetermineTargetPosition(
    ...
    if (gpCurAIHero->m_secondarySkills[(HERO_SKILL_PATHFINDING)]
...
void philAI::ValueOfBuyingBuilding(
    ...
    switch (building) {
        ...
        case BUILDING_SLOT_SPECIAL_THIRTY:
            ...
            if (static_cast<i32>(building) - (BUILDING_SLOT_DWELLING_FIRST)
                < highestDwellingId)
...
void philAI::ValueOfBuyingCreature(
    ...
    if (townPtr->m_occupyingHeroId != AI_TROOP_EMPTY_SLOT) {
        ...
        if ((((gMonsterDatabase[(creature)].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
    ...
    if ((((gMonsterDatabase[(creature)].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
        for (garrisonSlot = 0; garrisonSlot < CREATURE_PURCHASE_ARMY_SLOT_COUNT;
            ...
            if (townPtr->m_army.m_creatureTypes[garrisonSlot] != CREATURE_NONE
                && (((gMonsterDatabase[(townPtr->m_army.m_creatureTypes[garrisonSlot])].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
                missileStacks++;
            }
        }
        creatureValue = static_cast<i32>(
...
void philAI::GetBestCreature(town* townPtr, BHC& best, float& bestValue) {
    ...
    for (dwelling = 0; dwelling < CREATURE_PURCHASE_DWELLING_COUNT; dwelling++) {
        candidateMonster = gDwellingType[(townPtr->m_type)][dwelling];
        ...
        if ((townPtr->m_buildings & (1 << (dwelling + (BUILDING_SLOT_DWELLING_FIRST))))
            ...
            if (gMonsterDatabase[(candidateMonster)].randomValue > weakestArmyValue
                && gMonsterDatabase[(candidateMonster)].randomValue
...
i32 philAI::CreaturesToBuy(town* t, i32 level) {
    ...
    return CreaturesToBuy(gDwellingType[(t->m_type)][level], nGarrison);
}
...
void philAI::ValueOfBuyingHero(
    ...
    costs2[(RES_WOOD)] = 0;
    costs2[(RES_MERCURY)] = 0;
    costs2[(RES_ORE)] = 0;
    costs2[(RES_SULFUR)] = 0;
    costs2[(RES_CRYSTAL)] = 0;
    costs2[(RES_GEMS)] = 0;
    costs2[(RES_GOLD)] = AI_HERO_PURCHASE_GOLD_COST;
...
i32 philAI::RVOfPosition(
    ...
    if ((((triggerType3) & (MAP_TRIGGER_ACTION_FLAG)))
        || (gpCurPlayer->m_ultimateArtifactHintX == x
            && gpCurPlayer->m_ultimateArtifactHintY == y)) {
    ...
    if ((((gpCurAIHero->m_eventFlags) & (HERO_EVENT_EMBARKED))) && triggerType3 == MAP_OBJECT_COAST) {
    ...
    if (giDebugLevel > POSITION_DEBUG_LEVEL - 1) {
        ...
        char* objectName5;
        if (debugObjectType > MAP_OBJECT_NONE && (debugObjectType) < POSITION_OBJECT_NAME_COUNT)
            objectName5 = gQuickViewText[(debugObjectType)];
        else
            objectName5 = " ";
...
i32 philAI::FightValueOfStack(
    ...
    if (useEnemyMods) {
        if (heroPtr->HasArtifact(ARTIFACT_BALLISTA) || heroPtr->HasSpell(SPELL_EARTHQUAKE)
            || heroPtr->m_secondarySkills[(HERO_SKILL_BALLISTICS)]
                   != HERO_SKILL_LEVEL_NONE) {
            enemyMeleeModifierIndex = 1.05f;
            enemyFlyingModifier36 = 0.95f;
        }
        if (heroPtr->m_secondarySkills[(HERO_SKILL_ARCHERY)] != HERO_SKILL_LEVEL_NONE
    ...
    for (armySlotRecord = 0; armySlotRecord < AI_TOWN_ARMY_SLOTS; armySlotRecord++) {
        if (group->m_creatureTypes[armySlotRecord] != CREATURE_NONE) {
            stackValueMap =
                gMonsterDatabase[(group->m_creatureTypes[armySlotRecord])].fightValue
            ...
            if (useHero) {
                ...
                if ((((gMonsterDatabase[(group->m_creatureTypes[armySlotRecord])].attributes) & (MONSTER_ATTRIBUTE_RANGED)))
                ...
                if (useTown) {
                    ...
                    if ((((gMonsterDatabase[(group->m_creatureTypes[armySlotRecord])].attributes) & (MONSTER_ATTRIBUTE_FLYING))))
                        stackValueMap = static_cast<i32>(stackValueMap * 0.95);
                }
                if ((((gMonsterDatabase[(group->m_creatureTypes[armySlotRecord])].attributes) & (MONSTER_ATTRIBUTE_RANGED)))
                    && heroPtr
                    && heroPtr->m_secondarySkills[(HERO_SKILL_ARCHERY)]
                           != HERO_SKILL_LEVEL_NONE) {
        ...
        if (heroPtr->m_spellPoints >= 3) {
            ...
            for (armySlotRecord = 0; armySlotRecord < FIGHT_VALUE_SPELL_COUNT; armySlotRecord++) {
                if (heroPtr->HasSpell(SpellType(armySlotRecord))) {
                    ...
                    if ((((gsSpellInfo[armySlotRecord].attributes) & (SPELL_INFO_ATTRIBUTE_DURATION))))
                        spellScoreTotal = static_cast<i32>(spellScoreTotal * durationModifier7);
                    else if ((((gsSpellInfo[armySlotRecord].attributes) & (SPELL_INFO_ATTRIBUTE_POWER))))
...
void philAI::EvaluateOneTimeCreaturePurchase(
    ...
    if (purchaseCount == 0)
        return;
    {
        purchaseFightValue15 = gMonsterDatabase[(creature)].fightValue * purchaseCount;
...
i32 philAI::QuickCombat(
    ...
    if (defeatedHero5 != NULL
        && defeatedHero5->m_secondarySkills[(HERO_SKILL_EAGLE_EYE)]
               != HERO_SKILL_LEVEL_NONE
        && victoriousHero4 != NULL) {
        for (armyIndex0 = 0; armyIndex0 < (SPELL_COUNT); armyIndex0++) {
            if (defeatedHero5->HasSpell(SpellType(armyIndex0)) != 0
                ...
                && (((gsSpellInfo[armyIndex0].attributes) & (SPELL_INFO_ATTRIBUTE_COMBAT)))) {
    ...
    *value = 0;
        ...
        return;
    {
        ...
        for (heroIndex9 = 0; heroIndex9 < HERO_INTERACTION_HERO_COUNT; heroIndex9++) {
            ...
            for (statIndex8 = (HERO_SKILL_PATHFINDING); statIndex8 < (HERO_SKILL_COUNT);
                 statIndex8++) {
    ...
    *value = 0;
    ...
    if ((townPtr->m_buildings & AI_BUILDING_MAGE_GUILD_MASK)
        ...
        *value += ManaRefreshValue(heroPtr, 1);
        for (spellLevel14 = 1;
             spellLevel14
             <= (heroPtr->m_secondarySkills[(HERO_SKILL_WISDOM)])
            ...
            for (spellIndex = 0; spellIndex < townPtr->m_spellCounts[spellLevel14];
                ...
                if (!heroPtr->HasSpell(townPtr->m_spells[spellLevel14 - 1][spellIndex])) {
                    if ((((gsSpellInfo[(townPtr->m_spells[spellLevel14 - 1][spellIndex])]
            ...
            for (sourceIndex15 = 0; sourceIndex15 < AI_TOWN_ARMY_SLOTS; sourceIndex15++) {
                if (sourceArmy->m_creatureTypes[sourceIndex15] != CREATURE_NONE) {
                    ...
                    if ((preferFast != 0
                        ...
                        selectedIndex5 = sourceIndex15;
                    } else if (gMonsterDatabase[(sourceArmy->m_creatureTypes[sourceIndex15])].speed
                                   == selectedSpeed19
...
i32 philAI::ChooseGoldOrExperience(i32, i32) {
    ...
    return gpCurPlayer->m_resources[(RES_GOLD)] > 4000 ? 1 : 0;
}
    ...
    switch (eventType16) {
            ...
            switch (cell->m_objectMetadata) {
                case FIGHT_EVENT_LEVEL_1:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[(RES_GOLD)] * (FIGHT_EVENT_REWARD_500)
                    );
                    break;
                case FIGHT_EVENT_LEVEL_2:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[(RES_GOLD)] * (FIGHT_EVENT_REWARD_1000)
                    );
                    break;
                case FIGHT_EVENT_LEVEL_3:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[(RES_GOLD)]
                        * (FIGHT_EVENT_REWARD_3000)
                    );
                    break;
        ...
        case MAP_OBJECT_SHIPWRECK:
            switch (cell->m_objectMetadata) {
                ...
                case FIGHT_EVENT_LEVEL_2:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[(RES_GOLD)] * (FIGHT_EVENT_REWARD_2000)
                    );
                    break;
                case FIGHT_EVENT_LEVEL_3:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[(RES_GOLD)]
                        * (FIGHT_EVENT_REWARD_5000)
                    );
                    break;
                case FIGHT_EVENT_LEVEL_4:
                    rewardValue7 = static_cast<i32>(
        ...
        case MAP_OBJECT_DERELICT_SHIP:
            switch (cell->m_objectMetadata) {
                ...
                case FIGHT_EVENT_LEVEL_4:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[(RES_GOLD)] * (FIGHT_EVENT_REWARD_5000)
                    );
                    break;
            }
            break;
...
i32 philAI::ComputeUpgradeValue(CreatureType a1, CreatureType a2) {
    ...
    i32 result = static_cast<i32>(
        static_cast<float>(
            (gMonsterDatabase[(a2)].fightValue - gMonsterDatabase[(a1)].fightValue) * cnt
        )
        * gpCurPlayer->m_aiData.m_upgradeValueWeight
    );
        ...
        case HERO_SKILL_ARCHERY:
            ...
            for (armyIndex4 = 0; armyIndex4 < SECONDARY_SKILL_ARMY_SLOTS; armyIndex4++) {
                if (h->m_army.m_creatureTypes[armyIndex4] != CREATURE_NONE) {
                    ...
                    if ((((gMonsterDatabase[(h->m_army.m_creatureTypes[armyIndex4])].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
...
i32 philAI::ComputeValueOfFreeSS(
    ...
    if (h->m_secondarySkills[(skill)] != HERO_SKILL_LEVEL_NONE
...
i32 philAI::ValueOfEventAtPosition(i32 x, i32 y, i32 immediate, i32* liveChance) {
    ...
    *liveChance = POSITION_FULL_CHANCE;
        ...
        switch (cell_k->m_triggerType & MAP_TRIGGER_TYPE_MASK) {
            ...
            case MAP_OBJECT_SHRINE_THIRD_CIRCLE:
                if (gpCurAIHero->Stats(HERO_PRIMARY_KNOWLEDGE) > 0
                    ...
                    if ((gsSpellInfo[cell_k->m_objectMetadata - 1].level)
                        <= (gpCurAIHero->m_secondarySkills[(HERO_SKILL_WISDOM)])
                               + WISDOM_SPELL_LEVEL_BONUS) {
                        value_h = gsSpellInfo[cell_k->m_objectMetadata - 1].aiValue;
                        if ((((gsSpellInfo[cell_k->m_objectMetadata - 1].attributes) & (SPELL_INFO_ATTRIBUTE_POWER)))) {
            ...
            case MAP_OBJECT_PYRAMID:
                if (!cell_k->m_objectMetadata) {
                    ...
                    if ((((gsSpellInfo[index_k].attributes) & (SPELL_INFO_ATTRIBUTE_POWER)))) {
                        battleStatMod_n =
    ...
    if (!immediate) {
        if (value_h > 0 && (mapExtra[MAP_WIDTH * y + x] & (MAP_EXTRA_ADJACENT_MONSTER))
...
i32 philAI::EvaluateRecruitSite(mapCell* cell) {
    ...
    recruitmentSiteType =
        static_cast<RecruitSiteType>((recruitmentSiteType) & EVENT_RECRUIT_TYPE_MASK);
...
i32 philAI::EvaluateArtifactEvent(ArtifactType artifact, i32 eventData) {
    ...
    if (eventData & MAP_EVENT_ARTIFACT_GUARD_FLAG) {
        ...
        guardedValue2 =
            static_cast<i32>(gArtifactBaseRV[(artifact)] * winChance9 + outcomeValue17);
        if (guardedValue2 < 0)
            guardedValue2 = 0;
        ...
        switch (static_cast<AIArtifactEventMode>(eventData & AI_ARTIFACT_EVENT_MODE_MASK)) {
            ...
            case AI_ARTIFACT_EVENT_REQUIRES_WISDOM:
                if (gpCurAIHero->m_secondarySkills[(HERO_SKILL_WISDOM)]
                    != HERO_SKILL_LEVEL_NONE)
                    result5 = defaultValue37;
                else
                    result5 = 0;
            ...
            case AI_ARTIFACT_EVENT_REQUIRES_LEADERSHIP:
                if (gpCurAIHero->m_secondarySkills[(HERO_SKILL_LEADERSHIP)]
                    != HERO_SKILL_LEVEL_NONE)
                    result5 = defaultValue37;
                else
                    result5 = 0;
            ...
            case AI_ARTIFACT_EVENT_PAY_GOLD:
                result5 = NetValueOfArtifact((artifact), AI_ARTIFACT_EVENT_GOLD_COST, 0, 0);
                break;
...
i32 philAI::EvaluateMonsterEvent(CreatureType monsterType, i32 eventData, i32* liveChance) {
    ...
    monsterCount4 = eventData & (MAP_MONSTER_COUNT_MASK);
    willJoin15 = eventData & (MAP_MONSTER_GUARD_FLAG);
    result5 = 0;
    memset(gpMonGroup->m_creatureTypes, ARMY_GROUP_EMPTY_SLOT, ARMY_GROUP_SLOT_COUNT);
    memset(
        gpMonGroup->m_quantities,
        ...
        *liveChance = static_cast<i32>(winChance9 * AI_EVENT_CERTAIN_CHANCE);
        ...
        if (result5 > 0
            && EVENT_EARLY_TURN_BASE
                       - (gpGame->m_difficulty) * EVENT_EARLY_TURN_DIFFICULTY_STEP
                   > giCurTurn
