    ...
    for (node = 0; node < gpCurPlayer->m_townCount; node++) {
        ...
        for (armyNo = 0; armyNo < CREATURE_UPGRADE_ARMY_COUNT; armyNo++) {
            ...
            for (slot = 0; slot < ARMY_GROUP_SLOT_COUNT; slot++) {
                ...
                if (upgradable) {
                    goldAmount = (gMonsterDatabase[H2EnumIndex(newType)].cost
                    ...
                    if (goldAmount <= gpCurPlayer->m_resources[H2EnumIndex(RES_GOLD)]
                        && (material == RES_NONE
                            || resourceAmount <= gpCurPlayer->m_resources[H2EnumIndex(material)])) {
                        gpCurPlayer->m_resources[H2EnumIndex(RES_GOLD)] -= goldAmount;
                        if (material != RES_NONE)
                            gpCurPlayer->m_resources[H2EnumIndex(material)] -= resourceAmount;
...
void philAI::CheckBuyStuff(void) {
    ...
    while (!done) {
        ...
        if (H2EnumIndex(best.type) >= 0 && CanBuyBHC(best)) {
            switch (best.type) {
...
i32 philAI::GoodAdjacent(MapDirection* direction) {
    ...
    for (dirIdx = MAP_DIRECTION_NORTH; H2EnumIndex(dirIdx) < NORMAL_DIRECTION_COUNT;
        ...
        if (gpAdvManager->ValidMoveWithEvent(gpCurAIHero, dirIdx)) {
            kn = normalDirTable[H2EnumIndex(dirIdx)].x + gpCurAIHero->m_x;
            nb = normalDirTable[H2EnumIndex(dirIdx)].y + gpCurAIHero->m_y;
            if ((H2EnumIndex((gpAdvManager->GetCell(kn, nb)->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG)))
                && !(mapExtra[kn + MAP_WIDTH * nb] & H2EnumIndex(MAP_EXTRA_ADJACENT_MONSTER))
                                ...
                                if (positionValue > bestValue) {
                                    for (adjacentIndex = 0;
                                        ...
                                        if (giGroundToTerrain[cell->m_terrainImageIndex]
                                            == TERRAIN_WATER)
                                            continue;
                                        if (cell->m_flags & H2EnumIndex(MAP_CELL_OCCUPIED))
                                            continue;
                                        if ((H2EnumIndex((cell->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG))))
                                            continue;
    ...
    for (node = gpSearchArray->m_pathLength - 1; node >= 1; node--) {
        ...
        if (abs(kn - pHero->m_x) <= 7 && abs(nb - pHero->m_y) <= 7) {
            ...
            if (!(jb->m_triggerType & MAP_TRIGGER_ACTION_FLAG)
                && !(jb->m_flags & H2EnumIndex(MAP_CELL_OCCUPIED))) {
                bestX = kn;
                bestY = nb;
...
void philAI::DoAI(i32 player) {
    ...
    for (pathIndex3[0] = 0; pathIndex3[0] < H2EnumIndex(RES_COUNT); pathIndex3[0]++) {
        sprintf(
            gText,
            "RES - %15s  %d  %d",
    ...
    while ((currentHero15 = DetermineHeroToMove(player)) != NULL) {
        ...
        heroDone5 = 0;
        ResetHeroRVs(0, 0, 0);
        stepLimit36 = (H2EnumIndex((gpCurAIHero->m_eventFlags) & (HERO_EVENT_EMBARKED))) ? 15 : 5;
        minimumValue9 = gpCurAIHero->m_mobility + 800;
        stepLimit36 =
            static_cast<i32>(stepLimit36 * (1.7 - H2EnumIndex(gpGame->m_difficulty) * 0.1));
        minimumValue9 = static_cast<i32>(
            minimumValue9 * ((H2EnumIndex(gpGame->m_difficulty) - 1) * 0.06 + 0.8)
        );
    ...
    for (playerIndex0 = 0; playerIndex0 < gpGame->m_playerCount; playerIndex0++) {
        ...
        for (innerIndex27 = 0; innerIndex27 < gpGame->m_players[playerIndex0].m_heroCount;
            ...
            gpSearchArray->SeedPosition(
                ...
                H2EnumIndex(threatHeroPtr6->m_eventFlags) & H2EnumIndex(HERO_EVENT_EMBARKED),
                1,
                threatHeroPtr6->m_remainingMobility + 100,
                H2EnumIndex(threatHeroPtr6->m_secondarySkills[H2EnumIndex(HERO_SKILL_PATHFINDING)]),
                -1,
                -1,
                0,
                0
                                ...
                                if (threatTownId8 == -1) {
                                    ...
                                    if (threatTownPtr10->m_owner == giCurPlayer
                                        && (threatTownPtr10->m_buildings
                                            & H2EnumIndex(KB_DWELLING_UPGRADE_SIXTH_FLAG))) {
...
void philAI::GetBestBHC(i32 player, BHC& best) {
    i32 townStrength6[H2EnumIndex(GAME_TOWN_COUNT)];
    i32 desiredStrength5[H2EnumIndex(GAME_TOWN_COUNT)];
    BHC candidate10;
    float value19 = 1.0f;
    ...
    for (i36 = 0; i36 < gpCurPlayer->m_townCount; i36++) {
        ...
        if (townPtr7->m_buildings & H2EnumIndex(TOWN_BUILDING_CASTLE))
            totalWeight1 += 10;
        else
            totalWeight1 += 7;
    }
    ...
    for (i36 = 0; i36 < gpCurPlayer->m_townCount; i36++) {
        townPtr7 = &gpGame->m_castleRecs[gpCurPlayer->m_townIds[i36]];
        desiredStrength5[i36] = ((townPtr7->m_buildings & H2EnumIndex(TOWN_BUILDING_CASTLE)) ? 10 : 7)
                                     * averageStrength19
                                 + 400;
    }
    ...
    for (i36 = 0; i36 < gpCurPlayer->m_townCount; i36++) {
            ...
            continue;
        {
            ...
            if (gpCurPlayer->m_heroCount < giMaxHeroesForThisPlayer
                && (townPtr7->m_buildings & H2EnumIndex(TOWN_BUILDING_CASTLE))) {
...
i32 philAI::DetermineTargetPosition(
    ...
    H2EnumStorage<TerrainType, i32> mapTerrain;
    town* shipyardPtrSearch;
    i32 scanSpacingStep;

    if (GoodAdjacent(&adjacentDirection)) {
        targetX = normalDirTable[H2EnumIndex(adjacentDirection)].x + gpCurAIHero->m_x;
        targetY = normalDirTable[H2EnumIndex(adjacentDirection)].y + gpCurAIHero->m_y;
        ...
        for (x = scanMinXValue; x < scanMaxXLocal; x++) {
            ...
            for (y = scanMinY; y < scanMaxYCounter; y++) {
                ...
                if (gpSearchArray->GetNode(x, y).visited) {
                    ...
                    } else if ((H2EnumIndex((candidateCell->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG)))
                ...
                candidate_scored:
                    if (bestValue < targetScoreLocal) {
                        ...
                        if ((H2EnumIndex((gpAdvManager->GetCell(targetBestXRange, targetBestYLocal)
    ...
    if (attackerHero != NULL) {
        for (artifactIndex15 = 0; artifactIndex15 < AI_BATTLE_ARTIFACT_SLOT_COUNT;
            ...
            if (attackerHero->m_artifacts[artifactIndex15] >= ARTIFACT_ULTIMATE_BOOK
                && H2EnumIndex(attackerHero->m_artifacts[artifactIndex15])
                       < AI_BATTLE_BASE_ARTIFACT_LIMIT)
                attackerArtifacts7 +=
                    gArtifactBaseRV[H2EnumIndex(attackerHero->m_artifacts[artifactIndex15])];
        }
    ...
    if (defenderHero != NULL) {
        for (artifactIndex15 = 0; artifactIndex15 < AI_BATTLE_ARTIFACT_SLOT_COUNT;
            ...
            if (defenderHero->m_artifacts[artifactIndex15] >= ARTIFACT_ULTIMATE_BOOK
                && H2EnumIndex(defenderHero->m_artifacts[artifactIndex15])
                       < AI_BATTLE_BASE_ARTIFACT_LIMIT)
                defenderArtifacts18 +=
                    gArtifactBaseRV[H2EnumIndex(defenderHero->m_artifacts[artifactIndex15])];
        }
    ...
    switch (building) {
        ...
        case BUILDING_SLOT_SPECIAL_THIRTY:
            if (currentOccupiedSlots == AI_TOWN_ARMY_SLOTS) {
                creatureLocated = 0;
                for (indexBuilding = 0; indexBuilding < AI_TOWN_ARMY_SLOTS; indexBuilding++) {
                    if (gDwellingType[H2EnumIndex(townPtr->m_type)]
...
void philAI::ValueOfBuyingCreature(
    ...
    if (townPtr->m_occupyingHeroId != AI_TROOP_EMPTY_SLOT) {
        ...
        if (gMonsterDatabase[H2EnumIndex(creature)].race == visitingHeroCandidate->m_cursorType)
        ...
        if ((H2EnumIndex((gMonsterDatabase[H2EnumIndex(creature)].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
            for (visitingArmySlot = 0; visitingArmySlot < CREATURE_PURCHASE_ARMY_SLOT_COUNT;
                ...
                if (visitingHeroCandidate->m_army.m_creatureTypes[visitingArmySlot]
                        != CREATURE_NONE
                    && (H2EnumIndex((gMonsterDatabase[H2EnumIndex(visitingHeroCandidate->m_army
    ...
    if ((H2EnumIndex((gMonsterDatabase[H2EnumIndex(creature)].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
        for (garrisonSlot = 0; garrisonSlot < CREATURE_PURCHASE_ARMY_SLOT_COUNT;
            ...
            if (townPtr->m_army.m_creatureTypes[garrisonSlot] != CREATURE_NONE
                && (H2EnumIndex((gMonsterDatabase[H2EnumIndex(townPtr->m_army.m_creatureTypes[garrisonSlot])].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
                missileStacks++;
            }
        }
        creatureValue = static_cast<i32>(
...
void philAI::GetBestCreature(town* townPtr, BHC& best, float& bestValue) {
    ...
    for (dwelling = 0; dwelling < CREATURE_PURCHASE_DWELLING_COUNT; dwelling++) {
        ...
        if ((townPtr->m_buildings & (1 << H2EnumIndex(dwelling + H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST))))
            ...
            if (weakestArmyValue < gMonsterDatabase[H2EnumIndex(candidateMonster)].randomValue
...
void philAI::ValueOfBuyingHero(
    ...
    costs2[H2EnumIndex(RES_WOOD)] = 0;
    costs2[H2EnumIndex(RES_MERCURY)] = 0;
    costs2[H2EnumIndex(RES_ORE)] = 0;
    costs2[H2EnumIndex(RES_SULFUR)] = 0;
    costs2[H2EnumIndex(RES_CRYSTAL)] = 0;
    costs2[H2EnumIndex(RES_GEMS)] = 0;
    costs2[H2EnumIndex(RES_GOLD)] = AI_HERO_PURCHASE_GOLD_COST;
    ...
    for (artifactIndex10 = 0; artifactIndex10 < AI_BATTLE_ARTIFACT_SLOT_COUNT; artifactIndex10++) {
        if (heroPtr->m_artifacts[artifactIndex10] >= ARTIFACT_ULTIMATE_BOOK
            ...
            value27 += gArtifactBaseRV[H2EnumIndex(heroPtr->m_artifacts[artifactIndex10])];
        }
    }
...
i32 philAI::RVConversion(i32* const p) {
    return (i32)((((((((float)p[H2EnumIndex(RES_ORE)] * gafAITurnCostResource[H2EnumIndex(RES_ORE)])
                    ...
                    + (float)p[H2EnumIndex(RES_GOLD)] * gafAITurnCostResource[H2EnumIndex(RES_GOLD)])
...
i32 philAI::RVOfPosition(
    ...
    if ((H2EnumIndex((triggerType3) & (MAP_TRIGGER_ACTION_FLAG)))
        || (gpCurPlayer->m_ultimateArtifactHintX == x
            && gpCurPlayer->m_ultimateArtifactHintY == y)) {
    ...
    if ((H2EnumIndex((gpCurAIHero->m_eventFlags) & (HERO_EVENT_EMBARKED))) && triggerType3 == MAP_OBJECT_COAST) {
    ...
    if (giDebugLevel > POSITION_DEBUG_LEVEL - 1) {
        ...
        char* objectName5;
        if (debugObjectType > MAP_OBJECT_NONE && H2EnumIndex(debugObjectType) < POSITION_OBJECT_NAME_COUNT)
...
i32 philAI::StrategicValueOfPosition(
    ...
    i32 danger26;
    i32 heroIndex2;
    H2EnumStorage<TerrainType, i32> heroTerrain7;
    i32 shortDistance13;
    i32 penalty;
    i32 allocatedSearch2;
    i32 unusedStrategicValue17;
    H2EnumStorage<TerrainType, i32> targetTerrain29;
    i32 score4;
    ...
    *liveChance = POSITION_FULL_CHANCE;
    ...
    eventFlags8 = (H2EnumIndex((gpCurAIHero->m_eventFlags) & (HERO_EVENT_EMBARKED)));
    ...
    for (x13 = 0; MAP_WIDTH > x13; x13++) {
        for (yCounter = 0; MAP_HEIGHT > yCounter; yCounter++) {
            if (search5->GetNode(x13, yCounter).visited) {
                ...
                if ((!immediate && (H2EnumIndex((cell9->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG)))
...
float philAI::TurnValueOfObelisk(i32 player) {
    ...
    jb = gArtifactBaseRV[H2EnumIndex(gpGame->m_ultimateArtifactId)];
        ...
        if (heroPtr->m_spellPoints >= 3) {
            ...
            for (armySlotRecord = 0; armySlotRecord < FIGHT_VALUE_SPELL_COUNT; armySlotRecord++) {
                if (heroPtr->HasSpell(SpellType(armySlotRecord))) {
                    ...
                    if ((H2EnumIndex((gsSpellInfo[armySlotRecord].attributes) & (SPELL_INFO_ATTRIBUTE_DURATION))))
                        spellScoreTotal = static_cast<i32>(spellScoreTotal * durationModifier7);
                    else if ((H2EnumIndex((gsSpellInfo[armySlotRecord].attributes) & (SPELL_INFO_ATTRIBUTE_POWER))))
...
void philAI::EvaluateOneTimeCreaturePurchase(
    ...
    if (purchaseCount == 0)
        return;
    {
        purchaseFightValue15 = gMonsterDatabase[H2EnumIndex(creature)].fightValue * purchaseCount;
...
i32 philAI::QuickCombat(
    ...
    attackerExperience37 = static_cast<i32>(
        (H2EnumIndex(gpGame->m_difficulty) * AI_QUICK_COMBAT_EXPERIENCE_DIFFICULTY_STEP + 1.0)
    ...
    if (defeatedHero5 != NULL
        && defeatedHero5->m_secondarySkills[H2EnumIndex(HERO_SKILL_EAGLE_EYE)]
               != HERO_SKILL_LEVEL_NONE
        && victoriousHero4 != NULL) {
        for (armyIndex0 = 0; armyIndex0 < H2EnumIndex(SPELL_COUNT); armyIndex0++) {
            if (defeatedHero5->HasSpell(SpellType(armyIndex0)) != 0
                && victoriousHero4->HasSpell(SpellType(armyIndex0)) == 0
                && H2EnumIndex(gsSpellInfo[armyIndex0].level)
                       <= H2EnumIndex(victoriousHero4->m_secondarySkills[H2EnumIndex(HERO_SKILL_EAGLE_EYE)]) + 1
                && (H2EnumIndex((gsSpellInfo[armyIndex0].attributes) & (SPELL_INFO_ATTRIBUTE_COMBAT)))) {
    ...
    *value = 0;
        ...
        return;
    {
        ...
        for (heroIndex9 = 0; heroIndex9 < HERO_INTERACTION_HERO_COUNT; heroIndex9++) {
            ...
            for (statIndex8 = H2EnumIndex(HERO_SKILL_PATHFINDING); statIndex8 < H2EnumIndex(HERO_SKILL_COUNT);
                 statIndex8++) {
                ...
                if (currentHero9->m_secondarySkills[statIndex8] != HERO_SKILL_LEVEL_NONE) {
                    heroValues27[heroIndex9] +=
                        gSSValues[statIndex8]
                                 [H2EnumIndex(currentHero9->m_secondarySkills[statIndex8])
            ...
            if (evaluateOnly != 0) {
                for (statIndex8 = 0; statIndex8 < AI_BATTLE_ARTIFACT_SLOT_COUNT; statIndex8++) {
                    ...
                    if (artifactType15 != ARTIFACT_NONE && artifactType15 != ARTIFACT_MAGIC_BOOK)
                        interactionValue8 += gArtifactBaseRV[H2EnumIndex(artifactType15)];
                }
            } else {
        ...
        for (spellLevel14 = 1;
            ...
            for (spellIndex = 0; spellIndex < townPtr->m_spellCounts[spellLevel14];
                ...
                if (!heroPtr->HasSpell(townPtr->m_spells[spellLevel14 - 1][spellIndex])) {
                    if ((H2EnumIndex((gsSpellInfo[H2EnumIndex(townPtr->m_spells[spellLevel14 - 1][spellIndex])]
                    ...
                    *value +=
                        spellMultiplier1
                        * gsSpellInfo[H2EnumIndex(townPtr->m_spells[spellLevel14 - 1][spellIndex])].aiValue;
                }
            }
        }
    }
    ...
    while (keepGoing5) {
        ...
        if (selectedIndex5 == AI_TROOP_EMPTY_SLOT) {
            ...
            for (sourceIndex15 = 0; sourceIndex15 < AI_TOWN_ARMY_SLOTS; sourceIndex15++) {
                if (sourceArmy->m_creatureTypes[sourceIndex15] != CREATURE_NONE) {
                    stackValue7 =
                        gMonsterDatabase[H2EnumIndex(sourceArmy->m_creatureTypes[sourceIndex15])].fightValue
                        * sourceArmy->m_quantities[sourceIndex15];
                    if ((preferFast != 0
                         && gMonsterDatabase[H2EnumIndex(sourceArmy->m_creatureTypes[sourceIndex15])].speed
                                > selectedSpeed19)
                        || (preferFast == 0
                            && gMonsterDatabase[H2EnumIndex(sourceArmy->m_creatureTypes[sourceIndex15])].speed
                                   < selectedSpeed19)) {
                        ...
                        selectedIndex5 = sourceIndex15;
                    } else if (gMonsterDatabase[H2EnumIndex(sourceArmy->m_creatureTypes[sourceIndex15])].speed
                                   == selectedSpeed19
        ...
        if (selectedIndex5 == AI_TROOP_EMPTY_SLOT) {
            ...
            transferCount16 = static_cast<i32>(
                static_cast<double>(transferBudget)
                    / gMonsterDatabase[H2EnumIndex(sourceArmy->m_creatureTypes[selectedIndex5])].fightValue
            ...
            if (transferCount16 > 0) {
                ...
                if (gMonsterDatabase[H2EnumIndex(sourceArmy->m_creatureTypes[selectedIndex5])].fightValue
...
i32 philAI::ChooseToFightForArtifact(
    ...
    i32 idx;
    i32 o5;
    jb = gArtifactBaseRV[H2EnumIndex(a)];
...
void philAI::BuildCreature(town* townPtr, i32 dwelling, i32 purchaseCount) {
    ...
    creatureType13 = gDwellingType[H2EnumIndex(townPtr->m_type)][dwelling];
    canJoin6 = 0;
    ...
    if (canJoin6 == 0) {
        for (armyIndex4 = 0; armyIndex4 < AI_TOWN_ARMY_SLOTS; armyIndex4++) {
            stackValue9 = static_cast<float>(
                gMonsterDatabase[H2EnumIndex(townPtr->m_army.m_creatureTypes[armyIndex4])].fightValue
                * townPtr->m_army.m_quantities[armyIndex4]
            );
            if (gMonsterDatabase[H2EnumIndex(townPtr->m_army.m_creatureTypes[armyIndex4])].race
                != townPtr->m_type) {
        ...
        gpCurPlayer->m_resources[H2EnumIndex(RES_GOLD)] +=
            gMonsterDatabase[H2EnumIndex(townPtr->m_army.m_creatureTypes[weakestSlot8])].cost
...
i32 philAI::CombatMonsterEvent(
    hero* h, CreatureType monType, i32* pCount, mapCell* cell
) {
    i32 kn;
    i32 jb;
    float f2;
    ...
    switch (eventType16) {
            ...
            switch (cell->m_objectMetadata) {
                case FIGHT_EVENT_LEVEL_1:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * H2EnumIndex(FIGHT_EVENT_REWARD_500)
                    );
                    break;
                case FIGHT_EVENT_LEVEL_2:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * H2EnumIndex(FIGHT_EVENT_REWARD_1000)
                    );
                    break;
                case FIGHT_EVENT_LEVEL_3:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[H2EnumIndex(RES_GOLD)]
                        * H2EnumIndex(FIGHT_EVENT_REWARD_3000)
                    );
                    break;
        ...
        case MAP_OBJECT_SHIPWRECK:
            switch (cell->m_objectMetadata) {
                ...
                case FIGHT_EVENT_LEVEL_2:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * H2EnumIndex(FIGHT_EVENT_REWARD_2000)
                    );
                    break;
                case FIGHT_EVENT_LEVEL_3:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[H2EnumIndex(RES_GOLD)]
                        * H2EnumIndex(FIGHT_EVENT_REWARD_5000)
                    );
                    break;
                case FIGHT_EVENT_LEVEL_4:
                    rewardValue7 = static_cast<i32>(
        ...
        case MAP_OBJECT_DERELICT_SHIP:
            switch (cell->m_objectMetadata) {
                ...
                case FIGHT_EVENT_LEVEL_4:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * H2EnumIndex(FIGHT_EVENT_REWARD_5000)
                    );
                    break;
            }
            break;
...
i32 philAI::ComputeValueOfSS(
    ...
    value28 = gSSValues[H2EnumIndex(skill)][H2EnumIndex(level) - SECONDARY_SKILL_LEVEL_OFFSET];
        ...
        case HERO_SKILL_ARCHERY:
            ...
            for (armyIndex4 = 0; armyIndex4 < SECONDARY_SKILL_ARMY_SLOTS; armyIndex4++) {
                if (h->m_army.m_creatureTypes[armyIndex4] != CREATURE_NONE) {
                    stackValue7 =
                        gMonsterDatabase[H2EnumIndex(h->m_army.m_creatureTypes[armyIndex4])].fightValue
                                  * h->m_army.m_quantities[armyIndex4];
                    totalArmyValue2 += stackValue7;
                    if ((H2EnumIndex((gMonsterDatabase[H2EnumIndex(h->m_army.m_creatureTypes[armyIndex4])].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
            ...
            value28 = static_cast<i32>(
                (rangedShare2 / AI_SECONDARY_SKILL_ARCHERY_SHARE + AI_SECONDARY_SKILL_ARCHERY_BASE)
                * value28
            );
            break;
...
i32 philAI::ValueOfEventAtPosition(i32 x, i32 y, i32 immediate, i32* liveChance) {
    ...
    H2EnumStorage<ResourceType, i32> resource;
    ...
    *liveChance = POSITION_FULL_CHANCE;
    ...
    if (gpCurPlayer->m_ultimateArtifactHintChance > 15 && gpCurPlayer->m_ultimateArtifactHintX == x
    ...
    } else if ((H2EnumIndex((cell_k->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG)))) {
        switch (cell_k->m_triggerType & MAP_TRIGGER_TYPE_MASK) {
                ...
                switch (resource) {
                    case RES_GOLD:
                        value_h = static_cast<i32>(
                            gafAITurnCostResource[H2EnumIndex(resource)]
                            * (cell_k->m_objectMetadata * AI_GOLD_RESOURCE_MULTIPLIER)
                        );
                        break;
                    default:
                        value_h = static_cast<i32>(
                            gafAITurnCostResource[H2EnumIndex(resource)] * cell_k->m_objectMetadata
                        );
                        break;
                }
                break;
            case MAP_OBJECT_TREASURE_CHEST:
                value_h = static_cast<i32>(
                    gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * AI_TREASURE_CHEST_GOLD_AMOUNT
                );
                break;
            ...
            case MAP_OBJECT_CAMPFIRE:
                value_h = static_cast<i32>(
                    ...
                    + gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * AI_CAMPFIRE_GOLD_AMOUNT
                );
                break;
            case MAP_OBJECT_ARTIFACT:
                value_h = EvaluateArtifactEvent(
                    ArtifactType(cell_k->m_objectIndex >> 1),
                    H2EnumIndex(cell_k->m_objectMetadata)
                );
                break;
            ...
            case MAP_OBJECT_SHRINE_THIRD_CIRCLE:
                if (gpCurAIHero->Stats(HERO_PRIMARY_KNOWLEDGE) > 0
                    ...
                    if (H2EnumIndex(gsSpellInfo[cell_k->m_objectMetadata - 1].level)
                        ...
                        if ((H2EnumIndex((gsSpellInfo[cell_k->m_objectMetadata - 1].attributes) & (SPELL_INFO_ATTRIBUTE_POWER)))) {
            ...
            case MAP_OBJECT_MAGIC_GARDEN:
                if (!cell_k->m_objectMetadata)
                    value_h = 0;
                else if (cell_k->m_objectMetadata - 1 == H2EnumIndex(RES_GOLD))
                    value_h = static_cast<i32>(
                        gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * AI_MAGIC_GARDEN_GOLD_AMOUNT
                    );
                else
                    value_h =
                        static_cast<i32>(
            ...
            case MAP_OBJECT_FLOTSAM:
                value_h =
                    static_cast<i32>(gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * AI_FLOTSAM_GOLD_AMOUNT);
                value_h =
                    static_cast<i32>(gafAITurnCostResource[H2EnumIndex(RES_WOOD)] * AI_FLOTSAM_WOOD_AMOUNT);
                break;
            case MAP_OBJECT_SEA_CHEST:
                if (cell_k->m_objectMetadata & MAP_EVENT_ARTIFACT_GUARD_FLAG)
                    value_h = static_cast<i32>(
                        gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * AI_SEA_CHEST_ARTIFACT_GOLD_AMOUNT
                        + gArtifactBaseRV[cell_k->m_objectMetadata & MAP_EVENT_ARTIFACT_ID_MASK]
                    );
                else if (cell_k->m_objectMetadata == 1)
                    value_h = static_cast<i32>(
                        gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * AI_SEA_CHEST_LARGE_GOLD_AMOUNT
                    );
                else
                    value_h = static_cast<i32>(
                        gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * AI_SEA_CHEST_SMALL_GOLD_AMOUNT
                    );
                if (value_h <= 0)
            ...
            case MAP_OBJECT_LEAN_TO:
                if (!cell_k->m_objectMetadata)
                    ...
                    value_h = static_cast<i32>(gafAITurnCostResource[H2EnumIndex(resource)] * amount_j);
                }
                break;
            ...
            case MAP_OBJECT_PYRAMID:
                if (!cell_k->m_objectMetadata) {
                    ...
                    if ((H2EnumIndex((gsSpellInfo[index_k].attributes) & (SPELL_INFO_ATTRIBUTE_POWER)))) {
                        battleStatMod_n =
                ...
                if (cell_k->m_objectMetadata == 1)
                    ...
                    value_h = static_cast<i32>(
                        (gpCurAIHero->m_aiFightValue * AI_DAEMON_FIGHT_VALUE_FACTOR
                         + gafAITurnCostResource[H2EnumIndex(RES_GOLD)] * AI_DAEMON_GOLD_VALUE_FACTOR)
    ...
    if (!immediate) {
        if (value_h > 0 && (mapExtra[MAP_WIDTH * y + x] & H2EnumIndex(MAP_EXTRA_ADJACENT_MONSTER))
...
i32 philAI::EvaluateGenericSite(mapCell* cell) {
    ...
    do {
        (siteType0) = static_cast<GenericSiteType>((cell->m_objectMetadata) & (GENERIC_SITE_TYPE_MASK));
    } while (0);
            ...
            if (!(gpCurAIHero->m_eventFlags & HERO_EVENT_SIRENS)) {
                ...
                for (artifactIndex1 = 0; artifactIndex1 < AI_GENERIC_SITE_ARMY_SLOTS;
                    ...
                    if (creatureType2 != CREATURE_NONE) {
                        ...
                        armyValue7 += gMonsterDatabase[H2EnumIndex(creatureType2)].hitPoints
...
i32 philAI::EvaluateArtifactEvent(ArtifactType artifact, i32 eventData) {
    ...
    result5 = 0;
    artifactValue15 = gArtifactBaseRV[H2EnumIndex(artifact)];
    ...
    if (eventData & MAP_EVENT_ARTIFACT_GUARD_FLAG) {
        ...
        guardedValue2 =
            static_cast<i32>(gArtifactBaseRV[H2EnumIndex(artifact)] * winChance9 + outcomeValue17);
        if (guardedValue2 < 0)
            guardedValue2 = 0;
        ...
        switch (static_cast<AIArtifactEventMode>(eventData & AI_ARTIFACT_EVENT_MODE_MASK)) {
            ...
            case AI_ARTIFACT_EVENT_PAY_GOLD:
                result5 = NetValueOfArtifact(H2EnumIndex(artifact), AI_ARTIFACT_EVENT_GOLD_COST, 0, 0);
                break;
    ...
    if (willJoin15 && strengthRatio26 > AI_MONSTER_JOIN_RATIO
        ...
        *liveChance = static_cast<i32>(
        ...
        if (gpCurAIHero->GetSSLevel(HERO_SKILL_NECROMANCY) != 0)
            ...
            result5 = 0;
        result5 += gMonsterDatabase[H2EnumIndex(monsterType)].hitPoints * monsterCount4;
    } else {
    ...
    if (gpGame->m_availableHeroes[heroId] == gpCurAIHero->m_owner) {
        ...
        *liveChance = static_cast<i32>(winChance9 * AI_EVENT_CERTAIN_CHANCE);
        ...
        if (winChance9 > AI_EVENT_CERTAIN_ODDS)
            ...
            gaiHeroLiveChance[heroId] = static_cast<i16>(winChance9 * AI_EVENT_HIGH_CHANCE_SCALE);
        else if (winChance9 > AI_EVENT_GOOD_ODDS)
            gaiHeroLiveChance[heroId] = static_cast<i16>(winChance9 * AI_EVENT_GOOD_CHANCE_SCALE);
        else if (winChance9 > AI_EVENT_POOR_ODDS)
            gaiHeroLiveChance[heroId] = static_cast<i16>(winChance9 * AI_EVENT_POOR_CHANCE_SCALE);
        else if (winChance9 > AI_EVENT_BAD_ODDS)
            gaiHeroLiveChance[heroId] = static_cast<i16>(winChance9 * AI_EVENT_BAD_CHANCE_SCALE);
        else
