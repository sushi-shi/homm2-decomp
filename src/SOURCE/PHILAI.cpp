    ...
    switch (eventType16) {
            ...
            switch (cell->m_objectMetadata) {
                case AI_FIGHT_EVENT_LEVEL_1:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[IDX(RES_GOLD)] * IDX(AI_FIGHT_EVENT_REWARD_500)
                    );
                    break;
                case AI_FIGHT_EVENT_LEVEL_2:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[IDX(RES_GOLD)] * IDX(AI_FIGHT_EVENT_REWARD_1000)
                    );
                    break;
                case AI_FIGHT_EVENT_LEVEL_3:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[IDX(RES_GOLD)] * IDX(AI_FIGHT_EVENT_REWARD_3000)
                    );
                    break;
        ...
        case MAP_OBJECT_SHIPWRECK:
            switch (cell->m_objectMetadata) {
                ...
                case AI_FIGHT_EVENT_LEVEL_2:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[IDX(RES_GOLD)] * IDX(AI_FIGHT_EVENT_REWARD_2000)
                    );
                    break;
                case AI_FIGHT_EVENT_LEVEL_3:
                    rewardValue7 = static_cast<i32>(
                        gafAITurnCostResource[IDX(RES_GOLD)] * IDX(AI_FIGHT_EVENT_REWARD_5000)
                    );
                    break;
                case AI_FIGHT_EVENT_LEVEL_4:
                    rewardValue7 = static_cast<i32>(
    ...
    if (gpCurPlayer->m_ultimateArtifactHintChance > 15 && gpCurPlayer->m_ultimateArtifactHintX == x
        ...
        switch (cell_k->m_triggerType & MAP_TRIGGER_TYPE_MASK) {
            ...
            case MAP_OBJECT_ARTIFACT:
                value_h =
                    EvaluateArtifactEvent(ArtifactType(cell_k->m_objectIndex) >> 1, IDX(cell_k->m_objectMetadata));
                break;
            ...
            case MAP_OBJECT_BUOY:
                if (!(gpCurAIHero->m_eventFlags & 2) && IDX(giCurAIHeroMorale) < 3)
            ...
            case MAP_OBJECT_FOUNTAIN:
                if (!(gpCurAIHero->m_eventFlags & 4) && IDX(giCurAIHeroLuck) < 3)
                    value_h = static_cast<i32>(
                        gpCurAIHero->m_aiFightValue * AI_MORALE_LUCK_SITE_VALUE_FACTOR
                    );
                break;
