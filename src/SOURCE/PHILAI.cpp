...
i32 philAI::GoodAdjacent(MapDirection* direction) {
    ...
    if ((gpAdvManager->GetCell(gpCurAIHero->m_x, gpCurAIHero->m_y)->m_triggerType
        ...
        return 0;
    for (directionIndex = MAP_DIRECTION_NORTH; H2EnumIndex(directionIndex) < NORMAL_DIRECTION_COUNT;
         directionIndex++) {
...
void philAI::CheckReload(void) {
    ...
    enemyPressure = 0.0f;
    heroFightValue = FightValueOfStack(&gpCurAIHero->m_army, gpCurAIHero, 0, 0, 0, 0);
                ...
                if (gpCurAIHero->m_destinationX != -1 && gpCurAIHero->m_destinationY != -1) {
                    ...
                    if (gpSearchArray->m_pathLength == 0) {
                        ...
                        while (pathIndex >= 0 && steps < stepLimit) {
                            ...
                            direction = MapDirectionFromCode(
                                gpSearchArray->m_storage.aiPath.directions[pathIndex]
                            );
                        aiMoveDirection:
...
firstWeekDone:
    ...
    for (generalIndex = 0; generalIndex < gpCurPlayer->m_heroCount; generalIndex++) {
        ...
        fightValueFloat =
            static_cast<float>(FightValueOfStack(&heroPointer->m_army, heroPointer, 0, 0, 0, 0));
    ...
    for (generalIndex = 0; generalIndex < gpCurPlayer->m_townCount; generalIndex++)
        gpGame->m_castleRecs[gpCurPlayer->m_townIds[generalIndex]].m_threat = 0;
    ...
    townCountValue = 0.1f;
    for (generalIndex = 0; generalIndex < gpCurPlayer->m_townCount; generalIndex++) {
        if (GetCastleSlot(gpCurPlayer->m_townIds[generalIndex])->m_buildings
    ...
    switch (building) {
        ...
        case BUILDING_SLOT_SPECIAL_ONE:
            if (townPointer->m_type == FACTION_NECROMANCER
                && (townPointer->m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_DWELLING_THIRD))))
                adjustedValue = 1500.0f;
            else if (giCurTurn < 21)
                adjustedValue = 0.0;
            break;
        case BUILDING_SLOT_SPECIAL_SEVEN:
            if (giCurTurn < 3 && !(townPointer->m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_DWELLING_THIRD))))
                adjustedValue = 0.0f;
            break;
        ...
        case BUILDING_SLOT_SPECIAL_FOUR:
            ...
            if ((townPointer->m_type != FACTION_KNIGHT
                 || !(townPointer->m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_DWELLING_SECOND))))
                && gpGame->m_day < 6)
                adjustedValue = 0.0f;
            break;
        ...
        case BUILDING_SLOT_SPECIAL_THIRTY:
            ...
            if (townPointer->m_buildings & (1 << H2EnumIndex(BUILDING_SLOT_SPECIAL_FOUR)))
    ...
    if ((H2EnumIndex((gMonsterDatabase[H2EnumIndex(creature)].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
        for (townSlot = 0; townSlot < CREATURE_PURCHASE_ARMY_SLOT_COUNT;
            ...
            if (townPointer->m_army.m_creatureTypes[townSlot] != CREATURE_NONE
                && (H2EnumIndex((gMonsterDatabase[H2EnumIndex(townPointer->m_army.m_creatureTypes[townSlot])].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
                archers++;
            }
        }
        creatureRV = static_cast<i32>(
...
void philAI::GetBestCreature(town* townPointer, BHC& best, float& bestValue) {
    ...
    for (dwelling = 0; dwelling < CREATURE_PURCHASE_DWELLING_COUNT; dwelling++) {
        ...
        if ((townPointer->m_buildings & (1 << H2EnumIndex(dwelling + H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST))))
            ...
            for (armyIndex = 0; armyIndex < CREATURE_PURCHASE_ARMY_SLOT_COUNT; armyIndex++) {
                if (townPointer->m_army.m_creatureTypes[armyIndex] != CREATURE_NONE
                    && gMonsterDatabase[H2EnumIndex(townPointer->m_army.m_creatureTypes[armyIndex])].randomValue
                           < weakestArmyValue) {
                    weakestArmyValue =
                        gMonsterDatabase[H2EnumIndex(townPointer->m_army.m_creatureTypes[armyIndex])].randomValue;
                }
            ...
            if (dwelling == 0 && townPointer->m_occupyingHeroId != AI_TROOP_EMPTY_SLOT
                && ((townPointer->m_buildings & H2EnumIndex(TOWN_BUILDING_UPGRADED_DWELLING_5))
                    || (townPointer->m_buildings & H2EnumIndex(TOWN_BUILDING_DWELLING_5))
                    || (townPointer->m_buildings & H2EnumIndex(TOWN_BUILDING_UPGRADED_DWELLING_6))
                    || (townPointer->m_buildings & H2EnumIndex(TOWN_BUILDING_ALTERNATE_UPGRADED_DWELLING_6))
                    || (townPointer->m_buildings & H2EnumIndex(TOWN_BUILDING_DWELLING_6)))) {
                canAddUnit = false;
            }
            if (canAddUnit) {
...
i32 philAI::RVConversion(i32* const resources) {
    return static_cast<i32>(
        ((((((static_cast<float>(resources[H2EnumIndex(RES_GOLD)])
              * gafAITurnCostResource[H2EnumIndex(RES_GOLD)])
             + static_cast<float>(resources[H2EnumIndex(RES_WOOD)])
                   * gafAITurnCostResource[H2EnumIndex(RES_WOOD)])
            + static_cast<float>(resources[H2EnumIndex(RES_ORE)])
                  * gafAITurnCostResource[H2EnumIndex(RES_ORE)])
           + static_cast<float>(resources[H2EnumIndex(RES_CRYSTAL)])
                 * gafAITurnCostResource[H2EnumIndex(RES_CRYSTAL)])
          + static_cast<float>(resources[H2EnumIndex(RES_SULFUR)])
                * gafAITurnCostResource[H2EnumIndex(RES_SULFUR)])
         + static_cast<float>(resources[H2EnumIndex(RES_MERCURY)])
               * gafAITurnCostResource[H2EnumIndex(RES_MERCURY)])
        + static_cast<float>(resources[H2EnumIndex(RES_GEMS)])
...
i32 philAI::RVOfPosition(
    ...
    if ((H2EnumIndex((gpCurAIHero->m_eventFlags) & (HERO_EVENT_EMBARKED))) && triggerType == MAP_OBJECT_COAST) {
...
void philAI::TurnCostResource(i32 player) {
    ...
    for (resourceIndex = 0; resourceIndex < AI_PURCHASE_RESOURCE_COUNT; resourceIndex++) {
        resValue[resourceIndex] = static_cast<i32>(
            static_cast<double>(gResourceBaseValue[resourceIndex])
            * (static_cast<double>(playerAI->m_income[resourceIndex] * 5) * 0.7
               + static_cast<double>(gpGame->m_players[player].m_resources[resourceIndex]))
        );
        sum += resValue[resourceIndex];
    }
    average = static_cast<float>(sum / AI_PURCHASE_RESOURCE_COUNT);
    for (resourceIndex = 0; resourceIndex < AI_PURCHASE_RESOURCE_COUNT; resourceIndex++) {
        frac[resourceIndex] = static_cast<float>(resValue[resourceIndex]) / average;
        gafAITurnCostResource[resourceIndex] =
            static_cast<float>(gResourceBaseValue[resourceIndex] / (frac[resourceIndex] / 2.0f + 0.5));
    }
}
...
void philAI::HeroInteractionAtHero(
    ...
    *value = 0;
        ...
        return;
    {
        ...
        dominantFightValue = FightValueOfStack(&dominantHero->m_army, NULL, 0, 0, 0, 0);
        recipientFightValue = FightValueOfStack(&recipientHero->m_army, NULL, 0, 0, 0, 0);
...
void philAI::HeroInteractionAtTown(hero* heroPointer, town* townPointer, i32 doInteraction, i32* value) {
    ...
    *value = 0;
    if (doInteraction != 0) {
        ...
        if (!heroPointer->HasArtifact(ARTIFACT_MAGIC_BOOK)
            ...
            if (gpCurPlayer->m_resources[H2EnumIndex(RES_GOLD)] >= AI_MAGIC_BOOK_COST) {
                GiveArtifact(heroPointer, ARTIFACT_MAGIC_BOOK, true, -1);
    ...
    battlePower = FightValueOfStack(&heroPointer->m_army, NULL, 0, 0, 0, 0);
    townFV = FightValueOfStack(&townPointer->m_army, NULL, 0, 0, 0, 0);
...
i32 philAI::NetValueOfArtifact(i32 artifact, i32 goldCost, i32 resourceType, i32 resourceCost) {
    return static_cast<i32>(
        ...
        - (static_cast<float>(goldCost) * gafAITurnCostResource[H2EnumIndex(RES_GOLD)]
...
i32 philAI::ComputeValueOfSS(
    ...
    armyValue = FightValueOfStack(&heroPointer->m_army, heroPointer, 1, 0, 0, 0);
    ...
    switch (skill) {
        case HERO_SKILL_NAVIGATION:
            if ((H2EnumIndex((heroPointer->m_eventFlags) & (HERO_EVENT_EMBARKED))))
        ...
        case HERO_SKILL_ARCHERY:
            ...
            for (index = 0; index < SECONDARY_SKILL_ARMY_SLOTS; index++) {
                if (heroPointer->m_army.m_creatureTypes[index] != CREATURE_NONE) {
                    ...
                    if ((H2EnumIndex((gMonsterDatabase[H2EnumIndex(heroPointer->m_army.m_creatureTypes[index])].attributes) & (MONSTER_ATTRIBUTE_RANGED)))) {
...
i32 philAI::ManaRefreshValue(hero* heroPointer, i32 level) {
    ...
    missingFraction = static_cast<float>(deficit) / static_cast<float>(maximumSpellPoints);
    if (deficit > 0)
