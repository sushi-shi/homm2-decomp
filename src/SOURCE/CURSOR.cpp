...
typedef enum CursorPrivateConstant {
    ...
    DIRECTION_HALF_COUNT     = (MAP_DIRECTION_COUNT) / 2,
        ...
        if (m_cursorType == HERO_TYPE_BOAT) {
            ...
            if (m_cursorCycle == 0) {
                drawFrame = (m_cursorFrame & CURSOR_FRAME_MASK)
                            + m_updateMaxY % (MAP_DIRECTION_COUNT) + CURSOR_FLAG_FRAME_BASE;
            }
            DRAW_FLIPPED_ADVENTURE_ICON(
...
void advManager::TurnTo(MapDirection direction) {
    ...
    i32 delayTime =
        giStepDelay[(gConfig.walkSpeeds[gbThisNetHumanPlayer[giCurPlayer]])];
    if (gConfig.walkSpeeds[gbThisNetHumanPlayer[giCurPlayer]]
        == CONFIG_WALK_SPEED_SLOWEST)
...
b32 advManager::GetMoveShowIt(
    ...
    i32 dx = normalDirTable[(direction)].x;
    i32 dy = normalDirTable[(direction)].y;
...
mapCell* advManager::MoveHero(
    ...
    *eventY = movingHero->m_y + directionY;
    ...
    pixelsPerStep =
        giPixelsPerStep[(gConfig.walkSpeeds[gbThisNetHumanPlayer[giCurPlayer]])];
    stepDelay =
        giStepDelay[(gConfig.walkSpeeds[gbThisNetHumanPlayer[giCurPlayer]])];
    ...
    if (mapEvent) {
        if (processEvent) {
            if (mapEvent->applyToComputer) {
                for (step = 0; step < (RES_COUNT); ++step) {
...
i32 advManager::ValidMoveWithEvent(
    ...
    dirY = normalDirTable[(direction)].y;
...
i32 advManager::ValidMove(MapDirection direction, i32 eventMode) {
    ...
    if (destinationCell->m_flags & (MAP_CELL_OCCUPIED))
        return 0;
