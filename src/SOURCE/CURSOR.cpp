...
mapCell* advManager::MoveHero(
    ...
    *eventY = movingHero_g->m_y + directionY;
    ...
    if ((((movingHero_g->m_eventFlags) & (HERO_EVENT_EMBARKED)))
    ...
    if ((((destinationCell->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG)))
        ...
        switch (destinationCell->m_triggerType & MAP_TRIGGER_TYPE_MASK) {
            ...
            case MAP_OBJECT_HERO_INTERACTION:
                if ((((movingHero_g->m_eventFlags) & (HERO_EVENT_EMBARKED)))) {
                    if ((((gpGame->GetHero(destinationCell->m_objectMetadata)->m_eventFlags) & (HERO_EVENT_EMBARKED))))
                        goto stoppingEvent;
                    else
                        goto movementDone;
                } else {
    ...
    gpGame->SetVisibility(
        ...
        giCurPlayer,
        giVisRange[(movingHero_g->m_secondarySkills[(HERO_SKILL_SCOUTING)])]
    ...
    *eventY = m_mapOriginY + m_cursorMapY;
    if ((((cursorCell_h->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG)))
        || ((((movingHero_g->m_eventFlags) & (HERO_EVENT_EMBARKED)))
