...
void advManager::StartCursor(MapDirection direction) {
    ...
    directionY = normalDirTable[H2EnumIndex(direction)].y;
...
void advManager::DrawCursor(void) {
    ...
    if (m_cursorFrame & CURSOR_FLIP_FLAG) {
        ...
        if (m_cursorType == HERO_TYPE_BOAT
            ...
            DRAW_FLIPPED_ADVENTURE_ICON(m_heroIcons[CURSOR_BOAT_WAKE_TYPE], drawX, drawY, drawFrame, ICON_DRAW_CLIP);
        }
        DRAW_FLIPPED_ADVENTURE_ICON(m_heroIcons[H2EnumIndex(m_cursorType)], drawX, drawY, drawFrame, ICON_DRAW_CLIP);
        if (m_cursorType == HERO_TYPE_BOAT) {
        ...
        if (m_cursorType == HERO_TYPE_BOAT
            ...
            DRAW_ADVENTURE_ICON(m_heroIcons[CURSOR_BOAT_WAKE_TYPE], drawX, drawY, drawFrame, ICON_DRAW_CLIP);
        }
        DRAW_ADVENTURE_ICON(m_heroIcons[H2EnumIndex(m_cursorType)], drawX, drawY, drawFrame, ICON_DRAW_CLIP);
        if (m_cursorType == HERO_TYPE_BOAT) {
...
void advManager::DrawCursorShadow(void) {
    ...
    if (m_cursorFrame & CURSOR_FLIP_FLAG) {
        ...
        if (m_drawHeroShadows && m_cursorType == HERO_TYPE_BOAT) {
            boatIndex = frame;
            DRAW_ADVENTURE_ICON(m_boatShadowIcon, drawX - CURSOR_SHADOW_FLIP_X_ADJUST, drawY, boatIndex
...
void advManager::TurnTo(MapDirection direction) {
    ...
    } while (frameIndex != H2EnumIndex(direction) * TURN_FRAME_MULTIPLIER);
...
i32 advManager::GetMoveShowIt(
    ...
    i32 dy = normalDirTable[H2EnumIndex(direction)].y;
...
mapCell* advManager::MoveHero(
    ...
    i32 terrainCost;
    i32 stepDelay;
    H2EnumStorage<TerrainType, i32> currentTerrain;
    i32 oldHeroX;
    i32 directionY;
    i32 oldHeroY;
    i32 directionX;
    ...
    *eventY = movingHero->m_y + directionY;
    ...
    if ((H2EnumIndex((destinationCell->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG)))
    ...
    *eventY = m_mapOriginY + m_cursorMapY;
    if ((H2EnumIndex((cursorCell->m_triggerType) & (MAP_TRIGGER_ACTION_FLAG)))
        || ((H2EnumIndex((movingHero->m_eventFlags) & (HERO_EVENT_EMBARKED)))
            && cursorCell->m_triggerType == MAP_OBJECT_COAST)) {
        ...
        if (processEvent) {
            if (mapEvent->applyToComputer) {
                ...
                if (mapEvent->artifact != -1
                    ...
                    GiveArtifact(movingHero, ArtifactType(mapEvent->artifact), 1, -1);
...
i32 advManager::ValidMoveWithEvent(
    ...
    dirX = normalDirTable[H2EnumIndex(direction)].x;
    dirY = normalDirTable[H2EnumIndex(direction)].y;
...
i32 advManager::ValidMove(MapDirection direction, i32 eventMode) {
    ...
    if (CELL_TERRAIN(destinationCell) == TERRAIN_WATER) {
        if (m_cursorType != HERO_TYPE_BOAT
            ...
            return 0;
        if (CELL_TERRAIN(currentCell) == TERRAIN_WATER
            && directionX != 0 && directionY != 0) {
            if (giGroundToTerrain[m_mapData->GetCell(centerX + directionX, centerY)
                                      ->m_terrainImageIndex]
                    != TERRAIN_WATER
                || giGroundToTerrain[m_mapData->GetCell(centerX, centerY + directionY)
                                         ->m_terrainImageIndex]
                       != TERRAIN_WATER)
                return 0;
        }
