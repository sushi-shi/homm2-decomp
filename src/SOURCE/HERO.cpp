...
typedef enum HeroUiConstant {
    UI_DIALOG_SPLIT               = DIALOG_BUTTON_2,
    UI_DISMISS                    = DIALOG_BUTTON_3,
    UI_CLOSE                      = DIALOG_BUTTON_0,
    ...
    if (IsEmbarked()) {
        ...
        for (creatureIndex = 0; creatureIndex < ARMY_GROUP_SLOT_COUNT; creatureIndex++) {
            if (m_army.m_creatureTypes[creatureIndex] != CREATURE_NONE
                && gMonsterDatabase[(m_army.m_creatureTypes[creatureIndex])].speed
                       < slowestSpeedValue) {
                slowestSpeedValue =
                    gMonsterDatabase[(m_army.m_creatureTypes[creatureIndex])].speed;
            }
        ...
        movePoints = static_cast<i32>(
            movePoints * gfSSLogisticsMod[(m_secondarySkills[(HERO_SKILL_LOGISTICS)])]
        );
...
i32 hero::GetNumSpells(HeroSpellType type) {
    ...
    for (spell = SPELL_FIREBALL; spell < SPELL_COUNT; spell++) {
        if (HasSpell(spell)) {
            if ((((gsSpellInfo[(spell)].attributes) & (SPELL_INFO_ATTRIBUTE_COMBAT))))
                numCombatSpells++;
            else
                numAdventureSpells++;
        }
...
void HeroMessageUpdate(const char* text) {
    ...
    SET_WIDGET_MESSAGE(message, WIDGET_COMMAND_SET_TEXT, UI_STATUS_TEXT_WIDGET);
    message.payload.widget.data.text = text;
    gheroWin->BroadcastMessage(message);
    gheroWin->DrawWindow(WINDOW_DRAW_BUFFER_ONLY, UI_PREVIOUS_HERO, UI_STATUS_TEXT_WIDGET);
    gpWindowManager
    ...
    for (slot = 0; slot < ARMY_GROUP_SLOT_COUNT; slot++) {
        if (m_army.m_creatureTypes[slot] == CREATURE_NONE) {
            ...
            message.payload.widget.data.value =
                (gMonsterDatabase[(m_army.m_creatureTypes[slot])].race)
...
void hero::Deallocate(i32 updateMap) {
    ...
    m_eventFlags =
        HeroEventFlag(static_cast<i32>(m_eventFlags) & ~(HERO_EVENT_GROUPED_FORMATION));
        ...
        if (!gbInNewGameSetup && m_owner >= 0 && gbThisNetHumanPlayer[(m_owner)]) {
            ...
            if (choices[0] == HERO_SKILL_NONE) {
                ...
                NormalDialog(
                    ...
                    (choices[0]) * HERO_SECONDARY_SKILL_ICON_STRIDE
                        + (m_secondarySkills[(choices[0])]),
                    -1,
                    0,
                    -1,
                    0
                ...
                sprintf(
                    ...
                    gSecondarySkillLevels[(m_secondarySkills[(choices[1])])]
                );
                strcat(gText, text);
                NormalDialog(
                    gText,
                    ...
                    (choices[0]) * HERO_SECONDARY_SKILL_ICON_STRIDE
                        + (m_secondarySkills[(choices[0])]),
                    NORMAL_DIALOG_SECONDARY_SKILL,
                    (choices[1]) * HERO_SECONDARY_SKILL_ICON_STRIDE
                        + (m_secondarySkills[(choices[1])]),
                    -1,
                    0
                );
            ...
            if (choices[0] != HERO_SKILL_NONE) {
                if (choices[1] != HERO_SKILL_NONE) {
                    if (gSSValues[(choices[0])][0]
                        > gSSValues[(choices[1])][0]) {
                        GiveSS(choices[0], HERO_SKILL_LEVEL_BASIC);
                    } else {
                        GiveSS(choices[1], HERO_SKILL_LEVEL_BASIC);
                    }
    ...
    switch (message.payload.widget.id) {
        ...
        case UI_MORALE_LAST:
            if (gpHVHero->m_army.GetMorale(gpHVHero, gpHVHero->GetOccupiedTown(), NULL) > 0)
                sprintf(gText, cHeroScreen[(TEXT_GOOD_MORALE)]);
            else if (gpHVHero->m_army.GetMorale(gpHVHero, gpHVHero->GetOccupiedTown(), NULL) == 0)
                sprintf(gText, cHeroScreen[(TEXT_NEUTRAL_MORALE)]);
            else
                sprintf(gText, cHeroScreen[(TEXT_BAD_MORALE)]);
            break;
        ...
        case UI_LUCK_LAST:
            if (gpGame->GetLuck(gpHVHero, NULL, gpHVHero->GetOccupiedTown()) > 0)
                sprintf(gText, cHeroScreen[(TEXT_GOOD_LUCK)]);
            else if (gpGame->GetLuck(gpHVHero, NULL, gpHVHero->GetOccupiedTown()) == 0)
                sprintf(gText, cHeroScreen[(TEXT_NEUTRAL_LUCK)]);
            else
                sprintf(gText, cHeroScreen[(TEXT_BAD_LUCK)]);
            break;

        case UI_EXPERIENCE_FIRST:
        case UI_EXPERIENCE_LAST:
            sprintf(gText, cHeroScreen[(TEXT_EXPERIENCE)]);
            break;

        case UI_SPELL_POINTS_FIRST:
        case UI_SPELL_POINTS_LAST:
            sprintf(gText, cHeroScreen[(TEXT_SPELL_POINTS)]);
            break;

        case UI_FORMATION_SPREAD:
            sprintf(gText, cHeroScreen[(TEXT_SPREAD_FORMATION)]);
            break;

        case UI_FORMATION_GROUPED:
            sprintf(gText, cHeroScreen[(TEXT_GROUPED_FORMATION)]);
            break;
        ...
        case UI_ARMY_SELECTOR_SLOT_4:
            ...
            if (giHeroScreenSrcIndex == UI_ARMY_SELECTION_NONE) {
                if (gpHVHero->m_army.m_creatureTypes[armySlot] != CREATURE_NONE)
                    sprintf(
                        gText,
                        cHeroScreen[(TEXT_SELECT_ARMY)],
                        gArmyNamesPlural[(gpHVHero->m_army.m_creatureTypes[armySlot])]
                    );
                else
                    strcpy(gText, cHeroScreen[(TEXT_EMPTY)]);
                ...
                if ((((message.payload.widget.modifiers) & (MESSAGE_MODIFIER_SHIFT_KEYS))))
                    sprintf(
                        gText,
                        cHeroScreen[(TEXT_SPLIT_ARMY)],
                        gArmyNamesPlural[(gpHVHero->m_army
                                                 .m_creatureTypes[giHeroScreenSrcIndex])]
                    );
                else
                    sprintf(
                        gText,
                        cHeroScreen[(TEXT_MOVE_ARMY)],
                ...
                sprintf(
                    gText,
                    cHeroScreen[(TEXT_COMBINE_ARMIES)],
                    gArmyNamesPlural[(gpHVHero->m_army.m_creatureTypes[armySlot])]
                );
            } else {
                sprintf(
                    gText,
                    cHeroScreen[(TEXT_EXCHANGE_ARMIES)],
                    gArmyNamesPlural[(gpHVHero->m_army.m_creatureTypes[giHeroScreenSrcIndex])],
                    gArmyNamesPlural[(gpHVHero->m_army.m_creatureTypes[armySlot])]
                );
            }
        ...
        case UI_ARTIFACT_SLOT_13:
            if (gpHVHero->m_artifacts[message.payload.widget.id - UI_ARTIFACT_FIRST]
                == ARTIFACT_NONE)
                sprintf(gText, cHeroScreen[(TEXT_EMPTY)]);
            else if (gpHVHero->m_artifacts[message.payload.widget.id - UI_ARTIFACT_FIRST]
                     == ARTIFACT_MAGIC_BOOK)
                strcpy(gText, cHeroScreen[(TEXT_VIEW_SPELLS)]);
            else
                sprintf(
                    gText,
                    cHeroScreen[(TEXT_ARTIFACT)],
                    gArtifactNames
        ...
        case UI_CLOSE:
            strcpy(gText, cHeroScreen[(TEXT_EXIT)]);
            break;
        ...
        default:
            ...
            secondary_skill_text:
                if (secondarySkillSlot < gpHVHero->m_secondarySkillCount) {
                    sprintf(
                        gText,
                        cHeroScreen[(TEXT_SECONDARY_SKILL)],
                        gSecondarySkillLevels[(gpHVHero->m_secondarySkills
            ...
            strcpy(gText, cHeroScreen[(TEXT_SCREEN)]);
            break;
    }
...
MessageDispatchResult HeroHandler(struct tag_message& message) {
    ...
    i32 iHero;
    i32 secondarySkillSlot;
    tag_message newMessage [[maybe_unused]];
    i32 j [[maybe_unused]];
    i32 nextExperience;
                    ...
                    case UI_ARMY_SELECTOR_SLOT_4:
                        ...
                        if (quickView == 0 && giHeroScreenSrcIndex == UI_ARMY_SELECTION_NONE) {
                            ...
                            if (quickView == 0 && gpTownManager->m_castleDialogActive != 0) {
                                ...
                                temporary = (gpHVHero->m_army.m_creatureTypes[armySlot]);
                                if ((((message.payload.widget.modifiers) & (MESSAGE_MODIFIER_SHIFT_KEYS)))
                                    && (gpHVHero->m_army.m_creatureTypes[armySlot] == CREATURE_NONE
                                        || gpHVHero->m_army.m_creatureTypes[armySlot]
                                               == gpHVHero->m_army
...
i32 HeroView(i32 heroId, b32 noDismiss, b32 fadeAlreadyOut) {
    ...
    if (gpWindowManager->m_dialogResult == UI_DISMISS) {
...
void SetupHeroView(void) {
    ...
    message.payload.widget.data.value = (WIDGET_FLAG_DRAW);
    for (i = 0; i < ARMY_GROUP_SLOT_COUNT; i++) {
        message.payload.widget.id = UI_PRIMARY_STAT_FIRST + i;
        heroWin->BroadcastMessage(message);
        message.payload.widget.id = UI_ARMY_SELECTOR_FIRST + i;
        heroWin->BroadcastMessage(message);
    }
    ...
    for (i = 0; i < HERO_PRIMARY_STAT_COUNT; i++) {
        ...
        message.payload.widget.id = UI_PRIMARY_STAT_VALUE_FIRST + i;
        message.payload.widget.data.text = gText;
        heroWin->BroadcastMessage(message);
    }
    ...
    for (i = 0; i < UI_MODIFIER_ICON_COUNT; i++) {
        message.payload.widget.command =
            i >= magnitude ? WIDGET_COMMAND_CLEAR_FLAGS : WIDGET_COMMAND_SET_FLAGS;
        if (i == 1 && heroLuck != 0)
            displayIndex = 0;
    ...
    for (i = 0; i < UI_MODIFIER_ICON_COUNT; i++) {
        message.payload.widget.command = WIDGET_COMMAND_SET_FRAME;
        message.payload.widget.id = UI_LUCK_FIRST + i;
        if (heroLuck < 0)
    ...
    for (i = 0; i < UI_MODIFIER_ICON_COUNT; i++) {
        message.payload.widget.command = WIDGET_COMMAND_SET_FRAME;
        message.payload.widget.id = UI_MORALE_FIRST + i;
        if (moraleValue < 0)
    ...
    message.payload.widget.command = (((gpHVHero->m_eventFlags) & (HERO_EVENT_GROUPED_FORMATION)))
    ...
    for (i = 0; i < HERO_SECONDARY_SKILL_CAPACITY; i++) {
        if (i < gpHVHero->m_secondarySkillCount) {
            ...
            message.payload.widget.id = UI_SECONDARY_SKILL_ROW1_FIRST + i;
            message.payload.widget.command = WIDGET_COMMAND_SET_FRAME;
            message.payload.widget.data.value = (skill) + 1;
            heroWin->BroadcastMessage(message);
            message.payload.widget.command = WIDGET_COMMAND_SET_FLAGS;
            message.payload.widget.id = UI_SECONDARY_SKILL_ROW2_FIRST + i;
            message.payload.widget.data.value = (WIDGET_FLAG_ENABLED | WIDGET_FLAG_DRAW);
            heroWin->BroadcastMessage(message);
            message.payload.widget.command = WIDGET_COMMAND_SET_FLAGS;
            message.payload.widget.id = UI_SECONDARY_SKILL_ROW3_FIRST + i;
            message.payload.widget.data.value = (WIDGET_FLAG_ENABLED | WIDGET_FLAG_DRAW);
            heroWin->BroadcastMessage(message);
            message.payload.widget.command = WIDGET_COMMAND_SET_TEXT;
            message.payload.widget.id = UI_SECONDARY_SKILL_ROW2_FIRST + i;
            message.payload.widget.data.text = gSecondarySkills[(skill)];
            heroWin->BroadcastMessage(message);
            message.payload.widget.command = WIDGET_COMMAND_SET_TEXT;
            message.payload.widget.id = UI_SECONDARY_SKILL_ROW3_FIRST + i;
            tempBonus = gpHVHero->GetSSLevel(skill)
                                  - (gpHVHero->m_secondarySkills[(skill)]);
            if (tempBonus > 0) {
                sprintf(
                    gText,
                    "%s+%d",
                    gSecondarySkillLevels
                        [(gpHVHero->m_secondarySkills[(skill)]) - 1],
                    tempBonus
                );
            } else {
                sprintf(
                    gText,
                    "%s",
                    gSecondarySkillLevels[(gpHVHero->m_secondarySkills[(skill)]) - 1]
                );
            }
            message.payload.widget.data.text = gText;
            heroWin->BroadcastMessage(message);
    ...
    for (i = 0; i < HERO_ARTIFACT_SLOT_COUNT; i++) {
        message.payload.widget.id = UI_ARTIFACT_FIRST + i;
        if (gpHVHero->m_artifacts[i] != ARTIFACT_NONE) {
            message.payload.widget.command = WIDGET_COMMAND_SET_FLAGS;
            message.payload.widget.data.value = (WIDGET_FLAG_ENABLED);
            heroWin->BroadcastMessage(message);
            message.payload.widget.command = WIDGET_COMMAND_SET_FRAME;
            message.payload.widget.data.value = (gpHVHero->m_artifacts[i]) + 1;
            heroWin->BroadcastMessage(message);
        } else {
            message.payload.widget.command = WIDGET_COMMAND_SET_FRAME;
    ...
    statusMessage.type = MESSAGE_WIDGET;
...
i8 hero::GetSSLevel(HeroSecondarySkill skill) {
    ...
    ssLevel = static_cast<i8>(m_secondarySkills[(skill)]);
    if (skill != HERO_SKILL_NECROMANCY)
        return (ssLevel);
    if (ssLevel == (HERO_SKILL_LEVEL_NONE))
        return (ssLevel);
...
void hero::DoSSLevelDialog(HeroSecondarySkill skill, i32 quickView) {
    ...
    NormalDialog(
        ...
        NORMAL_DIALOG_SECONDARY_SKILL,
        (skill) * HERO_SECONDARY_SKILL_ICON_STRIDE + (m_secondarySkills[(skill)])
