...
void hero::Deallocate(i32 updateMap) {
    ...
    if (gbRetreatWin) {
        ...
        m_eventFlags = HeroEventFlag(
            static_cast<i32>(m_eventFlags) | (HERO_EVENT_WEEKLY_VISIT)
        );
    }

    m_eventFlags = HeroEventFlag(
        static_cast<i32>(m_eventFlags) & ~(HERO_EVENT_GROUPED_FORMATION)
    );
            ...
            for (indexValue = 0; indexValue < HERO_SECONDARY_SKILL_CHOICE_COUNT; indexValue++) {
                ...
                if (indexValue == 0 && m_cursorType != FACTION_BARBARIAN
                    ...
                    while (attempts < HERO_SECONDARY_SKILL_SEARCH_LIMIT) {
                        ...
                        skillIndexValue =
                            HeroSecondarySkill(((skillIndexValue) + 1) % (HERO_SKILL_COUNT));
                    }
                nextAttempt:
                    attempts--;
                }
        ...
        if (index < gpHVHero->m_secondarySkillCount) {
            ...
            if (secondarySkillBonus > 0) {
                sprintf(
                    ...
                    "%s+%d",
                    gSecondarySkillLevels
                        [(gpHVHero->m_secondarySkills[(secondarySkill)]) - 1],
                    secondarySkillBonus
                );
            } else {
                sprintf(
...
i8 hero::GetSSLevel(HeroSecondarySkill skill) {
    ...
    baseLevel = static_cast<i8>(m_secondarySkills[(skill)]);
