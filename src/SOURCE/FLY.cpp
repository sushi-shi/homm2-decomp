...
i32 army::ValidFlight(i32 destination, ArmyPathTarget pathMode) {
    ...
    armyPointer = &gpCombatManager->m_armies[(m_targetSide)][m_targetIndex];
    ...
    while (attackMask != ARMY_ALL_ATTACK_DIRECTIONS) {
        ...
        if (ValidAttack(
            ...
            attackMask |= 1 << (moveDirection);
        }
    }

    directionMask = 0;
    if ((((armyPointer->m_monster.flags.all) & (MONSTER_FLAGS_WIDE)))
        && pathMode == ARMY_PATH_ANY_TARGET_HEX) {
        enemyHex += armyPointer->m_facing == ARMY_FACING_RIGHT ? 1 : -1;
        directionMask = armyPointer->m_facing == ARMY_FACING_RIGHT ? (1 << (COMBAT_DIRECTION_WEST))
    ...
    while (directionMask != ALL_ADJACENT_DIRECTIONS) {
        ...
        if (ValidHex(adjHex)
            ...
            if (!(((m_monster.flags.all) & (MONSTER_FLAGS_WIDE)))) {
                ...
                for (currentDirection = COMBAT_DIRECTION_NORTHEAST; (currentDirection) < ARMY_COMBAT_DIRECTION_COUNT; currentDirection++) {
                    if (attackMask & (1 << (currentDirection))) {
    ...
    if ((((armyPointer->m_monster.flags.all) & (MONSTER_FLAGS_WIDE)))
        && pathMode == ARMY_PATH_ANY_TARGET_HEX) {
        enemyHex += armyPointer->m_facing == ARMY_FACING_RIGHT ? -1 : 1;
        directionMask = armyPointer->m_facing == ARMY_FACING_RIGHT ? (1 << (COMBAT_DIRECTION_EAST))
...
i32 army::FlyTo(i32 destination) {
    ...
    if (!gbNoShowCombat) {
        ...
        for (leg = 0; leg < stepCount; leg++) {
            ...
            if (stepCount == 0) {
                frameCount = m_frameInfo.animationFrameCount[(ARMY_ANIMATION_WALK)];
                frameStart = 0;
            } else {
                if (leg > 0) {
                    frameCount +=
                ...
                frameCount = m_frameInfo.animationFrameCount[(ARMY_ANIMATION_WALK_MIDDLE)];
                midCount = frameCount;
                if (leg + 1 < stepCount) {
                    frameCount += m_frameInfo.animationFrameCount[(ARMY_ANIMATION_WALK_END)];
                }
            }
