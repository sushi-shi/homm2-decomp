...
i32 army::ValidFlight(i32 destination, ArmyPathTarget pathMode) {
    ...
    directionMask = 0;
    if ((((armyPointer->m_monster.attributes) & (MONSTER_FLAGS_WIDE)))
    ...
    while (directionMask != ALL_ADJACENT_DIRECTIONS) {
        ...
        if (ValidHex(adjHex)
            ...
            if (!(((m_monster.attributes) & (MONSTER_FLAGS_WIDE)))) {
                ...
                for (currentDirection = COMBAT_DIRECTION_NORTHEAST; (currentDirection) < (COMBAT_DIRECTION_COUNT); currentDirection++) {
