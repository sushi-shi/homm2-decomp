...
typedef enum OverviewStorageConstant {
    ...
    OVERVIEW_MINE_COUNT_CAPACITY   = (RES_COUNT),
...
void game::SetupDynamicStuff(i32 redraw, i32 updateKnob, i32 forceUpdate) {
    ...
    if (updateKnob) {
        if (giOverviewItems[(giOverviewType)] <= OVERVIEW_VISIBLE_ROWS) {
            ...
            OVScrollKnob->m_y = (giOverviewTop[(giOverviewType)] * pixelsPerItem + OVERVIEW_SCROLL_MIN_Y_FLOAT
    ...
    for (rowIndex = 0; rowIndex < OVERVIEW_VISIBLE_ROWS; rowIndex++) {
        ...
        OVERVIEW_ICON_WIDGET(rowIndex, icons) = new iconWidget(
            ...
            "overview.icn",
            (giOverviewType) + ROW_BACKGROUND_FRAME_BASE,
            ICON_DRAW_NORMAL,
        ...
        if (giOverviewType == OVERVIEW_TOWNS) {
            ...
            if (heroData != NULL || capt) {
                ...
                OVERVIEW_ICON_WIDGET(rowIndex, icons) = new iconWidget(
                    ...
                    capt ? (record->m_type) : (heroData->m_portrait),
                    ICON_DRAW_NORMAL,
...
void game::SetupNewOverviewType(OverviewType overviewType, i32 redrawFrom) {
    ...
    for (column = 0; column < OVERVIEW_TITLE_COUNT; column++) {
        ALLOC_COPY_STRING(titleText, cOverviewText[column + (giOverviewType) * OVERVIEW_TITLE_COUNT]);
        textWidgetTitle[column] = new textWidget(
            titleX[(giOverviewType)][column],
            OVERVIEW_TITLE_Y,
            titleWidths[(giOverviewType)][column],
...
void game::SetupResources(void) {
    ...
    for (resourceIndex = RES_WOOD; resourceIndex < RES_COUNT; resourceIndex++) {
        ...
        sprintf(gText, "%d", gpCurPlayer->m_resources[(resourceIndex)]);
        message.payload.widget.id = (resourceIndex) + RESOURCE_FIRST_WIDGET;
        overWin->BroadcastMessage(message);
    }
}
        ...
        while (widgetMessage.type != MESSAGE_LEFT_BUTTON_UP
            ...
            if (widgetMessage.type == MESSAGE_MOUSE_MOVE) {
                ...
                if ((widgetMessage.payload.mouse.y)
                    ...
                    widgetMessage.payload.mouse.y = (topValue + scrollRange - (SCROLL_LAST_PIXEL_ADJUST));
                }
