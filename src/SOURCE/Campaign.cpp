...
void game::InitCampaignMap(void) {
    ...
    if (m_campaignType != m_campaignStartingSide && iCurViewMap == CAMPAIGN_SWITCHING_SCENARIO) {
        choiceBest1 =
            &campaignChoices[(iCurViewSide)][CAMPAIGN_SWITCHING_MAP][selectedChoicePosition0];
    } else {
        choiceBest1 =
            &campaignChoices[(m_campaignType)][m_campaignScenario][selectedChoicePosition0];
    }
    ...
    switch (choiceBest1->type) {
        case CAMPAIGN_CHOICE_RESOURCE:
            m_players[0].m_resources[(choiceBest1->resource)] += choiceBest1->amount;
            break;
        case CAMPAIGN_CHOICE_ARTIFACT:
