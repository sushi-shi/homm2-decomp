...
void game::UpdateNewGameWindow(void) {
    ...
    for (playerIndex3 = 0; playerIndex3 < (DIFFICULTY_COUNT); ++playerIndex3) {
    ...
    messageTemp.payload.widget.id =
        NEW_GAME_DIFFICULTY_FIRST + (m_difficulty);
    m_newGameWindow->BroadcastMessage(messageTemp);
    ...
    if ((((message.payload.widget.modifiers) & (MESSAGE_MODIFIER_RIGHT_BUTTON)))) {
        if (message.payload.widget.command == NEW_GAME_EVENT_PRESS
            ...
            if ((message.payload.widget.id >= NEW_GAME_DIFFICULTY_HELP_FIRST
                 && message.payload.widget.id <= NEW_GAME_DIFFICULTY_HELP_FIRST + (DIFFICULTY_COUNT) - 1)
                || (message.payload.widget.id >= NEW_GAME_DIFFICULTY_FIRST
                    && message.payload.widget.id <= NEW_GAME_DIFFICULTY_FIRST + (DIFFICULTY_COUNT) - 1))
                helpDialogIndexLocal = GAME_HELP_DIFFICULTY;
            if ((message.payload.widget.id >= NEW_GAME_HANDICAP_FIRST
                 && message.payload.widget.id <= NEW_GAME_HANDICAP_FIRST + (GAME_PLAYER_CONTROL_COUNT) - 1)
                || (message.payload.widget.id >= NEW_GAME_PLAYER_HUMAN_FIRST
                    && message.payload.widget.id <= NEW_GAME_PLAYER_HUMAN_FIRST + (GAME_PLAYER_CONTROL_COUNT) - 1))
            ...
            if ((message.payload.widget.id >= NEW_GAME_COLOR_FIRST
                ...
                || (message.payload.widget.id >= NEW_GAME_RACE_FIRST
                    && message.payload.widget.id <= NEW_GAME_RACE_FIRST + (GAME_PLAYER_CONTROL_COUNT) - 1)
            ...
            if ((message.payload.widget.id >= NEW_GAME_RACE_CYCLE_FIRST
                 && message.payload.widget.id <= NEW_GAME_RACE_CYCLE_FIRST + (GAME_PLAYER_CONTROL_COUNT) - 1)
                || (message.payload.widget.id >= NEW_GAME_RACE_ICON_FIRST
                    && message.payload.widget.id <= NEW_GAME_RACE_ICON_FIRST + (GAME_PLAYER_CONTROL_COUNT) - 1))
