...
static const H2EnumStorage<BuildingSlotType, i8> gTownObjectOrder[H2EnumIndex(FACTION_COUNT)][H2EnumIndex(BUILDING_SLOT_COUNT)] = {
...
SBuildingInfo sBuildingInfo[H2EnumIndex(FACTION_COUNT)][H2EnumIndex(BUILDING_SLOT_COUNT)] = {
...
void townObject::Draw(i32 advanceAnimation) {
    ...
    if (m_buildingId == TOWN_OBJECT_KNIGHT_LEFT_OVERLAY
        ...
        && (!(H2EnumIndex((gpTownManager->m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_TAVERN))))
            || (!(H2EnumIndex((gpTownManager->m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_MAGE_GUILD))))
                && !(H2EnumIndex((gpTownManager->m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_DWELLING_6))))
                && !(H2EnumIndex((gpTownManager->m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_UPGRADED_DWELLING_6)))))))
        return;
    ...
    if (m_buildingId == TOWN_OBJECT_KNIGHT_RIGHT_OVERLAY
        ...
        && (!(H2EnumIndex((gpTownManager->m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_MAGE_GUILD))))
            || !(H2EnumIndex((gpTownManager->m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_COLISEUM))))))
        return;
    if (m_buildingId == TOWN_OBJECT_BARBARIAN_OVERLAY
        && gpTownManager->m_town->m_type == FACTION_BARBARIAN
        && (!(H2EnumIndex((gpTownManager->m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_THIEVES_GUILD))))
            || !(H2EnumIndex((gpTownManager->m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_DWELLING_3))))))
        return;
    if (gpTownManager->m_town->m_type == FACTION_SORCERESS
        && m_buildingId == TOWN_OBJECT_KNIGHT_LEFT_OVERLAY
        && (!(H2EnumIndex((gpTownManager->m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_CAPTAIN))))
...
void townManager::SetupExtraStuff(void) {
    ...
    if ((m_town->m_type == FACTION_WARLOCK || m_town->m_type == FACTION_KNIGHT
        ...
        && m_town->CanBuildDock())
        m_town->m_buildings |= H2EnumIndex(TOWN_EXTRA_RACE_FIRST_MASK);
    if ((H2EnumIndex((m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_DOCK))))
...
void townManager::SetupTown(void) {
        ...
        for (i = 0; i < H2EnumIndex(BUILDING_SLOT_COUNT); ++i) {
            ...
            if (buildId != TOWN_OBJECT_NONE) {
                ...
                if (m_townObjects[m_townObjectCount]->m_border != NULL) {
                    if (!(H2EnumIndex((m_town->m_buildings) & ((1 << H2EnumIndex(buildId)))))) {
    ...
    if (m_town->m_occupyingHeroId != TOWN_OCCUPYING_HERO_NONE) {
        ...
        if (m_heroStrip == NULL)
            MemError();
        if ((H2EnumIndex((m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_MAGE_GUILD)))))
            m_town->GiveSpells(NULL);
    } else if ((H2EnumIndex((m_town->m_buildings) & (H2EnumIndex(TOWN_BUILDING_CAPTAIN_QUARTERS))))) {
...
void townManager::SetCommandAndText(struct tag_message& message) {
    ...
    switch (objectId) {
        ...
        case H2EnumIndex(TOWN_OBJECT_MAGE_GUILD):
            ...
            break;
        case H2EnumIndex(TOWN_OBJECT_THIEVES_GUILD):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_THIEVES_GUILD)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_TAVERN):
            ...
            break;
        case H2EnumIndex(TOWN_OBJECT_DOCK):
        case H2EnumIndex(TOWN_OBJECT_BOAT):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_DOCK)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_WELL):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_WELL)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_TENT):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_TENT)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_CASTLE):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_CASTLE)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_STATUE):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_STATUE)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_LEFT_TURRET):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_LEFT_TURRET)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_RIGHT_TURRET):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_RIGHT_TURRET)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_MOAT):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_MOAT)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_MARKETPLACE):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_MARKETPLACE)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_CAPTAIN_QUARTERS):
            strcpy(m_statusText, cTownCommand[H2EnumIndex(TEXT_CAPTAIN_QUARTERS)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_SPECIAL_BUILDING):
            strcpy(m_statusText, gSpecialBuildingNames[H2EnumIndex(m_town->m_type)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_SECOND_WELL):
            strcpy(m_statusText, gWellExtraNames[H2EnumIndex(m_town->m_type)]);
            break;
        case H2EnumIndex(TOWN_OBJECT_DWELLING_1):
        case H2EnumIndex(TOWN_OBJECT_DWELLING_2):
        case H2EnumIndex(TOWN_OBJECT_DWELLING_3):
        case H2EnumIndex(TOWN_OBJECT_DWELLING_4):
        case H2EnumIndex(TOWN_OBJECT_DWELLING_5):
        case H2EnumIndex(TOWN_OBJECT_DWELLING_6):
        case H2EnumIndex(TOWN_OBJECT_UPGRADED_DWELLING_2):
        case H2EnumIndex(TOWN_OBJECT_UPGRADED_DWELLING_3):
        case H2EnumIndex(TOWN_OBJECT_UPGRADED_DWELLING_4):
        case H2EnumIndex(TOWN_OBJECT_UPGRADED_DWELLING_5):
        case H2EnumIndex(TOWN_OBJECT_UPGRADED_DWELLING_6):
        case H2EnumIndex(TOWN_OBJECT_ALTERNATE_UPGRADED_DWELLING_6):
            sprintf(
                m_statusText,
            ...
            switch (message.payload.widget.command) {
                ...
                case WIDGET_NOTIFY_RIGHT_CLICK: {
                    switch (message.payload.widget.id) {
                        ...
                        case H2EnumIndex(TOWN_OBJECT_CASTLE_UPGRADE):
                            if (quickView) {
                                goto showBuildingInformation;
                            }
                            {
                        ...
                        case H2EnumIndex(TOWN_OBJECT_MARKETPLACE):
                            ...
                            }
                            {
                                ...
                                for (loop = 0; loop < gpCurPlayer->m_townCount; ++loop) {
                                    if ((H2EnumIndex((gpGame->GetTown(gpCurPlayer->m_townIds[loop])
                                                ->m_buildings) & (H2EnumIndex(TOWN_BUILDING_MARKETPLACE)))))
                                        ++tradeCount;
                                }
...
void townManager::Toggle(BuildingSlotType building) {
    ...
    if ((H2EnumIndex((m_town->m_buildings) & ((1 << H2EnumIndex(building)))))) {
        for (index = 0; index < m_townObjectCount; ++index) {
...
i32 townManager::BuyBuild(
    ...
    sprintf(iconName, "cstl%s.icn", cHeroTypeShortName[H2EnumIndex(m_town->m_type)]);
...
void townManager::BuildObj(BuildingSlotType building) {
    ...
    }
    {
        ...
        for (index = 0; index < m_townObjectCount; ++index) {
            if ((H2EnumIndex((m_town->m_buildings) & ((1 << H2EnumIndex(m_townObjects[index]->m_buildingId)))))) {
                m_townObjects[index]->m_visible = 1;
...
void townManager::SetupWell(heroWindow* window) {
    ...
    for (dwellingResult = 0; dwellingResult < TOWN_WELL_DWELLING_COUNT; ++dwellingResult) {
        ...
        if ((H2EnumIndex((m_town->m_buildings) & ((1L << (dwellingTypes[dwellingResult] + H2EnumIndex(BUILDING_SLOT_DWELLING_FIRST))))))) {
        ...
        if (gpGame->m_playerDead[player]) {
            ...
            switch (category) {
                case THIEVES_CATEGORY_TOWNS:
                    for (townIndex = 0; townIndex < GAME_TOWN_COUNT; ++townIndex) {
                        if (gpGame->m_castleRecs[townIndex].m_owner == player
                            && (H2EnumIndex((gpGame->m_castleRecs[townIndex].m_buildings) & (H2EnumIndex(TOWN_BUILDING_TENT))))) {
                            ++townCount;
                        }
                    }
                ...
                case THIEVES_CATEGORY_CASTLES:
                    for (townIndex = 0; townIndex < GAME_TOWN_COUNT; ++townIndex) {
                        if (gpGame->m_castleRecs[townIndex].m_owner == player
                            && (H2EnumIndex((gpGame->m_castleRecs[townIndex].m_buildings) & (H2EnumIndex(TOWN_BUILDING_CASTLE))))) {
                            ++castleCount;
                        }
                    }
