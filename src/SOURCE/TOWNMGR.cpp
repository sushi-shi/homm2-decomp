...
void townManager::SetupTown(void) {
    ...
    if (m_town->m_type != m_lastTownType) {
        ...
        for (index = 0; index < TOWN_BUILDING_COUNT; ++index) {
            buildId = gTownObjectOrder[(m_town->m_type)][index];
        ...
        for (index = 0; index < TOWN_BUILDING_COUNT; ++index) {
            i32 existingObjectId =
                gTownObjectOrder[(m_town->m_type)][index];
            if (existingObjectId != TOWN_OBJECT_NONE) {
                if (m_townObjects[m_townObjectCount]->m_border != NULL) {
                    if (!(m_town->m_buildings & (1 << (existingObjectId)))) {
                        m_townObjects[m_townObjectCount]->m_border->m_flags &=
                            ~WIDGET_FLAG_ENABLED;
                        m_townObjects[m_townObjectCount]->m_visible = 0;
                    } else {
    ...
    switch (static_cast<TownManagerWidgetId>(objectId)) {
        ...
        case TOWN_WIDGET_BUILDING_ALTERNATE_DWELLING_6:
            sprintf(
                ...
                gArmyNames[(gDwellingType[(m_town->m_type)][objectId
                                                       - (TOWN_OBJECT_DWELLING_1)])]
            );
            break;
    }
...
MessageDispatchResult townManager::Main(tag_message& message) {
    ...
    i32 exitTown_i = 0;
    i32 quickView_k = ((((message.payload.widget.modifiers) & (MESSAGE_MODIFIER_RIGHT_BUTTON)))) != 0;
    i32 debugBuilding_e;
    i32 index_i;
    ...
    if (giDebugBuildingToBuild != -1) {
        ...
        if (debugBuilding_e == TOWN_DEBUG_BUILD_ALL) {
            for (BuildingSlotType building = BUILDING_SLOT_MAGE_GUILD;
                 (building) < TOWN_BUILDING_COUNT;
                 ++building) {
                if ((gTownEligibleBuildMask[(m_town->m_type)] & (1 << (building)))
                    || building == BUILDING_SLOT_CASTLE)
                    BuildObj(building);
            }
        } else {
            if ((gTownEligibleBuildMask[(m_town->m_type)]
                 & (1 << (static_cast<BuildingSlotType>(debugBuilding_e))))
                || debugBuilding_e == (BUILDING_SLOT_CASTLE))
    ...
    if (building == BUILDING_SLOT_TAVERN && m_town->m_type == FACTION_NECROMANCER) {
        ...
        for (index_h = 0; index_h < TOWN_RESOURCE_COUNT; ++index_h) {
            if (gSpecialBuildingCosts[(gpTownManager->m_town->m_type)][index_h] > 0) {
                resourceTypes_o[costCount_o] = static_cast<i8>(index_h);
                costs_e[costCount_o] =
                    static_cast<i16>(gSpecialBuildingCosts[(gpTownManager->m_town->m_type)][index_h]);
                ++costCount_o;
            }
        }
        ...
        for (index_h = 0; index_h < TOWN_RESOURCE_COUNT; ++index_h) {
            if (gNeutralBuildingCosts[(building)][index_h] > 0) {
                resourceTypes_o[costCount_o] = static_cast<i8>(index_h);
                costs_e[costCount_o] =
                    static_cast<i16>(gNeutralBuildingCosts[(building)][index_h]);
                ++costCount_o;
            }
        }
    } else {
        for (index_h = 0; index_h < TOWN_RESOURCE_COUNT; ++index_h) {
            if (gDwellingCosts[(gpTownManager->m_town->m_type)][dwelling_k][index_h] > 0) {
                resourceTypes_o[costCount_o] = static_cast<i8>(index_h);
                costs_e[costCount_o] = static_cast<i16>(
                    gDwellingCosts[(gpTownManager->m_town->m_type)][dwelling_k][index_h]
                );
                ++costCount_o;
            }
        }
...
void townManager::BuildObj(BuildingSlotType building) {
    ...
    }
    {
        ...
        for (index_j = 0; index_j < m_townObjectCount; ++index_j) {
            if (m_town->m_buildings & (1 << (m_townObjects[index_j]->m_buildingId))) {
...
MessageDispatchResult MageGuildHandler(tag_message& message) {
    ...
    if (message.type == MESSAGE_WIDGET) {
        switch (message.payload.widget.command) {
            case WIDGET_COMMAND_SELECT:
            case WIDGET_COMMAND_ALTERNATE_SELECT:
                quickView_f = (((message.payload.widget.modifiers) & (MESSAGE_MODIFIER_RIGHT_BUTTON)));
                spellSlot = -1;
...
i32 townManager::RecruitHero(i32 availableHeroIndex, i32 cannotRecruit) {
    ...
    if (m_recruitState != -1) {
        ...
        newHeroClass =
            (newHeroClass + Random(1, (FACTION_COUNT) - 1)) % TOWN_FACTION_COUNT;
        gpCurPlayer->m_availableHeroIds[m_recruitState] =
...
void townManager::SetupWell(heroWindow* window) {
    ...
    for (dwellingResult = 0; dwellingResult < TOWN_WELL_DWELLING_COUNT; ++dwellingResult) {
        ...
        messaged.payload.widget.data.value =
            dwellingTypesValue[dwellingResult] + (BUILDING_SLOT_DWELLING_FIRST);
        window->BroadcastMessage(messaged);
        sprintf(
            gText,
            "monh%04d.icn",
    ...
    for (dwellingResult = 0; dwellingResult < TOWN_WELL_DWELLING_COUNT; ++dwellingResult) {
        ...
        if (m_town->m_buildings
            & (1L << (dwellingTypesValue[dwellingResult] + (BUILDING_SLOT_DWELLING_FIRST)))) {
        ...
        strcpy(
            gText,
            gArmyNamesPlural[(gDwellingType[(m_town->m_type)][dwellingTypesValue[dwellingResult]])]
        );
        gText[0] -= ' ';
        messaged.payload.widget.data.text = gText;
        window->BroadcastMessage(messaged);
    ...
    for (dwellingResult = 0; dwellingResult < TOWN_WELL_DWELLING_COUNT; ++dwellingResult) {
        monsterInfoi = gMonsterDatabase[(gDwellingType[(m_town->m_type)][dwellingTypesValue[dwellingResult]])];
        strcpy(gText, "");
        ...
        if (m_town->m_buildings
            & (1L << (dwellingTypesValue[dwellingResult] + (BUILDING_SLOT_DWELLING_FIRST)))) {
            growthd = gMonsterDatabase[(gDwellingType[(m_town->m_type)][dwellingTypesValue[dwellingResult]])]
                          .growth;
...
void townManager::SetupThievesGuild(heroWindow* window, i32 informationLevel) {
    ...
    for (category_stat = THIEVES_CATEGORY_TOWNS;
         !((category_stat) >= maxCategories_hero);
         ++category_stat) {
        ...
        for (rank = 0; rank < TOWN_THIEVES_PLAYER_COUNT; ++rank) {
            ...
            for (position_current = firstAtRank_rank; !(lastAtRank_j < position_current);
                ...
                iconControl_last = new iconWidget(
                    ...
                    static_cast<i16>(
                        (category_stat) * THIEVES_CATEGORY_ROW_HEIGHT
