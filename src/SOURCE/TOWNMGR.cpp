...
i32 townManager::BuyBuild(
    ...
    textWidget* amountWidgets_b[IDX(RES_COUNT)];
    char* amountText_n[IDX(RES_COUNT)];
    iconWidget* resourceWidgets_m[IDX(RES_COUNT)];
    ...
    i32 rowY_o;
    i32 windowY_m;
    i8 resourceTypes_o[IDX(RES_COUNT)];
    i32 costCount_o;
    ...
    if (resourceCount_a <= 4) {
        ...
        topRowCount_c = 3;
        bottomRowCount_o = 3;
    } else if (resourceCount_a == IDX(RES_COUNT)) {
        topRowCount_c = 3;
        bottomRowCount_o = 4;
    }
    ...
    if (dwelling_k >= 0) {
        ...
        for (index_h = 0; index_h < IDX(BUILDING_SLOT_COUNT); ++index_h) {
...
void townManager::BuildObj(H2_ENUM_PARAM(BuildingSlotType, i32) building) {
    ...
    }
    {
        ...
        for (index_h = 0; index_h < m_townObjectCount; ++index_h) {
            if (HAS(m_town->m_buildings, BIT(m_townObjects[index_h]->m_buildingId))) {
...
void townManager::SetupMage(heroWindow* window) {
    i16 H2_UNUSED(unusedZero_g) = 0;
    i16 H2_UNUSED(unusedAvailable_n) = 1;
    i16 H2_UNUSED(unusedInvalid_d) = TOWN_MAGE_SPELL_UNAVAILABLE;
    i16 H2_UNUSED(unusedIconState_m) = 2;
    i16 H2_UNUSED(unusedFirstSpell_c) = TOWN_MAGE_FIRST_SPELL_CONTROL;
    ...
    i16 H2_UNUSED(unusedGuildIcon_i) = TOWN_MAGE_GUILD_ICON_CONTROL;
    i16 H2_UNUSED(unusedDescription_b) = TOWN_MAGE_DESCRIPTION_CONTROL;
    ...
    for (level_f = 0; level_f < TOWN_MAGE_GUILD_LEVEL_COUNT; ++level_f) {
        for (slot_o = 0; slot_o < TOWN_MAGE_GUILD_SPELLS_PER_LEVEL; ++slot_o) {
            if (slot_o >= gSpellLimits[level_f]
            ...
            message_i.payload.widget.id =
                level_f * TOWN_MAGE_GUILD_SPELLS_PER_LEVEL + slot_o + TOWN_MAGE_FIRST_SPELL_CONTROL;
            message_i.payload.widget.data.value = IDX(WIDGET_FLAG_DRAW);
            window->BroadcastMessage(message_i);
            ...
            if (spellState_c != 0) {
                ...
                message_i.payload.widget.data.value = IDX(WIDGET_FLAG_DRAW);
                message_i.payload.widget.id =
                    level_f * TOWN_MAGE_GUILD_SPELLS_PER_LEVEL + slot_o + TOWN_MAGE_FIRST_ICON_CONTROL;
                window->BroadcastMessage(message_i);
                message_i.payload.widget.id = level_f * TOWN_MAGE_GUILD_SPELLS_PER_LEVEL + slot_o
                ...
                message_i.payload.widget.data.value =
                    gsSpellInfo[IDX(m_town->m_spells[level_f][slot_o])].iconIndex;
                window->BroadcastMessage(message_i);
                lineCount_k = smallFont->LineLength(
                    gSpellNames[IDX(m_town->m_spells[level_f][slot_o])],
    ...
    if (message.type == MESSAGE_WIDGET) {
        switch (message.payload.widget.command) {
            ...
            case WIDGET_COMMAND_ALTERNATE_SELECT:
                ...
                if (spellSlot_b != -1) {
                    level_d = spellSlot_b / TOWN_MAGE_GUILD_SPELLS_PER_LEVEL;
                    slot_j = spellSlot_b % TOWN_MAGE_GUILD_SPELLS_PER_LEVEL;
...
i32 townManager::RecruitHero(i32 availableHeroIndex, i32 cannotRecruit) {
    ...
    artifactCount_h = 0;
    for (index_j = 0; index_j < HERO_ARTIFACT_SLOT_COUNT; ++index_j) {
...
void townManager::SetupThievesGuild(heroWindow* window, i32 informationLevel) {
    i16 H2_UNUSED(unusedRankX_i) = THIEVES_RANK_FIRST_X;
    i16 H2_UNUSED(unusedRankWidth_g) = THIEVES_PLAYER_COLUMN_WIDTH;
    i16 H2_UNUSED(unusedRankY_n) = THIEVES_FIRST_CATEGORY_Y;
    i16 H2_UNUSED(unusedRankHeight_k) = THIEVES_CATEGORY_ROW_HEIGHT;
    i16 H2_UNUSED(unusedRankIconHeight_o) = THIEVES_RANK_ICON_HEIGHT;
    i16 H2_UNUSED(unusedIconWidth_f) = THIEVES_RANK_ICON_WIDTH;
    ...
    i32 H2_UNUSED(unusedThievesValue_i);
    town* playerTown_j;
    i16 H2_UNUSED(unusedFirstRankControl_j) = TOWN_THIEVES_FIRST_RANK_CONTROL;
    i16 H2_UNUSED(unusedFirstPlayerControl_e) = TOWN_THIEVES_FIRST_PLAYER_CONTROL;
    i16 H2_UNUSED(unusedHeroY_k) = THIEVES_HERO_Y;
    i16 H2_UNUSED(unusedPrimaryStatsY) = THIEVES_PRIMARY_STATS_Y;
    i16 H2_UNUSED(unusedPersonalityY_m) = THIEVES_PERSONALITY_Y;
    i16 H2_UNUSED(unusedCreatureY_d) = THIEVES_CREATURE_Y;
    i32 position_a;
    ...
    for (category_l = THIEVES_CATEGORY_TOWNS; !(IDX(category_l) >= maxCategories_n);
         ++category_l) {
        ...
        for (rank_a = 0; rank_a < GAME_PLAYER_COUNT; ++rank_a) {
            ...
            for (position_a = firstAtRank_k; !(position_a > lastAtRank_b); ++position_a) {
                iconControl_a = new iconWidget(
                    ...
                    static_cast<i16>(
                        IDX(category_l) * THIEVES_CATEGORY_ROW_HEIGHT + THIEVES_FIRST_CATEGORY_Y
                    ),
                ...
                goto nextRank;
            {
                if (strongestHeroPosition_j != -1) {
                    ...
                    for (heroPosition_d = 0;
                         heroPosition_d < HERO_PRIMARY_STAT_COUNT;
                         ++heroPosition_d) {
                        sprintf(
                            statText_h,
                            "%d\n",
                        ...
                        for (heroPosition_d = 0;
                            ...
                            for (armySlot_n = 0; armySlot_n < ARMY_GROUP_SLOT_COUNT;
                                ...
                                if (ARMY_GROUP_HAS_POSITIVE_STACK(
                                    ...
                                    && gMonsterDatabase[IDX(strongestHero_d->m_army
                                                                .m_creatureTypes[armySlot_n])]
                                               .fightValue
                                           > strongestCreatureValue_l) {
                                    strongestCreature_o =
            ...
            switch (category) {
                ...
                case THIEVES_CATEGORY_ARTIFACTS:
                    ...
                    for (townIndex_c = 0; townIndex_c < gpGame->m_players[player].m_heroCount;
                        ...
                        for (heroIndex_n = 0; heroIndex_n < HERO_ARTIFACT_SLOT_COUNT; ++heroIndex_n) {
