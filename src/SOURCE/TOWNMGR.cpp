...
static const i8 gTownObjectOrder[(FACTION_COUNT)][(BUILDING_SLOT_COUNT)] = {
...
SBuildingInfo sBuildingInfo[(FACTION_COUNT)][(BUILDING_SLOT_COUNT)] = {
...
void townObject::Draw(i32 advanceAnimation) {
    ...
    if (m_buildingId == TOWN_OBJECT_KNIGHT_LEFT_OVERLAY
        ...
        && (!(((gpTownManager->m_town->m_buildings) & ((TOWN_BUILDING_TAVERN))))
            || (!(((gpTownManager->m_town->m_buildings) & ((TOWN_BUILDING_MAGE_GUILD))))
                && !(((gpTownManager->m_town->m_buildings) & ((TOWN_BUILDING_DWELLING_6))))
                && !(((gpTownManager->m_town->m_buildings) & ((TOWN_BUILDING_UPGRADED_DWELLING_6)))))))
        return;
    ...
    if (m_buildingId == TOWN_OBJECT_KNIGHT_RIGHT_OVERLAY
        ...
        && (!(((gpTownManager->m_town->m_buildings) & ((TOWN_BUILDING_MAGE_GUILD))))
            || !(((gpTownManager->m_town->m_buildings) & ((TOWN_BUILDING_COLISEUM))))))
        return;
    if (m_buildingId == TOWN_OBJECT_BARBARIAN_OVERLAY
        && gpTownManager->m_town->m_type == FACTION_BARBARIAN
        && (!(((gpTownManager->m_town->m_buildings) & ((TOWN_BUILDING_THIEVES_GUILD))))
            || !(((gpTownManager->m_town->m_buildings) & ((TOWN_BUILDING_DWELLING_3))))))
        return;
...
MessageDispatchResult townManager::Main(tag_message& message) {
    ...
    if (giDebugBuildingToBuild != -1) {
        ...
        if (dbgBuild == TOWN_DEBUG_BUILD_ALL) {
            for (loop = 0; loop < (BUILDING_SLOT_COUNT); ++loop) {
...
i32 townManager::BuyBuild(
    ...
    textWidget* amountWidgets[(RES_COUNT)];
    char* amountText[(RES_COUNT)];
    iconWidget* resourceWidgets[(RES_COUNT)];
    ...
    if (resourceCount <= 4) {
        ...
        topRowCount = 3;
        bottomRowCount = 3;
    } else if (resourceCount == (RES_COUNT)) {
        topRowCount = 3;
        bottomRowCount = 4;
    }
    ...
    if (dwelling >= 0) {
        ...
        for (index = 0; index < (BUILDING_SLOT_COUNT); ++index) {
