...
void ExpCampaign::InitMap(void) {
    ...
    switch (campaignChoice->type) {
        case CAMPAIGN_CHOICE_RESOURCE:
            player->m_resources[H2EnumIndex(campaignChoice->resource)] += campaignChoice->amount;
            break;
        ...
        case CAMPAIGN_CHOICE_SPELL:
            if (player->m_heroCount > 0)
                gpGame->GetHero(player->m_heroIds[0])
                    ->m_spells[H2EnumIndex(campaignChoice->spell)] = 1;
            break;
    ...
    for (award = AWARD_ELVEN_ALLIANCE; H2EnumIndex(award) < EXPANSION_CAMPAIGN_AWARD_COUNT; ++award) {
        if (m_awards[H2EnumIndex(award)] != 0) {
            switch (award) {
...
void ExpCampaign::ShowInfo(i32 viewOnly, i32) {
    ...
    for (map = 0; map < m_mapCount; ++map) {
        trackWidget = new iconWidget(
            expansionCampaignTrackXY[H2EnumIndex(m_campaignId)][map][0],
            expansionCampaignTrackXY[H2EnumIndex(m_campaignId)][map][1],
...
void ExpCampaign::UpdateInfo(i32 redraw) {
    ...
    for (map = 0; map < EXPANSION_CAMPAIGN_BONUS_CHOICE_COUNT; ++map) {
        choice = xCampaignChoices[0][H2EnumIndex(m_viewMap)]
                 + H2EnumIndex(m_campaignId) * EXPANSION_CAMPAIGN_MAX_MAP_COUNT
        ...
        switch (choice->type) {
            ...
            case CAMPAIGN_CHOICE_SECONDARY_SKILL:
                ...
                break;
            case CAMPAIGN_CHOICE_CREATURES:
                strcpy(armyName8, gArmyNamesPlural[H2EnumIndex(choice->creature)]);
                armyName8[0] -= 'a' - 'A';
            ...
            case CAMPAIGN_CHOICE_SPELL_SCROLL: {
                ...
                if (showScroll != 0) {
                    sprintf(gText, "%s %s", gSpellNames[H2EnumIndex(choice->spell)], "Scroll");
                } else {
                    sprintf(gText, "%s", gSpellNames[H2EnumIndex(choice->spell)]);
                }
                break;
    ...
    for (map = 0; map < EXPANSION_CAMPAIGN_BONUS_CHOICE_COUNT; ++map) {
        ...
        if (m_bonusChoices[H2EnumIndex(m_viewMap)] == map)
