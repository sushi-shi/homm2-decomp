...
SCampaignChoice xCampaignChoices[H2EnumIndex(EXPANSION_CAMPAIGN_COUNT)][EXPANSION_CAMPAIGN_MAX_MAP_COUNT][CAMPAIGN_BONUS_CHOICE_COUNT] = {
...
void ExpCampaign::UpdateInfo(i32 redraw) {
    ...
    utf8::Format(gText, GLOBAL_TEXT_BUFFER_SIZE, "%s", xScenarioName[H2EnumIndex(m_campaignId)][H2EnumIndex(m_viewMap)]);
    m_window->BroadcastMessage(message);

    message.payload.widget.id = CAMPAIGN_SCENARIO_DESCRIPTION_WIDGET;
    utf8::Format(gText, GLOBAL_TEXT_BUFFER_SIZE, "%s", xScenarioDescription[H2EnumIndex(m_campaignId)][H2EnumIndex(m_viewMap)]);
    m_window->BroadcastMessage(message);
    ...
    for (i = 0; i < CAMPAIGN_BONUS_CHOICE_COUNT; ++i) {
        ...
        switch (choice->type) {
            ...
            case CAMPAIGN_CHOICE_ARTIFACT:
                switch (choice->artifact) {
                    ...
                    case ARTIFACT_MAJOR_SCROLL:
                        utf8::Copy(gText, GLOBAL_TEXT_BUFFER_SIZE, localization::Tr("campaign.bonus.artifact.major_scroll"));
                        break;
                    case ARTIFACT_FOREMOST_SCROLL:
                        utf8::Copy(gText, GLOBAL_TEXT_BUFFER_SIZE, localization::Tr("campaign.bonus.artifact.foremost_scroll"));
                        break;
                    case ARTIFACT_BALLISTA:
                        utf8::Copy(gText, GLOBAL_TEXT_BUFFER_SIZE, localization::Tr("campaign.bonus.artifact.ballista")  );
                        break;
                    case ARTIFACT_STEALTH_SHIELD:
                        utf8::Copy(gText, GLOBAL_TEXT_BUFFER_SIZE, localization::Tr("campaign.bonus.artifact.stealth_shield"));
                        break;
                    case ARTIFACT_NOMAD_BOOTS:
                        utf8::Copy(gText, GLOBAL_TEXT_BUFFER_SIZE, localization::Tr("campaign.bonus.artifact.nomad_boots"));
                        break;
                    case ARTIFACT_TRAVELER_BOOTS:
                        utf8::Copy(gText, GLOBAL_TEXT_BUFFER_SIZE, localization::Tr("campaign.bonus.artifact.traveler_boots"));
                        break;
            ...
            case CAMPAIGN_CHOICE_SPELL:
                ...
                break;
            case CAMPAIGN_CHOICE_SECONDARY_SKILL:
                if ((choice->amount == H2EnumIndex(HERO_SKILL_LEVEL_BASIC)
                     && choice->value == H2EnumIndex(HERO_SKILL_NECROMANCY))
                    || (choice->amount == H2EnumIndex(HERO_SKILL_LEVEL_ADVANCED)
                        && choice->value == H2EnumIndex(HERO_SKILL_LOGISTICS))) {
                    utf8::Format(
                        gText, GLOBAL_TEXT_BUFFER_SIZE,
