...
const char* ExpCampaign::JosephName(void) {
    ...
    return xJosephName[H2EnumIndex(m_currentMap) - H2EnumIndex(EXPANSION_CAMPAIGN_FIRST_ALTERNATE_NAME_MAP)];
}

const char* ExpCampaign::IvanName(void) {
    if (m_currentMap < EXPANSION_CAMPAIGN_FIRST_ALTERNATE_NAME_MAP)
        return xUncleIvanName[0];
    return xUncleIvanName[H2EnumIndex(m_currentMap) - H2EnumIndex(EXPANSION_CAMPAIGN_FIRST_ALTERNATE_NAME_MAP)];
}
