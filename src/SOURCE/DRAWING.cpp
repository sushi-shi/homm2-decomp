...
void combatManager::CombatMessage(char* message, i32 updateScreen, i32 retainPrevious, i32 clear) {
    ...
    if (clear != 0) {
        ...
        if (retainPrevious == 0) {
            if (platform::Ticks() < m_combatMessageExpiration) {
                return;
            } else {
                strcpy(m_previousCombatMessage, "");
                m_previousCombatMessageExpiration = 0;
...
void combatManager::CombatMessage(CombatMessageCommand messageType) {
    ...
    currentArmyPtr = &m_armies[H2EnumIndex(m_currentArmySide)][m_currentArmyIndex];
    ...
    if (currentArmyPtr->m_targetSide >= COMBAT_ATTACKER_SIDE
        ...
        targetArmy = &m_armies[H2EnumIndex(currentArmyPtr->m_targetSide)][currentArmyPtr->m_targetIndex];
        targetMonsterType = targetArmy->m_monsterType;
    }

    switch (messageType) {
        case COMBAT_MESSAGE_COMMAND_DEFAULT:
            if ((H2EnumIndex((currentArmyPtr->m_monster.flags.all) & (COMBAT_ARMY_FLAG_SHOOTER))) != 0
        ...
        case COMBAT_MESSAGE_COMMAND_ATTACK:
            sprintf(
                ...
                gArmyNames[H2EnumIndex(targetMonsterType)]
            );
            break;
...
void combatManager::ResetLimitCreature(void) {
    ...
    for (side = COMBAT_ATTACKER_SIDE; H2EnumIndex(side) < COMBAT_SIDE_COUNT; side++) {
        for (armySlotIndex = 0; armySlotIndex < COMBAT_ARMY_SLOT_COUNT_DRAWING; armySlotIndex++) {
            if ((H2EnumIndex((m_armies[H2EnumIndex(side)][armySlotIndex].m_monster.flags.all) & (COMBAT_ARMY_FLAG_MIRROR_IMAGE)))
                != 0)
                m_limitCreatureCount[H2EnumIndex(side)][armySlotIndex] = -1;
            else
                m_limitCreatureCount[H2EnumIndex(side)][armySlotIndex] = 0;
        }
    }
    m_drawHero[0] = 0;
    m_drawHero[1] = 0;
    ...
    for (cellIndex = 0; cellIndex < COMBAT_HEX_COUNT; cellIndex++) {
        if (m_gridState[cellIndex] != GRID_SHADE_NONE) {
            DimIconToBitmap(
                ...
                1,
                H2EnumIndex(m_gridState[cellIndex]) - 1,
                ICON_DRAW_CLIP,
                0,
                0,
                COMBAT_SCREEN_WIDTH,
...
void combatManager::DrawFrame(
    ...
    i32 side5;
    H2SteppedEnumStorage<ArmyDrawState, i32> drawState5;
    CombatDrawLayer row;
    ...
    for (row = DRAW_FIRST_LAYER; row < DRAW_LAYER_COUNT; row++) {
        ...
        if (m_inCastleCombat != 0
            ...
            for (drawState5 = ARMY_DRAW_BEHIND; drawState5 < ARMY_DRAW_PHASE_COUNT; drawState5++) {
                m_hexCells[H2EnumIndex(COMBAT_CASTLE_SPECIAL_HEX_FIRST)].DrawOccupant(drawState5, 0);
                m_hexCells[H2EnumIndex(COMBAT_CASTLE_SPECIAL_HEX_SECOND)].DrawOccupant(drawState5, 0);
            }
        }

        for (column1 = firstColumn; endColumn != column1; column1 += columnStep3)
            m_hexCells[H2EnumIndex(row) * COMBAT_GRID_ROW_LENGTH + column1].DrawLowerDeadOccupants();

        for (column1 = firstColumn; endColumn != column1; column1 += columnStep3)
            m_hexCells[H2EnumIndex(row) * COMBAT_GRID_ROW_LENGTH + column1].DrawUpperDeadOccupant();

        for (drawState5 = ARMY_DRAW_BEHIND; drawState5 < ARMY_DRAW_PHASE_COUNT; drawState5++) {
            if (drawState5 == ARMY_DRAW_NORMAL) {
                for (column1 = firstColumn; endColumn != column1; column1 += columnStep3) {
                    if (m_hexCells[H2EnumIndex(row) * COMBAT_GRID_ROW_LENGTH + column1].m_obstacleIndex
                        != -1) {
                        m_hexCells[H2EnumIndex(row) * COMBAT_GRID_ROW_LENGTH + column1].DrawObstacle();
                    }
                }
            }
            ...
            for (column1 = firstColumn; endColumn != column1; column1 += columnStep3) {
                ...
                i32 wallFrame1;
                i32 wallY;
                H2EnumStorage<CombatCastleHex, i32> hexIndex6;

                hexIndex6 = H2EnumIndex(row) * COMBAT_GRID_ROW_LENGTH + column1;
                ...
                if (m_inCastleCombat != 0 && drawState5 == ARMY_DRAW_BEHIND) {
                    ...
                    if (wallFrame1 != 0) {
                        m_combatIcons[H2EnumIndex(COMBAT_ICON_TOWER)]->CombatClipDrawToBuffer(
                            ...
                            wallY,
                            wallFrame1,
                            &m_hexCells[H2EnumIndex(hexIndex6)].m_limits[0],
                            ICON_DRAW_NORMAL,
                            0,
                            NULL,
                            NULL
                ...
                if (skipSpecialOccupants6 == 0
                    ...
                    m_hexCells[H2EnumIndex(row) * COMBAT_GRID_ROW_LENGTH + column1].DrawOccupant(
                        drawState5,
                        0
                    );
                }
        ...
        if (moatCell[H2EnumIndex(row)] == giWalkingTo || moatCell[H2EnumIndex(row)] == giWalkingTo2
            || moatCell[H2EnumIndex(row)] == giWalkingFrom || moatCell[H2EnumIndex(row)] == giWalkingFrom2) {
            ...
            if (gpCombatManager->m_drawbridgeState != COMBAT_CASTLE_GATE_OPEN
                ...
                if (giWalkingFrom <= giWalkingTo)
                    goto checkWalkingTo;
                if (moatCell[H2EnumIndex(row)] == giWalkingFrom || moatCell[H2EnumIndex(row)] == giWalkingFrom2)
                    goto drawMoat;
                goto endRow;
            checkWalkingTo:
                if (moatCell[H2EnumIndex(row)] == giWalkingTo || moatCell[H2EnumIndex(row)] == giWalkingTo2)
...
void combatManager::DrawSmallView(i32 viewIndex, i32 updateScreen) {
    ...
    if (gConfig.combatArmyInfoLevel == COMBAT_SMALL_VIEW_FULL_INFO) {
        if (viewIndex == 0) {
            viewX = m_smallViewSide[viewIndex] == COMBAT_ATTACKER_SIDE
                        ? COMBAT_SMALL_VIEW_LEFT_X
    ...
    viewArmy1 = &m_armies[H2EnumIndex(m_smallViewSide[viewIndex])][m_smallViewArmyIndex[viewIndex]];
    drawn6 = m_combatIcons[H2EnumIndex(COMBAT_ICON_SMALL_VIEW_BACKGROUND)]->CombatClipDrawToBuffer(
        viewX,
        viewY2,
    ...
    if (drawn6 != ICON_DRAW_SKIPPED) {
        ...
        if (gConfig.combatArmyInfoLevel == COMBAT_SMALL_VIEW_FULL_INFO) {
            ...
            if ((H2EnumIndex((viewArmy1->m_monster.flags.all) & (COMBAT_ARMY_FLAG_SHOOTER))))
                smallFont->DrawBoundedString(
        ...
        for (spellSlot = 0; spellSlot < visibleSpellCount5; spellSlot++) {
            ...
            iconX +=
                (COMBAT_SMALL_VIEW_ICON_SIZE
                 - GetIconEntry(m_combatIcons[H2EnumIndex(COMBAT_ICON_SMALL_VIEW_SPELL)], spellFrame1)->w)
                >> 1;
            iconY2 +=
                (COMBAT_SMALL_VIEW_ICON_SIZE
                 - GetIconEntry(m_combatIcons[H2EnumIndex(COMBAT_ICON_SMALL_VIEW_SPELL)], spellFrame1)->h)
                >> 1;
            m_combatIcons[H2EnumIndex(COMBAT_ICON_SMALL_VIEW_SPELL)]
                ->DrawToBuffer(iconX, iconY2, spellFrame1, ICON_DRAW_NORMAL);
        }
    }
