...
typedef i32 CombatSmallViewText;
enum {
...
typedef i32 CombatSmallViewModifierFrame;
enum {
...
void combatManager::CombatMessage(CombatMessageCommand messageType) {
    ...
    switch (messageType) {
        case COMBAT_MESSAGE_COMMAND_DEFAULT:
            if ((((currentArmy->m_monster.flags.all) & (COMBAT_ARMY_FLAG_SHOOTER))) != 0
                && currentArmy->m_monster.shots == 0 && targetArmy != NULL)
                strcpy(gText, cCombatMessage[(MESSAGE_TEXT_NO_SHOTS)]);
            else
                strcpy(gText, cCombatMessage[(MESSAGE_TEXT_DEFAULT)]);
            break;
        case COMBAT_MESSAGE_COMMAND_MOVE:
            sprintf(
                gText,
                cCombatMessage[(MESSAGE_TEXT_MOVE)],
        ...
        case COMBAT_MESSAGE_COMMAND_FLY:
            sprintf(
                gText,
                cCombatMessage[(MESSAGE_TEXT_FLY)],
                gArmyNames[(actingMonsterType)]
            );
            break;
        case COMBAT_MESSAGE_COMMAND_ATTACK:
            sprintf(
                gText,
                cCombatMessage[(MESSAGE_TEXT_ATTACK)],
        ...
        case COMBAT_MESSAGE_COMMAND_SHOOT_THROUGH_WALL:
            sprintf(
                gText,
                cCombatMessage[(MESSAGE_TEXT_SHOOT)],
                gArmyNamesPlural[(targetMonsterType)],
        ...
        case COMBAT_MESSAGE_COMMAND_OPTIONS:
            if (m_heroes[(m_currentSide)] != NULL && m_heroes[(m_currentSide)]->m_isCaptain != 0)
                strcpy(gText, cCombatMessage[(MESSAGE_TEXT_CAPTAIN_OPTIONS)]);
            else
                strcpy(gText, cCombatMessage[(MESSAGE_TEXT_HERO_OPTIONS)]);
            break;
        case COMBAT_MESSAGE_COMMAND_OPPOSING_OPTIONS:
            if (m_heroes[(OppositeCombatSide(m_currentSide))] != NULL
                && m_heroes[(OppositeCombatSide(m_currentSide))]->m_isCaptain != 0)
                strcpy(gText, cCombatMessage[(MESSAGE_TEXT_OPPOSING_CAPTAIN)]);
            else
                strcpy(gText, cCombatMessage[(MESSAGE_TEXT_OPPOSING_HERO)]);
            break;
        case COMBAT_MESSAGE_COMMAND_VIEW_INFO:
            if (m_selectedHex == COMBAT_BALLISTA_HEX) {
                sprintf(gText, cCombatMessage[(MESSAGE_TEXT_BALLISTA)]);
            } else {
                actingMonsterType =
                ...
                if (actingMonsterType >= CREATURE_PEASANT)
                    sprintf(
                        gText,
                        cCombatMessage[(MESSAGE_TEXT_VIEW_INFO)],
                        gArmyNames[(actingMonsterType)]
                    );
                else
                    sprintf(
...
void combatManager::ResetLimitCreature(void) {
    ...
    for (side = COMBAT_ATTACKER_SIDE; (side) < COMBAT_SIDE_COUNT; side++) {
        for (armySlotIndex = 0; armySlotIndex < COMBAT_ARMY_SLOT_COUNT_DRAWING; armySlotIndex++) {
            m_limitCreatureCount[(side)][armySlotIndex]
                = (((m_armies[(side)][armySlotIndex].m_monster.flags.all) & (COMBAT_ARMY_FLAG_MIRROR_IMAGE)))
                        != 0
                    ? -1
                    : 0;
        }
...
i32 combatManager::UpdateGrid(i32 resetGridDisplay, i32 rebuildGrid) {
    ...
    for (cell = 0; cell < COMBAT_HEX_COUNT; cell++) {
        if (m_gridState[cell] != GRID_SHADE_NONE) {
            DimIconToBitmap(
                m_combatIcons[(COMBAT_ICON_GRID)],
                m_backgroundBuffer,
...
void combatManager::DrawBackground(void) {
    icon* backgroundIcon;
    i32 unusedBackgroundHeight [[maybe_unused]];
    i32 unusedBackgroundSide [[maybe_unused]];
    ...
    if (m_battlefieldFringe != FRINGE_NONE) {
        ...
        if (m_inCastleCombat != 0)
            IconToBitmap(
                ...
                0,
                0,
                COMBAT_SCREEN_WIDTH / (COMBAT_SIDE_COUNT),
                COMBAT_AREA_HEIGHT,
                0
            );
        else
...
void combatManager::DrawFrame(
    ...
    i32 unusedDrawState [[maybe_unused]];
    i32 gridWasShowing [[maybe_unused]];
    i32 endColumn;
    ...
    if (computeExtent != 0) {
        extentChanged = false;
        for (state = 0; (state) < COMBAT_SIDE_COUNT; state++) {
            for (armyIndex = 0; armyIndex < COMBAT_ARMY_SLOT_COUNT_DRAWING; armyIndex++) {
                if (m_limitCreatureCount[(state)][armyIndex] > 0) {
                    extentChanged = true;
                    gbComputeExtent = true;
                    gbSaveBiggestExtent = true;
                    gbReturnAfterComputeExtent = true;
                    m_armies[(state)][armyIndex].DrawToBuffer(
                        m_hexCells[m_armies[(state)][armyIndex].m_hex].m_x,
                        m_hexCells[m_armies[(state)][armyIndex].m_hex].m_y,
                        0
                    );
        ...
        for (state = 0; (state) < COMBAT_SIDE_COUNT; state++) {
            if (m_drawHero[(state)] != 0) {
                extentChanged = true;
                gbComputeExtent = true;
                gbSaveBiggestExtent = true;
                gbReturnAfterComputeExtent = true;
                m_heroIcons[(state)]->CombatClipDrawToBuffer(
                    (state) == 0 ? COMBAT_HERO_LEFT_X : COMBAT_HERO_RIGHT_X,
                    (state) == 0 ? COMBAT_HERO_LEFT_Y : COMBAT_HERO_RIGHT_Y,
                    sCmbtHero[m_heroSpriteIndex[(state)]]
                        .animationFrames[m_heroAnimationState[(state)]]
                                        [m_heroAnimationFrame[(state)]],
                    &m_heroLimits[(state)],
                    ICON_DRAW_FLIPPED
                );
                gbReturnAfterComputeExtent = false;
    ...
    for (row = DRAW_FIRST_LAYER; row < DRAW_LAYER_COUNT; row++) {
        ...
        if (m_inCastleCombat != 0
            ...
            for (state = ARMY_DRAW_BEHIND; state < ARMY_DRAW_PHASE_COUNT; state++) {
                m_hexCells[(COMBAT_CASTLE_SPECIAL_HEX_FIRST)].DrawOccupant(state, 0);
                m_hexCells[(COMBAT_CASTLE_SPECIAL_HEX_SECOND)].DrawOccupant(state, 0);
            }
        }
        ...
        for (state = ARMY_DRAW_BEHIND; state < ARMY_DRAW_PHASE_COUNT; state++) {
            if (state == ARMY_DRAW_NORMAL) {
                for (column = startColumn; column != endColumn; column += columnStep) {
                    if (m_hexCells[(row) * COMBAT_GRID_ROW_LENGTH + column].m_obstacleIndex
                        != -1) {
                        m_hexCells[(row) * COMBAT_GRID_ROW_LENGTH + column].DrawObstacle();
                    }
                }
            }

            for (column = startColumn; column != endColumn; column += columnStep) {
                u16 wallCoordinates[(FACTION_COUNT)][WALL_COORDINATE_COUNT] = {
                ...
                if (m_inCastleCombat != 0 && state == ARMY_DRAW_BEHIND) {
                    castleHex = (row) * COMBAT_GRID_ROW_LENGTH + column;
                    wallFrame = 0;
                    wallX = 0;
                    wallY = 0;
                    switch (castleHex) {
                        case COMBAT_CASTLE_HEX_TOP_TOWER:
                            wallFrame =
                                wallFrameOffsets[(m_wallStates[(COMBAT_WALL_SLOT_SECTION_FIRST)])]
                                + TOP_TOWER_BASE_FRAME;
                            break;
                        case COMBAT_CASTLE_HEX_SECOND_TOWER:
                            wallFrame =
                                wallFrameOffsets[(m_wallStates[(COMBAT_WALL_SLOT_SECTION_SECOND)])]
                                + SECOND_TOWER_BASE_FRAME;
                            break;
                        case COMBAT_CASTLE_HEX_THIRD_TOWER:
                            wallFrame =
                                wallFrameOffsets[(m_wallStates[(COMBAT_WALL_SLOT_SECTION_THIRD)])]
                                + THIRD_TOWER_BASE_FRAME;
                            break;
                        case COMBAT_CASTLE_HEX_BOTTOM_TOWER:
                            wallFrame =
                                wallFrameOffsets[(m_wallStates[(COMBAT_WALL_SLOT_SECTION_FOURTH)])]
                                + BOTTOM_TOWER_BASE_FRAME;
                            break;
                        case COMBAT_CASTLE_HEX_TOP_WALL:
                            wallFrame = (m_wallStates[(COMBAT_WALL_SLOT_TOP_TOWER)])
                                         + COMBAT_CASTLE_WALL_BASE_FRAME;
                            wallX =
                                wallCoordinates[(m_combatTowns[(COMBAT_DEFENDER_SIDE)]->m_type)]
                                                [WALL_TOP_X];
                            wallY =
                                wallCoordinates[(m_combatTowns[(COMBAT_DEFENDER_SIDE)]->m_type)]
                                                [WALL_TOP_Y];
                        ...
                        case COMBAT_CASTLE_HEX_SECOND_WALL:
                            wallFrame = (m_wallStates[(COMBAT_WALL_SLOT_SECOND_TOWER)])
                                         + COMBAT_CASTLE_WALL_BASE_FRAME;
                            wallX =
                                wallCoordinates[(m_combatTowns[(COMBAT_DEFENDER_SIDE)]->m_type)]
                                                [WALL_SECOND_X];
                        ...
                        case COMBAT_CASTLE_HEX_THIRD_WALL:
                            wallFrame = (m_wallStates[(COMBAT_WALL_SLOT_THIRD_TOWER)])
                                         + COMBAT_CASTLE_WALL_BASE_FRAME;
                            wallX =
                                wallCoordinates[(m_combatTowns[(COMBAT_DEFENDER_SIDE)]->m_type)]
                                                [WALL_THIRD_X];
                        ...
                        case COMBAT_CASTLE_HEX_BOTTOM_WALL:
                            wallFrame = (m_wallStates[(COMBAT_WALL_SLOT_BOTTOM_TOWER)])
                                         + COMBAT_CASTLE_WALL_BASE_FRAME;
                            wallX =
                                wallCoordinates[(m_combatTowns[(COMBAT_DEFENDER_SIDE)]->m_type)]
                                                [WALL_BOTTOM_X];
                    ...
                    if (wallFrame != 0) {
                        m_combatIcons[(COMBAT_ICON_TOWER)]->CombatClipDrawToBuffer(
                            ...
                            wallY,
                            wallFrame,
                            &m_hexCells[(castleHex)].m_limits[0],
                            ICON_DRAW_NORMAL
                        );
                    }
                }
                ...
                if (skipSpecialOccupants == 0
                    ...
                    m_hexCells[(row) * COMBAT_GRID_ROW_LENGTH + column].DrawOccupant(
                        state,
                        0
                    );
                }
        ...
        if (m_inCastleCombat == 0
            || (((m_combatTowns[(COMBAT_DEFENDER_SIDE)]->m_buildings) & ((TOWN_BUILDING_MOAT))))
                   == 0)
            goto endRow;
        if (row == DRAW_CASTLE_GATE_ROW && m_drawbridgeState != COMBAT_CASTLE_GATE_OPEN)
            goto endRow;

        if (giWalkingTo == moatCell[(row)] || giWalkingTo2 == moatCell[(row)]
            || giWalkingFrom == moatCell[(row)] || giWalkingFrom2 == moatCell[(row)]) {
...
void combatManager::DrawSmallView(i32 viewIndex, i32 updateScreen) {
    ...
    b32 oldLimit;
    i32 unusedSpell [[maybe_unused]];
    army* smallArmy;
    IconDrawResult drawResult;
    i32 unusedSlot [[maybe_unused]];
    i32 viewX;
    i32 iconX;
    i32 spellSlot;
    i32 spellIcon;
    ...
    smallArmy = &m_armies[(m_smallViewSide[viewIndex])][m_smallViewArmyIndex[viewIndex]];
    drawResult = m_combatIcons[(COMBAT_ICON_SMALL_VIEW_BACKGROUND)]->CombatClipDrawToBuffer(
        viewX,
        viewY,
    ...
    if (drawResult != ICON_DRAW_SKIPPED) {
        if (smallArmy->m_quantity > 1)
            sprintf(
                gText,
                cMiniViewText[(SMALL_VIEW_TEXT_QUANTITY_PLURAL)],
                smallArmy->m_quantity
            );
        else
            sprintf(
                gText,
                cMiniViewText[(SMALL_VIEW_TEXT_QUANTITY_SINGULAR)],
                smallArmy->m_quantity
            );
        ...
        if (gConfig.combatArmyInfoLevel == COMBAT_SMALL_VIEW_FULL_INFO) {
            smallFont->DrawBoundedString(
                cMiniViewText[(SMALL_VIEW_TEXT_ATTACK)],
                viewX + COMBAT_SMALL_VIEW_TEXT_X,
            ...
            smallFont->DrawBoundedString(
                cMiniViewText[(SMALL_VIEW_TEXT_DEFENSE)],
                viewX + COMBAT_SMALL_VIEW_TEXT_X,
                viewY + COMBAT_SMALL_VIEW_FIRST_STAT_Y
            ...
            smallFont->DrawBoundedString(
                cMiniViewText[(SMALL_VIEW_TEXT_HIT_POINTS)],
                viewX + COMBAT_SMALL_VIEW_TEXT_X,
                viewY + COMBAT_SMALL_VIEW_FIRST_STAT_Y
            ...
            smallFont->DrawBoundedString(
                cMiniViewText[(SMALL_VIEW_TEXT_DAMAGE)],
                viewX + COMBAT_SMALL_VIEW_TEXT_X,
                viewY + COMBAT_SMALL_VIEW_FIRST_STAT_Y
            ...
            smallFont->DrawBoundedString(
                cMiniViewText[(SMALL_VIEW_TEXT_MORALE)],
                viewX + COMBAT_SMALL_VIEW_TEXT_X,
                viewY + COMBAT_SMALL_VIEW_FIRST_STAT_Y
            ...
            smallFont->DrawBoundedString(
                cMiniViewText[(SMALL_VIEW_TEXT_LUCK)],
                viewX + COMBAT_SMALL_VIEW_TEXT_X,
                viewY + COMBAT_SMALL_VIEW_FIRST_STAT_Y
            ...
            if ((((smallArmy->m_monster.flags.all) & (COMBAT_ARMY_FLAG_SHOOTER))))
                smallFont->DrawBoundedString(
                    cMiniViewText[(SMALL_VIEW_TEXT_SHOTS)],
                    viewX + COMBAT_SMALL_VIEW_TEXT_X,
            ...
            if (smallArmy->m_morale > 0) {
                ...
                m_combatIcons[(COMBAT_ICON_SMALL_VIEW_MODIFIER)]->DrawToBuffer(
                    ...
                    viewY + COMBAT_SMALL_VIEW_FIRST_STAT_Y
                        + COMBAT_SMALL_VIEW_STAT_ROW_HEIGHT * (STAT_ROW_MORALE),
                    (SMALL_VIEW_NEUTRAL_MORALE_FRAME),
                    ICON_DRAW_NORMAL
                );
            }
        ...
        for (spellSlot = 0; spellSlot < visibleSpellCount; spellSlot++) {
            ...
            iconX +=
                (COMBAT_SMALL_VIEW_ICON_SIZE
                 - GetIconEntry(m_combatIcons[(COMBAT_ICON_SMALL_VIEW_SPELL)], spellIcon)->w)
                >> 1;
            iconY +=
                (COMBAT_SMALL_VIEW_ICON_SIZE
                 - GetIconEntry(m_combatIcons[(COMBAT_ICON_SMALL_VIEW_SPELL)], spellIcon)->h)
                >> 1;
