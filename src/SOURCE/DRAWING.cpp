...
void combatManager::DrawFrame(
    ...
    i32 col;
    CombatDrawLayer row;
    i32 unusedDrawState1 [[maybe_unused]];
    i32 state;
    b32 extentChanged1;
    i32 unusedDrawState26 [[maybe_unused]];
    i32 unused7 [[maybe_unused]];
    i32 unusedDrawState8 [[maybe_unused]];
    i32 unusedDrawState [[maybe_unused]];
    i32 gridWasShowing1 [[maybe_unused]];
    i32 endColumn;
    ...
    if (computeExtent != 0) {
        ...
        for (state = 0; (state) < COMBAT_SIDE_COUNT; state++) {
            for (armyIndex7 = 0; armyIndex7 < COMBAT_ARMY_SLOT_COUNT_DRAWING; armyIndex7++) {
                if (m_limitCreatureCount[(state)][armyIndex7] > 0) {
                    extentChanged1 = true;
                    gbComputeExtent = true;
                    gbSaveBiggestExtent = true;
                    gbReturnAfterComputeExtent = true;
                    m_armies[(state)][armyIndex7].DrawToBuffer(
                        m_hexCells[m_armies[(state)][armyIndex7].m_hex].m_x,
                        m_hexCells[m_armies[(state)][armyIndex7].m_hex].m_y,
                        0
                    );
    ...
    for (row = DRAW_FIRST_LAYER; row < DRAW_LAYER_COUNT; row++) {
        ...
        for (col = startColumn; col != endColumn; col += columnStep1)
            m_hexCells[(row) * COMBAT_GRID_ROW_LENGTH + col].DrawLowerDeadOccupants();

        for (col = startColumn; col != endColumn; col += columnStep1)
            m_hexCells[(row) * COMBAT_GRID_ROW_LENGTH + col].DrawUpperDeadOccupant();

        for (state = ARMY_DRAW_BEHIND; state < ARMY_DRAW_PHASE_COUNT; state++) {
            if (state == ARMY_DRAW_NORMAL) {
                for (col = startColumn; col != endColumn; col += columnStep1) {
                    if (m_hexCells[(row) * COMBAT_GRID_ROW_LENGTH + col].m_obstacleIndex
                        != -1) {
                        m_hexCells[(row) * COMBAT_GRID_ROW_LENGTH + col].DrawObstacle();
                    }
                }
            }

            for (col = startColumn; col != endColumn; col += columnStep1) {
                u16 wallCoordinates1[(FACTION_COUNT)][WALL_COORDINATE_COUNT] = {
                ...
                if (m_inCastleCombat != 0 && state == ARMY_DRAW_BEHIND) {
                    castleHex = (row) * COMBAT_GRID_ROW_LENGTH + col;
                    wallFrame1 = 0;
                    wallX7 = 0;
                    wallY = 0;
                    switch (castleHex) {
                ...
                if (skipSpecialOccupants8 == 0
                    || ((row) * COMBAT_GRID_ROW_LENGTH + col
                            != (COMBAT_CASTLE_SPECIAL_HEX_FIRST)
                        && (row) * COMBAT_GRID_ROW_LENGTH + col
                               != (COMBAT_CASTLE_SPECIAL_HEX_SECOND))) {
                    m_hexCells[(row) * COMBAT_GRID_ROW_LENGTH + col].DrawOccupant(
                        state,
                        0
                    );
                }
...
void combatManager::DrawSmallView(i32 viewIndex, i32 updateScreen) {
    ...
    drawResult1 = m_combatIcons[(COMBAT_ICON_SMALL_VIEW_BACKGROUND)]->CombatClipDrawToBuffer(
        viewX1,
        viewY,
