...
void army::SpecialAttack(void) {
    ...
    m_palette = NULL;
    target_1 = m_targetIndex + gpCombatManager->m_armies[IDX(m_targetSide)];
    ...
    if (initialXDistance_6 == 0) {
        directionFrame_2 =
            initialYDistance_8 > 0 ? m_frameInfo.projectileDirectionCount - 1 : 0;
        angle_2 = static_cast<float>(
            initialYDistance_8 > 0 ? -VERTICAL_ANGLE : VERTICAL_ANGLE
        );
    } else {
        /* The parenthesised divisor cast keeps both operands on the x87 stack;
           without it VC6 folds the divisor into a single `fidiv`. */
        slope = static_cast<float>(-initialYDistance_8)
                / (static_cast<float>(initialXDistance_6));
        angle_2 = static_cast<float>(
    ...
    if (m_monsterType == CREATURE_MAGE || m_monsterType == CREATURE_ARCHMAGE) {
        ...
        for (direction = 0; direction < missileSteps_7; direction++) {
            ...
            if (minX_1 < 0)
                minX_1 = 0;
            if (oldX_16 + missileHalfWidth > maxX_3) {
            ...
            if (minY_1 < 0)
                minY_1 = 0;
            if (oldY_9 + missileHalfHeight_1 > maxY_3) {
            ...
            if (direction != 0) {
                ...
                if (minX_1 < giMinExtentX)
                    giMinExtentX = minX_1;
                if (maxX_3 > giMaxExtentX)
                    giMaxExtentX = maxX_3;
                if (minY_1 < giMinExtentY)
                    giMinExtentY = minY_1;
    ...
    if (killed_13 > 0) {
        if (damage == -1) {
            ...
            strcpy(gTargetName, gArmyNames[IDX(target_1->m_monsterType)]);
            ...
            sprintf(
                ...
                killed_13,
                killed_13 <= 1 ? gTargetName : gArmyNamesPlural[IDX(target_1->m_monsterType)],
                killed_13 <= 1 ? "\xf3\xec\xe8\xf0\xe0\xe5\xf2" : "\xf3\xe1\xe8\xf2\xee"
            );
...
void army::DoAttack(i32 retaliation) {
    ...
    if (m_monsterType == CREATURE_HYDRA) {
        ...
        if (m_facing != desiredFacing) {
            ...
            if (targetOriginalFacing_5 != target_1->m_facing
        ...
        if (damage == -1) {
            ...
            if (killed_13 > 0) {
                ...
                sprintf(
                    ...
                    "\xe5\xe4. \xf3\xf0\xee\xed\xe0",
                    killed_13,
                    killed_13 <= 1 ? gTargetName
                                   : gArmyNamesPlural[IDX(target_1->m_monsterType)],
                    killed_13 <= 1 ? "\xf3\xec\xe8\xf0\xe0\xe5\xf2" : "\xf3\xe1\xe8\xf2\xee"
                );
                gText[0] = ToUpperCp1251(gText[0]);
            } else {
        ...
        switch (m_monsterType) {
            ...
            case CREATURE_VAMPIRE_LORD:
                gpCombatManager->m_killBenefit[IDX(gpCombatManager->m_hexCells[m_hex].m_occupantSide)] =
                    killed_13 * target_1->m_monster.hitPoints;
                break;
        }
        PowEffect(COMBAT_EFFECT_INVALID, 0, -1, -1);
        gpCombatManager->CombatMessage(combatText, 1, 1, 0);
        ...
        if (target_1 && target_1->m_quantity > 0
            ...
            if (gbRemoteOn && gpCombatManager->m_networkArmyPresent[0]
                ...
                target_1->m_quantity +=
                    gpCombatManager
                        ->m_killBenefit[IDX(gpCombatManager->m_hexCells[target_1->m_hex]
                                               .m_occupantSide)];
            }
        }
...
void army::PowEffect(
    ...
    if (effectX != NO_POW_EFFECT_COORDINATE) {
        ...
        for (side_4 = COMBAT_ATTACKER_SIDE; IDX(side_4) < COMBAT_SIDE_COUNT; side_4++) {
            for (index_10 = 0; index_10 < gpCombatManager->m_armyCount[IDX(side_4)]; index_10++) {
                if (gpCombatManager->m_armies[IDX(side_4)][index_10].m_drawSpellEffect) {
                    drawEffect_1 = 1;
                }
            }
        }
    ...
    for (side_4 = COMBAT_ATTACKER_SIDE; IDX(side_4) < COMBAT_SIDE_COUNT; side_4++) {
        for (index_10 = 0; index_10 < gpCombatManager->m_armyCount[IDX(side_4)]; index_10++) {
            if (gpCombatManager->m_armies[IDX(side_4)][index_10].m_animationSequence
    ...
    if (effectX != NO_POW_EFFECT_COORDINATE) {
        for (index_10 = 0; index_10 < gCurLoadedSpellIcon->m_frameCount; index_10++) {
            ...
            giMinExtentX = giMinExtentX < effectX + entry_1->x ? giMinExtentX
                                                                : effectX + entry_1->x;
            giMinExtentY = giMinExtentY < effectY + entry_1->y ? giMinExtentY
                                                                : effectY + entry_1->y;
            giMaxExtentX = giMaxExtentX > effectX + entry_1->x + entry_1->w - 1
                               ? giMaxExtentX
                               : effectX + entry_1->x + entry_1->w - 1;
            giMaxExtentY = giMaxExtentY > effectY + entry_1->y + entry_1->h - 1
                               ? giMaxExtentY
                               : effectY + entry_1->y + entry_1->h - 1;
        }
            ...
            for (index_10 = 0; index_10 < gpCombatManager->m_armyCount[IDX(side_4)]; index_10++) {
                ...
                if (current->m_effectAnimationStart != ARMY_ANIMATION_NONE
                    ...
                    if (current->m_animationSequence != current->m_effectAnimationStart
                        ...
                        if (!gbNoShowCombat
                            && current->m_effectAnimationStart == ARMY_ANIMATION_WINCE) {
                            gpSoundManager->MemorySample(gpCombatManager->m_armies[IDX(side_4)][index_10]
    ...
    for (side_4 = COMBAT_ATTACKER_SIDE; IDX(side_4) < COMBAT_SIDE_COUNT; side_4++) {
        for (index_10 = 0; index_10 < gpCombatManager->m_armyCount[IDX(side_4)]; index_10++) {
            gpCombatManager->m_armies[IDX(side_4)][index_10].WaitSample(ARMY_SAMPLE_WINCE);
        }
    }
}
...
void army::GoBerserk(void) {
    ...
    if (attackMask != ARMY_ALL_ATTACK_DIRECTIONS) {
        ...
        } while (attackMask & BIT(direction_4));
    ...
    }
    {
        ...
        for (side_8 = COMBAT_ATTACKER_SIDE; IDX(side_8) < COMBAT_SIDE_COUNT; side_8++) {
            for (index_11 = 0; index_11 < gpCombatManager->m_armyCount[IDX(side_8)]; index_11++) {
                if (side_8 == m_side && index_11 == m_index) {
                    continue;
                }
                army* candidate_16 = &gpCombatManager->m_armies[IDX(side_8)][index_11];
        ...
        if (nearestIndex_19 != -1 && HAS(m_monster.flags.all, MONSTER_FLAGS_SHOOTER)
            ...
            giNextActionGridIndex =
                gpCombatManager->m_armies[IDX(nearestSide_8)][nearestIndex_19].m_hex;
        } else {
...
walkToward:
    if (HAS(m_monster.flags.all, MONSTER_FLAGS_FLYING)
        || (!gpCombatManager->WalkTowardArmy(this, m_side, masks_28[IDX(m_side)])
