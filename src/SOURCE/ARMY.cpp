...
void army::DrawToBuffer(i32 x, i32 y, i32 effectsOnly) {
    ...
    if (standing7 && gpCombatManager->m_showArmyQuantities && m_showQuantity) {
        ...
        neighborOccupied14 = gpCombatManager->m_hexCells[neighboringHex].m_occupantSide != COMBAT_SIDE_NONE;
        quantityOffset0 = m_frameInfo.quantityX[(OppositeArmyFacing(m_facing))];
        ...
        goodEffects0 = 0;
        badEffects0 = 0;
        for (effect6 = ARMY_SPELL_INFLUENCE_HASTE;
             effect6 < ARMY_SPELL_INFLUENCE_COUNT;
             effect6++) {
        ...
        if (giSpellEffectShowType == SPELL_EFFECT_DISPLAY_HIGHLIGHT || !standing7
            || m_spellCount < 1) {
            drawn1 =
                gpCombatManager->m_combatIcons[(COMBAT_ICON_STATUS)]->CombatClipDrawToBuffer(
                    quantityX5,
                    quantityY0,
...
void army::Walk(CombatHexDirection direction, i32 finishStanding, i32 skipDrawing) {
    ...
    if (m_side == COMBAT_DEFENDER_SIDE && gpCombatManager->m_inCastleCombat
        && (destination_3 == COMBAT_CASTLE_GATE_APPROACH_HEX
            || destination_3 == (COMBAT_CASTLE_HEX_GATE)
    ...
    if (m_monsterType == CREATURE_HYDRA) {
        ...
        if (target_1 && target_1->m_quantity > 0
            ...
            if (gbRemoteOn && gpCombatManager->m_networkArmyPresent[0]
                ...
                target_1->m_quantity +=
                    gpCombatManager
                        ->m_killBenefit[(gpCombatManager->m_hexCells[target_1->m_hex]
                                               .m_occupantSide)];
            }
        }
...
i32 army::Damage(i32l damage, SpellType spell) {
    ...
    m_facing = static_cast<ArmyFacing>(
        ((gpCombatManager->m_armies[0]
             + (gpCombatManager->m_currentArmySide) * COMBAT_ARMY_STORAGE_SLOT_COUNT
...
void army::PowEffect(
    ...
    if (effectX != NO_POW_EFFECT_COORDINATE) {
        ...
        for (side_4 = COMBAT_ATTACKER_SIDE; (side_4) < COMBAT_SIDE_COUNT; side_4++) {
...
void army::SpellEffect(
    ...
    if (!gbNoShowCombat) {
        if (animateCreature) {
            ...
            m_animationSequence = ARMY_ANIMATION_WINCE_RETURN;
            for (frame_4 = 0; frame_4 < m_frameInfo.animationFrameCount[(ARMY_ANIMATION_WINCE_RETURN)];
                 frame_4++) {
                m_animationFrame = frame_4;
                glTimers[1] = static_cast<i32>(
                    KBTickCount() + gfCombatSpeedMod[gConfig.combatSpeed] * frameDelay_2
...
void army::GoBerserk(void) {
    i32 masks_28[(BERSERK_MASK_COUNT)];
    ...
    masks_28[(BERSERK_MASK_TARGET_FOUND)] = 0;
    direction_4 = COMBAT_DIRECTION_NORTHEAST;
    masks_28[(BERSERK_MASK_UNUSED)] = 0;
    savedQuantity_10 = m_quantity;
    m_quantity = 0;
    masks_28[(BERSERK_MASK_SIDE_ZERO)] = gpCombatManager->GetAllMask(COMBAT_ATTACKER_SIDE);
    masks_28[(BERSERK_MASK_SIDE_ONE)] = gpCombatManager->GetAllMask(COMBAT_DEFENDER_SIDE);
    m_quantity = savedQuantity_10;
    masks_28[(BERSERK_MASK_ATTACK)] =
        GetAttackMask(m_hex, ARMY_ATTACK_TARGET_OCCUPIED, ARMY_HEX_INVALID);
    if (masks_28[(BERSERK_MASK_ATTACK)] != ARMY_ALL_ATTACK_DIRECTIONS) {
        do {
            if (!masks_28[(BERSERK_MASK_TARGET_FOUND)]) {
        ...
        } while (masks_28[(BERSERK_MASK_ATTACK)] & (1 << (direction_4)));
        ...
        masks_28[(BERSERK_MASK_TARGET_FOUND)] = 1;
        goto berserkFinish;
    }
    {
        nearestIndex_19 = -1;
        ...
        if (nearestIndex_19 != -1 && (((m_monster.flags.all) & (MONSTER_FLAGS_SHOOTER)))
            && m_monster.shots > 0) {
            giNextAction = ACTION_MOVE;
            ...
            if (gpCombatManager->AttemptAttack(
                    ...
                    masks_28[(BERSERK_MASK_SIDE_ZERO)]
                )) {
                giNextAction = ACTION_MOVE;
            ...
            if (gpCombatManager->AttemptAttack(
                    ...
                    masks_28[(BERSERK_MASK_SIDE_ONE)]
                )) {
                giNextAction = ACTION_MOVE;
...
float army::SpellCastWorkChance(SpellType spell) {
    ...
    if ((spell == SPELL_RESURRECT || spell == SPELL_TRUE_RESURRECT)
        && ((((m_monster.flags.all) & (MONSTER_FLAGS_UNDEAD))) || m_initialQuantity == m_quantity)) {
        return ARMY_SPELL_CHANCE_NONE;
    }
