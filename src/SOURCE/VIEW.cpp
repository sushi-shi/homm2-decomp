...
i32 combatManager::ViewGeneral(
    ...
    message15.payload.widget.data.value = H2EnumIndex(WIDGET_FLAG_DRAW);
    generalWindow18->BroadcastMessage(message15);
    if (m_heroes[H2EnumIndex(side)]->m_isCaptain) {
        message15.payload.widget.command = VIEW_GENERAL_SET_FRAME;
        if (m_playerId[H2EnumIndex(side)] == -1)
    ...
    message15.payload.widget.data.value = gpGame->GetPlayerColor(m_heroes[H2EnumIndex(side)]->m_owner) + 1;
    generalWindow18->BroadcastMessage(message15);
    ...
    morale8 = m_heroes[H2EnumIndex(side)]->m_army.GetMorale(
        m_heroes[H2EnumIndex(side)],
        m_combatTowns[H2EnumIndex(side)],
        gpCombatManager->m_armyGroups[H2EnumIndex(OppositeCombatSide(side))]
    );
    luck10 = gpGame->GetLuck(m_heroes[H2EnumIndex(side)], NULL, m_combatTowns[H2EnumIndex(side)]);
    sprintf(
        gText,
        "\n%s%d\n%s%d\n%s%d\n%s%d\n%s%s\n%s%s\n\n%s%d/%d",
    ...
    if (m_heroes[H2EnumIndex(side)] == NULL || allowActions == 0
        ...
        message15.payload.widget.data.value = H2EnumIndex(WIDGET_FLAG_ENABLED);
        generalWindow18->BroadcastMessage(message15);
        message15.payload.widget.command = WIDGET_COMMAND_SET_FLAGS;
        message15.payload.widget.data.value = H2EnumIndex(WIDGET_COMMAND_DIMMED);
        generalWindow18->BroadcastMessage(message15);
    }
