...
i32 combatManager::ViewGeneral(
    ...
    morale = m_heroes[H2EnumIndex(side)]->m_army.GetMorale(
        m_heroes[H2EnumIndex(side)],
        m_combatTowns[H2EnumIndex(side)],
        gpCombatManager->m_armyGroups[H2EnumIndex(OppositeCombatSide(side))]
    );
    luck = gpGame->GetLuck(m_heroes[H2EnumIndex(side)], NULL, m_combatTowns[H2EnumIndex(side)]);
    utf8::Format(
        gText, GLOBAL_TEXT_BUFFER_SIZE,
        "\n%s%d\n%s%d\n%s%d\n%s%d\n%s%s\n%s%s\n\n%s%d/%d",
    ...
    if (quickView == 0)
        DoCommand(CombatMessageCommandFromCode(gpWindowManager->m_dialogResult));
    return 0;
}
