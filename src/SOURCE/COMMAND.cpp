...
void combatManager::CheckSetMouseDirection(i32 mouseX, i32 mouseY, i32 targetHex) {
    ...
    army* currentArmy = &m_armies[(m_currentArmySide)][m_currentArmyIndex];
    army* targetArmy1 = &m_armies[(currentArmy->m_targetSide)][currentArmy->m_targetIndex];
...
CombatMessageCommand combatManager::GetCommand(i32 hexIndex) {
    ...
    switch (hexIndex) {
        ...
        default: {
            ...
            army* ourArmy = &m_armies[(m_currentArmySide)][m_currentArmyIndex];
                ...
                if (side != COMBAT_SIDE_NONE) {
                    switch (side) {
                        ...
                        case COMBAT_DEFENDER_SIDE:
                            gpMouseManager->SetPointer(COMBAT_POINTER_DEFAULT);
                            ViewArmy(
                                &m_armies[(side)][m_hexCells[m_selectedHex].m_occupantIndex], 1
                            );
                            ResetMouse();
                            return 0;
                    }
...
void combatManager::DoVictory(CombatResult winningSide) {
    ...
    for (combatSide = COMBAT_ATTACKER_SIDE; (combatSide) < COMBAT_SIDE_COUNT; ++combatSide) {
        living = 0;
        lastLivingArmy0 = -1;
        for (i = 0; i < gpCombatManager->m_armyCount[(combatSide)]; ++i) {
            currentArmy26 = &m_armies[(combatSide)][i];
            ...
            if (CombatResultForSide(combatSide) == winningSide && currentArmy26->m_quantity > 0
                && (((currentArmy26->m_monster.flags.all) & (MONSTER_FLAGS_LIGHT_PALETTE))) == 0
        ...
        if (living == 0 && lastLivingArmy0 != -1)
            m_armies[(combatSide)][lastLivingArmy0].m_quantity = 1;
    }
                ...
                if (gbRetreatWin == 0) {
                    ...
                    if (m_heroes[(COMBAT_ATTACKER_SIDE)] != NULL
                        ...
                        for (fadeIndex26 = 0; fadeIndex26 < HERO_ARTIFACT_SLOT_COUNT; ++fadeIndex26) {
                            if (m_heroes[(winningSide)]->m_artifacts[fadeIndex26]
                                == ARTIFACT_NONE) {
                                ++emptySlots;
                            }
                        }
                        for (fadeIndex26 = 0; fadeIndex26 < HERO_ARTIFACT_SLOT_COUNT; ++fadeIndex26) {
                            if (m_heroes[(OppositeCombatResult(winningSide))]->m_artifacts[fadeIndex26]
...
i32 combatManager::DoSurrender(void) {
    ...
    if (m_heroes[(OppositeCombatSide(m_currentSide))]->m_isCaptain != 0) {
        ...
        if (m_playerId[(OppositeCombatSide(m_currentSide))] == -1)
...
void combatManager::ResetCyclingCreatures(void) {
    ...
    for (side = COMBAT_ATTACKER_SIDE; (side) < COMBAT_SIDE_COUNT; ++side) {
        for (index = 0; index < gpCombatManager->m_armyCount[(side)]; ++index) {
            ...
            if ((((currentArmy_p->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                    == 0
...
void combatManager::CycleCombatScreen(void) {
    ...
    for (side = COMBAT_ATTACKER_SIDE; (side) < COMBAT_SIDE_COUNT; ++side) {
        if (m_heroOverlayIcons[(side)] == NULL) {
            ...
            if (m_heroes[(side)] != NULL)
                m_heroOverlayFrame[(side)] = (m_heroOverlayFrame[(side)] + 1) % COMBAT_HERO_OVERLAY_FRAME_COUNT;
    ...
    for (side = COMBAT_ATTACKER_SIDE; (side) < COMBAT_SIDE_COUNT; ++side) {
        ...
        if (m_heroIcons[(side)] == NULL) {
            ...
            if (m_heroAnimationState[(side)] == HERO_ANIMATION_DEATH_FIRST
                ...
                if (sCmbtHero[m_heroSpriteIndex[(side)]].idleAnimationCount > 1) {
    ...
    if (nextHeroAnimation[COMBAT_SIDE_COUNT] == 0 && m_drawHero[(COMBAT_ATTACKER_SIDE)] == 0
...
void combatManager::AddArmy(
    ...
    ResetLimitCreature();
    ++m_limitCreatureCount[(side)][armyIndex_r];
