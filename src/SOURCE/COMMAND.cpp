...
typedef i32 CombatBattleResultText;
    enum {
        CONTROL_ATTACK = 1,
        CONTROL_WAIT = 2,
...
typedef i32 CombatActionDataIndex;
...
typedef i32 CombatHelpTextIndex;
    enum {
        LONG_HELP_ATTACK = 0,
        LONG_HELP_WAIT = 1,
...
typedef i32 CombatLongHelpIndex;
...
void combatManager::SetCombatDirections(i32 targetHex) {
    ...
    i32 previous_1;
    i32 next_28;
    army* currentArmy_1 = &m_armies[(m_currentArmySide)][m_currentArmyIndex];
    ...
    army* targetArmy_13 = &m_armies[(targetSide_28)][targetIndex_9];

    i32 direction_28;
    for (direction_28 = 0; direction_28 < COMBAT_DIRECTION_COUNT; direction_28++) {
        if (direction_28 == (COMBAT_DIRECTION_WIDE_WEST)
            || direction_28 == (COMBAT_DIRECTION_WIDE_EAST)) {
            if ((((currentArmy_1->m_monster.flags.all) & (MONSTER_FLAGS_WIDE))) != 0) {
                if (currentArmy_1->m_facing == ARMY_FACING_RIGHT) {
                    if (direction_28 == (COMBAT_DIRECTION_WIDE_WEST))
                        directionHexes[direction_28] =
                            m_adjacency[targetHex][(COMBAT_DIRECTION_NORTHWEST)];
                    if (direction_28 == (COMBAT_DIRECTION_WIDE_EAST))
                        directionHexes[direction_28] =
                            m_adjacency[targetHex][(COMBAT_DIRECTION_SOUTHWEST)];
                } else {
                    if (direction_28 == (COMBAT_DIRECTION_WIDE_WEST))
                        directionHexes[direction_28] =
                            m_adjacency[targetHex][(COMBAT_DIRECTION_NORTHEAST)];
                    if (direction_28 == (COMBAT_DIRECTION_WIDE_EAST))
                        directionHexes[direction_28] =
                            m_adjacency[targetHex][(COMBAT_DIRECTION_SOUTHEAST)];
                }
            } else {
        ...
        if ((((currentArmy_1->m_monster.flags.all) & (MONSTER_FLAGS_WIDE))) != 0
            && directionHexes[direction_28] != INVALID_HEX) {
            if (currentArmy_1->m_facing == ARMY_FACING_RIGHT) {
                if (direction_28 == (COMBAT_DIRECTION_NORTHWEST)
                    || direction_28 == (COMBAT_DIRECTION_WEST)
                    || direction_28 == (COMBAT_DIRECTION_SOUTHWEST)) {
                ...
                if (direction_28 == (COMBAT_DIRECTION_NORTHEAST)
                    || direction_28 == (COMBAT_DIRECTION_EAST)
                    || direction_28 == (COMBAT_DIRECTION_SOUTHEAST)) {
                    if (directionHexes[direction_28] % COMBAT_GRID_ROW_LENGTH
                        == COMBAT_GRID_REVERSE_FIRST_COLUMN)
                        directionHexes[direction_28] = INVALID_HEX;
                    else
    ...
    if ((((currentArmy_1->m_monster.flags.all) & (MONSTER_FLAGS_FLYING))) != 0) {
        for (direction_28 = 0; direction_28 < COMBAT_DIRECTION_COUNT; direction_28++)
    ...
    if (m_validDirectionCount == 0)
        pathValid_28[(COMBAT_DIRECTION_WIDE_WEST)] = true;
    ...
    for (direction_28 = 0; direction_28 < COMBAT_DIRECTION_COUNT; direction_28++) {
        ...
        if (direction_28 < COMBAT_DIRECTION_ADJACENT_COUNT)
            ...
            mappedDirection_5 = direction_28 == (COMBAT_DIRECTION_WIDE_WEST)
                ? (COMBAT_DIRECTION_WIDE_EAST)
                : (COMBAT_DIRECTION_WIDE_WEST);

        if (pathValid_28[mappedDirection_5] != 0) {
            if ((((targetArmy_13->m_monster.flags.all) & (MONSTER_FLAGS_WIDE))) != 0) {
                if (direction_28 == (COMBAT_DIRECTION_NORTHEAST)
                    && HEX_HAS_OCCUPANT(m_hexCells[targetHex - 1], targetSide_28, targetIndex_9)) {
                    outputDirection_7 = (COMBAT_DIRECTION_WIDE_WEST);
                } else if (direction_28 == (COMBAT_DIRECTION_NORTHWEST)
                           && HEX_HAS_OCCUPANT(
                    ...
                    outputDirection_7 = (COMBAT_DIRECTION_WIDE_WEST);
                } else if (direction_28 == (COMBAT_DIRECTION_SOUTHEAST)
                           && HEX_HAS_OCCUPANT(
                               m_hexCells[targetHex - 1],
                               targetSide_28,
                               targetIndex_9
                           )) {
                    outputDirection_7 = (COMBAT_DIRECTION_WIDE_EAST);
                } else if (direction_28 == (COMBAT_DIRECTION_SOUTHWEST)
                           && HEX_HAS_OCCUPANT(
                               m_hexCells[targetHex + 1],
                               targetSide_28,
                               targetIndex_9
            ...
            } else if (direction_28 == (COMBAT_DIRECTION_WIDE_WEST)) {
        ...
        for (direction_28 = 0; direction_28 < DIRECTION_SECTOR_COUNT; direction_28++) {
            if (m_directionMap[direction_28] == -1) {
                ...
                if (m_directionMap[next_28] >= 0
                    && m_directionMap[next_28] <= (COMBAT_DIRECTION_WIDE_EAST)) {
...
void combatManager::CheckSetMouseDirection(i32 mouseX, i32 mouseY, i32 targetHex) {
    ...
    army* targetArmy_13 [[maybe_unused]] =
    ...
    m_directionTargetHex = m_adjacency[targetHex][(direction_5)];
...
MessageDispatchResult combatManager::ProcessCombatMsg(tag_message& message) {
    ...
    i32 unusedResult [[maybe_unused]] = 0;
    i32 selectedHex_0;
        ...
        case MESSAGE_MOUSE_MOVE:
            ...
            if (InCombatArea(message.payload.mouse.screenX, message.payload.mouse.screenY) != 0) {
                ...
                if (mouseX >= CONTROL_RIGHT_MIN_X) {
                    CombatMessage(cCombatHelp[(HELP_SKIP_UNIT)], 1, 0, 0);
                } else if (mouseX <= CONTROL_LEFT_MAX_X && mouseY < CONTROL_SYSTEM_OPTIONS_MIN_Y) {
                    CombatMessage(cCombatHelp[(HELP_AUTO_COMBAT)], 1, 0, 0);
                } else if (mouseX <= CONTROL_LEFT_MAX_X) {
                    CombatMessage(cCombatHelp[(HELP_SYSTEM_OPTIONS)], 1, 0, 0);
                } else {
                    CombatMessage(cCombatHelp[(HELP_OTHER_CONTROL)], 1, 0, 0);
                }
        ...
        case MESSAGE_KEY_DOWN:
            switch (static_cast<CombatKeyCommand>(message.payload.keyboard.keyCode)) {
                ...
                case KEY_DEBUG_CREATURE_EFFECT:
                    if ((((message.payload.keyboard.modifiers) & (MESSAGE_MODIFIER_SHIFT_KEYS)))) {
                        VaporizeCreature(COMBAT_DEFENDER_SIDE, 1);
                    } else if ((((message.payload.keyboard.modifiers) & (MESSAGE_MODIFIER_CONTROL_KEYS)))) {
                ...
                case KEY_CAST_SPELL:
                    if (m_heroes[(m_currentSide)] == NULL) {
                        ...
                        NormalDialog(
                            "Сфера антимагии в действии. Не может быть направлено ни одно боевое заклинание."
                            ,
                            NORMAL_DIALOG_INFO
                        );
...
void combatManager::ResetRound(void) {
    i32 unusedResetRoundWord6 [[maybe_unused]];
...
CombatMessageCommand combatManager::GetCommand(i32 hexIndex) {
    i32 column [[maybe_unused]] = hexIndex % COMBAT_GRID_ROW_LENGTH;
    i32 rowPos [[maybe_unused]] = hexIndex / COMBAT_GRID_ROW_LENGTH;
    ...
    switch (hexIndex) {
        ...
        default: {
            ...
            ourArmy_13 = &m_armies[(m_currentArmySide)][m_currentArmyIndex];
...
i32 combatManager::RightClick(i32 hexIndex) {
    i32 col [[maybe_unused]] = hexIndex % COMBAT_GRID_ROW_LENGTH;
    i32 row [[maybe_unused]] = hexIndex / COMBAT_GRID_ROW_LENGTH;
    if (hexIndex == INVALID_HEX)
        return 0;
    ...
    switch (hexIndex) {
        ...
        default:
            ...
            i32 armyIdx [[maybe_unused]] = m_hexCells[hexIndex].m_occupantIndex;
            if (m_hexCells[hexIndex].m_blocked != 0 && !CAN_PASS_CASTLE_GATE(hexIndex)) {
                return 0;
            } else {
...
void combatManager::DoCommand(CombatMessageCommand command) {
    i32 unusedCommandWord2 [[maybe_unused]];
    i32 unusedCommandWord5 [[maybe_unused]];
    army* currentArmy [[maybe_unused]] = &m_armies[(m_currentArmySide)][m_currentArmyIndex];
    switch (command) {
        ...
        case COMBAT_MESSAGE_COMMAND_RETREAT:
            NormalDialog(
                "Вы действительно хотите отступить?"
                ,
...
MessageDispatchResult WinCombatHandler(struct tag_message& message) {
    ...
    i32 finalDelay [[maybe_unused]] = WIN_LOSE_INITIAL_DELAY;
    i32 frame;
    i32 iDelay_3;
...
void combatManager::ShowWinLoseArtifact(
    ...
    i16 width [[maybe_unused]] = CASUALTY_WINDOW_WIDTH;
    i16 bottom_4 [[maybe_unused]] = CASUALTY_WINDOW_BOTTOM;
    tag_message message;
    char* artifactName;
...
void combatManager::ShowSkeletons(class heroWindow* window) {
    ...
    if (giSkeletonsCreated > 1) {
        sprintf(
            gText,
            "Использование черной магии некромантии позволило вам вернуть на службу %d поверженных врагов в виде скелетов."
...
,
        ...
        sprintf(
            gText,
            "Использование черной магии некромантии позволило вам вернуть на службу поверженного врага в виде скелета."
...
void combatManager::ShowEagleEyeSpell(class heroWindow* window) {
    SpellType newSpell = m_eagleEyeSpell[(m_combatResult)];
    i32 x = EAGLE_PANEL_X;
    i32 y = EAGLE_PANEL_Y;
    tag_message message;
    char* nameText;
    ...
    m_winLoseBottomWidgets[1] = new iconWidget(
        ...
        EAGLE_ICON_HEIGHT,
        "spells.icn",
        static_cast<i16>(gsSpellInfo[(newSpell)].iconIndex),
        ICON_DRAW_NORMAL,
    ...
    sprintf(nameText, "%s", gSpellNames[(newSpell)]);
    m_winLoseBottomTextWidgets[0] = new textWidget(
    ...
    sprintf(
        gText,
        "С помощью искусства орлиного взора, %s смог(ла) подглядеть и выучить заклинание '%s'."
,
        m_heroes[(m_combatResult)]->m_name,
        gSpellNames[(newSpell)]
    );
    SET_WIDGET_MESSAGE(message, COMBAT_WIN_LOSE_TEXT_COMMAND, WIN_LOSE_TEXT_ID);
    message.payload.widget.data.text = gText;
    m_winLoseWindow->BroadcastMessage(message);
...
void combatManager::ShowDeadArmies(class heroWindow* window) {
    ...
    i32 side_4;
    i32 unusedCasualtyWord116 [[maybe_unused]];
    i32 unusedCasualtyWord92 [[maybe_unused]];
    i32 unusedCasualtyWord86 [[maybe_unused]];
    i32 unusedCasualtyWord52 [[maybe_unused]];
    i32 unusedCasualtyWord46 [[maybe_unused]];
    i32 unusedCasualtyWord13 [[maybe_unused]];
    i32 unusedCasualtyWord0 [[maybe_unused]];
        ...
        for (armyIndex = 0; armyIndex < COMBAT_ARMY_SLOT_COUNT; ++armyIndex) {
            if (m_armies[side_4][armyIndex].m_monsterType != CREATURE_NONE
                ...
                casualtyType_1[side_4][casualtyQuantity_13[side_4]] =
                    (m_armies[side_4][armyIndex].m_monsterType);
...
void combatManager::DoVictory(CombatResult winningSide) {
    ...
    for (combatSide = COMBAT_ATTACKER_SIDE; (combatSide) < COMBAT_SIDE_COUNT; ++combatSide) {
        livingCount = 0;
        lastLivingArmy = -1;
        for (troopIndex = 0; troopIndex < gpCombatManager->m_armyCount[(combatSide)];
             ++troopIndex) {
            pTroop = &m_armies[(combatSide)][troopIndex];
            ...
            if (CombatResultForSide(combatSide) == winningSide && pTroop->m_quantity > 0
                && (((pTroop->m_monster.flags.all) & (MONSTER_FLAGS_LIGHT_PALETTE))) == 0
        ...
        if (livingCount == 0 && lastLivingArmy != -1)
            m_armies[(combatSide)][lastLivingArmy].m_quantity = 1;
    }
        ...
        case COMBAT_RESULT_DEFENDER:
            if (m_heroes[(winningSide)] != NULL) {
                if (m_eagleEyeSpell[(winningSide)] != SPELL_NONE) {
                    m_heroes[(winningSide)]->m_spells[(m_eagleEyeSpell[(winningSide)])] =
                        1;
                }
                m_experienceValue[(OppositeCombatResult(winningSide))] =
                ...
                if (gbRetreatWin == 0) {
                    ...
                    if (m_heroes[(COMBAT_ATTACKER_SIDE)] != NULL
                        ...
                        for (loop = 0; loop < HERO_ARTIFACT_SLOT_COUNT; ++loop) {
                            if (m_heroes[(winningSide)]->m_artifacts[loop]
                                == ARTIFACT_NONE) {
                                ++emptySlots;
                            }
                        }
                        for (loop = 0; loop < HERO_ARTIFACT_SLOT_COUNT; ++loop) {
                            if (m_heroes[(OppositeCombatResult(winningSide))]
                                        ->m_artifacts[loop]
                                    >= ARTIFACT_ARCANE_NECKLACE
                                && m_heroes[(OppositeCombatResult(winningSide))]
                                           ->m_artifacts[loop]
                                       != ARTIFACT_MAGIC_BOOK
            ...
            if (!(giCurPlayer == -1 || gbThisNetHumanPlayer[giCurPlayer] == 0
                ...
                if (m_heroes[(winningSide)] != NULL) {
                    if (gbCombatSurrender != 0) {
                        sprintf(gText, cBattleResults[(RESULT_TEXT_ENEMY_SURRENDERED)]);
                    } else if (gbRetreatWin != 0) {
                        sprintf(gText, cBattleResults[(RESULT_TEXT_ENEMY_FLED)]);
                    } else {
                        sprintf(gText, cBattleResults[(RESULT_TEXT_VICTORY)]);
                    }
                    if (levelsGained > 0 && winningSide == COMBAT_RESULT_DEFENDER
                        && giNumHumanPlayers > 1) {
                        sprintf(
                            experienceText,
                            cBattleResults[(RESULT_TEXT_NETWORK_EXPERIENCE)],
                        ...
                        sprintf(
                            experienceText,
                            cBattleResults[(RESULT_TEXT_EXPERIENCE)],
                            m_heroes[(winningSide)]->m_name,
                            m_experienceValue[(OppositeCombatResult(winningSide))]
                        );
                    }
                ...
                if (m_heroes[(OppositeCombatResult(winningSide))] != NULL)
                    m_heroes[(OppositeCombatResult(winningSide))]->ApplyBattleLossTemps();
            } else {
                if (m_heroes[(winningSide)] != NULL)
                    m_heroes[(winningSide)]->ApplyBattleWinTemps();
                if (m_heroes[(OppositeCombatResult(winningSide))] != NULL)
...
void combatManager::DoLoseWindow(void) {
    ...
    i32 unusedLoseWord [[maybe_unused]] = (COMBAT_RESULT_ATTACKER);
    tag_message message;
    ...
    if (m_heroes[(losingSide)] != NULL) {
        if (gbCombatSurrender != 0) {
            sprintf(
                gText,
                cBattleResults[(RESULT_TEXT_HERO_SURRENDER)],
                m_heroes[(losingSide)]->m_name
            );
        } else if (gbRetreatWin != 0) {
            sprintf(
                gText,
                cBattleResults[(RESULT_TEXT_HERO_FLEE)],
                m_heroes[(losingSide)]->m_name
            );
        } else {
            sprintf(
                gText,
                cBattleResults[(RESULT_TEXT_HERO_DEFEAT)],
                m_heroes[(losingSide)]->m_name
            );
        }
    } else {
        if (gbCombatSurrender != 0) {
            sprintf(gText, cBattleResults[(RESULT_TEXT_FORCES_SURRENDER)]);
        } else if (gbRetreatWin != 0) {
            sprintf(gText, cBattleResults[(RESULT_TEXT_FORCES_FLEE)]);
        } else {
            sprintf(gText, cBattleResults[(RESULT_TEXT_FORCES_DEFEAT)]);
        }
    }
...
i32 combatManager::DoSurrender(void) {
    i32 armyNum;
    i16 dlgType [[maybe_unused]];
    i16 dlgResult [[maybe_unused]];
    i16 lineWidth [[maybe_unused]];
    heroWindow* window;
    tag_message message;

    giSurrenderCost = 0;
    for (armyNum = 0; armyNum < COMBAT_ARMY_SLOT_COUNT; ++armyNum) {
        if (m_armies[(m_currentSide)][armyNum].IsAlive()) {
            giSurrenderCost += m_armies[(m_currentSide)][armyNum].m_quantity
                * gMonsterDatabase[(m_armies[(m_currentSide)][armyNum].m_monsterType)]
                      .cost;
        }
    }
    ...
    sprintf(
        gText,
        "port%04d.icn",
        static_cast<i32>(m_heroes[(OppositeCombatSide(m_currentSide))]->m_portrait)
    );
    message.payload.widget.data.text = gText;
    window->BroadcastMessage(message);
    message.payload.widget.command =
        m_heroes[(OppositeCombatSide(m_currentSide))]->m_isCaptain
    ...
    window->BroadcastMessage(message);
    if (m_heroes[(OppositeCombatSide(m_currentSide))]->m_isCaptain != 0) {
        message.payload.widget.command = COMBAT_SURRENDER_CAPTAIN_OVERLAY_COMMAND;
        message.payload.widget.data.value =
            m_playerId[(OppositeCombatSide(m_currentSide))] == -1
            ? COMBAT_NEUTRAL_HERO_COLOR
            : gpGame->m_players[m_playerId[(OppositeCombatSide(m_currentSide))]].m_color;
        window->BroadcastMessage(message);
    }
    message.payload.widget.data.text = gText;
...
MessageDispatchResult combatManager::ProcessNextAction(struct tag_message& message) {
    i32 actionData[(ACTION_DATA_COUNT)];
    i32 transmitResult;
    army* actingArmy_29;
    b32 shouldAdvance;
    b32 redraw_10;
    ...
    if (m_smallViewSide[(COMBAT_ATTACKER_SIDE)] != COMBAT_SIDE_NONE
        || m_smallViewSide[(COMBAT_DEFENDER_SIDE)] != COMBAT_SIDE_NONE) {
        m_smallViewSide[(COMBAT_DEFENDER_SIDE)] = COMBAT_SIDE_NONE;
        m_smallViewSide[(COMBAT_ATTACKER_SIDE)] = COMBAT_SIDE_NONE;
        redraw_10 = true;
    }
    if (giNextAction != ACTION_NONE) {
        LogInt(
    ...
    if (gbThisNetHasControl != 0 && gbRemoteOn != 0 && m_playerId[(COMBAT_ATTACKER_SIDE)] >= 0
        ...
        && gbHumanPlayer[m_playerId[(COMBAT_DEFENDER_SIDE)]] != 0
        && gbHumanPlayer[m_playerId[(COMBAT_ATTACKER_SIDE)]] != 0) {
        actionData[(ACTION_DATA_ACTION)] = (giNextAction);
        actionData[(ACTION_DATA_EXTRA)] = giNextActionExtra;
        actionData[(ACTION_DATA_GRID)] = giNextActionGridIndex;
        actionData[(ACTION_DATA_SECOND_GRID)] = giNextActionGridIndex2;
        LogInt("About to T", iCombatControlNetPos[(COMBAT_DEFENDER_SIDE) - (m_currentSide)]);
    ...
    actingArmy_29 = &m_armies[(m_currentArmySide)][m_currentArmyIndex];
    shouldAdvance = false;
    if (CheckWin(&message) != 0)
        goto Finished;
    switch (giNextAction) {
        ...
        case ACTION_CAST_SPELL:
            ...
            if (m_armies[(m_currentArmySide)][m_currentArmyIndex].m_quantity <= 0)
                shouldAdvance = true;
            ResetCycleTimers();
            break;
        case ACTION_MOVE:
...
void combatManager::ResetCyclingCreatures(void) {
    ...
    i32 unusedRotateWord [[maybe_unused]];

    for (sideIndex = COMBAT_ATTACKER_SIDE; (sideIndex) < COMBAT_SIDE_COUNT; ++sideIndex) {
        for (index = 0; index < gpCombatManager->m_armyCount[(sideIndex)]; ++index) {
            currentTroop = &gpCombatManager->m_armies[(sideIndex)][index];
            if ((((currentTroop->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                    == 0
                ...
                ++rotateCount;
                ++gpCombatManager->m_limitCreatureCount[(sideIndex)][index];
            }
        }
    }
    if (rotateCount == 0)
...
void combatManager::ResetCycleTimers(void) {
    ...
    m_heroCycleTimer[(COMBAT_ATTACKER_SIDE)] = KBTickCount();
    m_heroCycleTimer[(COMBAT_DEFENDER_SIDE)] = KBTickCount();
    for (which = COMBAT_ATTACKER_SIDE; (which) < COMBAT_SIDE_COUNT; ++which) {
        for (i = 0; i < gpCombatManager->m_armyCount[(which)]; ++i) {
            ap = &gpCombatManager->m_armies[(which)][i];
...
void combatManager::CycleCombatScreen(void) {
    ...
    for (side_7 = COMBAT_ATTACKER_SIDE; (side_7) < COMBAT_SIDE_COUNT; ++side_7) {
        for (index_0 = 0; index_0 < gpCombatManager->m_armyCount[(side_7)]; ++index_0) {
            currentArmy_2 = gpCombatManager->m_armies[(side_7)] + index_0;
            if ((((currentArmy_2->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                    == 0
                && currentArmy_2->m_spellInfluence[(ARMY_SPELL_INFLUENCE_PARALYZE)] == 0
                && currentArmy_2->m_spellInfluence[(ARMY_SPELL_INFLUENCE_BLIND)] == 0
                && currentArmy_2->m_spellInfluence[(ARMY_SPELL_INFLUENCE_PETRIFIED)] == 0
                && ((currentArmy_2->m_animationSequence >= COMBAT_CREATURE_CYCLE_SEQUENCE_FIRST
                    ...
                    || (currentArmy_2->m_animationSequence == ARMY_ANIMATION_STAND
                        && currentArmy_2->m_lastAnimationTime
                               ...
                               < KBTickCount()))) {
                ++cyclingCount_1;
                ++cycleArmy[(side_7)][index_0];
                ++m_limitCreatureCount[(side_7)][index_0];
            }
        }
    }
...
void combatManager::AddArmy(
    ...
    for (index = 0; index < COMBAT_ARMY_CAPACITY; ++index) {
        ...
        if (m_armies[(side)][index].m_quantity == 0
            && (((m_armies[(side)][index].m_monster.flags.all) & (MONSTER_FLAGS_AI_EXCLUDED))) != 0
            && ((((m_armies[(side)][index].m_monster.flags.all) & (MONSTER_FLAGS_MIRROR_IMAGE))) != 0
                || IS_ELEMENTAL_CREATURE(m_armies[(side)][index].m_monsterType))) {
            armyIdx = index;
            reusedArmy = true;
            break;
        }
    ...
    newStack = &m_armies[(side)][armyIdx];
    ...
    ResetLimitCreature();
    ++m_limitCreatureCount[(side)][armyIdx];
...
void combatManager::SetupSmallView(void) {
    i32 unusedSmallViewWord1 [[maybe_unused]];
    i32 unusedSmallViewWord2 [[maybe_unused]];

    if (m_smallViewSide[(COMBAT_DEFENDER_SIDE)] == COMBAT_SIDE_NONE
...
i8 iTransferArtifacts[COMBAT_TRANSFER_ARTIFACT_COUNT];
i32 giNextAction;
