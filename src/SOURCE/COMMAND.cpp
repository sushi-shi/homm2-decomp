    ...
    if (gbNoShowCombat == 0) {
        if (platform::Ticks() > glTimers[0]) {
            ...
            glTimers[0] = static_cast<i32>(
                platform::Ticks() + gfCombatSpeedMod[gConfig.combatSpeed] * COMBAT_SOUND_POLL_DELAY
            );
        }
        if (platform::Ticks() > glTimers[GLOBAL_COMBAT_CYCLE_TIMER_SLOT]
...
void combatManager::SetCombatDirections(i32 targetHex) {
    ...
    army* currentArmy_3 = &m_armies[H2EnumIndex(m_currentArmySide)][m_currentArmyIndex];
    ...
    army* targetArmy_1 = &m_armies[H2EnumIndex(targetSide_36)][targetIndex_6];

    i32 direction_27;
    for (direction_27 = 0; direction_27 < COMBAT_DIRECTION_COUNT; direction_27++) {
        if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_WIDE_WEST)
            || direction_27 == H2EnumIndex(COMBAT_DIRECTION_WIDE_EAST)) {
            if ((H2EnumIndex((currentArmy_3->m_monster.flags.all) & (MONSTER_FLAGS_WIDE))) != 0) {
                if (currentArmy_3->m_facing == ARMY_FACING_RIGHT) {
                    if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_WIDE_WEST))
                        directionHexes_4[direction_27] = m_adjacency[targetHex][H2EnumIndex(COMBAT_DIRECTION_NORTHWEST)];
                    if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_WIDE_EAST))
                        directionHexes_4[direction_27] = m_adjacency[targetHex][H2EnumIndex(COMBAT_DIRECTION_SOUTHWEST)];
                } else {
                    if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_WIDE_WEST))
                        directionHexes_4[direction_27] = m_adjacency[targetHex][H2EnumIndex(COMBAT_DIRECTION_NORTHEAST)];
                    if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_WIDE_EAST))
                        directionHexes_4[direction_27] = m_adjacency[targetHex][H2EnumIndex(COMBAT_DIRECTION_SOUTHEAST)];
                }
            } else {
        ...
        if ((H2EnumIndex((currentArmy_3->m_monster.flags.all) & (MONSTER_FLAGS_WIDE))) != 0
            && directionHexes_4[direction_27] != INVALID_HEX) {
            if (currentArmy_3->m_facing == ARMY_FACING_RIGHT) {
                if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_NORTHWEST)
                    || direction_27 == H2EnumIndex(COMBAT_DIRECTION_WEST)
                    || direction_27 == H2EnumIndex(COMBAT_DIRECTION_SOUTHWEST)) {
                ...
                if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_NORTHEAST)
                    || direction_27 == H2EnumIndex(COMBAT_DIRECTION_EAST)
                    || direction_27 == H2EnumIndex(COMBAT_DIRECTION_SOUTHEAST)) {
                    if (directionHexes_4[direction_27] % COMBAT_GRID_ROW_LENGTH
                        == COMBAT_GRID_REVERSE_FIRST_COLUMN)
                        directionHexes_4[direction_27] = INVALID_HEX;
                    else
    ...
    if ((H2EnumIndex((currentArmy_3->m_monster.flags.all) & (MONSTER_FLAGS_FLYING))) != 0) {
        for (direction_27 = 0; direction_27 < COMBAT_DIRECTION_COUNT; direction_27++)
    ...
    if (m_validDirectionCount == 0)
        pathValid_1[H2EnumIndex(COMBAT_DIRECTION_WIDE_WEST)] = 1;
    ...
    for (direction_27 = 0; direction_27 < COMBAT_DIRECTION_COUNT; direction_27++) {
        ...
        else if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_WIDE_WEST))
            mappedDirection_7 = H2EnumIndex(COMBAT_DIRECTION_WIDE_EAST);
        else
            mappedDirection_7 = H2EnumIndex(COMBAT_DIRECTION_WIDE_WEST);

        if (pathValid_1[mappedDirection_7] != 0) {
            if ((H2EnumIndex((targetArmy_1->m_monster.flags.all) & (MONSTER_FLAGS_WIDE))) != 0) {
                if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_NORTHEAST)
                    && m_hexCells[targetHex - 1].m_occupantSide == targetSide_36
                    && m_hexCells[targetHex - 1].m_occupantIndex == targetIndex_6) {
                    outputDirection_11 = H2EnumIndex(COMBAT_DIRECTION_WIDE_WEST);
                } else if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_NORTHWEST)
                           && m_hexCells[targetHex + 1].m_occupantSide == targetSide_36
                           && m_hexCells[targetHex + 1].m_occupantIndex == targetIndex_6) {
                    outputDirection_11 = H2EnumIndex(COMBAT_DIRECTION_WIDE_WEST);
                } else if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_SOUTHEAST)
                           && m_hexCells[targetHex - 1].m_occupantSide == targetSide_36
                           && m_hexCells[targetHex - 1].m_occupantIndex == targetIndex_6) {
                    outputDirection_11 = H2EnumIndex(COMBAT_DIRECTION_WIDE_EAST);
                } else if (direction_27 == H2EnumIndex(COMBAT_DIRECTION_SOUTHWEST)
                           && m_hexCells[targetHex + 1].m_occupantSide == targetSide_36
                           && m_hexCells[targetHex + 1].m_occupantIndex == targetIndex_6) {
                    outputDirection_11 = H2EnumIndex(COMBAT_DIRECTION_WIDE_EAST);
                }
        ...
        for (direction_27 = 0; direction_27 < DIRECTION_SECTOR_COUNT; direction_27++) {
            if (m_directionMap[direction_27] == -1) {
                ...
                if (m_directionMap[next_3] >= 0
                    && m_directionMap[next_3] <= H2EnumIndex(COMBAT_DIRECTION_WIDE_EAST)) {
...
void combatManager::CheckSetMouseDirection(i32 mouseX, i32 mouseY, i32 targetHex) {
    ...
    army* currentArmy = &m_armies[H2EnumIndex(m_currentArmySide)][m_currentArmyIndex];
    army* targetArmy1 = &m_armies[H2EnumIndex(currentArmy->m_targetSide)][currentArmy->m_targetIndex];
...
i32 combatManager::IsNegationSphereInEffect(void) {
    ...
    for (side = COMBAT_ATTACKER_SIDE; H2EnumIndex(side) < COMBAT_SIDE_COUNT; side++) {
        if (m_heroes[H2EnumIndex(side)] != NULL && m_heroes[H2EnumIndex(side)]->HasArtifact(ARTIFACT_SPHERE_NEGATION) != 0)
            return 1;
    }
    return 0;
}
...
CombatMessageCommand combatManager::GetCommand(i32 hexIndex) {
    ...
    switch (hexIndex) {
        ...
        default: {
            ...
            army* ourArmy = &m_armies[H2EnumIndex(m_currentArmySide)][m_currentArmyIndex];
            ...
            if (m_hexCells[hexIndex].m_blocked != 0
                ...
                if (m_armies[H2EnumIndex(m_currentArmySide)][m_currentArmyIndex].ValidPath(
                    ...
                    command = CombatMessageCommand(static_cast<i8>(
                        (H2EnumIndex((m_armies[H2EnumIndex(m_currentArmySide)][m_currentArmyIndex]
                            ...
                            ? H2EnumIndex(COMBAT_MESSAGE_COMMAND_FLY)
                            : H2EnumIndex(COMBAT_MESSAGE_COMMAND_MOVE)
                    ));
                }
            }
            break;
                ...
                if (side != COMBAT_SIDE_NONE) {
                    switch (side) {
                        ...
                        case COMBAT_DEFENDER_SIDE:
                            gpMouseManager->SetPointer(COMBAT_POINTER_DEFAULT);
                            ViewArmy(
                                &m_armies[H2EnumIndex(side)][m_hexCells[m_selectedHex].m_occupantIndex], 1
                            );
                            ResetMouse();
                            return 0;
                    }
                ...
                switch (message.payload.widget.id) {
                    case WIN_LOSE_NEXT_CONTROL:
                        ...
                        if (iCurTransferArtifact + 1 < iMaxTransferArtifacts) {
                            ...
                            if (gpCombatManager->m_eagleEyeSpell[H2EnumIndex(gpCombatManager->m_combatResult)]
                                != SPELL_NONE) {
                                ...
                                gpCombatManager->m_eagleEyeSpell[H2EnumIndex(gpCombatManager->m_combatResult)] =
                                    SPELL_NONE;
                            } else {
                            ExitDialog:
                                gpWindowManager->m_dialogResult = message.payload.widget.id;
...
void combatManager::ShowWinLoseArtifact(
    class heroWindow* window, ArtifactType artifact
) {
    ...
    sprintf(capturedArtifactName, gArtifactNames[H2EnumIndex(artifact)]);
...
void combatManager::ShowEagleEyeSpell(class heroWindow* window) {
    ...
    SpellType displayedSpell = m_eagleEyeSpell[H2EnumIndex(m_combatResult)];
    i32 x = EAGLE_PANEL_X;
    i32 y = EAGLE_PANEL_Y;
    ...
    m_winLoseBottomWidgets[1] = new iconWidget(
        ...
        EAGLE_ICON_HEIGHT,
        "spells.icn",
        static_cast<i16>(gsSpellInfo[H2EnumIndex(displayedSpell)].iconIndex),
        ICON_DRAW_NORMAL,
    ...
    sprintf(spellName, "%s", gSpellNames[H2EnumIndex(displayedSpell)]);
    m_winLoseBottomTextWidgets[0] = new textWidget(
    ...
    sprintf(
        ...
        gSpellNames[H2EnumIndex(displayedSpell)]
    );
...
void combatManager::ShowDeadArmies(class heroWindow* window) {
        ...
        for (armyIndex_8 = 0; armyIndex_8 < COMBAT_ARMY_SLOT_COUNT; ++armyIndex_8) {
            if (m_armies[side_9][armyIndex_8].m_monsterType != CREATURE_NONE
                ...
                *(&casualtyType_1[0][0] + side_9 * COMBAT_ARMY_SLOT_COUNT
                  + casualtyQuantity_0[side_9]) = H2EnumIndex(m_armies[side_9][armyIndex_8].m_monsterType);
    ...
    for (side_9 = 0; side_9 < COMBAT_SIDE_COUNT; ++side_9) {
        if (side_9 == H2EnumIndex(COMBAT_ATTACKER_SIDE))
            y_29 = CASUALTY_ATTACKER_Y;
        else
            y_29 = CASUALTY_DEFENDER_Y;
        text_27 = static_cast<char*>(H2_ALLOC(CASUALTY_HEADING_CAPACITY));
        sprintf(text_27, side_9 == H2EnumIndex(COMBAT_ATTACKER_SIDE) ? "Attacker" : "Defender");
...
void combatManager::DoVictory(CombatResult winningSide) {
    ...
    if (m_heroes[H2EnumIndex(COMBAT_DEFENDER_SIDE)] != NULL && m_heroes[H2EnumIndex(COMBAT_DEFENDER_SIDE)]->m_isCaptain != 0)
    ...
    for (combatSide = COMBAT_ATTACKER_SIDE; H2EnumIndex(combatSide) < COMBAT_SIDE_COUNT; ++combatSide) {
        living = 0;
        lastLivingArmy0 = -1;
        for (i = 0; i < gpCombatManager->m_armyCount[H2EnumIndex(combatSide)]; ++i) {
            currentArmy26 = &m_armies[H2EnumIndex(combatSide)][i];
            ...
            if (CombatResultForSide(combatSide) == winningSide && currentArmy26->m_quantity > 0
                && (H2EnumIndex((currentArmy26->m_monster.flags.all) & (MONSTER_FLAGS_LIGHT_PALETTE))) == 0
        ...
        if (living == 0 && lastLivingArmy0 != -1)
            m_armies[H2EnumIndex(combatSide)][lastLivingArmy0].m_quantity = 1;
    }
    ...
    timer = platform::Ticks();
    for (fadeIndex26 = 0; fadeCount > fadeIndex26; ++fadeIndex26) {
        PollSound();
        DelayTil(&timer);
        timer = platform::Ticks() + VICTORY_FADE_DELAY;
        DimBitmapArea(
                ...
                if (gbRetreatWin == 0) {
                    ...
                    if (m_heroes[H2EnumIndex(COMBAT_ATTACKER_SIDE)] != NULL
                        ...
                        for (fadeIndex26 = 0; fadeIndex26 < HERO_ARTIFACT_SLOT_COUNT; ++fadeIndex26) {
                            if (m_heroes[H2EnumIndex(winningSide)]->m_artifacts[fadeIndex26]
                                == ARTIFACT_NONE) {
                                ++emptySlots;
                            }
                        }
                        for (fadeIndex26 = 0; fadeIndex26 < HERO_ARTIFACT_SLOT_COUNT; ++fadeIndex26) {
                            if (m_heroes[H2EnumIndex(OppositeCombatResult(winningSide))]->m_artifacts[fadeIndex26]
...
void combatManager::DoLoseWindow(void) {
    ...
    if (m_heroes[H2EnumIndex(losingSide_h)] != NULL) {
        if (gbCombatSurrender != 0) {
            sprintf(
                gText,
                cBattleResults[H2EnumIndex(RESULT_TEXT_HERO_SURRENDER)],
                m_heroes[H2EnumIndex(losingSide_h)]->m_name
            );
        } else if (gbRetreatWin != 0) {
            sprintf(
                gText,
...
i32 combatManager::DoSurrender(void) {
    ...
    for (armyIndex_n = 0; armyIndex_n < COMBAT_ARMY_SLOT_COUNT; ++armyIndex_n) {
        if (m_armies[H2EnumIndex(m_currentSide)][armyIndex_n].IsAlive()) {
            giSurrenderCost +=
                gMonsterDatabase[H2EnumIndex(m_armies[H2EnumIndex(m_currentSide)][armyIndex_n].m_monsterType)].cost
                * m_armies[H2EnumIndex(m_currentSide)][armyIndex_n].m_quantity;
        }
    }
    ...
    if (m_heroes[H2EnumIndex(OppositeCombatSide(m_currentSide))]->m_isCaptain != 0) {
        ...
        if (m_playerId[H2EnumIndex(OppositeCombatSide(m_currentSide))] == -1)
...
void combatManager::CheckGetAIMove(void) {
    ...
    if (m_heroCastSpell[H2EnumIndex(m_currentSide)] == 0
        && (m_playerId[H2EnumIndex(m_currentSide)] == -1 || gbThisNetHumanPlayer[m_playerId[H2EnumIndex(m_currentSide)]] == 0
...
MessageDispatchResult combatManager::ProcessNextAction(struct tag_message& message) {
    ...
    if (m_smallViewSide[H2EnumIndex(COMBAT_ATTACKER_SIDE)] != COMBAT_SIDE_NONE
        || m_smallViewSide[H2EnumIndex(COMBAT_DEFENDER_SIDE)] != COMBAT_SIDE_NONE) {
        m_smallViewSide[H2EnumIndex(COMBAT_DEFENDER_SIDE)] = COMBAT_SIDE_NONE;
        m_smallViewSide[H2EnumIndex(COMBAT_ATTACKER_SIDE)] = m_smallViewSide[H2EnumIndex(COMBAT_DEFENDER_SIDE)];
        redraw = 1;
    }
    if (giNextAction != ACTION_NONE) {
        LogInt(
...
void combatManager::ResetCyclingCreatures(void) {
    ...
    for (side = COMBAT_ATTACKER_SIDE; H2EnumIndex(side) < COMBAT_SIDE_COUNT; ++side) {
        for (index = 0; index < gpCombatManager->m_armyCount[H2EnumIndex(side)]; ++index) {
            currentArmy_p = &gpCombatManager->m_armies[H2EnumIndex(side)][index];
            if ((H2EnumIndex((currentArmy_p->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                    == 0
                ...
                ++cyclingCount;
                ++gpCombatManager->m_limitCreatureCount[H2EnumIndex(side)][index];
            }
        }
    }
    if (cyclingCount == 0)
    ...
    for (side = COMBAT_ATTACKER_SIDE; H2EnumIndex(side) < COMBAT_SIDE_COUNT; ++side) {
        for (index = 0; index < gpCombatManager->m_armyCount[H2EnumIndex(side)]; ++index) {
            ...
            if ((H2EnumIndex((currentArmy_p->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                ...
                currentArmy_p->m_lastAnimationTime = platform::Ticks();
            }
        }
    }
...
void combatManager::ResetCycleTimers(void) {
    i32l currentTime_r = platform::Ticks();
    CombatSide side;
    i32 index;
    army* currentArmy_p;
...
void combatManager::CycleCombatScreen(void) {
    ...
    for (side = COMBAT_ATTACKER_SIDE; H2EnumIndex(side) < COMBAT_SIDE_COUNT; ++side) {
        if (m_heroOverlayIcons[H2EnumIndex(side)] == NULL) {
        } else {
            if (m_heroes[H2EnumIndex(side)] != NULL)
                m_heroOverlayFrame[H2EnumIndex(side)] = (m_heroOverlayFrame[H2EnumIndex(side)] + 1) % COMBAT_HERO_OVERLAY_FRAME_COUNT;
            ++m_drawHeroOverlay[H2EnumIndex(side)];
        }
    }
    ...
    for (side = COMBAT_ATTACKER_SIDE; H2EnumIndex(side) < COMBAT_SIDE_COUNT; ++side) {
        for (index = 0; index < gpCombatManager->m_armyCount[H2EnumIndex(side)]; ++index) {
            ...
            if ((H2EnumIndex((currentArmy->m_monster.flags.abilityFlags) & (MONSTER_ABILITY_FLAG_AI_EXCLUDED)))
                    == 0
            ...
            if (m_heroAnimationState[H2EnumIndex(side)] == HERO_ANIMATION_DEATH_FIRST
                ...
                if (m_playerId[H2EnumIndex(m_currentSide)] == -1
                    ...
                    if (sCmbtHero[m_heroSpriteIndex[H2EnumIndex(side)]]
                       ...
                       && m_heroCycleTimer[H2EnumIndex(side)] + HERO_IDLE_DELAY < platform::Ticks()) {
                if (sCmbtHero[m_heroSpriteIndex[H2EnumIndex(side)]].idleAnimationCount > 1) {
                    nextHeroAnimation[H2EnumIndex(side)] =
                        Random(
                            0,
                            sCmbtHero[m_heroSpriteIndex[H2EnumIndex(side)]].idleAnimationCount
                                - 1
                        )
                        + HERO_ANIMATION_IDLE_FIRST;
                } else {
                    nextHeroAnimation[H2EnumIndex(side)] = HERO_ANIMATION_IDLE_FIRST;
                }
                m_drawHero[H2EnumIndex(side)] = 1;
            }
        }
    ...
    if (nextHeroAnimation[COMBAT_SIDE_COUNT] == 0 && m_drawHero[H2EnumIndex(COMBAT_ATTACKER_SIDE)] == 0
    ...
    for (side = COMBAT_ATTACKER_SIDE; H2EnumIndex(side) < COMBAT_SIDE_COUNT; ++side) {
        if (m_drawHero[H2EnumIndex(side)] != 0) {
            if (nextHeroAnimation[H2EnumIndex(side)] != -1) {
                m_heroAnimationState[H2EnumIndex(side)] = nextHeroAnimation[H2EnumIndex(side)];
                m_heroAnimationFrame[H2EnumIndex(side)] = 0;
            } else {
                ++m_heroAnimationFrame[H2EnumIndex(side)];
                if (m_heroAnimationFrame[H2EnumIndex(side)]
                    >= sCmbtHero[m_heroSpriteIndex[H2EnumIndex(side)]]
                           .animationFrameCount[m_heroAnimationState[H2EnumIndex(side)]]) {
                    m_heroAnimationState[H2EnumIndex(side)] = HERO_ANIMATION_STAND;
                    m_heroAnimationFrame[H2EnumIndex(side)] = 0;
                    m_heroCycleTimer[H2EnumIndex(side)] = platform::Ticks();
                }
            }
        }
    }
...
setCycleTimer:
    glTimers[GLOBAL_COMBAT_CYCLE_TIMER_SLOT] = static_cast<i32>(
        platform::Ticks() + gfCombatSpeedMod[gConfig.combatSpeed] * COMBAT_CYCLE_TIMER_FACTOR
    );
}
...
void combatManager::AddArmy(
    ...
    for (index_g = 0; index_g < COMBAT_ARMY_CAPACITY; ++index_g) {
        if (m_armies[H2EnumIndex(side)][index_g].m_monsterType == CREATURE_NONE) {
            armyIndex_r = index_g;
            break;
        }
        if (m_armies[H2EnumIndex(side)][index_g].m_quantity == 0
            && (H2EnumIndex((m_armies[H2EnumIndex(side)][index_g].m_monster.flags.all) & (MONSTER_FLAGS_AI_EXCLUDED))) != 0
            && ((H2EnumIndex((m_armies[H2EnumIndex(side)][index_g].m_monster.flags.all) & (MONSTER_FLAGS_MIRROR_IMAGE))) != 0
                || m_armies[H2EnumIndex(side)][index_g].m_monsterType == CREATURE_EARTH_ELEMENTAL
                || m_armies[H2EnumIndex(side)][index_g].m_monsterType == CREATURE_AIR_ELEMENTAL
                || m_armies[H2EnumIndex(side)][index_g].m_monsterType == CREATURE_FIRE_ELEMENTAL
                || m_armies[H2EnumIndex(side)][index_g].m_monsterType == CREATURE_WATER_ELEMENTAL)) {
            armyIndex_r = index_g;
            reusedArmy_m = 1;
            break;
        }
    ...
    newArmy = &m_armies[H2EnumIndex(side)][armyIndex_r];
    ...
    ResetLimitCreature();
    ++m_limitCreatureCount[H2EnumIndex(side)][armyIndex_r];
...
void combatManager::ViewBallista(i32 quickView) {
    ...
    m_combatTowns[H2EnumIndex(COMBAT_DEFENDER_SIDE)]->CalcNumLevelArchers(&archerCount, &attackBonus);
    ...
    strcpy(description, "");
    if ((m_combatTowns[H2EnumIndex(COMBAT_DEFENDER_SIDE)]->m_buildings & H2EnumIndex(TOWN_BUILDING_LEFT_TURRET)) != 0) {
    ...
    if ((m_combatTowns[H2EnumIndex(COMBAT_DEFENDER_SIDE)]->m_buildings & H2EnumIndex(TOWN_BUILDING_RIGHT_TURRET)) != 0) {
        strcpy(description, "");
