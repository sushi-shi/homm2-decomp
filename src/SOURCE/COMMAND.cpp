...
void combatManager::SetCombatDirections(i32 targetHex) {
    ...
    if ((H2EnumIndex((currentArmy->m_monster.flags.all) & (MONSTER_FLAGS_FLYING))) != 0) {
        for (direction = 0; direction < COMBAT_DIRECTION_COUNT; direction++)
    ...
    for (direction = 0; direction < COMBAT_DIRECTION_COUNT; direction++) {
        ...
        if (pathValid[mappedDirection] != 0) {
            if ((H2EnumIndex((targetArmy->m_monster.flags.all) & (MONSTER_FLAGS_WIDE))) != 0) {
                if (direction == H2EnumIndex(COMBAT_DIRECTION_NORTHEAST)
...
void combatManager::CheckSetMouseDirection(i32 mouseX, i32 mouseY, i32 targetHex) {
    ...
    army* targetArmy [[maybe_unused]] =
        &m_armies[H2EnumIndex(currentArmy->m_targetSide)][currentArmy->m_targetIndex];
...
void combatManager::DoCommand(CombatMessageCommand command) {
    ...
    army* currentArmy [[maybe_unused]] = &m_armies[H2EnumIndex(m_currentArmySide)][m_currentArmyIndex];
    switch (command) {
...
void combatManager::ShowSkeletons(class heroWindow* window) {
    ...
    if (giSkeletonsCreated > 1) {
        sprintf(
            gText,
            localization::Tr("combat.necromancy.multiple.buka")
...
,
        ...
        sprintf(
            gText,
            localization::Tr("combat.necromancy.one.buka")
        ...
        case COMBAT_RESULT_DEFENDER:
            if (m_heroes[H2EnumIndex(winningSide)] != NULL) {
                ...
                levelsGained = gpAdvManager->GiveExperience(
                    ...
                    m_experienceValue[H2EnumIndex(OppositeCombatResult(winningSide))],
                    gbThisNetHumanPlayer[H2EnumIndex(m_heroes[H2EnumIndex(winningSide)]->m_owner)] == 0
                );
...
void combatManager::DoLoseWindow(void) {
    ...
    i32 unusedLoseWord [[maybe_unused]] = H2EnumIndex(COMBAT_RESULT_ATTACKER);
    tag_message message;
...
MessageDispatchResult combatManager::ProcessNextAction(struct tag_message& message) {
    ...
    switch (giNextAction) {
        ...
        case ACTION_ATTACK:
            ...
            break;
        case ACTION_RETREAT:
            m_sideRetreated[H2EnumIndex(m_currentSide)] = true;
            gbRetreatWin = true;
            ResetCycleTimers();
            break;
...
void combatManager::CycleCombatScreen(void) {
    ...
    if (m_heroIcons[H2EnumIndex(COMBAT_ATTACKER_SIDE)] != NULL)
        m_drawHero[H2EnumIndex(COMBAT_ATTACKER_SIDE)] = true;
...
void combatManager::AddArmy(
    ...
    for (index = 0; index < COMBAT_ARMY_CAPACITY; ++index) {
        ...
        if (m_armies[H2EnumIndex(side)][index].m_quantity == 0
            ...
            && ((H2EnumIndex((m_armies[H2EnumIndex(side)][index].m_monster.flags.all) & (MONSTER_FLAGS_MIRROR_IMAGE))) != 0
