...
void combatManager::SetCombatDirections(i32 targetHex) {
    ...
    for (direction = 0; direction < COMBAT_DIRECTION_COUNT; direction++) {
        ...
        if ((((currentArmy->m_monster.attributes) & (MONSTER_FLAGS_WIDE))) != 0
            && directionHexes[direction] != INVALID_HEX) {
            if (currentArmy->m_facing == ARMY_FACING_RIGHT) {
                if (direction == (COMBAT_DIRECTION_NORTHWEST)
    ...
    if ((((currentArmy->m_monster.attributes) & (MONSTER_FLAGS_FLYING))) != 0) {
        for (direction = 0; direction < COMBAT_DIRECTION_COUNT; direction++)
    ...
    for (direction = 0; direction < COMBAT_DIRECTION_COUNT; direction++) {
        ...
        if (direction < COMBAT_DIRECTION_ADJACENT_COUNT)
            ...
            mappedDirection = direction == (COMBAT_DIRECTION_WIDE_WEST)
                ? (COMBAT_DIRECTION_WIDE_EAST)
                : (COMBAT_DIRECTION_WIDE_WEST);

        if (pathValid[mappedDirection] != 0) {
            if ((((targetArmy->m_monster.attributes) & (MONSTER_FLAGS_WIDE))) != 0) {
                if (direction == (COMBAT_DIRECTION_NORTHEAST)
                    && HEX_HAS_OCCUPANT(m_hexCells[targetHex - 1], targetSide, targetIndex)) {
                    outputDirection = (COMBAT_DIRECTION_WIDE_WEST);
                } else if (direction == (COMBAT_DIRECTION_NORTHWEST)
                           && HEX_HAS_OCCUPANT(
                    ...
                    outputDirection = (COMBAT_DIRECTION_WIDE_WEST);
                } else if (direction == (COMBAT_DIRECTION_SOUTHEAST)
                           && HEX_HAS_OCCUPANT(
                               m_hexCells[targetHex - 1],
                               targetSide,
                               targetIndex
                           )) {
                    outputDirection = (COMBAT_DIRECTION_WIDE_EAST);
                } else if (direction == (COMBAT_DIRECTION_SOUTHWEST)
                           && HEX_HAS_OCCUPANT(
                               m_hexCells[targetHex + 1],
                               targetSide,
                               targetIndex
            ...
            } else if (direction == (COMBAT_DIRECTION_WIDE_WEST)) {
...
void combatManager::CheckSetMouseDirection(i32 mouseX, i32 mouseY, i32 targetHex) {
    ...
    if (direction == COMBAT_DIRECTION_WIDE_WEST || direction == COMBAT_DIRECTION_WIDE_EAST) {
        ...
        if (currentArmy->m_facing == ARMY_FACING_RIGHT
            && (((currentArmy->m_monster.attributes) & (MONSTER_FLAGS_WIDE))) != 0) {
                ...
                if (IS_WIDGET_SELECTION_NOTIFICATION(message.payload.widget.command)) {
                    ...
                    switch (static_cast<CombatControlId>(message.payload.widget.id)) {
                        ...
                        case CONTROL_DEFER_TURN:
                            helpIndex = (LONG_HELP_DEFER_TURN);
                            break;
                        case CONTROL_SKIP_TURN:
                            helpIndex = (LONG_HELP_SKIP_TURN);
                            break;
...
void combatManager::ResetRound(void) {
    i32 unusedResetRoundWord [[maybe_unused]];
...
CombatMessageCommand combatManager::GetCommand(i32 hexIndex) {
    ...
    i32 rowPosition [[maybe_unused]] = hexIndex / COMBAT_GRID_ROW_LENGTH;
    ...
    switch (hexIndex) {
        ...
        default: {
            ...
            ourArmy = &m_armies[(m_currentArmySide)][m_currentArmyIndex];
...
i32 combatManager::RightClick(i32 hexIndex) {
    ...
    switch (hexIndex) {
        ...
        default:
            ...
            i32 armyIndex [[maybe_unused]] = m_hexCells[hexIndex].m_occupantIndex;
            if (m_hexCells[hexIndex].m_blocked != 0 && !CAN_PASS_CASTLE_GATE(hexIndex)) {
                return 0;
            } else {
...
void combatManager::ShowWinLoseArtifact(
    ...
    i16 bottom [[maybe_unused]] = CASUALTY_WINDOW_BOTTOM;
    tag_message message;
    char* artifactName;
...
void combatManager::ShowEagleEyeSpell(class heroWindow* window) {
    ...
    m_winLoseBottomWidgets[1] = new iconWidget(
        ...
        EAGLE_ICON_HEIGHT,
        "spells.icn",
        gsSpellInfo[(newSpell)].iconIndex,
        ICON_DRAW_NORMAL,
...
void combatManager::ShowDeadArmies(class heroWindow* window) {
    ...
    for (side = 0; side < COMBAT_SIDE_COUNT; ++side) {
        ...
        for (armyIndex = 0; armyIndex < displayedCount; ++armyIndex) {
            ...
            sprintf(text, "%d", casualtyCount[side][armyIndex]);
    ...
    for (combatSide = COMBAT_ATTACKER_SIDE; (combatSide) < COMBAT_SIDE_COUNT; ++combatSide) {
        ...
        for (troopIndex = 0; troopIndex < gpCombatManager->m_armyCount[(combatSide)];
            ...
            if (CombatResultForSide(combatSide) == winningSide && pTroop->m_quantity > 0
                && (((pTroop->m_monster.attributes) & (MONSTER_FLAGS_LIGHT_PALETTE))) == 0
...
i32 combatManager::DoSurrender(void) {
    ...
    sprintf(
        gText,
        "port%04d.icn",
        m_heroes[(OppositeCombatSide(m_currentSide))]->m_portrait
    );
    message.payload.widget.data.text = gText;
...
MessageDispatchResult combatManager::ProcessNextAction(struct tag_message& message) {
    ...
    actingArmy = &m_armies[(m_currentArmySide)][m_currentArmyIndex];
    shouldAdvance = false;
    if (CheckWin(&message) != 0)
        goto Finished;
    switch (giNextAction) {
...
void combatManager::ResetCyclingCreatures(void) {
    ...
    for (sideIndex = COMBAT_ATTACKER_SIDE; (sideIndex) < COMBAT_SIDE_COUNT; ++sideIndex) {
        for (index = 0; index < gpCombatManager->m_armyCount[(sideIndex)]; ++index) {
            ...
            if ((((currentTroop->m_monster.attributes) & (MONSTER_FLAGS_DEAD))) == 0
    ...
    for (sideIndex = COMBAT_ATTACKER_SIDE; (sideIndex) < COMBAT_SIDE_COUNT; ++sideIndex) {
        for (index = 0; index < gpCombatManager->m_armyCount[(sideIndex)]; ++index) {
            currentTroop = &gpCombatManager->m_armies[(sideIndex)][index];
            if ((((currentTroop->m_monster.attributes) & (MONSTER_FLAGS_DEAD)))
                == 0) {
                currentTroop = &gpCombatManager->m_armies[(sideIndex)][index];
...
void combatManager::ResetCycleTimers(void) {
    ...
    for (which = COMBAT_ATTACKER_SIDE; (which) < COMBAT_SIDE_COUNT; ++which) {
        for (i = 0; i < gpCombatManager->m_armyCount[(which)]; ++i) {
            armyPointer = &gpCombatManager->m_armies[(which)][i];
...
void combatManager::CycleCombatScreen(void) {
    ...
    for (side = COMBAT_ATTACKER_SIDE; (side) < COMBAT_SIDE_COUNT; ++side) {
        for (index = 0; index < gpCombatManager->m_armyCount[(side)]; ++index) {
            ...
            if ((((currentArmy->m_monster.attributes) & (MONSTER_FLAGS_DEAD))) == 0
        ...
        if (m_heroAnimationState[index] == HERO_ANIMATION_DEATH_FIRST
            ...
            if (m_playerId[(m_currentSide)] == -1
                ...
                if (sCmbtHero[m_heroSpriteIndex[index]]
                   ...
                   && m_heroCycleTimer[index] + HERO_IDLE_DELAY < KBTickCount()) {
    ...
    if (cyclingCount == 0 && m_drawHero[(COMBAT_ATTACKER_SIDE)] == 0
        ...
        if (m_drawHero[index] != 0) {
            if (nextHeroAnimation[index] != -1) {
                ...
                if (m_heroAnimationFrame[index]
                    ...
                    m_heroAnimationFrame[index] = 0;
                    m_heroCycleTimer[index] = KBTickCount();
                }
            }
        }
    }
...
void combatManager::AddArmy(
    ...
    for (index = 0; index < COMBAT_ARMY_SLOT_COUNT; ++index) {
        ...
        if (m_armies[(side)][index].m_quantity == 0
            && (((m_armies[(side)][index].m_monster.attributes) & (MONSTER_FLAGS_DEAD))) != 0
            && ((((m_armies[(side)][index].m_monster.attributes) & (MONSTER_FLAGS_MIRROR_IMAGE))) != 0
    ...
    newStack = &m_armies[(side)][armyIndex];
