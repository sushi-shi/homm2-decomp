...
#define currentArmy currentArmy_1
#define direction direction_28
#define mappedDirection mappedDirection_5
#define next next_28
#define outputDirection outputDirection_7
#define pathValid pathValid_28
#define previous previous_1
#define rearHexes rearHexes_2
#define standable standable_0
#define targetArmy targetArmy_13
#define targetIndex targetIndex_9
#define targetSide targetSide_28
#define unresolved unresolved_1
#endif
VA(0x0042bc31, 0x758)
void combatManager::SetCombatDirections(i32 targetHex) {
    ...
    army* targetArmy = &m_armies[IDX(targetSide)][targetIndex];
        ...
        if (direction == IDX(COMBAT_DIRECTION_WIDE_WEST)
            ...
            if (HAS(currentArmy->m_monster.attributes, MONSTER_FLAGS_WIDE) != 0) {
                if (currentArmy->m_facing == ARMY_FACING_RIGHT) {
                    ...
                    if (direction == IDX(COMBAT_DIRECTION_WIDE_EAST))
                        directionHexes[direction] =
        ...
        if (HAS(currentArmy->m_monster.attributes, MONSTER_FLAGS_WIDE) != 0
            ...
            if (currentArmy->m_facing == ARMY_FACING_RIGHT) {
                ...
                if (direction == IDX(COMBAT_DIRECTION_NORTHEAST)
                    || direction == IDX(COMBAT_DIRECTION_EAST)
    ...
    if (m_validDirectionCount == 0)
        pathValid[IDX(COMBAT_DIRECTION_WIDE_WEST)] = true;
    ...
    for (direction = 0; direction < COMBAT_DIRECTION_COUNT; direction++) {
        ...
        if (direction < COMBAT_DIRECTION_ADJACENT_COUNT)
            ...
            mappedDirection = direction == IDX(COMBAT_DIRECTION_WIDE_WEST)
        ...
        if (pathValid[mappedDirection] != 0) {
            if (HAS(targetArmy->m_monster.attributes, MONSTER_FLAGS_WIDE) != 0) {
                if (direction == IDX(COMBAT_DIRECTION_NORTHEAST)
                ...
                } else if (direction == IDX(COMBAT_DIRECTION_NORTHWEST)
                           && HEX_HAS_OCCUPANT(
                    ...
                    outputDirection = IDX(COMBAT_DIRECTION_WIDE_WEST);
                } else if (direction == IDX(COMBAT_DIRECTION_SOUTHEAST)
                           && HEX_HAS_OCCUPANT(
                               m_hexCells[targetHex - 1],
                               targetSide,
                               targetIndex
                           )) {
                    outputDirection = IDX(COMBAT_DIRECTION_WIDE_EAST);
                } else if (direction == IDX(COMBAT_DIRECTION_SOUTHWEST)
                           && HEX_HAS_OCCUPANT(
                               m_hexCells[targetHex + 1],
                               targetSide,
                               targetIndex
                ...
                if (m_directionMap[next] >= 0
                    ...
                    m_directionMap[direction] =
                        m_directionMap[next] + DIRECTION_PENDING_OFFSET;
                           ...
                           && m_directionMap[previous] <= IDX(COMBAT_DIRECTION_WIDE_EAST)) {
                    m_directionMap[direction] =
...
#undef currentArmy
#undef direction
#undef mappedDirection
#undef next
#undef outputDirection
#undef pathValid
#undef previous
#undef rearHexes
#undef standable
...
#if H2_RETAIL_COMPILER
#define direction direction_5
#define directionCopy directionCopy_1
#define ratio ratio_3
#define rearHex rearHex_5
#define relativeX relativeX_7
#define relativeY relativeY_9
#define sector sector_6
#define targetArmy targetArmy_13
#endif
VA(0x0042c389, 0x575)
...
#if H2_RETAIL_COMPILER
#define currentMouseX currentMouseX_18
#define currentMouseY currentMouseY_18
#define pendingMessage pendingMessage_2
#define selectedHex selectedHex_0
#endif
VA(0x0042c958, 0x7f5)
...
#if H2_RETAIL_COMPILER
#define unusedResetRoundWord unusedResetRoundWord6
#endif
VA(0x0042d1a5, 0x1c4)
...
CombatMessageCommand combatManager::GetCommand(i32 hexIndex) {
    ...
    switch (hexIndex) {
        ...
        default: {
            ...
            ourArmy = &m_armies[IDX(m_currentArmySide)][m_currentArmyIndex];
...
#define unusedValue2 unusedCommandWord5
#endif
VA(0x0042db4f, 0x32e)
...
#if H2_RETAIL_COMPILER
#define iDelay iDelay_3
#define iconFile iconFile_3
#endif
VA(0x0042de7d, 0x4f3)
...
void combatManager::ShowWinLoseArtifact(
    ...
    i16 H2_UNUSED(bottom) = CASUALTY_WINDOW_BOTTOM;
    tag_message message;
    char* artifactName;
...
#if H2_RETAIL_COMPILER
#define bottom bottom_4
#define casualtyQuantity casualtyQuantity_13
#define casualtyType casualtyType_1
#define casualtyY y_7
#define side side_4
#define spacing spacing_6
#define startX startX_0
#define text text_1
#define unusedValue1 unusedCasualtyWord116
#define unusedValue2 unusedCasualtyWord92
#define unusedValue3 unusedCasualtyWord86
#define unusedValue4 unusedCasualtyWord52
#define unusedValue5 unusedCasualtyWord46
#define unusedValue6 unusedCasualtyWord13
#define unusedValue7 unusedCasualtyWord0
#endif
VA(0x0042ec2c, 0x9d5)
void combatManager::ShowDeadArmies(class heroWindow* window) {
        ...
        for (armyIndex = 0; armyIndex < COMBAT_ARMY_SLOT_COUNT; ++armyIndex) {
            if (m_armies[side][armyIndex].m_monsterType != CREATURE_NONE
                ...
                casualtyType[side][casualtyQuantity[side]] =
                    IDX(m_armies[side][armyIndex].m_monsterType);
    ...
    sprintf(text, localization::Tr("combat.casualties.title"));
    ...
    for (side = 0; side < COMBAT_SIDE_COUNT; ++side) {
        ...
        if (casualtyQuantity[side] <= 0) {
            ...
            sprintf(text, localization::Tr("common.none"));
...
#if H2_RETAIL_COMPILER
#define actingArmy actingArmy_29
#define dispatchResult dispatchResult_1
#define redraw redraw_10
#endif
VA(0x00430cbe, 0x60b)
...
#if H2_RETAIL_COMPILER
#define animationIndex animationIndex_27
#define currentArmy currentArmy_2
#define cyclingCount cyclingCount_1
#define index index_0
#define nextHeroAnimation nextHeroAnimation_0
#define roll roll_3
#define side side_7
#endif
VA(0x004315fa, 0x930)
void combatManager::CycleCombatScreen(void) {
    ...
    for (side = COMBAT_ATTACKER_SIDE; IDX(side) < COMBAT_SIDE_COUNT; ++side) {
        for (index = 0; index < gpCombatManager->m_armyCount[IDX(side)]; ++index) {
            ...
            if (HAS(currentArmy->m_monster.attributes, MONSTER_FLAGS_DEAD) == 0
...
#if H2_RETAIL_COMPILER
#undef animationIndex
#undef currentArmy
#undef cyclingCount
#undef index
#undef nextHeroAnimation
#undef roll
#undef side
#endif
...
#define unusedValue2 unusedSmallViewWord2
#endif
VA(0x00432338, 0x13d)
