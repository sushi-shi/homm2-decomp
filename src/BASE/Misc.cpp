...
void ReadPrefsFromRegistry(void) {
    ...
    if (lRet == 0) {
        ...
        RegQueryValueExA(
            ...
            NULL,
            &dwType,
            reinterpret_cast<u8*>(&gConfig.walkSpeeds[H2EnumIndex(CONFIG_WALK_SPEED_HUMAN)]),
            &dwcbData
        );
        RegQueryValueExA(
            hKey,
            ...
            NULL,
            &dwType,
            reinterpret_cast<u8*>(&gConfig.walkSpeeds[H2EnumIndex(CONFIG_WALK_SPEED_COMPUTER)]),
            &dwcbData
        );
        RegQueryValueExA(
            hKey,
...
void FadeToColorTable(u8* colorTable, i32 increment) {
    ...
    for (i = 0; i < H2EnumIndex(PALETTE_DATA_SIZE) / H2EnumIndex(PALETTE_CHANNEL_COUNT); ++i) {
