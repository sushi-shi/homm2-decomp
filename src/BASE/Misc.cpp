...
void SetPalette(i8* paletteData, i32 updateDisplay) {
    ...
    memcpy(
        gCyclePal,
        paletteData + (CYCLE_RANGE_ONE_FIRST) * (PALETTE_CHANNEL_COUNT),
        sizeof(gCyclePal)
    );
    if (updateDisplay != 0)
        UpdatePalette(gpBufferPalette->m_data);
...
void FadeToColorTable(u8* colorTable, i32 increment) {
    ...
    for (i = 0; i < (PALETTE_DATA_SIZE) / (PALETTE_CHANNEL_COUNT); ++i) {
        tempPal[i * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_RED)] =
            paletteData[colorTable[i] * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_RED)];
        tempPal[i * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_GREEN)] =
            paletteData[colorTable[i] * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_GREEN)];
        tempPal[i * (PALETTE_CHANNEL_COUNT) + (PALETTE_CHANNEL_BLUE)] =
