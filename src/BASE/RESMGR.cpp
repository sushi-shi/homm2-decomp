...
i32 resourceManager::LoadAggregateHeader(char* aggregateName) {
    ...
    aggregateFile = platform::FileOpen(aggregateName, platform::FileMode::Read);
    ...
    platform::FileRead(m_aggregateFd[m_curAggregate], fileCountBuffer, sizeof(i16));
...
void resourceManager::PointToFile(u32l fileId) {
    ...
    i32l ignoredPosition =
        platform::FileSeek(m_aggregateFd[m_curAggregate], m_aggregateDir[m_curAggregate][entryIndex].offset);
}
...
i8 resourceManager::ReadByte(void) {
    ...
    i8 value = 0;
    i32 bytesRead = platform::FileRead(m_aggregateFd[m_curAggregate], &value, sizeof(value));
    if (bytesRead == 0) {
        i32 errorCode = errno;
        i32 debugTrap = 0;
        debugTrap++;
