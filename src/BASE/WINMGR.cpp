...
void CycleColors(i32 forceUpdate) {
    i8 savedColor[(PALETTE_CHANNEL_COUNT)];
    iCycle1Count++;
    ...
    if (giCycleType == WINDOW_COLOR_CYCLE_WORLD_VIEW) {
        ...
        for (i32 colorIndex = 0; colorIndex < WORLD_CYCLE_COLOR_COUNT; colorIndex++)
            memcpy(
                gCyclePal + colorIndex * (PALETTE_CHANNEL_COUNT),
        ...
        memcpy(gCyclePal, savedColor, (PALETTE_CHANNEL_COUNT));
    ...
    cycleType:
        if (giCycleType == WINDOW_COLOR_CYCLE_DEFAULT) {
            memcpy(savedColor, gCyclePal + DEFAULT_CYCLE_SAVE_OFFSET, (PALETTE_CHANNEL_COUNT));
            memmove(
                gCyclePal + DEFAULT_CYCLE_SAVE_OFFSET,
                gCyclePal + DEFAULT_CYCLE_SOURCE_OFFSET,
                (PALETTE_CHANNEL_COUNT)
            );
            memcpy(gCyclePal + DEFAULT_CYCLE_SOURCE_OFFSET, savedColor, (PALETTE_CHANNEL_COUNT));
