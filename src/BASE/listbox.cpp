...
void listBoxWidget::Read(void) {
    ...
    iconEntry = &m_icon->Entries()[H2EnumIndex(FRAME_SCROLL_THUMB)];
    m_scrollThumbWidth = iconEntry->w;
    frameHeight[0] = iconEntry->h;
    m_scrollThumbHeight = frameHeight[0];
    firstRowHeight = m_icon->Entries()[H2EnumIndex(FRAME_FIRST_ROW)].h;
    m_firstRowHeight = firstRowHeight;
    listX = m_x;
    rowHeight = m_icon->Entries()[H2EnumIndex(FRAME_MIDDLE_ROW)].h;
    listY = m_y;
    m_rowHeight = rowHeight;
    lastRowHeight = m_icon->Entries()[H2EnumIndex(FRAME_LAST_ROW)].h;
    m_lastRowHeight = lastRowHeight;
    m_listX = listX;
    m_listY = listY;
    m_listWidth = m_icon->Entries()[H2EnumIndex(FRAME_FIRST_ROW)].w;
    m_listHeight = (m_maxVisibleItems - LIST_EDGE_ROW_COUNT) * m_rowHeight + m_firstRowHeight
                   + m_lastRowHeight;
    iconEntry = &m_icon->Entries()[H2EnumIndex(FRAME_SCROLL_UP)];
    m_scrollUpWidth = iconEntry->w;
    m_scrollUpHeight = iconEntry->h;
    iconEntry = &m_icon->Entries()[H2EnumIndex(FRAME_SCROLL_DOWN)];
