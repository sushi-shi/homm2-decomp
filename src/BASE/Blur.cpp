...
void DoBlur(
    ...
    for (i = 0; i < PALETTE_COLOR_COUNT; i++) {
        ...
        newPalette[i * H2EnumIndex(PALETTE_CHANNEL_COUNT) + 1] =
            oldPalette[i * H2EnumIndex(PALETTE_CHANNEL_COUNT) + 1] + greenAdjust;
        newPalette[i * H2EnumIndex(PALETTE_CHANNEL_COUNT) + 2] =
            oldPalette[i * H2EnumIndex(PALETTE_CHANNEL_COUNT) + 2] + blueAdjust;
        ...
        if (newPalette[i * H2EnumIndex(PALETTE_CHANNEL_COUNT) + 1] > PALETTE_CHANNEL_MAX)
            newPalette[i * H2EnumIndex(PALETTE_CHANNEL_COUNT) + 1] = PALETTE_CHANNEL_MAX;
        if (newPalette[i * H2EnumIndex(PALETTE_CHANNEL_COUNT) + 1] < 0)
            newPalette[i * H2EnumIndex(PALETTE_CHANNEL_COUNT) + 1] = 0;
        if (newPalette[i * H2EnumIndex(PALETTE_CHANNEL_COUNT) + 2] > PALETTE_CHANNEL_MAX)
            newPalette[i * H2EnumIndex(PALETTE_CHANNEL_COUNT) + 2] = PALETTE_CHANNEL_MAX;
