...
void DoBlur(class bitmap *param_1, class bitmap *param_2, int param_3, int param_4, int param_5, int param_6)
{
    ...
    memcpy(work->field_0x16, param_2->field_0x16, nbytes);
    ...
    memcpy(param_1->field_0x16, param_2->field_0x16, nbytes);
    PollSound();
    for (y = 4; y < param_3 - 4; y++) {
        if ((y & 0x3f) == 0x3f)
            PollSound();
        unsigned char *iVar25 = param_1->field_0x16 + 4 + y * 0x280;
        char *dstRow = reinterpret_cast<char *>(param_2->field_0x16) + 4 + y * 0x280;
        for (x = 0x278; x != 0; x--) {
            bVar1 = iVar25[0x280];
            bVar2 = iVar25[-0xa00];
            bVar3 = iVar25[0x500];
