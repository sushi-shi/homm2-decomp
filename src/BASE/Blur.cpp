...
typedef enum BlurConstant {
    ...
    PALETTE_BYTE_COUNT        = PALETTE_COLOR_COUNT * (PALETTE_CHANNEL_COUNT),
...
void DoBlur(
    ...
    for (i = 0; i < PALETTE_COLOR_COUNT; i++) {
        newPalette[i * (PALETTE_CHANNEL_COUNT)] =
            oldPalette0[i * (PALETTE_CHANNEL_COUNT)] + redAdjust;
        newPalette[i * (PALETTE_CHANNEL_COUNT) + 1] =
            oldPalette0[i * (PALETTE_CHANNEL_COUNT) + 1] + greenAdjust;
        newPalette[i * (PALETTE_CHANNEL_COUNT) + 2] =
            oldPalette0[i * (PALETTE_CHANNEL_COUNT) + 2] + blueAdjust;
        if (newPalette[i * (PALETTE_CHANNEL_COUNT)] > PALETTE_COMPONENT_MAXIMUM)
            newPalette[i * (PALETTE_CHANNEL_COUNT)] = PALETTE_COMPONENT_MAXIMUM;
        if (newPalette[i * (PALETTE_CHANNEL_COUNT)] < 0)
            newPalette[i * (PALETTE_CHANNEL_COUNT)] = 0;
        if (newPalette[i * (PALETTE_CHANNEL_COUNT) + 1] > PALETTE_COMPONENT_MAXIMUM)
            newPalette[i * (PALETTE_CHANNEL_COUNT) + 1] = PALETTE_COMPONENT_MAXIMUM;
        if (newPalette[i * (PALETTE_CHANNEL_COUNT) + 1] < 0)
            newPalette[i * (PALETTE_CHANNEL_COUNT) + 1] = 0;
        if (newPalette[i * (PALETTE_CHANNEL_COUNT) + 2] > PALETTE_COMPONENT_MAXIMUM)
            newPalette[i * (PALETTE_CHANNEL_COUNT) + 2] = PALETTE_COMPONENT_MAXIMUM;
