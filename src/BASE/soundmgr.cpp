...
void soundManager::CDPlay(i32 track, i32 resume, i32 volume, i32 restart) {
    ...
    if (m_fadeSteps > 0) {
        ...
        CDSetVolume((CONFIG_VOLUME_MAX), 0);
    } else {
...
i32 soundManager::ConvertVolume(i32 volume, SoundVolumeConversionMode soundType) {
    ...
    if (soundType == SOUND_VOLUME_MUSIC) {
        if (gConfig.musicVolume >= CONFIG_VOLUME_MIN
            ...
            local_8 =
                ((FADE_TOTAL_STEPS - (gConfig.musicVolume)) * volume)
                / (CONFIG_VOLUME_MAX);
            if (local_8 < 1)
                local_8 = 1;
        }
        ...
        local_8 =
            ((FADE_TOTAL_STEPS - (gConfig.soundVolume)) * volume) / (CONFIG_VOLUME_MAX);
        if (local_8 < 1)
            local_8 = 1;
    }
    if (local_8 < 0)
...
void soundManager::PollSound(void) {
    ...
    if (m_fadeSteps > 0) {
        ...
        if (gConfig.musicSource != CONFIG_MUSIC_SOURCE_MIDI) {
            volume = ((CONFIG_VOLUME_MAX) + 1 - (gConfig.musicVolume)) * volume
