...
int main() {
    ...
    save::XmlFile saved;
    assert(saved.Save("GAMES/state.GX1") == tinyxml2::XML_SUCCESS);
    ...
    script::Shutdown();
    save::XmlFile empty;
    assert(empty.Save("GAMES/state.GX1") == tinyxml2::XML_SUCCESS);
    ...
    old.ReadRoot(old.tempDoc->RootElement());
    ...
    invalid.ReadRoot(invalid.tempDoc->RootElement());
    assert(!gpGame->IsHeroChaseForced(2, 3));
    script::Shutdown();
    mapExtra = nullptr;
    delete gpGame;
