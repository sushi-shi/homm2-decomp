...
RVA_BASE = 0x400000
MARKER_PATTERN = re.compile(r"@(?:early-stop|match-note|semantic)")
    ...
    for path in sorted((ROOT / "src/SOURCE").glob("*.cpp")):
        ...
        for match in va_pattern.finditer(text):
            ...
            if marker_index is not None:
                ...
                marker = marker_kind(marker_line)
                evidence_lines = []
                ...
                evidence = re.sub(r"@(?:early-stop|match-note|semantic)", "", evidence)
...
def category(record):
    ...
    if marker == "early-stop":
        return "accepted"
    if marker in ("match-note", "semantic"):
        return "provisional"
    return "unmarked"
