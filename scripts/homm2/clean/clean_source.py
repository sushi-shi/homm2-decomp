...
def publish(
    ...
    if exists:
        ...
        if "Generated by scripts/clean_source.py." not in message:
            ...
            # Generated branches may carry a later README-only metadata commit.
            # Absorb it on regeneration, but never adopt hand-edited source or
            # build content.
...
def verify(out_root: Path) -> int:
    ...
    if len(outputs) != 1:
        ...
        return 1
    executable = Path(outputs[0]) / "HEROES2W.EXE"
