...
#
# Each src/<tier>/<unit>.cpp gets `#include <va.h>` (so VA()/DATA()/OVERRIDE expand
...
# breaks the MSVC compile. VA()/DATA() lines stay as live annotations + reference.
...
for cpp in sorted((REPO / "src").rglob("*.cpp")):
    ...
    have_inc = any("va.h" in l and "include" in l for l in lines)
    out, inserted = [], have_inc
    for l in lines:
        s = l.rstrip(); st = s.lstrip()
        if not inserted and st and not st.startswith("//"):
            out.append("#include <va.h>"); inserted = True
