...
Metadata and steering rules reproduce their production macro expansion. Enum
rules select the strict typed branch, and documented native-only patches remain
explicit. `--verify` checks the result with the native compatibility headers.

Usage:
    python3 scripts/clean_source.py --out build/clean
    python3 scripts/clean_source.py --out build/clean --verify \\
        --compat-include ../homm2-decomp-port/include/COMPAT
"""
...
# Rules
#
# Each rule maps a macro invocation to either its production expansion or the
# strict typed form documented below. Keep these in step with include/va.h and
# include/Ints.h.
# --------------------------------------------------------------------------
...
WORD_RULES = {
    ...
    # Preserve explicit calling-convention contracts. On 32-bit Win32 these
    # control stack cleanup, argument passing, and function-pointer types:
    # DirectDrawCreate and NetBIOS callbacks are __stdcall, while the naked asm
    # blitters and variadic entry points rely on __cdecl. Ints.h maps these
    # portable names to the compiler's target-specific spelling.
    "__cdecl": "H2_CDECL",
    "__stdcall": "H2_STDCALL",
    "__fastcall": "H2_FASTCALL",
    "__pascal": "H2_PASCAL",
...
PORT_PATCHES = {
    ...
    "src/SOURCE/HERO.cpp": [(
        # The matching spelling materializes the i32 subtraction required by
        # MSVC 4.2. Restore the typed flag-domain operation only in clean output.
        r"m_eventFlags = static_cast<i32>\(m_eventFlags\) - (HERO_EVENT_[A-Z0-9_]+);",
        r"m_eventFlags = m_eventFlags - \1;", True,
    )],
...
# Tokens the clean tree keeps on purpose: the allocation wrappers survive in
# their argument-free form, `__declspec` still carries `naked`, and the
# generated ABI macros expand to compiler calling-convention keywords.
INTENTIONALLY_KEPT = {
    "H2_ALLOC",
    "H2_FREE",
    "H2_ASSERT",
...
EXPECTED_NATIVE_FAILURES = {"src/BASE/BITS.cpp", "src/BASE/TILE.cpp"}
...
def verify(out_root: Path, compat_include: str) -> int:
    ...
    unexpected = [
        ...
        if path.as_posix() not in EXPECTED_NATIVE_FAILURES
    ]
