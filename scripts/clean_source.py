...
made is selecting the strict typed enum branch, which is why STRICT_ENUM_PATCHES
...
Usage:
    ...
    python3 scripts/clean_source.py --out build/clean --verify \\
        --compat-include path/to/win32-compat/include
"""
...
# injected text (CLEAN_ALLOC_MACROS, RETAIL_FILE) and finally STRICT_ENUM_PATCHES.
...
# `__declspec(naked)` blitters of `__asm` blocks. Excluded because --verify runs
# GCC, which has no such syntax; an MSVC build has all 95.
MSVC_ONLY_UNITS = {"src/BASE/BITS.cpp", "src/BASE/TILE.cpp"}
...
# was measured at 68 of 95 translation units against 93 for STRICT_ENUM_PATCHES,
...
def apply_patches(relative: str, text: str) -> str:
    for kind, sites, pattern, replacement in STRICT_ENUM_PATCHES.get(relative, []):
        ...
        if count != sites:
            raise SystemExit(
                "strict-enum patch hit %d site(s) in %s, expected %d: %r\n"
...
def generate(out_root: Path) -> tuple[int, int, list[str]]:
    ...
    for key in STRICT_ENUM_PATCHES:
        if key not in patched_files:
            raise SystemExit("strict-enum patch names a file that was not generated: %s" % key)
...
def publish(out_root: Path, branch: str) -> int:
    ...
    try:
        ...
        unchanged = not subprocess.run(
            ("git", "diff", "--cached", "--quiet", "--", "include", "src"),
            cwd=worktree,
            capture_output=True,
            check=False,
        ).returncode
...
def verify(out_root: Path, compat_include: str) -> int:
    """Syntax-check every generated translation unit.

    Needs a compat layer supplying <windows.h> and friends -- the game speaks
    Win32 and the generator does not change that. The two hand-written MSVC
    assembly TUs are reported but accepted; every other failure is fatal.

    This is a syntax check, not a build: it proves the emitted tree is valid
    C++, using whichever compiler CXX names.
    """
    import subprocess
    ...
    for source in sources:
        # A failing compile is the thing being measured, not an error to raise on.
    ...
    for path, message in failures[:MAX_REPORTED]:
        print("          %s\n            %s" % (path, message.strip()[:120]), file=sys.stderr)
    unexpected = [
        path for path, _ in failures
        if path.as_posix() not in MSVC_ONLY_UNITS
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.verify and not args.compat_include:
        parser.error("--verify requires --compat-include")
